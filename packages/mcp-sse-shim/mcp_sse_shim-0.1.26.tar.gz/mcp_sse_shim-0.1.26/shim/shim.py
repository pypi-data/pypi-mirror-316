...
ssl_context = False if not use_ssl else True
...
async def connect_sse_backend():
    ...
    try:
        # Create session with explicit SSL context and force IPv4
        connector = aiohttp.TCPConnector(
            ssl=ssl_context, 
            force_close=True,
            family=socket.AF_INET  # Force IPv4
        )
        debug(f"SSE: Creating connector with ssl={ssl_context}, family=AF_INET")
        ...
        async with aiohttp.ClientSession(connector=connector, trust_env=False) as session:
            while retry_count < max_retries:
                try:
                    ...
                    debug(f"Connection failed: {e}")
                    debug(f"Check network settings for {BACKEND_URL_SSE}")
                    retry_count += 1
                    ...
                    debug(f"SSL Error: {ssl_err}")
                    debug("Ensure SSL certificate and URL scheme match.")
                    raise
        ...
        debug(f"Full connection details: MCP_HOST={MCP_HOST}, ssl={ssl_context}")
        raise
...
async def run_bridge():
    ...
    try:
        ...
        # Use same SSL settings for message processing session
