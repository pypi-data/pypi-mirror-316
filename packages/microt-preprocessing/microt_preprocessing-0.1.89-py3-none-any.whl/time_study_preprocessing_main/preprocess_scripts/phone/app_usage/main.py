...
app_categorization_dict_path = "/work/mhealthresearchgroup/TIME_STD/location_label_aggregate/top_337_used_apps.csv"
...
def pre_process(microT_root_path, intermediate_file_save_path, p_id, decrypt_password, date):
    ...
    if path.exists(area_folder_path):
        ...
        if path.exists(date_folder_path):
            ...
            app_foreground_file_name = device + "_" + "app_foreground" + "_clean_" + date + ".csv"
            app_foreground_minute_file_name = device + "_" + "app_foreground" + "_minute_" + date + ".csv"
            app_foreground_hour_file_name = device + "_" + "app_foreground" + "_hour_" + date + ".csv"
            app_foreground_day_file_name = device + "_" + "app_foreground" + "_day_" + date + ".csv"

            day_file_save_path = file_save_date_path + sep + day_file_name
            app_foreground_file_save_path = file_save_date_path + sep + app_foreground_file_name
            app_foreground_minute_file_save_path = file_save_date_path + sep + app_foreground_minute_file_name
            app_foreground_hour_file_save_path = file_save_date_path + sep + app_foreground_hour_file_name
            app_foreground_day_file_save_path = file_save_date_path + sep + app_foreground_day_file_name
            ...
            if True:
                ...
                if len(df_participant) != 0:
                    ...
                    df_participant.reset_index(inplace=True, drop=True)
                    # step 3:  write out day file for participant
                    df_participant.to_csv(day_file_save_path, index=False)

                ############### app foreground start and end time ###############
                yesterday_date = get_previous_date(date)
                yesterday_file_name = device + "_" + file_shortname + "_clean_" + yesterday_date + ".csv"
                df_participant_yesterday_path = os.path.join(intermediate_file_save_path, "intermediate_file", p_id, yesterday_date, yesterday_file_name)
                df_participant_currentday_path = day_file_save_path
                df_foreground = get_foreground_matrix(df_participant_currentday_path, df_participant_yesterday_path, date)
                df_foreground.to_csv(app_foreground_file_save_path, index=False)
                # print(app_foreground_file_save_path)
                #
                ############### app foreground minute level ###############
                df_foreground_minute = get_foreground_matrix_minute(df_foreground, app_categorization_dict_path, date)
                df_foreground_minute.to_csv(app_foreground_minute_file_save_path, index=False)
                #
                #
                ############### app foreground hour level ###############
                df_foreground_hour = get_foreground_matrix_hour(df_foreground_minute)
                df_foreground_hour.to_csv(app_foreground_hour_file_save_path, index=False)
                #
                #
                # ############### app foreground day level ###############
                df_foreground_day = get_foreground_matrix_day(df_foreground_hour)
                df_foreground_day.to_csv(app_foreground_day_file_save_path, index=False)
