...
class RNNAttention2D(nn.Module):
	""" forward()方法有inputs和sequence_lengths两个参数, 不能直接作为模型用moddel-wrapper训练，
	否则会把y作为sequence_lengths参数传入
	...
	def __init__(self, embed_dim: int, in_features: int, out_features: int, hidden_size: int = 64, 
			  num_heads: int = 1, num_layers: int = 2, rnn=nn.GRU, bidirectional=True, layer_norm=False, 
			  residual=False, dropout: float = 0.1):
		super().__init__()
		self.head = nn.Linear(in_features, hidden_size)
		self.unflatten = nn.Unflatten(1, (-1, embed_dim))
		self.att = RNNAttention(embed_dim, out_features, hidden_size, num_layers, num_heads,rnn, bidirectional, 
	...
	def forward(self, inputs: torch.Tensor):
		...
		:return: [(batch_size, out_features)]
		"""
		output = self.head(inputs)  # [(batch_size, hidden_size)]
		output = self.unflatten(output)  # [(batch_size, hidden_size / embed_dim, embed_dim)]]
...
class CNNRNNAttention(nn.Module):
	...
	Examples
	--------
	>>> classifier = CNNRNNAttention(embed_dim, out_features=2)
	"""
...
class CNNRNNAttention2D(nn.Module):
	"""
	Examples
	--------
	>>> classifier = CNNRNNAttention2D(embed_dim, out_features=2)
	"""
	
	def __init__(self, embed_dim: int, in_features: int, out_features: int, seq_length: int = 16, cnn_channels: int = 64, 
			  	 kernel_sizes=(2, 3, 4), activation=None, hidden_size: int = 64, num_layers: int = 2, 
				 num_heads: int = 1, rnn=nn.GRU, bidirectional=True, layer_norm=False, batch_norm=False,
	             residual=False, dropout: float = 0.0, bias=False):
		super().__init__()
		self.head = nn.Linear(in_features, hidden_size)
		self.unflatten = nn.Unflatten(1, (-1, embed_dim))
		self.att = CNNRNNAttention(embed_dim, out_features, seq_length, cnn_channels, kernel_sizes, activation, hidden_size, 
...
class RNNCNNAttention2D(nn.Module):
	"""
	Examples
	--------
	>>> classifier = RNNCNNAttention2D(embed_dim, out_features=2)
	"""
	...
	def __init__(self, embed_dim: int, in_features: int, out_features: int, seq_length: int = 16, cnn_channels: int = 64,
		...
		self.head = nn.Linear(in_features, hidden_size)
		self.unflatten = nn.Unflatten(1, (-1, embed_dim))
		self.att = RNNCNNAttention(embed_dim, out_features, seq_length, cnn_channels, kernel_sizes, activation, hidden_size, 
...
class ResRNNCNNAttention2D(nn.Module):
	"""
	Examples
	--------
	>>> classifier = ResRNNCNNAttention2D(embed_dim, out_features=2)
	"""
	
	def __init__(self, embed_dim: int, in_features: int, out_features: int, eq_length: int = 16, cnn_channels: int = 64, 
			  	 kernel_sizes=(2, 3, 4), activation=None, hidden_size: int = 64, num_layers: int = 2, 
				 num_heads: int = 1, rnn=nn.GRU, bidirectional=True, layer_norm=False, batch_norm=False,
	             dropout: float = 0.0, bias=False):
		super().__init__()
		self.head = nn.Linear(in_features, hidden_size)
		self.unflatten = nn.Unflatten(1, (-1, embed_dim))
		self.att = ResRNNCNNAttention(embed_dim, out_features, eq_length, cnn_channels, kernel_sizes, activation, hidden_size, 
								  num_layers, num_heads, rnn, bidirectional, layer_norm, batch_norm, dropout, bias)
...
class TransformerEncoder2D(nn.Module):
	def __init__(self, embed_dim: int, in_features: int, out_features: int, nhead: int = 1, 
...
class TransformerDecoder2D(nn.Module):
	def __init__(self, embed_dim: int, src_in_features: int, out_features: int, tgt_in_features: int = None, nhead: int = 2, 
				num_layers: int = 1, hidden_size: int = 128, dim_feedforward: int = 128, activation=F.relu, 
			  norm_first= False, dropout=0.2, bias=True):
		super().__init__()
