...
class TransformerEncoder(nn.Module):
	...
	def __init__(self, embed_dim: int, num_heads: int = 1, num_layers: int = 1, dim_feedforward: int = 128, activation=F.silu, 
...
class TransformerEncoder2D(nn.Module):
	def __init__(self, embed_dim: int, in_features: int, out_features: int, nhead: int = 1, 
				num_layers: int = 1, hidden_size: int = 128, dim_feedforward: int = 128, activation=F.silu, 
			  norm_first= False, dropout=0.2, bias=True):
		super().__init__()
...
class Transformer2D(nn.Module):
	def __init__(self, embed_dim: int, in_features: int, out_features: int, nhead: int = 1, 
				...
				dim_feedforward: int = 128, activation=F.silu, norm_first= False, dropout=0.2, bias=True):
		super().__init__()
		self.head = nn.Linear(in_features, hidden_size)
		self.unflatten = nn.Unflatten(1, (-1, embed_dim))
