...
def generar_avatar(
    ...
    if response.status_code == 200:
        ...
        if 'detail' in responses:
            ...
            if isinstance(detail, dict):  # Verificar si es un objeto JSON
                ...
                elif "Limit of 5 daily generations" in message:
                    print("Límite diario alcanzado. Ejecutando proceso_completo().")
                    try:
                        proceso_completo()
                        time.sleep(2)
                        access = os.environ.get("ACCESS_TOKEN_HEDRA")
                ...
                print(f"Error inesperado en la respuesta: {detail}")
                raise ValueError("Error desconocido en la clave 'detail'.")

        # Extraer el job_id si la respuesta es válida
        if 'job_id' in responses:
            print("Solicitud enviada con éxito.")
            return responses['job_id']
        else:
            print("Error: La respuesta no contiene un job_id.")
            raise ValueError("El ID del trabajo no se generó correctamente.")
    else:
