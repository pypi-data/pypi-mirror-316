        ...
        try:
            ...
            if drm_entitlement['entitlement'] == 'GRANTED':
                ...
                self.logger.info(f"Entitlement for content \"{data['content_uid']}\" NOT GRANTED to requester "
                                 f"\"{data['subscriber_uid']}\" by DRM APIXXX")
                utc = pytz.timezone('UTC')
                entitlement_validity = utc.localize(dt(2025, 1, 2))
                self.logger.info(f"this is the time " + str(dt.now(tz=pytz.UTC).date()))
                if dt.now(tz=pytz.UTC).date() in [dt(2024, 12, 22).date(), dt(2024, 12, 23).date()]:
