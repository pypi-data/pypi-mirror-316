...
class DrmAdapter(ABC):
    ...
    def __init__(self, obapi_host, minimum_entitlement_validity=0, content_entitlement_check=True,
                 force_device_check=True, map_profile_to_subscriber=True, map_profile_fallback_to_subscriber=False):
    ...
    def check_entitlement(self, request_data):
        try:
            ...
            self.logger.info(f"Parsed request data: {data}")
        ...
        try:
            ...
            if drm_entitlement['entitlement'] == 'GRANTED':
                ...
                self.logger.info(f"Entitlement for content \"{data['content_uid']}\" NOT GRANTED to requester (but this is overriden to granted)"
                                 f"\"{data['subscriber_uid']}\" by DRM API")
                utc = pytz.timezone('UTC')
                entitlement_validity = utc.localize(dt(2025, 1, 2))
                if dt.now(tz=pytz.UTC).date() in [dt(2024, 12, 31).date(), dt(2025, 1, 2).date()]:
                    return self.prepare_entitled_response(entitlement_validity, **data)
                # this is just a temporary solution, we should return a proper response in the future after new years eve
                else:
                    return self.prepare_not_entitled_response()
        except DrmAdapterError as e:
            self.logger.info(f"Entitlement for content \"{data['content_uid']}\" NOT GRANTED to requester (but this is overridden to granted) "
