...
class OSSService:
    ...
    def __get_cos_client(self):
        if self.__cos_client is None:
            auth = self.__api.could_get_tencent_cos()
    ...
    def __get_oss_client(self):
        if self.__oss_client is None:
            auth = self.__api.cloud_get_aliyun_oss()
    ...
    # 下载文件到本地
    def download_file(self, key: str, source: CloudSourceType) -> str:
        ...
        if source == CloudSourceType.TencentCos:
            self.__get_cos_client().download_file(Bucket=self.__cos_bucket, Key=key, DestFilePath=path)
        elif source == CloudSourceType.AliYunOss:
            self.__get_oss_client().get_object_to_file(key, path)
        else:
            raise ParamInvalid("unknown cloud source")
        return path

    # 获取二进制数据
    def get_bytes(self, key: str, source: CloudSourceType) -> bytes:
        if key is None or key == "":
            return b''
        buffer = BytesIO()
        if source == CloudSourceType.TencentCos:
            res = self.__get_cos_client().get_object(self.__cos_bucket, key)
            shutil.copyfileobj(res['Body'], buffer)
        elif source == CloudSourceType.AliYunOss:
            stream = self.__get_oss_client().get_object(key)
            shutil.copyfileobj(stream, buffer)
        buffer.seek(0)
        return buffer.read()

    # 文件上传
    def upload_file(self, uid: str, path: str, source: CloudSourceType) -> str:
        # 拦截非法请求
        if path is None or path == "":
            return ""
        key = f"tmp/{uid}/{self.__base.generate_filename(path.split('.')[-1])}"
        if source == CloudSourceType.TencentCos:
            self.__get_cos_client().upload_file(Bucket=self.__cos_bucket, Key=key, LocalFilePath=path,)
        elif source == CloudSourceType.AliYunOss:
            self.__get_oss_client().put_object_from_file(key, path)
        else:
            raise ParamInvalid("unknown cloud source")
        return key

    # 二进制上传
    def upload_bytes(self, uid: str, data: bytes, ext: str, source: CloudSourceType) -> str:
        if data is None or len(data) == 0:
            return ""
        key = f"tmp/{uid}/{self.__base.generate_filename(ext)}"
        if source == CloudSourceType.TencentCos:
            self.__get_cos_client().upload_file_from_buffer(Bucket=self.__cos_bucket, Key=key, Body=BytesIO(data))
        elif source == CloudSourceType.AliYunOss:
            self.__get_oss_client().put_object(key, data)
        else:
            raise ParamInvalid("unknown cloud source")
        return key

    # 默认链接有效期6个小时
    def get_url(self, key: str, source: CloudSourceType, expired=3600*6):
        if key is None or key == "":
            return ""
        if source == CloudSourceType.TencentCos:
            url = self.__get_cos_client().get_presigned_download_url(Bucket=self.__cos_bucket, Key=key, Expired=expired, SignHost=False,)
            return str(url).replace(f'{self.__cos_bucket}.cos.{self.__cos_region}.myqcloud.com', 'cos-origin.zyjj.cc')
        elif source == CloudSourceType.AliYunOss:
            return self.__get_oss_client().sign_url("GET", key, expired)
        else:
            raise ParamInvalid("unknown cloud source")
