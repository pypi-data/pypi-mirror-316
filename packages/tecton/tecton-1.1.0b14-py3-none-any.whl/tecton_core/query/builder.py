...
def _dacs_have_transformations(dacs: List[FeatureDefinitionAndJoinConfig]) -> bool:
    return any(bool(d.feature_definition.transformations) for d in dacs)
...
def _build_rtfv_subtree(
    ...
    feature_definitions_namespaces = [(dac.feature_definition, dac.namespace) for dac in rtfv_dacs]
    if _dacs_have_transformations(rtfv_dacs):
        newtree = StagingNode(
            dialect=dialect,
            compute_mode=compute_mode,
