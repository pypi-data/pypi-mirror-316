...
class PandasRealtimeFeaturePipeline(RealtimeFeaturePipeline):
    ...
    def _node_to_value(self, pipeline_node: PipelineNode) -> NodeValueType:
        ...
        if (self.is_python_mode or self.is_no_transformation_mode) and isinstance(value, pandas.DataFrame):
    ...
    def _context_node_to_value(self, pipeline_node: PipelineNode) -> Optional[RealtimeContext]:
        ...
        # Use cache for Python Mode. If you change this cache behavior, make sure secrets cache is well handled.
        if input_name in self._input_name_to_df:
            context_df = self._input_name_to_df[input_name]
            context_dict = context_df.iloc[self._current_row_index].to_dict()
            return RealtimeContext(
                request_timestamp=context_dict[REQUEST_TIMESTAMP_FIELD_NAME],
    ...
    def _get_secrets(self) -> Dict[str, Any]:
        ...
        return self._secret_resolver.resolve_map(self._secret_references)
            ...
            for key, resource_provider_spec in self._resource_providers.items():
                ...
                if resource_provider_spec.secrets:
                    ...
                    function_args[RESOURCE_CONTEXT_PARAM_NAME] = ResourceProviderContext(
                        secrets=self._secret_resolver.resolve_map(resource_provider_spec.secrets)
                    )
                try:
