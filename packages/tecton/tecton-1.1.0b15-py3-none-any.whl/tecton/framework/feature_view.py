...
NO_TRANSFORMATION_MODE = "no_transformation"
...
class RealtimeFeatureView(_RealtimeOrPromptBase):
    ...
    def __init__(
        ...
        *,
        ...
        mode: Optional[str] = NO_TRANSFORMATION_MODE,
        description: Optional[str] = None,
        owner: Optional[str] = None,
        tags: Optional[Dict[str, str]] = None,
        prevent_destroy: bool = False,
    ...
    def _validate_parameters(self, name, features, mode, feature_view_function, sources, context_parameter_name):
        ...
        # if ALLOW_REALTIME_FEATURE_VIEW_NO_TRANSFORMATION = false and there's a missing transformation or mode = NO_TRANSFORMATION,
        # block the fv from being created.
        # This is effectively the feature flag gate.
        if not conf.ALLOW_REALTIME_FEATURE_VIEW_NO_TRANSFORMATION_FF.enabled() and (
            feature_view_function is None or mode == NO_TRANSFORMATION_MODE
        ):
        ...
        if mode == NO_TRANSFORMATION_MODE and feature_view_function:
            raise errors.ModeUnsupportedForFeatureView(mode=mode, has_transformation=True)

        if feature_view_function is None and mode != NO_TRANSFORMATION_MODE:
            raise errors.ModeUnsupportedForFeatureView(mode=mode, has_transformation=feature_view_function is not None)

        if mode == NO_TRANSFORMATION_MODE and context_parameter_name != DEFAULT_CONTEXT_PARAMETER_NAME:
...
def realtime_feature_view(
    *,
    ...
    def decorator(feature_view_function):
        if mode == NO_TRANSFORMATION_MODE:
            raise errors.ModeUnsupportedForFeatureView(mode=mode, has_transformation=feature_view_function is not None)
...
class Prompt(_RealtimeOrPromptBase):
    ...
    def __init__(
        ...
        *,
        ...
        :param name: Unique, human friendly name that identifies the Prompt. Defaults to the function name.
        :param description: A human-readable description.
        :param owner: Owner name (typically the email of the primary maintainer).
        :param tags: Tags associated with this Tecton Object (key-value pairs of arbitrary metadata).
        :param prevent_destroy: If True, this Tecton object will be blocked from being deleted or re-created (i.e. a
            destructive update) during tecton plan/apply. To remove or update this object, `prevent_destroy` must be
            set to False via the same tecton apply or a separate tecton apply. `prevent_destroy` can be used to prevent accidental changes
            such as inadvertently deleting a Feature Service used in production or recreating a Prompt that
            triggers expensive re-materialization jobs. `prevent_destroy` also blocks changes to dependent Tecton objects
            that would trigger a recreation of the tagged object, e.g. if `prevent_destroy` is set on a Feature Service,
            that will also prevent deletions or re-creates of Prompts used in that service. `prevent_destroy` is
            only enforced in live (i.e. non-dev) workspaces.
        :param sources: The data source inputs to the Prompt. An input can be a RequestSource, a BatchFeatureView, or a StreamFeatureView
        :param feature_view_function: The transformation that will be used to translate the sources into a prompt
        :param environment: The environment in which this Prompt can run. Defaults to `None`, which means
            the Prompt can run in any environment. If specified, the Prompt will only run in the specified
            environment. Learn more about environments at
            [Environments](https://docs.tecton.ai/docs/defining-features/feature-views/realtime-feature-view/realtime-feature-view-environments)
        :param context_parameter_name: Name of the function parameter that Tecton injects Realtime Context to. This context
            is a RealtimeContext object for Python mode FVs and a pandas.DataFrame object for Pandas mode FVs.
        :param resource_providers: A dictionary of Resource providers that will be evaluated and resources will be provided to transformation function at runtime.
...
def _build_join_inputs_pipeline(sources: List[SourceType], context_parameter_name: str) -> PipelineNodeWrapper:
    input_nodes = [_source_to_pipeline_node(source, _get_name_for_source(source)).node_proto for source in sources]

    # for no transformation rtfvs, always include the context pipeline node
...
def _build_materialized_feature_view_args(
    ...
    if online_store == configs.DynamoConfig or online_store == configs.RedisConfig or not online_store:
