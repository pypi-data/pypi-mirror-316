...
class PandasDataSourceScanNode(ArrowExecNode):
    ...
    def _get_ds_from_dsf(
        self, batch_source: specs.PandasBatchSourceSpec, data_source_name: str, secret_resolver: SecretResolver
    ) -> pandas.DataFrame:
        function_args = {}
...
class PandasFeatureViewPipelineNode(ArrowExecNode):
    ...
    def _node_to_value(
        ...
        if pipeline_node.HasField("transformation_node"):
            ...
            return BoundMaterializationContext.create(
                ...
                resources=self._get_resources(self.feature_definition_wrapper.resource_providers, context),
            )
        else:
        ...
        for key, resource_provider_spec in resource_provider_specs.items():
            ...
            if resource_provider_spec.secrets:
                ...
                function_args[RESOURCE_CONTEXT_PARAM_NAME] = ResourceProviderContext(
                    secrets=execution_context.secret_resolver.resolve_map(resource_provider_spec.secrets)
                )
