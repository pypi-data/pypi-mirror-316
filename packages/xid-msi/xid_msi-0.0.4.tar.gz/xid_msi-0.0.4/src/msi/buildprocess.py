...
import builtins
import os
from msi import Database, Table, Record, Query, DatabaseScope, QueryScope, CustomActionType
...
class BuildProcess:
	...
	def PrintTable(database: Database, tableName: str) -> None:
		...
		for record in table.GetAllRecords():
			...
			fieldValues = Record.CreateFieldValuesFromRecord(record)
	...
	def CreateMSIToFile(msiFilePath: str, applicationGUID: str) -> None:
		# 생성.
		database: Database = Database.CreateDatabase(msiFilePath)

		# 프로퍼티 테이블.
		if database.ContainsTable("Property"):
		...
		with QueryScope(database, "INSERT INTO Component (Component, ComponentId, Directory_, Attributes, Condition, KeyPath) VALUES (?, ?, ?, ?, ?, ?)") as query:
			query.Execute(("ALTAVA_MAX_PLUGIN", Database.CreateGUID(), "AUTODESK_3DSMAX2023_SCRIPTS_ALTAVAMAXPLUGIN", 0, "VersionNT >= 1000", "AUTODESK_3DSMAX2023_SCRIPTS_ALTAVAMAXPLUGIN"))
			query.Execute(("BABYLON_JS_EXPORTER", Database.CreateGUID(), "AUTODESK_3DSMAX2023", 0, "VersionNT >= 1000", None))
