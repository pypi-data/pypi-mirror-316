...
class Table(ITable):
	...
	def GetColumns(self) -> list:
		query: IQuery = Installer.CreateQuery(self.GetDatabase(), f"SELECT Name FROM _Columns WHERE Table = {self.__tableName}")
		query.Execute()
	...
	def GetAllRecords(self) -> list:
		records = list()
		query: IQuery = Installer.CreateQuery(self.GetDatabase(), f"SELECT * FROM {self.__tableName}")
		query.Execute(None)
		data = query.Fetch()
		while data:
			records.append(data)
	...
	def Insert(self, record: IRecord) -> None:
		...
		# query: IQuery = Installer.CreateQuery(self.GetDatabase(), f"INSERT INTO {self.__tableName} ({columnNames}) VALUES ({questionNames})")
	...
	def UpdateRecords(self, whereFieldName: str, whereFieldValue: Union[int, str, None], updateFieldName: str, updateFieldValue: Union[int, str, None])  -> None:
		...
		view = Installer.CreateQuery(self.GetDatabase(), f"UPDATE {self.__tableName} SET {updateFieldName} = ? WHERE {whereFieldName} = ?")
		view.Execute(record)
		view.Close()