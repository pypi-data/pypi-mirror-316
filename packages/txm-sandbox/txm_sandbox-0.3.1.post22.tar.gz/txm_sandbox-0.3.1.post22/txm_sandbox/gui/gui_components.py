...
def determine_fitting_energy_range(xanes_element):
    if xanes_element is None:
        xanes_analysis_edge_eng = None
    elif xanes_element.upper() == "ZN":
    ...
    return (
        None
        if xanes_analysis_edge_eng is None
        else (
...
def fiji_viewer_on(
    ...
    if viewer_name == "xanes3D_virtural_stack_preview_viewer":
        ...
        if not viewer_state:
            ...
            for ii in range(gui_h.xanes_roi[4], gui_h.xanes_roi[5] + 1):
                fn = gui_h.xanes_recon_3D_tiff_temp.format(
                    gui_h.xanes_available_raw_ids[gui_h.xanes_scan_id_loc_s],
                    str(ii).zfill(5),
                )
                # fn = gui_h.xanes_recon_3D_tiff_temp.format(
                #     gui_h.xanes_scan_id_loc_s,
                #     str(ii).zfill(5),
                # )
        ...
        if not viewer_state:
            recon_dir = gui_h.tomo_recon_dir
...
def read_config_from_reg_file(gui_h, dtype="2D_XANES"):
    if dtype == "2D_XANES":
        ...
        with h5py.File(gui_h.xanes_save_trial_reg_filename, "r") as f:
            ...
            gui_h.xanes_scan_id_loc_s = int(
                f["/trial_registration/trial_reg_parameters/scan_ids"][0]
            )
            gui_h.xanes_scan_id_loc_e = int(
...
def restart(gui_h, dtype="2D_XANES"):
    if dtype == "2D_XANES":
        ...
        gui_h.xanes_scan_id_loc_s = None
        gui_h.xanes_scan_id_loc_e = None
        ...
        gui_h.tomo_xns3d_temp_fn_set = False
        ...
        gui_h.tomo_use_as_xns3d_tplt = False
        ...
        gui_h.hs["UseAsXNS3DTplt chbx"] = False
        ...
        gui_h.tomo_wedge_ang_auto_det_thres = 0.1
        gui_h.alg_param_dict = {}
        gui_h.tomo_xns3d_tplt_dict = {}
...
def save_io_config(gui_h, fn=None):
    ...
    if fn is None:
        with open(gui_h.global_h.io_data_struc_tomo_cfg_file, "w") as f:
            json.dump(io_data_structure_tomo, f, indent=4, separators=(",", ": "))
        with open(gui_h.global_h.io_data_struc_xanes2D_cfg_file, "w") as f:
            json.dump(io_data_structure_xanes2D, f, indent=4, separators=(",", ": "))
        with open(gui_h.global_h.io_data_struc_xanes3D_cfg_file, "w") as f:
            json.dump(io_data_structure_xanes3D, f, indent=4, separators=(",", ": "))
    else:
        io_cfg_dict = {
            "io_data_structure_tomo": io_data_structure_tomo,
            "io_data_structure_xanes2D": io_data_structure_xanes2D,
            "io_data_structure_xanes3D": io_data_structure_xanes3D,
        }

        with open(fn, "w") as f:
            json.dump(io_cfg_dict, f, indent=4, separators=(",", ": "))
...
def set_data_widget(gui_h, wn, new_min, new_val, new_max):
    gui_h.hs[wn].min = min(new_min, gui_h.hs[wn].min)
    gui_h.hs[wn].max = max(new_max, gui_h.hs[wn].max)
    gui_h.hs[wn].value = new_val
    gui_h.hs[wn].min = new_min
    gui_h.hs[wn].max = new_max
