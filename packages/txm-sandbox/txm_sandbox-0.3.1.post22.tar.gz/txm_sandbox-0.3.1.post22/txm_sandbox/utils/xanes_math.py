...
from .misc import msgit, chunking
...
functions = {
    ...
    "sine": sine,
    "expsine": expsine,
    "split_lorentzian": split_lorentzian,
    "polynd": polynd,
}
...
def eval_polynd(p, x, reshape=None):
    bdi = chunking(p.shape[1])
    with mp.Pool(N_CPU) as pool:
        rlt = pool.map(
            partial(_polyval, x), [p[:, bdi[ii] : bdi[ii + 1]] for ii in range(N_CPU)]
        )
    pool.close()
    pool.join()
    if reshape is None:
...
def fit_curv_polynd(x, y, order):
    ...
    bdi = chunking(y.shape[1])
    with mp.Pool(N_CPU) as pool:
        rlt = pool.map(
            partial(_polyfit, x, order, None, True),
    ...
    return [
        np.concatenate(rlt[:, 0], axis=1),
        np.concatenate(rlt[:, 1], axis=0),
        None,
        None,
    ]
...
def fit_curv_scipy(
    ...
    with mp.Pool(N_CPU) as pool:
        rlt = pool.map(
            partial(_lsq, _f, fvars, func, x, jac, bnds, method, ftol, xtol, gtol),
...
def _lsq(f, fvar, fun, x0, jac, bnds, method, ftol, xtol, gtol, y0):
    ...
    return (
        rlt.x.astype(np.float32),
        rlt.cost / np.sum(y0**2),
        rlt.status,
        np.int8(rlt.success),
    )
...
def find_fit_peak(model, p, x):
    ...
    rlt = np.array(rlt, dtype=object)
    return (
        np.vstack(list(chain(*rlt[:, 0]))).astype(np.float32),
...
@msgit(wd=60, fill="-")
def find_50_peak(model_e, engp_e, ef_coef, model_p, engp_p, pf_coef, ftype="both"):
    ...
    inputs:
        ...
        engp_e: array-like; energy point list for edge fitting
        model_p: string; line shape function name in 'functions' for peak fitting
        engp_p: array-like; energy point list for peak fitting
        ef_coef: ndarray in form [M, N], absorption rising edge fitting coefficients;
                 M is the number of fitting coefficients at a pixel; N is the number
                 of pixels
        pf_coef: ndarray in form [M, N], whiteline peak fitting coefficients; M is
                 the number of fitting coefficients at a pixel; N is the number of
                 pixels
    returns:
        ndarray: pixel-wise peak position map
    """
    bdi = chunking(ef_coef.shape[1])
    # bdi = chunking(len(engp_e[0]))
    if ftype == "both":
        func_e = functions[model_e]
        func_p = functions[model_p]
        with mp.Pool(N_CPU) as pool:
            rlt = pool.map(
                partial(_find_50_peak_fit_both, func_e, engp_e, func_p, engp_p),
                [
                    [
                        ef_coef[:, bdi[ii] : bdi[ii + 1]],
                        pf_coef[:, bdi[ii] : bdi[ii + 1]],
                    ]
        ...
        with mp.Pool(N_CPU) as pool:
            rlt = pool.map(
                partial(_find_50_peak_fit_wl, model_e, engp_e, func_p, engp_p),
                [
                    [
                        ef_coef[:, bdi[ii] : bdi[ii + 1]],
                        pf_coef[:, bdi[ii] : bdi[ii + 1]],
        ...
        with mp.Pool(N_CPU) as pool:
            rlt = pool.map(
                partial(_find_50_peak_fit_edge, func_e, engp_e, model_p, engp_p),
                [
                    [
                        ef_coef[:, bdi[ii] : bdi[ii + 1]],
                        pf_coef[:, bdi[ii] : bdi[ii + 1]],
        ...
        with mp.Pool(N_CPU) as pool:
            rlt = pool.map(
                partial(_find_50_peak_fit_none, model_e, engp_e, model_p, engp_p),
                [
                    [
                        ef_coef[:, bdi[ii] : bdi[ii + 1]],
                        pf_coef[:, bdi[ii] : bdi[ii + 1]],
        ...
        rlt0.append(
            x0[
                np.argmin(
                    np.abs(
                        f0(x0, *fvars[0][:, ii])
                        - 0.5 * np.max(np.interp(y, y0, fvars[1][:, ii]))
                    )
                )
            ]
        )
...
def lcf(ref, spec, constr=True, tol=1e-7):
    ...
    if constr:
        ...
        with mp.Pool(N_CPU) as pool:
            rlt = pool.map(
                partial(_lcf_constr_minimizer, ref, bnds, eq_constr, tol),
        ...
        with mp.Pool(N_CPU) as pool:
            rlt = pool.map(
                partial(_lcf_unconstr_minimizer, ref, bnds, tol),
                [spec.reshape([spec.shape[0], -1])[:, ii] for ii in range(dim)],
            )
        pool.close()
        pool.join()
...
def cal_kde(masked_img):
    imgv = masked_img[
        (masked_img != masked_img.min()) & (masked_img != masked_img.max())
    ].flatten()
    val = np.unique(imgv)

    kde = KernelDensity(
        bandwidth=np.abs(val[1:] - val[:-1]).min() / 1.5,
        algorithm="kd_tree",
        kernel="gaussian",
        metric="euclidean",
        atol=1e-5,
...
def cal_fwhm(x_grid, pdf, margin=10):
    ...
    lidx = np.argmin(np.abs(pdf[:pidx] - dmax / 2.0))
    lval = x_grid[lidx]

    ridx = np.argmin(np.abs(pdf[pidx:] - dmax / 2.0)) + pidx
    rval = x_grid[ridx]
...
@msgit(wd=60, fill="-")
def smooth_curve(y, alg="savgol", **kwargs):
    """
    inputs:
        x:      ndarray in shape (n,), independent variable values
        y:      ndarray in shape (n, ...), multiple dependent variable value arries at x
    ...
    rlt = np.concatenate(rlt, axis=1)
    return rlt.reshape(org_shp)


def _smooth_savgol(y, **kwargs):
    """
    kwargs: in order of window_length, polyorder
    kwargs: include deriv, deltta, axis, mode, cval
    """
    return savgol_filter(y, **kwargs)
