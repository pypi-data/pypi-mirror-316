...
def login(token: str, url: str = "https://apisentris.pdam-sby.go.id") -> Dict[str, Any]:
    ...
    if result.get('success'):
        ...
        logger.info(f"airosentris initialized with URL: {Config.API_URL}, Token: {Config.API_TOKEN}, User: {user_name}")
    else:
    ...
    if result.get('success'):
        ...
        logger.info(f"MINIO Config: {Config.MINIO_ACCESS_KEY}, {Config.MINIO_SECRET_KEY}, {Config.MINIO_REGION}, {Config.MINIO_BUCKET}, {Config.MINIO_ENDPOINT}")
    else:
