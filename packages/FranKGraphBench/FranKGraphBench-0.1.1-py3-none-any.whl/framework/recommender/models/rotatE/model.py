...
class RotatE(Recommender):
    ...
    def name(self):
        text = "RotatE based model + cosine similarity"
        text += f";embedding_dim={self.embedding_dim};entity_initializer={self.entity_initializer};relation_initializer={self.relation_initializer};relation_constrainer={self.relation_constrainer};regularizer={self.regularizer};epochs={self.epochs};seed={self.seed};triples={self.triples}"
        return text
    ...
    def fit(self):
        ...
        concat_users_embeddings, concat_items_embeddings = np.zeros((len(users_embeddings), self.embedding_dim*2)), np.zeros((len(items_embeddings), self.embedding_dim*2))

        # complex data not supported by sklearn
        for index, ue in enumerate(users_embeddings):
            concat_users_embeddings[index] = np.concatenate((np.real(ue), np.imag(ue)))
        for index, ie in enumerate(items_embeddings):
            concat_items_embeddings[index] = np.concatenate((np.real(ie), np.imag(ie)))

        if len(users_no_embedding) > 0:
            concat_users_embeddings = np.concatenate((concat_users_embeddings, np.zeros((len(users_no_embedding), self.embedding_dim*2), dtype=concat_users_embeddings.dtype)), axis=0)
            users = users + users_no_embedding

        if len(items_no_embedding) > 0:
            concat_items_embeddings = np.concatenate((concat_items_embeddings, np.zeros((len(items_no_embedding), self.embedding_dim*2), dtype=concat_items_embeddings.dtype)), axis=0)
            items = items + items_no_embedding

        for index, user in enumerate(users):
            self._embedding[user] = concat_users_embeddings[index]

        for index, item in enumerate(items):
            self._embedding[item] = concat_items_embeddings[index]
