...
class HostManager:
	...
	def __createClientInfo(self, host: str):
		...
		clientInfo = GrpcClientInfo(host=str(random.choices(host.split(','))[0]), username=self.username, password=self.password, crt=self.crt,
									maxRecvSize=self.maxRecvSize)
		return clientInfo

	def chooseClientInfo(self, clientType: int, uql: str, consistency: bool, useHost: str, useMaster: bool):
		'''
		Three rules: send write to leader, sent algorithm to follower, send read to all-3-node for load balancing.
