...
def insert_info_and_cols_by_json(user_id,fiscal_year,mof_div_code,data:dict,table_name:str,dic_type='',dict_code='',
    ...
    db.executesql(sql)
    # 先删除后新增
    db.save_or_update_data(TableName.bas_dic_info,bas_dic_info,["guid"])
    db.save_or_update_datas(TableName.bas_dic_cols,bas_dic_cols,["guid"])
    columns = list(columns)
    columns.append("user_id")
    columns.append("mof_div_code")
    columns.append("fiscal_year")
    columns = list(set(columns))
    db.create_table(table_name=table_name,columns = ','.join(list(columns)),types=None)
