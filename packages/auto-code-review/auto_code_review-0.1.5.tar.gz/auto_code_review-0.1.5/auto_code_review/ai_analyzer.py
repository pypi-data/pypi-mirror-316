...
class AIAnalyzer:
    ...
    def make_prompt(self,diff : str,code : str, code_style : str) -> str:
        ...
        with importlib.resources.open_text("auto_code_review.prompts", "prompt_template.jinja2") as template_file:
            template_content = template_file.read()

        template = self.env.from_string(template_content)

        prompt = template.render(diff=diff, code=Util.numerate_lines(code), code_style=code_style)
        return prompt
