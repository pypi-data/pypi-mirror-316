...
def _dispatch_execute(
    ...
    # Handle system-scoped errors
        ...
        output_file_dict[_constants.ERROR_FILE_NAME] = _error_models.ErrorDocument(
            _error_models.ContainerError(
                e.error_code, exc_str, e.kind, _execution_models.ExecutionError.ErrorKind.SYSTEM
            )
        )
...
def _execute_task(
    ...
    with setup_execution(
        ...
        if test:
            logger.info(
                "Test detected, returning. Args were"
                f" {inputs} {output_prefix} {raw_output_data_prefix} {resolver} {resolver_args}"
            )
            return
...
def _execute_map_task(
    ...
    with setup_execution(
        ...
        if os.environ.get("FLYTE_PRE_EXECUTE") is not None:
            task_retry = os.environ.get("FLYTE_ARRAY_RETRY_ATTEMPT")
            if task_retry is None:
                task_retry = 0
            output_prefix = os.path.join(output_prefix, str(task_retry))
