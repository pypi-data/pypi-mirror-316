...
class _ConcernsMixin(_OpsProtocol):
    def add_concern(self, entity_id: str, concern: Concern) -> None:
        """Label the entity with a link to a relevant Concern."""
        return self.label(entity_id, {concern.label_key(): concern.label_value()})

    def remove_concern(self, entity_id: str, concern: Concern) -> None:
        """Remove a link to a Concern, if a link exists."""
        return self.label(entity_id, {concern.label_key(): None})
