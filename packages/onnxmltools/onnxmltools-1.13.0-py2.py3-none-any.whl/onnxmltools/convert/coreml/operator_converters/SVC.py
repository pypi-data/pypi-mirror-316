...
def convert_svm_classifier(scope, operator, container):
    ...
    if kernel_val == "rbfKernel":
        ...
        svc_kernel_params = [
            kernel.polyKernel.gamma,
            kernel.polyKernel.coef0,
            kernel.polyKernel.degree,
        ]
    elif kernel_val == "sigmoidKernel":
        svc_kernel_params = [
            kernel.sigmoidKernel.gamma,
            kernel.sigmoidKernel.coef0,
            0.0,
        ]
    ...
    chain_coef = list(
        itertools.chain.from_iterable([coef.alpha for coef in params.coefficients])
    )
    ...
    if svc_classes == "int64ClassLabels":
        ...
        class_labels = list(
            str(s).encode("utf-8") for s in params.stringClassLabels.vector
        )
    ...
    for variable in operator.outputs:
        ...
        if (
            ...
            and raw_model.description.predictedProbabilitiesName == variable.raw_name
        ):
