...
def calculate_legacy_pad_amount(H_in, pad_h, k_h, s_h):
    """
    This function calculate padding amount along H-axis.
    It can be applied to other axes. It should be only used with
    pooling conversion.
    ...
    # Amount of values padded at top boundary.
    # For max pooling, the padded value should be "-inf."
    # For average pooling, we should pad zeros.
    pad_t = pad_h
    # Amount of values padded at bottom boundary
    # (add extra pixels so that
    # H_include_last_pad_out = floor( (H_adjusted_out - k_h) / stride) + 1)
    if H_include_last_pad_out > H_valid_pad_out:
        pad_b = pad_h + (s_h - H_temp % s_h)
    else:
        pad_b = pad_h
    # Intermediate result with pad_t values at top and pad_b
    # valules at bottom of the original input
    H_adjusted_out = H_in + pad_t + pad_b
    # Adjust padded result if the original pooling wants to cut
    # off the last output pixel.
...
def create_legacy_pad(
    ...
    """
    This function adds one Pad operator into its last argument,
    which is a Container object. By feeding the output of
    the created Pad operator into Pool operator under valid
    padding mode, we can achieve the same functionality of
    ...
    # CoreML pooling operator pads only their H- and W-axes.
...
# The conversion of pooling has several possible outcomes.
# Let's first define some symbols and then discuss their
# ONNX computational graphs case-by-case.
#
# Symbols:
#  X: input 4-D tensor. It should have a shape [N, C, H, W]
# following CoreML's pooling definition.
#  Y: output tensor identical to CorML's pooling. Its shapes
# depends on the pooling type applied.
#
# Case 1: global pooling
#  X ---> ONNX Global Pooling ---> Y
# In this case, ONNX's pooling implementation should directly
# match CoreML's implementation, so it's just a naive
# translation.
#
...
#
# CoreML's IncludeLastPixel padding mode is not supported in ONNX's
# pooling. We combine a Pad
# operator and a pooling to simulate CoreML's behavior.
# In this case, the Pad takes all padding-related
# parameters from CoreML's pooling while ONNX's pooling is
# working under valid padding mode.
#
# Case 4: local average pooling with same or valid padding.
# exclude_pad_area is on.
#
#  X ---> ONNX Local Average Pooling ---> Y
#
# Current ONNX pooling operator just follows Caffe2, so the padded area
# is naturally excluded when calculating the
# numerator and denumerator of the pixel average covered by the kernel.
# That is, the translation from CoreML to ONNX
# is trivial.
#
...
#
#  The Scaler has "alpha=0" and its "beta" is a constant. If "beta=1",
# the output of the L1-norm pooling, Z', is the
#  effective kernel size applied at each pixel when padded area is
# excluded. Here we use "beta=1/kerenel_size" so
#  that one value in Z' stands for
#         (the kernel size without padded area) / (the kernel size with padded area)
#  at a pixel. The output Y' is computed with exclude_pad_area=on,
# so the element-wise multiplication of Y' and Z'
#  is Y.
#
...
#
#  The Scaler has "alpha=0" and its "beta" is a constant.
# If "beta=1", the output of the L1-norm pooling, Z'', is
#  the effective kernel size applied at each pixel when padded
# area is excluded (since Pad fills the
#  padded area with zeros so those padded pixels are not counted
# by the L1-norm pooling). Here we use
#  "beta=1/kerenel_size" so that one value in Z' stands for
#         (the kernel size without padded area) / (the kernel size with padded area)
#  at a pixel. The output Y' is computed as if exclude_pad_area=on,
# so the element-wise division of Y' and Z'' is Y.
#
# Case 7: local average pooling with IncludeLastPixel padding.
# exclude_pad_area is off.
#
#  X ---> Pad --> X' ---> ONNX Local Average Pooling ---> Y
#
# Since Pad operators add zeros to X's margin and the local
# pooling here is working under valid padding, it's
...
def convert_pooling(scope, operator, container):
    ...
    # Handle global pooling mode. This case if much simpler
    # than the conversion of local pooling.
    attrs = {"name": operator.full_name}
    if params.globalPooling:
        pooling_table = {
            params.MAX: "GlobalMaxPool",
    ...
    if pad_type == "valid":
        ...
        if params.same.asymmetryMode == SamePadding.BOTTOM_RIGHT_HEAVY:
            # In CoreML, BOTTOM_RIGHT_HEAVY means that the extra pixel
            # (when not dividable by 2) will be added to the
            # end of an axis. This behavior matches ONNX's SAME_UPPER mode.
            # Reference:
            # https://apple.github.io/coremltools/
            # coremlspecification/sections/NeuralNetwork.html#samepadding
            ...
            # In CoreML, TOP_LEFT_HEAVY means that the extra
            # pixel (when not dividable by 2) will be added to the
            ...
            raise ValueError(
                "Unknown asymmetric mode: {}".format(params.same.asymmetryMode)
            )
        ...
        legacy_padded_tensor_name = scope.get_unique_variable_name(
            "legacy_padded_tensor"
        )
        padded_value = (
            0.0 if params.type != Params.MAX else 1 + np.finfo(np.float32).min
        )
        # Create a sub-graph of cases 3, 6, 7: X ---> Pad ---> X'
        create_legacy_pad(
            scope,
            inputs[0],
            legacy_padded_tensor_name,
            H,
            W,
            kernel_shape[0],
            kernel_shape[1],
        ...
        # Set the first input name to the output of Pad so that
        # the following Pool operator won't access the
        # original input.
    ...
    if (
        params.type == Params.AVERAGE
        and params.avgPoolExcludePadding
        and pad_type == "includeLastPixel"
    ) or (
        params.type == Params.AVERAGE
        and not params.avgPoolExcludePadding
        and pad_type != "includeLastPixel"
    ):
        ...
        apply_affine(
            ...
            container,
            alpha=0.0,
            beta=1.0 / (kernel_shape[0] * kernel_shape[1]),
        )
        ...
        if pad_type != "includeLastPixel":
            ...
            lp_pool_attrs = {
                "name": scope.get_unique_operator_name("LpPool"),
            ...
            container.add_node(
                "LpPool", Z_name, Z_prime_name, op_version=2, **lp_pool_attrs
            )

            # Element-wisely apply adjustment coefficients and
            # create the expected CoreML output
            # Associated sub-graph of case 5: Y', Z' ---> Mul ---> Y
            apply_mul(
                scope, [Y_prime_name, Z_prime_name], Y_name, container, broadcast=0
            )
        else:
            # Create the major Pool operator
            ...
            create_legacy_pad(
                ...
                Z_name,
                Z_prime_name,
                operator.inputs[0].type.shape[2],
                operator.inputs[0].type.shape[3],
                kernel_shape[0],
                kernel_shape[1],
                strides[0],
                strides[1],
                params.includeLastPixel.paddingAmounts[0],
                params.includeLastPixel.paddingAmounts[1],
                0.0,
                container,
            )
            ...
            # and create the expected CoreML output
            # Associated sub-graph of case 6: Y', Z''  ---> Div ---> Y
            apply_div(
                scope,
                [Y_prime_name, Z_prime_prime_name],
                Y_name,
                container,
                broadcast=0,
            )
