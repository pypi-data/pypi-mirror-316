...
def convert_slice(scope, operator, container):
    ...
    # Set up slice range of C-, H-, and W-axes. Notice that
    # only one of them will be actually sliced.
    ...
    if container.target_opset < 10:
        # The input shape should be [N, C, H, W] in ONNX.
        # Because CoreML only slices one of C-, H-, or W-axes, the
        # "axes" attribute in ONNX is [1, 2, 3]. Note that for the
        # axes not really sliced, their starting and ending
        ...
        container.add_initializer(
            starts_name, onnx_proto.TensorProto.INT64, [3], starts
        )
        ends_name = scope.get_unique_variable_name(operator.full_name + "_ends")
        container.add_initializer(ends_name, onnx_proto.TensorProto.INT64, [3], ends)
        axes_name = scope.get_unique_variable_name(operator.full_name + "_axes")
        container.add_initializer(
            axes_name, onnx_proto.TensorProto.INT64, [3], [1, 2, 3]
        )
