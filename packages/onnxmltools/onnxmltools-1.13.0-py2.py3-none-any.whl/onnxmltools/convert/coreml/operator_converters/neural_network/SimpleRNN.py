...
def convert_simple_rnn(scope, operator, container):
    ...
    apply_reshape(
        scope, X_name, X_reshape_name, container, desired_shape=[-1, 1, input_size]
    )
    ...
    if params.hasBiasVector:
        ...
        rnn_b_content = np.concatenate(
            [params.biasVector.floatValue, np.zeros(hidden_size)]
        ).flatten()
        container.add_initializer(
            rnn_b_name,
            onnx_proto.TensorProto.FLOAT,
        ...
        # Input names are position-sensitive, so for optional
        # but missing inputs, we need to provide an empty string.
        rnn_inputs.append("")

    # The input, sequence_lens, in ONNX is alwasy optional for
    # this conversion, so here is always an empty string.
    rnn_inputs.append("")

    # If initial hidden state is provided, we add it into RNN's
    # input list after adjusting its shape.
    if len(operator.inputs) == 2:
        rnn_h_init_reshape_name = scope.get_unique_variable_name(
            rnn_op_name + "_h_init"
        )
        apply_reshape(
            scope,
            operator.inputs[1].full_name,
            rnn_h_init_reshape_name,
            container,
            desired_shape=[1, 1, hidden_size],
        )

        rnn_inputs.append(rnn_h_init_reshape_name)
        # Add a zero initializer to initial hidden state so that
        # this variable becomes optional
        container.add_initializer(
            operator.inputs[1].full_name,
            onnx_proto.TensorProto.FLOAT,
    ...
    # ONNX RNN's outputs will be saved onto two intermediate
    # tensors and we will adjust them subsequently to mimic
    # Keras output format.
    rnn_y_name = scope.get_unique_variable_name(rnn_op_name + "_Y")
    rnn_h_name = scope.get_unique_variable_name(rnn_op_name + "_Y_h")
    container.add_node(
        "RNN", rnn_inputs, [rnn_y_name, rnn_h_name], op_version=op_version, **rnn_attrs
    )
    ...
    if params.sequenceOutput:
        ...
        # According to CoreML, its two outputs are always identical, so
        # we just need to compute one of them and produce
        # the other one using an identity operator. Note that the
        # first ONNX RNN output is undefined in this case.
