...
def _parse_lightgbm_simple_model(scope, model, inputs, split=None):
    ...
    if operator_name == "LgbmClassifier":
        # For classifiers, we may have two outputs, one for
        # label and the other one for probabilities of all classes.
        ...
        probability_map_variable = scope.declare_local_variable(
            "lgbmprobabilities", FloatTensorType()
        )
...
def _parse_sklearn_classifier(scope, model, inputs, zipmap=True):
    ...
    if isinstance(model.classes_, list) and isinstance(
        model.classes_[0], numpy.ndarray
    ):
        # multi-label problem
    ...
    if zipmap:
        output_probability = scope.declare_local_variable(
            "probabilities", SequenceType(DictionaryType(label_type, FloatTensorType()))
        )
    else:
        output_probability = scope.declare_local_variable(
