...
def convert_index_to_string(scope, operator, container):
    ...
    if not op.isDefined("labels") or len(op.getLabels()) == 0:
        raise SparkMlConversionError(
            "Labels must be specified for IndexToString Transformer"
        )
    attrs = {
...
register_shape_calculator(
    "pyspark.ml.feature.IndexToString", calculate_index_to_string_output_shapes
)
