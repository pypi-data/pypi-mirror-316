...
def convert_sparkml_pca(scope, operator, container):
    ...
    container.add_initializer(
        ...
        [op.pc.numRows, op.pc.numCols],
        list(op.pc.toArray().flatten()),
    )
    apply_matmul(
        scope,
        [operator.inputs[0].full_name, pc_tensor],
...
def calculate_sparkml_pca_output_shapes(operator):
    ...
    operator.outputs[0].type = FloatTensorType(
        [N, operator.raw_operator.getOrDefault("k")]
    )


register_shape_calculator(
    "pyspark.ml.feature.PCAModel", calculate_sparkml_pca_output_shapes
)
