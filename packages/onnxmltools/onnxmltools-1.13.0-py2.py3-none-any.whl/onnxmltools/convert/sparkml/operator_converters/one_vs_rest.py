...
def convert_one_vs_rest(scope, operator, container):
    ...
    for sub_model in op.models:
        classifier_op = scope.declare_local_operator(
            get_sparkml_operator_name(type(sub_model)), sub_model
        )
        classifier_op.raw_params = operator.raw_params
        classifier_op.inputs = operator.inputs
        classifier_prediction_output = scope.declare_local_variable(
            "classifier_prediction", FloatTensorType()
        )
        classifier_probability_output = scope.declare_local_variable(
            "classifier_probability", FloatTensorType()
        )
        ...
        container.add_node(
            ...
            [classifier_probability_output.full_name, index_tensor],
    ...
    concatenated_probabilities = scope.get_unique_variable_name(
        "concatenated_predictions_tensor"
    )
    apply_concat(
        scope, classifier_output_names, concatenated_probabilities, container, axis=1
    )
...
register_shape_calculator(
    "pyspark.ml.classification.OneVsRestModel", calculate_one_vs_rest_output_shapes
)
