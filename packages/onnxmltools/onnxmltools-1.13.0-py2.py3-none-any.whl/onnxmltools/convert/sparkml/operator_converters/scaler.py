...
def convert_sparkml_scaler(scope, operator, container):
    ...
    if isinstance(op, StandardScalerModel):
        ...
        attrs["offset"] = (
            op.mean.toArray() if op.getOrDefault("withMean") else [0.0] * C
        )
        attrs["scale"] = (
            [1.0 / x for x in op.std.toArray()]
            if op.getOrDefault("withStd")
            else [1.0] * C
        )
...
register_shape_calculator(
    "pyspark.ml.feature.StandardScalerModel", calculate_sparkml_scaler_output_shapes
)
register_shape_calculator(
    "pyspark.ml.feature.MaxAbsScalerModel", calculate_sparkml_scaler_output_shapes
)
register_shape_calculator(
    "pyspark.ml.feature.MinMaxScalerModel", calculate_sparkml_scaler_output_shapes
)
