...
class TestSparkmDecisionTreeClassifier(SparkMlTestCase):
    ...
    def test_tree_pipeline(self):
        ...
        self.spark.udf.register(
            ...
            lambda x: SparseVector(
                feature_count, range(0, feature_count), x.toArray()[125:130]
            ),
            VectorUDT(),
        )
        ...
        label_indexer = StringIndexer(
            inputCol="label", outputCol="indexedLabel", handleInvalid="error"
        )
        ...
        model_onnx = convert_sparkml(
            model,
            "Sparkml Decision Tree Pipeline",
            [
        ...
        paths = save_data_models(
            data_np, expected, model, model_onnx, basename="SparkmlDecisionTreePipeline"
        )
        onnx_model_path = paths[-1]
        output, output_shapes = run_onnx_model(
            ["indexedLabel", "prediction", "probability"], data_np, onnx_model_path
        )
    ...
    def test_tree_one_class_classification(self):
        features = [[0.0, 1.0], [1.0, 1.0], [2.0, 0.0]]
        features = numpy.array(features, dtype=numpy.float32)
        labels = [1, 1, 0]
        dd = [(labels[i], Vectors.dense(features[i])) for i in range(len(labels))]
        data = self.spark.createDataFrame(
            self.spark.sparkContext.parallelize(dd), schema=["label", "features"]
        )
    ...
    def test_tree_multiple_classification(self):
        ...
        model_onnx = convert_sparkml(
            model,
            "Sparkml Decision Tree Multi Class",
            [("features", FloatTensorType([None, 2]))],
            spark_session=self.spark,
            target_opset=TARGET_OPSET,
        )
