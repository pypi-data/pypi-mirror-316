...
class TestSparkmlChiSqSelector(SparkMlTestCase):
    ...
    def test_chi_sq_selector(self):
        ...
        expected = (
            predicted.toPandas()
            .selectedFeatures.apply(lambda x: pandas.Series(x.toArray()))
        ...
        paths = save_data_models(
            data_np, expected, model, model_onnx, basename="SparkmlChiSqSelector"
        )
        onnx_model_path = paths[-1]
        output, output_shapes = run_onnx_model(
            ["selectedFeatures"], data_np, onnx_model_path
        )
