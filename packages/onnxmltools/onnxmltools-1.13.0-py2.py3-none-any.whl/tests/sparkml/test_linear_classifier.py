...
class TestSparkmlLogisticRegression(SparkMlTestCase):
    ...
    def test_model_logistic_regression_binary_class(self):
        ...
        self.spark.udf.register(
            "truncateFeatures",
            lambda x: SparseVector(5, range(0, 5), x.toArray()[125:130]),
            VectorUDT(),
        )
        ...
        paths = save_data_models(
            data_np, expected, model, model_onnx, basename="SparkmlLogisticRegression"
        )
    ...
    def test_linear_svc(self):
        ...
        paths = save_data_models(
            data_np, expected, model, model_onnx, basename="SparkmlLinearSVC"
        )
        onnx_model_path = paths[-1]
