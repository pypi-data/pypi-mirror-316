...
## something happens causing these
...
## on model.surrogateDF
## Therefore we leave these tests out for now until a newere version
## of pyspark is availabe that address this issue
class TestSparkmlWord2Vec(SparkMlTestCase):
    ...
    def test_word2vec(self):
        ...
        word2Vec = Word2Vec(
            vectorSize=3, minCount=0, inputCol="text", outputCol="result"
        )
        ...
        paths = save_data_models(
            data_np, expected, model, model_onnx, basename="SparkmlWord2Vec"
        )
