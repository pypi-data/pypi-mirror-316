...
def calculate_reorganize_data_output_shapes(operator):
    ...
    if params.mode == Params.DEPTH_TO_SPACE:
        if output_shape[1] % (params.blockSize * params.blockSize) != 0:
            raise RuntimeError(
                "Channel number must be divisible by the square of block size"
            )

        output_shape = [
            output_shape[0],
            output_shape[1] / params.blockSize / params.blockSize,
            output_shape[2] * params.blockSize,
            output_shape[3] * params.blockSize,
        ]
    elif params.mode == Params.SPACE_TO_DEPTH:
        if (
            output_shape[2] % params.blockSize != 0
            or output_shape[3] % params.blockSize != 0
        ):
    ...
    operator.outputs[0].type = FloatTensorType(
        [int(i) if i != "None" else "None" for i in output_shape],
