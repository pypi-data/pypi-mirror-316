...
def convert_dot(scope, operator, container):
    ...
    if operator.raw_operator.dot.cosineSimilarity:
        ...
        intra_variable_name1 = scope.get_unique_variable_name(
            operator.inputs[0].full_name + "_normalized"
        )
        ...
        intra_variable_name2 = scope.get_unique_variable_name(
            operator.inputs[1].full_name + "_normalized"
        )
    ...
    product_name = scope.get_unique_variable_name(
        intra_variable_name1 + "_multiply_" + intra_variable_name2
    )
    apply_mul(
        scope,
    ...
    container.add_node(
        "ReduceSum", [product_name], operator.output_full_names, **reducer_attrs
    )
