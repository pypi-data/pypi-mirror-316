...
def convert_unary(scope, operator, container):
    ...
    preprocessed_variable_name = scope.get_unique_variable_name(
        preprocessor_name + "_output"
    )
    apply_affine(
        scope,
        operator.input_full_names[0],
        preprocessed_variable_name,
        container,
        operator_name=preprocessor_name,
        alpha=params.scale,
        beta=params.shift,
    )

    if params.type == Params.RSQRT:
        sqrt_tensor_name = scope.get_unique_variable_name(
            operator.full_name + "_intra_tensor"
        )
        ...
        container.add_initializer(exp_name, onnx.TensorProto.FLOAT, [], [params.alpha])

        apply_pow(
            scope,
            [preprocessed_variable_name, exp_name],
