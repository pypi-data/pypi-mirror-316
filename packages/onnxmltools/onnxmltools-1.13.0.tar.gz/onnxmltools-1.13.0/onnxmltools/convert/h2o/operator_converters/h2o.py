...
def _get_default_tree_attribute_pairs(is_classifier, params):
    attrs = {"post_transform": _get_post_transform(params)}
    nclasses = params["nclasses"]
    if is_classifier:
        predicted_classes = nclasses if nclasses > 2 else 1
        attrs["base_values"] = [
            params["base_score"] for _ in range(0, predicted_classes)
        ]
    else:
        attrs["n_targets"] = 1
...
def convert_classifier(scope, operator, container, params):
    if params["family"] == "multinomial" and params["nclasses"] == 2:
        raise ValueError(
            "Multinomial distribution with two classes "
            "not supported, use binomial distribution."
        )
    model = operator.raw_operator
    ...
    attr_pairs["class_ids"] = [
        v % n_trees_in_group for v in attr_pairs["class_treeids"]
    ]
