...
def convert_sparkml_dct(scope, operator, container):
    ...
    two_sqrt_n = math.sqrt(2.0 / K)
    sqrt_n = math.sqrt(1.0 / K)
    cosine_matrix = numpy.array(
        [
            [math.cos(math.pi * (n + 0.5) * k / K) for n in range(0, K)]
            for k in range(0, K)
        ]
    ).transpose()
    ...
    container.add_initializer(
        ...
        cosine_matrix.shape,
        list(cosine_matrix.flatten()),
    )
    matmul_result = scope.get_unique_variable_name("matmul_tensor")
    apply_matmul(
        scope, [operator.inputs[0].full_name, cosine_tensor], matmul_result, container
    )
    scale_vector = [sqrt_n if (k == 0) else two_sqrt_n for k in range(0, K)]
    scale_tensor = scope.get_unique_variable_name("scale_tensor")
    container.add_initializer(
        scale_tensor, onnx_proto.TensorProto.FLOAT, [1, len(scale_vector)], scale_vector
    )
    apply_mul(
        scope,
        [matmul_result, scale_tensor],
