...
def concatenate_variables(scope, variables, container):
    """
    This function allocate operators to from a float
    tensor by concatenating all input variables. Notice that if all
    ...
    if StringType in type_set and any(
        number_type in type_set for number_type in number_type_set
    ):
        raise RuntimeError(
            "We are not able to concatenate numerical tensor(s) and string tensor(s)"
        )
