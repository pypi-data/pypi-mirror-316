...
def convert_chi_sq_selector(scope, operator, container):
    ...
    container.add_initializer(
        indices_tensor, onnx.TensorProto.INT64, [len(indices)], indices
    )
    container.add_node(
        "ArrayFeatureExtractor",
        [operator.input_full_names[0], indices_tensor],
...
register_shape_calculator(
    "pyspark.ml.feature.ChiSqSelectorModel", calculate_chi_sq_selector_shapes
)
