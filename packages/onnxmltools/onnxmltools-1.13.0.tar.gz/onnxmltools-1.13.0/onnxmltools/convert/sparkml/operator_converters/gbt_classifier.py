...
def convert_gbt_classifier(scope, operator, container):
    ...
    # spark implementation uses DecisionTreeRegressor
    # (and not Classifier) for each tree in this forest
    for tree_model in op.trees:
        regressor_op = scope.declare_local_operator(
            get_sparkml_operator_name(type(tree_model)), tree_model
        )
        regressor_op.raw_params = operator.raw_params
        regressor_op.inputs = operator.inputs
        regressor_output = scope.declare_local_variable(
            "regressor_prediction", FloatTensorType()
        )
    ...
    container.add_initializer(
        ...
        [len(op.treeWeights), 1],
        op.treeWeights,
    )
    concatenated_predictions = scope.get_unique_variable_name(
        "concatenated_predictions_tensor"
    )
    apply_concat(
        scope, regressor_output_names, concatenated_predictions, container, axis=1
    )
    apply_matmul(
        scope, [concatenated_predictions, weights_tensor], targets_tensor, container
    )

    # this is to calculate prediction and probability
    # given the raw_prediction (= [-target, target])
    ...
    apply_concat(
        scope,
        [targets_neg_tensor, targets_tensor],
        raw_prediction_tensor,
        container,
        axis=1,
    )
    if isinstance(op, GBTClassificationModel):
        # this section is only for the classifier;
        # for the regressor we don't calculate the probability
        ...
        apply_mul(
            scope, [raw_prediction_tensor, minus_two], mul_output_tensor, container
        )
...
register_converter(
    "pyspark.ml.classification.GBTClassificationModel", convert_gbt_classifier
)
...
register_shape_calculator(
    "pyspark.ml.regression.GBTRegressionModel", calculate_gbt_classifier_output_shapes
)
