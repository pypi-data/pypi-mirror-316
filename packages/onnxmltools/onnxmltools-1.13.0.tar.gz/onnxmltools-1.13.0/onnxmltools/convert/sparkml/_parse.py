...
def _get_variable_for_input(scope, input_name, global_inputs, output_dict):
    ...
    The variable is either supplied as graph/global
    nputs or has been generated as output by previous ops
...
def _parse_sparkml(spark, scope, model, global_inputs, output_dict):
    ...
    if isinstance(model, PipelineModel):
        ...
        return _parse_sparkml_simple_model(
            spark, scope, model, global_inputs, output_dict
        )
...
def parse_sparkml(
    ...
    # Put spark-ml object into an abstract container
    # so that our framework can work seamlessly on models created
    # with different machine learning tools.
    raw_model_container = SparkmlModelContainer(model)

    # Declare a computational graph. It will become
    # a representation of the input spark-ml model after parsing.
    topology = Topology(
        raw_model_container,
    ...
    # Declare an object to provide variables' and operators'
    # naming mechanism. In contrast to CoreML, one global scope
    # is enough for parsing spark-ml models.
    scope = topology.declare_scope("__root__")

    # Declare input variables. They should be the inputs of the
    # spark-ml model you want to convert into ONNX
    inputs = []
    ...
    # THe object raw_model_container is a part of the topology
    # we're going to return. We use it to store the outputs of
    # the spark-ml's computational graph.
