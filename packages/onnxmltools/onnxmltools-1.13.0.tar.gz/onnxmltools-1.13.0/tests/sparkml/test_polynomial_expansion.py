...
class TestSparkmlPolynomialExpansion(SparkMlTestCase):
    ...
    def test_model_polynomial_expansion(self):
        ...
        expected = (
            predicted.toPandas()
            .expanded.apply(lambda x: pandas.Series(x.toArray()))
            .values.astype(numpy.float32)
        )
        data_np = (
            data.toPandas()
            .dense.apply(lambda x: pandas.Series(x.toArray()))
            .values.astype(numpy.float32)
        )
        paths = save_data_models(
            data_np, expected, model, model_onnx, basename="SparkmlPolynomialExpansion"
        )
