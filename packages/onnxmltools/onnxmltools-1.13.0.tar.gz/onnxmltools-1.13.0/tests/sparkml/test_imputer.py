...
## For some reason during the spark bring up and shutdown
## something happens causing Imputer
...
# 'setCallSite' on model.surrogateDF
## Therefore we leave these tests out for now until a newere
## version of pyspark is availabe that address this issue
class TestSparkmlImputer(SparkMlTestCase):
    ...
    def _imputer_test_multi(self):
        ...
        model_onnx = convert_sparkml(
            model,
            "Sparkml Imputer Multi Input",
            [("a", FloatTensorType([None, 1])), ("b", FloatTensorType([None, 1]))],
        ...
        paths = save_data_models(
            data_np, expected, model, model_onnx, basename="SparkmlImputerMulti"
        )
    ...
    def _imputer_test_single(self):
        ...
        paths = save_data_models(
            data_np, expected, model, model_onnx, basename="SparkmlImputerSingle"
        )
        onnx_model_path = paths[-1]
