...
class LLMUrlConnector:
    ...
    def get_llm_prediction(self, prompt_list: list) -> List[str]:
        ...
        for index in range(0, len(prompt_list), self.llm_api_batch_size):
            ...
            # if prompt_batch is not None and isinstance(prompt_batch, list):
            #     prompt_text = [message_prompt[0] for message_prompt in prompt_batch]

            # prompt_batch = prompt_text if prompt_text is not None else prompt_batch
