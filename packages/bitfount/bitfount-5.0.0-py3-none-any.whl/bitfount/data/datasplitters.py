...
class DatasetSplitter(ABC):
    ...
    def _handle_selected_file_names_iter_data_split(
        ...
        """
        # If we're not dealing with a FileSystemIterableSource then just return the
        # kwargs unchanged.
        if not isinstance(datasource, FileSystemIterableSource):
            return original_kwargs

        # If there's already a data_keys entry in the kwargs, don't change it
        if "data_keys" in original_kwargs:
            return original_kwargs

        # Otherwise, check if the datasource is just using the normal file_names or
        # will have a different set selected
        if not datasource.selected_file_names_differ:
            return original_kwargs

        original_kwargs["data_keys"] = datasource.selected_file_names
        return original_kwargs
...
class _InferenceSplitter(DatasetSplitter):
    ...
    def get_filenames(
        ...
        if split == DataSplit.TEST:
            return datasource.selected_file_names
        else:
            return []
...
class SplitterDefinedInData(DatasetSplitter):
    ...
    def iter_dataset_split_indices(
        ...
        for i, data in enumerate(datasource):
            if data[self.column_name] == label_to_use:
                yield i
    ...
    def iter_filenames(
        ...
        for chunk in datasource.yield_data():
            ...
            for filename in filenames:
                if filename in ds_selected_file_names:
                    yield filename
