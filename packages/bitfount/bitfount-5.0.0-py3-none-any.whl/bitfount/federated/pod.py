...
from bitfount.federated.schema_management import (
    SchemaManagement,
    _get_minimal_datasource_config,
    _register_dataset,
)
...
class Pod:
    ...
    def _setup_schema(
            ...
            if force_stypes is not None:
                ...
                for stype, cols in force_stypes.items():
                    if stype == "image_prefix":
                        ...
                        # the schem every time for file iterable datasets.
                        continue
        ...
        if schema_needs_regeneration:
            ...
            if (
                ...
                cls._add_one_batch_to_schema(schema=schema, datasource=datasource)
                datasource.remove_hook(hook)
        else:
    ...
    @staticmethod
    def _add_one_batch_to_schema(
        schema: BitfountSchema, datasource: BaseSource
    ) -> BitfountSchema:
        ...
        try:
            ...
            if not data.empty:
                ...
                logger.info(
                    ...
                    f"schema generation. Found {schema.number_of_records} "
                    "records."
                )
            else:
                raise StopIteration()
        except StopIteration:
            logger.warning("Pod has no data.")
    ...
    def _get_public_metadata(
        cls, datasource_container: DatasourceContainer, pod_dp: bool
    ) -> PodPublicMetadata:
    ...
    async def _create_and_run_worker(self, message: _BitfountMessage) -> None:
        ...
        # Patch in default values for older task request message types
        ...
        # Suspend the schema generation flow until the task is over
        flow_run_id = await SchemaManagement.suspend_active_flow("schema-manager")
        ...
        for datasource_name, datasource_container in self.base_datasources.items():
            if datasource_container.schema.schema_type == "partial":
                try:
                    schema = get_pod_schema(
                        pod_identifier=datasource_name,
                        hub=self._hub,
                    )
                    datasource_container.schema = schema
                    self.base_datasources[datasource_name] = datasource_container
                except Exception as ex:
                    logger.error(
        ...
        (
            worker_datasource_name,
            worker_datasource,
            worker_schema,
            worker_data_splitter,
            worker_data_config,
        ) = self._get_target_datasource(message)
        # Check if the datasource is compatible with the task request
        datasource_is_compatible = self._check_datasource_compatibility(
            task_request_message, type(worker_datasource).__name__
        )
        schema_requirements = (
        ...
        if datasource_is_compatible:
            if schema_requirements != worker_schema.schema_type:
                # TODO: [BIT-4655] Allow schemaless tasks
                if schema_requirements == "full":
                    logger.info(
                        f"Task from '{message.sender}' requires full schema. "
                        f"Generating full schema for '{worker_datasource_name}'."
                    )
                    force_stypes, _, _, ignore_cols = _schema_config_migration(
                        worker_data_config, worker_datasource_name
                    )
                    worker_schema.generate_full_schema(
                        datasource=worker_datasource,
                        force_stypes=force_stypes,
                        ignore_cols=ignore_cols,
                    )
                elif schema_requirements == "partial":
                    logger.info(
                        f"Task from '{message.sender}' requires partial schema. "
                        f"Generating partial schema for '{worker_datasource_name}'."
                    )
                    self._add_one_batch_to_schema(worker_schema, worker_datasource)
        # Check that data is not empty
        empty_data = False
        if (
            self.get_schema_number_of_records(worker_schema) == 0
            and schema_requirements != "empty"
        ):
            empty_data = True
        ...
        # there is at least one datapoint.
            ...
            data = next(worker_datasource.yield_data(partition_size=1))
            if data.empty:
                empty_data = True
        if empty_data is False and datasource_is_compatible:
            worker = _Worker(
                datasource=worker_datasource,
                datasource_name=worker_datasource_name,
                ...
                data_splitter=worker_data_splitter,
            ...
            # If interacting with an older modeller version then task_id
            # won't be supplied
            task_monitor_cm: AbstractContextManager
            ...
            with task_monitor_cm:
                ...
                for hook in get_hooks(HookType.POD):
                    hook.on_task_start(
                        ...
                        task_id=worker_mailbox.task_id,
                        project_id=project_id,
                        modeller_username=message.sender,
                    )
                ...
                try:
                    ...
                    if datasource_container_ := self.base_datasources.get(
                        worker_datasource_name
                    ):
                        logger.debug(f"Updating schema for {worker_datasource_name}")
                        datasource_container_.schema = worker_schema
                        self._register_schema(datasource_container_)

                # Run post-task hooks
                logger.debug("Running post-task hooks.")
                for hook in get_hooks(HookType.POD):
                    hook.on_task_end(self, task_id=worker_mailbox.task_id)
        elif empty_data is True:
            msg = (
                f"Task from '{message.sender}' rejected as the datasource"
                f" '{worker_datasource_name}' has no data."
            )
            logger.info(msg)

            error = _PodResponseMessage(message.sender, target_identifier)
            error.add(_PodResponseType.NO_DATA)
            await worker_mailbox.reject_task(error.messages)
        elif not datasource_is_compatible:
            msg = (
                f"Task from '{message.sender}' rejected as the datasource"
                f" '{worker_datasource_name}' is not compatible with the task."
            )
            logger.info(msg)

            error = _PodResponseMessage(message.sender, target_identifier)
            error.add(_PodResponseType.INCOMPATIBLE_DATASOURCE)
            logger.error(type(worker_datasource).__name__)
            await worker_mailbox.reject_task(error.messages)
        logger.info("Ready for next task...")

        # Resume the schema generation flow if it was suspended
        await SchemaManagement.resume_suspended_flow(
            flow_run_id, list(self.base_datasources.values())
        )
    ...
    def _check_datasource_compatibility(
        self, task_request_message: _TaskRequestMessage, datasource_type: str
    ) -> bool:
        """Check the compatibility of the datasource type.
        ...
        Returns:
            True if the datasource type is compatible, False otherwise.
        """
        algorithms = task_request_message.serialized_protocol.get("algorithm")
        if algorithms is None:
            algorithms = []
        ...
        for algo in algorithms:
            ...
            if compatible_ds and datasource_type in compatible_ds:
                return True  # Return early if compatibility is found
        return False

    def _register_schema(self, datasource_container: DatasourceContainer) -> None:
        ...
        try:
            public_metadata = self._get_public_metadata(
                datasource_container, bool(self._pod_dp)
            )
            hub_upload_config = MinimalSchemaUploadConfig(
                public_metadata=public_metadata,
                pod_public_key=self.pod_public_key,
    ...
    async def schema_manager(self) -> None:
        ...
        # Iterate through all datasources and submit schema generation tasks
        for datasource_name, datasource_container in self.base_datasources.items():
            if (
                datasource_container.schema.schema_type is not None
                and datasource_container.schema.schema_type != "full"
            ):
                _logger.info(f"Submitting schema generation task for {datasource_name}")
                datasource_config = _get_minimal_datasource_config(datasource_container)
                force_stypes, column_descriptions, description, ignore_cols = (
                    _schema_config_migration(
                        datasource_container.data_config, datasource_name
                    )
                )
                ...
                task = SchemaManagement.schema_worker.submit(
        ...
        # Wait for tasks to complete or to acknowledge cancellation and finish
        total_num_tasks_done = 0
        while True:
            done, not_done = cast(
                tuple[set[PrefectFuture], set[PrefectFuture]], wait(tasks, timeout=3)
            )
            total_num_tasks_done += len(done)
            _logger.info(f"Schema generation tasks done: {total_num_tasks_done}")
            tasks = list(not_done)

            # Update schema in local dictionary for datasources with completed tasks
            for task in done:
                try:
                    datasource_name, schema_str = cast(tuple[str, str], task.result())
                    schema = BitfountSchema.loads(schema_str)
                    self.base_datasources[datasource_name].schema = schema
                except Exception as e:
                    _logger.error(f"Schema generation task failed: {e}")

            await asyncio.sleep(5)  # Relinquish event loop to other async tasks
            if not tasks:
                _logger.info("All schema generation tasks completed")
                # Intentionally not breaking here to keep the flow running indefinitely
    ...
    async def start_async(self) -> None:
        ...
        try:
            ...
            if self._prefect_unavailable:
                await self._mailbox.listen_indefinitely()
            else:
                await asyncio.gather(
                    self._mailbox.listen_indefinitely(),
                    self.schema_manager(),
                )
    ...
    @staticmethod
    def get_schema_number_of_records(schema: Optional[BitfountSchema]) -> int:
        """Returns the number of records used for schema generation."""
        if schema:
            return schema.number_of_records
        else:
            return 0
...
def _schema_config_migration(
    ...
    Optional[MutableMapping[Union[_ForceStypeValue, _SemanticTypeValue], list[str]]],
