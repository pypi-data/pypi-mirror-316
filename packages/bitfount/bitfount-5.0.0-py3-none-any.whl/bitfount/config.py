...
class Settings(BaseSettings):
    ...
    fast_load_maximum_number_of_files_to_load: SkipJsonSchema[int] = Field(
        default=50, gt=0
    )
...
def _get_compatible_yaml_versions() -> Optional[list[str]]:
    ...
    if os.path.exists(settings.paths.plugin_path / "__version__.py"):
        spec = util.spec_from_file_location(
            "__version__", settings.paths.plugin_path / "__version__.py"
        )
        spec = cast(
            ModuleSpec, spec
