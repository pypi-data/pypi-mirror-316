...
class _ModellerMailbox(_BaseMailbox):
    ...
    def _setup_task_abort_handler(self) -> None:
        ...
        async def task_abort_handler(message: _BitfountMessage) -> None:
            """Abort the task."""
            task_abort_message_body: Optional[str] = self._decrypt_message(message).body

            error_message = f"Received TASK_ABORT message from {message.sender}"
            if task_abort_message_body is not None:
                error_message = f"{error_message}: {task_abort_message_body}"

            logger.info(error_message)
            raise TaskAbortError(error_message)
