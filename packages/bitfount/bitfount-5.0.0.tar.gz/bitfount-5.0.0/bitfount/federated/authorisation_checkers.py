...
from typing import TYPE_CHECKING, Final, List, Optional, Protocol, TypeVar, Union
...
class _AuthorisationChecker(Protocol[_M_co]):
    ...
    def _additional_messages_for_limits(
        ...
        if (
            access_check_result["limits"] is not None
            and access_check_result["response_type"] == _PodResponseType.NO_ACCESS
        ):
            additional_messages = []
            for resource in access_check_result["limits"]:
                try:
                    if resource["totalUsage"] >= resource["limit"]:
