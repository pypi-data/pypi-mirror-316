...
class _WorkerSide(BaseWorkerAlgorithm):
    ...
    def run(
        ...
        if len(combined_test_data_df) != len(fovea_predictions):
            raise ValueError(
                f"Number of fovea predictions ({len(fovea_predictions)})"
                f" does not match number of test rows ({len(combined_test_data_df)})."
            )
        ...
        for (
            ...
            original_filename,
        ), bscan_predictions, fovea_prediction in zip(
            combined_test_data_df.itertuples(index=False),
            predictions.itertuples(index=False),
            fovea_predictions.itertuples(index=False),
        ):
            ...
            try:
                ...
                fovea_centre_coordinates = self._get_central_slice_landmark_point(
                    fovea_prediction, landmark_idx=self.fovea_landmark_idx
                )
                ...
                if distance_from_fovea_centre is None:
                    logger.warning("Distance from fovea centre is not available."
            ...
            metrics = GAMetricsWithFovea(
                ...
                smallest_lesion_size=float(min(lesion_sizes, default=0.0)),
                largest_lesion_size=float(max(lesion_sizes, default=0.0)),
                ...
                fovea_landmarks=(
                    fovea_prediction[1]
                    if fovea_prediction and len(fovea_prediction) > 1
                    else None
                ),
    ...
    def _get_central_slice_landmark_point(
        self,
        fovea_prediction: tuple,
        central_slice_column_idx: int = 0,
        landmarks_column_idx: int = 1,
        landmark_idx: Optional[int] = FOVEA_CENTRE_LANDMARK_INDEX,
    ) -> Optional[np.ndarray]:
        """Gets the central slice middle point from the fovea preiction.

        Args:
            fovea_prediction: Tuple of predictions for a single B-scan.
            central_slice_column_idx: index of a central slice in the tuple.
            landmarks_column_idx: index of the landmarks in the tuple.
            default value for this model is 3 (start, end, middle).
        ...
        """
        if (
            landmarks_column_idx >= len(fovea_prediction)
            or fovea_prediction[landmarks_column_idx] is None
            or len(fovea_prediction[landmarks_column_idx]) == 0
        ):
            logger.warning("Landmarks missing. Skipping distance calculation.")
            return None

        # Get the landmarks and the central slice from the fovea prediction
        landmarks_list = fovea_prediction[landmarks_column_idx]
        central_slice = fovea_prediction[central_slice_column_idx]
    ...
    def _get_shortest_distance_from_image_centre(
        ...
        return float(min(distances_mm, default=0.0))
...
class GATrialCalculationAlgorithmBronze(BaseNonModelAlgorithmFactory):
