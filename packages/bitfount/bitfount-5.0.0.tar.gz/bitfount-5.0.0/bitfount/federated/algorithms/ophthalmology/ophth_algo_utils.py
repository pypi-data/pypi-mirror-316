...
def _add_partial_filtering_to_df(
    ...
    if len(columns) == 0:
        ...
        for matching_col in columns:
            ...
            if op != operator.eq and op != operator.ne:
                value = float(value)
            # We need different cases for filtering in the "any" and "all" cases
            if filter.how == "any":
                if truth_val is None:
                    truth_val = op(df[matching_col], value)
                else:
                    # If filtering by "any", we need an "or" operation
                    truth_val = truth_val | op(df[matching_col], value)  # type: ignore[unreachable] # Reason: Mypy issue, unit tests confirm that it is indeed reachable # noqa: E501
            elif filter.how == "all":
                if truth_val is None:
                    truth_val = op(df[matching_col], value)
                else:
                    # If filtering by "all", we need an "and" operation
                    truth_val = truth_val & op(df[matching_col], value)

            if drop_filtered_cols:
                df = df.drop(columns=matching_col)
            if add_new_col_for_filter:
                df[f"{matching_col} {filter.operator} {filter.value}"] = op(
                    df[matching_col], value
                )
