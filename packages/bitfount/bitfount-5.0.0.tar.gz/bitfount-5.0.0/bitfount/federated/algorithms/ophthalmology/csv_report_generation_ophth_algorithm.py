...
from bitfount.utils.pandas_utils import append_dataframe_to_csv, dataframe_iterable_join
...
class _WorkerSide(BaseWorkerAlgorithm):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if self.trial_name is not None:
            self.file_name = f"{trial_name}-prescreening-patients-{datetime.today().strftime('%Y-%m-%d')}"  # noqa: E501
        else:
                ...
                try:
                    ...
                    if isinstance(e, KeyError):
                        logger.warning(
                            ...
                            f"Filtering only on remaining given columns"
                        )
                    else:
                        # if TypeError
                        logger.warning(
                            f"Filter column {col_filter.column} is incompatible "  # noqa: E501
                            f"with operator type {col_filter.operator}. "
    ...
    def _add_filtering_to_matched_df(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.matched_filter is not None:
            for i, col_filter in enumerate(self.matched_filter):
                logger.debug(f"Running filter {i + 1}")
                try:
                    df = _add_partial_filtering_to_df(
                        df,
                        col_filter,
    ...
    def run(
        ...
        Args:
            ...
            final_batch: Whether this is the final batch of data. Only applicable if
                batched execution is enabled.
        ...
        csv_path = self._get_csv_path(task_id)
        ...
        # Check that we have the expected number of results for the number of files
        if filenames:
            assert len(filenames) == len_aux_results_df  # nosec [assert_used]
        ...
        for df in test_data_dfs:
            ...
            this_csv_path = append_dataframe_to_csv(csv_path, csv_df.round(decimals=6))
        ...
        if filenames and isinstance(self.datasource, FileSystemIterableSource):
            # Assert that the number of predictions (aux_results_df) matched the number
            ...
            # through that number
            assert len_aux_results_df == len_test_data_dfs  # nosec [assert_used]
        unique_patient_count = self._get_unique_patients_id_count(csv_path)
        # If we have finished processing everything then we can consider patient
        # matching, if requested
        matched_csv_path = None
        matched_data = False
        if final_batch and self.matcher is not None:
        ...
        if matched_csv_path and self.produce_trial_notes_csv:
            ...
            self._produce_trial_notes_csv(csv_path, task_id)
    ...
    def _produce_matched_csv(
        ...
        if not csv_data.empty:
            all_data = self._match_same_patient_visit(csv_data)
            ...
            if not all_data.empty:
                ...
                # Write the dataframe to CSV, handling appending to existing CSV
                matched_csv_path = append_dataframe_to_csv(
                    matched_csv_path, all_data.round(decimals=6)
                )
    ...
    def _get_unique_patients_id_count(self, csv_path: Path) -> int:
        ...
        try:
            ...
            try:
                ...
                all_data = pd.read_csv(csv_path, index_col=False)
                return len(all_data)
    ...
    def _trial_notes_csv_path(
        ...
        if self.trial_name is not None and eligible_only:
            self.file_name = f"{self.trial_name}-eligible-patients-notes-template"
        elif self.trial_name:
            self.file_name = f"{self.trial_name}-patients-notes-template"
        else:
            self.file_name = "bitfount-trial-notes-template"
        return self._get_unique_csv_path(self.path, self.file_name, task_id)
    ...
    def _produce_trial_notes_csv(
        ...
        if self.trial_notes_csv_args is None:
            ...
            if len(df) > 0:
                ...
                # Make sure that NaNs are replaced by empty strings
                df.fillna("", inplace=True)
                # Write the dataframe to CSV, handling appending to existing CSV
                trial_notes_csv_path = append_dataframe_to_csv(
                    trial_notes_csv_path, df.round(decimals=6)
                )
    ...
    def _get_csv_path(self, task_id: Optional[str]) -> Path:
        """Get the path to save the CSV output to.
        ...
        """
        if self.trial_name is not None:
            self.file_name = f"{self.trial_name}-prescreening-patients-{datetime.today().strftime('%Y-%m-%d')}"  # noqa: E501
        else:
            self.file_name = "results"
        return self._get_unique_csv_path(self.path, self.file_name, task_id)
    ...
    def _get_unique_csv_path(
        ...
        # If we have task_id, then we put it in that subfolder
...
class CSVReportGeneratorOphthalmologyAlgorithm(BaseNonModelAlgorithmFactory):
    ...
    Args:
        ...
        matched_csv_path: Path to save the matched patients CSV to, if requested.
            Defaults to `save_path` (i.e. overwrites the non-matched CSV) if
            `produce_matched_only` is True. Otherwise, will create a file based off of
            the `save_path` argument.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.matched_csv_path: Optional[Path] = (
            Path(matched_csv_path) if matched_csv_path is not None else None
        )
        self.produce_matched_only = produce_matched_only
        self.produce_trial_notes_csv = produce_trial_notes_csv
