...
class ReportJade(FPDF):
    ...
    Args:
        ...
        task_id: The task ID. Defaults to None.
    ...
    def _details_table(self, y: int) -> int:
        ...
        for index in range(len(self.report_info)):
            self._add_cell(
                self.report_info[index][0].title(),
    ...
    def _summary_table(self, y: int, metric: GAMetrics, scan: AltrisScan) -> int:
        ...
        for key in SUMMARY_TABLE_KEYS:
            ...
            if (
                ...
                self._add_cell(
                    GA_DICT_KEYS_TO_LABELS[key],
                    (str(round(float(metric[key]), 3)) + GA_METRICS_UNITS.get(key, '')   # type: ignore[literal-required] # Reason: key is explicitly iterating over SUMMARY_TABLE_KEYS # noqa: E501
                    if metric[key] is not None and not math.isnan(metric[key])           # type: ignore[literal-required] # Reason: key is explicitly iterating over SUMMARY_TABLE_KEYS # noqa: E501
                    else 'Not detected'),
                    x=x,
                    y=y,
                    horizontal_distance=table_horizontal_distance,
