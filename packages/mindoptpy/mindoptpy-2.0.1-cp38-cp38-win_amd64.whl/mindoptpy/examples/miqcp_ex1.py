 ...
 *  -----------
 *
 *  Mixed Integer Quadratically constrained quadratic optimization (row-wise input).
 *
 *  Formulation
 *  -----------
 *  Minimize
...
if __name__ == "__main__":
    ...
    try:
        ...
        model.addConstr(1.0 * x[0] + 1.0 * x[1] + 2.0 * x[2] + 3.0 * x[3] - \
                        0.5 * x[0] * x[0] - 0.5 * x[1] * x[1] - 0.5 * x[2] * x[2] - 0.5 * x[3] * x[3] - 0.5 * x[0] * x[1] >= 1, "c0")
        model.addConstr(1.0 * x[0] - 1.0 * x[2] + 6.0 * x[3] + 0.5 * x[1] * x[1] <= 1, "c1")
