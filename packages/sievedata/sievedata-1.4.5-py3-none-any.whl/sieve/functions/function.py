...
from ..api.utils import ApiClient, make_api_client
...
class IterFuture:
    ...
    def result(self):
        if self._cancelled:
            raise RuntimeError("Future has been cancelled.")
...
class _Function:
    ...
    def push(self, *inputs, upload_obj=None, webhooks=None, **kwinputs):
        ...
        if stream_output:
            future = IterFuture(iter_call)
        else:
            future = SingleFuture(single_call)

        # TODO: return the Job type that acts like a future
        future.job = result.model_dump()
        return future
