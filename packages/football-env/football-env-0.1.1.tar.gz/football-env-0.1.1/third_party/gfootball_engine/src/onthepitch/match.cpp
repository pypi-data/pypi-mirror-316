...
void Match::GetTeamState(SharedInfo *state,
  ...
  for (auto player : players) {
    ...
    if (player->CastHumanoid() != NULL) {
      ...
      // TODO: test
      for (auto command : player->CastHumanoid()->commandQueueTemp)
      {
          info.actions.push_back(CastToAction(command, preAction));
          /*info.directions.push_back({command.desiredDirection.coords[0], command.desiredDirection.coords[1], command.desiredDirection.coords[2]});
          info.powerlist.push_back(command.touchInfo.desiredPower);
          info.speedlist.push_back(command.desiredVelocityFloat);*/
      }
      info.designated_player = player == main_player;
      team.push_back(info);
      player->CastHumanoid()->commandQueueTemp.clear();
    }
  }
  ...
  for (auto controller : controllers)
  {
      if (team_id == 0)
      {
          state->left_controllers[controller_mapping[controller]]
              .direction = std::vector<float>{ controller->GetDirection().GetEnvCoord(0),controller->GetDirection().GetEnvCoord(1),controller->GetDirection().GetEnvCoord(2) };
          state->left_controllers[controller_mapping[controller]]
              .actions.assign(controller->buttons_pressed_, controller->buttons_pressed_ + e_ButtonFunction_Size);
      }
      else
      {
          state->right_controllers[controller_mapping[controller]]
              .direction = std::vector<float>{ controller->GetDirection().GetEnvCoord(0),controller->GetDirection().GetEnvCoord(1),controller->GetDirection().GetEnvCoord(2) };
          state->right_controllers[controller_mapping[controller]]
              .actions.assign(controller->buttons_pressed_, controller->buttons_pressed_ + e_ButtonFunction_Size);
      }
...
int Match::CastToAction(PlayerCommand command, int& preAction)
{
    switch (command.desiredFunctionType)
    {
    case e_FunctionType_Movement:
        {
        if (command.desiredDirection.coords[0] > 0.5f)
        {
            if (command.desiredDirection.coords[1] > 0.5f)
                return action_top_right;
            else if (command.desiredDirection.coords[1] < -0.5f)
                return action_bottom_right;
            else
                return action_right;
        }
        else if (command.desiredDirection.coords[0] < -0.5f)
        {
            if (command.desiredDirection.coords[1] > 0.5f)
    ...
    case e_FunctionType_LongPass: return action_long_pass; break;
    case e_FunctionType_HighPass: return action_high_pass; break;
    case e_FunctionType_ShortPass: return action_short_pass; break;
    case e_FunctionType_Shot: return action_shot; break; 
    case e_FunctionType_Sliding: return action_sliding; break;
    default:
        {
        if (preAction != action_sprint && FloatToEnumVelocity(command.desiredVelocityFloat) == e_Velocity_Sprint)
        { preAction = action_sprint;  return action_sprint;}
        if (preAction != action_dribble && FloatToEnumVelocity(command.desiredVelocityFloat) == e_Velocity_Dribble)
        { preAction = action_dribble;  return action_dribble; }
        if (preAction == action_sprint && FloatToEnumVelocity(command.desiredVelocityFloat) != e_Velocity_Sprint)
        { preAction = action_release_sprint;  return action_release_sprint; }
        if (preAction == action_dribble && FloatToEnumVelocity(command.desiredVelocityFloat) != e_Velocity_Dribble)
        { preAction = action_release_dribble;  return action_release_dribble; }
        return action_idle;
        }   
    }
}
