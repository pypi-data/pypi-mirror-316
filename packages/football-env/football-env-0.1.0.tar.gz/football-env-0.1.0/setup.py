...
class CustomBuild(build_ext):
  ...
  def run_windows(self):
    ...
    # boost_path = 'D:/projects/boost_1_78_0/bin.v2/libs/filesystem/build/msvc-14.3/debug/threading-multi/'
    libs = glob.glob(f'{lib_path}*.pyd') + glob.glob(f'{lib_path}*.dll') + glob.glob(f'{lib_path}*.pdb')
    copy_compiled_libs(libs, dest_dir)
    copy_fonts(dest_dir)
