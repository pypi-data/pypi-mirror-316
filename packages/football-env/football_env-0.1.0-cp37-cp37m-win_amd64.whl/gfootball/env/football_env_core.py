...
class FootballEnvCore(object):
  ...
  def _retrieve_observation(self):
    ...
    for i in range(11):
      result['left_actions'].append(np.array(info.left_team[i].actions, dtype=np.uint8))
      result['right_actions'].append(np.array(info.right_team[i].actions, dtype=np.uint8))
    with open("D:\\projects\\test\\football\\gfootball\\log.txt", "a") as file:
        for i in range(11):
          file.write(f"player{i}: ")
          file.write(str(result['left_actions'][i]))
          file.write("\n")
        file.write("\n")
