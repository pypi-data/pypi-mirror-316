...
*/
  ...
  for (unsigned int time_ms = startTime_ms; time_ms < ballPredictionSize_ms;
    ...
    if (timeNeeded_ms <= time_ms) {
      ...
      < sprintVelocity * 2.0f) { DO_VALIDATION; if (dot >= 0.4f) forced = true;
    ...
    if (forced) {
      ...
      if (bestChoice.time_ms == -10) {
        DO_VALIDATION;  // this happens if timeNeeded_ms > time_ms
        bestChoice.time_ms = time_ms;
...
unsigned int AI_GetBallControlMovement(
  ...
  if (bestVelocityFloat < dribbleVelocity) {
    DO_VALIDATION;  // don't quantize low velos
...
/**
 * @brief Get the closest players from the given team, considering certain conditions.
 *
 * This function retrieves the closest players from the specified team based on a given position and certain conditions. It returns a vector of Player* objects representing the closest players.
 *
 * @param team The team from which to retrieve the closest players.
 * @param position The position to consider when determining the closest players.
 * @param onlyAIControlled If true, only consider players that are controlled by AI.
 * @param result The vector to store the closest players.
 * @param playerCount The maximum number of players to retrieve.
 * @param onlySelectable If true, only consider players that are selectable.
 *
 * @return A vector of Player* objects representing the closest players based on the given conditions.
 */
  ...
  if (designated->GetTeam() != team) {
    ...
    for (unsigned int i = 0; i < teamPlayers.size(); i++) {
      ...
      if (mateGoalDist < oppGoalDist + tooLateDistance +
        ...
        DO_VALIDATION;  // doesn't matter much when opp is still far away from
                        // goal
        bestPlayerIndex = i;
        break;
      }
...
void AI_GetAutoPass(e_FunctionType passType, const Vector3 &vector,
...
void AI_GetPass(Player *player, e_FunctionType passType,
