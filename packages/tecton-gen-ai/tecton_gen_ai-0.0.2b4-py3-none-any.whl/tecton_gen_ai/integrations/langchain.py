...
def _parse_dict(data: Dict[str, Any]) -> Tuple[Type, Dict]:
    ...
    if "request_timeout" not in conf:
        conf["request_timeout"] = Configs.get_default().get_timeout_sec(None)
    if provider == "openai":
...
class _LangChainAgent(_IntegratedAgent):
        ...
        if len(tools) > 0:
            ...
            def parse(message: Dict[str, Any]) -> Any:
                ...
                if issubclass(output_schema, BaseModel):
                    ...
                    js = convert_to_openai_function(output_schema)
            ...
            return AgentExecutor(agent=agent, tools=tools, **kwargs) | parse
        else:
            if issubclass(output_schema, BaseModel):
    ...
    def run_emitters(self, input: Dict[str, Any], **kwargs: Any) -> None:
        ...
        if len(emitters_no_history) > 0:
            ...
            executor = AgentExecutor(agent=agent, tools=emitters_no_history, **kwargs, verbose=True)
            executor.invoke({"input": message})
        if len(emitters_with_history) > 0:
            agent = create_tool_calling_agent(
                self.llm, emitters_with_history, prompt=prompt
            )
            executor = AgentExecutor(agent=agent, tools=emitters_with_history, **kwargs, verbose=True)
