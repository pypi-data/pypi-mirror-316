...
from tecton_gen_ai.agent._tool_wrappers import FeatureViewTool, FunctionTool, SearchTool
from tecton_gen_ai.constants import _DEV_MODE_AGENT_KEY
from tecton_gen_ai.tecton_utils._internal import get_local_source_attrs, is_local_source
from tecton_gen_ai.utils._serialization import to_openai_function
from tecton_gen_ai.utils.runtime import (
...
class AgentTool(ToolWrapper):
    subtype: str = "agent"

    def __init__(self, agent: AgentBase, use_when: Optional[Callable] = None):
        if not isinstance(agent, Agent):
            raise ValueError("You can only use a locally defined agent")
        if agent.description is None or agent.description == "":
            raise ValueError("Agent must have a description")
        self.agent = agent
    ...
    @classmethod
    def invoke(cls, client: AgentBase, name: str, kwargs: Dict[str, Any]) -> Any:
        meta = client.metastore["tools"][name]
        ctx = client._get_context()
        ctx.update(kwargs)
        query = ctx.pop("query", None)
        ctx.pop("chat_history", None)  # chat_history is not used in agent as tool
        return client._invoke_entrypoint(
            query,
            context=ctx,
...
class Agent(AgentBase):
    ...
    def __init__(
        ...
        input_schema: Union[str, Type[AgentInputModel]] = str,
        output_schema: Union[str, Type[BaseModel]] = str,
        llm: Any = None,
        ...
        self.input_schema = AgentInputModel if input_schema is str else input_schema
        ...
        if tools:
            for _tool in tools:
                if isinstance(_tool, ToolWrapper):
                    ...
                    self._add_tool(AgentTool(_tool))
                elif callable(_tool):
    ...
    def _make_entrypoint(self) -> RealtimeFeatureView:
            ...
            def entrypoint(request_context, context) -> str:
                ...
                try:
                    ...
                    with runtime_context(
                        ...
                        from tecton_gen_ai.utils._serialization import (
    ...
    def _make_metastore(self) -> FeatureView:
        ...
        with set_serialization():
            ...
            def metastore(request_context):
                ...
                from tecton_gen_ai.constants import _OUTPUT_JSON_FIELD

                return {_OUTPUT_JSON_FIELD: json.dumps({"result": _metastore})}

            return metastore

    def _run(self, name: str) -> List[FeatureService]:
