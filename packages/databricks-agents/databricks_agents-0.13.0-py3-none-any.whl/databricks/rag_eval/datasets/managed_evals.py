...
def add_evals(
    ...
    *,
    ...
    for row in evals:
        ...
        if (
            isinstance(tags, str)
            or not isinstance(tags, Iterable)
...
def tag_evals(
    ...
    *,
    ...
    eval_tags = [
        entities.EvalTag(eval_id=eval_id, tag_id=tag_id)
        for eval_id in eval_ids
        for tag_id in tag_ids
    ]
    _get_managed_evals_client().batch_create_eval_tags(
        instance_id=evals_table_name, eval_tags=eval_tags
    )
...
def untag_evals(
    ...
    *,
    ...
    existing_tag_ids = [
        tag_name_to_id[tag_name] for tag_name in tag_names if tag_name in tag_name_to_id
    ]
    ...
    _get_managed_evals_client().batch_delete_eval_tags(
        instance_id=evals_table_name, eval_tags=eval_tags
    )
...
@context.eval_context
def get_ui_links(
    evals_table_name: str, *, display_html: bool = True
) -> List[Dict[str, str]]:
    """
    ...
    workspace_url = workspace_url_resolver.WorkspaceUrlResolver(
...
def grant_access(
    ...
    *,
    ...
    agent_serving_endpoint = instance.agent_serving_endpoint
    endpoint_id = _get_updatable_endpoint_id_from_endpoint_name(
        deploy_client, agent_serving_endpoint
    )
    if endpoint_id is not None:
        try:
            _update_permissions_on_endpoint(
                endpoint_id, user_emails, ServingEndpointPermissionLevel.CAN_QUERY
            )
        except ValueError as e:
            _logger.warning(
                f"Could not update permissions for `{agent_serving_endpoint}`: {e!r}"
            )
    try:
        _get_managed_evals_client().update_eval_permissions(
            instance_id=evals_table_name, add_emails=user_emails
        )
    except Exception:
        # Tolerate failures; backend isn't ready yet.
        pass
    ...
    *,
    ...
    for suffix in [
        ...
        workspace_client.grants.update(
            ...
            full_name=table_name,
            changes=[
                catalog.PermissionsChange(principal=user, remove=remove)
    ...
    try:
        _get_managed_evals_client().update_eval_permissions(
            instance_id=evals_table_name, remove_emails=user_emails
        )
    except Exception:
        # Tolerate failures; backend isn't ready yet.
        pass
...
def _get_updatable_endpoint_id_from_endpoint_name(
    deploy_client, agent_serving_endpoint
) -> Optional[str]:
    """
    ...
    if endpoint.get("endpoint_type") == "FOUNDATION_MODEL_API":
        _logger.info(
            f"Foundation model endpoint found: `{agent_serving_endpoint}`; permission update not needed."
        )
        return None
    return endpoint["id"]
