...
def batch_by_size_limit(items: List[Any], size_limit_bytes: int) -> Iterator[List[Any]]:
    ...
    for item in items:
        ...
        if item_size > size_limit_bytes:
            raise ValueError(
                f"Item {repr(item)[:200]}... ({item_size} bytes) exceeds Databricks HTTP request size limit ({size_limit_bytes} bytes)."
            )
        if batch_size + item_size > size_limit_bytes:
            yield batch
            batch = []
            batch_size = 0
        batch.append(item)
