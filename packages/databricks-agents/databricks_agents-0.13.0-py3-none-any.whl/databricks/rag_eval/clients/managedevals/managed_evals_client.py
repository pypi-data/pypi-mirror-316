...
from databricks.rag_eval.utils import NO_CHANGE, collection_utils, input_output_utils
...
_REQUEST_SIZE_LIMIT = 1_000_000  # Technically 1MB = 1048576 bytes, but we leave 48kB for overhead of HTTP headers/other json fluff.
...
class ManagedEvalsClient(databricks_api_client.DatabricksAPIClient):
    ...
    def add_evals(
        ...
        *,
        ...
        with self.get_default_request_session(
            ...
            for eval_batch in collection_utils.batch_by_size_limit(
                evals, size_limit_bytes=_REQUEST_SIZE_LIMIT
            ):
                request_body = {"evals": eval_batch}
                ...
                _raise_for_status(response)
                eval_ids.extend(response.json().get("eval_ids", []))
        return eval_ids
    ...
    def batch_create_tags(
        ...
        *,
        ...
        _raise_for_status(response)
        return response.json().get("tag_ids", [])
    ...
    def batch_create_eval_tags(
        ...
        *,
        ...
        """
        request_body = {
            "eval_tags": [et.to_json() for et in eval_tags],
        }
        with self.get_default_request_session(
            headers=_get_default_headers()
        ) as session:
