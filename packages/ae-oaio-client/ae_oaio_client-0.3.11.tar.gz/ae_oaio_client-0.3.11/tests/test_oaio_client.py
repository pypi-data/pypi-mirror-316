...
from ae.base import (
    TESTS_FOLDER, app_name_guess, load_dotenvs, os_host_name, os_path_abspath, os_path_dirname, os_path_isdir,
...
from ae.oaio_model import (
    CREATE_ACCESS_RIGHT, DELETE_ACTION, DOWNLOAD_ACTION, FILES_DIR, FILES_VALUES_KEY, HTTP_HEADER_USR_ID, OBJECTS_DIR,
    REGISTER_ACTION,
    ROOT_VALUES_KEY, UPDATE_ACCESS_RIGHT, OaiObject, UPLOAD_ACTION, now_stamp, object_dict)
...
class TestInstantiation:
    ...
    def test_args(self):
        ...
        try:
            ocl = OaioClient("hOsT", cre_args, app_id='app_id', device_id='dev_id', csh_default_id='csh_id',
                             auto_sync=369.12, client_root=loc_path)
    ...
    def test_args_defaults(self):
        ...
        assert ocl.device_id == os_host_name()
        assert ocl.csh_default_id == 'Digi'
        assert ocl.client_root == normalize("{ado}/oaio_root/")
...
other_user = 'tst_oth_usr'      # other user, for subscription sharing tests


def _csh_file_content(client_obj: OaioClient, oai_obj: OaiObject, file_count: int = 1) -> dict[str, bytes]:
    """ download content of attached file(s) from mocked or live cloud storage host. """
...
class CshMockApi(CshApiBase):
    ...
    def delete_file_or_folder(self, file_path: str) -> str:
        ...
        if found and len(left_parts := file_path.split('/')) > 2:
...
def _mocked_get_or_post_success_response(url: str, **kwargs) -> requests.Response:
    ...
    if act_slug == 'current_stamp':
        ...
        mock_res.json.return_value = {'message': f"mocked success for url {url} and kwargs={kwargs}"}
...
def _mocked_get_or_post_failure_response(url: str, **kwargs) -> requests.Response:
    ...
    mock_res.json.return_value = {'e_r_r': f"mocked error for url {url} and kwargs={kwargs}"}
    return mock_res
...
def user_client(usr_name: str):
    ...
    used as fixture via yield from for username parameterized unit tests.
    ...
    with patch('ae.oaio_client.requests') as mock_req:
        ...
        root_path = os_path_join(TESTS_FOLDER, f'tst_rut_{usr_name}')
        ocl = OaioClient('any_host:8769', {'username': usr_name, 'password': 'any_password'},
                         ...
                         auto_sync=0,  # disable auto sync for testing
                         client_root=root_path,
                         )
        assert ocl.error_message == ""
        assert os_path_isdir(root_path)
...
class TestOaioClientWithMockedServers:
    ...
    def test_register_file(self, mocked_client):
        ...
        assert mocked_client.synchronize_with_server_if_online()
    ...
    def test_synchronize_with_server_if_online(self, mocked_client):
        assert mocked_client.synchronize_with_server_if_online() is True
    ...
    def test_synchronize_with_server_if_online_with_2nd_usr(self, mocked_client, other_client):
        ...
        assert oai_obj and mocked_client.synchronize_with_server_if_online()
        oaio_id = oai_obj.oaio_id
        ...
        assert other_client.synchronize_with_server_if_online()
        upd_stamp = now_stamp()
        ...
        assert other_client.synchronize_with_server_if_online()   # 2nd user uploading updated values to server
        ...
        assert mocked_client.synchronize_with_server_if_online()  # registering creator get update done by other_client
    ...
    def test_unregister_object_file_wipe_err(self, mocked_client):
        ...
        oai_obj = mocked_client.register_file(reg_name, file_content=b"unregister wipe err test file content")
        assert mocked_client.synchronize_with_server_if_online()
        assert _csh_file_remove(mocked_client, oai_obj) == ""  # force error by removing file from the cloud storage
        assert _csh_mock_cleared_errors() == ""
    ...
    def test_upsert_subscription(self, mocked_client):
        ...
        try:
            ...
            mocked_client.user_name = mocked_client.session.headers[HTTP_HEADER_USR_ID] = other_user
            assert oai_obj.oaio_id in mocked_client.client_objectz

        finally:
            if pid:
                mocked_client.user_name = mocked_client.session.headers[HTTP_HEADER_USR_ID] = oai_obj.username
...
def live_client():
    ...
    client_root = os_path_join(TESTS_FOLDER, 'tst_liv_rud')
    ...
    ocl = OaioClient(os.environ['OAIO_HOST_NAME'],
                     ...
                     auto_sync=0,                       # disable auto sync in all tests
                     client_root=client_root,
                     )

    liv_api = DigiApi(root_folder='tst_liv_rut',        # TEST_DIGI_ROOT_FOLDER_PATH exclusive to test_cloud_storage.py
                      email=os.environ.get('TEST_DIGI_API_EMAIL'),
                      password=os.environ.get('TEST_DIGI_API_PASSWORD'))
    ocl._csh_api = lambda *_args: liv_api               # monkey patch to prevent Cshz test rec in live server database

    assert ocl.error_message == ""
    assert ocl.synchronize_with_server_if_online(), f"Err={ocl.error_message}"
    local_obj_cache_path = os_path_join(ocl.client_root, OBJECTS_DIR)

    # noinspection PyProtectedMember,PyTypeChecker
    assert not os.listdir(local_obj_cache_path) and not ocl._load_server_objectz()  # ensure empty/clean test env
    ...
    # noinspection PyProtectedMember,PyTypeChecker
    assert not os.listdir(local_obj_cache_path) and not ocl._load_server_objectz()  # ensure empty/clean test env
    all_users = [_['username'] for _ in ocl.userz_access('any_existing_or_not_existing_oaio_id')]
    assert other_user not in all_users
...
class TestIntegrationWithLiveServers:
    ...
    def test_connect(self, live_client):
        ...
        assert live_client.synchronize_with_server_if_online(), f"Err={live_client.error_message}"
        assert live_client.connected
    ...
    def test_synchronize_with_server_if_online(self, live_client):
        assert live_client.synchronize_with_server_if_online() is True, f"Err={live_client.error_message}"
    ...
    def test_unregister_object(self, live_client):
        ...
        try:
            ...
            # to allow check the subscription of other user and to mock a request, OAIO_USERNAME has to be supervisor
            live_client.user_name = live_client.session.headers[HTTP_HEADER_USR_ID] = other_user
            assert oai_obj.oaio_id in live_client._load_server_objectz(), f"Err={live_client.error_message}"

        finally:
            if pid:
                live_client.user_name = live_client.session.headers[HTTP_HEADER_USR_ID] = oai_obj.username
                ...
                assert live_client.unregister_object(oai_obj.oaio_id) == ""
                assert oai_obj.oaio_id not in live_client._load_server_objectz(), f"Err={live_client.error_message}"
    ...
    def test_wipe_user(self, live_client):
        ...
        try:
            oai_obj = live_client.register_object({})
            assert live_client.synchronize_with_server_if_online(), f"Err={live_client.error_message}"
            assert live_client.upsert_subscription(oai_obj.oaio_id, other_user)  # create User/Userz recs for other user
