...
def krx():
    ...
    parser_sync.add_argument('-r', '--refresh', action='store_true', help='krx300파일을 다운로드할지')
    parser_sync.add_argument('-p', '--period', type=int, help='krx300을 재다운로드할 날짜기준(몇일전)')
    ...
    if args.command == 'sync':
        ...
        if args.period:
            krx300.make_db(period=args.period, refresh=args.refresh)
        else:
            krx300.make_db(period=30, refresh=args.refresh)
