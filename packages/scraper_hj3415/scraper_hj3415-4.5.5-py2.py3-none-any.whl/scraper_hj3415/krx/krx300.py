...
from scraper_hj3415.krx.tiger import download_krx300, TEMP_DIR, WORKING_DIR
...
def sync_with_mongo():
    ...
    if len(add_difference) == 0 and len(del_difference) == 0:
        ...
        log_mongo = mypeewee.LogManager('mongo')
        for code in del_difference:
            log_mongo.save('INFO', f'{code}/{mymongo.Corps.get_name(code)}를 삭제')
        ...
        if len(add_difference) != 0:
            ...
            for code in add_difference:
                log_mongo.save('INFO', f'{code}/{get_name(code)}을 추가')
