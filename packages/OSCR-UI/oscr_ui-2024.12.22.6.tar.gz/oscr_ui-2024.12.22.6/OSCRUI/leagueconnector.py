...
def download_and_view_combat(self):
    ...
    analyze_log_callback(self, path=file.name, hidden_path=True)
...
def upload_callback(self):
    ...
    """
    try:
        current_combat = self.parser.combats[
            self.current_combats.currentIndex().data()[0]
        ]
    except IndexError:
    ...
    with tempfile.NamedTemporaryFile(
        delete=False, dir=self.config["templog_folder_path"]
    ) as temp:
