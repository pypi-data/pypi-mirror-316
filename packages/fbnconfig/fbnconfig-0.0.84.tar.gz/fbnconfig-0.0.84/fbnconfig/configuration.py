...
class SetResource(BaseModel, Resource):
    ...
    def read(self, client, old_state):
        ...
        code = self.code
        type = old_state.type
        return client.get(f"/configuration/api/sets/{type}/{scope}/{code}").json()
...
class ItemResource(BaseModel, Resource):
    ...
    def read(self, client, old_state):
        ...
        code = old_state.code
        key = old_state.key
        return client.get(f"/configuration/api/sets/{type}/{scope}/{code}/items/{key}").json()
    ...
    def update(self, client, old_state):
        ...
        if self.isSecret != remote["isSecret"] or self.valueType != remote["valueType"]:
            self.delete(client, old_state)
            return self.create(client)
        self.ref = remote["ref"]
