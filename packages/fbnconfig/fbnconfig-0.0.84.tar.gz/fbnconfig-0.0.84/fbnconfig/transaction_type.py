...
from typing import Dict, Any, Optional, List, Union, Self, Sequence
...
from .side_definition import SideRef, SideResource
...
class TransactionTypeCalculation(BaseModel):
    ...
    side: Union[SideRef, SideResource]
    formula: Optional[str] = None

    @field_serializer("side")
    def serialize_side(self, v: Union[SideRef, SideResource]):
        return v.side
...
class PerpetualProperty(BaseModel):
    property_key: Union[DefinitionResource, DefinitionRef] = Field(serialization_alias="propertyKey")
...
class TransactionTypePropertyMapping(BaseModel):
    property_key: Union[DefinitionResource, DefinitionRef] = Field(serialization_alias="propertyKey")
    map_from: Optional[Union[DefinitionResource, DefinitionRef]] = Field(
        serialization_alias="mapFrom", default=None
    )
    set_to: str = Field(serialization_alias="setTo", default=None)

    @field_serializer("property_key", "map_from", when_used="always")
    def serialize_property_key(self, v: Union[DefinitionRef, DefinitionResource]):
        return f"{v.domain.value}/{v.scope}/{v.code}"
...
def compare_json_structures(json1, json2) -> bool:
...
class TransactionTypeMovement(BaseModel):
    movement_types: MovementType = Field(serialization_alias="movementTypes")
    side: SideRef | SideResource
    direction: int
    properties: Optional[List[PerpetualProperty]] = []
    mappings: Optional[List[TransactionTypePropertyMapping]] = []
    name: Optional[str] = None
    movement_options: Optional[List[MovementOption] | str] = Field(
        default=None, serialization_alias="movementOptions"
    )
    settlement_date_override: Optional[str] = Field(
        default=None, serialization_alias="settlementDateOverride"
    )
    condition: Optional[str] = ""

    @field_serializer("side", when_used="always")
    def serialize_side(self, v: SideRef | SideResource):
        return v.side

    @field_serializer("properties", when_used="always")
    def serialize_properties(self, v: List[PerpetualProperty]):
        return serialize_properties(v)
...
class TransactionTypeResource(BaseModel, Resource):
    ...
    source: str = Field(exclude=True, init=True)
    ...
    def update(self, client: httpxClient, old_state) -> Union[None, Dict[str, Any]]:
        ...
        if compare_json_structures(remote, desired):
            return None
    ...
    def delete(client: httpxClient, old_state) -> None:
        transaction_type, scope, source = (
            old_state.transaction_type,
            old_state.scope,
            old_state.source,
        )
        client.delete(
