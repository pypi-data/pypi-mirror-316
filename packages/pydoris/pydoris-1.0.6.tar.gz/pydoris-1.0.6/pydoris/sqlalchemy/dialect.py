...
class DorisDialect(MySQLDialect_mysqldb):
    ...
    def get_schema_names(self, connection, **kw):
        catalog = connection.engine.url.database.split('.')[0]

        rp = connection.exec_driver_sql("SHOW schemas FROM %s"
                                        % self.identifier_preparer.quote_identifier(catalog))  # 获取库名
        return [r[0] for r in rp]

    def get_catalog_names(self, connection, **kw):
        """
        Get all catalogs.
        """
        rp = connection.exec_driver_sql("SHOW CATALOGS")  # 获取库名
        return [r[0] for r in rp]
