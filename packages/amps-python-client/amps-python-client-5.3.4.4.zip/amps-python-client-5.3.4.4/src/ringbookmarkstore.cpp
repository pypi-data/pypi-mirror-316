...
namespace ampspy
{
  namespace ringbookmarkstore
  {
    ...
    static void* __ctor(obj* self, PyObject* args, PyObject* kwds)
    {
      ...
      try
      {
        self->impl = new BookmarkStore(new RingBookmarkStore(filename));
      } DISPATCH_EXCEPTION;
...
//    def __init__(self, name):
    ...
    get_most_recent(obj* self, PyObject* args)
    {
      ...
      CALL_RETURN_FIELD(self->impl->getMostRecent(
    ...
    AMPSDLL ampspy::ampspy_type_object ringbookmarkstore_type;

    void add_types(PyObject* module_)
    {
      ringbookmarkstore_type.setName("AMPS.RingBookmarkStore")
      .setBasicSize(sizeof(obj))
      .setDestructorFunction(_dtor)
      .setConstructorFunction(_ctor)
      .setBaseType()
      .setDoc("AMPS RingBookmarkStore Object")
      .notCopyable()
      .addMethod("log", log,
                 "log(message)\n\n"
      ...
      .addMethod("get_most_recent", get_most_recent,
                 ...
                 "Returns the most recent bookmark from the log that ought to be used\n"
                 "for (re-)subscriptions.\n")
      ...
      .createType()
      .registerType("RingBookmarkStore", module_);
