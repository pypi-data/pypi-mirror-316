...
#define STRINGIFYX(x) STRINGIFY(x)
                                                    ...
                                                    char* hostname_, void* me_)
  {
    ...
    case AF_INET:
    {
      PyTuple_SetItem(args, 0, PyObject_GetAttrString(_ampspy_socket_module, "AF_INET"));
    }
    break;
    case AF_INET6:
    {
      PyTuple_SetItem(args, 0, PyObject_GetAttrString(_ampspy_socket_module, "AF_INET6"));
    }
    break;
    case AF_UNIX:
    {
      PyTuple_SetItem(args, 0, PyObject_GetAttrString(_ampspy_socket_module, "AF_UNIX"));
    }
    break;
    default:
    {
      PyTuple_SetItem(args, 0, PyLong_FromLong((long)pAddrInfo_->ai_family));
    }
    }
    PyObject* kwargs = PyDict_New();
    if (pAddrInfo_->ai_protocol)
    {
      PyDict_SetItemString(kwargs, "proto", PyLong_FromLong(pAddrInfo_->ai_protocol));
    }
    ...
    if (hostname_)
    {
      ...
      pySSLSocket = PyObject_CallMethodObjArgs(_ampspy_ssl_context,
                                               PyString_FromString("wrap_socket"),
    ...
    if (!retVal)
    {
      ...
      // Ignore errors here, just being sure the socket is released
#ifdef _WIN32
      ...
      close(fd_);
#endif
      amps_tcps_set_external_error(me_, "Failed to connect Python SSLSocket");
      return NULL;
    }
    Py_DECREF(retVal);
  ...
  AMPSDLL int _ampspy_close_all(AMPS_SOCKET fd_, _amps_SSL* pSSL_)
  {
    ...
    if (pySSLSocket)
    {
      ...
      PyObject_CallMethodObjArgs(pySSLSocket, PyString_FromString("shutdown"), arg, NULL);
      PyObject_CallMethodObjArgs(pySSLSocket, PyString_FromString("_real_close"), NULL);
