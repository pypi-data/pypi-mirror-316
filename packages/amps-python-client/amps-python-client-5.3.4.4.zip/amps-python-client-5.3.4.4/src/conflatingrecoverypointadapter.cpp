...
//                 update_interval_millis):
    static int _ctor(obj* self, PyObject* args, PyObject* kwds)
    {
      ...
      if (!PyArg_ParseTupleAndKeywords(args, kwds, (char*)"O|idl", (char**)kwlist,
                                       &pPythonDelegate, &updateThreshold,
                                       &timeoutMillis, &updateIntervalMillis))
      {
        return -1;
      }
      if (sowrecoverypointadapter::type.isInstanceOf(pPythonDelegate))
      {
        self->pImpl = new ConflatingRecoveryPointAdapter(
          ((sowrecoverypointadapter::obj*)pPythonDelegate)->pImpl,
      ...
      {
        self->pImpl = new ConflatingRecoveryPointAdapter(
          std::make_shared<recoverypointadapter::wrapper>(pPythonDelegate),
          (unsigned)updateThreshold,
          timeoutMillis,
          updateIntervalMillis);
      }
...
// def purge() or purge(subId)
    ...
    static const char* conflatingrecoverypointadapter_class_doc = "\n"
        " This class can be used as an adapter on an AMPS.MemoryBookmarkStore."
        " It is a pass-through to another adapter type that provides"
        " conflation of updates to help reduce the load on the underlying"
        " adapter. Conflation can be on an interval, a set number of updates"
        " or both.";
    ...
    void add_types(PyObject* module_)
    {
      ...
      .addMethod("update", update,
                 ...
                 "Conflate the new information in recoveryPoint.\n"
                 ":param recovery_point: The new recovery information to save.\n"
                 ":type recovery_point: :ampspy:recoveryPoint\n\n")
