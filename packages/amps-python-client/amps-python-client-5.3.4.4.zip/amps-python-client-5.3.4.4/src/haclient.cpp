...
namespace ampspy
{
  ...
  public:
                         ...
                         const AMPSException& exception_)
    {
      ...
      PyObject* result = PyObject_Call(_impl, Py_BuildValue("(OkO)", message, (unsigned long)requestedAckTypes_, exception), (PyObject*)NULL);
      if (result == NULL)
      {
        ampspy::unhandled_exception();
        return false;
      ...
      exc::throwError();
      return PyObject_IsTrue(result) != 0;
    }
...
//    def __init__(self, name):
    ...
    static PyObject* get_most_recent(obj* self, PyObject* args)
    {
      ...
      HAClient& haClient = *(HAClient*)(AMPS::Client*)(self->_client.pClient);
      CALL_RETURN_STRING(haClient.getBookmarkStore().getMostRecent(subId));
    }
