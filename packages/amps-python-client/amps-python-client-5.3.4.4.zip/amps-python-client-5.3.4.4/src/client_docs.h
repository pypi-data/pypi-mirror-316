...
Connects to the AMPS instance through the provided :class:`URI`.
       ...
       The components of the :class:`URI` are as follows:
...
on construction, then the client will store before forwarding the message to AMPS. If a ``DisconnectException``
...
Send a :class:`Message` to AMPS via the :class:`Transport` used in the :class:`Client`.
...
- ``Client.sow_delete_by_data(topic, data, timeout=0)``
  ...
  1. When a message handler is provided, this method submits the ``sow_delete_by_data`` on a background thread, 
     and calls the message handler with the results of the delete.
  2. When no message handler is provided, this method provides an acknowledgment message with the result of 
     the delete command.
...
:param sub_id: The subscription ID. You may optionally provide a
               ...
               this is the subscription to be replaced.
:type sub_id: str
...
Gets the :class:`URI` string passed into the :class:`Client` during the :meth:`connect` invocation.
...
:param listener_callable: The function or callable to be called. This function will be passed True if a
                          ...
                          The application should not issue commands on the client until recovery is completed.
)docstring";
...
:param transport_filter_callable: The function or callable to be called. This function is passed a string ('data')
                                  containing the raw bytes sent or received. The 'direction' parameter is False if
                                  the data is being sent to the server, or True if data is received from the server.
)docstring";
...
Called to check if the Store will throw :exc:`PublishStoreGapException`.

:returns: If true, :exc:`PublishStoreGapException` can be thrown
