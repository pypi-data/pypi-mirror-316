...
import astroid
from astroid import NameInferenceError, nodes
...
def get_invalid_type_nodes(annotation_node, depth=0, seen=None, unset: Optional[astroid.bases.Instance] = None) -> List[nodes.NodeNG]:
    """
    ...
    if isinstance(annotation_node, astroid.BinOp):
        return get_invalid_type_nodes(annotation_node.left) + get_invalid_type_nodes(annotation_node.right)
    ...
    if inferred_node.qname() in simple_type_checks:
        return []
    if inferred_node.qname() == "typing.Dict":
    ...
    if inferred_node.qname() in ("typing.Optional", "typing.List"):
    ...
    if inferred_node.qname() == "typing.Union":
        ...
        return list(itertools.chain(*[get_invalid_type_nodes(annotation_node, depth + 1, add_seen(inferred_node), unset=unset_instance) for annotation_node in annotation_nodes]))
...
def get_invalid_type_nodes_for_procedure_outputs(annotation_node) -> List[nodes.NodeNG]:
    ...
    if any(get_invalid_type_nodes(annotation_node, depth=0, seen=None)):
        return [annotation_node]
...
def is_list_type(annotation_node) -> bool:
    ...
    if inferred_node.qname() == "typing.List":
        return True
...
def check_connection_required(node: nodes.FunctionDef) -> bool:
    """
    Checks if a function has the @procedure decorator with connection_required=True.
    Args:
        node: The Astroid FunctionDef node to check.

    Returns:
        True if connection_required is True, False otherwise.
    """
