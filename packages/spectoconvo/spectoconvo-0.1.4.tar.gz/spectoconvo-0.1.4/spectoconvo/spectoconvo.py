...
def text_to_image(text, font_paths=["NotoSansSC-Bold.ttf","NotoSansKR-SemiBold.ttf","NotoSansJP-SemiBold.ttf","NotoSans-Regular.ttf"], font_size=20, image_path='message_image.png'):
    """
    ...
    for char in text:
        ...
        for font in font_objects:
            try:
                # Create a copy of the image before drawing
                temp_image = image.copy()
                temp_draw = ImageDraw.Draw(temp_image)
                
                # Try rendering the character with the current font
                temp_draw.text((x, y), char, font=font, fill='black')
                
                # Measure the size of the character
                char_bbox = temp_draw.textbbox((x, y), char, font=font)
                char_width = char_bbox[2] - char_bbox[0]
                char_height = char_bbox[3] - char_bbox[1]
                if char_width == 20 and char_height == 21:
                    raise OSError("Character rendered as a box")
