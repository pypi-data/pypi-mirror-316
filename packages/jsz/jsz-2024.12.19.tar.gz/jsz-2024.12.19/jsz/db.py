...
def dickcache(directory: str = None, timeout: int = 60):
    """
    通过 diskcache 实现缓存

    directory: 缓存目录, 默认值为当前目录
...
def mongodb(host, database=None, port: int | None = None, **kwargs):
    """
...
def duckdb(query="select now()", *, db=None):
    """
    使用 DuckDB 执行 SQL 语句
...
def mongo_create_index(table, index_name, unique=False, background=True):
    ...
    """
    table.create_index(index_name, unique=unique, background=background)
...
def mongo_queue(table, use_existing: bool = True):
    ...
    class MongoQueue:
        ...
        def push(self, item):
            _id = mongo_id()
            self.table.insert_one({"_id": _id, "key": _id, "value": item})
...
def mongo_dict(table):
    ...
    class MongoDict:
        ...
        def __setitem__(self, key, value):
            self.table.update_one({"key": key}, {"$set": {"value": value}}, upsert=True)
