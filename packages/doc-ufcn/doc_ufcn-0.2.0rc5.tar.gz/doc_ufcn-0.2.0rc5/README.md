...
model_path, parameters = models.download_model('Teklia/doc-ufcn-generic-page')
...
```python
model_path, parameters = models.download_model('Teklia/doc-ufcn-generic-page', version="main")
```
...
⚠ Doc-UFCN does not support overlapping objects. If there are overlapping zones in the JSON label files, they will be overwritten with the latest polygon.

Example 1: the text line is defined **before** the paragraph.
...
Page overrides the text line in the final mask image.

<img src="./resources/paragraph_overrides.png" alt="Line is overshadowed by the paragraph." title="Line (in red) is overshadowed by the paragraph (in blue)." width="300x"/>

Example 2: the text line is defined **after** the paragraph.
...
<img src="./resources/line_overrides.png" alt="Line is drawn on the paragraph." title="Line (in red) is drawn on the paragraph (in blue)." width="300x"/>
...
Note that the polygons in the JSON labels should match the expected image input size (*img_size*) to avoid resizing (that can cause mergings of regions).
...
After running the command, the ground truth masks should be in the `labels/' directories.
