...
from mvneureval.relative_validation_multiview  import RelativeValidationMultiview
...
class FindBestClustCVMultiview(RelativeValidationMultiview):
    """
    ...
    def __init__(self, c, s, nrand, nfold=2, n_jobs=1, nclust_range=None):
        """
        Construct method.
        """
        super().__init__(c, s, nrand)
        self.nfold = nfold
    ...
    def best_nclust(self, data, modalities, covariates, iter_cv=1, strat_vect=None):
        """
        ...
        mv_reval = RelativeValidationMultiview(self.clust_method, self.class_method, self.nrand)
        ...
        if self.nclust_range is not None:
            params = list(itertools.product([(modalities, mv_reval, covariates)],
                                            fold_gen, self.nclust_range))
        else:
            params = list(itertools.product([(modalities, mv_reval, covariates)],fold_gen))
        if self.n_jobs > 1:
            p = mp.Pool(processes=self.n_jobs)
            miscl = list(zip(*p.starmap(self._fit_multiview, params)))
            p.close()
            p.join()
            out = list(zip(*miscl))
        else:
            ...
            for p in params:
                if len(p) > 2:
                    miscl.append(self._fit_multiview(data_obj=p[0],
                                                     idxs=p[1],
                                                     ncl=p[2]))
                else:
                    miscl.append(self._fit_multiview(data_obj=p[0],
        ...
        if 0 < ctrl_rows - cv_results_.shape[0] < ctrl_rows:
            ...
            FindBestClustCVMultiview.cv_results_ = cv_results_
    ...
    def _fit_multiview(data_obj, idxs, ncl=None):
        ...
        modalities, mv_reval, covariates = data_obj
        tr_idx, val_idx = idxs
        mv_reval.clust_method.n_clusters = ncl
