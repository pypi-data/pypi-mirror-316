...
class FfmpegFlag:
    ...
    def scale_with_size(size: tuple):
        ...
        """
        return f'-vf scale={size[0]}:{size[1]}'
    ...
    def crop(size: tuple, origin: tuple):
        ...
        """
        return f'-vf "crop={str(size[0])}:{str(size[1])}:{str(origin[0])}:{str(origin[1])}"'
    ...
    def resize_video(video_filename: str, size: tuple, output_filename: Union[str, None] = None):
        ...
        if (w, h) == size:
            ...
            FfmpegHandler.run_command([
                ...
                FfmpegFlag.crop(new_size, top_left),
                output_filename
            ])
