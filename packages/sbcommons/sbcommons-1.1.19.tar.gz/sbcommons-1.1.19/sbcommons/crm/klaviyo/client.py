...
class KlaviyoClient(CrmClient):
    ...
    def bulk_create_events(self, data) -> bool:
        ...
        Args:
            A list of dictionaries where each dictionary has the email along with the events data
            for a user.
                                ...
                                "attributes": {
                                    ...
                                    "metric": {
                                        "data": {
                                            "type": "metric",
                                            "attributes": {
                                                "name": event_properties['event_name']
                                            }
                                        }
                                    }
                                }
        ...
        for chunk in chunks:
            ...
            try:
                ...
                self.logger.exception(f'Failed to create event #{event_count} got exception {e}')

        self.logger.info(f'Created {event_count} events...')
        return True
    ...
    def _start_bulk_import_profile_job(self, format_method: Callable[[Any], dict],
                                       profiles: List[Any], list_id: str = None) -> bool:
        """
        Chunks bulk jobs and posts to klaviyo
        Args:
            format_method: method to format the payload correctly
            profiles: list of profiles to be sent to klaviyo
        ...
        for chunk in chunks:
            ...
            job_id = self._submit_bulk_import_profile_job(
                url=url,
                payload=payload
            )
            jobs.append(job_id)
