  ...
  async def set_user_rights(login: int, rights_to_set=None, rights_to_remove=None, server_id=None, server_type=None):
    ...
    try:
        ...
        if not user:
            ...
            raise MT5ConnectionError(f"Failed to get user: {error}")
        ...
        if not connection.manager.UserUpdate(user):
            error = MT5Manager.LastError()
            raise MT5ConnectionError(f"Failed to update user rights: {error}")
                
        return user
            
    except Exception as e:
        error = MT5Manager.LastError()
        raise MT5ConnectionError(f"Failed to set user rights: {error}")
    ...
    try:
        ...
        if result:
            ...
            if error[1] == MT5Manager.EnMTAPIRetcode.MT_RET_USR_LOGIN_EXHAUSTED:
                raise MT5ConnectionError("No free logins on server")
            elif error[1] == MT5Manager.EnMTAPIRetcode.MT_RET_USR_LOGIN_PROHIBITED:
                raise MT5ConnectionError("Can't add user for non current server")
            elif error[1] == MT5Manager.EnMTAPIRetcode.MT_RET_USR_LOGIN_EXIST:
                raise MT5ConnectionError("User with same login already exists")
            else:
                raise MT5ConnectionError(f"Failed to create account: {error}")
        ...
        raise MT5ConnectionError(f"Failed to create account: {str(e)}")
  ...
  async def restore(login: Union[int, List[int]], server_id=None, server_type=None) -> Union[MT5Manager.MTUser, Dict[int, MT5Manager.MTUser]]:
    ...
    try:
        ...
        if is_single:
            ...
            if active_user:
                raise ValueError(f"User {login} already exists in active database")
                    
            # Fetch user from archive
            archived_user = connection.admin.UserArchiveRequest(login)
            if not archived_user:
                raise ValueError(f"User {login} not found in archive")
                    
            # Restore user
            if not connection.admin.UserRestore(archived_user):
                error = MT5Manager.LastError()
                raise MT5ConnectionError(f"Failed to restore user: {error}")
                    
            # Verify restoration
            restored_user = connection.manager.UserRequest(login)
            if not restored_user:
                raise MT5ConnectionError(f"Failed to verify restored user")
            ...
            if active_users:
                ...
                raise ValueError(f"Users {existing_logins} already exist in active database")
            ...
            if not archived_users:
                raise ValueError(f"Users {logins} not found in archive")
            ...
            try:
                ...
                if result is None:
                    error = MT5Manager.LastError()
                    raise MT5ConnectionError(f"Failed to restore users: {error}")
            except Exception as e:
                error = MT5Manager.LastError()
            ...
            if failed:
                ...
                raise MT5ConnectionError(f"Failed to verify restoration for users: {failed}")
