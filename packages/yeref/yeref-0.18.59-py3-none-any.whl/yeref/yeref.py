...
from aiogram.types import KeyboardButtonRequestChat, ChatAdministratorRights, LabeledPrice, LinkPreviewOptions, \
    ReactionTypeEmoji, CopyTextButton
...
from yeref.l_ import l_inline_demo, l_inline_bot, l_inline_post, l_inline_media, l_inline_channel, l_inline_group, \
    ...
    l_bot_need_start_add, l_chn_need_boost_for_story
...
allowed_paths = ['ferey_extra', 'ferey_bot', 'ferey_user', 'ferey_channel', 'ferey_group', 'ferey_ai', 'ferey_demo',
                 ...
                 'ferey_target', 'ferey_codex', 'ferey_fest']
        ...
        for message in messages:
            try:
                ...
                if message.text:
                    ...
                    file_name = f"{datetime.now(timezone.utc).strftime('%d-%m-%Y_%H-%M-%S-%f.jpg')}"
                    ...
                    file_name = f"{datetime.now(timezone.utc).strftime('%d-%m-%Y_%H-%M-%S-%f.gif')}"
                    dst = os.path.join(MEDIA_D, str(BOT_TID_TEST), file_name)
                    await app.download_media(file_id, dst)
                    ...
                    # file_name = message.video.file_name or f"{datetime.now(timezone.utc).strftime('%d-%m-%Y_%H-%M-%S-%f.mp4')}"
                    ...
                    file_name = f"{datetime.now(timezone.utc).strftime('%d-%m-%Y_%H-%M-%S-%f.mp4')}"
                    dst = os.path.join(MEDIA_D, str(BOT_TID_TEST), file_name)
                    await app.download_media(file_id, dst)
                    ...
                    file_name = datetime.now(timezone.utc).strftime('%d-%m-%Y_%H-%M-%S-%f.ogg')
                    dst = os.path.join(MEDIA_D, str(BOT_TID_TEST), file_name)
                    await app.download_media(file_id, dst)
                    ...
                    file_name = datetime.now(timezone.utc).strftime('%d-%m-%Y_%H-%M-%S-%f.webp')
                    if message.sticker.is_video:
                        file_name = datetime.now(timezone.utc).strftime('%d-%m-%Y_%H-%M-%S-%f.webm')
                    elif message.sticker.is_animated:
                        file_name = datetime.now(timezone.utc).strftime('%d-%m-%Y_%H-%M-%S-%f.tgs')
                    dst = os.path.join(MEDIA_D, str(BOT_TID_TEST), file_name)
                    await app.download_media(file_id, dst)
...
async def export_template(bot, chat_id, lz, BOT_TID, BOT_USERNAME, EXTRA_D, MEDIA_D, keys, is_premium_owner_=False):
    ...
    try:
        ...
        zip_dir = datetime.now(timezone.utc).strftime('%d-%m-%Y_%H-%M-%S-%f_dir')
        zip_dir = os.path.join(MEDIA_D, str(BOT_TID), zip_dir)
        zip_name = datetime.now(timezone.utc).strftime(f'@{BOT_USERNAME}-%d-%m-%Y.bot')
        ...
        if stage_json == '':  # if True:
            ...
            async with aiofiles.open(stage_json, 'w', encoding='utf-8') as f:
                await f.write(json.dumps({},
                                         ensure_ascii=False))  # with open(stage_json, 'w', encoding='utf-8') as f:  #     json.dump({}, f, ensure_ascii=False)
        else:
        ...
        for state_file in state_files:
            ...
            if GROUP_VID in tmp_json:
                ...
                async with aiofiles.open(tmp_file, 'w', encoding='utf-8') as f:
                    await f.write(json.dumps(tmp_json,
                                             ensure_ascii=False))  # with open(tmp_file, 'w', encoding='utf-8') as f:  #     json.dump(tmp_json, f, ensure_ascii=False)
    except Exception as e:
        logger.info(log_ % str(e) + str(GROUP_USERS))
        await asyncio.sleep(round(random.uniform(0, 1), 2))
...
async def block_chain(BASE_ENT):
    ...
    try:
        ...
        for unkey, unchain in res_unfull_chain.items():
            ...
            key = random.choices(chains_with_lengths, weights=[1 / len(l_[1]) for l_ in chains_with_lengths])[0][0]
...
async def upd_trg_json(state_file, lz, ent_vid, ent_type, content, x_, y_, views_=0):
    ...
    try:
        ...
        async with aiofiles.open(str(state_file), 'w', encoding='utf-8') as f:
            await f.write(json.dumps(result,
                                     ensure_ascii=False))  # with open(str(state_file), 'w', encoding='utf-8') as f:  #     json.dump(result, f, ensure_ascii=False)
    except Exception as e:
        logger.info(log_ % str(e))
        await asyncio.sleep(round(random.uniform(0, 1), 2))
    finally:
...
async def upd_act_json(state_file, lz, ent_vid, ent_type, content, x_, y_, views_=0):
    ...
    try:
        ...
        async with aiofiles.open(state_file, 'w', encoding='utf-8') as f:
            await f.write(json.dumps(result,
                                     ensure_ascii=False))  # with open(state_file, 'w', encoding='utf-8') as f:  #     json.dump(result, f, ensure_ascii=False)
    except Exception as e:
        logger.info(log_ % str(e))
        await asyncio.sleep(round(random.uniform(0, 1), 2))
    finally:
        ...
        for item_trg in data_trg:
            ...
            for k, v in result.items():
                ...
                for item in ent_json['children'][-1]['children']:
                    if getattr(item, 'children', None) and len(item['children']) == 3:
                        tmp_ = item['children'][-1]['attrs']['trg_vid']
                        if tmp_ == TRG_VID:
...
async def get_pay_btn(bot, chat_id, lz, payload='donate', stars=1):
    try:
        ...
        await bot.send_invoice(chat_id=chat_id, title='★', description=str_empty,
                               payload=f"{chat_id}_{stars}_{payload}", currency='XTR',
...
async def star_invoice_link(bot, PROJECT_USERNAME, chat_id, stars, payload='-', subscription_period=2592000,
    ...
    result = None
    try:
        invoice = await bot.create_invoice_link(title=PROJECT_USERNAME, description=f"{stars}", payload=payload,
                                                currency='XTR', photo_url=photo_url,
                                                prices=[LabeledPrice(label="XTR", amount=stars)],
...
async def list_star_invoice_link(bot, PROJECT_USERNAME, PROJECT_TYPE):
    try:
        ...
        r_ = await bot.create_invoice_link(title=f"#{PROJECT_TYPE}", description=f"@{PROJECT_USERNAME}",
...
async def refund_star_payment_fun(bot, state, chat_id, username, telegram_payment_id):
    res = None
    try:
        try:
            ...
            text = f"👩🏽‍💻 <b>Возврат</b>: @{username} ({chat_id})"
            await bot.send_message(chat_id=my_tid, text=text)
        except Exception as e:
            logger.info(log_ % str(e))  # CHARGE_NOT_FOUND
            if 'CHARGE_ALREADY_REFUNDED' in str(e):
                text = f"👩🏽‍💻 <b>Возврат</b> уже был произведен ранее"
        ...
        print(f"{res=}")
        if res:
            text = f"👩🏽‍💻 <b>Возврат</b> произведен успешно"
            await bot.send_message(chat_id=chat_id, text=text)
            await state.clear()
        else:
            text = f"👩🏽‍💻 <b>Некорректный</b> ID-операции"
            await bot.send_message(chat_id=chat_id, text=text)
    except Exception as e:
        logger.info(log_ % str(e))
        await asyncio.sleep(round(random.uniform(0, 1), 2))
...
async def train_ent_chatgpt(bot, ENT_TID, ENT_USERNAME, ENT_TYPE, BASE_ENT, EXTRA_D, BASE_D, lz, prompt='',
                            BOT_TOKEN_=None):
...
async def pst_gen_ent(bot, chat_id, state, lc, lz, POST_TYPE, ENT_TID, ENT_TYPE, ENT_USERNAME, ENT_TITLE, MEDIA_D,
    ...
    try:
        ...
        prompt_txt, prompt_img = await train_ent_chatgpt(bot, ENT_TID, ENT_USERNAME, ENT_TYPE, BASE_ENT, EXTRA_D,
                                                         BASE_D, lz, prompt, ENT_TOKEN_)
...
async def pst_gen_ent2(bot, chat_id, lc, lz, POST_TYPE, POST_MEDIA, ENT_TID, ENT_TYPE, ENT_USERNAME, ENT_TITLE, MEDIA_D,
                       EXTRA_D, BASE_D, prompt='', ENT_TOKEN_=None):
    result = dst = None
    try:
        ...
        if POST_TYPE == 'text':
            ...
            POST_MEDIA.append(
                {"file_id": POST_FID, "file_id_note": POST_FIDNOTE, "file_type": POST_TYPE, "file_name": POST_FILENAME,
                 "file_link": POST_LNK, "file_link2": POST_LNK})
        ...
        POSTB_FIDNOTE = POST_FID = POSTB_FID = POST_FIDNOTE = None
        print(f"{POSTB_FIDNOTE}, {POST_FID}, {POSTB_FID}, {POST_FIDNOTE}")  # result = True
    except Exception as e:
        logger.info(log_ % str(e))
        await asyncio.sleep(round(random.uniform(0, 1), 2))
    finally:
...
async def get_link_for_media(bot, chat_id, file_path, KEYS_JSON, is_del=True):
    ...
    try:
        ...
        len_ = len(data["media"]["pinata"])
        random_index = random.randint(0, len_ - 1)
        pinata_gateway = data['media']['pinata'][random_index]['GATEWAY']
        pinata_secret_key = data['media']['pinata'][random_index]['SECRET_KEY']
...
async def handle_mem_pack(bot, chat_id, lz, tid, username, title, desc, file_photo, IMGFLIP_UN, IMGFLIP_PS,
    ...
    try:
        ...
        if result and PROJECT_USERNAME == 'FereyChannelBot' and mem_type == 'regular':
            sql = "UPDATE CHANNEL SET CHANNEL_STICKERPACK=? WHERE CHANNEL_TID=?"
            await db_change(sql, (result, tid,), BASE_D)
            await bot.send_message(chat_id=chat_id, text=result)
        elif result and PROJECT_USERNAME == 'FereyChannelBot' and mem_type == 'custom_emoji':
            sql = "UPDATE CHANNEL SET CHANNEL_EMOJIPACK=? WHERE CHANNEL_TID=?"
            await db_change(sql, (result, tid,), BASE_D)
            await bot.send_message(chat_id=chat_id, text=result)
    except Exception as e:
        logger.info(log_ % str(e))
...
async def jpg_photo_preview(file_link, BOT_TID, MEDIA_D):
    ...
    try:
        ...
        res = await get_tgph_link(file_finish)
...
async def convert_png_to_mp4(ENT_TID, input_name, output_name, MEDIA_D):
    ...
    try:
        ...
        clip = concatenate_videoclips([clip] * 12, method="compose")
...
async def get_tgph_link(file_name):
    print(f"get_tgph_link {file_name=}")
    return None
...
async def check_sub_pay(chat_id, lz, BOT_TOKEN_E18B, BASE_D):
    ...
    try:
        ...
        sql = "SELECT USER_ISPAID, USER_DTPAID, USER_TYPAID, USER_DT, USER_LZ FROM USER WHERE USER_TID=?"
        data_usr = await db_select(sql, (chat_id,), BASE_D)

        if not len(data_usr):
            sql = "INSERT OR IGNORE INTO USER (USER_TID, USER_DT) VALUES (?, ?)"
            await db_change(sql, (chat_id, USER_DT,), BASE_D)
        else:
            USER_ISPAID, USER_DTPAID, USER_TYPAID, USER_DT, lz = data_usr[0]

        if is_paid:
            sql = "UPDATE USER SET USER_ISPAID=1, USER_DTPAID='', USER_TYPAID='all' WHERE USER_TID=?"
            ...
            if dt_now > till_paid:
                sql = "UPDATE USER SET USER_ISPAID=0, USER_DTPAID='', USER_TYPAID='' WHERE USER_TID=?"
                await db_change(sql, (chat_id,), BASE_D)
                till_paid = ''
            else:
                is_paid = True
...
async def region_web(bot, username, ENT_TID, ENT_LC, POST_ID, POST_TYPE, POST_TEXT, POST_LNK, POST_BUTTONS, ENT_TYPE,
    ...
    try:
        ...
        html_web = html_template.format(m_html, txt_html, btn_html, msg_views, ENT_LINK, ENT_USERNAME, ENT_TID, POST_ID,
                                        POST_LNKS, address_contract_friendly, address_owner_raw, state_init, shade)
        if POST_ID:
...
async def check_webapp_hash(init_data, TOKEN_BOT, BOT_TOKEN_MAIN=None, extra=None):
    ...
    try:
        ...
        print('seconds {(datetime.now() - auth_date).seconds}', init_data)
...
async def get_user_vars(chat_id, BASE_ENT):
    USER_GAMES = {}
    USER_VARS = {}
    USER_ISPAID = 0
    try:
        sql = "SELECT USER_GAMES, USER_VARS, USER_ISPAID FROM USER WHERE USER_TID=?"
        data_usr = await db_select(sql, (chat_id,), BASE_ENT)
        if not len(data_usr) or not data_usr[0][0]:
            if not len(data_usr):
                dt_now = datetime.now(timezone.utc).strftime('%d-%m-%Y_%H-%M-%S')
                sql = "INSERT OR IGNORE INTO USER (USER_TID, USER_DT) VALUES (?, ?)"
                await db_change(sql, (chat_id, dt_now,), BASE_ENT)
        else:
...
async def save_posts(chat_id, lz, username, bot, posts, is_sub, tz, ENT_TID, BASE_ENT, MEDIA_D, BASE_D,
                     PROJECT_USERNAME,
                     PROJECT_TYPE):
    result = {'result': True, 'bot': True, 'blog': None, 'hash': None}
    try:
        print(f"{posts['POST_TID']=}")
        print(f"{posts['POST_MEDIA']=}")
        POST_TID = posts['POST_TID']
        POST_TYPE = posts['POST_MEDIA'][0]['file_type'] if len(posts['POST_MEDIA']) else posts['POST_TYPE']
        POST_TEXT = posts['POST_TEXT']
        POST_TEXTF = posts['POST_TEXTF']

        POST_BLOG = await region_blog2(bot, ENT_TID, POST_TYPE, POST_TEXT, posts['POST_MEDIA'], BASE_D, PROJECT_TYPE)
        POST_MEDIA = json.dumps(posts['POST_MEDIA'], ensure_ascii=False) if not isinstance(posts['POST_MEDIA'],
                                                                                           str) else posts['POST_MEDIA']
        POST_BUTTONS = json.dumps(posts['POST_BUTTONS'], ensure_ascii=False) if not isinstance(posts['POST_BUTTONS'],
                                                                                               str) else posts[
            'POST_BUTTONS']
        POST_BUTTONSF = json.dumps(posts['POST_BUTTONSF'], ensure_ascii=False) if not isinstance(posts['POST_BUTTONSF'],
                                                                                                 str) else posts[
            'POST_BUTTONSF']

        POST_PAY = ''
        POST_ISPAY = posts['POST_CHKBOX']['POST_ISPAY'] if 'POST_ISPAY' in posts['POST_CHKBOX'] else False
        if POST_ISPAY and POST_TYPE in ['photo', 'video']:
            POST_PAY = f"https://t.me/{PROJECT_USERNAME}/web?startapp={ENT_TID}_{POST_TID}_0_pay"
        POST_ISPRIVATE = posts['POST_CHKBOX']['POST_ISPRIVATE'] if 'POST_ISPRIVATE' in posts['POST_CHKBOX'] else ''
        POST_CHKBOX = json.dumps(posts['POST_CHKBOX'], ensure_ascii=False)
        POST_WATER = posts['POST_WATER'] if 'POST_WATER' in posts else ''
        POST_STARS = posts['POST_STARS'] if 'POST_STARS' in posts else ''

        POST_PAYLINK = None
        POST_TOKEN = posts['POST_TOKEN'] if 'POST_TOKEN' in posts else ''
        POST_TZ = tz
        POST_DT = posts['POST_DT'] if 'POST_DT' in posts else ''
        POST_TR = posts['POST_TR'] if 'POST_TR' in posts else ''

        HASH_STR = f"{ENT_TID}_{POST_TID}_0_post"
        HASH_VAL = hashlib.md5(HASH_STR.encode('utf-8')).hexdigest()
        sql = "INSERT OR IGNORE INTO HASH(HASH_STR, HASH_VAL) VALUES (?, ?)"
        await db_change(sql, (HASH_STR, HASH_VAL, ), BASE_D)
        POST_WEB = f"https://t.me/{PROJECT_USERNAME}/web?startapp=post-{HASH_VAL}&mode=fullscreen"

        result = {'result': True, 'bot': True, 'blog': POST_BLOG, 'hash': HASH_VAL}
        if POST_ISPAY and POST_TOKEN and len(json.loads(POST_MEDIA)):
            POST_MEDIA, res_bot = await update_media(bot, chat_id, lz, ENT_TID, POST_TOKEN, POST_MEDIA,
                                                     MEDIA_D, PROJECT_USERNAME)
            result = {'result': True, 'bot': res_bot, 'blog': POST_BLOG, 'hash': HASH_VAL}
            POST_MEDIA = json.dumps(POST_MEDIA) if not isinstance(POST_MEDIA, str) else POST_MEDIA
        if len(json.loads(POST_BUTTONS)) and POST_TOKEN:
            POST_BUTTONS = json.loads(POST_BUTTONS)

            try:
                extra_bot = Bot(token=POST_TOKEN)
                for button in POST_BUTTONS:
                    try:
                        if button['knd'] != 'pay': continue
                        try:
                            amount = int(button['lnk'])
                        except:
                            amount = 1
                        prices = [LabeledPrice(label="XTR", amount=amount)]
                        pay_lnk = await extra_bot.create_invoice_link(title='Post', description='ent_title',
                                                                      payload=f"{ENT_TID}_{POST_TID}_{button['bid']}",
                                                                      currency='XTR', photo_url=post_photo,
                                                                      prices=prices)
                        button['dsc'] = pay_lnk
                    except TelegramRetryAfter as e:
                        logger.info(log_ % f"TelegramRetryAfter {e.retry_after}")
        ...
        if POST_ISPAY and POST_STARS and POST_TOKEN:
            try:
                extra_bot = Bot(token=POST_TOKEN)
                try:
                    ...
                    amount = 1
                prices = [LabeledPrice(label="XTR", amount=amount)]
                POST_PAYLINK = await extra_bot.create_invoice_link(title='Post', description='ent_title',
                                                                   payload=f"{ENT_TID}_{POST_TID}_0", currency='XTR',
                                                                   photo_url=post_photo, prices=prices)
                await extra_bot.session.close()
            except Exception as e:
                logger.info(log_ % str(e))
                await asyncio.sleep(round(random.uniform(0, 1), 2))
        if POST_TOKEN:
            print(f" = = = = = =  ={POST_TOKEN=}")
            sql = "UPDATE CHANNEL SET CHANNEL_CPAYTOKEN=? WHERE CHANNEL_TID=?"
            await db_change(sql, (POST_TOKEN, ENT_TID,), BASE_D)

        print(f"{POST_PAYLINK=}, {POST_STARS=}")
        sql = "INSERT OR IGNORE INTO POST (POST_TID, POST_CHATTID, POST_USERTID, POST_USERTUN, POST_TYPE, POST_TEXT, POST_TEXTF, POST_MEDIA, POST_BUTTONS, POST_BUTTONSF, POST_CHKBOX, POST_WEB, POST_PAY, POST_BLOG, POST_WATER, POST_STARS, POST_ISPRIVATE, POST_PAYLINK, POST_TZ, POST_DT, POST_TR) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        await db_change(sql, (
            POST_TID, int(ENT_TID), chat_id, username, POST_TYPE, POST_TEXT, POST_TEXTF, POST_MEDIA, POST_BUTTONS,
            POST_BUTTONSF, POST_CHKBOX, POST_WEB, POST_PAY, POST_BLOG, POST_WATER, POST_STARS, POST_ISPRIVATE,
            POST_PAYLINK, POST_TZ, POST_DT, POST_TR,), BASE_ENT)
        print('asd', POST_WEB)
        sql = "UPDATE POST SET POST_TYPE=?, POST_TEXT=?, POST_TEXTF=?, POST_MEDIA=?, POST_BUTTONS=?, POST_BUTTONSF=?, POST_CHKBOX=?, POST_WEB=?, POST_PAY=?, POST_BLOG=?, POST_WATER=?, POST_STARS=?, POST_ISPRIVATE=?, POST_PAYLINK=?, POST_TZ=?, POST_DT=?, POST_TR=? WHERE POST_TID=?"
        await db_change(sql, (
            POST_TYPE, POST_TEXT, POST_TEXTF, POST_MEDIA, POST_BUTTONS, POST_BUTTONSF, POST_CHKBOX, POST_WEB, POST_PAY,
            POST_BLOG, POST_WATER, POST_STARS, POST_ISPRIVATE, POST_PAYLINK, POST_TZ, POST_DT, POST_TR, POST_TID,),
                        BASE_ENT)
        print('after', POST_DT)
    except TelegramRetryAfter as e:
        logger.info(log_ % f"TelegramRetryAfter {e.retry_after}")
...
async def post_pub(bot, lz, chat_id, ENT_TID, item, MEDIA_D, BASE_S, BASE_D, PROJECT_USERNAME, is_private=True,
                   message_id=None, current=1):
    result = False
    try:
        POST_ID, POST_TID, POST_USERTID, POST_USERTUN, POST_TYPE, POST_TEXT, POST_TEXTF, POST_MEDIA, POST_BUTTONS, POST_BUTTONSF, POST_CHKBOX, POST_WATER, POST_STARS, POST_TZ, POST_DT = item

        len_ = 0
        index = 0
        POST_STARS = POST_STARS or 1
        ...
        if not is_private:
            ...
            try:
                await get_pay_btn(bot, chat_id, lz, 'post')
                await asyncio.sleep(round(random.uniform(0, 1), 2))
                address = f"https://t.me/FereyChannelBot/web?startapp={ENT_TID}_{POST_TID}_0_post"
                await bot.send_message(chat_id=chat_id, text=f"{l_web_version[lz]} {address}")
            except Exception as e:
                logger.info(log_ % str(e))
                await asyncio.sleep(round(random.uniform(0, 1), 2))
        # else:
        #     chat_id = int(POST_USERTID)
        if len(POST_MEDIA) > 1 and POST_CHKBOX['POST_ISGALLERY']:
            len_ = len(POST_MEDIA)
            if message_id:
                index = current - 1  # POST_MEDIA = [POST_MEDIA[current - 1]]  # else:  # POST_MEDIA = [POST_MEDIA[index]]
        if message_id:
            POST_CHKBOX['POST_ISCOLLAPSE'] = False
        # endregion

        # region text
        POST_TEXT = POST_TEXT or str_empty
        POST_TEXT = re.sub(r'<span style="background-color: rgba\(0, 123, 247, 0\.7\);">(.*?)<\/span>',
                           r'<blockquote>\1</blockquote>', POST_TEXT)
        POST_TEXT = re.sub(r'<span style="background-color: gray;">(.*?)<\/span>', r'<tg-spoiler>\1</tg-spoiler>',
                           POST_TEXT)
        POST_TEXT = re.sub(r'<span style="background-color: rgba\(0, 123, 247, 0\.8\);">(.*?)<\/span>',
                           r'<blockquote expandable>\1</blockquote>', POST_TEXT)
        POST_TEXT = POST_TEXT.replace('&nbsp;', '').replace('<br>', '')
        POST_TEXT = re.sub(r'</div>\s*<div>', '\n', POST_TEXT)
        ...
        POST_TEXT = POST_TEXT.strip()
        POST_TEXT = POST_TEXT[0:4096]
        if POST_CHKBOX['POST_ISTAG'] and POST_TEXT and len(POST_TEXT) < 900:
            sql = "SELECT USER_ID FROM USER"
            data_rnd = await db_select(sql, (), BASE_ENT)
            if len(data_rnd):
        ...
        # region reply_markup
        reply_markup = InlineKeyboardBuilder()
        print(f"replay m,akrp {len(POST_MEDIA)=}")
        PAY_BUTTONS = [{'lbl': btn['lbl'], 'bid': btn['bid']} for btn in POST_BUTTONS if btn['knd'] == 'pay']
        print(f"{len(PAY_BUTTONS)=}, {POST_TYPE=}")
        # if len(PAY_BUTTONS):
        #     print(1)
        #
        #     for it in PAY_BUTTONS:
        #         url = f"https://t.me/{PROJECT_USERNAME}?start={ENT_TID}_{POST_TID}_{it['bid']}_pay"
        #         reply_markup.row(types.InlineKeyboardButton(text=it['lbl'], url=url))
        #
        #     lp_options = LinkPreviewOptions(is_disabled=False, url=post_photo, prefer_small_media=True)
        #     await bot.send_message(chat_id=chat_id, text=str_empty, link_preview_options=lp_options, reply_markup=reply_markup.as_markup())
        #     return

        print(f"{POST_CHKBOX['POST_ISPAY']=}, {POST_TYPE=}")
        if POST_CHKBOX['POST_ISPAY'] and POST_TYPE not in ['photo', 'video']:
            print(2)
            url = f"https://t.me/{PROJECT_USERNAME}?start={ENT_TID}_{POST_TID}_0_pay"
            reply_markup.row(types.InlineKeyboardButton(text=l_payment_of_post[lz], url=url))
            lp_options = LinkPreviewOptions(is_disabled=False, url=post_photo, prefer_small_media=True)
            result = await bot.send_message(chat_id=chat_id, text=str_empty, link_preview_options=lp_options,
                                            reply_markup=reply_markup.as_markup())
            sql = "UPDATE POST SET POST_MSGID=? WHERE POST_TID=?"
            await db_change(sql, (result.message_id, POST_TID,), BASE_ENT)
            return

        print(f"========================")
        if len(POST_MEDIA) > 1 and POST_CHKBOX['POST_ISGALLERY'] and not POST_CHKBOX['POST_ISCOLLAPSE'] and not \
                POST_CHKBOX['POST_ISPAY']:
            POST_TEXT = '' if POST_TEXT == str_empty and POST_TYPE != 'text' else POST_TEXT
            print(f"ins gallery {chat_id=}, {ENT_TID=}")

            buttons = [
                types.InlineKeyboardButton(text="←", callback_data=f'gal_prev_{ENT_TID}_{POST_TID}_{current}_{len_}'),
                types.InlineKeyboardButton(text=f"{current}/{len_}",
                                           callback_data=f'gal_current_{ENT_TID}_{POST_TID}_{current}_{len_}'),
                types.InlineKeyboardButton(text="→", callback_data=f'gal_next_{ENT_TID}_{POST_TID}_{current}_{len_}'), ]
            reply_markup.row(*buttons)

        if POST_CHKBOX['POST_ISCOLLAPSE'] and not message_id and not POST_CHKBOX['POST_ISPAY']:
            reply_markup.row(types.InlineKeyboardButton(text="ᴿᵁᴺ", callback_data=f'btn_{ENT_TID}_{POST_TID}_run'))
        else:
            reply_markup = await get_ent_rm(bot, chat_id, reply_markup, ENT_TID, POST_USERTUN, POST_TID, POST_BUTTONS,
                                            POST_CHKBOX, BASE_ENT, BASE_D, PROJECT_USERNAME)
        # endregion

        # region send
        if POST_TYPE != 'text': POST_TEXT = POST_TEXT[0:1024]
        if message_id:
            print(f"inside {chat_id=}, {message_id=}, {POST_TYPE=}")
            if POST_TYPE == 'text':
                await bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=POST_TEXT,
                                            link_preview_options=LinkPreviewOptions(
                                                is_disabled=not POST_CHKBOX['POST_ISPREVIEW']),
                                            disable_web_page_preview=not POST_CHKBOX['POST_ISPREVIEW'],
                                            parse_mode=ParseMode.HTML, reply_markup=reply_markup.as_markup())
            elif POST_TYPE == 'photo':
                POST_TEXT = None if POST_TEXT == str_empty else POST_TEXT
                media = types.InputMediaPhoto(media=POST_MEDIA[index]['file_id'], caption=POST_TEXT,
                ...
                POST_TEXT = None if POST_TEXT == str_empty else POST_TEXT
                media = types.InputMediaAnimation(media=POST_MEDIA[index]['file_id'], caption=POST_TEXT,
                                                  parse_mode=ParseMode.HTML,
                                                  show_caption_above_media=POST_CHKBOX['POST_ISPREVIEW'],
                                                  has_spoiler=POST_CHKBOX['POST_ISSPOILER'])
                await bot.edit_message_media(media=media, chat_id=chat_id, message_id=message_id,
                ...
                POST_TEXT = None if POST_TEXT == str_empty else POST_TEXT
                media = types.InputMediaVideo(media=POST_MEDIA[index]['file_id'], caption=POST_TEXT,
                                              parse_mode=ParseMode.HTML,
                                              show_caption_above_media=POST_CHKBOX['POST_ISPREVIEW'],
                                              has_spoiler=POST_CHKBOX['POST_ISSPOILER'])
                await bot.edit_message_media(media=media, chat_id=chat_id, message_id=message_id,
                ...
                media = types.InputMediaVideo(media=POST_MEDIA[index]['file_id_note'], parse_mode=ParseMode.HTML,
                                              caption=POST_TEXT)
                await bot.edit_message_media(media=media, chat_id=chat_id, message_id=message_id,
                                             reply_markup=reply_markup.as_markup())
            elif POST_TYPE == 'audio':
                POST_TEXT = None if POST_TEXT == str_empty else POST_TEXT
                media = types.InputMediaAudio(media=POST_MEDIA[index]['file_id'], parse_mode=ParseMode.HTML,
                                              caption=POST_TEXT)
                await bot.edit_message_media(media=media, chat_id=chat_id, message_id=message_id,
                                             reply_markup=reply_markup.as_markup())
            elif POST_TYPE == 'voice':
                POST_TEXT = None if POST_TEXT == str_empty else POST_TEXT
                media = types.InputMediaAudio(media=POST_MEDIA[index]['file_id_note'], parse_mode=ParseMode.HTML,
                                              caption=POST_TEXT)
                await bot.edit_message_media(media=media, chat_id=chat_id, message_id=message_id,
                                             reply_markup=reply_markup.as_markup())
            elif POST_TYPE == 'document':
                POST_TEXT = None if POST_TEXT == str_empty else POST_TEXT
                media = types.InputMediaDocument(media=POST_MEDIA[index]['file_id'], parse_mode=ParseMode.HTML,
                                                 caption=POST_TEXT)
                await bot.edit_message_media(media=media, chat_id=chat_id, message_id=message_id,
                                             reply_markup=reply_markup.as_markup())
            else:
        ...
        elif POST_CHKBOX['POST_ISPAY'] and POST_TYPE in ['photo', 'video']:
            sql = "SELECT CHANNEL_CPAYTOKEN FROM CHANNEL WHERE CHANNEL_TID=?"
            data_chn = await db_select(sql, (ENT_TID,), BASE_D)
            if not len(data_chn): return
            CHANNEL_CPAYTOKEN = data_chn[0][0]
            print(f"POST_ISPAY {chat_id=}, {CHANNEL_CPAYTOKEN=}, {POST_MEDIA=}")
            media = []
            ...
            try:
                extra_bot = Bot(token=CHANNEL_CPAYTOKEN)
                result = await extra_bot.send_paid_media(chat_id=chat_id, star_count=int(POST_STARS), media=media,
                                                         caption=POST_TEXT, payload=f'{ENT_TID}_{POST_TID}_0',
                ...
                return
        elif len(POST_MEDIA) > 1 and not POST_CHKBOX['POST_ISGALLERY']:
            media = []
            for i in range(0, len(POST_MEDIA)):
                caption = POST_TEXT if i == 0 else None
                if caption: caption = caption if caption not in ['', str_empty] else None
            ...
            result = await bot.send_photo(chat_id=chat_id, photo=POST_MEDIA[index]['file_id'], caption=POST_TEXT,
                                          parse_mode=ParseMode.HTML,
                                          show_caption_above_media=POST_CHKBOX['POST_ISPREVIEW'],
                                          has_spoiler=POST_CHKBOX['POST_ISSPOILER'],
                                          disable_notification=not POST_CHKBOX['POST_ISSOUND'],
            ...
            result = await bot.send_video(chat_id=chat_id, video=POST_MEDIA[index]['file_id'], caption=POST_TEXT,
                                          parse_mode=ParseMode.HTML,
                                          show_caption_above_media=POST_CHKBOX['POST_ISPREVIEW'],
                                          has_spoiler=POST_CHKBOX['POST_ISSPOILER'],
                                          disable_notification=not POST_CHKBOX['POST_ISSOUND'],
            ...
            result = await bot.send_document(chat_id=chat_id, document=POST_MEDIA[index]['file_id'], caption=POST_TEXT,
                                             parse_mode=ParseMode.HTML,
                                             disable_notification=not POST_CHKBOX['POST_ISSOUND'],
                                             protect_content=POST_CHKBOX['POST_ISPROTECT'],
                                             message_effect_id=POST_CHKBOX['POST_EFFECT'],
        ...
        if result and int(chat_id) == int(ENT_TID):
            print('indeid')
            ...
            sql = "UPDATE POST SET POST_MSGID=? WHERE POST_TID=?"
            await db_change(sql, (POST_MSGID, POST_TID,), BASE_ENT)
        # endregion

        # region story
        try:
            if result and POST_TYPE == 'text' and POST_CHKBOX['POST_ISSTORY'] and int(chat_id) == int(ENT_TID):
        ...
        # region podcast
        try:
            if result and result.chat.username and POST_CHKBOX['POST_ISPODCAST'] and int(chat_id) == int(
                ...
                if member_.can_manage_video_chats and member_.can_promote_members:
                    ...
                    sql = "SELECT USER_ISPAID FROM USER WHERE USER_TID=?"
                    data = await db_select(sql, (chat_id,), BASE_D)
                    USER_ISPAID = True if len(data) and data[0][0] else False
                    _ = asyncio.get_event_loop().create_task(
                        podcast_start(bot, POST_USERTID, lz, ENT_TID, POST_TID, POST_TYPE, POST_TEXT,
                                      POST_MEDIA[0]['file_id'], POST_MEDIA[0]['file_name'], USER_ISPAID, BASE_S,
                                      MEDIA_D, BASE_D))
        except Exception as e:
            logger.info(log_ % str(e))
            await asyncio.sleep(round(random.uniform(0, 1), 2))
        ...
        try:
            await asyncio.sleep(round(random.uniform(0, 1), 2))
            if result and POST_CHKBOX['POST_REACTION'] and not message_id:
        ...
        try:
            await asyncio.sleep(round(random.uniform(0, 1), 2))
            if result and POST_CHKBOX['POST_ISPIN'] and not message_id:
                msg_id = result[0].message_id if isinstance(result, list) else result.message_id
                await bot.pin_chat_message(chat_id=chat_id, message_id=msg_id,
                                           disable_notification=not POST_CHKBOX['POST_ISSOUND'])
        except Exception as e:
            logger.info(log_ % str(e))
            await asyncio.sleep(round(random.uniform(0, 1), 2))  # endregion  # endregion
    except TelegramRetryAfter as e:
        logger.info(log_ % f"TelegramRetryAfter {e.retry_after}")
        await asyncio.sleep(e.retry_after + 1)
    except Exception as e:
...
async def region_blog2(bot, ENT_TID, POST_TYPE, POST_TEXT, POST_MEDIA, BASE_D, PROJECT_TYPE='bot', is_format=False):
    result = None
    try:
        # if PROJECT_TYPE in ['post']: return
        cnt = 0
        POST_MEDIA = json.loads(POST_MEDIA) if isinstance(POST_MEDIA, str) else POST_MEDIA
        print(ENT_TID, POST_TYPE, POST_MEDIA, PROJECT_TYPE, is_format)
        POST_TEXT = POST_TEXT if POST_TEXT else str_empty
        ...
        POST_TEXT = re.sub(r'<span style="background-color: rgba\(0, 123, 247, 0\.8\);">(.*?)<\/span>', r'\1',
                           POST_TEXT)
        POST_TEXT = re.sub(r'</?div>', '\n', POST_TEXT)
        POST_TEXT = POST_TEXT.replace('<strike>', '<s>').replace('</strike>', '</s>').replace('<span>', '').replace(
            '</span>', '')
        POST_TEXT = POST_TEXT.strip()
        POST_TEXT = POST_TEXT or str_empty
        print(f"{POST_TEXT=}")
...
async def update_media(bot, chat_id, lz, ENT_TID, POST_TOKEN, POST_MEDIA, MEDIA_D, PROJECT_USERNAME):
        ...
        try:
            ...
            for media in POST_MEDIA:
                ...
                try:
                    ...
                    file_name = os.path.join(MEDIA_D, str(ENT_TID), media['file_name'])
                    await bot.download_file(file.file_path, file_name)
...
async def get_ent_rm(bot, chat_id, reply_markup, ENT_TID, POST_USERTUN, POST_TID, POST_BUTTONS, POST_CHKBOX, BASE_ENT,
                     BASE_D, PROJECT_USERNAME):
    try:
        sql = "SELECT BUTTON_ID FROM PUSH WHERE POST_ID=?"
        data = await db_select(sql, (POST_TID,), BASE_ENT)
        ...
        for button in sorted_buttons:
            try:
                ...
                if button['knd'] == 'button_in':
                    cb_ = f"btn_{ENT_TID}_{POST_TID}_{button['bid']}"
                    ...
                    if button['dsc'] != '':
                        btn = types.InlineKeyboardButton(text=button['lbl'], url=button['dsc'])
                        rows[row_index].append(btn)
                    ...
                    print(f"===============================")
                    print(f"{button=}")
                    btn = types.InlineKeyboardButton(text=button['lbl'], switch_inline_query=str(POST_TID))
                    rows[row_index].append(btn)
                elif button['knd'] == 'web':
                    print(f"{button['lnk']=}")
                    web_id = button['lnk']
                    if button['lnk'].strip() == '':
                        sql = "SELECT POST_TID FROM POST"
                        data_post = await db_select(sql, (), BASE_ENT)
                        if len(data_post) and data_post[0][0]:
                            web_id = data_post[0][0]
                        else:
                            continue

                    print(f"{web_id=}")
                    HASH_STR = f"{ENT_TID}_{web_id}_0_post"
                    POST_HASH = hashlib.md5(HASH_STR.encode('utf-8')).hexdigest()
                    sql = "INSERT OR IGNORE INTO HASH(HASH_STR, HASH_VAL) VALUES (?, ?)"
                    await db_change(sql, (HASH_STR, POST_HASH,), BASE_D)

                    url = f'https://t.me/{PROJECT_USERNAME}/web?startapp=post-{POST_HASH}&mode=fullscreen'
                    btn = types.InlineKeyboardButton(text=button['lbl'], url=url)
                    rows[row_index].append(btn)
                elif button['knd'] == 'nft':
                    url = f'https://t.me/{PROJECT_USERNAME}/web?startapp={ENT_TID}_{POST_TID}_0_nft'
                    btn = types.InlineKeyboardButton(text=button['lbl'], url=url)
                    rows[row_index].append(btn)
...
async def podcast_start(bot, chat_id, lz, ENT_TID, POST_ID, POST_TYPE, POST_TEXT, POST_FID, POST_FILENAME, USER_ISPAID,
    ...
    try:
        ...
        for _ in data[:10]:
            ...
            try:
                sql = "UPDATE SESSION SET SESSION_STATUS=? WHERE SESSION_TID=?"
                await db_change(sql, (f'podcast_start', SESSION_TID,), BASE_S)
                ...
                logger.info(log_ % f"{SESSION_TID}: {str(e)}")
                await asyncio.sleep(round(random.uniform(0, 1), 2))
                sql = "UPDATE CHANNEL SET CHANNEL_ISPROMOTED=0 WHERE CHANNEL_TID=?"
                await db_change(sql, (ENT_TID,), BASE_D)
            finally:
                sql = "UPDATE SESSION SET SESSION_STATUS=? WHERE SESSION_TID=?"
                await db_change(sql, (SESSION_STATUS, SESSION_TID,), BASE_S)
        if file_name and os.path.exists(file_name): os.remove(file_name)
...
async def py_tg_calls_fun(bot, app, chat_id, lz, r, POST_ID, ENT_TID, stream, duration, MEDIA_D):
    ...
    try:
        ...
        async def participant_handler(_: PyTgCalls, update: Update):
            try:
                ...
                if not str(update.participant.user_id).startswith('-100'):
                    await add_to_push_podcast(bot, app, update.participant.user_id, POST_ID, BASE_ENT)
        ...
        for participant in participants:
            ...
            await add_to_push_podcast(bot, app, participant.user_id, POST_ID, BASE_ENT)
...
async def add_to_push_podcast(bot, app, user_id, POST_ID, BASE_ENT):
    result = None
    try:
        try:
            ...
            sql = "INSERT OR IGNORE INTO PUSH (CHAT_TID, CHAT_FULLNAME, CHAT_USERNAME, POST_ID, BUTTON_ID) " \
                  "VALUES (?, ?, ?, ?, ?)"
            await db_change(sql, (user_id, full_name, username, POST_ID, -1,), BASE_ENT)
        ...
        try:
            ...
            sql = "INSERT OR IGNORE INTO PUSH (CHAT_TID, CHAT_FULLNAME, CHAT_USERNAME, CHAT_ISPREMIUM, CHAT_LC, POST_ID, BUTTON_ID) VALUES (?, ?, ?, ?, ?, ?, ?)"
            await db_change(sql, (user_id, full_name, username, is_premium, lc, POST_ID, -1,), BASE_ENT)
...
async def story_start(bot, lz, chat_id, ENT_TID, POST_TYPE, POST_TEXT, POST_FID, POST_FILENAME, POST_CHKBOX,
    ...
    try:
        ...
        for _ in data[:10]:
            ...
            try:
                sql = "UPDATE SESSION SET SESSION_STATUS=? WHERE SESSION_TID=?"
                await db_change(sql, (f'story_start', SESSION_TID,), BASE_S)
                get_chat_ = await bot.get_chat(ENT_TID)
                link = f'@{get_chat_.username}' if get_chat_.username else get_chat_.invite_link
