...
def import_reducemean(onnx_node: onnx.NodeProto,
                      input_nodes: List[Tuple[aidge_core.Node, int]],
                      opset : int = None) -> aidge_core.Node:
    """
    ...
    reducemean_attrs : dict = {"axes" : [],"keep_dims" : 1, "noop_with_empty_axes" : 0}

    if 'keepdims' in onnx_attrs:
        reducemean_attrs['keep_dims'] = onnx_attrs['keepdims']
        del onnx_attrs['keepdims']

    if opset < 18 and 'axes' in onnx_attrs:
        reducemean_attrs['axes'] = onnx_attrs['axes']
        del onnx_attrs['axes']
    else:
        #try to get axes as input
        if input_nodes[1] is not None:
            reducemean_attrs['axes'] = input_nodes[1][0].get_operator().get_output(input_nodes[1][1])

        if 'noop_with_empty_axes' in onnx_attrs:
            reducemean_attrs['noop_with_empty_axes'] = onnx_attrs['noop_with_empty_axes']
            del onnx_attrs['noop_with_empty_axes']

    if len(onnx_attrs) > 0:
        Log.warn(f"Warning: unsupported attribute(s): {onnx_attrs.keys()} for operator 'ReduceMean' with opset {opset}.\nThis node will be filled by a GenericOperator.")
        return None
