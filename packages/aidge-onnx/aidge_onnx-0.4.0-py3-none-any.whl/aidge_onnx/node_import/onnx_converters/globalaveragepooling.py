...
@auto_register_import("globalaveragepool")
def import_globalaveragepooling(
    ...
    if len(onnx_attrs) > 0:
        Log.warn(f"Warning: unsupported attribute(s): {onnx_attrs.keys()} for operator 'GlobalAveragePool' with opset {opset}.\nThis node will be filled by a GenericOperator.")
        return None
