...
@auto_register_import("add")
def import_add(onnx_node:onnx.NodeProto, input_nodes:List[Tuple[aidge_core.Node, int]], opset=None) -> aidge_core.Node:
    """
    ...
    if opset <= 6:
        ...
        if 'axis' in onnx_attrs:
            ...
            del onnx_attrs['axis']
            warn_unsupported_attr('axis','Add',opset)
            return None

        if 'broadcast' in onnx_attrs:
            #Attribute not supported in Aidge
            del onnx_attrs['broadcast']
            warn_unsupported_attr('broadcast','Add',opset)
            return None
    ...
    if len(onnx_attrs) > 0:
        Log.warn(f"Warning: unsupported attribute(s): {onnx_attrs.keys()} for operator 'Add' with opset {opset}.")
        return None
