...
@auto_register_import("sigmoid")
def import_sigmoid(onnx_node:onnx.NodeProto, input_nodes:List[Tuple[aidge_core.Node, int]], opset=None) -> aidge_core.Node:
    """
    ...
    if len(onnx_attrs) > 0:
        Log.warn(f"Warning: unsupported attribute(s): {onnx_attrs.keys()} for operator 'Sigmoid' with opset {opset}.\nThis node will be filled by a GenericOperator.")
        return None
