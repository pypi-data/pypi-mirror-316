...
@auto_register_import("pow")
def import_pow(onnx_node:onnx.NodeProto, input_nodes:List[Tuple[aidge_core.Node, int]], opset=None) -> aidge_core.Node:
    """
    ...
    if opset < 7:
        if 'axis' in onnx_attrs:
            ...
            del onnx_attrs['axis']
            warn_unsupported_attr("axis","Pow",opset)
            return None
    ...
    if len(onnx_attrs) > 0:
        Log.warn(f"Warning: unsupported attribute(s): {onnx_attrs.keys()} for operator 'Pow' with opset {opset}.\nThis node will be filled by a GenericOperator.")
        return None
