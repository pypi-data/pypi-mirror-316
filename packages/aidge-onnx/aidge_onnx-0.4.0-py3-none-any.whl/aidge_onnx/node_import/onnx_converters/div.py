...
@auto_register_import("div")
def import_div(onnx_node:onnx.NodeProto, input_nodes:List[Tuple[aidge_core.Node, int]], opset=None) -> aidge_core.Node:
    """
    ...
    if opset < 7:
        ...
        if 'axis' in onnx_attrs:
            ...
            del onnx_attrs['axis']
            warn_unsupported_attr('axis','Div',opset)
            return None

        if 'broadcast' in onnx_attrs:
            #Attribute not supported in Aidge
            del onnx_attrs['broadcast']
            warn_unsupported_attr('broadcast','Div',opset)
            return None
    ...
    if len(onnx_attrs) > 0:
        Log.warn(f"Warning: unsupported attribute(s): {onnx_attrs.keys()} for operator 'Div' with opset {opset}.\nThis node will be filled by a GenericOperator.")
        return None
