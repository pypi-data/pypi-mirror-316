...
def import_reshape(onnx_node:onnx.NodeProto, input_nodes:List[Tuple[aidge_core.Node, int]], opset=None) -> aidge_core.Node:
    ...
    flatten_attrs : dict = {"axis" : 1}

    if 'axis' in onnx_attrs:
        flatten_attrs['axis'] = onnx_attrs['axis']
        del onnx_attrs['axis']

    if len(onnx_attrs) > 0:
        Log.warn(f"Warning: unsupported attribute(s): {onnx_attrs.keys()} for operator 'Flatten' with opset {opset}.\nThis node will be filled by a GenericOperator.")
        return None
