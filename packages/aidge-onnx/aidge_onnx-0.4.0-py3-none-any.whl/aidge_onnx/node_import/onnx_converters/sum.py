...
def import_sum(
    ...
    if len(onnx_attrs) > 0:
        Log.warn(
            f"Warning: unsupported attribute(s): {onnx_attrs.keys()} for operator 'Sum' with opset {opset}."
        )
        return None

    if len(input_nodes) != 2:
        Log.warn(
            f"Warning: unsupported number of inputs for Sum, expected 2: got {len(input_nodes)}"
        )
        return None
