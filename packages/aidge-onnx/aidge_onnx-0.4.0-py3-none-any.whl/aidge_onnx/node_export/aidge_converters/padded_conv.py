...
@auto_register_export("PaddedConv1D", "PaddedConv2D", "PaddedConv3D")
def export_padded_conv(
    ...
    **kwargs) -> List[helper.NodeProto]:
    ...
    for node in micro_graph.get_nodes():
        if node.type() == "Conv1D" or node.type() == "Conv2D" or node.type() == "Conv3D":
    ...
    if (
        ...
        and not aidge_node.input(2)[0].get_operator().get_output(0).has_impl()
    ):
