...
def groups_summary(
    ...
    #     df = groups[groups.group_type == row.group_type]
    #     df["group_code"] = df.group_code.apply(lambda code: f'<a href="/app/group_summary?group_code={code}&env={env}">{code}</a>')
    #     table = webapp_utils.frame_to_html(df)
    #     data.append((row.group_type, row.group_type_desc, df, table))
