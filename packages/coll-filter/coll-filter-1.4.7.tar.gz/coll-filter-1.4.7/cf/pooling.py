...
class PoolMultiProcessor:
    ...
    def _cal_cf_batch_params(self, size: int, batch_size: int) -> Tuple[int, int]:
        ...
        # 计算每个作业的分割大小 split_size，确保每个作业可以进一步细分为更小的批次。
        if n_jobs > 1:
            split_size = math.ceil(size / n_jobs)
            batch_size = min(max(batch_size, split_size), batch_size << 2)
        else:
            batch_size = size
...
class PoolCollFilter(BaseCollFilter):
    ...
    def _cf(self, user_ids, similar_dict, num_recalls, cf_type: CFType, batch_size: int):
        ...
        threshold = batch_size if cf_type == CFType.UCF else 8192
        if not similar_dict:
        ...
        if user_items_len >= threshold:
            cf_result = self.processor.cf(self.user_item_ratings, user_items_list, similar_dict, num_recalls, cf_func, max(threshold, batch_size), self.verbose)
        else:
