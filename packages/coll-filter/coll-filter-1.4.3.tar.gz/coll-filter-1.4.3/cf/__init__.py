...
class CollFilter(Generic[U, T]):
    ...
    def __init__(self, data: Iterable[Tuple[U, T, float]], n_jobs=os.cpu_count(), row_unique=False, num_item_users=128,
        ...
        if n_jobs > 1:
            ...
            self._coll_filter = PoolCollFilter(data, n_jobs, row_unique, num_item_users, similar_fn, cache_similar, verbose)
        else:
            # 如果n_jobs等于1，使用单进程协作过滤实现
            from cf.base import BaseCollFilter
            self._coll_filter = BaseCollFilter(data, row_unique, num_item_users, similar_fn, cache_similar, verbose)
