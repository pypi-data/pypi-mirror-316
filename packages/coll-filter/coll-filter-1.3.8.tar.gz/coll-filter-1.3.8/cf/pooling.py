...
class PoolMultiProcessor:
    ...
    def __init__(self, n_jobs):
        ...
        self.num_executor = cpu_count if self.n_jobs >= cpu_count else self.n_jobs
    ...
    def cal_similar(self, dict1, items_list, cal_fn, similar_fn, verbose: bool):
        ...
        with ProcessPoolExecutor(max_workers=self.num_executor) as executor:
            results = [executor.submit(cal_fn, dict1, items_list[i:i+split_size], similar_fn, verbose)
    ...
    def cf(self, user_item_ratings, user_items_list, similar_dict, num_recalls, cf_fn, verbose: bool):
        ...
        with ProcessPoolExecutor(max_workers=self.num_executor) as executor:
            results = [executor.submit(cf_fn, user_item_ratings, similar_dict, user_items_list[i:i + split_size], num_recalls, verbose)
                    for i in range(0, size, split_size)]
