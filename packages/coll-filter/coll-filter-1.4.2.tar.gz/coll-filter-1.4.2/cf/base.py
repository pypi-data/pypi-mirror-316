...
class CollFilterHelper:
    ...
    """
    @staticmethod
    def _rating_user_cf(user_item_ratings, similar_dict, user_items_list, num_recalls, verbose: bool):
        if verbose:
    ...
    @staticmethod
    def _rating_item_cf(_user_item_ratings, similar_dict, user_items_list, num_recalls, verbose: bool):
        if verbose:
            logger.info(f"\t进程 <{os.getpid()}> 开始 处理 {len(user_items_list)} 条记录...")
        start_time = time.perf_counter()
        result = {}
...
class BaseCollFilter(CollFilterHelper, Generic[U, T]):
    ...
    def __init__(self, data: Iterable[Tuple[U, T, float]], row_unique=False, num_user_items=64,
        ...
        self.item_users = dict([(item_id, [user[0] for user in list(sorted(users.items(), key=lambda x: x[1], reverse=True))[:num_user_items]]) for item_id, users in item_user_ratings.items()])
    ...
    def user_cf(self, num_recalls=10, num_similar=256, user_ids=None, user_similar=None, similar_fn=None, batch_size: int = 2024):
        """
        用户协同过滤
        ...
        """
        user_similar = user_similar or self.cal_similar(CFType.UCF, num_similar, similar_fn, batch_size)
        return self._cf(user_ids, user_similar, num_recalls, CFType.UCF, batch_size)

    def item_cf(self, num_recalls=10, num_similar=256, user_ids=None, item_similar=None, similar_fn=None, batch_size: int = 2024):
        """
        物品协同过滤
        ...
        @return {user_id: [(item, score),],}
        """
        item_similar = item_similar or self.cal_similar(CFType.ICF, num_similar, similar_fn, batch_size)
        return self._cf(user_ids, item_similar, num_recalls, CFType.ICF, batch_size)
    ...
    @staticmethod
    def _cal_ucf_similar(dict1: Mapping, items_list: list[Iterable], similar_func, verbose: bool):
        if verbose:
    ...
    @staticmethod
    def _cal_icf_similar(dict1: Mapping, items_list: list[Iterable], similar_func, verbose: bool):
        if verbose:
            logger.info(f"\t进程 <{os.getpid()}> 开始 处理 {len(items_list)} 条记录...")
        start_time = time.perf_counter()
        similar = defaultdict(dict)
    ...
    def _cf(self, user_ids, similar_dict, num_recalls, cf_type: CFType, batch_size: int):
        ...
        if cf_type == CFType.UCF:
            cf_result = self._rating_user_cf(self.user_item_ratings, similar_dict, user_items_list, num_recalls, self.verbose)
        else:
            cf_result = self._rating_item_cf(self.user_item_ratings, similar_dict, user_items_list, num_recalls, self.verbose)
