...
c	supply values of some fixed parameters here
      ...
      enddo
      read(1,*)
      read(1,'(a)')file_rf_out
      read(1,*)
      read(1,'(a)')file_stat_out
C       if(ival.ne.0)then
C          read(1,*)
C 	     Read(1,'(a)')filet
C       endif
      ...
      if(nstn.ge.nstnmax)then
      write(*,*)'Exceeded max No. of station specified : ',nstnmax-1
      stop
      endif

      if(nstn.eq.0)idrop=0
...
C          open(98,file='drop.out',status='old')
C          do kk=1,idrop
C              read(98,*)i1,idr(kk)
C          enddo
C          close(unit=98)
...
c     read nearest 'nstn' no of stations from the target location
 17   open(20,file=nearby,status='old')
      read(20,*)
      read(20,*)
      read(20,*)
      read(20,*)
      ...
      read(20,*)
      do i=1,nstn
          read(20,*)xi,idx(i),w(i),nyrs(i),nstrt(i),avrf(i)
      enddo
      close(unit=20)	  
...
c	form a moving window for each day and calculate probabilities, mean sd and serial corrl
      ...
      open(2,file=file_rf_out)
C       if(ival.gt.0)call stat_h(rfh,1,nsh,nyh,rain,nout,nday,isn)
      write(*,*)'Simulating rainfall.......'
...
C       subroutine station(kk,idr,tlat,tlon,tele,tdcoast,ttemp,
C      1               tanrf,idx,w,nstn,nsmax,nstnmax)
C       real*4 lat(nsmax),lon(nsmax),ele(nsmax),dcoast(nsmax),
C      1       temp(nsmax),avrf(nsmax),w(nstnmax),
C      2       aa(50),air(nsmax),dlatlon(nsmax)
C       integer indx(nsmax),ir(nsmax),ilat(nsmax),ilon(nsmax),
C      1     iele(nsmax),idcoast(nsmax),iavrf(nsmax),idx(nstnmax),
C      2     idlatlon(nsmax),idr(nsmax),itemp(nsmax)
       
C       open(1,file='stn_record.dat',status='old')
C       read(1,*)
C       read(1,*)
C       ii=0
C       do i=1,50000
C       read(1,10,end=11)i1,iy,id,tt,alt,aln,aele,adcoast,atemp,
C      1    (aa(j),j=1,16),anrf
C  10	  format(2i5,i10,a50,2f10.4,20f10.2)

C       if(kk.gt.0)then
C       do k1=1,kk
C       if(idr(k1).eq.id)goto 12
C       enddo
C       endif
C c	exclude the target location and the station to be dropped
C       if(alt.ne.tlat.and.aln.ne.tlon)then
C       ii=ii+1
C       ele(ii)=abs(aele-tele)
C       lat(ii)=abs(alt-tlat)
C       lon(ii)=abs(aln-tlon)
C       avrf(ii)=abs(anrf-tanrf)
C       temp(ii)=abs(atemp-ttemp)
C       dcoast(ii)=abs(tdcoast-adcoast)*2/(tdcoast+adcoast)
C       dlatlon(ii)=lat(ii)*lon(ii)
C       indx(ii)=id
C       endif
C  12	  continue
C       enddo
...
C c	rank nearest station for each physiographic attribute
C       call ranks(ele,iele,ii,nsmax)
C       call ranks(lat,ilat,ii,nsmax)
C       call ranks(lon,ilon,ii,nsmax)
C       call ranks(avrf,iavrf,ii,nsmax)
C       call ranks(dcoast,idcoast,ii,nsmax)
C       call ranks(dlatlon,idlatlon,ii,nsmax)
C       call ranks(temp,itemp,ii,nsmax)
C       air=0.0
C c combine nearest neighbours under each attribute
C c	lat 1.0, lon 0.25, ele 1.0, avrf 0.5, dcoast 0.5
C       do i=1,ii
C       air(ilat(i))=air(ilat(i))+float(i)
C       air(ilon(i))=air(ilon(i))+float(i)*0.5
C       air(iele(i))=air(iele(i))+float(i)*0.1
C       air(iavrf(i))=air(iavrf(i))+float(i)*0.5
C       air(idcoast(i))=air(idcoast(i))+float(i)*0.1
C       air(idlatlon(i))=air(idlatlon(i))+float(i)
C 	  air(itemp(i))=air(itemp(i))+float(i)*0.25
C       enddo
      
C       call ranks(air,ir,ii,nsmax)

C c	calculate stations weights on the basis of total ranking
C       sum=0.0
C       do i=1,nstn
C       w(i)=1.0/air(i)
C       sum=sum+w(i)
...
C       idx(i)=indx(ir(i))
C       enddo
C c	write(*,*)(idx(j),j=1,nstn)
C c	write(*,*)(w(j),j=1,nstn)
C c	pause
...
c      Implicit double precision (A-H,O-Z) 
     ...
     1       pcovl(2,nvmax,nvmax,nstnmax)
      ...
      write(*,*)'Calculating means and variances of long term variables'
...
c      Implicit double precision (A-H,O-Z) 
      ...
      write(*,*)'Storing variables for rf_amount'
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
C       subroutine stat_h(rf,nstn,ns,nyear,rain,nout,nday,isn)
C       include 'para.inc'
C       include 'stat.inc'
C       real*4 rf(nstnmax,nyrmax,monmax,ndmax)
C       integer nday(monmax)
C c	subroutine to calculate statistics
C       nh=7
C       call sur(rf,nyear,nstn,spellh,rfsplh,rfspnh,rfspndh,
C      1               solh,nout,nday,ns,rain,wmaxh,dmaxh)
C       call dmon(rf,nyear,nstn,rwavsh,rwsdsh,rfavmh,rfsdmh,
C      1          rfavsh,rfsdsh,roavmh,rosdmh,roavsh,rosdsh,nout,nday,
C      2          ns,rain,isn)
C       call percentile(rf,nyear,nstn,nout,nday,hperh,rain,isn,ns)
C       call rank_stat(spellh,nh,nyear,nstn)
C       call logodd(rf,nstn,nyear,crsodh,nday,ns,nout,rain,isn)
C       call crscors(rf,nyear,nday,ns,nstn,crsoh,nout,rain,isn,0)
C       call crscors(rf,nyear,nday,ns,nstn,crsrh,nout,rain,isn,1)
C       call crscor(rf,nyear,nday,ns,nstn,crsrdwh,nout,rain,isn,1)
C       call crscor(rf,nyear,nday,ns,nstn,crsrdah,nout,rain,isn,2)
C       return
C       end
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c            gen=gen*tavrf/avrf(nk)
c            if(gen.lt.rain)gen=rain
         else
             gen=0.0
         endif
      endif
      
c	  Only Recording wet dry state
 ...
 902  continue
      ...
      enddo
C       if(it.le.3)write(*,703)i,(sumho(is),sumg(is),sumhr(is),sumgr(is),
C      1           is=1,isn)
           enddo

c     just for last day repeat previous value 
	  if(iamt.eq.1)then
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
C       subroutine readdatah(fpath,rfh,ff,nday,nyear,nstart,nout,vrf)
C c      Implicit double precision (A-H,O-Z) 
C       include  'para.inc'
C c	subroutine to read the daily rainfall data from a file and
C c	if required  convert it into rainfall occurrances and
C c	 also put value of variable rain as one after transformation
C       real*4 rfh(nstnmax,nyrmax,monmax,ndmax)
C       integer nday(monmax)
C       character*80 ff,hh,fpath

C c	read daily rainfall
C       hh=' '
C       ii=0
C       do i=1,80
C       if(fpath(i:i).ne.' ')then
C       ii=ii+1
C       hh(ii:ii)=fpath(i:i)
C       endif
C       enddo
...
C       ind=0
C       do i=1,1000000000
C       read(1,*,err=107,end=302)iy,i2,i3,bb
C       if(i2.eq.1.and.i3.eq.1.and.i2.eq.1.and.ind.eq.0)then
C       nstart=iy
C       ind=1
C       ny=nstart-1
C       endif
C       if(iy.ge.nstart)then
C c	check for assigning the sequence of a year
C       if(iy.gt.ny)then
C c	write(*,*)'....... year ',ny, i1
C       i1=i1+1
C       ny=iy
C       endif
C       rfh(1,i1,i2,i3)=bb
...
C         enddo
C  302    close(unit=1)
C       if(i2.ne.nout.and.i3.ne.31)i1=i1-1
C       nyear=i1

C c	calculate average annual rainfall
C       vrf=0.0
C       do i=1,nyear
C       do j=1,nout
C       nd=nday(j)
C       if(j.eq.2)call daycount(nstart-1,i,nd)
C       do l=1,nd
C       vrf=vrf+rfh(1,i,j,l)
C       enddo
C       enddo
C       enddo
C       vrf=vrf/float(nyear)
C       return
C 100	  format(a)
C 107	  write(*,*)'Error in data file'
C       write(*,*)'year month day ',iy,i2,i3
C c	stop
C       return
C       end
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
C       subroutine find_anrf(alat,alon,anrf,nsmax)
C       real*4 rain(nsmax*nsmax),diff(nsmax*nsmax)
C       real*4 lat,lon,w(10)
C       open(19,file='raingrid.out',status='old')
C       ncols=1681
C       nrows=1361
C       ii=0
C       do i=1,nrows
C       do j=1,ncols
C       read(19,*)i1,j1,lat,lon,rr
C       dd=abs(alat-lat)+abs(alon-lon)*0.5
C       if(dd.gt.1.0)goto 10
C       ii=ii+1
C       diff(ii)=dd
C       rain(ii)=rr
C 10    continue
...
C       anrf=0.0
C       do i=1,6
C c	write(*,*)i,diff(i),rain(i),w(i)/sum
C       anrf=anrf+rain(i)*w(i)/sum
C       enddo
C       return
C       end
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
