...
class TkIcons():
    """TkIcons - Stellt Icons im Format ImageTk.PhotoImage zur Verfügung
        ...
        Zugriff auf die Icons gibt es auf zweierlei Weise:
            ...
            Es wird also der Anteil aus ICONS_DIR und die Datei-Endung unterdrückt.
    ...
    def __init__(self):
        ...
        #
        # ICONS_DIR scannen
        for pfad, unterordner, dateien in os.walk(ICONS_DIR):
...
class DialogHilfeNaviButtons(tk.Toplevel):
    def __init__(self, parent):
        ...
        ttk.Label(self, text='Formular leeren für neuen Datensatz').grid(row=1, column=1, sticky=tk.W)
        
        ttk.Label(self, image=glb.icons.save).grid(row=2, column=0)
        ttk.Label(self, text='Daten sichern und weiter bearbeiten').grid(row=2, column=1, sticky=tk.W)
        
        ttk.Label(self, image=glb.icons.save_clear).grid(row=3, column=0)
        ttk.Label(self, text='Daten sichern').grid(row=3, column=1, sticky=tk.W)
        
        ttk.Label(self, image=glb.icons.delete).grid(row=4, column=0)
        ttk.Label(self, text='Datensatz löschen').grid(row=4, column=1, sticky=tk.W)
        
        ttk.Label(self, image=glb.icons.refresh).grid(row=5, column=0)
        ttk.Label(self, text='Daten sichern und weiter bearbeiten').grid(row=5, column=1, sticky=tk.W)
        
        ttk.Label(self, image=glb.icons.undo).grid(row=6, column=0)
        ttk.Label(self, text='Änderungen verwerfen').grid(row=6, column=1, sticky=tk.W)
        
        ttk.Label(self, image=glb.icons.copy).grid(row=7, column=0)
        ttk.Label(self, text='Kopie des Datensatzes anlegen').grid(row=7, column=1, sticky=tk.W)
...
class BasisFormListe():
    """BasisFormListe - Grundlage für Listenansicht und -bearbeitung von Datensätzen
        ...
        FormListe hält für jeden (relevanten, anzuzeigenden) Datensatz aus einem
