...
class TextCNN2D(Layer):
    ...
    - **kwargs: 其他传递给 Layer 类的参数。
    """
    def __init__(self, units: int, embed_dim: int = 1, filters: int = 32, kernel_sizes=(2, 3, 4), hidden_size: int = 128, activation='softmax', **kwargs):
        super().__init__(**kwargs)
        if embed_dim <= 1:
...
class TextCNN2DModel(Model):
    ...
    def __init__(self,units: int, embed_dim: int = 1, filters: int = 32, kernel_sizes=(2, 3, 4), hidden_size: int = 128, activation='softmax', **kwargs):
        super().__init__(**kwargs)
        self.model = TextCNN2D(units, embed_dim, filters, kernel_sizes, hidden_size, activation=activation)
