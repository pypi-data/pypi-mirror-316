"""
Nome do Módulo: etl_sap_order.py
...
from .model.orders import Order, OrderItem  # Ajuste a importação para o novo modelo
...
def run_pipeline(**kwargs):
    ...
    for page in requested_data:
        orders = page.get("value", [])

        if len(orders) > 0:
            with Session(DWE.engine) as db_session:

                for order in orders:
                    order_data = {k: v for k, v in order.items() if k not in "odata.etag"}

                    # Inserindo a ordem na tabela 'Order'
                    order_obj = Order(**order_data)
                    db_session.merge(order_obj)

                    # Processando itens relacionados à ordem
                    if 'DocumentLines' in order:  # Certificando-se de que 'order_items' existe
                        document_line = order['DocumentLines']
                        for item in document_line:
                            item['DocNum'] = order['DocNum']  # Garantir que DocNum seja passado para OrderItem
                            item_obj = OrderItem(**item)  # Usando o dicionário diretamente
