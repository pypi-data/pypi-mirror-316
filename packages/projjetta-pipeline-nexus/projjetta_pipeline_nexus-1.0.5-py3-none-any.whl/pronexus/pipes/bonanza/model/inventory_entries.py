...
class InventoryEntry(SBB):
    __tablename__ = 'sap_inventory_entry'
    ...
    inventory_item = relationship("InventoryEntryItem", back_populates="inventory")

    def __repr__(self):
        return f"<entry(inventory_doc='{self.DocNum}', card_code='{self.CardCode}', doc_total='{self.DocTotal}')>"


class InventoryEntryItem(SBB):
    __tablename__ = 'sap_inventory_entry_item'

    id = Column(Integer, primary_key=True, autoincrement=True)
    DocNum = Column(Integer, ForeignKey(f'{SBB.__table_args__.get("schema")}.sap_inventory_entry.DocNum'))
    ...
    inventory = relationship("InventoryEntry", back_populates="inventory_item")
