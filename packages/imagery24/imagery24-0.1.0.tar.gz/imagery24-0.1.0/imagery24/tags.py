...
def get_metadata(file_path: Path) -> list[dict[bytes, int | str]]:
    result = [dict()]
    level = 0
    ...
    while offset < max_offset:
        for tag in KNOWN_TAGS:
            ...
            # If no tag is matched, increment offset to avoid an infinite loop
            offset += 1
...
if __name__ == "__main__":
    ...
    parsed_data = get_metadata(file_path)
