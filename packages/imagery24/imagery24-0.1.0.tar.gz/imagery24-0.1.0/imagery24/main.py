...
from imagery24.georeference import georeference_image
...
def convert(input_file: Path | str, output_file: Path | str):
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        extract_webp_images(input_file, tiles, temp_dir)
        ...
        corner_coords = [
            tuple(map(float, coord.split(","))) for coord in corner_coords_str.split(" ")
        ]
        corner_coords = corner_coords[:4]

        tiff_path = create_tiff(temp_dir, levels_bounds, max_zoom, width, height)

        georeference_image(tiff_path, corner_coords, output_file)
