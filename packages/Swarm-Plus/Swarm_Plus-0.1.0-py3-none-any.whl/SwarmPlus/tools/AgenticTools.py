...
from SwarmPlus.models import OpenAIVissionModel
...
class DescribePlots(BaseModel):
    ...
    def run(self):
        ...
        try:
            ...
            return "All the visual insights are stored in visual_insights.txt file",self.context_variables

        except Exception as e:
            return f"An error occurred while executing the file: {str(e)}",self.context_variables
        
# Image Generator

class ImageGenerator(BaseModel):

    "Use this tool to create an image based on some description"

    reasoning :str = Field(description="Why you are using this tool")

    image_description: str = Field("Describe the image you want")

    target_folder : str =Field(description="Provide the folder path to store the images")
    ...
    def run(self):
        ...
        if image_response.status_code == 200:
            ...
            return f"Image saved successfully at : {self.target_folder}/{self.file_name}",self.context_variables
        else:
           return "Failed to create the image.",self.context_variables