...
class RunPythonFile(BaseModel):
    ...
    def run(self) -> Dict[str, Any]:
        if not self.file_name.endswith('.py'):
            return {"status": "error", "message": f"Error: The file '{self.file_name}' does not have a .py extension. Only Python files are allowed."},self.context_variables
        
        return self.run_python_file(),self.context_variables
...
class InstallPythonPackage(BaseModel):
    ...
    def run(self):
        try:
            ...
            return f"Package '{self.package_name}' installed successfully. Output:\n{result.stdout}",self.context_variables
        except subprocess.CalledProcessError as e:
            error_message = e.stderr.strip().split('\n')[-1]  # Capture the exact error message
            return f"An error occurred while installing the package '{self.package_name}': {error_message}",self.context_variables
        except Exception as e:
            return f"An unexpected error occurred: {str(e)}",self.context_variables
...
class LoadExcelFile(BaseModel):
    ...
    def run(self):
        ...
        if not (self.file_name.endswith('.csv') or self.file_name.endswith('.xlsx')):
            return f"Error: The file '{self.file_name}' does not have a .csv or .xlsx extension. Only CSV and Excel files are allowed.",self.context_variables
        ...
        try:
            ...
            return f"First 5 rows of the file '{self.file_name}':\n\n{first_5_rows}",self.context_variables
        except Exception as e:
            return f"An error occurred while loading the file: {str(e)}",self.context_variables
