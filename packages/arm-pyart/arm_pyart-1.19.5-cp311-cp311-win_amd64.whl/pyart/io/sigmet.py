    ...
    **kwargs
    ...
    if time_ordered == "sequential":
        ...
        if task_config["task_scan_info"]["antenna_scan_mode"] == 2:
            ...
            # PPI scan
            if _is_time_ordered_by_roll(sigmet_data, sigmet_metadata, rays_per_sweep):
                time_ordered = "roll"
    ...
    dts = [
        ymds_time_to_datetime(d["sweep_start_time"])
    ...
    gate_0 = range_info["first_bin_range"] / 100.0  # meters
    gate_nbin = range_info["last_bin_range"] / 100.0  # meters
    ...
    metadata["sigmet_task_name"] = sigmetfile.product_hdr["product_configuration"][
        "task_name"
    ]
    ...
    beam_width_v["data"] = np.array(
        [bin4_to_angle(task_config["task_misc_info"]["vertical_beamwidth"])],
        dtype="float32",
    )
    pulse_width["data"] = np.array(
        [task_config["task_dsp_info"]["pulse_width"] * 1e-8] * len(time["data"]),
        dtype="float32",
    )
