...
def mk_marked_formula_over_ranges(ranges: list[list[GradableNode]],
    ...
    for i, local_range in enumerate(ranges):
        ...
        local_formula = f'SUMIF({rg},{crit},INDEX({GK_INFO_TABLE_NAME}[[{start_col}]:[{end_col}]],1,0))'
        sub_formulas.append(local_formula)
        cns = (mk_node_name(n, upper_label_level) for n in local_range)
        ifs = (f'IF({mk_is_checked_formula(c, config["CHECKBOX_SYMBOL"])},INDEX({GK_INFO_TABLE_NAME}[[{c}]],1),0)'
               for c in cns)
        sub_sums.extend(ifs)
