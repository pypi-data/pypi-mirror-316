...
def join_moodle_and_scores(grading_file: str, moodle_file: str, group_name_fmt: str,
    ...
    moodle_df = read_moodle_csv(moodle_file)
    ...
    return merged_df.loc[rows_with_total_score]


# unused
def join_and_create_overview(output_file: str, naming_dictionary: NamingDictionary, **config):
    updated_df = join_moodle_and_scores(naming_dictionary=naming_dictionary, **config)
...
def join_and_export(output_file: str, grading_column: str, feedback_column: str, naming_dictionary: NamingDictionary,
                    **config):
    updated_df = join_moodle_and_scores(grading_column=grading_column, feedback_column=feedback_column,
                                        naming_dictionary=naming_dictionary, **config)

    # decimal representation wrangling
    stringified_scores_col = updated_df[grading_column].apply(
        lambda x: f'{x:.2f}'.replace('.', naming_dictionary.MOODLE_CSV_DECIMAL_SEP))
    ...
    if len(overwritten_rows) > 0:
        if config['verbose']:
            print(f'Listing overwritten rows:')
...
def configure_parser(parser: argparse.ArgumentParser, naming_dictionary: NamingDictionary, **defaults):
    ...
    parser.add_argument('-m', '--moodle', required=True, help='Path to moodle grading im/export csv file.',
                        dest='moodle_file')
    ...
    parser.add_argument('-gfmt', '--group-name-fmt', required=True,
                        help='Format str of the group names used in moodle group creation.',
                        default=defaults['GROUP_NAME_STR_FORMAT'])
...
class GenGroupUpload(CommandModule):
    ...
    def additional_config(self) -> dict[str, Any]:
        return {'GROUP_NAME_STR_FORMAT': 'Group A2 {:02}', 'USED_JOIN_COL': 'MOODLE_GROUP_COL',
