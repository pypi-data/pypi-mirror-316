...
```"""
    try:
        answer = CallOpenAIAzure.getSingleChatResponse(content, temperature=0.0, keepSystemMessage=True)
        if not answer:
            answer = "high"
...
def getSingleFunctionCallResponse(messages: list[dict], function_name: str, temperature=None, **kwargs):
    ...
    function_call_response = CallOpenAIAzure.finetuneSingleFunctionCallResponse(func_arguments, function_name)
    return function_call_message_mini, function_call_response


class CallOpenAIAzure:
    ...
    def autoCorrectPythonCode(code, trace):
        for i in range(config.max_consecutive_auto_correction):
            ...
            function_call_message, function_call_response = CallOpenAIAzure.getSingleFunctionCallResponse(messages, "correct_python_code")
        ...
        if not config.selectedTool:
            ...
            try:
                if not tool_schema["parameters"]["properties"]:
                    ...
                    tool_parameters = CallOpenAIAzure.getDictionaryOutput(messages=messages, schema=tool_schema)
    ...
    def extractToolParameters(schema: dict, ongoingMessages: list = [], **kwargs) -> dict:
        ...
        """
        parameters = CallOpenAIAzure.getDictionaryOutput(messages=ongoingMessages, schema=schema, **kwargs)
        if config.developer:
            print2("```parameters")
