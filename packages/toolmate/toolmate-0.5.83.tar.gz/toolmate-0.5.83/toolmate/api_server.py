...
class Request(BaseModel):
    ...
    groupchatdocker: bool | None = None
    groupchatexecutiontimeout: int | None = None
    groupchatoaia: bool | None = None
...
async def process_instruction(request: Request, api_key: str = Depends(get_api_key) if config.toolmate_api_server_key else ""):
    ...
    groupchatdocker = request.groupchatdocker
    groupchatexecutiontimeout = request.groupchatexecutiontimeout
    ...
    if groupchatdocker:
        current_groupchatdocker = config.code_execution_use_docker
        config.code_execution_use_docker = groupchatdocker
    if groupchatoaia:
    ...
    if groupchatexecutiontimeout:
        current_groupchatexecutiontimeout = config.code_execution_timeout
        config.code_execution_timeout = groupchatexecutiontimeout
    if groupchatrounds:
    ...
    if backupsettings:
        ...
        if groupchatdocker:
            config.code_execution_use_docker = current_groupchatdocker
            print3(f"Group chat code execution in docker restored: {current_groupchatdocker}")
        if groupchatexecutiontimeout:
            config.code_execution_timeout = current_groupchatexecutiontimeout
            print3(f"Group chat timeout for each code execution restored: {current_groupchatexecutiontimeout}")
        if groupchatoaia:
            config.use_oai_assistant = current_groupchatoaia
            print3(f"OpenAI assistant option restored: {current_groupchatoaia}")
        if groupchatagents:
            config.max_agents = current_groupchatagents
            print3(f"Group chat maximum agents restored: {current_groupchatagents}")
        if groupchatrounds:
            config.max_group_chat_round = current_groupchatrounds
            print3(f"Group chat maximum rounds restored: {current_groupchatrounds}")
        if imagehd:
