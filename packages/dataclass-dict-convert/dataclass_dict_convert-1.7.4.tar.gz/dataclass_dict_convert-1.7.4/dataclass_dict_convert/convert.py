...
def _is_any(a_type: type):
    try:
        return a_type is Any
    except TypeError:
        # could be prevented by checking inspect.isclass(a_type)
        return False
...
def _find_convertor(
    ...
    try:
            ...
            if not hasattr(field_type, '__args__') or len(field_type.__args__) != 2 or \
                    ...
                    def dict_handler(d):
                        if not isinstance(d, Mapping):
                            raise DataclassConvertError(
                                f"field {field_name!r} of expected type dict is a {type(d)!r} instead")
                        return dict(d)
                    return dict_handler
                else:
                    return lambda d: d
                ...
                if is_from:
                    def dict_handler(d):
                        if not isinstance(d, Mapping):
                            raise DataclassConvertError(
                                f"field {field_name} of expected type dict is a {type(d)} instead")
        ...
        if _is_any(field_type):
            # Any only means anything that JSON can DIRECTLY represent (= Dict, List, float, int, bool and str)
            if is_from:
