    ...
    def _create_galleries_infos_view(self) -> None:
        ...
        with self.SQLConnector() as connector:
            match self.config.database.sql_type.lower():
                case "mysql":
                    query = """
                        CREATE VIEW IF NOT EXISTS some_duplicate_files_in_galleries AS
                        SELECT DISTINCT(galleries_gids.db_gallery_id) AS db_gallery_id,
                            duplicate_gids.gid AS gid,
                            galleries_names.full_name AS gallery_name
                        FROM (
                                SELECT gid
                                FROM (
                                        SELECT galleries_gids.gid AS gid,
                                            files_dbids.db_gallery_id AS db_gallery_id,
                                            files_dbids.name_part1 AS file_name,
                                            files_hashs_sha512.db_hash_id AS sha512
                                        FROM files_dbids
                                            INNER JOIN files_hashs_sha512 ON files_dbids.db_file_id = files_hashs_sha512.db_file_id
                                            INNER JOIN galleries_gids ON galleries_gids.db_gallery_id = files_dbids.db_gallery_id
                                    ) AS files
                            ...
                            INNER JOIN galleries_gids ON duplicate_gids.gid = galleries_gids.gid
                            INNER JOIN galleries_names ON galleries_gids.db_gallery_id = galleries_names.db_gallery_id
                    """
            connector.execute(query)
