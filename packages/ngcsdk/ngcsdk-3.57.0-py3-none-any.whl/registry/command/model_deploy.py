...
class ModelDeployCommand(ModelSubCommand):
    """Command tree for all notebook deployments of models."""

    CMD_NAME = "deploy"
    HELP = "Manage interactive container deployments for models."
    DESC = "Manage interactive container deployments for models."
    ...
    start_help = (
        "Start interactive deployment of a model from the NGC catalog to a cloud service provider (CSP). "
    ...
    MODEL_AND_VERSION_METAVAR = "<org>/[<team>/]<model_name>:<version>"
    CSP_METAVAR = "<csp>"
    GPU_METAVAR = "<gpu>"
    ...
    @CLICommand.arguments("target", metavar=MODEL_AND_VERSION_METAVAR, help=deploy_target_help, type=str)
    ...
    def start(self, args):
        ...
        deploy_response = self.deploy_api.start(
            mrt.name, args.csp, deployment_request, "models", mrt.version, mrt.org, team=mrt.team
        )
        self.printer.print_deployment_url_response(deploy_response)
        if args.dry_run:
            file_list = self.get_version_files(args.target, mrt.org, mrt.team)
            self.model_printer.print_file_list(file_list)
        else:
            webbrowser.open(deploy_response.deploymentUrl)

    create_deploy_help = (
        "Create the default deployment parameter set for a model deployment. For each flag parameter not provided, "
        "the CSP default will be used."
    )
    create_target_help = "Model to use for deployment."
    ...
    @CLICommand.arguments("target", metavar=MODEL_NO_VERSION_METAVAR, help=create_target_help, type=str)
    ...
    def create(self, args):
        ...
        self.deploy_api.create(mrt.org, mrt.name, "models", args.csp, create_request, team=mrt.team)
        self.printer.print_ok(f"Successfully created {args.csp} deployment parameters for {args.target}.")

    REMOVE_DEPLOY_HELP = "Delete deployment parameters for a model and unmark as being deployable."
    remove_target_help = "Model whose deployment parameters should be removed."

    @CLICommand.command(help=REMOVE_DEPLOY_HELP, description=REMOVE_DEPLOY_HELP)
    @CLICommand.arguments("target", metavar=MODEL_NO_VERSION_METAVAR, help=remove_target_help)
    @CLICommand.arguments("csp", metavar=CSP_METAVAR, help=csp_help, type=str)
    ...
    def remove(self, args):
        ...
        self.deploy_api.remove(mrt.org, mrt.name, "models", args.csp, team=mrt.team)
        self.printer.print_ok(f"Successfully removed '{args.csp}' deployment for model {args.target}.")

    info_deploy_help = "Get the default deployments parameters for a CSP deployment of an model."
    ...
    @CLICommand.arguments("target", metavar=MODEL_NO_VERSION_METAVAR, help="Model to lookup.", type=str)
    ...
    def info(self, args):
        ...
        self.config.validate_configuration()
        mrt = ModelRegistryTarget(args.target, org_required=True, name_required=True, version_allowed=False)
        deploy_params = self.deploy_api.info(mrt.org, mrt.name, "models", args.csp, team=mrt.team, inherit=args.inherit)
        self.printer.print_deployment_parameters(args.target, deploy_params)

    update_deploy_help = "Update the default deployment parameter set for a model and CSP."
    ...
    def update(self, args):
        ...
        self.deploy_api.update(mrt.org, mrt.name, "models", args.csp, update_request, team=mrt.team)
    ...
    def list(self, args):
        ...
        responses = self.deploy_api.list(mrt.org, mrt.name, "models", team=mrt.team)
