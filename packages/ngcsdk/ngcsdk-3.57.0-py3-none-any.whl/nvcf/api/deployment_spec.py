...
class TargetedDeploymentSpecification:
    ...
    def from_str(cls: Type[T2], in_str: str) -> T2:
        ...
        The string must be in the following form:
        gpu:instanceType:minInstances:maxInstances[:maxRequestConcurrency][:region_1,region_2][:attribute_1,attribute_2][:preferredOrder]
        ...
        try:
            ...
            regions = values[5].split(",") if len(values) > 5 else []
            attributes = values[6].split(",") if len(values) > 6 else []
            preferred_order = int(values[7]) if len(values) > 7 else 1
            ...
            raise NgcException(
                ...
                "[:maxRequestConcurrency][:region_1,region_2][:attribute_1,attribute_2][:preferredOrder]."
            ) from None
