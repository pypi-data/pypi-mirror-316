...
class NetworkSync(BaseModel):
    ...
    def _get_ipv4_nets(self) -> ManagedNetworks:
        ...
        for net in self.ipf.technology.addressing.managed_ip_ipv4.all(
            ...
            if devs[net["sn"]]["vendor"] in ["aws", "azure", "gcp"]:
                continue  # TODO: Do we want to exclude Cloud?
    ...
    def map_net_view(self):
        for network in self._networks.networks:
            ...
            for view in self.config.pyt[net]:
                if (
                    self._is_site_included(network.site_name, view)
                    and self._is_vrf_included(network.vrf, view)
                    and self._is_net_allowed(net, self.config.view_dict[view])
                ):
                    tmp.add(view)
            self._check_views(
                network,
    ...
    def _map_view(self, mapping, attr, error_gt_1: str, error_eq_0: str, success: str):
        for network in self._networks.networks:
            if network.net_view:
                continue
            elif attr == "vrf" and not getattr(network, attr):
                network.error.no_matching_view = True
                network.error.message.append("Network has no VRF assigned.")
                continue
            tmp = set()
    ...
    def map_default_view(self):
        ...
        for network in self._networks.networks:
            ...
            if (
                ...
                and (network.vrf in ["", None] or self._is_vrf_included(network.vrf, default.name))
