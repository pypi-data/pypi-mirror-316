...
from ipfabric_infoblox.ipf_models.managed_ip import VRF, VLAN, Interface, SNMP, ManagedIP
...
class IPSync(BaseModel):
    ...
    def build(self) -> list[ManagedIP]:
        ...
        managed_ips = list()
        for ip in self.ipf.technology.addressing.managed_ip_ipv4.all(filters={"net": ["empty", False]}):
            managed_ips.append(
                ManagedIP(
