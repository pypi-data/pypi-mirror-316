...
class Configuration(BaseModel):
    ...
    def check_view_names(cls, data: Any) -> Any:
        if isinstance(data, dict) and "networkViews" in data:
            ...
            assert len(view_names) == len(set(view_names)), "Duplicate View Names found."
        return data
    ...
    @property
    def default_view(self) -> NetworkView:
        return [_ for _ in self.network_views if _.default][0]
        ...
        for view in self.network_views:
            for network in view.include.networks:
                if self._pyt.has_key(network):
                    ...
                    self._pyt[network].append(view.name)
                else:
                    self._pyt[network] = [view.name]
