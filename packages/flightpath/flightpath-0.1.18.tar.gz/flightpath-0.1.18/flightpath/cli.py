...
@click.option("--end-task-id", required=True, help="ID of the task from which to trace the critical path")
@click.option(
    "--end-dag-id",
    help="ID of the dag from which to trace the critical path",
)
@click.option("--dag-run-id", required=True, help="DAG run ID in which to trace the critical path")
@click.option(
    "-e", 
    "--external",
    is_flag=True,
