...
class AirflowExternalTaskSensor(AirflowTaskInstance):
    ...
    def get_upstream_task_instances(self) -> Collection["TaskInstance"]:
        logger.info(f"  Retrieving upstream information for ExternalTaskSensor task {self.task_id} in dag {self.dag_id}")
        ...
        response = self.airflow_client.session.get(task_instance_endpoint, auth=self.airflow_client.auth)
...
class AirflowClient(Client):
    ...
    def get_task_instance(self, dag_id: str, task_id: str, dag_run_id: str) -> TaskInstance:
        ...
        logger.debug(f"Fetching task {task_id} in dag {dag_id} for run id {dag_run_id} from: {endpoint}")
    ...
    def get_all_task_dependencies_for_dag(self, dag_id: str, dag_run_id: str) -> Mapping[Task, Collection[Task]]:
        ...
        logger.info(f"  Extracting all dependencies for dag {dag_id} from endpoint: {endpoint}")
...
class CriticalPathTracer:
    ...
    def trace(self, end_dag_id: str, end_task_id: str, dag_run_id: str, external: bool = False) -> TaskInstance:
        logger.debug(f"Tracing critical path starting from dag {end_dag_id} task {end_task_id} for run id {dag_run_id}")
        ...
        while True:
            ...
            if not external and previous_ti.dag_id != end_dag_id:
                logger.info(f"Halting trace because --external was not specified andthe previous task {previous_ti.dag_id}:{previous_ti.task_id} is not in the same DAG as the end task {end_dag_id}:{end_task_id}.")
                break

            if any(previous_ti == t for t in path):
                logger.info(f"Halting trace because I found a cycle in the path at task {previous_ti.dag_id}:{previous_ti.task_id}. This can happen when using ExternalTaskSensor with an execution_delta to wait on a previous DAG run.")
                break

            logger.info(f"Found previous task {previous_ti.dag_id}:{previous_ti.task_id}")
    ...
    def print_critical_path(root_ti: TaskInstance, print_url: bool):
        ...
        for ti in results:
            data.append([
                ...
                ti.get_url() if print_url else f"\033]8;;{ti.get_url()}\033\\LINK\033]8;;\033\\"
            ])
