...
def test_spell_check(known_words):
    ...
    source_filenames = (
        ...
        + list((Path(__file__).parents[1] / "colcon_package_of").glob("**/*.py"))
    ...
    spell_check(
        [str(p) for p in source_filenames],
    ...
    ), f"Found {unknown_word_count} unknown words: " + ", ".join(
        sorted(report.unknown_words)
    )

    unused_known_words = set(known_words) - report.found_known_words
    unused_known_word_count = len(unused_known_words)
    assert (
        unused_known_word_count == 0
    ), f"{unused_known_word_count} words in the word list are not used: " + ", ".join(
        sorted(unused_known_words)
    )


@pytest.mark.linter
def test_spell_check_word_list_order(known_words):
    assert known_words == sorted(
        known_words
