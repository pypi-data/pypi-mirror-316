...
class Jaka_Coms:
    ...
    def read_digital_modbus_output(self, output_number: int) -> bool:
        """
        Reads the state of a digital output (coil).

        :param output_number: The digital output number (1 to 128).
        :return: Boolean state of the output (True for ON, False for OFF).
        ...
        # Map output_number to Modbus address; adjust 39 based on your server configuration
        address = 39 + output_number
        response = self.client.read_coils(address=address, count=1)

        if response.isError():
            raise ModbusException(f"Error reading digital output {output_number} at address {address}: {response}")
