...
DataClassTestFactory = Callable[[bool, tuple], DataClassTestDef]
...
@fixture(scope="session")
def str_test_data(test_data) -> ToStr:
...
def dc_test_factory(test_data: Data) -> DataClassTestFactory:
    def dc_test(
        frozen: bool = False, protocols: tuple = tuple()
    ) -> DataClassTestDef:
        Base: TypeAlias = DataClass
        if frozen:
            Base = DataClassFrozen

        class C(Base, DataClassLoadProtocol, *protocols):
            i: int
        ...
        class E(D, X):
            ...
            t: tuple[str, ...] = ("meh",)
            game: set[str] = {"match"}
            dct: dict[str, str] = Base._field(default={"ho": "hum"})

        assert E._cm() is E
        assert (
            E._frozen is frozen
        ), f"{E._frozen} {Base.__dataclass_params__.frozen}"
        for f in E._fields():
