try:
    ...
    from . import handler_data_files
...
class ReaderProject:
    ...
    def anti_cipher_files(self, key: str):
        ...
        text, key, path = handler_object.handler_with_path()

        return text, key, path, handler_object

    def anti_cipher_folder(self, key: str, path: str):
        handler_object = handler_data_files.AntiCipherFile(files_path=self.all_files, key=key, root_dir=self.root_dir)
        text, key, path = handler_object.handler(path_to_save=path)
