...
class Worker:
    ...
    def cipher_file(self, new_path):

        reader_project = reader.ReaderProject(root_dir=self.root_dir)
        reader_project.get_files_from_directory()
        
        text, key, path = reader_project.cipher_files(key=self.key, new_path=new_path)

        return text, key, path


    def anti_cipher_folder(self, path: str):
        reader_project = reader.ReaderProject(root_dir=self.root_dir)
        reader_project.get_files_from_directory()

        text, key, path, handler_object = reader_project.anti_cipher_folder(key=self.key, path=path)
