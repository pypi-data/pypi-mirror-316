...
## input voltage array, (normalised) current array, Vdepl
def find_breakdown_and_current(voltage, current, Vdepl, Vop, is3D=False):
    Vbd = -999
    Ilc = 0
    for idx, V in enumerate(voltage):
        ...
        if is3D and current[idx] > current[idx - 5] * 2 and voltage[idx - 5] > Vdepl:
            Vbd = current[idx - 5]
        ...
        if current[idx] > current[idx - 1] * 1.2 and current[idx - 1] != 0:
            Vbd = V
...
def update_iv_cuts(
    ...
    iv_cuts["BREAKDOWN_VOLTAGE"][qc_layer]["sel"][0] += Vdepl
...
def get_nominal_current(layer, nchips, chip_type):
    # Get the default measurement config from module-qc-tools
    meas_config = json.loads((data / "configs" / "meas_config.json").read_text())
    check_layer(layer)

    if layer == "L0":
        nom_current = meas_config["tasks"]["GENERAL"]["i_config"][chip_type][layer] / 3
    else:
        nom_current = meas_config["tasks"]["GENERAL"]["i_config"][chip_type][layer] / 4

    return nom_current * nchips
...
def get_nominal_RextA(layer, BOM_type):
    check_layer(layer)
    RextA = {
        "RD53B": {"L0": 511, "L1": 732, "L2": 866},
        "ITKPIXV2": {"L0": 475, "L1": 715, "L2": 845},
    }
    return RextA[BOM_type][layer]


def get_nominal_RextD(layer, BOM_type):
    check_layer(layer)
    RextD = {
        "RD53B": {"L0": 407, "L1": 549, "L2": 590},
        "ITKPIXV2": {"L0": 383, "L1": 576, "L2": 576},
    }
    return RextD[BOM_type][layer]
...
def perform_qc_analysis_AR_VDD_Trim(_test_type, qc_config, _layer, results, key):
    ...
    rounded_results_list = [
        round(elem, qc_config.get("VDD_TRIM_2").get("precision")) for elem in results
    ]
...
def perform_qc_analysis(test_type, qc_selections, layer_name, results):
    ...
    for key in results:
        ...
        if isinstance(lower_bound, list) and isinstance(upper_bound, list):
            rounded_results_x = round(results.get(key)[0], precision)
            rounded_results_y = round(results.get(key)[1], precision)
            rounded_results = [rounded_results_x, rounded_results_y]
            rounded_results_dict.update({key: rounded_results})
            passes_qc_test = True
            if (
                rounded_results_x < lower_bound[0]
                or rounded_results_x > upper_bound[0]
                or rounded_results_y < lower_bound[1]
                or rounded_results_y > upper_bound[1]
            ):
                passes_qc_test = False
            rounded_results_output_format = (
                str(rounded_results[0]) + ", " + str(rounded_results[1])
            )
            lower_bound_output_format = str(lower_bound[0]) + ", " + str(lower_bound[1])
            upper_bound_output_format = str(upper_bound[0]) + ", " + str(upper_bound[1])
            if passes_qc_test:
                print_output_pass(
                    key,
                    rounded_results_output_format,
