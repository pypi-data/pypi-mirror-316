...
def get_qc_config(
    qc_criteria_path: Path | None = None, test_type: str | None = None, mod_SN=""
) -> dict[str, Any]:
    if qc_criteria_path is None:
        ...
        if chip_type == "ITKPIXV2":
            qc_criteria_path = Path(str(data / "analysis_cuts_V2chip_V1bom.json"))
            log.warning(
                bcolors.WARNING + "Using QC cuts for V2chip+V1bom" + bcolors.ENDC
            )
        else:
            qc_criteria_path = Path(str(data / "analysis_cuts.json"))
        log.info(f"Using {qc_criteria_path} for QC criteria")

    try:
        with resources.as_file(Path(qc_criteria_path)) as path:
    ...
    if test_type and test_type not in qc_config:
        log.error(
            bcolors.ERROR
