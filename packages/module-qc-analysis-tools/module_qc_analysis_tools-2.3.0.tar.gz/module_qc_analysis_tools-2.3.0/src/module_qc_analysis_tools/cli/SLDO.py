    ...
    for filename in sorted(allinputs):
        ...
        for inputDF in inputDFs:
            ...
            try:
                ...
                log.error(
                    bcolors.WARNING
                    + " Couldn't get chip type from module SN"
                    + bcolors.ENDC
                )
            ...
            if fit_method.value == "root":
                slope, offset, _r1 = linear_fit(
                    calculated_data["Current"]["Values"], VinAvg
                )
            else:
                slope, offset, _r1 = linear_fit_np(
                    calculated_data["Current"]["Values"], VinAvg
                )
            ...
            SLDO_LINEARITY = np.sqrt(
                np.sum((np.array(residual_Vin)) ** 2) / len(residual_Vin)
            )
            SLDO_VINA_VIND = (
                calculated_data["VinA"]["Values"][idx]
                - calculated_data["VinD"]["Values"][idx]
            )
