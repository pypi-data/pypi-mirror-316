...
TEST_TYPE = "QUAD_MODULE_METROLOGY"
...
def analyze_module(
    ...
    results_unformat = {key: value["Values"] for key, value in df.items()}
    ...
    for key_lists in (
        ...
        results_lists = results_unformat.get(key_lists)
        results.update({key_lists: results_lists})
    ...
    if "FIDUCIAL" in results:
        results["FIDUCIAL"] = bool(results["FIDUCIAL"])
    else:
        log.warning(
    ...
    try:
        ...
        if time_start == "":
            log.warning(
                bcolors.WARNING
                + "Empty TimeStart entry found. Setting MEASUREMENT_DATE to None."
                + bcolors.ENDC
            )
            time_start = None
            data.add_property("MEASUREMENT_DATE", time_start)
        else:
            data.add_property(
                "MEASUREMENT_DATE",
        ...
        log.warning(
            bcolors.WARNING
            + "No TimeStart entry found. Setting MEASUREMENT_DATE to None."
            + bcolors.ENDC
        )
        time_start = None
        data.add_property("MEASUREMENT_DATE", time_start)
