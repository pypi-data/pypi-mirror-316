...
class OWAdaBoost(OWBaseLearner):
    name = "AdaBoost"
    description = (_tr.m[2090, "An ensemble meta-algorithm that combines weak learners "] + _tr.m[2091, "and adapts to the 'hardness' of each training sample. "])
    ...
    priority = 80
    keywords = _tr.m[2092, "adaboost, boost"]

    LEARNER = SklAdaBoostLearner

    class Inputs(OWBaseLearner.Inputs):
        learner = Input(_tr.m[2093, "Learner"], Learner)

    #: Losses for regression problems
    losses = [_tr.m[2094, "Linear"], _tr.m[2095, "Square"], _tr.m[2096, "Exponential"]]
    ...
    class Error(OWBaseLearner.Error):
        no_weight_support = Msg(_tr.m[2097, 'The base learner does not support weights.'])
    ...
    def add_main_layout(self):
        ...
        grid.addRow(_tr.m[2098, "Base estimator:"], self.base_label)

        self.n_estimators_spin = gui.spin(
            None, self, "n_estimators", 1, 10000,
            controlWidth=80, alignment=Qt.AlignRight,
            callback=self.settings_changed)
        grid.addRow(_tr.m[2099, "Number of estimators:"], self.n_estimators_spin)

        self.learning_rate_spin = gui.doubleSpin(
            None, self, "learning_rate", 1e-5, 1.0, 1e-5, decimals=5,
            alignment=Qt.AlignRight,
            callback=self.settings_changed)
        grid.addRow(_tr.m[2100, "Learning rate:"], self.learning_rate_spin)

        self.reg_algorithm_combo = gui.comboBox(
            None, self, "loss_index", items=self.losses,
            callback=self.settings_changed)
        grid.addRow(_tr.m[2101, "Loss (regression):"], self.reg_algorithm_combo)

        box = gui.widgetBox(self.controlArea, box=_tr.m[2102, "Reproducibility"])
        self.random_seed_spin = gui.spin(
            box, self, "random_seed", 0, 2 ** 31 - 1, controlWidth=80,
            label=_tr.m[2103, "Fixed seed for random generator:"], alignment=Qt.AlignRight,
    ...
    def set_base_learner(self, learner):
        ...
        if learner and not learner.supports_weights:
            ...
            self.base_label.setText(_tr.m[2104, "INVALID"])
        else:
    ...
    def get_learner_parameters(self):
        return ((_tr.m[2105, "Base estimator"], self.base_estimator),
                (_tr.m[2106, "Number of estimators"], self.n_estimators),
                (_tr.m[2107, "Loss (regression)"], self.losses[
