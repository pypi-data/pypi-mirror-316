...
class OWSGD(OWBaseLearner):
    name = _tr.m[2412, 'Stochastic Gradient Descent']
    description = (_tr.m[2413, 'Minimize an objective function using a stochastic '] + _tr.m[2414, 'approximation of gradient descent.'])
    icon = "icons/SGD.svg"
    replaces = [
        "Orange.widgets.regression.owsgdregression.OWSGDRegression",
    ]
    priority = 90
    keywords = _tr.m[2415, "stochastic gradient descent, sgd"]
    ...
    class Outputs(OWBaseLearner.Outputs):
        coefficients = Output(_tr.m[2416, "Coefficients"], Table, explicit=True)

    reg_losses = (
        (_tr.m[2417, 'Squared Loss'], 'squared_error'),
        (_tr.m[2418, 'Huber'], 'huber'),
        (_tr.m[2419, 'ε insensitive'], 'epsilon_insensitive'),
        (_tr.m[2420, 'Squared ε insensitive'], 'squared_epsilon_insensitive'))

    cls_losses = (
        (_tr.m[2421, 'Hinge'], 'hinge'),
        (_tr.m[2422, 'Logistic regression'], 'log'),
        (_tr.m[2423, 'Modified Huber'], 'modified_huber'),
        (_tr.m[2424, 'Squared Hinge'], 'squared_hinge'),
        ('Perceptron', 'perceptron')) + reg_losses

    #: Regularization methods
    penalties = (
        (_tr.m[2425, 'None'], 'none'),
        ('Lasso (L1)', 'l1'),
        ('Ridge (L2)', 'l2'),
        (_tr.m[2426, 'Elastic Net'], 'elasticnet'))

    learning_rates = (
        (_tr.m[2427, 'Constant'], 'constant'),
        (_tr.m[2428, 'Optimal'], 'optimal'),
        (_tr.m[2429, 'Inverse scaling'], 'invscaling'))
    ...
    def _add_algorithm_to_layout(self, layout):
        ...
        grid = QGridLayout()
        box = gui.widgetBox(None, _tr.m[2430, 'Loss functions'], orientation=grid)
        ...
        grid.addWidget(QLabel(_tr.m[2431, "Classification: "]), 0, 0)
        ...
        self.reg_epsilon_spin = gui.spin(
            ...
            callback=self.settings_changed)
        hbox.layout().addStretch()
        grid.addWidget(QLabel(_tr.m[2432, "Regression: "]), 2, 0)
    ...
    def _add_regularization_to_layout(self, layout):
        # this is part of init, pylint: disable=attribute-defined-outside-init
        box = gui.widgetBox(None, _tr.m[2433, 'Regularization'])
        layout.addWidget(box)
        ...
        self.l1_ratio_box = gui.spin(
            ...
            label=_tr.m[2434, 'Mixing: '], controlWidth=80,
        ...
        self.alpha_spin = gui.spin(
            ...
            label=_tr.m[2435, 'Strength (α): '], alignment=Qt.AlignRight,
            callback=self.settings_changed)
        hbox.layout().addStretch()
    ...
    def _add_learning_params_to_layout(self, layout):
        # this is part of init, pylint: disable=attribute-defined-outside-init
        box = gui.widgetBox(None, _tr.m[2436, 'Optimization'])
        layout.addWidget(box)
        self.learning_rate_combo = gui.comboBox(
            box, self, 'learning_rate_index', label=_tr.m[2437, 'Learning rate: '],
        ...
        self.shuffle_cbx = gui.checkBox(
            ...
            _tr.m[2442, 'Shuffle data after each iteration'],
            callback=self._on_shuffle_change)
        self.random_seed_spin = gui.spin(
            box, self, 'random_state', 0, MAXINT,
            label=_tr.m[2443, 'Fixed seed for random shuffling: '], controlWidth=80,
            alignment=Qt.AlignRight, callback=self.settings_changed,
    ...
    def get_learner_parameters(self):
        ...
        params[_tr.m[2444, 'Classification loss function']] = self.cls_losses[
            self.cls_loss_function_index][0]
        if self.cls_losses[self.cls_loss_function_index][1] in (
                'huber', 'epsilon_insensitive', 'squared_epsilon_insensitive'):
            params[_tr.m[2445, 'Epsilon (ε) for classification']] = self.cls_epsilon
        # Regression loss function
        params[_tr.m[2446, 'Regression loss function']] = self.reg_losses[
            self.reg_loss_function_index][0]
        if self.reg_losses[self.reg_loss_function_index][1] in (
                'huber', 'epsilon_insensitive', 'squared_epsilon_insensitive'):
            params[_tr.m[2447, 'Epsilon (ε) for regression']] = self.reg_epsilon

        params[_tr.m[2448, 'Regularization']] = self.penalties[self.penalty_index][0]
        # Regularization strength
        if self.penalties[self.penalty_index][1] in ('l1', 'l2', 'elasticnet'):
            params[_tr.m[2449, 'Regularization strength (α)']] = self.alpha
        # Elastic Net mixing
        if self.penalties[self.penalty_index][1] in ('elasticnet',):
            params[_tr.m[2450, 'Elastic Net mixing parameter (L1 ratio)']] = self.l1_ratio

        params[_tr.m[2451, 'Learning rate']] = self.learning_rates[
            self.learning_rate_index][0]
        # Eta
        if self.learning_rates[self.learning_rate_index][1] in \
                ('constant', 'invscaling'):
            params[_tr.m[2452, 'Initial learning rate (η<sub>0</sub>)']] = self.eta0
        # t
        if self.learning_rates[self.learning_rate_index][1] in \
                ('invscaling',):
            params[_tr.m[2453, 'Inverse scaling exponent (t)']] = self.power_t

        params[_tr.m[2454, 'Shuffle data after each iteration']] = bool_str(self.shuffle)
        if self.use_random_state:
            params[_tr.m[2455, 'Random seed for shuffling']] = self.random_state
    ...
    def update_model(self):
        ...
        if self.model is not None:
            if self.model.domain.class_var.is_discrete:
                ...
                coeffs.name = _tr.m[2456, "coefficients"]
