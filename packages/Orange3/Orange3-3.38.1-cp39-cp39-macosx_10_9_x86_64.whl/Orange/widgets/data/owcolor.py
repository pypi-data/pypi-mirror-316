    ...
    def from_dict(cls, var, data):
        ...
        if val_map is not None:
            ...
            if len(set(mapped_values)) != len(mapped_values):
                warnings.append(
                    (f"{var.name}: " + _tr.m[246, "renaming of values ignored due to duplicate names"]))
            else:
...
class OWColor(widget.OWWidget):
    name = _tr.m[248, "Color"]
    description = _tr.m[249, "Set color legend for variables."]
    icon = "icons/Colors.svg"

    class Inputs:
        data = Input(_tr.m[250, "Data"], Orange.data.Table)

    class Outputs:
        data = Output(_tr.m[251, "Data"], Orange.data.Table)
    ...
    def __init__(self):
        ...
        box = gui.hBox(self.controlArea, _tr.m[252, "Discrete Variables"])
        ...
        box = gui.hBox(self.controlArea, _tr.m[253, "Numeric Variables"])
        ...
        gui.button(box, self, _tr.m[254, "Save"], callback=self.save)
        gui.button(box, self, _tr.m[255, "Load"], callback=self.load)
        gui.button(box, self, _tr.m[256, "Reset"], callback=self.reset)
    ...
    def save(self):
        fname, _ = QFileDialog.getSaveFileName(
            self, _tr.m[257, "File name"], self._start_dir(),
            _tr.m[258, "Variable definitions (*.colors)"])
        if not fname:
            return
    ...
    def load(self):
        fname, _ = QFileDialog.getOpenFileName(
            self, _tr.m[259, "File name"], self._start_dir(),
            _tr.m[260, "Variable definitions (*.colors)"])
        if not fname:
            return
        ...
        try:
            ...
            QMessageBox.critical(self, _tr.m[261, "File error"], _tr.m[262, "File cannot be opened."])
            return
        except (json.JSONDecodeError, InvalidFileFormat):
            QMessageBox.critical(self, _tr.m[263, "File error"], _tr.m[264, "Invalid file format."])
            return
    ...
    def _parse_var_defs(self, js):
        ...
        if len(renamed_vars) != len(self.disc_descs) + len(self.cont_descs):
            QMessageBox.warning(
                self,
                _tr.m[265, "Duplicated variable names"],
                _tr.m[266, "Variables will not be renamed due to duplicated names."])
        ...
        if unused_vars:
            ...
            if len(unused_vars) == 1:
                warn = (_tr.e(_tr.c(267, f'Definition for variable {names[0]}, which does not ')) + _tr.e(_tr.c(268, f'appear in the data, was ignored.\n')))
            else:
                if len(unused_vars) <= 5:
                    warn = (_tr.e(_tr.c(269, 'Definitions for variables ')) + _tr.e(_tr.c(270, f'{", ".join(names[:-1])} and {names[-1]}')))
                else:
                    warn = (_tr.e(_tr.c(271, f'Definitions for {", ".join(names[:4])} ')) + _tr.e(_tr.c(272, f'and {len(names) - 4} other variables')))
                warn += _tr.m[273, ", which do not appear in the data, were ignored.\n"]
        ...
        if warnings:
            QMessageBox.warning(
                self, _tr.m[274, "Invalid definitions"], "\n".join(warnings))
    ...
    def send_report(self):
        ...
        def _report_variables(variables):
            def was(n, o):
                return n if n == o else _tr.e(_tr.c(275, f"{n} (was: {o})"))
            ...
            for var in variables:
                if var.is_discrete:
                    ...
                    value_cols = (f'<td colspan="{max_values}">' + f'{pal.friendly_name}</td>')
                else:
                    continue
        ...
        sections = (
            ...
            for name, variables in (
                (_tr.m[276, "Features"], dom.attributes),
                (_tr.e(_tr.c(277, f'{pl(len(dom.class_vars), "Outcome")}')), dom.class_vars),
                (_tr.m[278, "Meta attributes"], dom.metas)))
