...
class LocallyLinearEmbedding(SklProjector):
    ...
    name = _tr.m[189, 'Locally Linear Embedding']
...
class SpectralEmbedding(SklProjector):
    ...
    name = _tr.m[190, 'Spectral Embedding']
        ...
        if isinstance(self.embedding_.affinities, openTSNE.affinity.Multiscale):
            ...
            if perplexity:
                if not isinstance(self.perplexity, Iterable):
                    raise ValueError(
                        ("Perplexity should be an instance of `Iterable`, `%s` " + "given.") % type(self.perplexity).__name__)
...
class TSNE(Projector):
    ...
    def compute_affinities(self, X):
        ...
        if self.multiscale:
            ...
            if isinstance(self.perplexity, Iterable):
                raise ValueError(
                    ("Perplexity should be an instance of `float`, `%s` " + "given.") % type(self.perplexity).__name__
                )
    ...
    def compute_initialization(self, X):
        ...
        if isinstance(self.initialization, np.ndarray):
            ...
            raise ValueError(
                ("Invalid initialization `%s`. Please use either `pca` or " + "`random` or provide a numpy array.") % self.initialization
            )
    ...
    def convert_embedding_to_model(self, data: Union[Table, DistMatrix], embedding: np.ndarray):
        ...
        if self.metric == "precomputed":
            if not isinstance(data, DistMatrix):
                raise ValueError(
                    (f"Expected `data` to be instance of " + (f"{DistMatrix.__class__.__name__} when using " + (f"`metric='precomputed'. Got {data.__class__.__name__} " + f"instead!")))
                )
