...
class CA(ClassificationScore):
    ...
    name = _tr.m[95, "CA"]
    long_name = _tr.m[96, "Classification accuracy"]
    priority = 20
        ...
        if average == 'binary':
            if target is None:
                if len(results.domain.class_var.values) > 2:
                    raise ValueError(
                        ("Multiclass data: specify target class or select " + "averaging ('weighted', 'macro', 'micro')"))
...
class AUC(ClassificationScore):
    ...
    is_binary = True
    name = _tr.m[101, "AUC"]
    long_name = _tr.m[102, "Area under ROC curve"]
    priority = 10
    ...
    def calculate_weights(results):
        ...
        wsum = np.sum(weights)
        if wsum == 0:
            raise ValueError(_tr.m[103, "Class variable has less than two values"])
        else:
            return weights / wsum
    ...
    def compute_score(self, results, target=None, average=None):
        ...
        if n_classes < 2:
            raise ValueError(_tr.m[104, "Class variable has less than two values"])
        elif n_classes == 2:
...
class LogLoss(ClassificationScore):
    ...
    priority = 120
    name = _tr.m[105, "LogLoss"]
    long_name = _tr.m[106, "Logistic loss"]
    default_visible = False

    def compute_score(self, results, eps=_tr.m[107, "auto"], normalize=True,
                      sample_weight=None):
        if eps != _tr.m[108, "auto"]:
            # eps argument will be removed in scikit-learn 1.5
            warnings.warn(
                (_tr.m[109, "`LogLoss.compute_score`: eps parameter is unused. "] + _tr.m[110, "It will always have value of `np.finfo(y_pred.dtype).eps`."]),
...
class Specificity(ClassificationScore):
    ...
    priority = 110
    name = "Spec"
    long_name = _tr.m[111, "Specificity"]
    default_visible = False
    ...
    def compute_score(self, results, target=None, average="binary"):
        ...
        if target is None:
            if average == "weighted":
                ...
                if n_classes != 2:
                    raise ValueError(
                        (_tr.m[112, "Binary averaging needs two classes in data: "] + (_tr.m[113, "specify target class or use "] + _tr.m[114, "weighted averaging."])))
                return self.single_class_specificity(results, 1)
            else:
                raise ValueError(
                    ("Wrong parameters: For averaging select one of the " + "following values: ('weighted', 'binary')"))
...
class MatthewsCorrCoefficient(ClassificationScore):
    ...
    name = "MCC"
    long_name = _tr.m[115, "Matthews correlation coefficient"]
...
class MSE(RegressionScore):
    ...
    name = "MSE"
    long_name = _tr.m[116, "Mean square error"]
    priority = 20


class RMSE(RegressionScore):
    name = "RMSE"
    long_name = _tr.m[117, "Root mean square error"]
...
class MAE(RegressionScore):
    ...
    name = "MAE"
    long_name = _tr.m[118, "Mean absolute error"]
    priority = 40

class MAPE(RegressionScore):
    __wraps__ = skl_metrics.mean_absolute_percentage_error
    name = "MAPE"
    long_name = _tr.m[119, "Mean absolute percentage error"]
    priority = 45
...
class R2(RegressionScore):
    ...
    name = "R2"
    long_name = _tr.m[120, "Coefficient of determination"]
    priority = 50


class CVRMSE(RegressionScore):
    name = "CVRMSE"
    long_name = _tr.m[121, "Coefficient of variation of the RMSE"]
    priority = 110
    default_visible = False
    ...
    def compute_score(self, results):
        ...
        if mean < 1e-10:
            raise ValueError(_tr.m[122, "Mean value is too small"])
