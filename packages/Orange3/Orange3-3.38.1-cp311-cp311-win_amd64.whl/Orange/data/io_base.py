...
class _TableHeader:
    ...
    def _header1(cls, headers: List[List[str]]) -> Tuple[List, List, List]:
        ...
        res = ('^((?P<flags>' + (f'[{roles}{types}]|' + (f'([{roles}][{types}])|' + (f'([{types}][{roles}])' + ')#)?(?P<name>.*)'))))
            ...
            for row, num in enumerate(orig_vals):
                if not isnastr(num):
                    try:
                        ...
                        break
            raise ValueError((f'Non-continuous value in (1-based) ' + f'line {row + offset + 1}, column {col + 1}'))
...
class _FileFormatMeta(Registry):
    ...
    def img_writers(cls):
        warnings.warn(
            (f"'{__name__}.FileFormat.img_writers' is no longer used and " + ("will be removed. Please use " + "'Orange.widgets.io.FileFormat.img_writers' instead.")),
...
class FileFormatBase(_FileReader, _FileWriter, metaclass=_FileFormatMeta):
    ...
    def locate(cls, filename, search_dirs=('.',)):
        ...
        if not path.exists(absolute_filename):
            raise IOError(_tr.m[82, 'File "{}" was not found.'].format(filename))
