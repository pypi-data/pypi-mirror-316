...
def instance_tooltip(domain, row, skip_attrs=()):
    def show_part(_point_data, singular, plural, max_shown, _vars):
        ...
        n_vars = len(_vars)
        if n_vars > max_shown:
            cols[-1] = _tr.e(_tr.c(3099, f'... and {n_vars - max_shown + 1} others'))
        return \
            "<b>{}</b>:<br/>".format(singular if n_vars < 2 else plural) \
            + "<br/>".join(cols)

    parts = ((_tr.m[3100, "Class"], _tr.m[3101, "Classes"], 4, domain.class_vars),
             (_tr.m[3102, "Meta"], _tr.m[3103, "Metas"], 4, domain.metas),
             (_tr.m[3104, "Feature"], _tr.m[3105, "Features"], 10, domain.attributes))
