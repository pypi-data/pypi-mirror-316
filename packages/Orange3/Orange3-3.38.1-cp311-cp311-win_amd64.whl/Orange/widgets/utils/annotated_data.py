...
ANNOTATED_DATA_SIGNAL_NAME = _tr.m[3106, "Data"]
ANNOTATED_DATA_FEATURE_NAME = _tr.m[3107, "Selected"]
...
def domain_with_annotation_column(
        ...
        values=(_tr.m[3108, "No"], _tr.m[3109, "Yes"]),
...
def create_annotated_table(data, selected_indices):
    ...
    return _table_with_annotation_column(
        data, (_tr.m[3110, "No"], _tr.m[3111, "Yes"]), annotated, ANNOTATED_DATA_FEATURE_NAME)
...
def group_values(selection, include_unselected, values):
    ...
    if values is None:
        ...
        if include_unselected:
            values.append(_tr.m[3112, "Unselected"])
