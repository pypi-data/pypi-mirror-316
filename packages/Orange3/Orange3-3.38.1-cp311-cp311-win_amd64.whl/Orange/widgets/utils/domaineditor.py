...
class VarTableModel(QAbstractTableModel):
    ...
    places = _tr.m[3142, "feature"], _tr.m[3143, "target"], _tr.m[3144, "meta"], _tr.m[3145, "skip"]
    typenames = _tr.m[3146, "categorical"], _tr.m[3147, "numeric"], _tr.m[3148, "text"], _tr.m[3149, "datetime"]
    ...
    def headerData(self, i, orientation, role=Qt.DisplayRole):
        if orientation == Qt.Horizontal and role == Qt.DisplayRole and i < 4:
            return (_tr.m[3150, "Name"], _tr.m[3151, "Type"], _tr.m[3152, "Role"], _tr.m[3153, "Values"])[i]
...
class VarTypeDelegate(ComboDelegate):
    def setEditorData(self, combo, index):
        ...
        if no_numeric:
            ...
            items = [i for i in self.items if i not in (_tr.m[3154, "numeric"], _tr.m[3155, "datetime"])]
        else:
            items = self.items
