...
class OWConfusionMatrix(widget.OWWidget):
    ...
    name = _tr.m[1792, "Confusion Matrix"]
    description = (_tr.m[1793, "Display a confusion matrix constructed from "] + _tr.m[1794, "the results of classifier evaluations."])
    icon = "icons/ConfusionMatrix.svg"
    priority = 1001
    keywords = _tr.m[1795, "confusion matrix"]

    class Inputs:
        evaluation_results = Input(_tr.m[1796, "Evaluation Results"], Orange.evaluation.Results)

    class Outputs:
        selected_data = Output(_tr.m[1797, "Selected Data"], Orange.data.Table, default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Orange.data.Table)

    quantities = [_tr.m[1798, "Number of instances"],
                  _tr.m[1799, "Proportion of predicted"],
                  _tr.m[1800, "Proportion of actual"],
                  _tr.m[1801, "Sum of probabilities"]]
    qu_tooltips = [_tr.m[1802, "Number of correctly and incorrectly classified instances"],
                   _tr.m[1803, "Proportion of predicted"],
                   _tr.m[1804, "Proportion of actual"],
                   (_tr.m[1805, "Number of instances, distributed across columns "] + _tr.m[1806, "according to predicted probabilities"])]
    ...
    UserAdviceMessages = [
        widget.Message(
            (_tr.m[1807, "Clicking on cells or in headers outputs the corresponding "] + _tr.m[1808, "data instances"]),
            "click_cell")]

    class Error(widget.OWWidget.Error):
        no_regression = Msg(_tr.m[1809, "Confusion Matrix cannot show regression results."])
        invalid_values = Msg(_tr.m[1810, "Evaluation Results input contains invalid values"])
        empty_input = widget.Msg(_tr.m[1811, "Empty result on input. Nothing to display."])
    ...
    def __init__(self):
        ...
        self.learners_box = gui.listBox(
            self.controlArea, self, "selected_learner", "learners", box=_tr.m[1812, 'Learners'],
            callback=self._learner_changed
        )

        self.outputbox = gui.vBox(self.buttonsArea)
        box = gui.vBox(self.outputbox, box=_tr.m[1813, "Output"])
        gui.checkBox(box, self, "append_predictions",
                     _tr.m[1814, "Predictions"], callback=self._invalidate)
        gui.checkBox(box, self, "append_probabilities",
                     _tr.m[1815, "Probabilities"],
        ...
        sbox = gui.hBox(box)
        gui.rubber(sbox)
        gui.comboBox(sbox, self, "selected_quantity", label=_tr.m[1816, "Show: "],
        ...
        selbox = gui.hBox(box)
        gui.button(selbox, self, _tr.m[1817, "Select Correct"],
                   callback=self.select_correct, autoDefault=False)
        gui.button(selbox, self, _tr.m[1818, "Select Misclassified"],
                   callback=self.select_wrong, autoDefault=False)
        gui.button(selbox, self, _tr.m[1819, "Clear Selection"],
    ...
    def _init_table(self, nclasses):
        item = self._item(0, 2)
        item.setData(_tr.m[1820, "Predicted"], Qt.DisplayRole)
        item.setTextAlignment(Qt.AlignCenter)
        item.setFlags(Qt.NoItemFlags)

        self._set_item(0, 2, item)
        item = self._item(2, 0)
        item.setData(_tr.m[1821, "Actual"], Qt.DisplayRole)
    ...
    def set_results(self, results):
        ...
        self.learners = getattr(
            ...
            [_tr.e(_tr.c(1822, f"Learner #{i + 1}")) for i in range(nmodels)])
        ...
        if self.results is not None and self.selected_learner:
            ...
            for i in range(n):
                for j in range(n):
                    ...
                    item.setData(
                        _tr.m[1823, "NA"] if _isinvalid(val) else formatstr.format(val),
                        Qt.DisplayRole)
                    ...
                    item.setToolTip(_tr.m[1824, "actual: {}\npredicted: {}"].format(
    ...
    def send_report(self):
        ...
        if self.results is not None and self.selected_learner:
            self.report_table(
                _tr.m[1825, "Confusion matrix for {} (showing {})"].
