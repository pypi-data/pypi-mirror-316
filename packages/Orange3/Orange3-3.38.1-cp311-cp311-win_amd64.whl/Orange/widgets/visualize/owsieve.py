...
class OWSieveDiagram(OWWidget, VizRankMixin(SieveRank)):
    name = _tr.m[3815, "Sieve Diagram"]
    description = (_tr.m[3816, "Visualize the observed and expected frequencies "] + _tr.m[3817, "for a combination of values."])
    icon = _tr.m[3818, "icons/SieveDiagram.svg"]
    priority = 200
    keywords = _tr.m[3819, "sieve diagram"]

    class Inputs:
        data = Input(_tr.m[3820, "Data"], Table, default=True)
        features = Input(_tr.m[3821, "Features"], AttributeList)

    class Outputs:
        selected_data = Output(_tr.m[3822, "Selected Data"], Table, default=True)
    ...
    def __init__(self):
        ...
        button = self.vizrank_button(_tr.m[3823, "Score Combinations"])
    ...
    def init_vizrank(self):
        errmsg = ""
        if self.data is None:
            errmsg = _tr.m[3824, "No data"]
        elif len(self.data) <= 1 or len(self.data.domain.attributes) <= 1:
            errmsg = _tr.m[3825, "Not enough data"]
        elif self.data.is_sparse():
            errmsg = _tr.m[3826, "Data is sparse"]
    ...
    def resolve_shown_attributes(self):
        ...
        if not features:
            self.warning(
                _tr.m[3827, "Features from the input signal are not present in the data"])
            return
    ...
    def update_graph(self):
        ...
        def make_tooltip():
            ...
            def _oper(attr, txt):
                if self.data.domain[attr.name] == ddomain[attr.name]:
                    return " = "
                return " " if txt[0] in "<≥" else _tr.m[3828, " in "]
            ...
            ct = _tr.m[3829, """<b>combination of values: </b><br/>
        ...
        if not disc_x.values or not disc_y.values:
            text_ = _tr.m[3830, "Features {} and {} have no values"].format(disc_x, disc_y) \
                if not disc_x.values and \
                   not disc_y.values and \
                          disc_x != disc_y \
                else \
                    _tr.m[3831, "Feature {} has no values"].format(
    ...
    def get_widget_name_extension(self):
        if self.data is not None:
            return _tr.m[3832, "{} vs {}"].format(self.attr_x.name, self.attr_y.name)
        return None
