...
class ParameterSetter(CommonParameterSetter):
    GRID_LABEL, SHOW_GRID_LABEL = _tr.m[3359, "Gridlines"], _tr.m[3360, "Show"]
    DEFAULT_ALPHA_GRID, DEFAULT_SHOW_GRID = 80, True
    BOTTOM_AXIS_LABEL, GROUP_AXIS_LABEL = _tr.m[3361, "Bottom axis"], _tr.m[3362, "Group axis"]
    IS_VERTICAL_LABEL = _tr.m[3363, "Vertical ticks"]
...
class OWBarPlot(OWWidget):
    name = _tr.m[3364, "Bar Plot"]
    description = _tr.m[3365, "Visualizes comparisons among categorical variables."]
    icon = "icons/BarPlot.svg"
    priority = 190
    keywords = _tr.m[3366, "bar plot, chart"]

    class Inputs:
        data = Input(_tr.m[3367, "Data"], Table, default=True)
        data_subset = Input(_tr.m[3368, "Data Subset"], Table)

    class Outputs:
        selected_data = Output(_tr.m[3369, "Selected Data"], Table, default=True)
    ...
    class Error(OWWidget.Error):
        no_cont_features = Msg(_tr.m[3370, "Plotting requires a numeric feature."])

    class Information(OWWidget.Information):
        too_many_instances = Msg((_tr.m[3371, "Data has too many instances. Only first {}"] + _tr.m[3372, " are shown."]).format(MAX_INSTANCES))

    enumeration = _tr.m[3373, "Enumeration"]
    ...
    def _add_controls(self):
        ...
        gui.comboBox(
            box, self, "selected_var", label=_tr.m[3374, "Values:"],
        ...
        self._group_var_model = DomainModel(
            placeholder=_tr.m[3375, "None"], valid_types=DiscreteVariable
        )
        gui.comboBox(
            box, self, "group_var", label=_tr.m[3376, "Group by:"],
        ...
        gui.comboBox(
            box, self, "annot_var", label=_tr.m[3378, "Annotations:"],
            model=self._annot_var_model, contentsLength=12, searchable=True,
            orientation=Qt.Horizontal, callback=self.__parameter_changed,
        )

        self._color_var_model = DomainModel(
            placeholder=_tr.m[3379, "(Same color)"], valid_types=DiscreteVariable
        )
        gui.comboBox(
            box, self, "color_var", label=_tr.m[3380, "Color:"],
