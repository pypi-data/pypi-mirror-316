...
class OWTreeViewer2D(OWWidget, openclass=True):
    ...
    def __init__(self):
        ...
        box = gui.vBox(
            self.controlArea, _tr.m[3893, 'Tree'],
            sizePolicy=QSizePolicy(QSizePolicy.Minimum, QSizePolicy.Fixed))
        self.infolabel = gui.widgetLabel(box, _tr.m[3894, 'No tree.'])

        layout = QFormLayout()
        layout.setFieldGrowthPolicy(layout.ExpandingFieldsGrow)
        box = self.display_box = gui.widgetBox(self.controlArea, _tr.m[3895, "Display"],
                                               orientation=layout)
        layout.addRow(
            _tr.m[3896, "Zoom: "],
        ...
        layout.addRow(
            ...
            gui.comboBox(box, self, 'max_tree_depth',
                         items=[_tr.m[3899, "Unlimited"]] + [
                             _tr.e(_tr.c(3900, f"{x} levels")) for x in range(2, 10)],
                         addToLayout=False, sendSelectedValue=False,
                         callback=self.toggle_tree_depth, sizePolicy=policy))
        layout.addRow(
            _tr.m[3901, "Edge width: "],
            gui.comboBox(box, self, 'line_width_method',
                         items=[_tr.m[3902, 'Fixed'], _tr.m[3903, 'Relative to root'],
                                _tr.m[3904, 'Relative to parent']],
                         addToLayout=False,
    ...
    def send_report(self):
        ...
        if self.model:
            self.reportSection(_tr.m[3905, "Tree"])
