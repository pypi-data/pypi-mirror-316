...
METRICS = [(_tr.m[2837, "Euclidean"], "l2"), (_tr.m[2838, "Manhattan"], "l1"), (_tr.m[2839, "Cosine"], "cosine")]


class OWLouvainClustering(widget.OWWidget):
    name = _tr.m[2840, "Louvain Clustering"]
    description = _tr.m[2841, "Detects communities in a network of nearest neighbors."]
    ...
    class Inputs:
        data = Input(_tr.m[2842, "Data"], Table, default=True)

    class Outputs:
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table, default=True)
        if Network is not None:
            graph = Output(_tr.m[2843, "Network"], Network)
    ...
    class Information(widget.OWWidget.Information):
        modified = Msg(_tr.m[2844, "Press commit to recompute clusters and send new data"])

    class Error(widget.OWWidget.Error):
        empty_dataset = Msg(_tr.m[2845, "No features in data"])
    ...
    def __init__(self):
        ...
        # Set up UI
        info_box = gui.vBox(self.controlArea, _tr.m[2846, "Info"])
        self.info_label = gui.widgetLabel(info_box, _tr.m[2847, "No data on input."])  # type: QLabel

        preprocessing_box = gui.vBox(self.controlArea, _tr.m[2848, "Preprocessing"])
        self.normalize_cbx = gui.checkBox(
            preprocessing_box, self, "normalize", label=_tr.m[2849, "Normalize data"],
            callback=self._invalidate_preprocessed_data, attribute=Qt.WA_LayoutUsesWidgetRect
        )  # type: QCheckBox
        self.apply_pca_cbx = gui.checkBox(
            preprocessing_box, self, "apply_pca", label=_tr.m[2850, "Apply PCA preprocessing"],
            callback=self._apply_pca_changed, attribute=Qt.WA_LayoutUsesWidgetRect
        )  # type: QCheckBox
        self.pca_components_slider = gui.hSlider(
            preprocessing_box, self, "pca_components", label=_tr.m[2851, "PCA Components: "], minValue=2,
            maxValue=_MAX_PCA_COMPONENTS,
            callback=self._invalidate_pca_projection, tracking=False
        )  # type: QSlider

        graph_box = gui.vBox(self.controlArea, _tr.m[2852, "Graph parameters"])
        self.metric_combo = gui.comboBox(
            graph_box, self, "metric_idx", label=_tr.m[2853, "Distance metric"],
        ...
        self.k_neighbors_spin = gui.spin(
            ...
            label=_tr.m[2854, "k neighbors"], controlWidth=80, alignment=Qt.AlignRight,
            callback=self._invalidate_graph,
        )
        self.resolution_spin = gui.hSlider(
            graph_box, self, "resolution", minValue=0, maxValue=5., step=1e-1,
            label=_tr.m[2855, "Resolution"], intOnly=False, labelFormat="%.1f",
            callback=self._invalidate_partition, tracking=False,
        )  # type: QSlider
        self.resolution_spin.parent().setToolTip(
            (_tr.m[2856, "The resolution parameter affects the number of clusters to find. "] + (_tr.m[2857, "Smaller values tend to produce more clusters and larger values "] + _tr.m[2858, "retrieve less clusters."]))
        )
    ...
    def commit(self):
        ...
        self.info_label.setText(_tr.m[2859, "Running..."])
    ...
    def __set_results(self, results):
        ...
        self.info_label.setText(
            _tr.e(_tr.c(2860, f"{num_clusters} {pl(num_clusters, 'cluster')} found.")))
    ...
    def _send_data(self):
        ...
        cluster_var = DiscreteVariable(
            get_unique_names(domain, _tr.m[2861, "Cluster"]),
            values=[_tr.m[2862, "C%d"] % (i + 1) for i, _ in enumerate(np.unique(new_partition))]
        )
    ...
    def set_data(self, data):
        ...
        self.info_label.setText(_tr.m[2863, "Clustering not yet run."])
    ...
    def clear(self):
        ...
        self.info_label.setText(_tr.m[2864, "No data on input."])
    ...
    def send_report(self):
        ...
        if self.apply_pca:
            pca += _tr.e(_tr.c(2865, f", {self.pca_components} {pl(self.pca_components, 'component')}"))

        self.report_items((
            (_tr.m[2866, "Normalize data"], report.bool_str(self.normalize)),
            (_tr.m[2867, "PCA preprocessing"], pca),
            (_tr.m[2868, "Metric"], METRICS[self.metric_idx][0]),
            (_tr.m[2869, "k neighbors"], self.k_neighbors),
            (_tr.m[2870, "Resolution"], self.resolution),
        ))
...
def run_on_data(data, normalize, pca_components, k_neighbors, metric, resolution, state):
    ...
    if pca_components is not None:
        steps = 3
        state.set_status(_tr.m[2871, "Computing PCA..."])
    ...
    state.set_status(_tr.m[2872, "Building graph..."])
    ...
    step += 1
    state.set_progress_value(100 * step / steps)
    state.set_status(_tr.m[2873, "Detecting communities..."])
    if state.is_interuption_requested():
        return res
...
def run_on_graph(graph, resolution, state):
    ...
    louvain = Louvain(resolution=resolution, random_state=0)
    state.set_status(_tr.m[2874, "Detecting communities..."])
    if state.is_interuption_requested():
        return res
