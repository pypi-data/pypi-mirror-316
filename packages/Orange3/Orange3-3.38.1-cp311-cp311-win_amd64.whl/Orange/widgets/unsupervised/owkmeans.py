...
class ClusterTableModel(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            return "{:.3f}".format(score) if valid else _tr.m[2801, "NA"]
...
class OWKMeans(widget.OWWidget):
    name = _tr.m[2802, "k-Means"]
    description = (_tr.m[2803, "k-Means clustering algorithm with silhouette-based "] + _tr.m[2804, "quality estimation."])
    icon = "icons/KMeans.svg"
    priority = 2100
    keywords = _tr.m[2805, "k-means, kmeans, clustering"]

    class Inputs:
        data = Input(_tr.m[2806, "Data"], Table)

    class Outputs:
        annotated_data = Output(
            ANNOTATED_DATA_SIGNAL_NAME, Table, default=True,
            replaces=[_tr.m[2807, "Annotated Data"]]
        )
        centroids = Output(_tr.m[2808, "Centroids"], Table)

    class Error(widget.OWWidget.Error):
        failed = widget.Msg(_tr.m[2809, "Clustering failed\nError: {}"])
        not_enough_data = widget.Msg(
            _tr.m[2810, "Too few ({}) unique data instances for {} clusters"]
        )
        no_attributes = widget.Msg(_tr.m[2811, "Data is missing features."])

    class Warning(widget.OWWidget.Warning):
        no_silhouettes = widget.Msg(
            _tr.m[2812, "Silhouette scores are not computed for >{} samples"].format(
                SILHOUETTE_MAX_SAMPLES)
        )
        not_enough_data = widget.Msg(
            _tr.m[2813, "Too few ({}) unique data instances for {} clusters"]
        )
        no_sparse_normalization = widget.Msg(_tr.m[2814, "Sparse data cannot be normalized"])

    INIT_METHODS = ((_tr.m[2815, "Initialize with KMeans++"], "k-means++"),
                    (_tr.m[2816, "Random initialization"], "random"))
    ...
    def __init__(self):
        ...
        bg = gui.radioButtonsInBox(
            ...
            box=_tr.m[2817, "Number of Clusters"], callback=self.update_method,
        )

        layout.addWidget(
            gui.appendRadioButton(bg, _tr.m[2818, "Fixed:"], addToLayout=False), 1, 1)
        ...
        layout.addWidget(
            gui.appendRadioButton(bg, _tr.m[2819, "From"], addToLayout=False), 2, 1)
        ...
        gui.widgetLabel(ftobox, _tr.m[2820, "to"])
        gui.spin(
        ...
        box = gui.vBox(self.controlArea, _tr.m[2821, "Preprocessing"])
        gui.checkBox(box, self, "normalize", _tr.m[2822, "Normalize columns"],
                     callback=self.invalidate)

        box = gui.vBox(self.controlArea, _tr.m[2823, "Initialization"])
        gui.comboBox(
        ...
        layout.addWidget(gui.widgetLabel(None, _tr.m[2824, "Re-runs: "]), 0, 0, Qt.AlignLeft)
        sb = gui.hBox(None, margin=0)
        ...
        layout.addWidget(
            gui.widgetLabel(None, _tr.m[2825, "Maximum iterations: "]), 1, 0, Qt.AlignLeft)
        sb = gui.hBox(None, margin=0)
        ...
        box = gui.vBox(self.mainArea, box=_tr.m[2826, "Silhouette Scores"])
        if self.optimize_k:
    ...
    def send_data(self):
        ...
        cluster_var = DiscreteVariable(
            get_unique_names(domain, _tr.m[2827, "Cluster"]),
            values=[_tr.m[2828, "C%d"] % (x + 1) for x in range(km.k)]
        )
        clust_ids = km.labels
        silhouette_var = ContinuousVariable(
            get_unique_names(domain, _tr.m[2829, "Silhouette"]))
        ...
        if self.data.name == Table.name:
            centroids.name = _tr.m[2830, "centroids"]
        else:
            centroids.name = _tr.e(_tr.c(2831, f"{self.data.name} centroids"))
    ...
    def send_report(self):
        ...
        self.report_items((
            (_tr.m[2832, "Number of clusters"], k_clusters),
            (_tr.m[2833, "Optimization"], _tr.m[2834, "{}, {} re-runs limited to {} steps"].format(
                init_method, self.n_init, self.max_iterations))))
        if self.data is not None:
            self.report_data(_tr.m[2835, "Data"], self.data)
            if self.optimize_k:
                self.report_table(
                    _tr.m[2836, "Silhouette scores for different numbers of clusters"],
                    self.table_view)
