...
class OWDistanceFile(OWUrlDropBase, RecentPathsWComboMixin):
    name = _tr.m[2598, "Distance File"]
    id = "orange.widgets.unsupervised.distancefile"
    description = _tr.m[2599, "Read distances from a file."]
    icon = "icons/DistanceFile.svg"
    priority = 10
    keywords = _tr.m[2600, "distance file, load, read, open"]

    class Outputs:
        distances = Output(_tr.m[2601, "Distances"], DistMatrix, dynamic=False)

    class Error(widget.OWWidget.Error):
        invalid_file = Msg(_tr.m[2602, "Data was not loaded:{}"])
        non_square_matrix = Msg(
            (_tr.m[2603, "Matrix is not square. "] + _tr.m[2604, "Reformat the file and use the File widget to read it."]))
    ...
    def __init__(self):
        ...
        vbox = gui.vBox(self.controlArea, _tr.m[2605, "Distance File"])
        box = gui.hBox(vbox)
        ...
        button = gui.button(
            box, self, _tr.m[2606, "Reload"], callback=self.reload, default=True)
        button.setIcon(self.style().standardIcon(QStyle.SP_BrowserReload))
        button.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)

        vbox = gui.vBox(self.controlArea, _tr.m[2607, "Options"])
        gui.checkBox(
            vbox, self, "auto_symmetric",
            _tr.m[2608, "Treat triangular matrices as symmetric"],
            tooltip=(_tr.m[2609, "If matrix is triangular, this will copy the data to the "] + _tr.m[2610, "other triangle"]),
            callback=self.commit
        )

        gui.rubber(self.buttonsArea)
        gui.button(
            self.buttonsArea, self, _tr.m[2611, "Browse documentation datasets"],
        ...
        if in_demos:
            ...
            if not os.path.exists(start_file):
                QMessageBox.information(
                    None, _tr.m[2612, "File"],
                    _tr.m[2613, "Cannot find the directory with documentation datasets"])
                return
        else:
            start_file = self.last_path() or os.path.expanduser("~/")

        filename, _ = QFileDialog.getOpenFileName(
            self, _tr.m[2614, 'Open Distance File'], start_file,
            _tr.m[2615, "Excel File (*.xlsx);;Distance File (*.dst)"])
        if not filename:
            return
    ...
    def send_report(self):
        if not self.distances:
            self.report_paragraph(_tr.m[2617, "No data was loaded."])
        else:
            self.report_items([(_tr.m[2618, "File name"], self.distances.name)])
