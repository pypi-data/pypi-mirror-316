...
class OWNNLearner(OWBaseLearner):
    name = _tr.m[2263, "Neural Network"]
    description = (_tr.m[2264, "A multi-layer perceptron (MLP) algorithm with "] + _tr.m[2265, "backpropagation."])
    icon = "icons/NN.svg"
    priority = 90
    keywords = _tr.m[2266, "neural network, mlp"]

    LEARNER = NNLearner

    activation = ["identity", "logistic", "tanh", "relu"]
    act_lbl = [_tr.m[2267, "Identity"], _tr.m[2268, "Logistic"], "tanh", _tr.m[2269, "ReLu"]]
    ...
    class Warning(OWBaseLearner.Warning):
        no_layers = Msg((_tr.m[2270, "ANN without hidden layers is equivalent to logistic "] + (_tr.m[2271, "regression with worse fitting.\nWe recommend using "] + _tr.m[2272, "logistic regression."])))
    ...
    def add_main_layout(self):
        ...
        form.addRow(
            _tr.m[2273, "Neurons in hidden layers:"],
            gui.lineEdit(
                None, self, "hidden_layers_input",
                orientation=Qt.Horizontal, callback=self.settings_changed,
                tooltip=(_tr.m[2274, "A list of integers defining neurons. Length of list "] + _tr.m[2275, "defines the number of layers. E.g. 4, 2, 2, 3."]),
                placeholderText=_tr.m[2276, "e.g. 10,"]))
        form.addRow(
            _tr.m[2277, "Activation:"],
            gui.comboBox(
                None, self, "activation_index", orientation=Qt.Horizontal,
                label=_tr.m[2278, "Activation:"], items=[i for i in self.act_lbl],
        ...
        form.addRow(
            ...
            gui.comboBox(
                ...
                label=_tr.m[2280, "Solver:"], items=[i for i in self.solv_lbl],
                callback=self.settings_changed))
        ...
        form.addRow(
            _tr.m[2281, "Maximal number of iterations:"],
            gui.spin(
                None, self, "max_iterations", 10, 1000000, step=10,
                label=_tr.m[2282, "Max iterations:"], orientation=Qt.Horizontal,
                alignment=Qt.AlignRight, callback=self.settings_changed))

        form.addRow(
            gui.checkBox(
                None, self, "replicable", label=_tr.m[2283, "Replicable training"],
    ...
    def set_alpha(self):
        ...
        self.reg_label.setText(_tr.m[2284, "Regularization, α={}:"].format(self.strength_C))
    ...
    def setup_layout(self):
        ...
        b = gui.button(self.apply_button, self, _tr.m[2285, "Cancel"],
    ...
    def get_learner_parameters(self):
        return ((_tr.m[2286, "Hidden layers"], ', '.join(map(str, self.get_hidden_layers()))),
                (_tr.m[2287, "Activation"], self.act_lbl[self.activation_index]),
                (_tr.m[2288, "Solver"], self.solv_lbl[self.solver_index]),
                (_tr.m[2289, "Alpha"], self.alpha),
                (_tr.m[2290, "Max iterations"], self.max_iterations),
                (_tr.m[2291, "Replicable training"], bool_str(self.replicable)))
