...
class OWRandomForest(OWBaseLearner):
    name = _tr.m[2312, "Random Forest"]
    description = _tr.m[2313, "Predict using an ensemble of decision trees."]
    ...
    priority = 40
    keywords = _tr.m[2314, "random forest"]
    ...
    class Error(OWBaseLearner.Error):
        not_enough_features = Msg(_tr.m[2315, "Insufficient number of attributes ({})"])

    class Warning(OWBaseLearner.Warning):
        class_weights_used = Msg(_tr.m[2316, "Weighting by class may decrease performance."])

    def add_main_layout(self):
        # this is part of init, pylint: disable=attribute-defined-outside-init
        box = gui.vBox(self.controlArea, _tr.m[2317, 'Basic Properties'])
        self.n_estimators_spin = gui.spin(
            box, self, "n_estimators", minv=1, maxv=10000, controlWidth=80,
            alignment=Qt.AlignRight, label=_tr.m[2318, "Number of trees: "],
            callback=self.settings_changed)
        self.max_features_spin = gui.spin(
            box, self, "max_features", 1, 50, controlWidth=80,
            label=_tr.m[2319, "Number of attributes considered at each split: "],
        ...
        self.random_state = gui.checkBox(
            box, self, "use_random_state", label=_tr.m[2320, "Replicable training"],
            callback=self.settings_changed,
            attribute=Qt.WA_LayoutUsesWidgetRect)
        self.weights = gui.checkBox(
            box, self,
            "class_weight", label=_tr.m[2321, "Balance class distribution"],
            callback=self.settings_changed,
            tooltip=_tr.m[2322, "Weigh classes inversely proportional to their frequencies."],
            attribute=Qt.WA_LayoutUsesWidgetRect
        )

        box = gui.vBox(self.controlArea, _tr.m[2323, "Growth Control"])
        self.max_depth_spin = gui.spin(
            box, self, "max_depth", 1, 50, controlWidth=80,
            label=_tr.m[2324, "Limit depth of individual trees: "], alignment=Qt.AlignRight,
        ...
        self.min_samples_split_spin = gui.spin(
            ...
            label=_tr.m[2325, "Do not split subsets smaller than: "],
    ...
    def get_learner_parameters(self):
        ...
        return (
            (_tr.m[2326, "Number of trees"], self.n_estimators),
            (_tr.m[2327, "Maximal number of considered features"],
             self.max_features if self.use_max_features else _tr.m[2328, "unlimited"]),
            (_tr.m[2329, "Replicable training"], [_tr.m[2330, "No"], _tr.m[2331, "Yes"]][self.use_random_state]),
            (_tr.m[2332, "Maximal tree depth"],
             self.max_depth if self.use_max_depth else _tr.m[2333, "unlimited"]),
            (_tr.m[2334, "Stop splitting nodes with maximum instances"],
             self.min_samples_split if self.use_min_samples_split else
             _tr.m[2335, "unlimited"]),
            (_tr.m[2336, "Class weights"], self.class_weight)
        )
