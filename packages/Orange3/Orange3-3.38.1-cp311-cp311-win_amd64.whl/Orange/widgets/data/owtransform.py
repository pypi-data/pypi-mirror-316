...
class TransformRunner:
    ...
    def run(
        ...
        state.set_status(_tr.m[1670, "Transforming..."])
        transformed_data = data.transform(template_data.domain)
        return transformed_data


class OWTransform(OWWidget, ConcurrentWidgetMixin):
    name = _tr.m[1671, "Apply Domain"]
    description = _tr.m[1672, "Applies template domain on data table."]
    category = _tr.m[1673, "Transform"]
    icon = "icons/Transform.svg"
    priority = 1230
    keywords = _tr.m[1674, "apply domain, transform"]

    class Inputs:
        data = Input(_tr.m[1675, "Data"], Table, default=True)
        template_data = Input(_tr.m[1676, "Template Data"], Table)

    class Outputs:
        transformed_data = Output(_tr.m[1677, "Transformed Data"], Table)

    class Error(OWWidget.Error):
        error = Msg(_tr.m[1678, "An error occurred while transforming data.\n{}"])
...
"""].strip(), box=True)
    ...
    def send_report(self):
        if self.data:
            self.report_data(_tr.m[1680, "Data"], self.data)
        if self.template_data:
            self.report_domain(_tr.m[1681, "Template data"], self.template_data.domain)
        if self.transformed_info:
            self.report_items(_tr.m[1682, "Transformed data"], self.transformed_info)
