...
INSTANCEID = _tr.m[1002, "Instance id"]
INDEX = _tr.m[1003, "Row index"]
...
class ConditionBox(QWidget):
    ...
    def add_row(self):
        ...
        row_items = self.RowItems(
            QLabel(_tr.m[1004, "and"] if row else self.pre_label),
...
class DomainModelWithTooltips(DomainModel):
    def data(self, index, role=Qt.DisplayRole):
        if role == Qt.ToolTipRole \
            ...
            if index.row() == 0:
                return _tr.m[1005, "Match rows sequentially"]
            if index.row() == 1:
                return (_tr.m[1006, "Re-match rows from tables obtained from the same "] + (_tr.m[1007, "source,\n"] + (_tr.m[1008, "e.g. data from the same file that was split within "] + _tr.m[1009, "the workflow."])))
...
class OWMergeData(widget.OWWidget):
    name = _tr.m[1010, "Merge Data"]
    description = _tr.m[1011, "Merge datasets based on the values of selected features."]
    category = _tr.m[1012, "Transform"]
    icon = "icons/MergeData.svg"
    priority = 1110
    keywords = _tr.m[1013, "merge data, join"]

    class Inputs:
        data = Input(_tr.m[1014, "Data"], Orange.data.Table, default=True, replaces=["Data A"])
        extra_data = Input(_tr.m[1015, "Extra Data"], Orange.data.Table, replaces=["Data B"])

    class Outputs:
        data = Output(_tr.m[1016, "Data"],
                      Orange.data.Table,
                      replaces=["Merged Data A+B", "Merged Data B+A", "Merged Data"])

    LeftJoin, InnerJoin, OuterJoin = range(3)
    OptionNames = (_tr.m[1017, "Append columns from Extra data"],
                   _tr.m[1018, "Find matching pairs of rows"],
                   _tr.m[1019, "Concatenate tables"])
    OptionDescriptions = (
        (_tr.m[1020, "The first table may contain, for instance, city names,\n"] + (_tr.m[1021, "and the second would be a list of cities and their coordinates.\n"] + _tr.m[1022, "Columns with coordinates would then be appended to the output."])),

        (_tr.m[1023, "Input tables contain different features describing the same data "] + (_tr.m[1024, "instances.\n"] + _tr.m[1025, "Output contains matched instances. Rows without matches are removed."])),

        (_tr.m[1026, "Input tables contain different features describing the same data "] + (_tr.m[1027, "instances.\n"] + (_tr.m[1028, "Output contains all instances. Data from merged instances is "] + _tr.m[1029, "merged into single rows."])))
    )

    UserAdviceMessages = [
        widget.Message(
            _tr.m[1030, "Confused about merging options?\nSee the tooltips!"],
            "merging_types")]
    ...
    class Warning(widget.OWWidget.Warning):
        renamed_vars = Msg((_tr.m[1031, "Some variables have been renamed "] + _tr.m[1032, "to avoid duplicates.\n{}"]))
        nonunique_left = Msg(
            (_tr.m[1033, "Some (unused) combinations of values in Data appear in "] + _tr.m[1034, "multiple rows."]))
        nonunique_right = Msg(
            (_tr.m[1035, "Some (unused) combinations of values in Extra Data appear in "] + _tr.m[1036, "multiple rows."]))

    class Error(widget.OWWidget.Error):
        matching_numeric_with_nonnum = Msg(
            _tr.m[1037, "Numeric and non-numeric columns ({} and {}) cannot be matched."])
        matching_index_with_sth = Msg(_tr.m[1038, "Row index cannot be matched with {}."])
        matching_id_with_sth = Msg(_tr.m[1039, "Instance cannot be matched with {}."])
        nonunique_left_matched = Msg(
            (_tr.m[1040, "Some combinations of values in Data appear in multiple rows."] + _tr.m[1041, "\nEvery matched combination may appear at most once."]))
        nonunique_left = Msg(
            (_tr.m[1042, "Some combinations of values in Data appear in multiple rows."] + _tr.m[1043, "\nEvery combination may appear at most once."]))
        nonunique_right_matched = Msg(
            (_tr.m[1044, "Some combinations of values in Extra Data appear in multiple rows."] + _tr.m[1045, "\nEvery matched combination may appear at most once."]))
        nonunique_right = Msg(
            (_tr.m[1046, "Some combinations of values in Extra Data appear in multiple rows."] + _tr.m[1047, "\nEvery combination may appear at most once."]))
    ...
    def send_report(self):
        ...
        self.report_items((
            (_tr.m[1051, "Merging"], self.OptionNames[self.merging]),
            (_tr.m[1052, "Match"],
             ", ".join(
                 _tr.e(_tr.c(1053, f"{self._get_col_name(left)} with {self._get_col_name(right)}"))
