...
class OWDataSampler(OWWidget):
    name = _tr.m[559, "Data Sampler"]
    description = (_tr.m[560, "Randomly draw a subset of data points "] + _tr.m[561, "from the input dataset."])
    icon = "icons/DataSampler.svg"
    priority = 100
    category = _tr.m[562, "Transform"]
    keywords = _tr.m[563, "data sampler, random"]

    _MAX_SAMPLE_SIZE = 2 ** 31 - 1

    class Inputs:
        data = Input(_tr.m[564, "Data"], Table)

    class Outputs:
        data_sample = Output(_tr.m[565, "Data Sample"], Table, default=True)
        remaining_data = Output(_tr.m[566, "Remaining Data"], Table)
    ...
    class Information(OWWidget.Information):
        compatibility_mode = Msg(
            (_tr.m[567, "Compatibility mode\n"] + _tr.m[568, "New versions of widget have swapped outputs for cross validation"])
        )

    class Warning(OWWidget.Warning):
        could_not_stratify = Msg(_tr.m[569, "Stratification failed.\n{}"])
        bigger_sample = Msg(_tr.m[570, 'Sample is bigger than input.'])

    class Error(OWWidget.Error):
        too_many_folds = Msg(_tr.m[571, "Number of subsets exceeds data size."])
        sample_larger_than_data = Msg(_tr.m[572, "Sample can't be larger than data."])
        not_enough_to_stratify = Msg(_tr.m[573, "Data is too small to stratify."])
        no_data = Msg(_tr.m[574, "Dataset is empty."])
    ...
    def __init__(self):
        ...
        self.sampling_box = gui.vBox(self.controlArea, _tr.m[575, "Sampling Type"])
        ...
        gui.appendRadioButton(sampling, _tr.m[576, "Fixed proportion of data:"])
        ...
        gui.appendRadioButton(sampling, _tr.m[577, "Fixed sample size"])
        ibox = gui.indentedBox(sampling)
        self.sampleSizeSpin = gui.spin(
            ibox, self, "sampleSizeNumber", label=_tr.m[578, "Instances: "],
            minv=0, maxv=self._MAX_SAMPLE_SIZE,
            callback=set_sampling_type(self.FixedSize),
            controlWidth=90)
        gui.checkBox(
            ibox, self, "replacement", _tr.m[579, "Sample with replacement"],
            callback=set_sampling_type(self.FixedSize))

        gui.appendRadioButton(sampling, _tr.m[580, "Cross validation"])
        form = QFormLayout(
            formAlignment=Qt.AlignLeft | Qt.AlignTop,
            labelAlignment=Qt.AlignLeft,
            fieldGrowthPolicy=QFormLayout.AllNonFixedFieldsGrow)
        ibox = gui.indentedBox(sampling, orientation=form)
        form.addRow(_tr.m[581, "Number of subsets:"],
                    gui.spin(
        ...
        form.addRow(_tr.m[582, "Unused subset:"] if not self.compatibility_mode
                    else _tr.m[583, "Selected subset:"], self.selected_fold_spin)

        gui.appendRadioButton(sampling, _tr.m[584, "Bootstrap"])

        self.sql_box = gui.vBox(self.controlArea, _tr.m[585, "Sampling Type"])
        sampling = gui.radioButtons(self.sql_box, self, "sampling_type",
                                    callback=self.sampling_type_changed)
        gui.appendRadioButton(sampling, _tr.m[586, "Time:"])
        ibox = gui.indentedBox(sampling)
        spin = gui.spin(ibox, self, "sampleSizeSqlTime", minv=1, maxv=3600,
                        callback=set_sampling_type(self.SqlTime))
        spin.setSuffix(_tr.m[587, " sec"])
        gui.appendRadioButton(sampling, _tr.m[588, "Percentage"])
        ibox = gui.indentedBox(sampling)
        ...
        self.options_box = gui.vBox(self.controlArea, _tr.m[589, "Options"], addSpaceBefore=False)
        self.cb_seed = gui.checkBox(
            self.options_box, self, "use_seed",
            _tr.m[590, "Replicable (deterministic) sampling"],
            callback=self.settings_changed)
        self.cb_stratify = gui.checkBox(
            self.options_box, self, "stratify",
            _tr.m[591, "Stratify sample (when possible)"], callback=self.settings_changed)
        self.cb_sql_dl = gui.checkBox(
            self.options_box, self, "sql_dl", _tr.m[592, "Download data to local memory"],
            callback=self.settings_changed)
        self.cb_sql_dl.setVisible(False)

        gui.button(self.buttonsArea, self, _tr.m[593, "Sample Data"],
    ...
    def send_report(self):
        if self.sampling_type == self.FixedProportion:
            tpe = _tr.e(_tr.c(594, f"Random sample with {self.sampleSizePercentage} % of data"))
        elif self.sampling_type == self.FixedSize:
            if self.sampleSizeNumber == 1:
                tpe = _tr.m[595, "Random data instance"]
            else:
                tpe = _tr.e(_tr.c(596, f"Random sample with {self.sampleSizeNumber} data instances"))
                if self.replacement:
                    tpe += _tr.m[597, ", with replacement"]
        elif self.sampling_type == self.CrossValidation:
            tpe = (_tr.e(_tr.c(598, f"{self.number_of_folds}-fold cross-validation ")) + _tr.e(_tr.c(599, f"without subset #{self.selectedFold}")))
        elif self.sampling_type == self.Bootstrap:
            tpe = _tr.m[600, "Bootstrap"]
        else:  # pragma: no cover
            assert False
        if self.stratify:
            tpe += _tr.m[601, ", stratified (if possible)"]
        if self.use_seed:
            tpe += _tr.m[602, ", deterministic"]
        items = [(_tr.m[603, "Sampling type"], tpe)]
        if self.sampled_instances is not None:
            items += [
                (_tr.m[604, "Input"], _tr.e(_tr.c(605, f"{len(self.data)} {pl(len(self.data), 'instance')}"))),
                (_tr.m[606, "Sample"], _tr.e(_tr.c(607, f"{self.sampled_instances} {pl(self.sampled_instances, 'instance')}"))),
                (_tr.m[608, "Remaining"], _tr.e(_tr.c(609, f"{self.remaining_instances} {pl(self.remaining_instances, 'instance')}"))),
            ]
