...
class OWRandomize(OWWidget):
    name = _tr.m[1383, "Randomize"]
    description = _tr.m[1384, "Randomize features, class and/or metas in data table."]
    category = _tr.m[1385, "Transform"]
    icon = "icons/Random.svg"
    priority = 2200
    keywords = _tr.m[1386, "randomize, random"]

    class Inputs:
        data = Input(_tr.m[1387, "Data"], Table)

    class Outputs:
        data = Output(_tr.m[1388, "Data"], Table)
    ...
    def __init__(self):
        ...
        # GUI
        box = gui.hBox(self.controlArea, _tr.m[1389, "Shuffled columns"])
        box.layout().setSpacing(20)
        self.class_check = gui.checkBox(
            box, self, "shuffle_class", _tr.m[1390, "Classes"],
            callback=self._shuffle_check_changed)
        self.attrs_check = gui.checkBox(
            box, self, "shuffle_attrs", _tr.m[1391, "Features"],
            callback=self._shuffle_check_changed)
        self.metas_check = gui.checkBox(
            box, self, "shuffle_metas", _tr.m[1392, "Metas"],
            callback=self._shuffle_check_changed)

        box = gui.vBox(self.controlArea, _tr.m[1393, "Shuffled rows"])
        hbox = gui.hBox(box)
        gui.widgetLabel(hbox, _tr.m[1394, "None"])
        ...
        gui.widgetLabel(hbox, _tr.m[1395, "All"])
        ...
        self.replicable_check = gui.checkBox(
            box, self, "random_seed", _tr.m[1396, "Replicable shuffling"],
            callback=self._shuffle_check_changed)
    ...
    def send_report(self):
        labels = [_tr.m[1397, "classes"], _tr.m[1398, "features"], _tr.m[1399, "metas"]]
        include = [label for label, i in zip(labels, self.parts) if i]
        text = _tr.m[1400, "none"] if not include else \
            _tr.m[1401, " and "].join(filter(None, (", ".join(include[:-1]), include[-1])))
        self.report_items(
            _tr.m[1402, "Settings"],
            [(_tr.m[1403, "Shuffled columns"], text),
             (_tr.m[1404, "Proportion of shuffled rows"], "{}%".format(self.scope_prop)),
             (_tr.m[1405, "Replicable"], _tr.m[1406, "yes"] if self.random_seed else _tr.m[1407, "no"])])
