...
class OWSelectRows(widget.OWWidget):
    name = _tr.m[1499, "Select Rows"]
    description = _tr.m[1500, "Select rows from the data based on values of variables."]
    icon = "icons/SelectRows.svg"
    priority = 100
    category = _tr.m[1501, "Transform"]
    keywords = _tr.m[1502, "select rows, filter"]

    class Inputs:
        data = Input(_tr.m[1503, "Data"], Table)

    class Outputs:
        matching_data = Output(_tr.m[1504, "Matching Data"], Table, default=True)
        unmatched_data = Output(_tr.m[1505, "Unmatched Data"], Table)
    ...
    Operators = {
        ContinuousVariable: [
            (FilterContinuous.Equal, _tr.m[1506, "equals"], _tr.m[1507, "equal"]),
            (FilterContinuous.NotEqual, _tr.m[1508, "is not"], _tr.m[1509, "are not"]),
            (FilterContinuous.Less, _tr.m[1510, "is below"], _tr.m[1511, "are below"]),
            (FilterContinuous.LessEqual, _tr.m[1512, "is at most"], _tr.m[1513, "are at most"]),
            (FilterContinuous.Greater, _tr.m[1514, "is greater than"], _tr.m[1515, "are greater than"]),
            (FilterContinuous.GreaterEqual, _tr.m[1516, "is at least"], _tr.m[1517, "are at least"]),
            (FilterContinuous.Between, _tr.m[1518, "is between"], _tr.m[1519, "are between"]),
            (FilterContinuous.Outside, _tr.m[1520, "is outside"], _tr.m[1521, "are outside"]),
            (FilterContinuous.IsDefined, _tr.m[1522, "is defined"], _tr.m[1523, "are defined"]),
        ],
        DiscreteVariable: [
            (FilterDiscreteType.Equal, _tr.m[1524, "is"]),
            (FilterDiscreteType.NotEqual, _tr.m[1525, "is not"]),
            (FilterDiscreteType.In, _tr.m[1526, "is one of"]),
            (FilterDiscreteType.IsDefined, _tr.m[1527, "is defined"])
        ],
        StringVariable: [
            (FilterString.Equal, _tr.m[1528, "equals"], _tr.m[1529, "equal"]),
            (FilterString.NotEqual, _tr.m[1530, "is not"], _tr.m[1531, "are not"]),
            (FilterString.Less, _tr.m[1532, "is before"], _tr.m[1533, "are before"]),
            (FilterString.LessEqual, _tr.m[1534, "is equal or before"], _tr.m[1535, "are equal or before"]),
            (FilterString.Greater, _tr.m[1536, "is after"], _tr.m[1537, "are after"]),
            (FilterString.GreaterEqual, _tr.m[1538, "is equal or after"], _tr.m[1539, "are equal or after"]),
            (FilterString.Between, _tr.m[1540, "is between"], _tr.m[1541, "are between"]),
            (FilterString.Outside, _tr.m[1542, "is outside"], _tr.m[1543, "are outside"]),
            (FilterString.Contains, _tr.m[1544, "contains"], _tr.m[1545, "contain"]),
            (FilterString.NotContain, _tr.m[1546, "does not contain"], _tr.m[1547, "do not contain"]),
            (FilterString.StartsWith, _tr.m[1548, "begins with"], _tr.m[1549, "begin with"]),
            (FilterString.NotStartsWith, _tr.m[1550, "does not begin with"], _tr.m[1551, "do not begin with"]),
            (FilterString.EndsWith, _tr.m[1552, "ends with"], _tr.m[1553, "end with"]),
            (FilterString.NotEndsWith, _tr.m[1554, "does not end with"], _tr.m[1555, "do not end with"]),
            (FilterString.IsDefined, _tr.m[1556, "is defined"], _tr.m[1557, "are defined"]),
            (FilterString.NotIsDefined, _tr.m[1558, "is not defined"], _tr.m[1559, "are not defined"]),
        ]
    }
    ...
    for _all_name, _all_type, _all_ops in (
            (_tr.m[1560, "All variables"], 0,
             [(None, _tr.m[1561, "are defined"])]),
            (_tr.m[1562, "All numeric variables"], 2,
             [(v, t) for v, _, t in Operators[ContinuousVariable]]),
            (_tr.m[1563, "All string variables"], 3,
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[1564, 'Conditions'], stretch=100)
        ...
        self.add_button = gui.button(
            box2, self, _tr.m[1565, "Add Condition"], callback=self.add_row)
        self.add_all_button = gui.button(
            box2, self, _tr.m[1566, "Add All Variables"], callback=self.add_all)
        self.remove_all_button = gui.button(
            box2, self, _tr.m[1567, "Remove All"], callback=self.remove_all)
        gui.rubber(box2)

        box_setting = gui.vBox(self.buttonsArea)
        self.cb_pa = gui.checkBox(
            box_setting, self, "purge_attributes",
            _tr.m[1568, "Remove unused values and constant features"],
            callback=self.conditions_changed)
        self.cb_pc = gui.checkBox(
            box_setting, self, "purge_classes", _tr.m[1569, "Remove unused classes"],
            callback=self.conditions_changed)
    ...
    def add_all(self):
        if self.cond_list.rowCount():
            Mb = QMessageBox
            if Mb.question(
                    self, _tr.m[1570, "Remove existing filters"],
                    (_tr.m[1571, "This will replace the existing filters with "] + _tr.m[1572, "filters for all variables."]), Mb.Ok | Mb.Cancel) != Mb.Ok:
                return
            self.remove_all()
    ...
    def set_new_values(self, oper_combo, adding_all, selected_values=None):
        ...
        if _tr.m[1573, "defined"] in oper_combo.currentText():
            ...
            if vtype == 2:  # continuous:
                ...
                if oper > 5:
                    gui.widgetLabel(box, _tr.m[1575, " and "])
                    box.controls.append(add_numeric(lc[1]))
            elif vtype == 3:  # string:
                box.controls = [add_textual(lc[0])]
                if oper in [6, 7]:
                    gui.widgetLabel(box, _tr.m[1576, " and "])
                ...
                if oper > 5:
                    gui.widgetLabel(box, _tr.m[1577, " and "])
    ...
    def _values_to_floats(attr, values):
        ...
        try:
            ...
            if not all(ok):
                raise ValueError((_tr.m[1578, 'Some values could not be parsed as floats'] + _tr.e(_tr.c(1579, f' in the current locale: {values}'))))
        except TypeError:
    ...
    def send_report(self):
        if not self.data:
            self.report_paragraph(_tr.m[1580, "No data."])
            return

        pdesc = None
        describe_domain = False
        for d in (self.data_desc, self.match_desc, self.nonmatch_desc):
            if not d or not d[_tr.m[1581, "Data instances"]]:
                continue
            ndesc = d.copy()
            del ndesc[_tr.m[1582, "Data instances"]]
            ...
            if oper == len(names) - 1:
                ...
                if name == _tr.m[1583, "is one of"]:
                    ...
                    if len(valnames) == 1:
                        ...
                        valstr = _tr.e(_tr.c(1584, f"{', '.join(valnames[:-1])} or {valnames[-1]}"))
                    conditions.append(_tr.e(_tr.c(1585, f"{attr} is {valstr}")))
                elif values and values[0]:
                    value = values[0] - 1
                    conditions.append(f"{attr} {name} {attr.values[value]}")
            elif var_type == 3:  # string variable
                conditions.append(
                    _tr.e(_tr.c(1586, f"{attr} {name} {' and '.join(map(repr, values))}")))
            elif var_type == 4:  # time
                values = (value.toString(format=Qt.ISODate) for value in values)
                conditions.append(_tr.e(_tr.c(1587, f"{attr} {name} {' and '.join(values)}")))
            elif all(x for x in values):  # numeric variable
                conditions.append(_tr.e(_tr.c(1588, f"{attr} {name} {' and '.join(values)}")))
        items = OrderedDict()
        if describe_domain:
            items.update(self.data_desc)
        else:
            items[_tr.m[1589, "Instances"]] = self.data_desc[_tr.m[1590, "Data instances"]]
        items[_tr.m[1591, "Condition"]] = _tr.m[1592, " AND "].join(conditions) or _tr.m[1593, "no conditions"]
        self.report_items(_tr.m[1594, "Data"], items)
        if describe_domain:
            self.report_items(_tr.m[1595, "Matching data"], self.match_desc)
            self.report_items(_tr.m[1596, "Non-matching data"], self.nonmatch_desc)
        else:
            match_inst = \
                bool(self.match_desc) and \
                self.match_desc[_tr.m[1597, "Data instances"]]
            nonmatch_inst = \
                bool(self.nonmatch_desc) and \
                self.nonmatch_desc[_tr.m[1598, "Data instances"]]
            self.report_items(
                _tr.m[1599, "Output"],
                ((_tr.m[1600, "Matching data"],
                  _tr.e(_tr.c(1601, f"{match_inst} {pl(match_inst, 'instance')}")) if match_inst else _tr.m[1602, "None"]),
                 (_tr.m[1603, "Non-matching data"],
                  nonmatch_inst > 0 and _tr.e(_tr.c(1604, f"{nonmatch_inst} {pl(nonmatch_inst, 'instance')}")))))
