...
class NoGroupVariable(InputValidationError):
    message = _tr.m[3833, "Input does not have any suitable labels"]
...
class OWSilhouettePlot(widget.OWWidget):
    name = _tr.m[3834, "Silhouette Plot"]
    description = (_tr.m[3835, "Visually assess cluster quality and "] + _tr.m[3836, "the degree of cluster membership."])

    icon = "icons/SilhouettePlot.svg"
    priority = 300
    keywords = _tr.m[3837, "silhouette plot"]

    class Inputs:
        data = Input(_tr.m[3838, "Data"], (Orange.data.Table, Orange.misc.DistMatrix))

    class Outputs:
        selected_data = Output(_tr.m[3839, "Selected Data"], Orange.data.Table, default=True)
    ...
    Distances = [(_tr.m[3840, "Euclidean"], Orange.distance.Euclidean),
                 (_tr.m[3841, "Manhattan"], Orange.distance.Manhattan),
                 (_tr.m[3842, "Cosine"], Orange.distance.Cosine)]

    graph_name = "scene"  # QGraphicsScene

    class Error(widget.OWWidget.Error):
        need_two_clusters = Msg(_tr.m[3843, "Need at least two non-empty clusters"])
        singleton_clusters_all = Msg(_tr.m[3844, "All clusters are singletons"])
        memory_error = Msg(_tr.m[3845, "Not enough memory"])
        value_error = Msg(_tr.m[3846, "Distances could not be computed: '{}'"])
        input_validation_error = Msg("{}")
        not_symmetric = widget.Msg(_tr.m[3847, "Distance matrix is not symmetric."])

    class Warning(widget.OWWidget.Warning):
        missing_cluster_assignment = Msg(
            _tr.m[3848, "{} instance{s} omitted (missing cluster assignment)"])
        nan_distances = Msg(_tr.m[3849, "{} instance{s} omitted (undefined distances)"])
        ignoring_categorical = Msg(_tr.m[3850, "Ignoring categorical features"])
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[3852, "Grouping"])
        self.cluster_var_model = itemmodels.VariableListModel(
            parent=self, placeholder=_tr.m[3853, "(None)"])
        ...
        gui.checkBox(
            box, self, "group_by_cluster", _tr.m[3854, "Show in groups"],
            callback=self._replot)

        box = gui.vBox(self.controlArea, _tr.m[3855, "Bars"])
        gui.widgetLabel(box, _tr.m[3856, "Bar width:"])
        gui.hSlider(
            box, self, "bar_size", minValue=1, maxValue=10, step=1,
            callback=self._update_bar_size)
        gui.widgetLabel(box, _tr.m[3857, "Annotations:"])
        ...
        self.ann_hidden_warning = warning = gui.widgetLabel(
            ibox, _tr.m[3858, "(increase the width to show)"])
    ...
    def _set_distances(self, distances: DistMatrix):
        if not distances.is_symmetric():
            raise ValidationError(_tr.m[3859, "Distance matrix is not symmetric."])
        if isinstance(distances.row_items, Orange.data.Table) and \
                distances.axis == 1:
            data = distances.row_items
        else:
            raise ValidationError(_tr.m[3860, "Input matrix does not have associated data"])
    ...
    def commit(self):
        ...
        if self.data is not None:
            ...
            proposed = _tr.m[3861, "Silhouette ({})"].format(escape(self.cluster_var.name))
    ...
    def send_report(self):
        ...
        self.report_plot()
        caption = (_tr.m[3862, "Silhouette plot "] + (_tr.e(_tr.c(3863, f"({self.Distances[self.distance_idx][0]} distance), ")) + _tr.e(_tr.c(3864, f"clustered by '{self.cluster_var.name}'"))))
        if self.annotation_var and self._silplot.rowNamesVisible():
            caption += _tr.e(_tr.c(3865, f", annotated with '{self.annotation_var.name}'"))
