...
class DoNotImpute(BaseImputeMethod):
    name = _tr.m[157, "Don't impute"]
    short_name = _tr.m[158, "leave"]
    description = ""
...
class DropInstances(BaseImputeMethod):
    name = _tr.m[159, "Remove instances with unknown values"]
    short_name = _tr.m[160, "drop"]
    description = ""
...
class Average(BaseImputeMethod):
    name = _tr.m[161, "Average/Most frequent"]
    short_name = _tr.m[162, "average"]
    description = _tr.m[163, "Replace with average/mode of the column"]
    ...
    def __call__(self, data, variable, value=None):
        ...
        if value is None:
            if variable.is_continuous:
                ...
                raise TypeError(_tr.m[164, "Variable must be numeric or categorical."])
...
class FixedValueByType(BaseImputeMethod):
    name = _tr.m[166, "Fixed value"]
    short_name = _tr.m[167, "Fixed Value"]
    format = "{var.name}"
...
class Model(BaseImputeMethod):
    _name = _tr.m[168, "Model-based imputer"]
    short_name = _tr.m[169, "model"]
    description = ""
    ...
    def __call__(self, data, variable):
        ...
        if incompatibility_reason is None:
            ...
            raise ValueError(_tr.m[170, "`{}` doesn't support domain type"]
...
class AsValue(BaseImputeMethod):
    name = _tr.m[171, "As a distinct value"]
    short_name = _tr.m[172, "new value"]
    description = ""
    ...
    def __call__(self, data, variable):
        ...
        if variable.is_discrete:
            ...
            fmt = _tr.m[174, "{var.name}_def"]
            indicator_var = Orange.data.DiscreteVariable(
                fmt.format(var=variable),
                values=(_tr.m[175, "undef"], _tr.m[176, "def"]),
...
class ReplaceUnknownsRandom(Transformation):
    ...
    def __init__(self, variable, distribution):
        ...
        if variable.is_discrete:
            ...
            raise TypeError((_tr.m[177, "Only categorical and numeric "] + _tr.m[178, "variables are supported."]))
        csum = np.sum(counts)
        if csum > 0:
...
class Random(BaseImputeMethod):
    name = _tr.m[179, "Random values"]
    short_name = _tr.m[180, "random"]
    description = _tr.m[181, "Replace with a random value"]
