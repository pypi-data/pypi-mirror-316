...
class PLSModel(LinearModel):
    ...
    def components(self):
        ...
        meta_name = get_unique_names(names, _tr.m[195, 'components'])

        n_components = self.skl_model.x_loadings_.shape[1]

        meta_vars = [StringVariable(name=meta_name)]
        metas = np.array(
            [[_tr.e(_tr.c(196, f"Component {i + 1}")) for i in range(n_components)]], dtype=object
        ).T
        dom = Domain(
        ...
        components = Table(dom,
                           ...
                           metas=metas)
        components.name = _tr.m[197, 'components']
        return components

    def coefficients_table(self):
        coeffs = self.coefficients.T
        domain = Domain(
            [ContinuousVariable(_tr.e(_tr.c(198, f"coef {i}"))) for i in range(coeffs.shape[1])],
            metas=[StringVariable(_tr.m[199, "name"])]
        )
        waves = [[attr.name] for attr in self.domain.attributes]
        coef_table = Table.from_numpy(domain, X=coeffs, metas=waves)
        coef_table.name = _tr.m[200, "coefficients"]
        return coef_table
    ...
    def residuals_normal_probability(self, data: Table) -> Table:
        ...
        proposed = [f"{name} ({var.name})" for var in data.domain.class_vars
                    for name in (_tr.m[201, "Sample Quantiles"], _tr.m[202, "Theoretical Quantiles"])]
        ...
        res_table.name = _tr.m[203, "residuals normal probability"]
        return res_table
...
class PLSRegressionLearner(SklLearnerRegression, _FeatureScorerMixin):
    ...
    def incompatibility_reason(self, domain):
        ...
        if not domain.class_vars:
            reason = _tr.m[204, "Numeric targets expected."]
        else:
            for cv in domain.class_vars:
                if not cv.is_continuous:
                    reason = _tr.m[205, "Only numeric target variables expected."]
        return reason

    @property
    def fitted_parameters(self) -> list[Learner.FittedParameter]:
        return [self.FittedParameter("n_components", _tr.m[206, "Components"],
                                     int, 1, None)]
