...
class ScatterBaseParameterSetter(CommonParameterSetter):
    CAT_LEGEND_LABEL = _tr.m[3795, "Categorical legend"]
    NUM_LEGEND_LABEL = _tr.m[3796, "Numerical legend"]
    NUM_LEGEND_SETTING = {
...
class OWScatterPlotBase(gui.OWComponent, QObject):
    ...
    def _create_drag_tooltip(self):
        tip_parts = [
            (Qt.ControlModifier,
             _tr.m[3797, "{}: Append to group"].
             format("Cmd" if sys.platform == "darwin" else "Ctrl")),
            (Qt.ShiftModifier, _tr.m[3798, "Shift: Add group"]),
            (Qt.AltModifier, _tr.m[3799, "Alt: Remove"])
        ]
        all_parts = "<center>" + \
