...
class VizRankDialog(QDialog, ProgressBarMixin, ConcurrentMixin):
    ...
    """
    captionTitle = _tr.m[4010, "Score Plots"]
    show_bars = True

    selectionChanged = Signal(object)
    runStateChanged = Signal(int, dict)

    button_labels = {RunState.Initialized: _tr.m[4011, "Start"],
                     RunState.Ready: _tr.m[4012, "Start"],
                     RunState.Running: _tr.m[4013, "Pause"],
                     RunState.Paused: _tr.m[4014, "Continue"],
                     RunState.Hidden: _tr.m[4015, "Continue"],
                     RunState.Done: _tr.m[4016, "Finished"],
                     RunState.Invalid: _tr.m[4017, "Start"]}
    ...
    def __init__(self, parent):
        ...
        self.button = gui.button(self, self, _tr.m[4018, "Start"], default=True)
    ...
    def set_run_state(self, state: Any) -> None:
        ...
        if state == RunState.Paused:
            self.setWindowTitle(
                _tr.e(_tr.c(4019, f"{self.captionTitle} (paused at {self._progress}%)")))
...
class VizRankDialogNAttrs(VizRankDialogAttrs):
    ...
    def __init__(self, parent,
                 ...
                 n_attrs: int,
                 *, spin_label: str = _tr.m[4020, "Number of variables: "]):
    ...
    def on_n_attrs_changed(self) -> None:
        ...
        if new_attrs == self.n_attrs:
            ...
            self.set_button_state(label=_tr.e(_tr.c(4021, f"Restart with {new_attrs} variables")),
                                  enabled=True)
