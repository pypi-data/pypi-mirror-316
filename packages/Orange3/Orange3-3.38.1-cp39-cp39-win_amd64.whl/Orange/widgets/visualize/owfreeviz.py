...
def run_freeviz(data: Table, projector: FreeViz, state: TaskState):
    ...
    state.set_status(_tr.m[3449, "Calculating..."])
    while True:
...
class InitType(IntEnum):
    ...
    @staticmethod
    def items():
        return [_tr.m[3450, "Circular"], _tr.m[3451, "Random"]]


class OWFreeViz(OWAnchorProjectionWidget, ConcurrentWidgetMixin):
    MAX_INSTANCES = 10000

    name = _tr.m[3452, "FreeViz"]
    description = _tr.m[3453, "Displays FreeViz projection"]
    icon = "icons/Freeviz.svg"
    priority = 240
    keywords = _tr.m[3454, "freeviz, viz"]
    ...
    class Error(OWAnchorProjectionWidget.Error):
        no_class_var = widget.Msg(_tr.m[3455, "Data must have a target variable."])
        multiple_class_vars = widget.Msg(
            _tr.m[3456, "Data must have a single target variable."])
        not_enough_class_vars = widget.Msg(
            _tr.m[3457, "Target variable must have at least two unique values."])
        features_exceeds_instances = widget.Msg(
            _tr.m[3458, "Number of features exceeds the number of instances."])
        too_many_data_instances = widget.Msg(_tr.m[3459, "Data is too large."])
        constant_data = widget.Msg(_tr.m[3460, "All data columns are constant."])
        not_enough_features = widget.Msg(_tr.m[3461, "At least two features are required."])

    class Warning(OWAnchorProjectionWidget.Warning):
        removed_features = widget.Msg((_tr.m[3462, "Categorical features with more than"] + _tr.m[3463, " two values are not shown."]))
    ...
    def _add_controls(self):
        ...
        self.gui.add_control(
            self._effects_box, gui.hSlider, _tr.m[3464, "Hide radius:"], master=self.graph,
    ...
    def __add_controls_start_box(self):
        box = gui.vBox(self.controlArea, box=_tr.m[3465, "Optimize"], spacing=0)
        gui.comboBox(
            box, self, "initialization", label=_tr.m[3466, "Initialization:"],
        ...
        box2 = gui.hBox(box)
        gui.checkBox(
            box2, self, "balance", _tr.m[3467, "Gravity"],
        ...
        self.run_button = gui.button(box, self, _tr.m[3468, "Start"], self._toggle_run)
    ...
    def _toggle_run(self):
        if self.task is not None:
            ...
            self.run_button.setText(_tr.m[3469, "Resume"])
            self.commit.deferred()
        else:
            self._run()
    ...
    def _run(self):
        ...
        self.run_button.setText(_tr.m[3470, "Stop"])
    ...
    def on_done(self, result: Result):
        ...
        self.projection = result.projection
        self.graph.set_sample_size(None)
        self.run_button.setText(_tr.m[3471, "Start"])
    ...
    def on_exception(self, ex: Exception):
        ...
        self.graph.set_sample_size(None)
        self.run_button.setText(_tr.m[3472, "Start"])
    ...
    def enable_controls(self):
        ...
        self.run_button.setText(_tr.m[3473, "Start"])
