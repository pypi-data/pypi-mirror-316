...
class DiscreteTreeNode(TreeNode):
    ...
    def tooltip(self):
        ...
        return '<p>' \
            + text \
            + _tr.m[3963, '{}/{} samples ({:2.3f}%)'].format(
                int(samples), total, ratio * 100) \
            + '<hr>' \
            + (_tr.m[3964, 'Split by '] + splitting_attr.name
...
class ContinuousTreeNode(TreeNode):
    ...
    COLOR_METHODS = {
        _tr.m[3965, 'None']: COLOR_NONE,
        _tr.m[3966, 'Mean']: COLOR_MEAN,
        _tr.m[3967, 'Standard deviation']: COLOR_STD,
    }
    ...
    def tooltip(self):
        ...
        return _tr.m[3968, '<p>Mean: {:2.3f}'].format(mean) \
            + _tr.m[3969, '<br>Standard deviation: {:2.3f}'].format(std) \
            + _tr.m[3970, '<br>{} samples'].format(num_samples) \
            + '<hr>' \
            + (_tr.m[3971, 'Split by '] + splitting_attr.name
               if not self.tree.is_leaf(self.label) else '') \
            + ('<br><br>' if rules_str and not self.tree.is_leaf(self.label) else '') \
            + rules_str \
            + '</p>'
