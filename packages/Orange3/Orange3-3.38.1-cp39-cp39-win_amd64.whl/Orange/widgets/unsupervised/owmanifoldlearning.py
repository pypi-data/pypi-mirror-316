...
class TSNEParametersEditor(ManifoldParametersEditor):
    ...
    metric_values = [("euclidean", _tr.m[2875, "Euclidean"]), ("manhattan", _tr.m[2876, "Manhattan"]),
                     ("chebyshev", _tr.m[2877, "Chebyshev"]), ("jaccard", _tr.m[2878, "Jaccard"])]
    ...
    initialization_values = [("pca", "PCA"), ("random", _tr.m[2879, "Random"])]

    def __init__(self, parent):
        super().__init__(parent)
        self._create_combo_parameter("metric", _tr.m[2880, "Metric:"])
        self._create_spin_parameter("perplexity", 1, 100, _tr.m[2881, "Perplexity:"])
        self._create_spin_parameter("early_exaggeration", 1, 100,
                                    _tr.m[2882, "Early exaggeration:"])
        self._create_spin_parameter("learning_rate", 1, 1000, _tr.m[2883, "Learning rate:"])
        self._create_spin_parameter("n_iter", 250, 10000, _tr.m[2884, "Max iterations:"])
        self._create_radio_parameter("initialization", _tr.m[2885, "Initialization:"])

    def get_report_parameters(self):
        return {_tr.m[2886, "Metric"]: self.parameters_name["metric"],
                _tr.m[2887, "Perplexity"]: self.parameters["perplexity"],
                _tr.m[2888, "Early exaggeration"]: self.parameters["early_exaggeration"],
                _tr.m[2889, "Learning rate"]: self.parameters["learning_rate"],
                _tr.m[2890, "Max iterations"]: self.parameters["n_iter"],
                _tr.m[2891, "Initialization"]: self.parameters_name["initialization"]}
...
class MDSParametersEditor(ManifoldParametersEditor):
    ...
    init_type_values = (("PCA", "PCA (Torgerson)"),
                        ("random", _tr.m[2892, "Random"]))

    def __init__(self, parent):
        super().__init__(parent)
        self._create_spin_parameter("max_iter", 10, 10 ** 4, _tr.m[2893, "Max iterations:"])
        self._create_radio_parameter("init_type", _tr.m[2894, "Initialization:"])
    ...
    def get_report_parameters(self):
        return {_tr.m[2895, "Max iterations"]: self.parameters["max_iter"],
                _tr.m[2896, "Initialization"]: self.parameters_name["init_type"]}
...
class IsomapParametersEditor(ManifoldParametersEditor):
    ...
    def __init__(self, parent):
        super().__init__(parent)
        self._create_spin_parameter("n_neighbors", 1, 10 ** 2, _tr.m[2897, "Neighbors:"])

    def get_report_parameters(self):
        return {_tr.m[2898, "Neighbors"]: self.parameters["n_neighbors"]}

class LocallyLinearEmbeddingParametersEditor(ManifoldParametersEditor):
    n_neighbors = Setting(5)
    max_iter = Setting(100)
    method_index = Setting(0)
    method_values = (("standard", _tr.m[2899, "Standard"]),
                     ("modified", _tr.m[2900, "Modified"]),
                     ("hessian", _tr.m[2901, "Hessian eigenmap"]),
                     ("ltsa", _tr.m[2902, "Local"]))

    def __init__(self, parent):
        super().__init__(parent)
        self._create_combo_parameter("method", _tr.m[2903, "Method:"])
        self._create_spin_parameter("n_neighbors", 1, 10 ** 2, _tr.m[2904, "Neighbors:"])
        self._create_spin_parameter("max_iter", 10, 10 ** 4, _tr.m[2905, "Max iterations:"])

    def get_report_parameters(self):
        return {_tr.m[2906, "Method"]: self.parameters_name["method"],
                _tr.m[2907, "Neighbors"]: self.parameters["n_neighbors"],
                _tr.m[2908, "Max iterations"]: self.parameters["max_iter"]}

class SpectralEmbeddingParametersEditor(ManifoldParametersEditor):
    affinity_index = Setting(0)
    affinity_values = (("nearest_neighbors", _tr.m[2909, "Nearest neighbors"]),
                       ("rbf", _tr.m[2910, "RBF kernel"]))

    def __init__(self, parent):
        super().__init__(parent)
        self._create_combo_parameter("affinity", _tr.m[2911, "Affinity:"])

    def get_report_parameters(self):
        return {_tr.m[2912, "Affinity"]: self.parameters_name["affinity"]}

class OWManifoldLearning(OWWidget):
    name = _tr.m[2913, "Manifold Learning"]
    description = _tr.m[2914, "Nonlinear dimensionality reduction."]
    icon = "icons/Manifold.svg"
    priority = 2200
    keywords = _tr.m[2915, "manifold learning"]
    settings_version = 2

    class Inputs:
        data = Input(_tr.m[2916, "Data"], Table)

    class Outputs:
        transformed_data = Output(_tr.m[2917, "Transformed Data"], Table, dynamic=False,
                                  replaces=[_tr.m[2918, "Transformed data"]])
    ...
    class Error(OWWidget.Error):
        n_neighbors_too_small = Msg((_tr.m[2919, "For chosen method and components, "] + _tr.m[2920, "neighbors must be greater than {}"]))
        manifold_error = Msg("{}")
        sparse_not_supported = Msg(_tr.m[2921, "Sparse data is not supported."])
        out_of_memory = Msg(_tr.m[2922, "Out of memory"])

    class Warning(OWWidget.Warning):
        graph_not_connected = Msg(_tr.m[2923, "Disconnected graph, embedding may not work"])
    ...
    def __init__(self):
        ...
        # GUI
        method_box = gui.vBox(self.controlArea, _tr.m[2924, "Method"])
        ...
        output_box = gui.vBox(self.controlArea, _tr.m[2925, "Output"])
        self.n_components_spin = gui.spin(
            output_box, self, "n_components", 1, 10, label=_tr.m[2926, "Components:"],
    ...
    def commit(self):
        ...
        def _handle_disconnected_graph_warning(msg, *args, **kwargs):
            if msg.startswith(_tr.m[2927, "Graph is not fully connected"]):
    ...
    def send_report(self):
        method = self.MANIFOLD_METHODS[self.manifold_method_index]
        self.report_items(((_tr.m[2928, "Method"], method.name),))
        parameters = {_tr.m[2929, "Number of components"]: self.n_components}
        parameters.update(self.params_widget.get_report_parameters())
        self.report_items(_tr.m[2930, "Method parameters"], parameters)
        if self.data:
            self.report_data(_tr.m[2931, "Data"], self.data)
