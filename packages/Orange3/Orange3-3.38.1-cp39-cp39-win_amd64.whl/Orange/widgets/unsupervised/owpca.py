...
MAX_COMPONENTS = 100
LINE_NAMES = [_tr.m[2968, "component variance"], _tr.m[2969, "cumulative variance"]]


class OWPCA(widget.OWWidget, ConcurrentWidgetMixin):
    name = _tr.m[2970, "PCA"]
    description = _tr.m[2971, "Principal component analysis with a scree-diagram."]
    icon = "icons/PCA.svg"
    priority = 3050
    keywords = _tr.m[2972, "pca, principal component analysis, linear transformation"]

    class Inputs:
        data = Input(_tr.m[2973, "Data"], Table)

    class Outputs:
        transformed_data = Output(_tr.m[2974, "Transformed Data"], Table, replaces=[_tr.m[2975, "Transformed data"]])
        data = Output(_tr.m[2976, "Data"], Table, default=True)
        components = Output(_tr.m[2977, "Components"], Table)
        pca = Output(_tr.m[2978, "PCA"], PCA, dynamic=False)
    ...
    class Warning(widget.OWWidget.Warning):
        trivial_components = widget.Msg(
            (_tr.m[2979, "All components of the PCA are trivial (explain 0 variance). "] + _tr.m[2980, "Input data is constant (or near constant)."]))

    class Error(widget.OWWidget.Error):
        no_features = widget.Msg(_tr.m[2981, "At least 1 feature is required"])
        no_instances = widget.Msg(_tr.m[2982, "At least 1 data instance is required"])
    ...
    def __init__(self):
        ...
        form = QFormLayout()
        box = gui.widgetBox(self.controlArea, _tr.m[2983, "Components Selection"],
                            orientation=form)
        ...
        self.components_spin.setSpecialValueText(_tr.m[2984, "All"])
        ...
        form.addRow(_tr.m[2985, "Components:"], self.components_spin)
        form.addRow(_tr.m[2986, "Explained variance:"], self.variance_spin)

        # Options
        self.options_box = gui.vBox(self.controlArea, _tr.m[2987, "Options"])
        self.normalize_box = gui.checkBox(
            self.options_box, self, "normalize",
            _tr.m[2988, "Normalize variables"], callback=self._update_normalize,
            attribute=Qt.WA_LayoutUsesWidgetRect
        )

        self.maxp_spin = gui.spin(
            self.options_box, self, "maxp", 1, MAX_COMPONENTS,
            label=_tr.m[2989, "Show only first"], callback=self._setup_plot,
        ...
        self.plot = SliderGraph(
            _tr.m[2990, "Principal Components"], _tr.m[2991, "Proportion of variance"],
    ...
    def set_data(self, data):
        ...
        if isinstance(data, SqlTable):
            if data.approx_len() < AUTO_DL_LIMIT:
                ...
                self.information(_tr.m[2992, "Data has been sampled"])
    ...
    def commit(self):
        ...
        if self._pca is not None:
            ...
            meta_name = get_unique_names(proposed, _tr.m[2993, 'components'])
            meta_vars = [StringVariable(name=meta_name)]
            metas = numpy.array(
                [[_tr.e(_tr.c(2994, f"PC{i + 1}"))for i in range(self.ncomponents)]], dtype=object
            ).T
            if self._variance_ratio is not None:
            ...
            components = Table(dom, self._pca.components_[:self.ncomponents],
                               metas=metas)
            components.name = _tr.m[2995, 'components']
    ...
    def send_report(self):
        if self.data is None:
            return
        self.report_items((
            (_tr.m[2996, "Normalize data"], bool_str(self.normalize)),
            (_tr.m[2997, "Selected components"], self.ncomponents),
            (_tr.m[2998, "Explained variance"], f"{self.variance_covered:.3f} %")
        ))
        self.report_plot()
