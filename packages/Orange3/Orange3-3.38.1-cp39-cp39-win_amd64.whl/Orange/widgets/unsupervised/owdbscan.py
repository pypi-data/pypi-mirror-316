...
class OWDBSCAN(widget.OWWidget):
    name = "DBSCAN"
    description = _tr.m[2581, "Density-based spatial clustering."]
    icon = "icons/DBSCAN.svg"
    priority = 2150

    class Inputs:
        data = Input(_tr.m[2582, "Data"], Table)

    class Outputs:
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table)

    class Error(widget.OWWidget.Error):
        not_enough_instances = Msg((_tr.m[2583, "Not enough unique data instances. "] + _tr.m[2584, "At least two are required."]))

    METRICS = [
        (_tr.m[2585, "Euclidean"], "euclidean"),
        (_tr.m[2586, "Manhattan"], "cityblock"),
        (_tr.m[2587, "Cosine"], "cosine")
    ]
    ...
    def __init__(self):
        ...
        box = gui.widgetBox(self.controlArea, _tr.m[2588, "Parameters"])
        gui.spin(box, self, "min_samples", 1, 100, 1,
                 callback=self._min_samples_changed,
                 label=_tr.m[2589, "Core point neighbors"])
        gui.doubleSpin(box, self, "eps", EPS_BOTTOM_LIMIT, 1000, 0.01,
                       callback=self._eps_changed,
                       label=_tr.m[2590, "Neighborhood distance"])

        box = gui.widgetBox(self.controlArea, self.tr(_tr.m[2591, "Distance Metric"]))
        gui.comboBox(box, self, "metric_idx",
                     items=list(zip(*self.METRICS))[0],
                     callback=self._metirc_changed)
        gui.checkBox(box, self, "normalize", _tr.m[2592, "Normalize features"],
        ...
        self.plot = SliderGraph(
            x_axis_label=_tr.m[2593, "Data items sorted by score"],
            y_axis_label=_tr.m[2594, "Distance to the k-th nearest neighbour"],
    ...
    def send_data(self):
        ...
        u_clust_var = get_unique_names(names, _tr.m[2595, "Cluster"])
        clust_var = DiscreteVariable(
            u_clust_var, values=[_tr.m[2596, "C%d"] % (x + 1) for x in range(k)])

        u_in_core = get_unique_names(names + [u_clust_var], _tr.m[2597, "DBSCAN Core"])
