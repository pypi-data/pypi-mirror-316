...
def _get_points(numbers: list[str], typ: Type[T]) -> tuple[T, ...]:
    try:
        ...
        msg = str(exc)
        raise ValueError(_tr.e(_tr.c(3281, f"invalid value ({msg[msg.rindex(':') + 1:]})"))) from exc
...
def _numbers_from_no_dots(
    ...
    if under and over:
        raise ValueError(_tr.e(_tr.c(3282, f"value must be between {minimum} and {maximum}")))
    if under:
        raise ValueError(_tr.e(_tr.c(3283, f"value must be at least {minimum}")))
    if over:
        raise ValueError(_tr.e(_tr.c(3284, f"value must be at most {maximum}")))
    return steps
...
def _numbers_from_dots(
    ...
    points = text.replace("...", " ... ").replace(",", " ").split()
    if points.count("...") > 1:
        raise ValueError(_tr.m[3285, "multiple '...'."])
    dotind = points.index("...")
    pre = _get_points(points[:dotind], typ)
    post = _get_points(points[dotind + 1:], typ)
    if pre and post and pre[-1] >= post[0]:
        raise ValueError(_tr.m[3286, "values before '...' must be smaller than values after."])

    diffs = {y - x for x, y in zip(pre, pre[1:])} \
            | {y - x for x, y in zip(post, post[1:])}
    if not diffs:
        raise ValueError(_tr.m[3287, "at least two values are required before or after '...'."])
    diff_of_diffs = max(diffs) - min(diffs)
    if diff_of_diffs > 1e-10:
        raise ValueError(_tr.m[3288, "points must be in uniform order."])
    diff = next(iter(diffs))
    if typ is float:
        diff = round(diff, 7)
    if diff <= 0:
        raise ValueError(_tr.m[3289, "points must be in increasing order."])

    minpoint = pre[0] if pre else minimum
    maxpoint = post[-1] if post else maximum
    if minpoint is None:
        raise ValueError(_tr.m[3290, "minimum value is missing."])
    if maxpoint is None:
        raise ValueError(_tr.m[3291, "maximum value is missing."])
    if enforce_range:
        if minimum is not None and minpoint < minimum:
            raise ValueError(_tr.e(_tr.c(3292, f"minimum value is below the minimum {minimum}.")))
        if maximum is not None and maxpoint > maximum:
            raise ValueError(_tr.e(_tr.c(3293, f"maximum value is above the maximum {maximum}.")))
    steps = (maxpoint - minpoint) // diff
    if (minpoint - maxpoint) % diff > 1e-10:
        if pre and post:
            raise ValueError(
                _tr.m[3294, "the sequence before '...' does not end with the sequence after it."])
        if not pre:
