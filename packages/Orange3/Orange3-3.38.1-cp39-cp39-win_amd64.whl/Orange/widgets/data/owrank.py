...
# Default scores.
CLS_SCORES = [
    ScoreMeta(_tr.m[1408, "Information Gain"], _tr.m[1409, "Info. gain"],
              score.InfoGain, ProblemType.CLASSIFICATION, False),
    ScoreMeta(_tr.m[1410, "Information Gain Ratio"], _tr.m[1411, "Gain ratio"],
              score.GainRatio, ProblemType.CLASSIFICATION, True),
    ScoreMeta(_tr.m[1412, "Gini Decrease"], "Gini",
...
REG_SCORES = [
    ScoreMeta(_tr.m[1413, "Univariate Regression"], _tr.m[1414, "Univar. reg."],
...
class OWRank(OWWidget, ConcurrentWidgetMixin):
    name = _tr.m[1415, "Rank"]
    description = _tr.m[1416, "Rank and filter data features by their relevance."]
    icon = "icons/Rank.svg"
    priority = 1102
    keywords = _tr.m[1417, "rank, filter"]

    buttons_area_orientation = Qt.Vertical

    class Inputs:
        data = Input(_tr.m[1418, "Data"], Table)
        scorer = MultiInput(_tr.m[1419, "Scorer"], score.Scorer, filter_none=True)

    class Outputs:
        reduced_data = Output(_tr.m[1420, "Reduced Data"], Table, default=True)
        scores = Output(_tr.m[1421, "Scores"], Table)
        features = Output(_tr.m[1422, "Features"], AttributeList, dynamic=False)
    ...
    class Information(OWWidget.Information):
        no_target_var = Msg(_tr.m[1423, "Data does not have a (single) target variable."])
        missings_imputed = Msg(_tr.m[1424, 'Missing values will be imputed as needed.'])

    class Error(OWWidget.Error):
        invalid_type = Msg(_tr.m[1425, "Cannot handle target variable type {}"])
        inadequate_learner = Msg(_tr.m[1426, "Scorer {} inadequate: {}"])
        no_attributes = Msg(_tr.m[1427, "Data does not have a single attribute."])

    class Warning(OWWidget.Warning):
        renamed_variables = Msg(
            _tr.m[1428, "Variables with duplicated names have been renamed."])
    ...
    def __init__(self):
        ...
        for scoring_methods in (CLS_SCORES,
                                REG_SCORES,
                                []):
            box = gui.vBox(None, _tr.m[1429, "Scoring Methods"] if scoring_methods else None)
        ...
        selMethBox = gui.vBox(self.buttonsArea, _tr.m[1430, "Select Attributes"])
        ...
        b1 = button(self.tr(_tr.m[1431, "None"]), OWRank.SelectNone)
        b2 = button(self.tr(_tr.m[1432, "All"]), OWRank.SelectAll)
        b3 = button(self.tr(_tr.m[1433, "Manual"]), OWRank.SelectManual)
        b4 = button(self.tr(_tr.m[1434, "Best ranked:"]), OWRank.SelectNBest)
    ...
    def handleNewSignals(self):
        self.setStatusMessage(_tr.m[1435, 'Running'])
        self.update_scores()
    ...
    def send_report(self):
        if not self.data:
            return
        self.report_domain(_tr.m[1436, "Input"], self.data.domain)
        self.report_table(_tr.m[1437, "Ranks"], self.ranksView, num_format="{:.3f}")
        if self.out_domain_desc is not None:
            self.report_items(_tr.m[1438, "Output"], self.out_domain_desc)
    ...
    def create_scores_table(self, labels):
        ...
        if not model_list or len(model_list[0]) == 2:  # Empty or just first two columns
            return None
        unique, renamed = get_unique_names_duplicates(labels + (_tr.m[1439, 'Feature'],),
        ...
        new_table.name = _tr.m[1440, "Feature Scores"]
        return new_table
