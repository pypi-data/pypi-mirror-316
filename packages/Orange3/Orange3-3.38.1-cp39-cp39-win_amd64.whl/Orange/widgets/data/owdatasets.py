...
class OWDataSets(OWWidget):
    name = _tr.m[610, "Datasets"]
    description = _tr.m[611, "Load a dataset from an online repository"]
    icon = "icons/DataSets.svg"
    priority = 20
    replaces = ["orangecontrib.prototypes.widgets.owdatasets.OWDataSets"]
    keywords = _tr.m[612, "datasets, online, data, sets"]
    ...
    DEFAULT_LANG = _tr.m[613, "English"]
    ALL_LANGUAGES = _tr.m[614, "All Languages"]

    # These two combo options are translatable; others (domain names) are not
    GENERAL_DOMAIN_LABEL = _tr.m[615, "(General)"]
    ALL_DOMAINS_LABEL = _tr.m[616, "(Show all)"]
    ...
    HEADER_SCHEMA = [
        ['islocal', {'label': ''}],
        ['title', {'label': _tr.m[617, 'Title']}],
        ['size', {'label': _tr.m[618, 'Size']}],
        ['instances', {'label': _tr.m[619, 'Instances']}],
        ['variables', {'label': _tr.m[620, 'Variables']}],
        ['target', {'label': _tr.m[621, 'Target']}],
        ['tags', {'label': _tr.m[622, 'Tags']}]
    ]  # type: List[str, dict]

    IndicatorBrushes = (QBrush(Qt.darkGray), QBrush(QColor(0, 192, 0)))

    class Error(OWWidget.Error):
        no_remote_datasets = Msg(_tr.m[623, "Could not fetch dataset list"])

    class Warning(OWWidget.Warning):
        only_local_datasets = Msg((_tr.m[624, "Could not fetch datasets list, only local "] + _tr.m[625, "cached datasets are shown"]))

    class Outputs:
        data = Output(_tr.m[626, "Data"], Orange.data.Table)
    ...
    def __init__(self):
        ...
        self.filterLineEdit = QLineEdit(
            textChanged=self.filter, placeholderText=_tr.m[627, "Search for data set ..."]
        )
        self.filterLineEdit.setToolTip(
            _tr.m[628, "Typing four letters or more overrides domain and language filters"])
        layout.addWidget(self.filterLineEdit)

        self.combo_elements = []

        layout.addSpacing(20)
        label = QLabel(_tr.m[629, "Show data sets in "])
        ...
        self.view = TreeViewWithReturn(
            ...
            toolTip=_tr.m[631, "Press Return or double-click to send"]
        )
        ...
        box = gui.widgetBox(self.splitter, _tr.m[632, "Description"], addToLayout=False)
        ...
        self.setStatusMessage(_tr.m[633, "Initializing"])
    ...
    def set_model(self, model, current_index):
        ...
        scw(self.Header.size, 20 + max(width(_tr.m[634, "888 bytes "]), width("9999.9 MB ")))
    ...
    def commit(self):
        ...
        if di is not None:
            ...
            if not di.islocal:
                ...
                self.setStatusMessage(_tr.m[635, "Fetching..."])
                self.setBlocking(True)
...
def description_html(datainfo):
    ...
    source = _tr.e(_tr.c(637, f", from {datainfo.source}")) if datainfo.source else ""
    ...
    if seealso:
        html.append(_tr.m[638, "<small><b>See Also</b>\n"] + seealso + "</small>")
    refs = make_html_list(datainfo.references)
    if refs:
        html.append(_tr.m[639, "<small><b>References</b>\n"] + refs + "</small>")
    return "\n".join(html)
