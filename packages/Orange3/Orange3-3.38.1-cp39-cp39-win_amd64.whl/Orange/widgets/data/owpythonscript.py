...
class FunctionSignature(FakeSignatureMixin, QLabel):
    def __init__(self, parent, highlighting_scheme, font, function_name="python_script"):
        ...
        self.prefix = ('<b style="color: ' +
                       ...
                       (';">def </b>' + '<span style="color: ') +
                       self.highlighting_scheme[Name.Function].split(' ')[-1] +
                       ';">' + function_name + ('</span>' + '<span style="color: ') +
...
class OWPythonScript(OWWidget):
    name = _tr.m[1353, "Python Script"]
    description = _tr.m[1354, "Write a Python script and run it on input data or models."]
    category = _tr.m[1355, "Transform"]
    icon = "icons/PythonScript.svg"
    priority = 3150
    keywords = _tr.m[1356, "program, function"]
    ...
    def __init__(self):
        ...
        self.editorBox = gui.vBox(self, box=_tr.m[1357, "Editor"], spacing=4)
        ...
        self.editor_controls = gui.vBox(self.controlArea, box=_tr.m[1358, 'Preferences'])
        ...
        gui.checkBox(
            self.vim_box, self, 'vimModeEnabled', _tr.m[1359, 'Vim mode'],
            tooltip=_tr.m[1360, "Only for the coolest."],
        ...
        self.controlBox = gui.vBox(self.controlArea, _tr.m[1361, 'Library'])
        ...
        action.setToolTip(_tr.m[1362, "Add a new script to the library"])
        action.triggered.connect(self.onAddScript)
        w.addAction(action)

        action = QAction(unicodedata.lookup("MINUS SIGN"), self)
        action.setToolTip(_tr.m[1363, "Remove script from library"])
        action.triggered.connect(self.onRemoveScript)
        w.addAction(action)

        action = QAction(_tr.m[1364, "Update"], self)
        action.setToolTip(_tr.m[1365, "Save changes in the editor to library"])
        action.setShortcut(QKeySequence(QKeySequence.Save))
        action.triggered.connect(self.commitChangesToLibrary)
        w.addAction(action)

        action = QAction(_tr.m[1366, "More"], self, toolTip=_tr.m[1367, "More actions"])

        new_from_file = QAction(_tr.m[1368, "Import Script from File"], self)
        save_to_file = QAction(_tr.m[1369, "Save Selected Script to File"], self)
        restore_saved = QAction(_tr.m[1370, "Undo Changes to Selected Script"], self)
        ...
        self.execute_button = gui.button(self.buttonsArea, self, _tr.m[1371, 'Run'], callback=self.commit)

        self.run_action = QAction(_tr.m[1372, "Run script"], self, triggered=self.commit,
                                  shortcut=QKeySequence(Qt.ControlModifier | Qt.Key_R))
        self.addAction(self.run_action)

        self.saveAction = action = QAction(_tr.m[1373, "&Save"], self.text)
        action.setToolTip(_tr.m[1374, "Save script to file"])
        action.setShortcut(QKeySequence(QKeySequence.Save))
        action.setShortcutContext(Qt.WidgetWithChildrenShortcut)
        action.triggered.connect(self.saveScript)

        self.consoleBox = gui.vBox(self.splitCanvas, _tr.m[1375, 'Console'])
    ...
    def onAddScript(self, *_):
        self.libraryList.append(Script(_tr.m[1376, "New script"], self.text.toPlainText(), 0))
        self.setSelectedScript(len(self.libraryList) - 1)

    def onAddScriptFromFile(self, *_):
        filename, _ = QFileDialog.getOpenFileName(
            self, _tr.m[1377, 'Open Python Script'],
            os.path.expanduser("~/"),
            _tr.m[1378, 'Python files (*.py)\nAll files(*.*)']
        )
        if filename:
    ...
    def saveScript(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(
            self, _tr.m[1379, 'Save Python Script'],
            filename,
            _tr.m[1380, 'Python files (*.py)\nAll files(*.*)']
        )
        ...
        for signal in self.signal_names:
            ...
            if not isinstance(out_var, signal_type) and out_var is not None:
                self.Error.add_message(signal,
                                       _tr.m[1382, "'{}' has to be an instance of '{}'."].
