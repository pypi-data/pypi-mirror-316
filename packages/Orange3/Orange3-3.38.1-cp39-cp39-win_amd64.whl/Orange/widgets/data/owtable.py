...
class OWTable(OWWidget):
    name = _tr.m[1651, "Data Table"]
    description = _tr.m[1652, "View the dataset in a spreadsheet."]
    icon = "icons/Table.svg"
    priority = 50
    keywords = _tr.m[1653, "data table, view"]

    class Inputs:
        data = Input(_tr.m[1654, "Data"], Table, default=True)
        data_subset = Input(_tr.m[1655, "Data Subset"], Table)

    class Outputs:
        selected_data = Output(_tr.m[1656, "Selected Data"], Table, default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table)

    class Warning(OWWidget.Warning):
        missing_sort_columns = Msg(
            (_tr.m[1657, "Cannot restore sorting.\n"] + _tr.m[1658, "Missing columns in input table: {}"])
        )
        non_sortable_input = Msg(
            (_tr.m[1659, "Cannot restore sorting.\n"] + _tr.m[1660, "Input table cannot be sorted due to implementation constraints."])
        )
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[1661, "Variables"])
        self.c_show_attribute_labels = gui.checkBox(
            box, self, "show_attribute_labels",
            _tr.m[1662, "Show variable labels (if present)"],
            callback=self._update_variable_labels)

        gui.checkBox(box, self, "show_distributions",
                     _tr.m[1663, 'Visualize numeric values'],
                     callback=self._on_distribution_color_changed)
        gui.checkBox(box, self, "color_by_class", _tr.m[1664, 'Color by instance classes'],
                     callback=self._on_distribution_color_changed)

        box = gui.vBox(self.controlArea, _tr.m[1665, "Selection"])

        gui.checkBox(box, self, "select_rows", _tr.m[1666, "Select full rows"],
                     callback=self._on_select_rows_changed)

        gui.rubber(self.controlArea)

        gui.button(self.buttonsArea, self, _tr.m[1667, "Restore Original Order"],
                   callback=self.restore_order,
                   tooltip=_tr.m[1668, "Show rows in the original order"],
                   autoDefault=False,
