...
METRICS = [
    (_tr.m[1054, "Euclidean"], distance.Euclidean),
    (_tr.m[1055, "Manhattan"], distance.Manhattan),
    (_tr.m[1056, "Mahalanobis"], distance.Mahalanobis),
    (_tr.m[1057, "Cosine"], distance.Cosine),
    (_tr.m[1058, "Jaccard"], distance.Jaccard),
    (_tr.m[1059, "Spearman"], distance.SpearmanR),
    (_tr.m[1060, "Absolute Spearman"], distance.SpearmanRAbsolute),
    (_tr.m[1061, "Pearson"], distance.PearsonR),
    (_tr.m[1062, "Absolute Pearson"], distance.PearsonRAbsolute),
]


class OWNeighbors(OWWidget):
    name = _tr.m[1063, "Neighbors"]
    description = _tr.m[1064, "Compute nearest neighbors in data according to reference."]
    icon = "icons/Neighbors.svg"
    category = _tr.m[1065, "Unsupervised"]
    replaces = ["orangecontrib.prototypes.widgets.owneighbours.OWNeighbours"]

    class Inputs:
        data = Input(_tr.m[1066, "Data"], Table)
        reference = Input(_tr.m[1067, "Reference"], Table)

    class Outputs:
        data = Output(_tr.m[1068, "Neighbors"], Table)

    class Info(OWWidget.Warning):
        removed_references = \
            Msg((_tr.m[1069, "Input data includes reference instance(s).\n"] + _tr.m[1070, "Reference instances are excluded from the output."]))

    class Warning(OWWidget.Warning):
        all_data_as_reference = \
            Msg(_tr.m[1071, "Every data instance is same as some reference"])

    class Error(OWWidget.Error):
        diff_domains = Msg(_tr.m[1072, "Data and reference have different features"])
    ...
    def __init__(self):
        ...
        gui.comboBox(
            ...
            label=_tr.m[1073, "Distance metric: "], items=[d[0] for d in METRICS],
            callback=self.recompute)
        gui.spin(
            box, self, "n_neighbors", label=_tr.m[1074, "Limit number of neighbors to:"],
