...
def format_summary(summary: Union[ApproxSummary, Summary]) -> List[str]:
    def format_part(part: Optional[_ArrayStat]) -> str:
        if isinstance(part, DenseArray):
            ...
            return _tr.e(_tr.c(1717, f" ({perc:.1f} % missing data)"))

        if isinstance(part, SparseArray):
            tag = _tr.m[1718, "sparse"]
        elif isinstance(part, SparseBoolArray):
            tag = _tr.m[1719, "tags"]
        else:  # isinstance(part, NotAvailable)
            return ""
        dens = 100 * part.non_nans / (part.nans + part.non_nans)
        return _tr.e(_tr.c(1720, f" ({tag}, density {dens:.2f} %)"))

    text = []
    if isinstance(summary, ApproxSummary):
        if summary.len.done():
            ninst = summary.len.result()
            text.append(_tr.e(_tr.c(1721, f"{ninst} {pl(ninst, 'instance')}")))
        else:
            ninst = summary.approx_len
            text.append(_tr.e(_tr.c(1722, f"~{ninst} {pl(ninst, 'instance')}")))
    elif isinstance(summary, Summary):
        ninst = summary.len
        text.append(_tr.e(_tr.c(1723, f"{ninst} {pl(ninst, 'instance')}")))
        if sum(p.nans for p in [summary.X, summary.Y, summary.M]) == 0:
            text[-1] += _tr.m[1724, " (no missing data)"]

    nattrs = len(summary.domain.attributes)
    text.append(_tr.e(_tr.c(1725, f"{nattrs} {pl(nattrs, 'feature')}"))
                + format_part(summary.X))

    if not summary.domain.class_vars:
        text.append(_tr.m[1726, "No target variable."])
    else:
        nclasses = len(summary.domain.class_vars)
        if nclasses > 1:
            c_text = _tr.e(_tr.c(1727, f"{nclasses} {pl(nclasses, 'outcome')}"))
        elif summary.domain.has_continuous_class:
            c_text = _tr.m[1728, "Numeric outcome"]
        else:
            nvalues = len(summary.domain.class_var.values)
            c_text = _tr.e(_tr.c(1729, f"Target with {nvalues} {pl(nvalues, 'value')}"))
        text.append(c_text + format_part(summary.Y))

    nmetas = len(summary.domain.metas)
    if nmetas:
        text.append(_tr.e(_tr.c(1730, f"{nmetas} {pl(nmetas, 'meta attribute')}"))
                    + format_part(summary.M))
    else:
        text.append(_tr.m[1731, "No meta attributes."])
    return text
