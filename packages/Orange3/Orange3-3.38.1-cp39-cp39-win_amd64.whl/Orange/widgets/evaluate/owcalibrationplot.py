...
Metrics = [MetricDefinition(*args) for args in (
    (_tr.m[1734, "Calibration curve"], None, (), ""),
    (_tr.m[1735, "Classification accuracy"], (Curves.ca, ), (), ""),
    ("F1", (Curves.f1, ), (), ""),
    (_tr.m[1736, "Sensitivity and specificity"],
     (Curves.sensitivity, Curves.specificity),
     (_tr.m[1737, "sens"], _tr.m[1738, "spec"]),
     (_tr.m[1739, "<p><b>Sensitivity</b> (falling) is the proportion of correctly "] + (_tr.m[1740, "detected positive instances (TP&nbsp;/&nbsp;P).</p>"] + (_tr.m[1741, "<p><b>Specificity</b> (rising) is the proportion of detected "] + _tr.m[1742, "negative instances (TN&nbsp;/&nbsp;N).</p>"])))),
    (_tr.m[1743, "Precision and recall"],
     (Curves.precision, Curves.recall),
     (_tr.m[1744, "prec"], _tr.m[1745, "recall"]),
     (_tr.m[1746, "<p><b>Precision</b> (rising) is the fraction of retrieved instances "] + (_tr.m[1747, "that are relevant, TP&nbsp;/&nbsp;(TP&nbsp;+&nbsp;FP).</p>"] + (_tr.m[1748, "<p><b>Recall</b> (falling) is the proportion of discovered relevant "] + _tr.m[1749, "instances, TP&nbsp;/&nbsp;P.</p>"])))),
    (_tr.m[1750, "Pos and neg predictive value"],
     (Curves.ppv, Curves.npv),
     ("PPV", "TPV"),
     (_tr.m[1751, "<p><b>Positive predictive value</b> (rising) is the proportion of "] + (_tr.m[1752, "correct positives, TP&nbsp;/&nbsp;(TP&nbsp;+&nbsp;FP).</p>"] + (_tr.m[1753, "<p><b>Negative predictive value</b> is the proportion of correct "] + _tr.m[1754, "negatives, TN&nbsp;/&nbsp;(TN&nbsp;+&nbsp;FN).</p>"])))),
    (_tr.m[1755, "True and false positive rate"],
     (Curves.tpr, Curves.fpr),
     ("TPR", "FPR"),
     (_tr.m[1756, "<p><b>True and false positive rate</b> are proportions of detected "] + _tr.m[1757, "and omitted positive instances</p>"])),
)]
...
class OWCalibrationPlot(widget.OWWidget):
    name = _tr.m[1758, "Calibration Plot"]
    description = _tr.m[1759, "Calibration plot based on evaluation of classifiers."]
    icon = "icons/CalibrationPlot.svg"
    priority = 1030
    keywords = _tr.m[1760, "calibration plot"]

    class Inputs:
        evaluation_results = Input(_tr.m[1761, "Evaluation Results"], Results)

    class Outputs:
        calibrated_model = Output(_tr.m[1762, "Calibrated Model"], Model)

    class Error(widget.OWWidget.Error):
        non_discrete_target = Msg((_tr.m[1763, "Calibration plot requires a categorical "] + _tr.m[1764, "target variable."]))
        empty_input = widget.Msg(_tr.m[1765, "Empty result on input. Nothing to display."])
        nan_classes = \
            widget.Msg(_tr.m[1766, "Remove test data instances with unknown classes."])
        all_target_class = widget.Msg(
            _tr.m[1767, "All data instances belong to target class."])
        no_target_class = widget.Msg(
            _tr.m[1768, "No data instances belong to target class."])

    class Warning(widget.OWWidget.Warning):
        omitted_folds = widget.Msg(
            _tr.m[1769, "Test folds where all data belongs to (non)-target are not shown."])
        omitted_nan_prob_points = widget.Msg(
            (_tr.m[1770, "Instance for which the model couldn't compute probabilities are"] + _tr.m[1771, "skipped."]))
        no_valid_data = widget.Msg(_tr.m[1772, "No valid data for model(s) {}"])

    class Information(widget.OWWidget.Information):
        no_output = Msg(_tr.m[1773, "Can't output a model: {}"])
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, box=_tr.m[1774, "Settings"])
        self.target_cb = gui.comboBox(
            box, self, "target_index", label=_tr.m[1775, "Target:"],
            orientation=Qt.Horizontal, callback=self.target_index_changed,
            contentsLength=8, searchable=True)
        gui.checkBox(
            box, self, "display_rug", _tr.m[1776, "Show rug"],
            callback=self._on_display_rug_changed)
        gui.checkBox(
            box, self, "fold_curves", _tr.m[1777, "Curves for individual folds"],
            callback=self._replot)

        self.classifiers_list_box = gui.listBox(
            self.controlArea, self, "selected_classifiers", "classifier_names",
            box=_tr.m[1778, "Classifier"], selectionMode=QListWidget.ExtendedSelection,
            callback=self._on_selection_changed)
        self.classifiers_list_box.setMaximumHeight(100)

        box = gui.vBox(self.controlArea, _tr.m[1779, "Metrics"])
        combo = gui.comboBox(
        ...
        gui.radioButtons(
            ...
            btnLabels=(_tr.m[1780, "Sigmoid calibration"], _tr.m[1781, "Isotonic calibration"]),
            label=_tr.m[1782, "Output model calibration"], callback=self.commit.deferred)

        self.info_box = gui.widgetBox(self.controlArea, _tr.m[1783, "Info"])
    ...
    def _set_explanation(self):
        ...
        axis.setLabel(_tr.m[1784, "Predicted probability"] if self.score == 0
                      else _tr.m[1785, "Threshold probability to classify as positive"])
    ...
    def send_report(self):
        if self.results is None:
            return
        self.report_items((
            (_tr.m[1788, "Target class"], self.target_cb.currentText()),
            (_tr.m[1789, "Output model calibration"],
             self.score == 0
             and (_tr.m[1790, "Sigmoid calibration"],
                  _tr.m[1791, "Isotonic calibration"])[self.output_calibration])
        ))
