...
ERROR_OPTS = [_tr.m[1910, "(None)"], _tr.m[1911, "Difference"], _tr.m[1912, "Absolute difference"],
              _tr.m[1913, "Relative"], _tr.m[1914, "Absolute relative"]]
ERROR_TOOLTIPS = [
    _tr.m[1915, "Don't show columns with errors"],
    _tr.m[1916, "Show difference between predicted and actual value"],
    _tr.m[1917, "Show absolute difference between predicted and actual value"],
    _tr.m[1918, "Show relative difference between predicted and actual value"],
    _tr.m[1919, "Show absolute value of relative difference between predicted and actual value"]]


class OWPredictions(OWWidget):
    name = _tr.m[1920, "Predictions"]
    icon = "icons/Predictions.svg"
    priority = 200
    description = _tr.m[1921, "Display predictions of models for an input dataset."]
    keywords = _tr.m[1922, "predictions"]
    ...
    class Inputs:
        data = Input(_tr.m[1923, "Data"], Orange.data.Table)
        predictors = MultiInput(_tr.m[1924, "Predictors"], Model, filter_none=True)

    class Outputs:
        selected_predictions = Output(_tr.m[1925, "Selected Predictions"], Orange.data.Table,
                                      default=True, replaces=[_tr.m[1926, "Predictions"]])
        annotated = Output(_tr.m[1927, "Predictions"], Orange.data.Table)
        evaluation_results = Output(_tr.m[1928, "Evaluation Results"], Results)

    class Warning(OWWidget.Warning):
        empty_data = Msg(_tr.m[1929, "Empty dataset"])
        wrong_targets = Msg(
            _tr.m[1930, "Some model(s) predict a different target (see more ...)\n{}"])
        missing_targets = Msg((_tr.m[1931, "Instances with missing targets "] + _tr.m[1932, "are ignored while scoring."]))

    class Error(OWWidget.Error):
        predictor_failed = Msg(_tr.m[1933, "Some predictor(s) failed (see more ...)\n{}"])
        scorer_failed = Msg(_tr.m[1934, "Some scorer(s) failed (see more ...)\n{}"])

    settingsHandler = settings.ClassValuesContextHandler()
    score_table = settings.SettingProvider(ScoreTable)

    #: List of selected class value indices in the `class_values` list
    PROB_OPTS = [_tr.m[1935, "(None)"],
                 _tr.m[1936, "Classes in data"], _tr.m[1937, "Classes known to the model"], _tr.m[1938, "Classes in data and model"]]
    PROB_TOOLTIPS = [_tr.m[1939, "Don't show probabilities"],
                     _tr.m[1940, "Show probabilities for classes in the data"],
                     (_tr.m[1941, "Show probabilities for classes known to the model,\n"] + _tr.m[1942, "including those that don't appear in this data"]),
                     (_tr.m[1943, "Show probabilities for classes in data that are also\n"] + _tr.m[1944, "known to the model"])
                     ]
    ...
    TARGET_AVERAGE = _tr.m[1945, "(Average over classes)"]
    ...
    def __init__(self):
        ...
        self._prob_controls = [
            gui.widgetLabel(predopts, _tr.m[1946, "Show probabilities for"]),
            gui.comboBox(
        ...
        self._cls_error_controls = [
            gui.checkBox(
                ...
                _tr.m[1947, "Show classification errors"],
                tooltip=_tr.m[1948, "Show 1 - probability assigned to the correct class"],
                callback=self._update_errors_visibility
            )
        ]

        err_label = gui.widgetLabel(predopts, _tr.m[1949, "Shown regression error: "])
        err_combo = gui.comboBox(
            predopts, self, "show_reg_errors", items=ERROR_OPTS,
            callback=self._reg_error_changed,
            toolTip=_tr.m[1950, "See tooltips for individual options"])
        ...
        gui.rubber(predopts)
        self.reset_button = button = QPushButton(_tr.m[1951, "Restore Original Order"])
        button.clicked.connect(self._reset_order)
        button.setToolTip(_tr.m[1952, "Show rows in the original order"])
        ...
        self.score_opt_box = scoreopts = gui.hBox(
            None, sizePolicy=(QSizePolicy.MinimumExpanding, QSizePolicy.Fixed))
        gui.checkBox(
            scoreopts, self, "show_scores", _tr.m[1953, "Show perfomance scores"],
            callback=self._update_score_table_visibility
        )
        gui.separator(scoreopts, 32)
        self._target_controls = [
            gui.widgetLabel(scoreopts, _tr.m[1954, "Target class:"]),
            gui.comboBox(
    ...
    def _update_scores(self):
        ...
        for pred in self.predictors:
            ...
            row = [QStandardItem(learner_name(pred.predictor)),
                   QStandardItem(_tr.m[1955, "N/A"]), QStandardItem(_tr.m[1956, "N/A"])]
    ...
    def _get_details(self):
        ...
        if n_predictors:
            ...
            details += _tr.e(_tr.c(1959, f"Model: {n_predictors} {pl(n_predictors, 'model')}"))
            if n_valid != n_predictors:
                details += _tr.e(_tr.c(1960, f" ({n_predictors - n_valid} failed)"))
            details += "<ul>"
            for name in pred_names:
                details += f"<li>{name}</li>"
            details += "</ul>"
        else:
            details += _tr.m[1961, "Model:<br>No model on input."]
        return details
    ...
    def _add_error_out_columns(self, slot, newmetas, newcolumns, index):
        if self.shown_errors:
            name = _tr.e(_tr.c(1962, f"{slot.predictor.name} (error)"))
    ...
    def send_report(self):
        ...
        if self.data:
            ...
            if self.is_discrete_class and self.shown_probs != self.NO_PROBS:
                text += _tr.m[1963, '<br>Showing probabilities for ']
                if self.shown_probs == self.MODEL_PROBS:
                    text += _tr.m[1964, "all classes known to the model."]
                elif self.shown_probs == self.DATA_PROBS:
                    text += _tr.m[1965, "all classes that appear in the data."]
                elif self.shown_probs == self.BOTH_PROBS:
                    text += (_tr.m[1966, "all classes that appear in the data "] + _tr.m[1967, "and are known to the model."])
                else:
                    class_idx = self.shown_probs - len(self.PROB_OPTS)
                    text += f"'{self.class_var.values[class_idx]}.'"
            self.report_paragraph('Info', text)
            self.report_table(_tr.m[1968, "Data & Predictions"], merge_data_with_predictions(),
                              header_rows=1, header_columns=1)

            self.report_name(_tr.m[1969, "Scores"])
            if self.is_discrete_class:
                self.report_items([(_tr.m[1970, "Target class"], self.target_class)])
