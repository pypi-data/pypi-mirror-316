...
def check_results_adequacy(results, error_group, check_nan=True):
    ...
    if results is None:
        ...
        error_group.invalid_results(
            _tr.m[2076, "Categorical target variable is required."])
    elif not results.actual.size:
        error_group.invalid_results(
            _tr.m[2077, "Empty result on input. Nothing to display."])
        ...
        error_group.invalid_results(
            _tr.m[2078, "Results contain invalid values."])
    else:
        return results
    ...
    problems = [
        msg for condition, msg in (
            (results.folds is not None and len(results.folds) > 1,
             _tr.m[2079, "each training data sample produces a different model"]),
            (results.models is None,
             (_tr.m[2080, "test results do not contain stored models - try testing "] + _tr.m[2081, "on separate data or on training data"])),
            (len(selection) != 1,
             _tr.m[2082, "select a single model - the widget can output only one"]),
            (require_binary and len(results.domain.class_var.values) != 2,
             _tr.m[2083, "cannot calibrate non-binary models"]))
        if condition]
...
class ScoreTable(OWComponent, QObject):
    ...
    def update_header(self, scorers: List[Score]):
        ...
        for i, name, long_name, id_, in ((0, "Model", "Model", "Model_"),
                                         (1, _tr.m[2084, "Train"], _tr.m[2085, "Train time [s]"], "Train_"),
                                         (2, _tr.m[2086, "Test"], _tr.m[2087, "Test time [s]"], "Test_")):
