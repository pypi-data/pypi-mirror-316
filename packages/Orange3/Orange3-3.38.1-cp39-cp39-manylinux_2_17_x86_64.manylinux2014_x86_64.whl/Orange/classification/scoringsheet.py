...
class ScoringSheetLearner(Learner):
    ...
    def incompatibility_reason(self, domain):
        ...
        if len(domain.class_vars) > 1 and not self.supports_multiclass:
            reason = _tr.m[58, "Too many target variables."]
        elif not domain.has_discrete_class:
            reason = _tr.m[59, "Categorical class variable expected."]
        elif len(domain.class_vars[0].values) > 2:
            reason = _tr.m[60, "Too many target variable values."]
        return reason
    ...
    def fit_storage(self, table):
        if not isinstance(table, Storage):
            ...
            raise ValueError(_tr.m[61, "Class variable contains missing values."])
        ...
        while True:
            try:
                ...
                if learner.k < 1:
                    ...
                    raise ValueError(
                        _tr.m[62, "The number of input features is too low for the current settings."]
                    ) from e
