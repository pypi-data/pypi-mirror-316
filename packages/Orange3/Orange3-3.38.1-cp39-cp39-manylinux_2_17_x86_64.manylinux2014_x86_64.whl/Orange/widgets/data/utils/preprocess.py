...
class StandardItemModel(QStandardItemModel):
    ...
    def moveRow(self, sourceParent, sourceRow, destParent, destRow):
        ...
        if not didmove:
            warnings.warn(
                ("`moveRow` did not succeed! Data model might be " + "in an inconsistent state."),
                RuntimeWarning)
        return didmove
    ...
    def moveRows(self, sourceParent, sourceRow, count,
        ...
        if not didmove:
            warnings.warn(
                ("`moveRows` did not succeed! Data model might be " + "in an inconsistent state."),
                RuntimeWarning)
        return didmove
    ...
    class Frame(QDockWidget):
        ...
        def __init__(self, parent=None, widget=None, title=None, **kwargs):
            ...
            self.__deleteaction = QAction(
                _tr.m[1716, "Remove"], self, shortcut=QKeySequence.Delete,
