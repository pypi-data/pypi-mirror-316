...
class OWCreateClass(widget.OWWidget):
    name = _tr.m[410, "Create Class"]
    description = _tr.m[411, "Create class attribute from a string attribute"]
    icon = "icons/CreateClass.svg"
    category = _tr.m[412, "Transform"]
    keywords = _tr.m[413, "create class"]
    priority = 2300

    class Inputs:
        data = Input(_tr.m[414, "Data"], Table)

    class Outputs:
        data = Output(_tr.m[415, "Data"], Table)
    ...
    class_name = ContextSetting(_tr.m[416, "class"])
    ...
    class Warning(widget.OWWidget.Warning):
        no_nonnumeric_vars = Msg(_tr.m[417, "Data contains only numeric variables."])

    class Error(widget.OWWidget.Error):
        class_name_duplicated = Msg(_tr.m[418, "Class name duplicated."])
        class_name_empty = Msg(_tr.m[419, "Class name should not be empty."])
    ...
    def __init__(self):
        ...
        gui.lineEdit(
            ...
            orientation=Qt.Horizontal, box=_tr.m[420, "New Class Name"])

        variable_select_box = gui.vBox(self.controlArea, _tr.m[421, "Match by Substring"])

        combo = gui.comboBox(
            variable_select_box, self, "attribute", label=_tr.m[422, "From column:"],
        ...
        rules_box.addWidget(QLabel(_tr.m[423, "Name"]), 0, 1)
        rules_box.addWidget(QLabel(_tr.m[424, "Substring"]), 0, 2)
        rules_box.addWidget(QLabel(_tr.m[425, "Count"]), 0, 3, 1, 2)
        self.update_rules()
        ...
        optionsbox = gui.vBox(self.controlArea, _tr.m[426, "Options"])
        gui.checkBox(
            optionsbox, self, "match_beginning", _tr.m[427, "Match only at the beginning"],
            callback=self.options_changed)
        gui.checkBox(
            optionsbox, self, "case_sensitive", _tr.m[428, "Case sensitive"],
            callback=self.options_changed)

        gui.rubber(self.controlArea)

        gui.button(self.buttonsArea, self, _tr.m[429, "Apply"], callback=self.apply)
        ...
        def _set_labels():
            ...
            for (n_matched, n_total), (lab_matched, lab_total), (lab, patt) in \
                ...
                if n_before and (lab or patt):
                    ...
                    if n_matched:
                        tip = (_tr.e(_tr.c(430, f"{n_before} o")) + (_tr.e(_tr.c(431, f"f {n_total} matching {pl(n_total, 'instance')} ")) + _tr.e(_tr.c(432, f"{pl(n_before, 'is|are')} already covered above."))))
                    else:
                        tip = _tr.m[433, "All matching instances are already covered above"]
            ...
            for n_matched, (_, patt) in zip(matches, self.line_edits):
                if not patt.text():
                    patt.setPlaceholderText(
                        _tr.m[434, "(remaining instances)"] if n_matched else _tr.m[435, "(unused)"])
    ...
    def send_report(self):
        ...
        def _cond_part():
            ...
            if patt:
                rule += _tr.e(_tr.c(436, f"if <b>{self.attribute.name}</b> contains <b>{patt}</b>"))
            else:
                rule += _tr.m[437, "otherwise"]
            return rule

        def _count_part():
            aca = _tr.m[438, "already covered above"]
            if not n_matched:
                if n_total == 1:
                    return _tr.e(_tr.c(439, f"the single matching instance is {aca}"))
                elif n_total == 2:
                    return _tr.e(_tr.c(440, f"both matching instances are {aca}"))
                else:
                    return _tr.e(_tr.c(441, f"all {n_total} matching instances are {aca}"))
            elif not patt:
                return _tr.e(_tr.c(442, f"{n_matched} {pl(n_matched, 'instance')}"))
            else:
                m = _tr.e(_tr.c(443, f"{n_matched} matching {pl(n_matched, 'instance')}"))
                if n_matched < n_total:
                    n_already = n_total - n_matched
                    m += _tr.e(_tr.c(444, f" (+{n_already} that {pl(n_already, 'is|are')} {aca})"))
                return m

        if not self.attribute:
            return
        self.report_items(_tr.m[445, "Input"], [(_tr.m[446, "Source attribute"], self.attribute.name)])
        output = ""
        ...
        if output:
            self.report_items(_tr.m[447, "Output"], [(_tr.m[448, "Class name"], self.class_name)])
