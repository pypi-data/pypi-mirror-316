...
class OWSave(OWSaveBase):
    name = _tr.m[1441, "Save Data"]
    description = _tr.m[1442, "Save data to an output file."]
    icon = "icons/Save.svg"
    category = _tr.m[1443, "Data"]
    keywords = _tr.m[1444, "save data, export"]

    settings_version = 3

    class Inputs:
        data = Input(_tr.m[1445, "Data"], Table)

    class Error(OWSaveBase.Error):
        unsupported_sparse = widget.Msg(_tr.m[1446, "Use Pickle format for sparse data."])
    ...
    def __init__(self):
        ...
        self.grid.addWidget(
            gui.checkBox(
                ...
                _tr.m[1447, "Add type annotations to header"],
                tooltip=
                (_tr.m[1448, "Some formats (Tab-delimited, Comma-separated) can include \n"] + _tr.m[1449, "additional information about variables types in header rows."]),
    ...
    def get_filters(cls):
        ...
        return {
            **{f"{w.DESCRIPTION} (*{w.EXTENSIONS[0]})": w
               for w in writers},
            **{_tr.e(_tr.c(1450, f"Compressed {w.DESCRIPTION} (*{w.EXTENSIONS[0]}.gz)")): w
    ...
    def send_report(self):
        ...
        writer = self.writer
        noyes = [_tr.m[1451, "No"], _tr.m[1452, "Yes"]]
        self.report_items((
            (_tr.m[1453, "File name"], self.filename or _tr.m[1454, "not set"]),
            (_tr.m[1455, "Format"], writer and writer.DESCRIPTION),
            (_tr.m[1456, "Type annotations"],
