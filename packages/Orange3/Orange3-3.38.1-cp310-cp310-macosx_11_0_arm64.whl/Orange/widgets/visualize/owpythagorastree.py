...
class OWPythagorasTree(OWWidget):
    name = _tr.m[3684, "Pythagorean Tree"]
    description = _tr.m[3685, 'Pythagorean Tree visualization for tree like-structures.']
    icon = 'icons/PythagoreanTree.svg'
    keywords = _tr.m[3686, "pythagorean tree, fractal"]

    priority = 1000

    class Inputs:
        tree = Input(_tr.m[3687, "Tree"], TreeModel)

    class Outputs:
        selected_data = Output(_tr.m[3688, "Selected Data"], Table, default=True)
    ...
    def __init__(self):
        ...
        self.SIZE_CALCULATION = [
            (_tr.m[3689, 'Normal'], lambda x: x),
            (_tr.m[3690, 'Square root'], lambda x: sqrt(x)),
            (_tr.m[3691, 'Logarithmic'], lambda x: log(x * self.size_log_scale + 1)),
        ]

        # CONTROL AREA
        # Tree info area
        box_info = gui.widgetBox(self.controlArea, _tr.m[3692, 'Tree Info'])
        self.infolabel = gui.widgetLabel(box_info)

        # Display settings area
        box_display = gui.widgetBox(self.controlArea, _tr.m[3693, 'Display Settings'])
        ...
        self.log_scale_box = gui.hSlider(
            ...
            label=_tr.m[3697, 'Log scale factor'], minValue=1, maxValue=100, ticks=False,
            callback=self.invalidate_tree)

        # Plot properties area
        box_plot = gui.widgetBox(self.controlArea, _tr.m[3698, 'Plot Properties'])
        self.cb_show_tooltips = gui.checkBox(
            box_plot, self, 'tooltips_enabled', label=_tr.m[3699, 'Enable tooltips'],
            callback=self.update_tooltip_enabled)
        self.cb_show_legend = gui.checkBox(
            box_plot, self, 'show_legend', label=_tr.m[3700, 'Show legend'],
            callback=self.update_show_legend)

        gui.rubber(self.controlArea)

        gui.button(self.buttonsArea, self, label=_tr.m[3701, "Redraw"], callback=self.redraw)
    ...
    def _update_info_box(self):
        self.infolabel.setText(_tr.m[3702, 'Nodes: {}\nDepth: {}'].format(
    ...
    def _update_log_scale_slider(self):
        ...
        self.log_scale_box.parent().setEnabled(
            self.SIZE_CALCULATION[self.size_calc_idx][0] == _tr.m[3703, 'Logarithmic'])

    def _clear_info_box(self):
        self.infolabel.setText(_tr.m[3704, 'No tree on input'])
    ...
    def _update_target_class_combo(self):
        ...
        if self.data.domain.has_discrete_class:
            label_text = _tr.m[3705, 'Target class']
            values = [c.title() for c in self.data.domain.class_vars[0].values]
            values.insert(0, _tr.m[3706, 'None'])
        else:
            label_text = _tr.m[3707, 'Node color']
