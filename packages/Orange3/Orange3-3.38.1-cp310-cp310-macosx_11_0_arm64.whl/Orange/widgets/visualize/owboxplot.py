...
class OWBoxPlot(widget.OWWidget):
    name = _tr.m[3381, "Box Plot"]
    description = _tr.m[3382, "Visualize the distribution of feature values in a box plot."]
    icon = "icons/BoxPlot.svg"
    priority = 100
    keywords = _tr.m[3383, "box plot, whisker"]

    class Inputs:
        data = Input(_tr.m[3384, "Data"], Orange.data.Table)

    class Outputs:
        selected_data = Output(_tr.m[3385, "Selected Data"], Orange.data.Table, default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Orange.data.Table)

    class Warning(widget.OWWidget.Warning):
        no_vars = widget.Msg(
            _tr.m[3386, "Data contains no categorical or numeric variables"])
    ...
    def __init__(self):
        ...
        sorted_model.sort(0)
        box = gui.vBox(self.controlArea, _tr.m[3387, "Variable"])
        ...
        gui.checkBox(
            ...
            _tr.m[3388, "Order by relevance to subgroups"],
            tooltip=_tr.m[3389, "Order by 𝜒² or ANOVA over the subgroups"],
            callback=self.apply_attr_sorting)

        self.group_vars = VariableListModel(placeholder=_tr.m[3390, "None"])
        sorted_model = SortProxyModel(sortRole=Qt.UserRole)
        sorted_model.setSourceModel(self.group_vars)
        sorted_model.sort(0)

        box = gui.vBox(self.controlArea, _tr.m[3391, "Subgroups"])
        ...
        gui.checkBox(
            ...
            _tr.m[3392, "Order by relevance to variable"],
            tooltip=_tr.m[3393, "Order by 𝜒² or ANOVA over the variable values"],
        ...
        self.display_box = gui.vBox(
            self.controlArea, _tr.m[3394, "Display"],
            sizePolicy=(QSizePolicy.Minimum, QSizePolicy.Maximum))

        gui.checkBox(self.display_box, self, "show_annotations", _tr.m[3395, "Annotate"],
                     callback=self.update_graph)
        self.compare_rb = gui.radioButtonsInBox(
            self.display_box, self, 'compare',
            btnLabels=[_tr.m[3396, "No comparison"], _tr.m[3397, "Compare medians"], _tr.m[3398, "Compare means"]],
            callback=self.update_graph)

        # The vertical size policy is needed to let only the list views expand
        self.stretching_box = box = gui.vBox(
            self.controlArea, box=_tr.m[3399, "Display"],
            sizePolicy=(QSizePolicy.Minimum, QSizePolicy.Fixed))
        self.stretching_box.sizeHint = self.display_box.sizeHint
        gui.checkBox(
            box, self, 'stretched', _tr.m[3400, "Stretch bars"],
            callback=self.update_graph,
            stateWhenDisabled=False)
        gui.checkBox(
            box, self, 'show_labels', _tr.m[3401, "Show box labels"],
            callback=self.update_graph)
        self.sort_cb = gui.checkBox(
            box, self, 'sort_freqs', _tr.m[3402, "Sort by subgroup frequencies"],
            callback=self.update_graph,
            stateWhenDisabled=False)
    ...
    def compute_box_data(self):
        ...
        if self.group_var:
            self.dist = None
            missing_val_str = _tr.e(_tr.c(3403, f"missing '{self.group_var.name}'"))
    ...
    def _compute_tests_cont(self):
        ...
        if self.compare == OWBoxPlot.CompareNone or len(self.stats) < 2:
            ...
            t = (_tr.m[3404, "At least one group has just one instance, "] + _tr.m[3405, "cannot compute significance"])
            ...
            if self.compare == OWBoxPlot.CompareMedians:
                ...
                t, p = stat_ttest()
                t = "" if np.isnan(t) else _tr.e(_tr.c(3406, f"Student's t: {t:.3f} (p={p:.3f}, N={n})"))
        else:
            if self.compare == OWBoxPlot.CompareMedians:
                t = ""
                # U, p = -1, -1
    ...
    def strudel(self, dist, group_val=None):
        ...
        cum = 0
        missing_val_str = _tr.e(_tr.c(3407, f"missing '{attr.name}'"))
        values = attr.values + ("",)
    ...
    def send_report(self):
        ...
        text = ""
        if self.attribute:
            text += _tr.e(_tr.c(3408, f"Box plot for attribute '{self.attribute.name}' "))
        if self.group_var:
            text += _tr.e(_tr.c(3409, f"grouped by '{self.group_var.name}'"))
        if text:
