...
class ParameterSetter(CommonParameterSetter):
    BOTTOM_AXIS_LABEL, IS_VERTICAL_LABEL = _tr.m[3928, "Bottom axis"], _tr.m[3929, "Vertical tick text"]
...
class OWViolinPlot(OWWidget):
    name = _tr.m[3930, "Violin Plot"]
    description = (_tr.m[3931, "Visualize the distribution of feature"] + _tr.m[3932, " values in a violin plot."])
    icon = "icons/ViolinPlot.svg"
    priority = 110
    keywords = _tr.m[3933, "violin plot, kernel, density"]

    class Inputs:
        data = Input(_tr.m[3934, "Data"], Table)

    class Outputs:
        selected_data = Output(_tr.m[3935, "Selected Data"], Table, default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table)

    class Error(OWWidget.Error):
        no_cont_features = Msg(_tr.m[3936, "Plotting requires a numeric feature."])
        not_enough_instances = Msg(_tr.m[3937, "Plotting requires at least two instances."])

    KERNELS = ["gaussian", "epanechnikov", "linear"]
    KERNEL_LABELS = [_tr.m[3938, "Normal"], _tr.m[3939, "Epanechnikov"], _tr.m[3940, "Linear"]]
    SCALE_LABELS = [_tr.m[3941, "Area"], _tr.m[3942, "Count"], _tr.m[3943, "Width"]]
    ...
    def _add_controls(self):
        ...
        self._group_var_model = VariableListModel(placeholder=_tr.m[3944, "None"])
        sorted_model = SortProxyModel(sortRole=Qt.UserRole)
        ...
        box = gui.vBox(self.controlArea, _tr.m[3945, "Variable"])
        box.layout().addWidget(self._value_var_view)
        gui.checkBox(box, self, "order_by_importance",
                     _tr.m[3946, "Order by relevance to subgroups"],
                     tooltip=_tr.m[3947, "Order by 𝜒² or ANOVA over the subgroups"],
                     callback=self.apply_value_var_sorting)

        box = gui.vBox(self.controlArea, _tr.m[3948, "Subgroups"])
        box.layout().addWidget(self._group_var_view)
        gui.checkBox(box, self, "order_grouping_by_importance",
                     _tr.m[3949, "Order by relevance to variable"],
                     tooltip=_tr.m[3950, "Order by 𝜒² or ANOVA over the variable values"],
                     callback=self.apply_group_var_sorting)

        box = gui.vBox(self.controlArea, _tr.m[3951, "Display"],
                       sizePolicy=(QSizePolicy.Minimum, QSizePolicy.Maximum))
        gui.checkBox(box, self, "show_box_plot", _tr.m[3952, "Box plot"],
                     callback=self.__show_box_plot_changed)
        gui.checkBox(box, self, "show_strip_plot", _tr.m[3953, "Density dots"],
                     callback=self.__show_strip_plot_changed)
        gui.checkBox(box, self, "show_rug_plot", _tr.m[3954, "Density lines"],
                     callback=self.__show_rug_plot_changed)
        self._order_violins_cb = gui.checkBox(
            box, self, "order_violins", _tr.m[3955, "Order subgroups"],
            callback=self.__order_violins_changed,
        )
        gui.checkBox(
            box, self, "show_grid", _tr.m[3956, "Show grid"],
            callback=self.__show_grid_changed,
        )
        gui.radioButtons(box, self, "orientation_index",
                         [_tr.m[3957, "Horizontal"], _tr.m[3958, "Vertical"]], label=_tr.m[3959, "Orientation: "],
                         orientation=Qt.Horizontal,
                         callback=self.__orientation_changed)

        box = gui.vBox(self.controlArea, _tr.m[3960, "Density Estimation"],
                       sizePolicy=(QSizePolicy.Minimum, QSizePolicy.Maximum))
        gui.comboBox(box, self, "kernel_index", items=self.KERNEL_LABELS,
                     label=_tr.m[3961, "Kernel:"], labelWidth=60, orientation=Qt.Horizontal,
                     callback=self.__kernel_changed)
        self._scale_combo = gui.comboBox(
            box, self, "scale_index", items=self.SCALE_LABELS,
            label=_tr.m[3962, "Scale:"], labelWidth=60, orientation=Qt.Horizontal,
