...
def format_variables_string(variables):
    ...
    agg = []
    for var_type_name, var_type in [(_tr.m[3218, 'categorical'], DiscreteVariable),
                                    (_tr.m[3219, 'numeric'], ContinuousVariable),
                                    (_tr.m[3220, 'time'], TimeVariable),
                                    (_tr.m[3221, 'string'], StringVariable)]:
    ...
    if len(attrs) > 1:
        var_string = [_tr.e(_tr.c(3222, f'{i} {j}')) for i, j in zip(counts, attrs)]
        var_string = f'{sum(counts)} ({", ".join(var_string)})'
    elif counts[0] == 1:
        var_string = attrs[0]
    else:
        var_string = _tr.e(_tr.c(3223, f'{counts[0]} {attrs[0]}'))
    return var_string
...
def format_summary_details(data: Union[Table, Domain],
    ...
    if isinstance(data, Domain):
        ...
        if name == _tr.m[3224, "untitled"]:
            name = None
        basic = _tr.e(_tr.c(3225, f'{len(data):n} {pl(len(data), "instance")}, '))

    n_features = len(domain.variables) + len(domain.metas)
    basic += _tr.e(_tr.c(3226, f'{n_features} {pl(n_features, "variable")}'))

    features = format_variables_string(domain.attributes)
    features = _tr.e(_tr.c(3227, f'Features: {features}{features_missing}'))

    targets = format_variables_string(domain.class_vars)
    targets = _tr.e(_tr.c(3228, f'Target: {targets}'))

    metas = format_variables_string(domain.metas)
    metas = _tr.e(_tr.c(3229, f'Metas: {metas}'))

    if format == Qt.PlainText:
        details = f"{name}: " if name else _tr.m[3230, "Table with "]
        details += f"{basic}\n{features}\n{targets}"
        if domain.metas:
            details += f"\n{metas}"
    else:
        ...
        if name:
            ...
            descs.append(_nobr(_tr.e(_tr.c(3231, f"Table with {basic}"))))
...
def missing_values(value):
    if value:
        return _tr.e(_tr.c(3232, f' ({value*100:.1f}% missing values)'))
    elif value is None:
        return ''
    else:
        return _tr.m[3233, ' (no missing values)']


def format_multiple_summaries(data_list, type_io=_tr.m[3234, 'input']):
    """
    A function that forms the entire descriptive part of the input/output
    ...
    for (name, data) in data_list:
        if data:
            ...
            details = _tr.e(_tr.c(3235, f'No data on {type_io}.'))
...
def summarize_matrix(matrix: DistMatrix):  # pylint: disable=function-redefined
    ...
    return PartialSummary(
        f"{w}×{h}",
        _nobr(_tr.e(_tr.c(3236, f"{w}×{h} distance matrix"))),
        previewer
    )
...
def summarize_results(results: Results):  # pylint: disable=function-redefined
    ...
    details = (_tr.e(_tr.c(3237, f"{nmethods} {pl(nmethods, 'method')} ")) + _tr.e(_tr.c(3238, f"on {ninstances} test {pl(ninstances, 'instance')}")))
...
def summarize_preprocessor(preprocessor: Preprocess):  # pylint: disable=function-redefined
    if isinstance(preprocessor, PreprocessorList):
        if preprocessor.preprocessors:
            ...
            details = _nobr(_tr.e(_tr.c(3241, f"{_name_of(preprocessor)} (empty)")))
    else:
