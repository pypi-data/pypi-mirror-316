...
class OWCorrespondenceAnalysis(widget.OWWidget):
    name = _tr.m[2560, "Correspondence Analysis"]
    description = _tr.m[2561, "Correspondence analysis for categorical multivariate data."]
    icon = "icons/CorrespondenceAnalysis.svg"
    keywords = _tr.m[2562, "correspondence analysis"]

    class Inputs:
        data = Input(_tr.m[2563, "Data"], Table)

    class Outputs:
        coordinates = Output(_tr.m[2564, "Coordinates"], Table)
    ...
    class Error(widget.OWWidget.Error):
        empty_data = widget.Msg(_tr.m[2565, "Empty dataset"])
        no_disc_vars = widget.Msg(_tr.m[2566, "No categorical data"])
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[2567, "Variables"])
        ...
        axes_box = gui.vBox(self.controlArea, _tr.m[2568, "Axes"])
        ...
        self.infotext = gui.widgetLabel(
            gui.vBox(self.controlArea, _tr.m[2569, "Contribution to Inertia"]), "\n"
        )
    ...
    def commit(self):
        ...
        if self.ca is not None:
            ...
            output_table = Table(
                Domain([ContinuousVariable(_tr.e(_tr.c(2570, f"Component {i + 1}")))
                        for i in range(rf.shape[1])],
                       metas=[StringVariable(_tr.m[2571, "Variable"]),
                              StringVariable(_tr.m[2572, "Value"])]),
                rf, metas=vars_data
            )
    ...
    def _setup_plot(self):
        ...
        ax.setLabel(_tr.m[2573, "Component {} ({:.1f}%)"]
                    .format(p_axes[0] + 1, inertia[p_axes[0]]))
        ax = self.plot.getAxis("left")
        ax.setLabel(_tr.m[2574, "Component {} ({:.1f}%)"]
    ...
    def _update_info(self):
        if self.ca is None:
            ...
            fmt = (_tr.m[2575, "Axis 1: {:.2f}\n"] + _tr.m[2576, "Axis 2: {:.2f}"])
            inertia = self.ca.inertia_of_axis()
            if np.sum(inertia) == 0:
                inertia = 100 * inertia
            else:
    ...
    def send_report(self):
        ...
        items = OrderedDict()
        items[_tr.m[2577, "Data instances"]] = len(self.data)
        if len(vars) == 1:
            items[_tr.m[2578, "Selected variable"]] = vars[0]
        else:
            items[_tr.m[2579, "Selected variables"]] = _tr.m[2580, "{} and {}"].format(
