...
class OWConcatenate(widget.OWWidget):
    name = _tr.m[279, "Concatenate"]
    description = _tr.m[280, "Concatenate (append) two or more datasets."]
    category = _tr.m[281, "Transform"]
    priority = 1111
    icon = "icons/Concatenate.svg"
    keywords = _tr.m[282, "concatenate, append, join, extend"]

    class Inputs:
        primary_data = Input(_tr.m[283, "Primary Data"], Orange.data.Table)
        additional_data = MultiInput(
            _tr.m[284, "Additional Data"], Orange.data.Table, default=True
        )

    class Outputs:
        data = Output(_tr.m[285, "Data"], Orange.data.Table)

    class Error(widget.OWWidget.Error):
        bow_concatenation = Msg(_tr.m[286, "Inputs must be of the same type."])
        incompatible_domains = \
            Msg(_tr.m[287, "Ignoring column names requires matching column types"])

    class Warning(widget.OWWidget.Warning):
        renamed_variables = Msg(
            _tr.m[288, "Variables with duplicated names have been renamed."])
        unmergeable_attributes = Msg(
            (_tr.m[289, "Some variables may not be concatenated correctly due "] + _tr.m[290, "to attributes difference ({})."]))
    ...
    source_attr_name = settings.Setting(_tr.m[291, "Source ID"])
    ...
    domain_opts = (_tr.m[292, "all variables that appear in input tables"],
                   _tr.m[293, "only variables that appear in all tables"])

    id_roles = (_tr.m[294, "Class attribute"], _tr.m[295, "Attribute"], _tr.m[296, "Meta attribute"])
    ...
    def __init__(self):
        ...
        self.mergebox = gui.vBox(self.controlArea, _tr.m[297, "Variable Sets Merging"])
        gui.widgetLabel(
            self.mergebox, self.tr(_tr.m[298, "When there is no primary table, "] +
                                   _tr.m[299, "the output should contain"]))
        ...
        label = gui.widgetLabel(
            self.mergebox,
            self.tr(_tr.m[300, "The resulting table will have a class only if there "] +
                    _tr.m[301, "is no conflict between input classes."]))
        label.setWordWrap(True)

        box = gui.vBox(self.controlArea, _tr.m[302, "Variable matching"])
        gui.checkBox(
            box, self, "ignore_names",
            (_tr.m[303, "Use column names from the primary table,\n"] + _tr.m[304, "and ignore names in other tables."]),
            callback=self.ignore_names_changed, stateWhenDisabled=False)

        gui.separator(self.controlArea)
        gui.checkBox(
            box, self, "ignore_compute_value",
            (_tr.m[305, "Treat variables with the same name as the same variable,\n"] + _tr.m[306, "even if they are computed using different formulae."]),
            callback=self.commit.deferred, stateWhenDisabled=False)
        ###
        box = gui.vBox(
            self.controlArea, self.tr(_tr.m[307, "Source Identification"]),
        )

        cb = gui.checkBox(
            box, self, "append_source_column",
            self.tr(_tr.m[308, "Append data source IDs"]),
        ...
        form.addRow(
            self.tr(_tr.m[309, "Feature name:"]),
            gui.lineEdit(ibox, self, "source_attr_name", valueType=str,
                         callback=self._source_changed))

        form.addRow(
            self.tr(_tr.m[310, "Place:"]),
    ...
    def send_report(self):
        items = OrderedDict()
        if self.primary_data is not None:
            items[_tr.m[311, "Domain"]] = _tr.m[312, "from primary data"]
        else:
            items[_tr.m[313, "Domain"]] = self.tr(self.domain_opts[self.merge_type]).lower()
        if self.append_source_column:
            items[_tr.m[314, "Source data ID"]] = _tr.m[315, "{} (as {})"].format(
