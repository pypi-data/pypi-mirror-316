...
class LearnerItemModel(QStandardItemModel):
    ...
    def _add_data(self):
        for cls, opt_name, lib in self.LEARNERS:
            ...
            if not imported:
                item.setToolTip(_tr.e(_tr.c(2158, f"{lib} is not installed")))
            self.appendRow(item)
...
class BaseEditor(QWidget, gui.OWComponent):
    ...
    def _add_main_layout(self):
        ...
        self.basic_box = gui.vBox(self._layout, _tr.m[2159, "Basic Properties"])
        gui.spin(
            self.basic_box, self, "n_estimators", 1, 10000,
            label=_tr.m[2160, "Number of trees:"], **common_args
        )
        gui.doubleSpin(
            self.basic_box, self, "learning_rate", 0, 1, 0.001,
            label=_tr.m[2161, "Learning rate: "], **common_args
        )
        gui.checkBox(
            self.basic_box, self, "random_state", label=_tr.m[2162, "Replicable training"],
            callback=self.settings_changed, attribute=Qt.WA_LayoutUsesWidgetRect
        )

        self.growth_box = gui.vBox(self._layout, _tr.m[2163, "Growth Control"])
        gui.spin(
            self.growth_box, self, "max_depth", 1, 50,
            label=_tr.m[2164, "Limit depth of individual trees: "], **common_args
        )

        self.sub_box = gui.vBox(self._layout, _tr.m[2165, "Subsampling"])
    ...
    def get_learner_parameters(self) -> Tuple:
        return (
            (_tr.m[2166, "Method"], self.learner_class.name),
            (_tr.m[2167, "Number of trees"], self.n_estimators),
            (_tr.m[2168, "Learning rate"], self.learning_rate),
            (_tr.m[2169, "Replicable training"], _tr.m[2170, "Yes"] if self.random_state else _tr.m[2171, "No"]),
            (_tr.m[2172, "Maximum tree depth"], self.max_depth),
        )
...
class RegEditor(BaseEditor):
    ...
    def _add_main_layout(self):
        ...
        gui.widgetLabel(box, _tr.m[2173, "Regularization:"])
        gui.hSlider(
    ...
    def get_learner_parameters(self) -> Tuple:
        return super().get_learner_parameters() + (
            (_tr.m[2174, "Regularization strength"], self.lambda_),
        )
...
class GBLearnerEditor(BaseEditor):
    ...
    def _add_main_layout(self):
        ...
        gui.doubleSpin(
            ...
            label=_tr.m[2175, "Fraction of training instances: "],
            callback=self.settings_changed
        )
        # Growth control
        gui.spin(
            self.growth_box, self, "min_samples_split", 2, 1000,
            controlWidth=80, label=_tr.m[2176, "Do not split subsets smaller than: "],
    ...
    def get_learner_parameters(self) -> Tuple:
        return super().get_learner_parameters() + (
            (_tr.m[2177, "Fraction of training instances"], self.subsample),
            (_tr.m[2178, "Stop splitting nodes with maximum instances"],
...
class CatGBLearnerEditor(RegEditor):
    ...
    def _add_main_layout(self):
        ...
        gui.doubleSpin(
            ...
            controlWidth=80, alignment=Qt.AlignRight,
            label=_tr.m[2179, "Fraction of features for each tree: "],
            callback=self.settings_changed
        )
    ...
    def get_learner_parameters(self) -> Tuple:
        return super().get_learner_parameters() + (
            (_tr.m[2180, "Fraction of features for each tree"], self.colsample_bylevel),
        )
...
class XGBBaseEditor(RegEditor):
    ...
    def _add_main_layout(self):
        ...
        gui.doubleSpin(
            self.sub_box, self, "subsample", 0.05, 1, 0.05,
            label=_tr.m[2181, "Fraction of training instances: "], **common_args
        )
        gui.doubleSpin(
            self.sub_box, self, "colsample_bytree", 0.05, 1, 0.05,
            label=_tr.m[2182, "Fraction of features for each tree: "], **common_args
        )
        gui.doubleSpin(
            self.sub_box, self, "colsample_bylevel", 0.05, 1, 0.05,
            label=_tr.m[2183, "Fraction of features for each level: "], **common_args
        )
        gui.doubleSpin(
            self.sub_box, self, "colsample_bynode", 0.05, 1, 0.05,
            label=_tr.m[2184, "Fraction of features for each split: "], **common_args
        )
    ...
    def get_learner_parameters(self) -> Tuple:
        return super().get_learner_parameters() + (
            (_tr.m[2185, "Fraction of training instances"], self.subsample),
            (_tr.m[2186, "Fraction of features for each tree"], self.colsample_bytree),
            (_tr.m[2187, "Fraction of features for each level"], self.colsample_bylevel),
            (_tr.m[2188, "Fraction of features for each split"], self.colsample_bynode),
        )
...
class OWGradientBoosting(OWBaseLearner):
    ...
    description = _tr.m[2189, "Predict using gradient boosting on decision trees."]
    icon = "icons/GradientBoosting.svg"
    priority = 45
    keywords = _tr.m[2190, "gradient boosting, catboost, gradient, boost, tree, forest, xgb, gb, extreme"]
    ...
    def add_main_layout(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[2191, "Method"])
        gui.comboBox(
