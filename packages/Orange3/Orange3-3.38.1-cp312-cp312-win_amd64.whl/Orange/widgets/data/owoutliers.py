...
def run(data: Table, learner: Learner, state: TaskState) -> Results:
    ...
    callback(0, _tr.m[1075, "Initializing..."])
...
class SVMEditor(ParametersEditor):
    ...
    def __init__(self, parent):
        ...
        tooltip = (_tr.m[1076, "An upper bound on the fraction of training errors and a "] + _tr.m[1077, "lower bound of the fraction of support vectors"])
        gui.widgetLabel(self.param_box, _tr.m[1078, "Nu:"], tooltip=tooltip)
        ...
        gui.doubleSpin(self.param_box, self, "gamma",
                       label=_tr.m[1079, "Kernel coefficient:"], step=1e-2, minv=0.01,
    ...
    def get_report_parameters(self):
        return {_tr.m[1080, "Detection method"]: _tr.m[1081, "One class SVM with non-linear kernel (RBF)"],
                 _tr.m[1082, "Regularization (nu)"]: f"{self.nu/100:.0%}",
                 _tr.m[1083, "Kernel coefficient"]: self.gamma}
...
class CovarianceEditor(ParametersEditor):
    ...
    def __init__(self, parent):
        ...
        gui.widgetLabel(self.param_box, _tr.m[1084, "Contamination:"])
        ...
        gui.checkBox(ebox, self, "empirical_covariance",
                     _tr.m[1085, "Support fraction:"], callback=self.parameter_changed)
    ...
    def get_report_parameters(self):
        fraction = self.support_fraction if self.empirical_covariance else None
        return {_tr.m[1086, "Detection method"]: _tr.m[1087, "Covariance estimator"],
                _tr.m[1088, "Contamination"]: f"{self.cont/100:.0%}",
                 _tr.m[1089, "Support fraction"]: fraction}


class LocalOutlierFactorEditor(ParametersEditor):
    METRICS = ("euclidean", "manhattan", "cosine", "jaccard",
               "hamming", "minkowski")
    METRICS_NAMES = [_tr.m[1090, "Euclidean"], _tr.m[1091, "Manhattan"], _tr.m[1092, "Cosine"], _tr.m[1093, "Jaccard"], _tr.m[1094, "Hamming"], _tr.m[1095, "Minkowski"]]
    ...
    def __init__(self, parent):
        ...
        gui.widgetLabel(self.param_box, _tr.m[1096, "Contamination:"])
        gui.hSlider(self.param_box, self, "cont", minValue=1,
                    maxValue=50, ticks=5, labelFormat="%d %%",
                    callback=self.parameter_changed)
        gui.spin(self.param_box, self, "n_neighbors", label=_tr.m[1097, "Neighbors:"],
                 minv=1, maxv=100000, callback=self.parameter_changed)
        gui.comboBox(self.param_box, self, "metric_index", label=_tr.m[1098, "Metric:"],
    ...
    def get_report_parameters(self):
        return {_tr.m[1099, "Detection method"]: _tr.m[1100, "Local Outlier Factor"],
                 _tr.m[1101, "Contamination"]: f"{self.cont/100:.0%}",
                 _tr.m[1102, "Number of neighbors"]: self.n_neighbors,
                 # pylint: disable=invalid-sequence-index
                 _tr.m[1103, "Metric"]: self.METRICS_NAMES[self.metric_index]}
...
class IsolationForestEditor(ParametersEditor):
    ...
    def __init__(self, parent):
        ...
        gui.widgetLabel(self.param_box, _tr.m[1104, "Contamination:"])
        gui.hSlider(self.param_box, self, "cont", minValue=0,
                    maxValue=100, ticks=10, labelFormat="%d %%",
                    callback=self.parameter_changed)
        gui.checkBox(self.param_box, self, "replicable",
                     _tr.m[1105, "Replicable training"], callback=self.parameter_changed)
    ...
    def get_report_parameters(self):
        return {_tr.m[1106, "Detection method"]: _tr.m[1107, "Isolation Forest"],
                _tr.m[1108, "Contamination"]: f"{self.cont/100:.0%}",
                _tr.m[1109, "Replicable training"]: bool_str(self.replicable)}

class OWOutliers(OWWidget, ConcurrentWidgetMixin):
    name = _tr.m[1110, "Outliers"]
    description = _tr.m[1111, "Detect outliers."]
    icon = "icons/Outliers.svg"
    priority = 3000
    category = _tr.m[1112, "Unsupervised"]
    keywords = _tr.m[1113, "outliers, inlier"]

    class Inputs:
        data = Input(_tr.m[1114, "Data"], Table)

    class Outputs:
        inliers = Output(_tr.m[1115, "Inliers"], Table)
        outliers = Output(_tr.m[1116, "Outliers"], Table)
        data = Output(_tr.m[1117, "Data"], Table)
    ...
    class Warning(OWWidget.Warning):
        disabled_cov = Msg(_tr.m[1118, "Too many features for covariance estimation."])

    class Error(OWWidget.Error):
        singular_cov = Msg(_tr.m[1119, "Singular covariance matrix."])
        memory_error = Msg(_tr.m[1120, "Not enough memory"])
    ...
    def init_gui(self):
        box = gui.vBox(self.controlArea, _tr.m[1121, "Method"])
    ...
    def _init_editors(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[1122, "Parameters"])
    ...
    def send_report(self):
        if self.data is not None:
            if self.n_outliers is None or self.n_inliers is None:
                return
            self.report_items(_tr.m[1123, "Data"],
                              ((_tr.m[1124, "Input instances"], len(self.data)),
                               (_tr.m[1125, "Inliers"], self.n_inliers),
                               (_tr.m[1126, "Outliers"], self.n_outliers)))
        self.report_items(_tr.m[1127, "Detection"],
