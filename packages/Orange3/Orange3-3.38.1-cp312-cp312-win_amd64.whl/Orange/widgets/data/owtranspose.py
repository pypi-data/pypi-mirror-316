...
class OWTranspose(OWWidget, ConcurrentWidgetMixin):
    name = _tr.m[1683, "Transpose"]
    description = _tr.m[1684, "Transpose data table."]
    category = _tr.m[1685, "Transform"]
    icon = "icons/Transpose.svg"
    priority = 110
    keywords = _tr.m[1686, "transpose"]

    class Inputs:
        data = Input(_tr.m[1687, "Data"], Table)

    class Outputs:
        data = Output(_tr.m[1688, "Data"], Table, dynamic=False)
    ...
    DEFAULT_PREFIX = _tr.m[1689, "Feature"]
    ...
    class Warning(OWWidget.Warning):
        duplicate_names = Msg((_tr.m[1690, "Values are not unique.\nTo avoid multiple "] + (_tr.m[1691, "features with the same name, values \nof "] + _tr.m[1692, "'{}' have been augmented with indices."])))
        discrete_attrs = Msg(_tr.m[1693, "Categorical features have been encoded as numbers."])
    ...
    def __init__(self):
        ...
        box = gui.radioButtons(
            self.controlArea, self, "feature_type", box=_tr.m[1694, "Feature names"],
            callback=self.commit.deferred)

        button = gui.appendRadioButton(box, _tr.m[1695, "Generic"])
        edit = gui.lineEdit(
            gui.indentedBox(box, gui.checkButtonOffsetHint(button)), self,
            "feature_name",
            placeholderText=_tr.m[1696, "Type a prefix ..."], toolTip=_tr.m[1697, "Custom feature name"])
        edit.editingFinished.connect(self._apply_editing)

        self.meta_button = gui.appendRadioButton(box, _tr.m[1698, "From variable:"])
        ...
        self.remove_check = gui.checkBox(
            gui.indentedBox(box, gui.checkButtonOffsetHint(button)), self,
            "remove_redundant_inst", _tr.m[1699, "Remove redundant instance"],
            callback=self.commit.deferred)
    ...
    def send_report(self):
        if self.feature_type == self.GENERIC:
            ...
            names = _tr.m[1700, "from variable"]
            if self.feature_names_column:
                names += "  '{}'".format(self.feature_names_column.name)
        self.report_items("", [(_tr.m[1701, "Feature names"], names)])
        if self.data:
            self.report_data(_tr.m[1702, "Data"], self.data)
