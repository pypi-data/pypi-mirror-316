...
class OWDistanceMap(widget.OWWidget):
    name = _tr.m[2619, "Distance Map"]
    description = _tr.m[2620, "Visualize a distance matrix."]
    icon = "icons/DistanceMap.svg"
    priority = 1200
    keywords = _tr.m[2621, "distance map"]

    class Inputs:
        distances = Input(_tr.m[2622, "Distances"], Orange.misc.DistMatrix)

    class Outputs:
        selected_data = Output(_tr.m[2623, "Selected Data"], Orange.data.Table, default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Orange.data.Table)
        features = Output(_tr.m[2624, "Features"], widget.AttributeList, dynamic=False)

    class Error(widget.OWWidget.Error):
        empty_matrix = widget.Msg(_tr.m[2625, "Empty distance matrix"])
        not_symmetric = widget.Msg(_tr.m[2626, "Distance matrix is not symmetric."])
    ...
    def __init__(self):
        ...
        self.sorting_cb = gui.comboBox(
            self.controlArea, self, "sorting", box=_tr.m[2627, "Element Sorting"],
            items=[_tr.m[2628, "None"], _tr.m[2629, "Clustering"], _tr.m[2630, "Clustering with ordered leaves"]],
            callback=self._invalidate_ordering)

        box = gui.vBox(self.controlArea, _tr.m[2631, "Colors"])
        ...
        self.annot_combo = gui.comboBox(
            self.controlArea, self, "annotation_idx", box=_tr.m[2632, "Annotations"],
        ...
        self.annot_combo.model()[:] = [_tr.m[2633, "None"], _tr.m[2634, "Enumeration"]]
    ...
    def set_distances(self, matrix):
        ...
        if N > OWDistanceMap._MaxOrderedClustering:
            ...
            if self.sorting == OWDistanceMap.OrderedClustering:
                ...
                msg = (_tr.m[2635, "Cluster ordering was disabled due to the input "] + _tr.m[2636, "matrix being to big"])
        else:
            item.setFlags(item.flags() | Qt.ItemIsEnabled)
        ...
        if N > OWDistanceMap._MaxClustering:
            ...
            msg = (_tr.m[2637, "Clustering was disabled due to the input "] + _tr.m[2638, "matrix being to big"])
        else:
            item.setFlags(item.flags() | Qt.ItemIsEnabled)
    ...
    def set_items(self, items, axis=1):
        ...
        if items is None:
            model[:] = [_tr.m[2639, "None"], _tr.m[2640, "Enumeration"]]
        elif not axis:
            model[:] = [_tr.m[2641, "None"], _tr.m[2642, "Enumeration"], _tr.m[2643, "Attribute names"]]
        elif isinstance(items, Orange.data.Table):
            annot_vars = list(filter_visible(items.domain.variables)) + list(items.domain.metas)
            model[:] = [_tr.m[2644, "None"], _tr.m[2645, "Enumeration"]] + annot_vars
            self.annotation_idx = 0
            self.openContext(items.domain)
        elif isinstance(items, list) and \
                all(isinstance(item, Orange.data.Variable) for item in items):
            model[:] = [_tr.m[2646, "None"], _tr.m[2647, "Enumeration"], _tr.m[2648, "Name"]]
        else:
            model[:] = [_tr.m[2649, "None"], _tr.m[2650, "Enumeration"]]
    ...
    def _update_labels(self, ):
        ...
        elif self.annot_combo.model()[self.annotation_idx] == _tr.m[2651, "Attribute names"]:
    ...
    def send_report(self):
        ...
        if self.annotation_idx <= 1:
            annot = annot.lower()
        self.report_items((
            (_tr.m[2652, "Sorting"], self.sorting_cb.currentText().lower()),
            (_tr.m[2653, "Annotations"], annot)
        ))
        if self.matrix is not None:
            self.report_plot()
