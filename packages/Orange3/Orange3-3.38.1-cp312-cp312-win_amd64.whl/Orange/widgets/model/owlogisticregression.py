...
class OWLogisticRegression(OWBaseLearner):
    name = _tr.m[2240, "Logistic Regression"]
    description = (_tr.m[2241, "The logistic regression classification algorithm with "] + _tr.m[2242, "LASSO (L1) or ridge (L2) regularization."])
    icon = "icons/LogisticRegression.svg"
    replaces = [
        "Orange.widgets.classify.owlogisticregression.OWLogisticRegression",
    ]
    priority = 60
    keywords = _tr.m[2243, "logistic regression"]

    LEARNER = LogisticRegressionLearner

    class Outputs(OWBaseLearner.Outputs):
        coefficients = Output(_tr.m[2244, "Coefficients"], Table, explicit=True)
    ...
    penalty_types = ("Lasso (L1)", "Ridge (L2)", _tr.m[2245, "None"])
    penalty_types_short = ["l1", "l2", None]

    class Warning(OWBaseLearner.Warning):
        class_weights_used = Msg(_tr.m[2246, "Weighting by class may decrease performance."])

    def add_main_layout(self):
        # this is part of init, pylint: disable=attribute-defined-outside-init
        box = gui.widgetBox(self.controlArea, box=True)
        self.penalty_combo = gui.comboBox(
            box, self, "penalty_type", label=_tr.m[2247, "Regularization type: "],
            items=self.penalty_types, orientation=Qt.Horizontal,
            callback=self._penalty_type_changed)
        self.c_box = box = gui.widgetBox(box)
        gui.widgetLabel(box, _tr.m[2248, "Strength:"])
        box2 = gui.hBox(gui.indentedBox(box))
        gui.widgetLabel(box2, _tr.m[2249, "Weak"]).setStyleSheet("margin-top:6px")
        self.c_slider = gui.hSlider(
            ...
            createLabel=False)
        gui.widgetLabel(box2, _tr.m[2250, "Strong"]).setStyleSheet("margin-top:6px")
        box2 = gui.hBox(box)
        ...
        self.weights = gui.checkBox(
            box, self,
            "class_weight", label=_tr.m[2251, "Balance class distribution"],
            callback=self.settings_changed,
            tooltip=_tr.m[2252, "Weigh classes inversely proportional to their frequencies."]
        )
    ...
    def set_c(self):
        ...
        if enable_c:
            ...
            self.c_label.setText(_tr.m[2253, "N/A"])
    ...
    def get_learner_parameters(self):
        return ((_tr.m[2254, "Regularization"], _tr.m[2255, "{}, C={}, class weights: {}"].format(
