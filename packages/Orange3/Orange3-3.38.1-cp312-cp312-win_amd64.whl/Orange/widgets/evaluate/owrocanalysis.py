...
class OWROCAnalysis(widget.OWWidget):
    name = _tr.m[1971, "ROC Analysis"]
    description = (_tr.m[1972, "Display the Receiver Operating Characteristics curve "] + _tr.m[1973, "based on the evaluation of classifiers."])
    icon = "icons/ROCAnalysis.svg"
    priority = 1010
    keywords = _tr.m[1974, "roc analysis, analyse"]

    class Inputs:
        evaluation_results = Input(_tr.m[1975, "Evaluation Results"], Orange.evaluation.Results)

    class Outputs:
        calibrated_model = Output(_tr.m[1976, "Calibrated Model"], Model)

    class Information(widget.OWWidget.Information):
        no_output = Msg(_tr.m[1977, "Can't output a model: {}"])
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[1978, "Plot"])
        self.target_cb = gui.comboBox(
            box, self, "target_index",
            label=_tr.m[1979, "Target"], orientation=Qt.Horizontal,
            callback=self._on_target_changed,
            contentsLength=8, searchable=True)

        gui.widgetLabel(box, _tr.m[1980, "Classifiers"])
        ...
        abox = gui.vBox(self.controlArea, _tr.m[1981, "Curves"])
        gui.comboBox(abox, self, "roc_averaging",
                     items=[_tr.m[1982, "Merge Predictions from Folds"], _tr.m[1983, "Mean TP Rate"],
                            _tr.m[1984, "Mean TP and FP at Threshold"], _tr.m[1985, "Show Individual Curves"]],
                     callback=self._replot)

        gui.checkBox(abox, self, "display_convex_curve",
                     _tr.m[1986, "Show convex ROC curves"], callback=self._replot)
        gui.checkBox(abox, self, "display_convex_hull",
                     _tr.m[1987, "Show ROC convex hull"], callback=self._replot)

        box = gui.vBox(self.controlArea, _tr.m[1988, "Analysis"])

        gui.checkBox(box, self, "display_def_threshold",
                     _tr.m[1989, "Default threshold (0.5) point"],
                     callback=self._on_display_def_threshold_changed)

        gui.checkBox(box, self, "display_perf_line", _tr.m[1990, "Show performance line"],
        ...
        sp = gui.spin(box, self, "fp_cost", 1, 1000, 10,
                      ...
                      callback=self._on_display_perf_line_changed)
        grid.addWidget(QLabel(_tr.m[1991, "FP Cost:"]), 0, 0)
        grid.addWidget(sp, 0, 1)

        sp = gui.spin(box, self, "fn_cost", 1, 1000, 10,
                      alignment=Qt.AlignRight,
                      callback=self._on_display_perf_line_changed)
        grid.addWidget(QLabel(_tr.m[1992, "FN Cost:"]))
        ...
        grid.addWidget(QLabel(_tr.m[1993, "Prior probability:"]))
        ...
        axis.setLabel(_tr.m[1994, "FP Rate (1-Specificity)"])
        axis.setGrid(16)

        axis = self.plot.getAxis("left")
        axis.setTickFont(tickfont)
        axis.setLabel(_tr.m[1995, "TP Rate (Sensitivity)"])
        axis.setGrid(16)
    ...
    def _setup_plot(self):
        ...
        if not all(c.is_valid for c in hull_curves):
            if any(c.is_valid for c in hull_curves):
                warning = _tr.m[1996, "Some ROC curves are undefined"]
            else:
                warning = _tr.m[1997, "All ROC curves are undefined"]
    ...
    def send_report(self):
        if self.results is None:
            return
        items = OrderedDict()
        items[_tr.m[1999, "Target class"]] = self.target_cb.currentText()
        if self.display_perf_line:
            items[_tr.m[2000, "Costs"]] = \
                _tr.m[2001, "FP = {}, FN = {}"].format(self.fp_cost, self.fn_cost)
            items[_tr.m[2002, "Target probability"]] = "{} %".format(self.target_prior)
