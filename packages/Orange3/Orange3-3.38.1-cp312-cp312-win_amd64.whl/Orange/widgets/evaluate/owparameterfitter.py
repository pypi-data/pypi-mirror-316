...
def _search(
    ...
    progress_callback(0, _tr.m[1859, "Calculating..."])
    scores = []
...
class ParameterSetter(CommonParameterSetter):
    GRID_LABEL, SHOW_GRID_LABEL = _tr.m[1860, "Gridlines"], _tr.m[1861, "Show"]
...
class FitterPlot(PlotWidget):
    ...
    def set_data(
        ...
        self.legend.addItem(scatter_item_tr, _tr.m[1862, "Train"])
        self.legend.addItem(scatter_item_cv, _tr.m[1863, "CV"])
    ...
    def help_event(self, ev: QGraphicsSceneHelpEvent) -> bool:
        ...
        if index is not None:
            ...
            text = ("<table align=left>" + ("<tr>" + (_tr.m[1864, "<td><b>Train:</b></td>"] + (f"<td>{round(scores[0], 3)}</td>" + ("</tr><tr>" + (_tr.m[1865, "<td><b>CV:</b></td>"] + (f"<td>{round(scores[1], 3)}</td>" + ("</tr>" + "</table>"))))))))
        if text:
...
class RangePreview(QWidget):
    ...
    def paintEvent(self, _):
        ...
        elided_text = metrics.elidedText(
            _tr.m[1866, "Steps: "] + ", ".join(map(str, self.__steps[:-1])),
...
class OWParameterFitter(OWWidget, ConcurrentWidgetMixin):
    name = _tr.m[1867, "Parameter Fitter"]
    description = _tr.m[1868, "Fit learner for various values of fitting parameter."]
    ...
    class Inputs:
        data = Input(_tr.m[1869, "Data"], Table)
        learner = Input(_tr.m[1870, "Learner"], Learner)
    ...
    class Error(OWWidget.Error):
        unknown_err = Msg("{}")
        not_enough_data = Msg(_tr.e(_tr.c(1871, f"At least {N_FOLD} instances are needed.")))
        incompatible_learner = Msg("{}")
        manual_steps_error = Msg(_tr.m[1872, "Invalid values for '{}': {}"])
        min_max_error = Msg(_tr.m[1873, "Minimum must be less than maximum."])
        missing_target = Msg(_tr.m[1874, "Data has no target."])

    class Warning(OWWidget.Warning):
        no_parameters = Msg(_tr.m[1875, "{} has no parameters to fit."])
    ...
    def _add_controls(self):
        ...
        # pylint: disable=attribute-defined-outside-init
        layout = QGridLayout()
        gui.widgetBox(self.controlArea, _tr.m[1876, "Settings"], orientation=layout)
        ...
        button = gui.appendRadioButton(buttons, _tr.m[1877, "Range:"])
        ...
        box = gui.hBox(None)
        self.__spin_min = gui.spin(box, self, "minimum", label=_tr.m[1878, "From:"], **kw)
        layout.addWidget(box, 1, 1)

        box = gui.hBox(None)
        self.__spin_max = gui.spin(box, self, "maximum", label=_tr.m[1879, "To:"], **kw)
        layout.addWidget(box, 2, 1)

        self.range_preview = RangePreview()
        layout.addWidget(self.range_preview, 3, 0, 1, 2)

        gui.appendRadioButton(buttons, _tr.m[1880, "Manual:"])
        layout.addWidget(buttons, 4, 0)
        self.edit = gui.lineEdit(None, self, "manual_steps",
                                 placeholderText=_tr.m[1881, "e.g. 10, 20, ..., 50"],
    ...
    def _set_range_controls(self, param: Learner.FittedParameter):
        ...
        tip = _tr.m[1882, "Enter a list of values"]
        if param.min is not None:
            if param.max is not None:
                self.edit.setToolTip(_tr.e(_tr.c(1883, f"{tip} between {param.min} and {param.max}.")))
            else:
                self.edit.setToolTip(_tr.e(_tr.c(1884, f"{tip} greater or equal to {param.min}.")))
        elif param.max is not None:
            self.edit.setToolTip(_tr.e(_tr.c(1885, f"{tip} smaller or equal to {param.max}.")))
        else:
    ...
    def send_report(self):
        ...
        self.report_items(_tr.m[1886, "Settings"],
                          [(_tr.m[1887, "Parameter"], parameter),
                           (_tr.m[1888, "Range"], ", ".join(map(str, self.steps)))])
        self.report_name(_tr.m[1889, "Plot"])
        self.report_plot()
