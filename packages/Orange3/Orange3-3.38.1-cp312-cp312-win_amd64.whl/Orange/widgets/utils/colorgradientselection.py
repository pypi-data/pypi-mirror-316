...
class ColorGradientSelection(QWidget):
    ...
    def __init__(self, *args, thresholds=(0.0, 1.0), center=None, **kwargs):
        ...
        slider = self.slider = IntervalSlider(
            ...
            toolTip=self.tr(_tr.m[3113, "Low gradient threshold"]),
            whatsThis=self.tr((_tr.m[3114, "Applying a low threshold will squeeze the "] + _tr.m[3115, "gradient from the lower end"]))
        )
        form.addRow(self.tr(_tr.m[3116, "Range:"]), slider)
    ...
    def __update_center_visibility(self):
        ...
        if visible:
            ...
            self.layout().insertRow(1, _tr.m[3117, "Center at:"], self.center_edit)
