...
class OWSaveBase(widget.OWWidget, openclass=True):
    ...
    class Information(widget.OWWidget.Information):
        empty_input = widget.Msg(_tr.m[3343, "Empty input; nothing was saved."])

    class Warning(widget.OWWidget.Warning):
        auto_save_disabled = widget.Msg(
            (_tr.m[3344, "Auto save disabled.\n"] + (_tr.m[3345, "Due to security reasons auto save is only restored for paths "] + (_tr.m[3346, "that are in the same directory as the workflow file or in a "] + _tr.m[3347, "subtree of that directory."])))
        )

    class Error(widget.OWWidget.Error):
        no_file_name = widget.Msg(_tr.m[3348, "File name is not set."])
        unsupported_format = widget.Msg(_tr.m[3349, "File format is unsupported.\n{}"])
    ...
    def __init__(self, start_row=0):
        ...
        grid.addWidget(
            gui.checkBox(
                None, self, "auto_save", _tr.m[3350, "Autosave when receiving new data"],
                callback=self._on_auto_save_toggled),
            start_row, 0, 1, 2)
        self.bt_save = gui.button(
            self.buttonsArea, self,
            label=_tr.e(_tr.c(3351, f"Save as {self.stored_name}")) if self.stored_name else _tr.m[3352, "Save"],
            callback=self.save_file)
        gui.button(self.buttonsArea, self, _tr.m[3353, "Save as ..."], callback=self.save_file_as)
    ...
    def save_file_as(self):
        ...
        self.bt_save.setText(_tr.e(_tr.c(3354, f"Save as {self.stored_name}")))
        self.update_messages()
        self._try_save()
    ...
    if sys.platform in ("darwin", "win32"):
        ...
        def get_save_filename(self):  # pragma: no cover
            ...
            while True:
                dlg = QFileDialog(
                    None, _tr.m[3355, "Save File"], filename, ";;".join(no_ext_filters))
        ...
        def get_save_filename(self):
            dlg = self.SaveFileDialog(
                type(self),
                None, _tr.m[3356, "Save File"], self.initial_start_dir(),
