...
class OWScatterPlot(OWDataProjectionWidget, VizRankMixin(ScatterPlotVizRank)):
    ...
    name = _tr.m[3762, "Scatter Plot"]
    description = (_tr.m[3763, "Interactive scatter plot visualization with "] + _tr.m[3764, "intelligent data visualization enhancements."])
    icon = "icons/ScatterPlot.svg"
    priority = 140
    keywords = _tr.m[3765, "scatter plot"]

    class Inputs(OWDataProjectionWidget.Inputs):
        features = Input(_tr.m[3766, "Features"], AttributeList)

    class Outputs(OWDataProjectionWidget.Outputs):
        features = Output(_tr.m[3767, "Features"], AttributeList, dynamic=False)
    ...
    class Warning(OWDataProjectionWidget.Warning):
        missing_coords = Msg(
            (_tr.m[3768, "Plot cannot be displayed because '{}' or '{}' "] + _tr.m[3769, "is missing for all data points."]))

    class Information(OWDataProjectionWidget.Information):
        sampled_sql = Msg(_tr.m[3770, "Large SQL table; showing a sample."])
        missing_coords = Msg(
            _tr.m[3771, "Points with missing '{}' or '{}' are not displayed"])
    ...
    def _add_controls(self):
        ...
        gui.checkBox(
            ...
            label=_tr.m[3772, "Treat variables as independent"],
            callback=self.graph.update_regression_line,
            tooltip=
            (_tr.m[3773, "If checked, fit line to group (minimize distance from points);\n"] + _tr.m[3774, "otherwise fit y as a function of x (minimize vertical distances)"]),
            disabledBy=self.cb_reg_line)
        gui.checkBox(
            self._plot_box, self,
            value="graph.show_ellipse",
            label=_tr.m[3775, "Show confidence ellipse"],
            tooltip=_tr.m[3776, "Hotelling's T² confidence ellipse (α=95%)"],
    ...
    def _add_controls_axis(self):
        ...
        self.attr_box = gui.vBox(self.controlArea, _tr.m[3777, 'Axes'],
        ...
        self.cb_attr_x = gui.comboBox(
            hbox, self, "attr_x", label=_tr.m[3778, "Axis x:"],
        ...
        self.cb_attr_y = gui.comboBox(
            hbox, self, "attr_y", label=_tr.m[3779, "Axis y:"],
            callback=self.set_attr_from_combo,
            model=self.xy_model, **common_options,
        )
        ...
        button = self.vizrank_button(_tr.m[3780, "Find Informative Projections"])
    ...
    def _add_controls_sampling(self):
        self.sampling = gui.auto_commit(
            self.controlArea, self, "auto_sample", _tr.m[3781, "Sample"], box=_tr.m[3782, "Sampling"],
    ...
    def init_vizrank(self):
        ...
        if self.data is None:
            err_msg = _tr.m[3783, "No data on input"]
        elif self.data.is_sparse():
            err_msg = _tr.m[3784, "Data is sparse"]
        elif len(self.xy_model) < 3:
            err_msg = _tr.m[3785, "Not enough features for ranking"]
        elif self.attr_color is None:
            err_msg = _tr.m[3786, "Color variable is not selected"]
        elif np.isnan(self.data.get_column(self.attr_color)).all():
            err_msg = _tr.e(_tr.c(3787, "Color variable has no values"))
        if not err_msg:
    ...
    def set_subset_data(self, subset: Optional[Table]):
        ...
        if isinstance(subset, SqlTable):
            if subset.approx_len() < AUTO_DL_LIMIT:
                ...
                self.warning(_tr.m[3788, "Data subset does not support large Sql tables"])
                subset = None
    ...
    def get_widget_name_extension(self):
        if self.data is not None:
            return _tr.m[3789, "{} vs {}"].format(self.attr_x.name, self.attr_y.name)
        return None

    def _get_send_report_caption(self):
        return report.render_items_vert((
            (_tr.m[3790, "Color"], self._get_caption_var_name(self.attr_color)),
            (_tr.m[3791, "Label"], self._get_caption_var_name(self.attr_label)),
            (_tr.m[3792, "Shape"], self._get_caption_var_name(self.attr_shape)),
            (_tr.m[3793, "Size"], self._get_caption_var_name(self.attr_size)),
            (_tr.m[3794, "Jittering"], (self.attr_x.is_discrete or
