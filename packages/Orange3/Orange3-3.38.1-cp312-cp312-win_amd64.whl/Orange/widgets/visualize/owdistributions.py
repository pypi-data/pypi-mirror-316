...
class OWDistributions(OWWidget):
    name = _tr.m[3410, "Distributions"]
    description = _tr.m[3411, "Display value distributions of a data feature in a graph."]
    icon = "icons/Distribution.svg"
    priority = 120
    keywords = _tr.m[3412, "distributions, histogram"]

    class Inputs:
        data = Input(_tr.m[3413, "Data"], Table, doc=_tr.m[3414, "Set the input dataset"])

    class Outputs:
        selected_data = Output(_tr.m[3415, "Selected Data"], Table, default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table)
        histogram_data = Output(_tr.m[3416, "Histogram Data"], Table)

    class Error(OWWidget.Error):
        no_defined_values_var = \
            Msg(_tr.m[3417, "Variable '{}' does not have any defined values"])
        no_defined_values_pair = \
            Msg(_tr.m[3418, "No data instances with '{}' and '{}' defined"])

    class Warning(OWWidget.Warning):
        ignored_nans = Msg(_tr.m[3419, "Data instances with missing values are ignored"])
    ...
    Fitters = (
        (_tr.m[3420, "None"], None, (), ()),
        (_tr.m[3421, "Normal"], norm, ("loc", "scale"), ("μ", "σ")),
        ("Beta", beta, ("a", "b", "loc", "scale"),
         ("α", "β", "-loc", "-scale")),
        (_tr.m[3422, "Gamma"], gamma, ("a", "loc", "scale"), ("α", "β", "-loc", "-scale")),
        ("Rayleigh", rayleigh, ("loc", "scale"), ("-loc", "σ")),
        ("Pareto", pareto, ("b", "loc", "scale"), ("α", "-loc", "-scale")),
        (_tr.m[3423, "Exponential"], expon, ("loc", "scale"), ("-loc", "λ")),
        (_tr.m[3424, "Kernel density"], AshCurve, ("a",), ("",))
    )
    ...
    def __init__(self):
        ...
        varview = gui.listView(
            self.controlArea, self, "var", box=_tr.m[3425, "Variable"],
        ...
        gui.checkBox(
            varview.box, self, "sort_by_freq", _tr.m[3426, "Sort categories by frequency"],
            callback=self._on_sort_by_freq, stateWhenDisabled=False)

        box = self.continuous_box = gui.vBox(self.controlArea, _tr.m[3427, "Distribution"])
        gui.comboBox(
            box, self, "fitted_distribution", label=_tr.m[3428, "Fitted distribution"],
        ...
        slider = gui.hSlider(
            ...
            label=_tr.m[3429, "Bin width"], orientation=Qt.Horizontal,
        ...
        self.smoothing_box = gui.hSlider(
            ...
            label=_tr.m[3430, "Smoothing"], orientation=Qt.Horizontal,
            minValue=2, maxValue=20, callback=self.replot, disabled=True)
        gui.checkBox(
            box, self, "hide_bars", _tr.m[3431, "Hide bars"], stateWhenDisabled=False,
            callback=self._on_hide_bars_changed,
            disabled=not self.fitted_distribution)

        box = gui.vBox(self.controlArea, _tr.m[3432, "Columns"])
        gui.comboBox(
            box, self, "cvar", label=_tr.m[3433, "Split by"], orientation=Qt.Horizontal,
            searchable=True,
            model=DomainModel(placeholder=_tr.m[3434, "(None)"],
                              valid_types=(DiscreteVariable), ),
            callback=self._on_cvar_changed, contentsLength=18)
        gui.checkBox(
            box, self, "stacked_columns", _tr.m[3435, "Stack columns"],
            callback=self.replot)
        gui.checkBox(
            box, self, "show_probs", _tr.m[3436, "Show probabilities"],
            callback=self._on_show_probabilities_changed)
        gui.checkBox(
            box, self, "cumulative_distr", _tr.m[3437, "Show cumulative distribution"],
    ...
    def _on_show_probabilities_changed(self):
        ...
        if self.show_probs:
            label.setText(_tr.m[3438, "Fitted probability"])
            label.setToolTip(
                _tr.m[3439, "Chosen distribution is used to compute Bayesian probabilities"])
        else:
            label.setText(_tr.m[3440, "Fitted distribution"])
            label.setToolTip("")
        self.replot()
    ...
    def _set_axis_names(self):
        ...
        if self.show_probs and self.cvar:
            leftaxis.setLabel(
                _tr.e(_tr.c(3441, f"Probability of '{self.cvar.name}' at given '{self.var.name}'")))
        else:
            leftaxis.setLabel(_tr.m[3442, "Frequency"])
    ...
    def _disc_plot(self):
        ...
        for i, freq, desc in zip(count(), dist[order], self.ordered_values):
            tooltip = \
                ("<p style='white-space:pre;'>" + (f"<b>{escape(desc)}</b>: {int(freq)} " + f"({100 * freq / len(self.valid_data):.2f} %) "))
            self._add_bar(
    ...
    def _cont_plot(self):
        ...
        for i, (x0, x1), freq in zip(count(), zip(x, x[1:]), y):
            ...
            tooltip = \
                ("<p style='white-space:pre;'>" + (f"<b>{escape(desc)}</b>: " + f"{freq} ({100 * freq / total:.2f} %)</p>"))
    ...
    def _split_tooltip(valname, tot_group, total, gvalues, freqs):
        ...
        snp = f"style='{cs}'"
        return (f"<table style='border-collapse: collapse'>" + (f"<tr><th {s}>{escape(valname)}:</th>" + (f"<td {snp}><b>{int(tot_group)}</b></td>" + ("<td/>" + f"<td {s}><b>{100 * tot_group / total:.2f} %</b></td></tr>")))) + \
               (f"<tr><td/><td/><td {s}>(in group)</td><td {s}>(overall)</td>" + "</tr>") + \
               "".join(
                   ("<tr>" + (f"<th {s}>{value}:</th>" + (f"<td {snp}><b>{int(freq)}</b></td>" + (f"<td {s}>{100 * freq / div_group:.2f} %</td>" + (f"<td {s}>{100 * freq / total:.2f} %</td>" + "</tr>")))))
    ...
    def show_selection(self):
        ...
        for group in self.grouped_selection():
            ...
            if self.var.is_continuous:
                ...
                item.setToolTip(
                    ("<p style='white-space:pre;'>" + (f"<b>{escape(valname)}</b>: " + f"{inside} ({100 * inside / total:.2f} %)")))
        ...
        if self.is_valid:
            ...
            if self.var.is_continuous:  # annotate with bins
                ...
                annotated_data = create_groups_table(
                    annotated_data, hist_indices, var_name=_tr.m[3443, "Bin"], values=hist_values)
    ...
    def _get_histogram_table(self):
        ...
        var_bin = DiscreteVariable(_tr.m[3444, "Bin"], [bar.desc for bar in self.bar_items])
        var_freq = ContinuousVariable(_tr.m[3445, "Count"])
        X = []
        if self.cvar:
    ...
    def send_report(self):
        ...
        self.report_plot()
        if self.cumulative_distr:
            text = _tr.e(_tr.c(3446, f"Cummulative distribution of '{self.var.name}'"))
        else:
            text = _tr.e(_tr.c(3447, f"Distribution of '{self.var.name}'"))
        if self.cvar:
            text += _tr.e(_tr.c(3448, f" with columns split by '{self.cvar.name}'"))
