...
class ParameterSetter(CommonParameterSetter):
    MEAN_LABEL = _tr.m[3585, "Mean"]
    LINE_LABEL = _tr.m[3586, "Lines"]
    MISSING_LINE_LABEL = _tr.m[3587, "Lines (missing value)"]
    SEL_LINE_LABEL = _tr.m[3588, "Selected lines"]
    SEL_MISSING_LINE_LABEL = _tr.m[3589, "Selected lines (missing value)"]
    RANGE_LABEL = _tr.m[3590, "Range"]
    SEL_RANGE_LABEL = _tr.m[3591, "Selected range"]
    ...
    def update_setters(self):
        ...
        self.missing_line_settings = {
            Updater.WIDTH_LABEL: LinePlotStyle.UNSELECTED_LINE_WIDTH,
            Updater.ALPHA_LABEL: LinePlotStyle.UNSELECTED_LINE_ALPHA,
            Updater.STYLE_LABEL: _tr.m[3592, "Dash line"],
            Updater.ANTIALIAS_LABEL: True,
        }
        ...
        self.sel_missing_line_settings = {
            Updater.WIDTH_LABEL: LinePlotStyle.SELECTED_LINE_WIDTH,
            Updater.ALPHA_LABEL: LinePlotStyle.SELECTED_LINE_ALPHA,
            Updater.STYLE_LABEL: _tr.m[3593, "Dash line"],
            Updater.ANTIALIAS_LABEL: False,
        }
        self.range_settings = {
...
class OWLinePlot(OWWidget):
    name = _tr.m[3596, "Line Plot"]
    description = _tr.m[3597, "Visualization of data profiles (e.g., time series)."]
    icon = "icons/LinePlot.svg"
    priority = 180
    keywords = _tr.m[3598, "line plot"]

    buttons_area_orientation = Qt.Vertical
    enable_selection = Signal(bool)

    class Inputs:
        data = Input(_tr.m[3599, "Data"], Table, default=True)
        data_subset = Input(_tr.m[3600, "Data Subset"], Table)

    class Outputs:
        selected_data = Output(_tr.m[3601, "Selected Data"], Table, default=True)
    ...
    class Error(OWWidget.Error):
        not_enough_attrs = Msg(_tr.m[3602, "Need at least one numeric feature."])

    class Warning(OWWidget.Warning):
        no_display_option = Msg(_tr.m[3603, "No display option is selected."])

    class Information(OWWidget.Information):
        too_many_features = Msg((_tr.m[3604, "Data has too many features. Only first {}"] + _tr.m[3605, " are shown."]).format(MAX_FEATURES))
    ...
    def _add_controls(self):
        displaybox = gui.widgetBox(self.controlArea, _tr.m[3606, "Display"])
        gui.checkBox(displaybox, self, "show_profiles", _tr.m[3607, "Lines"],
                     callback=self.__show_profiles_changed,
                     tooltip=_tr.m[3608, "Plot lines"])
        gui.checkBox(displaybox, self, "show_range", _tr.m[3609, "Range"],
                     callback=self.__show_range_changed,
                     tooltip=_tr.m[3610, "Plot range between 10th and 90th percentile"])
        gui.checkBox(displaybox, self, "show_mean", _tr.m[3611, "Mean"],
                     callback=self.__show_mean_changed,
                     tooltip=_tr.m[3612, "Plot mean curve"])
        gui.checkBox(displaybox, self, "show_error", _tr.m[3613, "Error bars"],
                     callback=self.__show_error_changed,
                     tooltip=_tr.m[3614, "Show standard deviation"])

        self.group_vars = DomainModel(
            placeholder=_tr.m[3615, "None"], separators=False, valid_types=DiscreteVariable)
        self.group_view = gui.listView(
            self.controlArea, self, "group_var", box=_tr.m[3616, "Group by"],
    ...
    def send_report(self):
        ...
        caption = report.render_items_vert(((_tr.m[3617, "Group by"], self.group_var),))
        self.report_plot()
        if caption:
