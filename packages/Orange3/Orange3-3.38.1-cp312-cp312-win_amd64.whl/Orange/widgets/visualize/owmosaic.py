...
class MosaicVizRank(VizRankDialogAttrs):
    ...
    def __init__(self, parent, data, attr_color, attr_range_index):
        ...
        box = gui.hBox(self)
        label = QLabel(_tr.m[3618, "Score Mosaics with "])

        combo = self.attrs_combo = QComboBox()
        combo.addItems(
            [_tr.m[3619, "a single variable"], _tr.m[3620, "two variables"], _tr.m[3621, "three variables"],
             _tr.m[3622, "four variables"], _tr.m[3623, "at most two variables"],
             _tr.m[3624, "at most three variables"], _tr.m[3625, "at most four variables"]])
    ...
    def on_attrs_changed(self):
        ...
        if new_attrs == self.attr_range():
            ...
            self.set_button_state(
                label=_tr.m[3626, "Restart with new settings"],
                enabled=True)
...
class OWMosaicDisplay(OWWidget, VizRankMixin(MosaicVizRank)):
    name = _tr.m[3627, "Mosaic Display"]
    description = _tr.m[3628, "Display data in a mosaic plot."]
    icon = "icons/MosaicDisplay.svg"
    priority = 220
    keywords = _tr.m[3629, "mosaic display"]

    class Inputs:
        data = Input(_tr.m[3630, "Data"], Table, default=True)
        data_subset = Input(_tr.m[3631, "Data Subset"], Table)

    class Outputs:
        selected_data = Output(_tr.m[3632, "Selected Data"], Table, default=True)
    ...
    class Warning(OWWidget.Warning):
        incompatible_subset = Msg(_tr.m[3633, "Data subset is incompatible with Data"])
        no_valid_data = Msg(_tr.m[3634, "No valid data"])
        no_cont_selection_sql = \
            Msg(_tr.m[3635, "Selection of numeric features on SQL is not supported"])
    ...
    def __init__(self):
        ...
        self.attr_combos = [
            ...
            for i in range(1, 5)]
        box.layout().addWidget(self.vizrank_button(_tr.m[3637, "Find Informative Mosaics"]))
        self.vizrankSelectionChanged.connect(self.set_attr)
        self.vizrankRunStateChanged.connect(self.store_vizrank_attr_range)

        box2 = gui.vBox(self.controlArea, box=_tr.m[3638, "Interior Coloring"])
        self.color_model = DomainModel(
            order=DomainModel.MIXED, valid_types=DomainModel.PRIMITIVE,
            placeholder=_tr.m[3639, "(Pearson residuals)"])
        ...
        self.bar_button = gui.checkBox(
            box2, self, 'use_boxes', label=_tr.m[3640, 'Compare with total'],
    ...
    def init_vizrank(self):
        if self.discrete_data is not None and len(self.discrete_data) > 1 \
            ...
            self.disable_vizrank(_tr.m[3641, "Not enough data"])
    ...
    def update_graph(self):
        ...
        def add_rect(x0, x1, y0, y1, condition,
            ...
            if self.variable_color is None:
                ...
                outer_rect.setToolTip(
                    condition + "<hr/>" +
                    (_tr.m[3642, "Expected instances: %.1f<br>"] + (_tr.m[3643, "Actual instances: %d<br>"] + _tr.m[3644, "Standardized (Pearson) residual: %.1f"])) %
                ...
                if n_actual > 0:
                    ...
                    text = "<br/>".join(
                        _tr.m[3645, "<b>%s</b>: %d / %.1f%% (Expected %.1f / %.1f%%)"] %
                    ...
                    text = ""
                outer_rect.setToolTip(
                    _tr.m[3646, "{}<hr>Instances: {}<br><br>{}"].format(
                        condition, n_actual, text[:-4]))

        def create_legend():
            if self.variable_color is None:
                names = ["<-8", "-8:-4", "-4:-2", "-2:2", "2:4", "4:8", ">8",
                         _tr.m[3647, "Residuals:"]]
        ...
        if attrs:
            CanvasText(self.canvas,
                       _tr.m[3648, "Feature {} has no values"].format(attrs[0]),
