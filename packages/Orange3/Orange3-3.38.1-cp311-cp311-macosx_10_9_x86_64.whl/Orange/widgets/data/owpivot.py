...
class Pivot:
    ...
    def __init__(self, table: Table, agg_funs: Iterable[Function],
        ...
        self._total_var = DiscreteVariable(_tr.m[1173, "Total"], values=("total", ))
    ...
    def __get_pivot_tab_domain(self, val_var, X, X_h, X_v, X_t, agg_funs):
        ...
        if create_time_var:
            ...
            attrs.extend([[TimeVariable(_tr.m[1174, "Total"], **kwargs)]] * 2)
        elif create_cont_var:
            attrs = [[ContinuousVariable(f"{v}", 1) for v in vals]] * 2
            attrs.extend([[ContinuousVariable(_tr.m[1175, "Total"], 1)]] * 2)
        else:
            attrs = []
            for x in (X, X_h):
            ...
            for x in (X_v, X_t):
                attrs.append([DiscreteVariable(_tr.m[1176, "Total"], map_values(0, x))])
        row_var_h = DiscreteVariable(self._row_var.name, values=[_tr.m[1177, "Total"]])
        aggr_attr = DiscreteVariable(_tr.m[1178, 'Aggregate'], [str(f) for f in agg_funs])
    ...
    Functions = [
        Function(i, *fdef) for i, fdef in enumerate((
            (_tr.m[1179, "Count"], len),
            (_tr.m[1180, "Count defined"], count_defined),
            (_tr.m[1181, "Sum"], lambda x: nansum(x, axis=0) if x.shape[0] > 0 else 0),
            (_tr.m[1182, "Mean"], wrapstat(nanmean)),
            (_tr.m[1183, "Min"], wrapstat(nanmin)),
            (_tr.m[1184, "Max"], wrapstat(nanmax)),
            (_tr.m[1185, "Mode"], mode),
            (_tr.m[1186, "Median"], wrapstat(nanmedian)),
            (_tr.m[1187, "Var"], wrapstat(nanvar)),
            (_tr.m[1188, "Majority"], majority)
    ))]
...
class OWPivot(OWWidget):
    name = _tr.m[1190, "Pivot Table"]
    description = _tr.m[1191, "Reshape data table based on column values."]
    category = _tr.m[1192, "Transform"]
    icon = "icons/Pivot.svg"
    priority = 1220
    keywords = _tr.m[1193, "pivot table, pivot, group, aggregate"]

    class Inputs:
        data = Input(_tr.m[1194, "Data"], Table, default=True)

    class Outputs:
        pivot_table = Output(_tr.m[1195, "Pivot Table"], Table, default=True)
        filtered_data = Output(_tr.m[1196, "Filtered Data"], Table)
        grouped_data = Output(_tr.m[1197, "Grouped Data"], Table)

    class Warning(OWWidget.Warning):
        # TODO - inconsistent for different variable types
        no_col_feature = Msg(_tr.m[1198, "Column feature should be selected."])
        cannot_aggregate = Msg(_tr.m[1199, "Some aggregations ({}) cannot be computed."])
        renamed_vars = Msg((_tr.m[1200, "Some variables have been renamed in some tables"] + _tr.m[1201, "to avoid duplicates.\n{}"]))
        too_many_values = Msg(_tr.m[1202, "Selected variable has too many values."])
        no_variables = Msg(_tr.m[1203, "At least 1 primitive variable is required."])
    ...
    def _add_control_area_controls(self):
        gui.comboBox(gui.vBox(self.controlArea, box=_tr.m[1204, "Rows"]),
                     self, "row_feature",
                     contentsLength=14,
                     searchable=True,
        ...
        gui.comboBox(gui.vBox(self.controlArea, box=_tr.m[1205, "Columns"]),
                     self, "col_feature",
                     contentsLength=14,
                     searchable=True,
                     model=DomainModel(placeholder=_tr.m[1206, "(Same as rows)"],
                                       valid_types=DiscreteVariable),
                     callback=self.__feature_changed,
                     orientation=Qt.Horizontal)
        gui.comboBox(gui.vBox(self.controlArea, box=_tr.m[1207, "Values"]),
                     self, "val_feature",
                     contentsLength=14,
                     searchable=True,
                     model=DomainModel(placeholder=_tr.m[1208, "(None)"]),
    ...
    def __add_aggregation_controls(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[1209, "Aggregations"])
        row = col = 0
        inbox = None
        new_inbox()
    ...
    def send_report(self):
        self.report_items((
            (_tr.m[1210, "Row feature"], self.row_feature),
            (_tr.m[1211, "Column feature"], self.col_feature),
            (_tr.m[1212, "Value feature"], self.val_feature)))
        if self.data and self.val_feature is not None:
            self.report_table("", self.table_view)
        if not self.data:
            self.report_items(((_tr.m[1213, "Group by"], self.row_feature),))
            self.report_table(self.table_view)
