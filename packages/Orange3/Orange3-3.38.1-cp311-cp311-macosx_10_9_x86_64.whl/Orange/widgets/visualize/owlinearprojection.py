...
class OWLinearProjection(OWAnchorProjectionWidget,
    ...
    name = _tr.m[3553, "Linear Projection"]
    description = (_tr.m[3554, "A multi-axis projection of data onto "] + _tr.m[3555, "a two-dimensional plane."])
    icon = "icons/LinearProjection.svg"
    priority = 240
    keywords = _tr.m[3556, "linear projection"]

    Projection_name = {Placement.Circular: _tr.m[3557, "Circular Placement"],
                       Placement.LDA: _tr.m[3558, "Linear Discriminant Analysis"],
                       Placement.PCA: _tr.m[3559, "Principal Component Analysis"]}
    ...
    class Error(OWAnchorProjectionWidget.Error):
        no_cont_features = Msg(_tr.m[3560, "Plotting requires numeric features"])

    class Information(OWAnchorProjectionWidget.Information):
        no_lda = Msg(_tr.m[3561, "LDA placement is disabled due to unsuitable target.\n{}"])

    def _add_controls(self):
        box = gui.vBox(self.controlArea, box=_tr.m[3562, "Features"])
        self._add_controls_variables(box)
        self._add_controls_placement(box)
        super()._add_controls()
        self.gui.add_control(
            self._effects_box, gui.hSlider, _tr.m[3563, "Hide radius:"], master=self.graph,
    ...
    def _add_controls_variables(self, box):
        ...
        self.btn_vizrank = self.vizrank_button(_tr.m[3564, "Suggest Features"])
    ...
    def _check_options(self):
        ...
        if self.data is not None:
            if (class_var := self.data.domain.class_var) is None:
                problem = _tr.m[3565, "Current data has no target variable"]
            elif not class_var.is_discrete:
                problem = _tr.e(_tr.c(3566, f"{class_var.name} is not categorical"))
            elif (nclasses := len(distinct := np.unique(self.data.Y))) == 0:
                problem = _tr.e(_tr.c(3567, f"Data has no defined values for {class_var.name}"))
            elif nclasses < 3:
                vals = _tr.m[3568, " and "].join(f"'{class_var.values[int(i)]}'" for i in distinct)
                problem = \
                    (_tr.e(_tr.c(3569, f"Data contains just {['one', 'two'][nclasses - 1]} distinct ")) + (_tr.e(_tr.c(3570, f"{pl(nclasses, 'value')} ({vals}) for '{class_var.name}'; ")) + _tr.m[3571, "at least three are required."]))
        if problem is None:
    ...
    def init_vizrank(self):
        ...
        if self.data is None:
            msg = _tr.m[3572, "There is no data."]
        elif self.attr_color is None:
            msg = _tr.m[3573, "Color variable has to be selected"]
        elif self.attr_color.is_continuous and \
                self.placement == Placement.LDA:
            msg = (_tr.m[3574, "Suggest Features does not work for Linear "] + (_tr.m[3575, "Discriminant Analysis Projection when "] + _tr.m[3576, "continuous color variable is selected."]))
        elif len([v for v in self.continuous_variables
                  if v is not self.attr_color]) < 3:
            msg = _tr.m[3577, "Not enough available continuous variables"]
        elif np.sum(np.all(np.isfinite(self.data.X), axis=1)) < 2:
            msg = _tr.m[3578, "Not enough valid data instances"]
        if not msg:
            super().init_vizrank(
    ...
    def _get_send_report_caption(self):
        ...
        return report.render_items_vert((
            (_tr.m[3579, "Projection"], projection_name()),
            (_tr.m[3580, "Color"], self._get_caption_var_name(self.attr_color)),
            (_tr.m[3581, "Label"], self._get_caption_var_name(self.attr_label)),
            (_tr.m[3582, "Shape"], self._get_caption_var_name(self.attr_shape)),
            (_tr.m[3583, "Size"], self._get_caption_var_name(self.attr_size)),
            (_tr.m[3584, "Jittering"], self.graph.jitter_size != 0 and
