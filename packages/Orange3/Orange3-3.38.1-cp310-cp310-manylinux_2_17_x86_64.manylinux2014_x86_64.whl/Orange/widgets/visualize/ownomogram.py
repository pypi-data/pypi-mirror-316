...
class SortBy(IntEnum):
    ...
    @staticmethod
    def items():
        return [_tr.m[3649, "Original order"], _tr.m[3650, "Alphabetically"], _tr.m[3651, "Absolute importance"],
                _tr.m[3652, "Positive influence"], _tr.m[3653, "Negative influence"]]
...
class ProbabilitiesDotItem(DotItem):
    ...
    def get_tooltip_text(self):
        text = _tr.m[3654, "Total: {} <br/>Probability: {:.0%}"].format(
...
class DiscreteMovableDotItem(MovableDotItem):
    def get_tooltip_text(self):
        ...
        return self.TOOLTIP_TEMPLATE.format(
            self.TOOLTIP_STYLE, _tr.m[3655, "Points: {}"].format(np.round(self.value, 2)),
...
class ContinuousItemMixin:
    def get_tooltip_text(self):
        return self.TOOLTIP_TEMPLATE.format(
            self.TOOLTIP_STYLE, _tr.m[3656, "Points: {}"].format(np.round(self.value, 2)),
            _tr.m[3657, "Value: {}"].format(np.round(self._get_tooltip_label_value(), 1)))
...
class ProbabilitiesRulerItem(QGraphicsWidget):
    ...
    def __init__(self, name, values, scale, name_offset, offset, get_points,
        ...
        font = name.document().defaultFont()
        font.setWeight(QFont.Bold)
        name_total = QGraphicsTextItem(_tr.m[3658, "Total"], self)
...
class OWNomogram(OWWidget):
    name = "Nomogram"
    description = (_tr.m[3659, " Nomograms for Visualization of Naive Bayesian"] + _tr.m[3660, " and Logistic Regression Classifiers."])
    icon = "icons/Nomogram.svg"
    priority = 2000
    keywords = _tr.m[3661, "nomogram"]

    class Inputs:
        classifier = Input(_tr.m[3662, "Classifier"], Model)
        data = Input(_tr.m[3663, "Data"], Table)

    class Outputs:
        features = Output(_tr.m[3664, "Features"], AttributeList)
    ...
    class Error(OWWidget.Error):
        invalid_classifier = Msg((_tr.m[3665, "Nomogram accepts only Naive Bayes and "] + _tr.m[3666, "Logistic Regression classifiers."]))
    ...
    def __init__(self):
        ...
        self.class_combo = gui.comboBox(
            ...
            searchable=True)
        grid.addWidget(QLabel(_tr.m[3667, "Target class: "]), 0, 0, lab_align)
        grid.addWidget(self.class_combo, 0, 1)

        self.norm_check = gui.checkBox(
            None, self, "normalize_probabilities", _tr.m[3668, "Normalize probabilities"],
            hidden=True, callback=self.update_scene,
            tooltip=(_tr.m[3669, "For multiclass data 1 vs. all probabilities do not"] + _tr.m[3670, " sum to 1 and therefore could be normalized."]))
        ...
        grid.addWidget(QLabel(_tr.m[3671, "Scale: "]), 2, 0, lab_align)
        grid.addWidget(gui.appendRadioButton(
            group, _tr.m[3672, "Point scale"], addToLayout=False), 2, 1)
        grid.addWidget(gui.appendRadioButton(
            group, _tr.m[3673, "Log odds ratios"], addToLayout=False), 3, 1)

        grid = QGridLayout()
        gui.widgetBox(self.controlArea, _tr.m[3674, "Displayed features"], orientation=grid)
        ...
        grid.addWidget(QLabel(_tr.m[3675, "Order: "]), 0, 0, lab_align)
        ...
        radio_all = gui.appendRadioButton(
            radio_group, _tr.m[3676, "All features"], addToLayout=False)
        radio_best = gui.appendRadioButton(
            radio_group, _tr.m[3677, "Best ranked:"], addToLayout=False)
        self.n_spin = gui.spin(
            None, self, "n_attributes", 1, self.MAX_N_ATTRS,
            callback=self._n_spin_changed, alignment=Qt.AlignRight,
            sizePolicy=(QSizePolicy.MinimumExpanding, QSizePolicy.Fixed)
        )
        grid.addWidget(QLabel(_tr.m[3678, "Show: "]), 1, 0, lab_align)
        ...
        self.cont_feature_dim_combo = gui.comboBox(
            None, self, "cont_feature_dim_index", label=_tr.m[3679, "Numeric features:"],
            items=[_tr.m[3680, "1D projection"], _tr.m[3681, "2D curve"]], orientation=Qt.Horizontal,
            callback=self.update_scene,
            sizePolicy=(QSizePolicy.MinimumExpanding, QSizePolicy.Fixed))
    ...
    def update_scene(self):
        ...
        point_text = QGraphicsTextItem(_tr.m[3682, "Points"])
        ...
        probs_text = QGraphicsTextItem(_tr.m[3683, "Probabilities (%)"])
