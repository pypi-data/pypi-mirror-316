...
class OWSql(OWBaseSql):
    name = _tr.m[1605, "SQL Table"]
    id = "orange.widgets.data.sql"
    description = _tr.m[1606, "Load dataset from SQL."]
    icon = "icons/SQLTable.svg"
    priority = 30
    category = _tr.m[1607, "Data"]
    keywords = _tr.m[1608, "sql table, load"]

    class Outputs:
        data = Output(_tr.m[1609, "Data"], Table, doc=_tr.m[1610, "Attribute-valued dataset read from the input file."])
    ...
    class Information(OWBaseSql.Information):
        data_sampled = Msg(_tr.m[1611, "Data description was generated from a sample."])

    class Warning(OWBaseSql.Warning):
        missing_extension = Msg(_tr.m[1612, "Database is missing extensions: {}"])

    class Error(OWBaseSql.Error):
        no_backends = Msg(_tr.m[1613, "Please install a backend to use this widget."])
    ...
    def _add_tables_controls(self):
        vbox = gui.vBox(self.controlArea, _tr.m[1614, "Tables"])
        box = gui.vBox(vbox)
        ...
        self.tablecombo.setToolTip(_tr.m[1615, 'table'])
        ...
        cb = gui.checkBox(mt, self, 'materialize', _tr.m[1616, 'Materialize to table '])
        cb.setToolTip(_tr.m[1617, 'Save results of the query in a table'])
        le = gui.lineEdit(mt, self, 'materialize_table_name')
        le.setToolTip(_tr.m[1618, 'Save results of the query in a table'])

        gui.button(self.custom_sql, self, _tr.m[1619, 'Execute'], callback=self.open_table)

        box.layout().addWidget(self.custom_sql)

        gui.checkBox(box, self, "guess_values",
                     _tr.m[1620, "Auto-discover categorical variables"],
                     callback=self.open_table)

        self.downloadcb = gui.checkBox(box, self, "download",
                                       _tr.m[1621, "Download data to local memory"],
                                       callback=self.open_table)
    ...
    def refresh_tables(self):
        ...
        self.tables.append(_tr.m[1622, "Select a table"])
        self.tables.append(_tr.m[1623, "Custom SQL"])
    ...
    def select_table(self):
        ...
        if self.tablecombo.itemText(curIdx) != _tr.m[1624, "Custom SQL"]:
            self.custom_sql.setVisible(False)
            return self.open_table()
        else:
            self.custom_sql.setVisible(True)
            self.data_desc_table = None
            self.database_desc[_tr.m[1625, "Table"]] = _tr.m[1626, "(None)"]
            self.table = None
    ...
    def get_table(self):
        ...
        if curIdx <= 0:
            if self.database_desc:
                self.database_desc["Table"] = _tr.m[1627, "(None)"]
            self.data_desc_table = None
            return None

        if self.tablecombo.itemText(curIdx) != _tr.m[1628, "Custom SQL"]:
            self.table = self.tables[self.tablecombo.currentIndex()]
            self.database_desc["Table"] = self.table
            if _tr.m[1629, "Query"] in self.database_desc:
                del self.database_desc[_tr.m[1630, "Query"]]
            what = self.table
        else:
            what = self.sql = self.sqltext.toPlainText()
            self.table = _tr.m[1631, "Custom SQL"]
            if self.materialize:
                if not self.materialize_table_name:
                    self.Error.connection(
                        _tr.m[1632, "Specify a table name to materialize the query"])
                    return None
                try:
        ...
        if table.approx_len() > LARGE_TABLE and self.guess_values:
            ...
            confirm.setText((_tr.m[1633, "Attribute discovery might take "] + (_tr.m[1634, "a long time on large tables.\n"] + _tr.m[1635, "Do you want to auto discover attributes?"])))
            confirm.addButton(_tr.m[1636, "Yes"], QMessageBox.YesRole)
            no_button = confirm.addButton(_tr.m[1637, "No"], QMessageBox.NoRole)
            if is_postgres(self.backend):
                sample_button = confirm.addButton(_tr.m[1638, "Yes, on a sample"],
                                                  QMessageBox.YesRole)
            confirm.exec()
        ...
        if self.download:
            if table.approx_len() > AUTO_DL_LIMIT:
                if is_postgres(self.backend):
                    confirm = QMessageBox(self)
                    confirm.setIcon(QMessageBox.Warning)
                    confirm.setText((_tr.m[1639, "Data appears to be big. Do you really "] + (_tr.m[1640, "want to download it to local memory?\n"] + _tr.m[1641, "Table length: {:,}. Limit {:,}"])).format(table.approx_len(), MAX_DL_LIMIT))

                    if table.approx_len() <= MAX_DL_LIMIT:
                        confirm.addButton(_tr.m[1642, "Yes"], QMessageBox.YesRole)
                    no_button = confirm.addButton(_tr.m[1643, "No"], QMessageBox.NoRole)
                    sample_button = confirm.addButton(_tr.m[1644, "Yes, a sample"],
                                                      QMessageBox.YesRole)
                    confirm.exec()
                    if confirm.clickedButton() == no_button:
                        return None
                    ...
                    if table.approx_len() > MAX_DL_LIMIT:
                        QMessageBox.warning(
                            self, _tr.m[1645, 'Warning'],
                            (_tr.m[1646, "Data is too big to download.\n"] + _tr.m[1647, "Table length: {:,}. Limit {:,}"]).format(table.approx_len(), MAX_DL_LIMIT)
                        )
                        return None
                    else:
                        confirm = QMessageBox.question(
                            self, _tr.m[1648, 'Question'],
                            (_tr.m[1649, "Data appears to be big. Do you really "] + _tr.m[1650, "want to download it to local memory?"]),
