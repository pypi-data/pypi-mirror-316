...
class OWSelectAttributes(widget.OWWidget):
    ...
    name = _tr.m[1472, "Select Columns"]
    description = (_tr.m[1473, "Select columns from the data table and assign them to "] + _tr.m[1474, "data features, classes or meta variables."])
    category = _tr.m[1475, "Transform"]
    icon = "icons/SelectColumns.svg"
    priority = 100
    keywords = _tr.m[1476, "select columns, filter, attributes, target, variable"]

    class Inputs:
        data = Input(_tr.m[1477, "Data"], Table, default=True)
        features = Input(_tr.m[1478, "Features"], AttributeList)

    class Outputs:
        data = Output(_tr.m[1479, "Data"], Table)
        features = Output(_tr.m[1480, "Features"], AttributeList, dynamic=False)
    ...
    class Warning(widget.OWWidget.Warning):
        mismatching_domain = Msg(_tr.m[1481, "Features and data domain do not match"])
        multiple_targets = Msg(_tr.m[1482, "Most widgets do not support multiple targets"])
    ...
    def __init__(self):
        ...
        self.use_features_box = gui.auto_commit(
            ...
            _tr.m[1485, "Use input features"], _tr.m[1486, "Always use input features"],
        ...
        # footer
        gui.button(self.buttonsArea, self, _tr.m[1489, "Reset"], callback=self.reset)
        ...
        gui.checkBox(
            ...
            label=_tr.m[1490, "Ignore new variables by default"],
            tooltip=(_tr.m[1491, "When the widget receives data with additional columns "] + (_tr.m[1492, "they are added to the available attributes column if "] + _tr.m[1493, "<i>Ignore new variables by default</i> is checked."]))
        )
    ...
    def send_report(self):
        ...
        self.report_domain(_tr.m[1494, "Input data"], self.data.domain)
        if (in_domain.attributes, in_domain.class_vars, in_domain.metas) == (
                out_domain.attributes, out_domain.class_vars, out_domain.metas):
            self.report_paragraph(_tr.m[1495, "Output data"], _tr.m[1496, "No changes."])
        else:
            self.report_domain(_tr.m[1497, "Output data"], self.output_data.domain)
            ...
            if diff:
                ...
                self.report_items(((_tr.m[1498, "Removed"], text),))
