...
class ColumnType(enum.Enum):
    ...
    """
    # Skip column
    Skip = _tr.m[3242, "Skip"]
    # Autodetect column type
    Auto = "Auto"
    # Numeric (real) column
    Numeric = _tr.m[3243, "Numeric"]
    # Categorical column
    Categorical = _tr.m[3244, "Categorical"]
    # Text column
    Text = _tr.m[3245, "Text"]
    # Date time column
    Time = _tr.m[3246, "Time"]
...
class CSVOptionsWidget(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.encoding_cb = QComboBox(
            ...
            toolTip=_tr.m[3251, "Select file text encoding"],
        )
        ...
        self.delimiter_cb = QComboBox(
            ...
            toolTip=_tr.m[3252, "Select cell delimiter character."]
        )
        ...
        self.delimiter_cb.addItem(_tr.m[3253, "Other"])
        ...
        form.addRow(_tr.m[3254, "Encoding"], self.encoding_cb)
        form.addRow(QFrame(self, frameShape=QFrame.HLine))
        form.addRow(_tr.m[3255, "Cell delimiter"], delimlayout)
        form.addRow(_tr.m[3256, "Quote character"], self.quoteedit)
        self.setLayout(form)
    ...
    def __show_encodings_widget(self):
        ...
        w = encodings.SelectEncodingsWidget(
            self, Qt.Tool,
            windowTitle=_tr.m[3258, "Customize Encodings List"],
    ...
    def __set_visible_codecs(self, codecs):
        ...
        cb.addItem(_tr.m[3259, "Customize Encodings List..."], userData=...)
...
class CSVImportWidget(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.grouping_sep_edit_cb = TextEditCombo(
            ...
            toolTip=_tr.m[3260, "Thousands group separator"],
            minimumContentsLength=1,
            sizeAdjustPolicy=QComboBox.AdjustToMinimumContentsLengthWithIcon
        )
        items = [
            {Qt.DisplayRole: _tr.m[3261, "None"], Qt.EditRole: "",
             Qt.ToolTipRole: _tr.m[3262, "No separator"]},
            {Qt.DisplayRole: ".", Qt.EditRole: "."},
            {Qt.DisplayRole: ",", Qt.EditRole: ","},
            {Qt.DisplayRole: _tr.m[3263, "Space"], Qt.EditRole: " "},
            {Qt.DisplayRole: "'", Qt.EditRole: "'"},
        ]
        ...
        self.decimal_sep_edit_cb = TextEditCombo(
            ...
            toolTip=_tr.m[3264, "Decimal separator"],
            minimumContentsLength=1,
            sizeAdjustPolicy=QComboBox.AdjustToMinimumContentsLengthWithIcon
        )
        ...
        number_sep_layout.addWidget(QLabel(_tr.m[3265, "Grouping:"]))
        number_sep_layout.addWidget(self.grouping_sep_edit_cb)
        number_sep_layout.addWidget(QLabel(_tr.m[3266, "Decimal:"]))
        number_sep_layout.addWidget(self.decimal_sep_edit_cb)
        number_sep_layout.addStretch(10)
        form.addRow(_tr.m[3267, "Number separators:"], number_sep_layout)
        ...
        types = [
            {Qt.DisplayRole: _tr.m[3268, "Auto"],
             Qt.ToolTipRole: (_tr.m[3269, "The type will be determined automatically based "] + _tr.m[3270, "on column contents."]),
             Qt.UserRole: ColumnType.Auto},
            {Qt.DisplayRole: _tr.m[3271, "Numeric"], Qt.UserRole: ColumnType.Numeric},
            {Qt.DisplayRole: _tr.m[3272, "Categorical"],
             Qt.UserRole: ColumnType.Categorical},
            {Qt.DisplayRole: _tr.m[3273, "Text"], Qt.UserRole: ColumnType.Text},
            {Qt.DisplayRole: _tr.m[3274, "Datetime"], Qt.UserRole: ColumnType.Time},
            {Qt.AccessibleDescriptionRole: "separator"},
            {Qt.DisplayRole: _tr.m[3275, "Ignore"],
             Qt.UserRole: ColumnType.Skip,
             Qt.ToolTipRole: _tr.m[3276, "The column will not be loaded"]}
        ]
        ...
        form.addRow(_tr.m[3277, "Column type"], self.column_type_edit_cb)
    ...
    def __run_row_menu(self, pos: QPoint, rows: List[int]):
        ...
        m = QMenu(header)
        skip_action = m.addAction(_tr.m[3278, "Skip"])
        skip_action.setCheckable(True)
        skip_action.setChecked(current == TablePreview.Skipped)
        m.addSection("")
        mark_header = m.addAction(_tr.m[3279, "Header"])
...
def format_exception_csv(err):
    return _tr.m[3280, "CSV parsing error: "] + str(err)
