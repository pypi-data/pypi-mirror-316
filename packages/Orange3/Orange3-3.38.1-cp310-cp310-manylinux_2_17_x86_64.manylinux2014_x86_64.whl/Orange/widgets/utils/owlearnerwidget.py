...
class OWBaseLearner(OWWidget, metaclass=OWBaseLearnerMeta, openclass=True):
    ...
    class Error(OWWidget.Error):
        data_error = Msg("{}")
        fitting_failed = Msg(_tr.m[3189, "Fitting failed.\n{}"])
        sparse_not_supported = Msg(_tr.m[3190, "Sparse data is not supported."])
        out_of_memory = Msg(_tr.m[3191, "Out of memory."])

    class Warning(OWWidget.Warning):
        outdated_learner = Msg(_tr.m[3192, "Press Apply to submit changes."])

    class Information(OWWidget.Information):
        ignored_preprocessors = Msg(
            (_tr.m[3193, "Ignoring default preprocessing.\n"] + (_tr.m[3194, "Default preprocessing, such as scaling, one-hot encoding and "] + (_tr.m[3195, "treatment of missing data, has been replaced with user-specified "] + (_tr.m[3196, "preprocessors. Problems may occur if these are inadequate "] + _tr.m[3197, "for the given data."])))))

    class Inputs:
        data = Input(_tr.m[3198, "Data"], Table)
        preprocessor = Input(_tr.m[3199, "Preprocessor"], Preprocess)

    class Outputs:
        learner = Output(_tr.m[3200, "Learner"], Learner, dynamic=False)
        model = Output(_tr.m[3201, "Model"], Model, dynamic=False,
                       replaces=[_tr.m[3202, "Classifier"], _tr.m[3203, "Predictor"]])
        ...
        if data is not None and data.domain.class_var is None:
            if data.domain.class_vars:
                self.Error.data_error(
                    (_tr.m[3204, "Data contains multiple target variables.\n"] + _tr.m[3205, "Select a single one with the Select Columns widget."]))
            else:
                self.Error.data_error(
                    (_tr.m[3206, "Data has no target variable.\n"] + _tr.m[3207, "Select one with the Select Columns widget."]))
            self.data = None
    ...
    def check_data(self):
        ...
        if self.data is not None and self.learner is not None:
            ...
            if reason is not None:
                ...
                self.Error.data_error(_tr.m[3208, "Dataset is empty."])
            elif len(ut.unique(self.data.Y)) < 2:
                self.Error.data_error(_tr.m[3209, "Data contains a single target value."])
            elif self.data.X.size == 0:
                self.Error.data_error(_tr.m[3210, "Data has no features to learn from."])
    ...
    def send_report(self):
        self.report_items(((_tr.m[3211, "Name"], self.effective_learner_name()),))

        model_parameters = self.get_learner_parameters()
        if model_parameters:
            self.report_items(_tr.m[3212, "Model parameters"], model_parameters)

        if self.data:
            self.report_data(_tr.m[3213, "Data"], self.data)
        ...
        if issubclass(self.LEARNER, Fitter):
            ...
            if type(self).add_classification_layout is not \
                ...
                classification_box = gui.widgetBox(
                    self.controlArea, _tr.m[3214, 'Classification'])
            ...
            if type(self).add_regression_layout is not \
                ...
                regression_box = gui.widgetBox(self.controlArea, _tr.m[3215, 'Regression'])
    ...
    def add_learner_name_widget(self):
        self.name_line_edit = gui.lineEdit(
            self.controlArea, self, 'learner_name', box=_tr.m[3216, 'Name'],
            placeholderText=self.captionTitle,
            tooltip=_tr.m[3217, 'The name will identify this model in other widgets'],
