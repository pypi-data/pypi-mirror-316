...
class OWStackedLearner(OWBaseLearner):
    name = _tr.m[2457, "Stacking"]
    description = _tr.m[2458, "Stack multiple models."]
    icon = "icons/Stacking.svg"
    priority = 100

    LEARNER = StackedFitter

    learner_name = Setting(_tr.m[2459, "Stack"])

    class Inputs(OWBaseLearner.Inputs):
        learners = MultiInput(_tr.m[2460, "Learners"], Learner, filter_none=True)
        aggregate = Input(_tr.m[2461, "Aggregate"], Learner)
    ...
    def get_learner_parameters(self):
        return ((_tr.m[2462, "Base learners"], [l.name for l in self.learners]),
                (_tr.m[2463, "Aggregator"],
                 self.aggregate.name if self.aggregate else _tr.m[2464, 'default']))
