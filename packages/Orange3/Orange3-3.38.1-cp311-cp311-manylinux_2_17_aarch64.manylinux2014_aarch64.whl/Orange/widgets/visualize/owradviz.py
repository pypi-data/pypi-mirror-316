...
class RadvizVizRank(VizRankDialogNAttrs):
    ...
    def __init__(self, parent, data, attributes, color, n_attrs):
        super().__init__(parent, data, attributes, color, n_attrs,
                         spin_label=_tr.m[3728, "Maximum number of variables: "])
...
class OWRadviz(OWAnchorProjectionWidget, VizRankMixin(RadvizVizRank)):
    name = _tr.m[3729, "Radviz"]
    description = _tr.m[3730, "Display Radviz projection"]
    icon = "icons/Radviz.svg"
    priority = 241
    keywords = _tr.m[3731, "radviz, viz"]
    ...
    class Warning(OWAnchorProjectionWidget.Warning):
        removed_vars = widget.Msg(
            _tr.m[3732, "Categorical variables with more than two values are not shown."])
        max_vars_selected = widget.Msg(
            _tr.m[3733, "Maximum number of selected variables reached."])
    ...
    def _add_controls(self):
        box = gui.vBox(self.controlArea, box=_tr.m[3734, "Features"])
        ...
        box.layout().addWidget(self.vizrank_button(_tr.m[3735, "Suggest features"]))
    ...
    def init_vizrank(self):
        ...
        if self.data is None:
            msgerr = _tr.m[3736, "No data"]
        elif len(self.primitive_variables) <= 3:
            msgerr = _tr.m[3737, "Not enough variables"]
        elif self.attr_color is None:
            msgerr = _tr.m[3738, "Color is not set."]
        elif np.isnan(self.data.get_column(self.attr_color)).all():
            msgerr = _tr.e(_tr.c(3739, "No rows with defined color variable"))
        elif np.sum(np.all(np.isfinite(self.data.X), axis=1)) <= 1:
            msgerr = _tr.m[3740, "Not enough rows without missing data"]
        elif not np.all(np.nan_to_num(np.nanstd(self.data.X, 0)) != 0):
            msgerr = _tr.m[3741, "Constant data"]
