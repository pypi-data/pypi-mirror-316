...
DEFAULT_ITEM_NAME = _tr.m[974, "item"]
DEFAULT_VALUE_NAME = _tr.m[975, "value"]
DEFAULT_NAME_FOR_ROW = _tr.m[976, "row"]
...
class OWMelt(widget.OWWidget):
    name = _tr.m[977, "Melt"]
    description = _tr.m[978, "Convert wide data to narrow data, a list of item-value pairs"]
    category = _tr.m[979, "Transform"]
    icon = "icons/Melt.svg"
    keywords = _tr.m[980, "melt, shopping list, wide, narrow"]
    priority = 2230

    class Inputs:
        data = widget.Input(_tr.m[981, "Data"], Table)

    class Outputs:
        data = widget.Output(_tr.m[982, "Data"], Table)

    class Information(widget.OWWidget.Information):
        no_suitable_features = Msg(
            (_tr.m[983, "No columns with unique values\n"] + _tr.m[984, "Only columns with unique valules are useful for row identifiers."]))
    ...
    def __init__(self, *args, **kwargs):
        ...
        box = gui.widgetBox(self.controlArea, _tr.m[985, "Unique Row Identifier"])
        self.idvar_model = itemmodels.VariableListModel(
            [None], placeholder=_tr.m[986, "Row number"])
        self.var_cb = gui.comboBox(
            box, self, "idvar", model=self.idvar_model,
            callback=self._invalidate, minimumContentsLength=16,
            tooltip=_tr.m[987, "A column with identifier, like customer's id"])

        box = gui.widgetBox(self.controlArea, "Filter")
        gui.checkBox(
            box, self, "only_numeric", _tr.m[988, "Ignore non-numeric features"],
            callback=self._invalidate)
        gui.checkBox(
            box, self, "exclude_zeros", _tr.m[989, "Exclude zero values"],
            callback=self._invalidate,
            tooltip=_tr.m[990, "Besides missing values, also omit items with zero values"])

        form = QFormLayout()
        gui.widgetBox(
            self.controlArea, _tr.m[991, "Names for generated features"], orientation=form)
        form.addRow(_tr.m[992, "Item:"],
                    gui.lineEdit(
                        None, self, "item_var_name",
                        callback=self._invalidate,
                        placeholderText=DEFAULT_ITEM_NAME,
                        styleSheet="padding-left: 3px"))
        form.addRow(_tr.m[993, "Value:"],
                    gui.lineEdit(
    ...
    def send_report(self):
        self.report_items(_tr.m[994, "Settings"], (
            (_tr.m[995, "Row identifier"], self.controls.idvar.currentText()),
            (_tr.m[996, "Ignore non-numeric features"], bool_str(self.only_numeric)),
            (_tr.m[997, "Exclude zero values"], bool_str(self.exclude_zeros))
        ))
        if self._output_desc:
            self.report_items(_tr.m[998, "Output"], self._output_desc)

    def _store_output_desc(self, output):
        self._output_desc = {
            _tr.m[999, "Item column"]: output.domain.attributes[1].name,
            _tr.m[1000, "Value column"]: output.domain.class_var.name,
            _tr.m[1001, "Number of items"]: len(output)
        }
