...
DEFAULT_READER_TEXT = _tr.m[863, "Automatically detect type"]
...
class OWFile(OWUrlDropBase, RecentPathsWComboMixin):
    ...
    description = (_tr.m[865, "Read data from an input file or network "] + _tr.m[866, "and send a data table to the output."])
    icon = "icons/File.svg"
    priority = 10
    category = _tr.m[867, "Data"]
    keywords = _tr.m[868, "file, load, read, open"]

    class Outputs:
        data = Output(_tr.m[869, "Data"], Table,
                      doc=_tr.m[870, "Attribute-valued dataset read from the input file."])
    ...
    class Information(widget.OWWidget.Information):
        no_file_selected = Msg(_tr.m[871, "No file selected."])

    class Warning(widget.OWWidget.Warning):
        file_too_big = Msg((_tr.m[872, "The file is too large to load automatically."] + _tr.m[873, " Press Reload to load."]))
        load_warning = Msg(_tr.m[874, "Read warning:\n{}"])
        performance_warning = Msg(
            _tr.m[875, "Categorical variables with >100 values may decrease performance."])
        renamed_vars = Msg((_tr.m[876, "Some variables have been renamed "] + _tr.m[877, "to avoid duplicates.\n{}"]))
        multiple_targets = Msg(_tr.m[878, "Most widgets do not support multiple targets"])

    class Error(widget.OWWidget.Error):
        file_not_found = Msg(_tr.m[879, "File not found."])
        missing_reader = Msg(_tr.m[880, "Missing reader."])
        sheet_error = Msg(_tr.m[881, "Error listing available sheets."])
        unknown = Msg(_tr.m[882, "Read error:\n{}"])

    UserAdviceMessages = [
        widget.Message(
            (_tr.m[883, "Use CSV File Import widget for advanced options "] + _tr.m[884, "for comma-separated files"]),
            _tr.m[885, "use-csv-file-import"]),
        widget.Message(
            (_tr.m[886, "This widget loads only tabular data. Use other widgets to load "] + _tr.m[887, "other data types like models, distance matrices and networks."]),
            "other-data-types"
        )
    ]
    ...
    def __init__(self):
        ...
        layout = QGridLayout()
        layout.setSpacing(4)
        gui.widgetBox(self.controlArea, orientation=layout, box=_tr.m[888, 'Source'])
        vbox = gui.radioButtons(None, self, "source", box=True,
                                callback=self.load_data, addToLayout=False)

        rb_button = gui.appendRadioButton(vbox, _tr.m[889, "File:"], addToLayout=False)
        ...
        reload_button = gui.button(
            None, self, _tr.m[890, "Reload"], callback=self.load_data, autoDefault=False)
        ...
        self.sheet_label.setText(_tr.m[891, 'Sheet'])
        ...
        layout = QGridLayout()
        layout.setSpacing(4)
        gui.widgetBox(self.controlArea, orientation=layout, box=_tr.m[892, 'File Type'])
        ...
        self.infolabel = gui.widgetLabel(box, _tr.m[893, 'No data loaded.'])

        box = gui.widgetBox(self.controlArea, _tr.m[894, "Columns (Double click to edit)"])
        ...
        box = gui.hBox(box)
        gui.button(
            box, self, _tr.m[895, "Reset"], callback=self.reset_domain_edit,
            autoDefault=False
        )
        gui.rubber(box)
        self.apply_button = gui.button(
            box, self, _tr.m[896, "Apply"], callback=self.apply_domain_edit)
        ...
        gui.rubber(hBox)
        gui.button(
            hBox, self, _tr.m[897, "Browse documentation datasets"],
        ...
        if in_demos:
            ...
            if not os.path.exists(start_file):
                QMessageBox.information(
                    None, _tr.m[898, "File"],
                    _tr.m[899, "Cannot find the directory with documentation datasets"])
                return
        else:
    ...
    def load_data(self):
        ...
        if error:
            ...
            self.infolabel.setText(_tr.m[900, "No data."])
        ...
        if self.source == self.LOCAL_FILE:
            ...
            if self.recent_paths and self.recent_paths[0].file_format:
                ...
                try:
                    ...
                    raise MissingReaderException(_tr.e(_tr.c(901, f'Can not find reader "{qname}"'))) from ex
    ...
    def _describe(table):
        ...
        descs = [attrs[desc]
                 for desc in (_tr.m[902, "Name"], _tr.m[903, "Description"]) if desc in attrs]
        if len(descs) == 2:
            descs[0] = f"<b>{descs[0]}</b>"
        if descs:
            text += f"<p>{'<br/>'.join(descs)}</p>"

        text += _tr.e(_tr.c(904, f"<p>{len(table)} {pl(len(table), 'instance')}"))
        ...
        text += _tr.e(_tr.c(905, f"<br/>{nattrs} {pl(nattrs, 'feature')} {missing_in_attr}"))
        if domain.has_continuous_class:
            text += _tr.e(_tr.c(906, f"<br/>Regression; numerical class {missing_in_class}"))
        elif domain.has_discrete_class:
            nvals = len(domain.class_var.values)
            text += (_tr.m[907, "<br/>Classification; categorical class "] + _tr.e(_tr.c(908, f"with {nvals} {pl(nvals, 'value')} {missing_in_class}")))
        elif table.domain.class_vars:
            ntargets = len(table.domain.class_vars)
            text += (_tr.m[909, "<br/>Multi-target; "] + (_tr.e(_tr.c(910, f"{ntargets} target {pl(ntargets, 'variable')} ")) + f"{missing_in_class}"))
        else:
            text += _tr.m[911, "<br/>Data has no target variable."]
        nmetas = len(domain.metas)
        text += _tr.e(_tr.c(912, f"<br/>{nmetas} {pl(nmetas, 'meta attribute')}"))
        text += "</p>"

        if 'Timestamp' in table.domain:
            # Google Forms uses this header to timestamp responses
            text += (_tr.e(_tr.c(913, f"<p>First entry: {table[0, 'Timestamp']}<br/>")) + _tr.e(_tr.c(914, f"Last entry: {table[-1, 'Timestamp']}</p>")))
        return text
    ...
    def apply_domain_edit(self):
        ...
        if self.data is None:
            ...
            if renamed:
                self.Warning.renamed_vars(_tr.e(_tr.c(915, f"Renamed: {', '.join(renamed)}")))
    ...
    def send_report(self):
        def get_ext_name(filename):
            try:
                ...
                return _tr.m[916, "unknown"]

        if self.data is None:
            self.report_paragraph(_tr.m[917, "File"], _tr.m[918, "No file."])
            return
        ...
        if self.source == self.LOCAL_FILE:
            ...
            self.report_items(_tr.m[919, "File"], [(_tr.m[920, "File name"], name),
                                       (_tr.m[921, "Format"], get_ext_name(name))])
        else:
            self.report_items(_tr.m[922, "Data"], [(_tr.m[923, "Resource"], self.url),
                                       (_tr.m[924, "Format"], get_ext_name(self.url))])

        self.report_data(_tr.m[925, "Data"], self.data)
