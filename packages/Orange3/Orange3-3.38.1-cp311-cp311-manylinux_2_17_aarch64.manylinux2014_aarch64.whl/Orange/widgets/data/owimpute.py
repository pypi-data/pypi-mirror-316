...
class OWImpute(OWWidget):
    name = _tr.m[956, "Impute"]
    description = _tr.m[957, "Impute missing values in the data table."]
    icon = "icons/Impute.svg"
    priority = 2110
    keywords = _tr.m[958, "impute, substitute, missing"]
    category = _tr.m[959, "Transform"]

    class Inputs:
        data = Input(_tr.m[960, "Data"], Orange.data.Table)
        learner = Input(_tr.m[961, "Learner"], Learner)

    class Outputs:
        data = Output(_tr.m[962, "Data"], Orange.data.Table)

    class Error(OWWidget.Error):
        imputation_failed = Msg(_tr.m[963, "Imputation failed for '{}'"])
        model_based_imputer_sparse = \
            Msg(_tr.m[964, "Model based imputer does not work for sparse data"])

    class Warning(OWWidget.Warning):
        cant_handle_var = Msg(_tr.m[965, "Default method can not handle '{}'"])
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[966, "Default Method"])
        ...
        button = QRadioButton(_tr.m[967, "Fixed values; numeric variables:"])
        ...
        hlayout.addWidget(QLabel(_tr.m[968, ", time:"]))
        ...
        box = gui.hBox(self.controlArea, self.tr(_tr.m[969, "Individual Attribute Settings"]),
                       flat=False)
        ...
        self.reset_button = QPushButton(
            _tr.m[970, "Restore All to Default"], enabled=False, default=False,
    ...
    def send_report(self):
        ...
        if specific:
            self.report_items((
                (_tr.m[971, "Default method"], default.name),
                (_tr.m[972, "Specific imputers"], ", ".join(specific))
            ))
        else:
            self.report_items(((_tr.m[973, "Method"], default.name),))
