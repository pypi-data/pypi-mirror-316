...
class VizRankDialog(QDialog, ProgressBarMixin, WidgetMessagesMixin,
    ...
    class Information(WidgetMessagesMixin.Information):
        nothing_to_rank = Msg(_tr.m[3972, "There is nothing to rank."])
    ...
    def __init__(self, master):
        ...
        self.button = gui.button(
            self, self, _tr.m[3973, "Start"], callback=self.toggle, default=True)
    ...
    def initialize(self):
        ...
        self.button.setText(_tr.m[3974, "Start"])
    ...
    def on_done(self, result: Result):
        self.button.setText(_tr.m[3975, "Finished"])
    ...
    def toggle(self):
        ...
        if self.keep_running:
            self.button.setText(_tr.m[3976, "Pause"])
            self.button.repaint()
            ...
            self.button.setText(_tr.m[3977, "Continue"])
            self.button.repaint()
            self.cancel()
            self._stopped()
...
def run_vizrank(compute_score: Callable, iterate_states: Callable,
    ...
    task.set_status(_tr.m[3978, "Getting combinations..."])
    task.set_progress_value(0.1)
    states = iterate_states(saved_state)

    task.set_status(_tr.m[3979, "Getting scores..."])
