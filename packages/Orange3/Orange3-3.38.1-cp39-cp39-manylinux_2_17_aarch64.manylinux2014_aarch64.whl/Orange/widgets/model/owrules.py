...
class CustomRuleClassifier(_RuleClassifier):
    ...
    def __init__(self, domain, rule_list, params):
        ...
        self.rule_ordering = params[_tr.m[2337, "Rule ordering"]]
        self.covering_algorithm = params[_tr.m[2338, "Covering algorithm"]]
        self.params = params

    def predict(self, X):
        if (self.rule_ordering == _tr.m[2339, "ordered"] and
                self.covering_algorithm == _tr.m[2340, "exclusive"]):
            return self.ordered_predict(X)

        if (self.rule_ordering == _tr.m[2341, "unordered"] or
                self.covering_algorithm == _tr.m[2342, "weighted"]):
...
class CustomRuleLearner(_RuleLearner):
    ...
    def __init__(self, preprocessors, base_rules, params):
        ...
        self.rule_ordering = params[_tr.m[2343, "Rule ordering"]]

        # top-level control procedure (covering algorithm)
        self.covering_algorithm = params[_tr.m[2344, "Covering algorithm"]]
        if self.covering_algorithm == _tr.m[2345, "exclusive"]:
            self.cover_and_remove = self.exclusive_cover_and_remove
        elif self.covering_algorithm == _tr.m[2346, "weighted"]:
            self.gamma = params[_tr.m[2347, "Gamma"]]
            self.cover_and_remove = self.weighted_cover_and_remove

        # bottom-level search procedure (search algorithm)
        self.rule_finder.search_algorithm.beam_width = params[_tr.m[2348, "Beam width"]]
        ...
        evaluation_measure = params[_tr.m[2349, "Evaluation measure"]]
        if evaluation_measure == _tr.m[2350, "entropy"]:
            evaluator = EntropyEvaluator()
        elif evaluation_measure == _tr.m[2351, "laplace"]:
            evaluator = LaplaceAccuracyEvaluator()
        elif evaluation_measure == _tr.m[2352, "wracc"]:
        ...
        min_rule_cov = params[_tr.m[2353, "Minimum rule coverage"]]
        max_rule_length = params[_tr.m[2354, "Maximum rule length"]]
        ...
        # bottom-level search procedure (over-fitting avoidance heuristics)
        default_alpha = params[_tr.m[2355, "Default alpha"]]
        parent_alpha = params[_tr.m[2356, "Parent alpha"]]
    ...
    def fit_storage(self, data):
        ...
        if self.rule_ordering == _tr.m[2357, "ordered"]:
            ...
            if (not rule_list or rule_list and rule_list[-1].length > 0 or
                    self.covering_algorithm == _tr.m[2358, "weighted"]):
                rule_list.append(
                    self.generate_default_rule(X, Y, W, data.domain))

        elif self.rule_ordering == _tr.m[2359, "unordered"]:
...
class OWRuleLearner(OWBaseLearner):
    name = _tr.m[2360, "CN2 Rule Induction"]
    description = _tr.m[2361, "Induce rules from data using CN2 algorithm."]
    icon = "icons/CN2RuleInduction.svg"
    replaces = [
        "Orange.widgets.classify.owrules.OWRuleLearner",
    ]
    priority = 19
    keywords = _tr.m[2362, "cn2 rule induction"]

    LEARNER = CustomRuleLearner
    supports_sparse = False

    storage_orders = [_tr.m[2363, "ordered"], _tr.m[2364, "unordered"]]
    storage_covers = [_tr.m[2365, "exclusive"], _tr.m[2366, "weighted"]]
    storage_measures = [_tr.m[2367, "entropy"], _tr.m[2368, "laplace"], _tr.m[2369, "wracc"]]
    ...
    def add_main_layout(self):
        ...
        middle_box = gui.vBox(widget=self.controlArea, box=_tr.m[2376, "Rule search"])

        gui.comboBox(
            widget=middle_box, master=self, value="evaluation_measure",
            label=_tr.m[2377, "Evaluation measure:"], orientation=Qt.Horizontal,
            items=(_tr.m[2378, "Entropy"], _tr.m[2379, "Laplace accuracy"], _tr.m[2380, "WRAcc"]),
            callback=self.settings_changed, contentsLength=3)

        gui.spin(
            widget=middle_box, master=self, value="beam_width", minv=1,
            maxv=100, step=1, label=_tr.m[2381, "Beam width:"], orientation=Qt.Horizontal,
            callback=self.settings_changed, alignment=Qt.AlignRight,
            controlWidth=80)

        # bottom-level search procedure (over-fitting avoidance bias)
        bottom_box = gui.vBox(widget=self.controlArea, box=_tr.m[2382, "Rule filtering"])

        gui.spin(
            widget=bottom_box, master=self, value="min_covered_examples", minv=1,
            maxv=10000, step=1, label=_tr.m[2383, "Minimum rule coverage:"],
            orientation=Qt.Horizontal, callback=self.settings_changed,
            alignment=Qt.AlignRight, controlWidth=80)

        gui.spin(
            widget=bottom_box, master=self, value="max_rule_length",
            minv=1, maxv=100, step=1, label=_tr.m[2384, "Maximum rule length:"],
            orientation=Qt.Horizontal, callback=self.settings_changed,
            alignment=Qt.AlignRight, controlWidth=80)

        gui.doubleSpin(
            widget=bottom_box, master=self, value="default_alpha", minv=0.0,
            maxv=1.0, step=0.01, label=_tr.m[2385, "Statistical significance (default α):"],
            orientation=Qt.Horizontal, callback=self.settings_changed,
            alignment=Qt.AlignRight, controlWidth=80,
        ...
        gui.doubleSpin(
            ...
            maxv=1.0, step=0.01, label=_tr.m[2386, "Relative significance (parent α):"],
            orientation=Qt.Horizontal, callback=self.settings_changed,
            alignment=Qt.AlignRight, controlWidth=80,
    ...
    def get_learner_parameters(self):
        return OrderedDict([
            (_tr.m[2387, "Rule ordering"], self.storage_orders[self.rule_ordering]),
            (_tr.m[2388, "Covering algorithm"], self.storage_covers[self.covering_algorithm]),
            (_tr.m[2389, "Gamma"], self.gamma),
            (_tr.m[2390, "Evaluation measure"], self.storage_measures[self.evaluation_measure]),
            (_tr.m[2391, "Beam width"], self.beam_width),
            (_tr.m[2392, "Minimum rule coverage"], self.min_covered_examples),
            (_tr.m[2393, "Maximum rule length"], self.max_rule_length),
            (_tr.m[2394, "Default alpha"], (1.0 if not self.checked_default_alpha
                               else self.default_alpha)),
            (_tr.m[2395, "Parent alpha"], (1.0 if not self.checked_parent_alpha
