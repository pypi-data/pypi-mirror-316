...
class OWDataTable(OWWidget):
    category = _tr.m[2518, "Orange Obsolete"]
    replaces = ["Orange.widgets.data.owtable.OWDataTable"]

    name = _tr.m[2519, "Data Table"]
    description = _tr.m[2520, "View the dataset in a spreadsheet."]
    icon = "../data/icons/Table.svg"
    priority = 50
    keywords = []

    class Inputs:
        data = MultiInput(_tr.m[2521, "Data"], Table, auto_summary=False, filter_none=True)

    class Outputs:
        selected_data = Output(_tr.m[2522, "Selected Data"], Table, default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table)

    class Warning(OWWidget.Warning):
        multiple_inputs = Msg(
            (_tr.m[2523, "Multiple Data inputs are deprecated.\n"] + (_tr.m[2524, "This functionality will be removed soon.\n"] + _tr.m[2525, "Use multiple Tables instead."])))
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[2526, "Variables"])
        self.c_show_attribute_labels = gui.checkBox(
            box, self, "show_attribute_labels",
            _tr.m[2527, "Show variable labels (if present)"],
            callback=self._on_show_variable_labels_changed)

        gui.checkBox(box, self, "show_distributions",
                     _tr.m[2528, 'Visualize numeric values'],
                     callback=self._on_distribution_color_changed)
        gui.checkBox(box, self, "color_by_class", _tr.m[2529, 'Color by instance classes'],
                     callback=self._on_distribution_color_changed)

        box = gui.vBox(self.controlArea, _tr.m[2530, "Selection"])

        gui.checkBox(box, self, "select_rows", _tr.m[2531, "Select full rows"],
                     callback=self._on_select_rows_changed)

        gui.rubber(self.controlArea)

        gui.button(self.buttonsArea, self, _tr.m[2532, "Restore Original Order"],
                   callback=self.restore_order,
                   tooltip=_tr.m[2533, "Show rows in the original order"],
                   autoDefault=False,
    ...
    def set_dataset(self, index: int, data: Table):
        ...
        datasetname = getattr(data, "name", _tr.m[2534, "Data"])
    ...
    def insert_dataset(self, index: int, data: Table):
        datasetname = getattr(data, "name", _tr.m[2535, "Data"])
