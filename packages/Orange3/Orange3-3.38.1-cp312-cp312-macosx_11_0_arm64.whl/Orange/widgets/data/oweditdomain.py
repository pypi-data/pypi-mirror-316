...
class DictItemsModel(QStandardItemModel):
    ...
    def __init__(self, parent=None, a_dict=None):
        ...
        self._dict = {}
        self.setHorizontalHeaderLabels([_tr.m[706, "Key"], _tr.m[707, "Value"]])
...
class VariableEditor(BaseEditor):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        form.addRow(_tr.m[708, "Name:"], self.name_edit)

        self.unlink_var_cb = QCheckBox(
            _tr.m[709, "Unlink variable from its source variable"], self,
            toolTip=(_tr.m[710, "Make Orange forget that the variable is derived from "] + (_tr.m[711, "another.\n"] + (_tr.m[712, "Use this for instance when you want to consider variables "] + (_tr.m[713, "with the same name but from different sources as the same "] + _tr.m[714, "variable."]))))
        )
        ...
        button = FixedSizeButton(
            ...
            accessibleName=_tr.m[717, "Add"],
        )
        hlayout.addWidget(button)

        button = FixedSizeButton(
            self, defaultAction=self.remove_label_action,
            accessibleName=_tr.m[718, "Remove"],
        )

        hlayout.addWidget(button)
        hlayout.addStretch(10)
        vlayout.addLayout(hlayout)
        form.addRow(_tr.m[719, "Labels:"], vlayout)
...
class GroupItemsDialog(QDialog):
    ...
    def __init__(
        ...
        # grouping strategy
        self.selected_radio = radio1 = QRadioButton(_tr.m[721, "Group selected values"])
        self.frequent_abs_radio = radio2 = QRadioButton(
            _tr.m[722, "Group values with less than"]
        )
        self.frequent_rel_radio = radio3 = QRadioButton(
            _tr.m[723, "Group values with less than"]
        )
        self.n_values_radio = radio4 = QRadioButton(
            _tr.m[724, "Group all except"]
        )
        ...
        label2 = QLabel(_tr.m[725, "occurrences"])
        label3 = QLabel(_tr.m[726, "occurrences"])
        label4 = QLabel(_tr.m[727, "most frequent values"])
        ...
        new_name_label = QLabel(_tr.m[728, "New value name: "])
...
class DiscreteVariableEditor(VariableEditor):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.move_value_up = QAction(
            _tr.m[732, "Move up"], group,
            iconText="\N{UPWARDS ARROW}",
            toolTip=_tr.m[733, "Move the selected item up."],
        ...
        self.move_value_down = QAction(
            _tr.m[734, "Move down"], group,
            iconText="\N{DOWNWARDS ARROW}",
            toolTip=_tr.m[735, "Move the selected item down."],
            shortcut=QKeySequence(Qt.ControlModifier | Qt.AltModifier |
                                  Qt.Key_BracketRight),
            shortcutContext=Qt.WidgetShortcut,
        )
        ...
        form.insertRow(2, _tr.m[751, "Values:"], vlayout)
    ...
    def _merge_categories(self) -> None:
        ...
        dlg = GroupItemsDialog(
            ...
            windowTitle=_tr.m[752, "Import Options"],
...
class TimeVariableEditor(VariableEditor):
    def __init__(self, parent=None, **kwargs):
        ...
        for item, data in [(_tr.m[753, "Detect automatically"], (None, 1, 1))] + list(
        ...
        form.insertRow(2, _tr.m[754, "Format:"], self.format_cb)
...
class VariableEditDelegate(QStyledItemDelegate):
    ReinterpretNames = {
        AsCategorical: _tr.m[755, "categorical"], AsContinuous: _tr.m[756, "numeric"],
        AsString: _tr.m[757, "string"], AsTime: _tr.m[758, "time"]
    }
        ...
        if var is not None:
            ...
            for tr in transform:
                if isinstance(tr, ReinterpretTransformTypes):
                    text += (_tr.e(_tr.c(759, f" (reinterpreted as ")) + f"{self.ReinterpretNames[type(tr)]})")
            option.text = text
        if transform:
    ...
    def helpEvent(self, event: QHelpEvent, view: QAbstractItemView,
        ...
        if isinstance(multiplicity, int) and multiplicity > 1 \
            ...
            QToolTip.showText(
                event.globalPos(), _tr.e(_tr.c(760, f"Name `{name}` is duplicated")),
                view.viewport()
            )
            return True
...
class ReinterpretVariableEditor(VariableEditor):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        def decorate(editor: BaseEditor) -> VariableEditor:
            ...
            typecb.addItem(variable_icon(Categorical), _tr.m[761, "Categorical"], Categorical)
            typecb.addItem(variable_icon(Real), _tr.m[762, "Numeric"], Real)
            typecb.addItem(variable_icon(String), _tr.m[763, "Text"], String)
            typecb.addItem(variable_icon(Time), _tr.m[764, "Time"], Time)
            if type(editor) is BaseEditor:  # pylint: disable=unidiomatic-typecheck
                typecb.addItem(_tr.m[765, "(Restore original)"], RestoreOriginal)
                typecb.addItem("")
            ...
            form.insertRow(1, _tr.m[766, "Type:"], typecb)
...
class OWEditDomain(widget.OWWidget):
    name = _tr.m[767, "Edit Domain"]
    description = _tr.m[768, "Rename variables, edit categories and variable annotations."]
    icon = "icons/EditDomain.svg"
    priority = 3125
    keywords = _tr.m[769, "edit domain, rename, drop, reorder, order"]

    class Inputs:
        data = Input(_tr.m[770, "Data"], Orange.data.Table)

    class Outputs:
        data = Output(_tr.m[771, "Data"], Orange.data.Table)

    class Error(widget.OWWidget.Error):
        duplicate_var_name = widget.Msg(_tr.m[772, "A variable name is duplicated."])

    class Warning(widget.OWWidget.Warning):
        transform_restore_failed = widget.Msg(
            _tr.m[773, "Failed to restore transform {} for column {}"]
        )
        cat_mapping_does_not_apply = widget.Msg(
            _tr.m[774, "Categories mapping for {} does not apply to current input"]
        )
    ...
    def __init__(self):
        ...
        box = gui.vBox(main, _tr.m[775, "Variables"])
        ...
        gui.button(
            bbox, self, _tr.m[778, "Reset All"],
            objectName="button-reset-all",
            toolTip=_tr.m[779, "Reset all variables to their input state."],
            autoDefault=False,
            callback=self.reset_all
        )
        gui.button(
            bbox, self, _tr.m[780, "Reset Selected"],
            objectName="button-reset",
            toolTip=_tr.m[781, "Rest selected variable to its input state."],
            autoDefault=False,
            callback=self.reset_selected
        )
        gui.button(
            bbox, self, _tr.m[782, "Apply"],
            objectName="button-apply",
            toolTip=_tr.m[783, "Apply changes and commit data on output."],
            default=True,
            autoDefault=False,
...
def report_transform(var, trs):
    ...
    if reinterpret is not None:
        header = (f"{var.name} → ({type_char(reinterpret)}) " + f"{rename.name if rename is not None else var.name}")
    elif rename is not None:
        header = f"{var.name} → {rename.name}"
    else:
        header = var.name
    if unlink is not None:
        header += _tr.m[785, "(unlinked from source)"]

    values_section = None
    if catmap is not None:
        values_section = (_tr.m[786, "Values"], [])
        lines = values_section[1]
        for ci, cj in catmap.mapping:
            if ci is None:
                item = cj + ("&nbsp;" * 3) + _tr.m[787, "(added)"]
            elif cj is None:
                item = strike(ci)
            else:
                item = ci + " → " + cj
    ...
    if annotate is not None:
        annotate_section = (_tr.m[788, "Labels"], [])
        ...
        for name in sorted(set(new) - set(old)):
            lines.append(
                i(name) + " : " + text(new[name]) + "&nbsp;" * 3 + i(_tr.m[789, "(new)"])
            )
