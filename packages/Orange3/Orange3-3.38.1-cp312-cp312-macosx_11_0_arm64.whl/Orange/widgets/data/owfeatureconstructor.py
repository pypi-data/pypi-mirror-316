...
""".lstrip()
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.nameedit = QLineEdit(
            placeholderText=_tr.m[790, "Name..."],
            sizePolicy=QSizePolicy(QSizePolicy.Minimum,
                                   QSizePolicy.Fixed)
        )

        self.metaattributecb = QCheckBox(_tr.m[791, "Meta attribute"])

        self.expressionedit = QLineEdit(
            placeholderText=_tr.m[792, "Expression..."],
            toolTip=self.ExpressionTooltip)

        self.attrs_model = itemmodels.VariableListModel(
            [_tr.m[793, "Select Feature"]], parent=self)
        ...
        self.funcs_model = itemmodels.PyListModelTooltip(
            chain([_tr.m[794, "Select Function"]], sorted_funcs),
    ...
    def setEditorData(self, data, domain):
        ...
        self.attrs_model[:] = [_tr.m[795, "Select Feature"]]
...
class ContinuousFeatureEditor(FeatureEditor):
    ExpressionTooltip = _tr.m[796, "A numeric expression\n\n"] \
...
class DateTimeFeatureEditor(FeatureEditor):
    ExpressionTooltip = FeatureEditor.ExpressionTooltip + \
        (_tr.m[797, "Result must be a string in ISO-8601 format "] + (_tr.m[798, "(e.g. 2019-07-30T15:37:27 or a part thereof),\n"] + _tr.m[799, "or a number of seconds since Jan 1, 1970."]))
...
class DiscreteFeatureEditor(FeatureEditor):
    ExpressionTooltip = FeatureEditor.ExpressionTooltip + \
        (_tr.m[800, "Result must be a string, if values are not explicitly given\n"] + _tr.m[801, "or a zero-based integer indices into a list of values given below."])

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        tooltip = \
            (_tr.m[802, "If values are given, above expression must return zero-based "] + _tr.m[803, "integer indices into that list."])
        self.valuesedit = QLineEdit(placeholderText="A, B ...", toolTip=tooltip)
        self.valuesedit.textChanged.connect(self._invalidate)

        layout = self.layout()
        label = QLabel(self.tr(_tr.m[804, "Values (optional)"]))
...
class StringFeatureEditor(FeatureEditor):
    ExpressionTooltip = _tr.m[805, "A string expression\n\n"] \
                        + FeatureEditor.ExpressionTooltip
...
class OWFeatureConstructor(OWWidget, ConcurrentWidgetMixin):
    name = "Formula"
    description = (_tr.m[806, "Construct new features (data columns) from a set of "] + _tr.m[807, "existing features in the input dataset."])
    category = _tr.m[808, "Transform"]
    icon = "icons/FeatureConstructor.svg"
    keywords = _tr.m[809, "feature constructor, function, lambda, calculation"]
    priority = 2240

    class Inputs:
        data = Input(_tr.m[810, "Data"], Orange.data.Table)

    class Outputs:
        data = Output(_tr.m[811, "Data"], Orange.data.Table)
    ...
    class Error(OWWidget.Error):
        more_values_needed = Msg(_tr.m[812, "Categorical feature {} needs more values."])
        invalid_expressions = Msg(_tr.m[813, "Invalid expressions: {}."])
        transform_error = Msg("{}")

    class Information(OWWidget.Information):
        replaces_existing = Msg(
            (_tr.m[814, "A new variable that is named the same as an existing one will "] + _tr.m[815, "replace it on the output."])
        )
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[816, "Variable Definitions"])
        ...
        self.addbutton = QPushButton(
            _tr.m[817, "New"], toolTip=_tr.m[818, "Create a new variable"],
            minimumWidth=120,
        ...
        menu.addSeparator()
        self.duplicateaction = menu.addAction(_tr.m[823, "Duplicate Selected Variable"])
        self.duplicateaction.triggered.connect(self.duplicateFeature)
        self.duplicateaction.setEnabled(False)
        self.addbutton.setMenu(menu)

        self.removebutton = QPushButton(
            _tr.m[824, "Remove"], toolTip=_tr.m[825, "Remove selected variable"],
            minimumWidth=120,
        ...
        self.fix_button = gui.button(
            self.buttonsArea, self, _tr.m[826, "Upgrade Expressions"],
            callback=self.fix_expressions)
        self.fix_button.setHidden(True)
        gui.button(self.buttonsArea, self, _tr.m[827, "Send"], callback=self.apply, default=True)
    ...
    def send_report(self):
        ...
        for feature in self.featuremodel:
            if isinstance(feature, DiscreteDescriptor):
                desc = _tr.m[828, "categorical"]
                if feature.values:
                    desc += _tr.m[829, " with values "] \
                            + ", ".join(f"'{val}'" for val in feature.values)
                if feature.ordered:
                    desc += _tr.m[830, "; ordered"]
            elif isinstance(feature, ContinuousDescriptor):
                desc = _tr.m[831, "numeric"]
            elif isinstance(feature, DateTimeDescriptor):
                desc = _tr.m[832, "date/time"]
            else:
                desc = _tr.m[833, "text"]
            items[feature.name] = f"{feature.expression} ({desc})"
        self.report_items(
            report.plural(_tr.m[834, "Constructed feature{s}"], len(items)), items)

    def fix_expressions(self):
        dlg = QMessageBox(
            QMessageBox.Question,
            _tr.m[835, "Fix Expressions"],
            ("This widget's behaviour has changed. Values of categorical " + ("variables are now inserted as their textual representations " + ("(strings); previously they appeared as integer numbers, with an " + ("attribute '.value' that contained the text.\n\n" + ("The widget currently runs in compatibility mode. After " + "expressions are updated, manually check for their correctness."))))))
