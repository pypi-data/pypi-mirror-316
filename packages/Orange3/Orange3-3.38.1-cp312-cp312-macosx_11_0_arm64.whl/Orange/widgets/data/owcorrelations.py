...
class CorrelationType(IntEnum):
    ...
    def items():
        ...
        """
        return [_tr.m[391, "Pearson correlation"], _tr.m[392, "Spearman correlation"]]
...
class OWCorrelations(OWWidget):
    name = _tr.m[393, "Correlations"]
    description = _tr.m[394, "Compute all pairwise attribute correlations."]
    icon = "icons/Correlations.svg"
    priority = 1106
    category = _tr.m[395, "Unsupervised"]

    class Inputs:
        data = Input(_tr.m[396, "Data"], Table)

    class Outputs:
        data = Output(_tr.m[397, "Data"], Table)
        features = Output(_tr.m[398, "Features"], AttributeList)
        correlations = Output(_tr.m[399, "Correlations"], Table)
    ...
    class Information(OWWidget.Information):
        removed_cons_feat = Msg(_tr.m[400, "Constant features have been removed."])

    class Warning(OWWidget.Warning):
        not_enough_vars = Msg(_tr.m[401, "At least two numeric features are needed."])
        not_enough_inst = Msg(_tr.m[402, "At least two instances are needed."])
    ...
    def __init__(self):
        ...
        self.feature_model = DomainModel(
            ...
            placeholder=_tr.m[403, "(All combinations)"], valid_types=ContinuousVariable)
        gui.comboBox(
    ...
    def commit(self):
        ...
        attrs = [ContinuousVariable(_tr.m[404, "Correlation"]),
                 ContinuousVariable(_tr.m[405, "uncorrected p"]),
                 ContinuousVariable(_tr.m[406, "FDR"])]
        metas = [StringVariable(_tr.m[407, "Feature 1"]), StringVariable(_tr.m[408, "Feature 2"])]
        ...
        corr_table.name = _tr.m[409, "Correlations"]
