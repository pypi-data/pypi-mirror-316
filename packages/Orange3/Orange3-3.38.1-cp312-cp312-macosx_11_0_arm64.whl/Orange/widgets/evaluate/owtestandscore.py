...
class OWTestAndScore(OWWidget):
    name = _tr.m[2003, "Test and Score"]
    description = _tr.m[2004, "Cross-validation accuracy estimation."]
    icon = "icons/TestLearners1.svg"
    priority = 100
    keywords = _tr.m[2005, "test and score, cross validation, cv"]
    replaces = ["Orange.widgets.evaluate.owtestlearners.OWTestLearners"]

    class Inputs:
        train_data = Input(_tr.m[2006, "Data"], Table, default=True)
        test_data = Input(_tr.m[2007, "Test Data"], Table)
        learner = MultiInput(
            _tr.m[2008, "Learner"], Learner, filter_none=True
        )
        preprocessor = Input(_tr.m[2009, "Preprocessor"], Preprocess)

    class Outputs:
        predictions = Output(_tr.m[2010, "Predictions"], Table)
        evaluations_results = Output(_tr.m[2011, "Evaluation Results"], Results)
    ...
    TARGET_AVERAGE = _tr.m[2012, "(None, show average over classes)"]
    class_selection = settings.ContextSetting(TARGET_AVERAGE)

    class Error(OWWidget.Error):
        test_data_empty = Msg(_tr.m[2013, "Test dataset is empty."])
        class_required_test = Msg(_tr.m[2014, "Test data input requires a target variable."])
        too_many_folds = Msg(_tr.m[2015, "Number of folds exceeds the data size"])
        class_inconsistent = Msg((_tr.m[2016, "Test and train datasets "] + _tr.m[2017, "have different target variables."]))
        memory_error = Msg(_tr.m[2018, "Not enough memory."])
        test_data_incompatible = Msg(
            _tr.m[2019, "Test data may be incompatible with train data."])
        train_data_error = Msg("{}")

    class Warning(OWWidget.Warning):
        missing_data = \
            Msg(_tr.m[2020, "Instances with unknown target values were removed from{}data."])
        test_data_missing = Msg(_tr.m[2021, "Missing separate test data input."])
        scores_not_computed = Msg(_tr.m[2022, "Some scores could not be computed."])
        test_data_unused = Msg((_tr.m[2023, "Test data is present but unused. "] + _tr.m[2024, "Select 'Test on test data' to use it."]))
        cant_stratify = \
            Msg((_tr.m[2025, "Can't run stratified {}-fold cross validation; "] + _tr.m[2026, "the least common class has only {} instances."]))

    class Information(OWWidget.Information):
        data_sampled = Msg(_tr.m[2027, "Train data has been sampled"])
        test_data_sampled = Msg(_tr.m[2028, "Test data has been sampled"])
        test_data_transformed = Msg(
            _tr.m[2029, "Test data has been transformed to match the train data."])
        cant_stratify_numeric = Msg(_tr.m[2030, "Stratification is ignored for regression"])
        cant_stratify_multitarget = Msg((_tr.m[2031, "Stratification is ignored when there are"] + _tr.m[2032, " multiple target variables."]))
    ...
    def __init__(self):
        ...
        gui.appendRadioButton(rbox, _tr.m[2033, "Cross validation"])
        ibox = gui.indentedBox(rbox)
        gui.comboBox(
            ibox, self, "n_folds", label=_tr.m[2034, "Number of folds: "],
            items=[str(x) for x in self.NFolds],
            orientation=Qt.Horizontal, callback=self.kfold_changed)
        gui.checkBox(
            ibox, self, "cv_stratified", _tr.m[2035, "Stratified"],
            callback=self.kfold_changed)
        gui.appendRadioButton(rbox, _tr.m[2036, "Cross validation by feature"])
        ibox = gui.indentedBox(rbox)
        ...
        gui.appendRadioButton(rbox, _tr.m[2037, "Random sampling"])
        ibox = gui.indentedBox(rbox)
        gui.comboBox(
            ibox, self, "n_repeats", label=_tr.m[2038, "Repeat train/test: "],
            items=[str(x) for x in self.NRepeats], orientation=Qt.Horizontal,
            callback=self.shuffle_split_changed
        )
        gui.comboBox(
            ibox, self, "sample_size", label=_tr.m[2039, "Training set size: "],
            items=["{} %".format(x) for x in self.SampleSizes],
            orientation=Qt.Horizontal, callback=self.shuffle_split_changed
        )
        gui.checkBox(
            ibox, self, "shuffle_stratified", _tr.m[2040, "Stratified"],
            callback=self.shuffle_split_changed)

        gui.appendRadioButton(rbox, _tr.m[2041, "Leave one out"])

        gui.appendRadioButton(rbox, _tr.m[2042, "Test on train data"])
        gui.appendRadioButton(rbox, _tr.m[2043, "Test on test data"])
        ...
        self.class_selection_combo = gui.comboBox(
            ...
            label=_tr.m[2044, "Evaluation results for target"], orientation=Qt.Horizontal,
        ...
        cbox = gui.comboBox(
            box, self, "comparison_criterion", label=_tr.m[2045, "Compare models by:"],
            sizeAdjustPolicy=QComboBox.AdjustToMinimumContentsLengthWithIcon,
            sizePolicy=(QSizePolicy.MinimumExpanding, QSizePolicy.Fixed),
            orientation=Qt.Horizontal, callback=self.update_comparison_table).box

        gui.separator(cbox, 8)
        gui.checkBox(cbox, self, "use_rope", _tr.m[2046, "Negligible diff.: "],
        ...
        box.layout().addWidget(QLabel(
            (_tr.m[2047, "<small>Table shows probabilities that the score for the model in "] + (_tr.m[2048, "the row is higher than that of the model in the column. "] + (_tr.m[2049, "Small numbers show the probability that the difference is "] + _tr.m[2050, "negligible.</small>"]))), wordWrap=True))
    ...
    def set_train_data(self, data):
        ...
        if data is not None:
            data_errors = [
                (_tr.m[2051, "Train dataset is empty."], len(data) == 0),
                (
                    _tr.m[2052, "Train data input requires a target variable."],
                    not data.domain.class_vars
                ),
                (_tr.m[2053, "Target variable has no values."], np.isnan(data.Y).all()),
                (
                    _tr.m[2054, "Target variable has only one value."],
                    data.domain.has_discrete_class and len(unique(data.Y)) < 2
                ),
                (_tr.m[2055, "Data has no features to learn from."], data.X.shape[1] == 0),
            ]
    ...
    def _which_missing_data(self):
        return {(True, True): " ",  # both, don't specify
                (True, False): _tr.m[2056, " train "],
                (False, True): _tr.m[2057, " test "]}[(self.train_data_missing_vals,
                                          self.test_data_missing_vals)]
    ...
    def update_stats_model(self):
        ...
        for key, slot in self.learners.items():
            ...
            if isinstance(results, Try.Fail):
                ...
                head.setText(_tr.m[2058, "{} (error)"].format(name))
                ...
                if isinstance(results.exception, DomainTransformationError) \
                    ...
                    errors.append((_tr.m[2059, "{name} failed with error:\n"] + "{exc.__class__.__name__}: {exc!s}")
    ...
    def send_report(self):
        ...
        if self.resampling == self.KFold:
            stratified = _tr.m[2060, 'Stratified '] if self.cv_stratified else ''
            items = [(_tr.m[2061, "Sampling type"], _tr.m[2062, "{}{}-fold Cross validation"].
                      format(stratified, self.NFolds[self.n_folds]))]
        elif self.resampling == self.LeaveOneOut:
            items = [(_tr.m[2063, "Sampling type"], _tr.m[2064, "Leave one out"])]
        elif self.resampling == self.ShuffleSplit:
            stratified = _tr.m[2065, 'Stratified '] if self.shuffle_stratified else ''
            items = [(_tr.m[2066, "Sampling type"],
                      _tr.m[2067, "{}Shuffle split, {} random samples with {}% data "]
                      .format(stratified, self.NRepeats[self.n_repeats],
                              self.SampleSizes[self.sample_size]))]
        elif self.resampling == self.TestOnTrain:
            items = [(_tr.m[2068, "Sampling type"], _tr.m[2069, "No sampling, test on training data"])]
        elif self.resampling == self.TestOnTest:
            items = [(_tr.m[2070, "Sampling type"], _tr.m[2071, "No sampling, test on testing data"])]
        else:
            items = []
        if self.data.domain.has_discrete_class:
            items += [(_tr.m[2072, "Target class"], self.class_selection.strip("()"))]
        if items:
            self.report_items(_tr.m[2073, "Settings"], items)
        self.report_table(_tr.m[2074, "Scores"], self.score_table.view)
    ...
    def __submit(self, testfunc):
        ...
        self.setStatusMessage(_tr.m[2075, "Running"])
