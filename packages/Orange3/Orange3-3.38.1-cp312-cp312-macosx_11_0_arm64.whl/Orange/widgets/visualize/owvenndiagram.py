...
IDENTITY_STR = _tr.m[3907, "Instance identity"]
EQUALITY_STR = _tr.m[3908, "Instance equality"]
...
class OWVennDiagram(widget.OWWidget):
    name = _tr.m[3909, "Venn Diagram"]
    description = (_tr.m[3910, "A graphical visualization of the overlap of data instances "] + _tr.m[3911, "from a collection of input datasets."])
    icon = "icons/VennDiagram.svg"
    priority = 280
    keywords = _tr.m[3912, "venn diagram"]
    settings_version = 2

    class Inputs:
        data = MultiInput(_tr.m[3913, "Data"], Table)

    class Outputs:
        selected_data = Output(_tr.m[3914, "Selected Data"], Table, default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table)

    class Error(widget.OWWidget.Error):
        instances_mismatch = Msg(_tr.m[3915, "Data sets do not contain the same instances."])
        too_many_inputs = Msg(_tr.m[3916, "Venn diagram accepts at most five datasets."])

    class Warning(widget.OWWidget.Warning):
        renamed_vars = Msg((_tr.m[3917, "Some variables have been renamed "] + _tr.m[3918, "to avoid duplicates.\n{}"]))
    ...
    def __init__(self):
        ...
        box = gui.radioButtonsInBox(
            ...
            [_tr.m[3919, "Columns (features)"], _tr.m[3920, "Rows (instances), matched by"], ],
        ...
        gui.comboBox(
            ...
            tooltip=(_tr.m[3921, "Instances are identical if originally coming from the "] + (_tr.m[3922, "same row of the same table.\n"] + (_tr.m[3923, "Instances can be check for equality only if described by "] + _tr.m[3924, "the same variables."]))))
        ...
        self.output_duplicates_cb = gui.checkBox(
            box, self, "output_duplicates", _tr.m[3925, "Output duplicates"],
        ...
        for i, area in enumerate(self.vennwidget.vennareas()):
            ...
            if self._uses_feature() or not self.rowwise:
                ...
                if len(area_items) > 32:
                    tooltip += _tr.e(_tr.c(3927, f"</br>({len(area_items) - 32} items not shown)"))
                tooltip += "</span>"
