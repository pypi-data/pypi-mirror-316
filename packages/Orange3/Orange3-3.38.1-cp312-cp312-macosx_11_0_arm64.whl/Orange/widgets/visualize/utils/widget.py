...
class OWProjectionWidgetBase(OWWidget, openclass=True):
    ...
    class Information(OWWidget.Information):
        missing_size = Msg(
            _tr.m[4022, "Points with undefined '{}' are shown in smaller size"])
        missing_shape = Msg(
            _tr.m[4023, "Points with undefined '{}' are shown as crossed circles"])
    ...
    def get_column(self, attr, filter_valid=True,
        ...
        if return_labels:
            return [value for value, infreq in zip(attr.values, infrequent)
                    if not infreq] + [_tr.m[4024, "Other"]]
        else:
    ...
    def _point_tooltip(self, point_id, skip_attrs=()):
        def show_part(_point_data, name, max_shown, _vars):
            ...
            if n_vars > max_shown:
                ...
                cols[-1] = _tr.e(_tr.c(4025, f"... and {over} {pl(over, 'other')}"))
            return f"<b>{name}</b>:<br/>" + "<br/>".join(cols)

        dom = self.data.domain
        parts = (
            (_tr.e(_tr.c(4026, f"{pl(len(dom.class_vars), 'Class|Classes')}")), 4, dom.class_vars),
            (_tr.e(_tr.c(4027, f"{pl(len(dom.metas), 'Meta')}")), 4, dom.metas),
            (_tr.e(_tr.c(4028, f"{pl(len(dom.attributes), 'Feature')}")), 10, dom.attributes))
    ...
    def get_tooltip(self, point_ids):
        ...
        if len(point_ids) > MAX_POINTS_IN_TOOLTIP:
            text = _tr.e(_tr.c(4029, f"{len(point_ids)} instances<hr/>{text}<hr/>..."))
        return text
...
class OWDataProjectionWidget(OWProjectionWidgetBase, openclass=True):
    ...
    """
    class Inputs:
        data = Input(_tr.m[4030, "Data"], Table, default=True)
        data_subset = Input(_tr.m[4031, "Data Subset"], Table)

    class Outputs:
        selected_data = Output(_tr.m[4032, "Selected Data"], Table, default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table)

    class Warning(OWProjectionWidgetBase.Warning):
        too_many_labels = Msg(
            _tr.m[4033, "Too many labels to show (zoom in or label only selected)"])
        subset_not_subset = Msg(
            (_tr.m[4034, "Subset data contains some instances that do not appear in "] + _tr.m[4035, "input data"]))
        subset_independent = Msg(
            _tr.m[4036, "No subset data instances appear in input data"])
        transparent_subset = Msg(
            _tr.m[4037, "Increase opacity if subset is difficult to see"])
    ...
    def _get_selected_data(data, selection, group_sel):
        return create_groups_table(data, group_sel, False, _tr.m[4038, "Group"]) \
    ...
    def _get_send_report_caption(self):
        return report.render_items_vert((
            (_tr.m[4039, "Color"], self._get_caption_var_name(self.attr_color)),
            (_tr.m[4040, "Label"], self._get_caption_var_name(self.attr_label)),
            (_tr.m[4041, "Shape"], self._get_caption_var_name(self.attr_shape)),
            (_tr.m[4042, "Size"], self._get_caption_var_name(self.attr_size)),
            (_tr.m[4043, "Jittering"], self.graph.jitter_size != 0 and
...
class OWAnchorProjectionWidget(OWDataProjectionWidget, openclass=True):
    ...
    class Outputs(OWDataProjectionWidget.Outputs):
        components = Output(_tr.m[4044, "Components"], Table)

    class Error(OWDataProjectionWidget.Error):
        sparse_data = Msg(_tr.m[4045, "Sparse data is not supported"])
        no_valid_data = Msg(_tr.m[4046, "No projection due to no valid data"])
        no_instances = Msg(_tr.m[4047, "At least two data instances are required"])
        proj_error = Msg(_tr.m[4048, "An error occurred while projecting data.\n{}"])
    ...
    def send_components(self):
        ...
        if self.data is not None and self.projection is not None:
            ...
            components.name = _tr.m[4049, "components"]
