...
class OWPaintData(OWWidget):
    TOOLS = [
        (_tr.m[1128, "Brush"], _tr.m[1129, "Create multiple instances"], AirBrushTool, _icon("brush.svg")),
        (_tr.m[1130, "Put"], _tr.m[1131, "Put individual instances"], PutInstanceTool, _icon("put.svg")),
        (_tr.m[1132, "Select"], _tr.m[1133, "Select and move instances"], SelectTool,
         _icon("select-transparent_42px.png")),
        (_tr.m[1134, "Jitter"], _tr.m[1135, "Jitter instances"], JitterTool, _icon("jitter.svg")),
        (_tr.m[1136, "Magnet"], _tr.m[1137, "Attract multiple instances"], MagnetTool, _icon("magnet.svg")),
        (_tr.m[1138, "Clear"], _tr.m[1139, "Clear the plot"], ClearTool, _icon("../../../icons/Dlg_clear.png"))
    ]

    name = _tr.m[1140, "Paint Data"]
    description = _tr.m[1141, "Create data by painting data points on a plane."]
    icon = "icons/PaintData.svg"
    priority = 60
    keywords = _tr.m[1142, "paint data, create, draw"]

    class Inputs:
        data = Input(_tr.m[1143, "Data"], Table)

    class Outputs:
        data = Output(_tr.m[1144, "Data"], Table)

    autocommit = Setting(True)
    table_name = Setting(_tr.m[1145, "Painted data"])
    attr1 = Setting("x")
    attr2 = Setting("y")
    ...
    class Warning(OWWidget.Warning):
        no_input_variables = Msg(_tr.m[1146, "Input data has no variables"])
        continuous_target = Msg(_tr.m[1147, "Numeric target value can not be used."])
        sparse_not_supported = Msg(_tr.m[1148, "Sparse data is ignored."])
        renamed_vars = Msg((_tr.m[1149, "Some variables have been renamed "] + _tr.m[1150, "to avoid duplicates.\n{}"]))

    class Information(OWWidget.Information):
        use_first_two = \
            Msg(_tr.m[1151, "Paint Data uses data from the first two attributes."])
    ...
    def _init_ui(self):
        namesBox = gui.vBox(self.controlArea, _tr.m[1152, "Names"])

        hbox = gui.hBox(namesBox, margin=0, spacing=0)
        gui.lineEdit(hbox, self, "attr1", _tr.m[1153, "Variable X: "],
                     controlWidth=80, orientation=Qt.Horizontal,
                     callback=self._attr_name_changed)
        gui.separator(hbox, 21)
        hbox = gui.hBox(namesBox, margin=0, spacing=0)
        attr2 = gui.lineEdit(hbox, self, "attr2", _tr.m[1154, "Variable Y: "],
                             controlWidth=80, orientation=Qt.Horizontal,
                             callback=self._attr_name_changed)
        gui.separator(hbox)
        ...
        gui.widgetLabel(namesBox, _tr.m[1155, "Labels"])
        ...
        tBox = gui.vBox(self.buttonsArea, _tr.m[1158, "Tools"])
        ...
        form.addRow(_tr.m[1159, "Radius:"], slider)
        ...
        form.addRow(_tr.m[1160, "Intensity:"], slider)
        ...
        form.addRow(_tr.m[1161, "Symbol:"], slider)

        self.btResetToInput = gui.button(
            tBox, self, _tr.m[1162, "Reset to Input Data"], self.reset_to_input)
    ...
    def remove_selected_class_label(self):
        ...
        self.undo_stack.beginMacro(_tr.m[1163, "Delete class label"])
...
#             self.undo_stack.push(command)
    ...
    def _add_command(self, cmd):
        ...
        if isinstance(cmd, Append):
            ...
            if indices is not None and indices.size:
                self.undo_stack.push(
                    UndoCommand(DeleteIndices(indices), self, text=_tr.m[1165, "Delete"])
                )
            ...
            if indices is not None and indices.size:
                self.undo_stack.push(
                    UndoCommand(
                        ...
                        self, text=_tr.m[1166, "Move"])
                )
        elif isinstance(cmd, DeleteIndices):
            self.undo_stack.push(UndoCommand(cmd, self, text=_tr.m[1167, "Delete"]))
    ...
    def commit(self):
        ...
        if len(np.unique(Y)) >= 2:
            proposed.append(_tr.m[1168, "Class"])
    ...
    def send_report(self):
        ...
        if self.attr1 != "x" or self.attr2 != "y":
            settings += [(_tr.m[1169, "Axis x"], self.attr1), (_tr.m[1170, "Axis y"], self.attr2)]
        settings += [(_tr.m[1171, "Number of points"], len(self.data))]
        self.report_items(_tr.m[1172, "Painted data"], settings)
        self.report_plot()
