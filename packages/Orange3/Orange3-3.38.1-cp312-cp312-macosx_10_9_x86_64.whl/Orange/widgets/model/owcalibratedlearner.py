...
class OWCalibratedLearner(OWBaseLearner):
    name = _tr.m[2108, "Calibrated Learner"]
    description = (_tr.m[2109, "Wraps another learner with probability calibration and "] + _tr.m[2110, "decision threshold optimization"])
    icon = "icons/CalibratedLearner.svg"
    priority = 20
    keywords = _tr.m[2111, "calibrated learner, calibration, threshold"]

    LEARNER = CalibratedLearner

    SigmoidCalibration, IsotonicCalibration, NoCalibration = range(3)
    CalibrationOptions = (_tr.m[2112, "Sigmoid calibration"],
                          _tr.m[2113, "Isotonic calibration"],
                          _tr.m[2114, "No calibration"])
    CalibrationShort = (_tr.m[2115, "Sigmoid"], _tr.m[2116, "Isotonic"], "")
    CalibrationMap = {
    ...
    ThresholdOptions = (_tr.m[2117, "Optimize classification accuracy"],
                        _tr.m[2118, "Optimize F1 score"],
                        _tr.m[2119, "No threshold optimization"])
    ThresholdShort = (_tr.m[2120, "CA"], _tr.m[2121, "F1"], "")
    ThresholdMap = {
    ...
    class Inputs(OWBaseLearner.Inputs):
        base_learner = Input(_tr.m[2122, "Base Learner"], Learner)
    ...
    def get_learner_parameters(self):
        return ((_tr.m[2125, "Calibrate probabilities"],
                 self.CalibrationOptions[self.calibration]),
                (_tr.m[2126, "Threshold optimization"],
