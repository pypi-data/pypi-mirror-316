...
class BinSql:
    ...
    def __call__(self):
        return (f'width_bucket({self.var.to_sql()}, ' + f'ARRAY{str(self.points)}::double precision[])')
...
def _time_binnings(mn, mx, min_pts, max_pts):
    ...
    for place, step, fmt, unit in (
            [(5, x, "%H:%M:%S", _tr.m[139, "second"]) for x in (1, 5, 10, 15, 30)] +
            [(4, x, "%b %d %H:%M", _tr.m[140, "minute"]) for x in (1, 5, 10, 15, 30)] +
            [(3, x, "%y %b %d %H:%M", _tr.m[141, "hour"]) for x in (1, 2, 3, 6, 12)] +
            [(2, 1, "%y %b %d", _tr.m[142, "day"])] +
            [(2, x, "%y %b %d", _tr.m[143, "week"]) for x in (7, 14)] +
            [(1, x, "%y %b", _tr.m[144, "month"]) for x in (1, 2, 3, 6)] +
            [(0, x, "%Y", _tr.m[145, "year"]) for x in (1, 2, 5, 10, 25, 50, 100)]):
        ...
        if place == 2 and step >= 7:
            unit_label = _tr.e(_tr.c(146, f"{step // 7} week{'s' * (step > 7)}"))
        else:
            unit_label = _tr.e(_tr.c(147, f"{step} {unit}{'s' * (step > 1)}"))
...
short_time_units = dict(seconds=_tr.m[148, "sec"], minutes=_tr.m[149, "min"], hours=_tr.m[150, "hrs"],
                        weeks=_tr.m[151, "wks"], months=_tr.m[152, "mon"], years=_tr.m[153, "yrs"],
                        second=_tr.m[154, "sec"], minute=_tr.m[155, "min"], month=_tr.m[156, "mon"])
