...
def permutation(
    ...
    scores_cv = [score_cv]
    correlations = [100.0]
    progress_callback(0, _tr.m[1890, "Calculating..."])
    np.random.seed(0)
...
class ParameterSetter(CommonParameterSetter):
    GRID_LABEL, SHOW_GRID_LABEL = _tr.m[1891, "Gridlines"], _tr.m[1892, "Show"]
...
class PermutationPlot(PlotWidget):
    def __init__(self):
        ...
        text = _tr.m[1893, "Correlation between original Y and permuted Y (%)"]
    ...
    def set_data(
        ...
        self.legend.clear()
        self.legend.addItem(pg.ScatterPlotItem(**kwargs_tr), _tr.m[1894, "Train"])
        self.legend.addItem(pg.ScatterPlotItem(**kwargs_cv), _tr.m[1895, "CV"])
        self.legend.show()


class OWPermutationPlot(OWWidget, ConcurrentWidgetMixin):
    name = _tr.m[1896, "Permutation Plot"]
    description = _tr.m[1897, "Permutation analysis plotting"]
    ...
    class Inputs:
        data = Input(_tr.m[1898, "Data"], Table)
        learner = Input(_tr.m[1899, "Learner"], Learner)

    class Error(OWWidget.Error):
        domain_transform_err = Msg("{}")
        unknown_err = Msg("{}")
        not_enough_data = Msg(_tr.e(_tr.c(1900, f"At least {N_FOLD} instances are needed.")))
    ...
    def _add_controls(self):
        box = gui.vBox(self.controlArea, _tr.m[1901, "Settings"])
        gui.spin(box, self, "n_permutations", label=_tr.m[1902, "Permutations:"],
                 minv=1, maxv=1000, callback=self._run)
        gui.rubber(self.controlArea)

        box = gui.vBox(self.controlArea, _tr.m[1903, "Info"])
        self._info = gui.label(box, self, "", textFormat=Qt.RichText,
                               minimumWidth=180)
        self.__set_info(None)

    def __set_info(self, result: PermutationResults):
        html = _tr.m[1904, "No data available."]
...
<table width=100% align="center" style="font-size:11px">
    ...
    def send_report(self):
        if self._data is None or self._learner is None:
            return
        self.report_items(_tr.m[1906, "Settings"], [(_tr.m[1907, "Permutations"], self.n_permutations)])
        self.report_raw(_tr.m[1908, "Info"], self._info.text())
        self.report_name(_tr.m[1909, "Plot"])
        self.report_plot()
