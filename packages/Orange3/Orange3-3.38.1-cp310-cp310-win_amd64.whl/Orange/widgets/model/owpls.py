...
class OWPLS(OWBaseLearner):
    name = 'PLS'
    description = _tr.m[2292, "Partial Least Squares Regression widget for multivariate data analysis"]
    icon = "icons/PLS.svg"
    priority = 85
    keywords = [_tr.m[2293, "partial least squares"]]

    LEARNER = PLSRegressionLearner

    class Outputs(OWBaseLearner.Outputs):
        coefsdata = Output(_tr.m[2294, "Coefficients and Loadings"], Table, explicit=True)
        data = Output(_tr.m[2295, "Data with Scores"], Table)
        components = Output(_tr.m[2296, "Components"], Table)

    class Warning(OWBaseLearner.Warning):
        sparse_data = Msg(
            _tr.m[2297, 'Sparse input data: default preprocessing is to scale it.'])
    ...
    def _create_output_coeffs_loadings(self) -> Table:
        ...
        names = [_tr.e(_tr.c(2302, f"coef ({v.name})")) for v in self.model.domain.class_vars]
        names += [_tr.e(_tr.c(2303, f"coef * X_sd ({v.name})")) for v in self.model.domain.class_vars]
        names += [f"w*c {i + 1}" for i in range(n_components)]
        domain = Domain(
            [ContinuousVariable(n) for n in names],
            metas=[StringVariable(_tr.m[2304, "Variable name"]),
                   DiscreteVariable(_tr.m[2305, "Variable role"], (_tr.m[2306, "Feature"], _tr.m[2307, "Target"]))]
        )
        ...
        M = np.array([[v.name for v in variables] + [_tr.m[2308, "intercept"]],
                      [0] * n_features + [1] * n_targets + [np.nan]],
                     dtype=object).T

        table = Table.from_numpy(domain, X=X, metas=M)
        table.name = _tr.m[2309, "Coefficients and Loadings"]
        return table
    ...
    def set_data(self, data):
        ...
        if data is not None and data.domain.class_var is None and not data.domain.class_vars:
            self.Error.data_error(
                (_tr.m[2310, "Data has no target variable.\n"] + _tr.m[2311, "Select one with the Select Columns widget."]))
            self.data = None
