...
class OWTreeLearner(OWBaseLearner):
    ...
    name = _tr.m[2496, "Tree"]
    description = _tr.m[2497, "A tree algorithm with forward pruning."]
    icon = "icons/Tree.svg"
    replaces = [
    ...
    priority = 30
    keywords = _tr.m[2498, "tree, classification tree"]
    ...
    spin_boxes = (
        (_tr.m[2499, "Min. number of instances in leaves: "],
         "limit_min_leaf", "min_leaf", 1, 1000),
        (_tr.m[2500, "Do not split subsets smaller than: "],
         "limit_min_internal", "min_internal", 1, 1000),
        (_tr.m[2501, "Limit the maximal tree depth to: "],
         "limit_depth", "max_depth", 1, 1000))

    classification_spin_boxes = (
        (_tr.m[2502, "Stop when majority reaches [%]: "],
         "limit_majority", "sufficient_majority", 51, 100),)

    def add_main_layout(self):
        box = gui.widgetBox(self.controlArea, _tr.m[2503, 'Parameters'])
        # the checkbox is put into vBox for alignemnt with other checkboxes
        gui.checkBox(box, self, "binary_trees", _tr.m[2504, "Induce binary tree"],
    ...
    def get_learner_parameters(self):
        ...
        items = OrderedDict()
        items[_tr.m[2505, "Pruning"]] = ", ".join(s for s, c in (
            ((_tr.e(_tr.c(2506, f'at least {self.min_leaf} ')) + _tr.e(_tr.c(2507, f'{pl(self.min_leaf, "instance")} in leaves'))),
             self.limit_min_leaf),
            ((_tr.e(_tr.c(2508, f'at least {self.min_internal} ')) + _tr.e(_tr.c(2509, f'{pl(self.min_internal, "instance")} in internal nodes'))),
             self.limit_min_internal),
            (_tr.e(_tr.c(2510, f'maximum depth {self.max_depth}')),
             self.limit_depth)
        ) if c) or _tr.m[2511, "None"]
        if self.limit_majority:
            items[_tr.m[2512, "Splitting"]] = (_tr.m[2513, "Stop splitting when majority reaches %d%% "] + _tr.m[2514, "(classification only)"]) % \
                                 self.sufficient_majority
        items[_tr.m[2515, "Binary trees"]] = (_tr.m[2516, "No"], _tr.m[2517, "Yes"])[self.binary_trees]
        return items
