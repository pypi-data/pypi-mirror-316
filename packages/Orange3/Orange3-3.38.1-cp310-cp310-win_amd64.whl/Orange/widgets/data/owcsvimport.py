...
FileFormats = [
    FileFormat("text/csv", _tr.m[471, "Text - comma separated"], ("*.csv", "*")),
    FileFormat("text/tab-separated-values", _tr.m[472, "Text - tab separated"], ("*.tsv", "*")),
    FileFormat("text/plain", _tr.m[473, "Text - all files"], ("*.txt", "*")),
]
...
def default_options_for_mime_type(
    ...
    delimiters = None
    try_encodings = ["utf-8", "utf-16", _tr.m[474, "iso8859-1"]]
...
class OWCSVFileImport(OWUrlDropBase):
    name = _tr.m[475, "CSV File Import"]
    description = _tr.m[476, "Import a data table from a CSV formatted file."]
    icon = "icons/CSVFile.svg"
    priority = 11
    category = _tr.m[477, "Data"]
    keywords = _tr.m[478, "csv file import, file, load, read, open, csv"]

    class Outputs:
        data = widget.Output(
            name=_tr.m[479, "Data"],
            type=Orange.data.Table,
            doc=_tr.m[480, "Loaded data set."])
        data_frame = widget.Output(
            name=_tr.m[481, "Data Frame"],
            type=pd.DataFrame,
            doc="",
            auto_summary=False
        )
    ...
    class Error(widget.OWWidget.Error):
        error = widget.Msg(
            _tr.m[482, "Unexpected error"]
        )
        encoding_error = widget.Msg(
            (_tr.m[483, "Encoding error\n"] + (_tr.m[484, "The file might be encoded in an unsupported encoding or it "] + _tr.m[485, "might be binary"]))
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        grid = QGridLayout()
        grid.addWidget(QLabel(_tr.m[486, "File:"], self), 0, 0, 1, 1)
        ...
        self.recent_combo = ItemStyledComboBox(
            self, objectName=_tr.m[487, "recent-combo"], toolTip=_tr.m[488, "Recent files."],
            sizeAdjustPolicy=QComboBox.AdjustToMinimumContentsLengthWithIcon,
            minimumContentsLength=16, placeholderText=_tr.m[489, "Recent files…"]
        )
        ...
        self.browse_button = QPushButton(
            ...
            toolTip=_tr.m[490, "Browse filesystem"], autoDefault=False,
        )
        ...
        ac = menu.addAction(_tr.m[491, "Import any file…"])
        ac.triggered.connect(self.browse)

        ac = menu.addAction(_tr.m[492, "Import relative to workflow file…"])
        ac.setToolTip(_tr.m[493, "Import a file within the workflow file directory"])
        ...
        self.import_options_button = QPushButton(
            _tr.m[495, "Import Options…"], enabled=False, autoDefault=False,
    ...
    def _browse_dialog(self):
        dlg = FileDialog(
            self, windowTitle=self.tr(_tr.m[496, "Open Data File"]),
    ...
    def _might_be_binary_mb(self, path) -> QMessageBox:
        mb = QMessageBox(
            ...
            text=self.tr((_tr.m[497, "The '{basename}' may be a binary file.\n"] + _tr.m[498, "Are you sure you want to continue?"])).format(
    ...
    def _path_must_be_relative_mb(self, prefix: str) -> QMessageBox:
        mb = QMessageBox(
            parent=self, windowTitle=self.tr(_tr.m[499, "Invalid path"]),
            icon=QMessageBox.Warning,
            text=self.tr(_tr.m[500, "Selected path is not within '{prefix}'"]).format(
                prefix=prefix
            ),
        )
        ...
        if status == QFileDialog.Accepted:
            ...
            if idx is not None:
                ...
                if options_ is not None:
                    options = options_
            dlg = CSVImportDialog(
                self, windowTitle=_tr.m[501, "Import Options"], sizeGripEnabled=True)
    ...
    def _activate_import_dialog_for_item(
        ...
        dlg = CSVImportDialog(
            self, windowTitle=_tr.m[502, "Import Options"], sizeGripEnabled=True,
        )
        dlg.setWindowModality(Qt.WindowModal)
        dlg.setAttribute(Qt.WA_DeleteOnClose)
    ...
    def cancel(self):
        ...
        if self.__watcher is not None:
            ...
            self.setStatusMessage(_tr.m[503, "Cancelled"])
            self.summary_text.setText(
                _tr.m[504, "<div>Cancelled<br/><small>Press 'Reload' to try again</small></div>"]
            )

    def __set_running_state(self):
        self.progressBarInit()
        self.setBlocking(True)
        self.setStatusMessage(_tr.m[505, "Running"])
        self.cancel_button.setEnabled(True)
        self.load_button.setText(_tr.m[506, "Restart"])
        path = self.current_item().path()
        self.Error.clear()
        self.summary_text.setText(
            _tr.m[507, "<div>Loading: <i>{}</i><br/>"].format(prettyfypath(path))
        )
    ...
    def __clear_running_state(self, ):
        ...
        self.load_button.setText(_tr.m[508, "Reload"])
    ...
    def __set_error_state(self, err):
        ...
        basename = os.path.basename(path)
        if isinstance(err, UnicodeDecodeError):
            text = (_tr.m[509, "<div><i>{basename}</i> was not loaded due to a text encoding "] + (_tr.m[510, "error. The file might be saved in an unknown or invalid "] + _tr.m[511, "encoding, or it might be a binary file.</div>"])).format(
                basename=escape(basename)
            )
        else:
            text = (_tr.m[512, "<div><i>{basename}</i> was not loaded due to an error:"] + "<p style='white-space: pre;'>{err}</p>").format(
                basename=escape(basename),
    ...
    def _update_status_messages(self, data):
        ...
        self.summary_text.setText((_tr.e(_tr.c(513, f"{n_instances} {pl(n_instances, 'row')}, ")) + (_tr.e(_tr.c(514, f"{n_features} {pl(n_features, 'feature')}, ")) + _tr.e(_tr.c(515, f"{n_meta} {pl(n_meta, 'meta')}")))))
...
def _open(path, mode, encoding=None):
    ...
    if ext == ".gz":
        ...
        if len(filelist) == 1:
            ...
            raise ValueError(_tr.m[516, "Expected a single file in the archive."])
    else:
