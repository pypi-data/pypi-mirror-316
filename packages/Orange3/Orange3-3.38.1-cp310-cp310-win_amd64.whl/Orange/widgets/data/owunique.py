...
class OWUnique(widget.OWWidget):
    name = _tr.m[1703, 'Unique']
    icon = 'icons/Unique.svg'
    description = _tr.m[1704, 'Filter instances unique by specified key attribute(s).']
    category = _tr.m[1705, "Transform"]
    priority = 1120

    class Inputs:
        data = widget.Input(_tr.m[1706, "Data"], Table)

    class Outputs:
        data = widget.Output(_tr.m[1707, "Data"], Table)

    want_main_area = False

    TIEBREAKERS = {_tr.m[1708, 'Last instance']: itemgetter(-1),
                   _tr.m[1709, 'First instance']: itemgetter(0),
                   _tr.m[1710, 'Middle instance']: lambda seq: seq[len(seq) // 2],
                   _tr.m[1711, 'Random instance']: np.random.choice,
                   _tr.m[1712, 'Discard non-unique instances']:
    ...
    def __init__(self):
        ...
        var_list = gui.listView(
            self.controlArea, self, "selected_vars", box=_tr.m[1713, "Group by"],
        ...
        gui.comboBox(
            ...
            label=_tr.m[1714, 'Instance to select in each group:'],
            items=tuple(self.TIEBREAKERS),
            callback=self.commit.deferred, sendSelectedValue=True)
        gui.auto_commit(
            self.controlArea, self, 'autocommit', _tr.m[1715, 'Commit'],
