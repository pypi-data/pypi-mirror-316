...
time_units = [_tr.m[640, "year"], _tr.m[641, "month"], _tr.m[642, "day"], _tr.m[643, "week"], _tr.m[644, "hour"], _tr.m[645, "minute"], _tr.m[646, "second"]]
INVALID_WIDTH = _tr.m[647, "invalid width"]
TOO_MANY_INTERVALS = _tr.m[648, "too many intervals"]
...
def _mdl_discretization(
    ...
    if not data.domain.has_discrete_class:
        return _tr.m[649, "no discrete class"]
...
Options: Dict[Methods, MethodDesc] = {
    ...
    for method in (
        MethodDesc(Methods.Default,
                   _tr.m[651, "Use default setting"], _tr.m[652, "default"],
                   _tr.m[653, "Treat the variable as defined in 'default setting'"],
                   None,
                   ()),
        MethodDesc(Methods.Keep,
                   _tr.m[654, "Keep numeric"], _tr.m[655, "keep"],
                   _tr.m[656, "Keep the variable as is"],
                   lambda data, var: var,
                   ()),
        MethodDesc(Methods.MDL,
                   _tr.m[657, "Entropy vs. MDL"], _tr.m[658, "entropy"],
                   (_tr.m[659, "Split values until MDL exceeds the entropy (Fayyad-Irani)\n"] + _tr.m[660, "(requires discrete class variable)"]),
                   _mdl_discretization,
                   ()),
        MethodDesc(Methods.EqualFreq,
                   _tr.m[661, "Equal frequency, intervals: "], _tr.m[662, "equal freq, k={}"],
                   _tr.m[663, "Create bins with same number of instances"],
                   lambda data, var, k: disc.EqualFreq(k)(data, var),
                   ("freq_spin", )),
        MethodDesc(Methods.EqualWidth,
                   _tr.m[664, "Equal width, intervals: "], _tr.m[665, "equal width, k={}"],
                   _tr.m[666, "Create bins of the same width"],
                   lambda data, var, k: disc.EqualWidth(k)(data, var),
                   ("width_spin", )),
        MethodDesc(Methods.Remove,
                   _tr.m[667, "Remove"], _tr.m[668, "remove"],
                   _tr.m[669, "Remove variable"],
                   lambda *_: None,
                   ()),
        MethodDesc(Methods.Binning,
                   _tr.m[670, "Natural binning, desired bins: "], _tr.m[671, "binning, desired={}"],
                   (_tr.m[672, "Create bins with nice thresholds; "] + _tr.m[673, "try matching desired number of bins"]),
                   lambda data, var, nbins: disc.Binning(nbins)(data, var),
                   ("binning_spin", )),
        MethodDesc(Methods.FixedWidth,
                   _tr.m[674, "Fixed width: "], _tr.m[675, "fixed width {}"],
                   _tr.m[676, "Create bins with the given width (not for time variables)"],
                   _fixed_width_discretization,
                   ("width_line", )),
        MethodDesc(Methods.FixedWidthTime,
                   _tr.m[677, "Time interval: "], _tr.m[678, "time interval, {} {}"],
                   _tr.m[679, "Create bins with the give width (for time variables)"],
                   _fixed_time_width_discretization,
                   ("width_time_line", "width_time_unit")),
        MethodDesc(Methods.Custom,
                   _tr.m[680, "Custom: "], _tr.m[681, "custom: {}"],
                   _tr.m[682, "Use manually specified thresholds"],
...
def format_desc(hint: VarHint) -> str:
    ...
    if hint.method_id == Methods.FixedWidthTime:
        ...
        try:
            ...
            unit = _tr.e(_tr.c(683, f"{time_units[unit]}(s)"))
        else:
            unit = _tr.e(_tr.c(684, f"{pl(width, time_units[unit])}"))
    ...
    def data(self, index, role=Qt.DisplayRole):
        if role == Qt.ToolTipRole:
            ...
            if len(data.values) <= 3:
                return (f'<p style="white-space:pre">{tip}' + f'{",&nbsp;&nbsp;".join(values)}</p>')
            else:
                return tip + "<br/>" \
...
class DefaultDiscModel(QAbstractListModel):
    ...
    def data(self, _, role=Qt.DisplayRole):
        if role == Qt.DisplayRole:
            return _tr.m[685, "Default setting: "] + format_desc(self.hint)
        elif role == Qt.DecorationRole:
            return DefaultDiscModel.icon
        elif role == Qt.ToolTipRole:
            return _tr.m[686, "Default setting for variables without specific setings"]
        return None
...
globals().update(dict(
    ...
    Remove=namedtuple(_tr.m[687, "Remove"], []),
...
class OWDiscretize(widget.OWWidget):
    ...
    name = _tr.m[688, "Discretize"]
    description = _tr.m[689, "Discretize numeric variables"]
    category = _tr.m[690, "Transform"]
    icon = "icons/Discretize.svg"
    keywords = _tr.m[691, "discretize, bin, categorical, nominal, ordinal"]
    priority = 2130

    class Inputs:
        data = Input(_tr.m[692, "Data"], Table, doc=_tr.m[693, "Input data table"])

    class Outputs:
        data = Output(_tr.m[694, "Data"], Table, doc=_tr.m[695, "Table with categorical features"])
    ...
    def _create_buttons(self, box):
        ...
        def manual_cut_editline(text="", enabled=True) -> QLineEdit:
            edit = QLineEdit(
                text=text,
                placeholderText=_tr.m[696, "e.g. 0.0, 0.5, 1.0"],
                toolTip='<p style="white-space:pre">' +
                        (_tr.m[697, 'Enter cut points as a comma-separate list of \n'] + _tr.m[698, 'strictly increasing numbers e.g. 0.0, 0.5, 1.0).</p>']),
                enabled=enabled,
            )
        ...
        u.addItems([_tr.e(_tr.c(699, f"{unit}(s)")) for unit in time_units])
        ...
        self.copy_to_custom = FixedSizeButton(
            text=_tr.m[700, "CC"], toolTip=_tr.m[701, "Copy the current cut points to manual mode"])
    ...
    def _discretize_var(self, var: ContinuousVariable, hint: VarHint) \
        ...
        if isinstance(var, TimeVariable):
            if hint.method_id in (Methods.FixedWidth, Methods.Custom):
                return _tr.m[702, ": <keep, time var>"], var
        else:
            if hint.method_id == Methods.FixedWidthTime:
                return _tr.m[703, ": <keep, not time>"], var
    ...
    def send_report(self):
        ...
        self.report_items(_tr.m[705, "Variables"], reported)
