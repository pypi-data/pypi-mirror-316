...
DiscreteOptions: Dict[int, MethodDesc] = {
    ...
    for method in (
        MethodDesc(
            Continuize.Default, _tr.m[316, "Use preset"], _tr.m[317, "preset"],
            _tr.m[318, "Treat the variable as defined in preset"]),
        MethodDesc(
            Continuize.Leave, _tr.m[319, "Keep categorical"], _tr.m[320, "keep as is"],
            _tr.m[321, "Keep the variable discrete"]),
        MethodDesc(
            Continuize.FirstAsBase, _tr.m[322, "First value as base"], _tr.m[323, "first as base"],
            _tr.m[324, "One indicator variable for each value except the first"]),
        MethodDesc(
            Continuize.FrequentAsBase, _tr.m[325, "Most frequent as base"], _tr.m[326, "frequent as base"],
            _tr.m[327, "One indicator variable for each value except the most frequent"],
            False),
        MethodDesc(
            Continuize.Indicators, _tr.m[328, "One-hot encoding"], _tr.m[329, "one-hot"],
            _tr.m[330, "One indicator variable for each value"],
            False),
        MethodDesc(
            Continuize.RemoveMultinomial, _tr.m[331, "Remove if more than 2 values"], _tr.m[332, "remove if >2"],
            _tr.m[333, "Remove variables with more than two values; indicator otherwise"]),
        MethodDesc(
            Continuize.Remove, _tr.m[334, "Remove"], _tr.m[335, "remove"],
            _tr.m[336, "Remove variable"]),
        MethodDesc(
            Continuize.AsOrdinal, _tr.m[337, "Treat as ordinal"], _tr.m[338, "as ordinal"],
            _tr.m[339, "Each value gets a consecutive number from 0 to number of values - 1"]),
        MethodDesc(
            Continuize.AsNormalizedOrdinal, _tr.m[340, "Treat as normalized ordinal"], _tr.m[341, "as norm. ordinal"],
            _tr.m[342, "Same as above, but scaled to [0, 1]"])
    )}
...
ContinuousOptions: Dict[int, MethodDesc] = {
    ...
    for method in (
        MethodDesc(
            Normalize.Default, _tr.m[343, "Use preset"], _tr.m[344, "preset"],
            _tr.m[345, "Treat the variable as defined in 'default setting'"]),
        MethodDesc(
            Normalize.Leave, _tr.m[346, "Keep as it is"], _tr.m[347, "no change"],
            _tr.m[348, "Keep the variable as it is"]),
        MethodDesc(
            Normalize.Standardize, _tr.m[349, "Standardize to μ=0, σ²=1"], _tr.m[350, "standardize"],
            _tr.m[351, "Subtract the mean and divide by standard deviation"],
            False),
        MethodDesc(
            Normalize.Center, _tr.m[352, "Center to μ=0"], _tr.m[353, "center"],
            _tr.m[354, "Subtract the mean"],
            False),
        MethodDesc(
            Normalize.Scale, _tr.m[355, "Scale to σ²=1"], _tr.m[356, "scale"],
            _tr.m[357, "Divide by standard deviation"]),
        MethodDesc(
            Normalize.Normalize11, _tr.m[358, "Normalize to interval [-1, 1]"], _tr.m[359, "to [-1, 1]"],
            _tr.m[360, "Linear transformation into interval [-1, 1]"],
            False),
        MethodDesc(
            Normalize.Normalize01, _tr.m[361, "Normalize to interval [0, 1]"], _tr.m[362, "to [0, 1]"],
            _tr.m[363, "Linear transformation into interval [0, 1]"],
            False),
    )}
    ...
    def __init__(self, valid_type):
        super().__init__(
            order=(DomainModel.ATTRIBUTES,
                   LabelledSeparator(_tr.m[364, "Meta attributes"]), DomainModel.METAS,
                   LabelledSeparator(_tr.m[365, "Targets"]), DomainModel.CLASSES),
...
class DefaultContModel(QAbstractListModel):
    ...
    def data(self, _, role=Qt.DisplayRole):
        if role == Qt.DisplayRole:
            return _tr.e(_tr.c(366, f"Preset: {self.method}"))
        elif role == Qt.DecorationRole:
            return self.icon
        elif role == Qt.ToolTipRole:
            return _tr.m[367, "Default for variables without specific settings"]
        return None
...
class OWContinuize(widget.OWWidget):
    ...
    name = _tr.m[368, "Continuize"]
    description = (_tr.m[369, "Transform categorical attributes into numeric and, "] +
                   _tr.m[370, "optionally, scale numeric values."])
    icon = "icons/Continuize.svg"
    category = _tr.m[371, "Transform"]
    keywords = _tr.m[372, "continuize, encode, dummy, numeric, one-hot, binary, treatment, contrast"]
    priority = 2120

    class Inputs:
        data = Input(_tr.m[373, "Data"], Table)

    class Outputs:
        data = Output(_tr.m[374, "Data"], Table)

    class Error(widget.OWWidget.Error):
        unsupported_sparse = \
            widget.Msg(_tr.m[375, "Some chosen methods do not support sparse data: {}"])
    ...
    def __init__(self):
        ...
        self.disc_box, self.disc_view, self.disc_radios, self.disc_group = \
            create(_tr.m[376, "Categorical Variables"], DiscreteVariable, DiscreteOptions)
        ...
        self.cont_box, self.cont_view, self.cont_radios, self.cont_group = \
            create(_tr.m[377, "Numeric Variables"], ContinuousVariable, ContinuousOptions)
        ...
        gui.button(
            box, self, _tr.m[378, "Reset All"], callback=self._on_reset_hints,
            autoDefault=False)
        gui.rubber(box)
    ...
    def send_report(self):
        ...
        if single_disc and single_cont:
            self.report_items(
                ((_tr.m[379, "Categorical variables"], single_disc),
                 (_tr.m[380, "Numeric variables"], single_cont))
            )
        else:
            if single_disc:
                self.report_paragraph(_tr.m[381, "Categorical variables"], single_disc)
            elif len(self.disc_view.model()) > 0:
                self.report_items(
                    _tr.m[382, "Categorical variables"],
                    [(_tr.m[383, "Preset"] if name == DefaultKey else name,
                      DiscreteOptions[id_].label.lower())
                     for name, id_ in self.disc_var_hints.items()])
            if single_cont:
                self.report_paragraph(_tr.m[384, "Numeric variables"], single_cont)
            elif len(self.cont_view.model()) > 0:
                self.report_items(
                    _tr.m[385, "Numeric variables"],
                    [(_tr.m[386, "Preset"] if name == DefaultKey else name,
                      ContinuousOptions[id_].label.lower())
                     for name, id_ in self.cont_var_hints.items()])
            self.report_paragraph(_tr.m[387, "Unlisted"],
                (_tr.m[388, "Any unlisted attributes default to preset option, and "] + (_tr.m[389, "unlisted meta attributes and target variables are kept "] + _tr.m[390, "as they are"])))
