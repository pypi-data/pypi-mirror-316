...
AGGREGATIONS = {
    _tr.m[926, "Mean"]: Aggregation(
        "mean",
        {ContinuousVariable, TimeVariable},
        False),
    _tr.m[927, "Median"]: Aggregation(
        "median",
        {ContinuousVariable, TimeVariable},
        True),
    "Q1": Aggregation(
    ...
    "Q3": Aggregation(
        ...
        {ContinuousVariable, TimeVariable},
        True),
    _tr.m[928, "Min. value"]: Aggregation(
        "min",
        {ContinuousVariable, TimeVariable},
        True),
    _tr.m[929, "Max. value"]: Aggregation(
        "max",
        {ContinuousVariable, TimeVariable},
        True),
    _tr.m[930, "Mode"]: Aggregation(
        lambda x: pd.Series.mode(x).get(0, nan),
        {ContinuousVariable, DiscreteVariable, TimeVariable},
        True
    ),
    _tr.m[931, "Standard deviation"]: Aggregation(
        std,
        {ContinuousVariable, TimeVariable},
        ContinuousVariable
    ),
    _tr.m[932, "Variance"]: Aggregation(
        var,
        {ContinuousVariable, TimeVariable},
        ContinuousVariable
    ),
    _tr.m[933, "Sum"]: Aggregation(
        "sum",
        {ContinuousVariable},
        True),
    _tr.m[934, "Concatenate"]: Aggregation(
        concatenate,
        {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable},
        StringVariable
    ),
    _tr.m[935, "Span"]: Aggregation(
        span,
        {ContinuousVariable, TimeVariable},
        ContinuousVariable
    ),
    _tr.m[936, "First value"]: Aggregation(
        "first",
        {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable},
        True
    ),
    _tr.m[937, "Last value"]: Aggregation(
        "last",
        {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable},
        True
    ),
    _tr.m[938, "Random value"]: Aggregation(
        lambda x: x.sample(1, random_state=0),
        {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable},
        True
    ),
    _tr.m[939, "Count defined"]: Aggregation(
        "count",
        {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable},
        ContinuousVariable
    ),
    _tr.m[940, "Count"]: Aggregation(
        "size",
        {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable},
        ContinuousVariable
    ),
    _tr.m[941, "Proportion defined"]: Aggregation(
...
def _run(
    ...
    state.set_status(_tr.m[942, "Aggregating"])
    # group table rows
...
TABLE_COLUMN_NAMES = [_tr.m[943, "Attributes"], _tr.m[944, "Aggregations"]]
    ...
    def data(self, index, role=Qt.DisplayRole) -> Any:
        ...
        if role in (Qt.DisplayRole, Qt.EditRole):
            if col == TabColumn.attribute:
                ...
                n_more = "" if len(aggs) <= 3 else _tr.e(_tr.c(945, f" and {len(aggs) - 3} more"))
...
class OWGroupBy(OWWidget, ConcurrentWidgetMixin):
    name = _tr.m[946, "Group by"]
    description = ""
    category = _tr.m[947, "Transform"]
    icon = "icons/GroupBy.svg"
    keywords = _tr.m[948, "aggregate, group by"]
    priority = 1210

    class Inputs:
        data = Input(_tr.m[949, "Data"], Table, doc=_tr.m[950, "Input data table"])

    class Outputs:
        data = Output(_tr.m[951, "Data"], Table, doc=_tr.m[952, "Aggregated data"])
    ...
    def __init_main_area(self) -> None:
        ...
        gui.widgetBox(self.mainArea, orientation=grid_layout, box=_tr.m[954, "Aggregations"])
