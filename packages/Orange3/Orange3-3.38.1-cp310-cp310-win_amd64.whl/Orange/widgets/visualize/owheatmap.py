...
def split_domain(domain: Domain, split_label: str):
    ...
    if None in groups:
        na = groups.pop(None)
        return [*groups.items(), (_tr.m[3474, "N/A"], na)]
    else:
...
ClusteringModelData = [
    {
        Qt.DisplayRole: _tr.m[3475, "None"],
        Qt.ToolTipRole: _tr.m[3476, "No clustering"],
        ClusteringRole: Clustering.None_,
    }, {
        Qt.DisplayRole: _tr.m[3477, "Clustering"],
        Qt.ToolTipRole: _tr.m[3478, "Apply hierarchical clustering"],
        ClusteringRole: Clustering.Clustering,
    }, {
        Qt.DisplayRole: _tr.m[3479, "Clustering (opt. ordering)"],
        Qt.ToolTipRole: (_tr.m[3480, "Apply hierarchical clustering with optimal leaf "] + _tr.m[3481, "ordering."]),
...
ColumnLabelsPosData = [
    ...
    for name, value in [
        (_tr.m[3482, "None"], HeatmapGridWidget.NoPosition),
        (_tr.m[3483, "Top"], HeatmapGridWidget.PositionTop),
        (_tr.m[3484, "Bottom"], HeatmapGridWidget.PositionBottom),
        (_tr.m[3485, "Top and Bottom"], (HeatmapGridWidget.PositionTop |
...
class OWHeatMap(widget.OWWidget):
    name = _tr.m[3486, "Heat Map"]
    description = _tr.m[3487, "Plot a data matrix heatmap."]
    icon = "icons/Heatmap.svg"
    priority = 260
    keywords = _tr.m[3488, "heat map"]

    class Inputs:
        data = Input(_tr.m[3489, "Data"], Table)

    class Outputs:
        selected_data = Output(_tr.m[3490, "Selected Data"], Table, default=True)
    ...
    class Information(widget.OWWidget.Information):
        sampled = Msg(_tr.m[3491, "Data has been sampled"])
        discrete_ignored = Msg(_tr.m[3492, "Categorical features are ignored."])
        row_clust = Msg("{}")
        col_clust = Msg("{}")
        sparse_densified = Msg(_tr.m[3493, "Showing this data may require a lot of memory"])

    class Error(widget.OWWidget.Error):
        no_continuous = Msg(_tr.m[3494, "No numeric features"])
        not_enough_features = Msg(_tr.m[3495, "Not enough features for column clustering"])
        not_enough_instances = Msg(_tr.m[3496, "Not enough instances for clustering"])
        not_enough_instances_k_means = Msg(
            _tr.m[3497, "Not enough instances for k-means merging"])
        not_enough_memory = Msg(_tr.m[3498, "Not enough memory to show this data"])

    class Warning(widget.OWWidget.Warning):
        empty_clusters = Msg(_tr.m[3499, "Empty clusters were removed"])

    UserAdviceMessages = [
        widget.Message(
            (_tr.m[3500, "For data with a meaningful mid-point, "] + _tr.m[3501, "choose one of diverging palettes."]),
            "diverging_palette")]
    ...
    def __init__(self):
        ...
        # GUI definition
        colorbox = gui.vBox(self.controlArea, _tr.m[3502, "Color"])
        ...
        mergebox = gui.vBox(self.controlArea, _tr.m[3503, "Merge"],)
        gui.checkBox(mergebox, self, "merge_kmeans", _tr.m[3504, "Merge by k-means"],
                     callback=self.__update_row_clustering)
        ibox = gui.indentedBox(mergebox)
        gui.spin(ibox, self, "merge_kmeans_k", minv=5, maxv=500,
                 label=_tr.m[3505, "Clusters:"], keyboardTracking=False,
                 callbackOnReturn=True, callback=self.update_merge)

        cluster_box = gui.vBox(self.controlArea, _tr.m[3506, "Clustering"])
        # Row clustering
        ...
        form.addRow(_tr.m[3507, "Rows:"], self.row_cluster_cb)
        form.addRow(_tr.m[3508, "Columns:"], self.col_cluster_cb)
        cluster_box.layout().addLayout(form)
        box = gui.vBox(self.controlArea, _tr.m[3509, "Split By"])
        form = QFormLayout(
        ...
        self.row_split_cb = cb = ComboBoxSearch(
            ...
            toolTip=_tr.m[3511, "Split the heatmap vertically by a categorical column"]
        )
        ...
        self.col_split_cb = cb = ComboBoxSearch(
            sizeAdjustPolicy=ComboBox.AdjustToMinimumContentsLengthWithIcon,
            minimumContentsLength=14,
            toolTip=_tr.m[3513, "Split the heatmap horizontally by column annotation"]
        )
        ...
        form.addRow(_tr.m[3514, "Rows:"], self.row_split_cb)
        form.addRow(_tr.m[3515, "Columns:"], self.col_split_cb)

        box = gui.vBox(self.controlArea, _tr.m[3516, 'Annotation && Legends'])

        gui.checkBox(box, self, 'legend', _tr.m[3517, 'Show legend'],
                     callback=self.update_legend)

        gui.checkBox(box, self, 'averages', _tr.m[3518, 'Stripes with averages'],
                     callback=self.update_averages_stripe)
        gui.separator(box)
        annotbox = QGroupBox(_tr.m[3519, "Row Annotations"])
        form = QFormLayout(
            annotbox,
        ...
        self.annotation_model = DomainModel(placeholder=_tr.m[3520, "(None)"])
        ...
        self.row_side_color_model = DomainModel(
            order=(DomainModel.CLASSES, DomainModel.Separator,
                   DomainModel.METAS),
            placeholder=_tr.m[3521, "(None)"], valid_types=DomainModel.PRIMITIVE,
        ...
        form.addRow(_tr.m[3522, "Text"], self.annotation_text_cb)
        form.addRow(_tr.m[3523, "Color"], self.row_side_color_cb)
        box.layout().addWidget(annotbox)
        annotbox = QGroupBox(_tr.m[3524, "Column annotations"])
        form = QFormLayout(
            annotbox,
            formAlignment=Qt.AlignLeft,
            labelAlignment=Qt.AlignLeft,
        ...
        form.addRow(_tr.m[3526, "Position"], cb)
        form.addRow(_tr.m[3527, "Color"], self.col_side_color_cb)
        box.layout().addWidget(annotbox)

        gui.checkBox(self.controlArea, self, "keep_aspect",
                     _tr.m[3528, "Keep aspect ratio"], box=_tr.m[3529, "Resize"],
        ...
        self.__font_inc = QAction(
            _tr.m[3530, "Increase Font"], self, shortcut=QKeySequence("ctrl+>"))
        self.__font_dec = QAction(
            _tr.m[3531, "Decrease Font"], self, shortcut=QKeySequence("ctrl+<"))
        ...
        if group_var is not None:
            ...
            if np.any(_col_data.mask):
                row_groups.append(RowPart(
                    title=_tr.m[3532, "N/A"], indices=np.flatnonzero(_col_data.mask),
                    cluster=None, cluster_ordered=None
                ))
        else:
    ...
    def __update_clustering_enable_state(self, parts: Optional['Parts']):
        ...
        if not rco_enabled and row_clust == Clustering.OrderedClustering:
            ...
            row_clust_msg = (_tr.m[3533, "Row cluster ordering was disabled due to the "] + _tr.m[3534, "estimated runtime cost"])
        if not rc_enabled and row_clust == Clustering.Clustering:
            row_clust = Clustering.None_
            row_clust_msg = (_tr.m[3535, "Row clustering was was disabled due to the "] + _tr.m[3536, "estimated runtime cost"])

        if not cco_enabled and col_clust == Clustering.OrderedClustering:
            col_clust = Clustering.Clustering
            col_clust_msg = (_tr.m[3537, "Column cluster ordering was disabled due to "] + _tr.m[3538, "estimated runtime cost"])
        if not cc_enabled and col_clust == Clustering.Clustering:
            col_clust = Clustering.None_
            col_clust_msg = (_tr.m[3539, "Column clustering was disabled due to the "] + _tr.m[3540, "estimated runtime cost"])
    ...
    def update_annotations(self):
        ...
        if widget is not None:
            ...
            if merge_indices is not None and annot_col is not None:
                join = lambda _1: join_elided(", ", 42, _1, _tr.m[3541, " ({} more)"])
    ...
    def send_report(self):
        self.report_items((
            (_tr.m[3542, "Columns:"], _tr.m[3543, "Clustering"] if self.col_clustering else _tr.m[3544, "No sorting"]),
            (_tr.m[3545, "Rows:"], _tr.m[3546, "Clustering"] if self.row_clustering else _tr.m[3547, "No sorting"]),
            (_tr.m[3548, "Split:"],
             self.split_by_var is not None and self.split_by_var.name),
            (_tr.m[3549, "Row annotation"],
...
def colorize(var: Variable, data: np.ndarray) -> Tuple[np.ndarray, ColorMap]:
    ...
    if var.is_discrete:
        ...
        data[mask] = -1
        if mask.any():
            values = (*var.values, _tr.m[3550, "N/A"])
        else:
            values = var.values
            colors = colors[: -1]
...
def aggregate(
    ...
    if var.is_string:
        join = lambda values: (join_elided(", ", 42, values, _tr.m[3551, " ({} more)"]))
...
def agg_join_str(var, data, groupindices, maxlen=50, elidetemplate=_tr.m[3552, " ({} more)"]):
