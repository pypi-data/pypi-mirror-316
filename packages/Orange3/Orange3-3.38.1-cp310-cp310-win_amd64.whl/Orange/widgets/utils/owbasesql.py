...
class OWBaseSql(OWWidget, openclass=True):
    ...
    """
    class Outputs:
        data = Output(_tr.m[3171, "Data"], Table)
    ...
    def _setup_gui(self):
        ...
        vbox = gui.vBox(self.controlArea, _tr.m[3172, "Server"])
        self.serverbox = gui.vBox(vbox)
        self.servertext = QLineEdit(self.serverbox)
        self.servertext.setPlaceholderText(_tr.m[3173, "Server"])
        self.servertext.setToolTip(_tr.m[3174, "Server"])
        ...
        self.databasetext.setPlaceholderText(_tr.m[3175, "Database[/Schema]"])
        self.databasetext.setToolTip(_tr.m[3176, "Database or optionally Database/Schema"])
        if self.database:
        ...
        self.usernametext.setPlaceholderText(_tr.m[3177, "Username"])
        self.usernametext.setToolTip(_tr.m[3178, "Username"])

        self.serverbox.layout().addWidget(self.usernametext)
        self.passwordtext = QLineEdit(self.serverbox)
        self.passwordtext.setPlaceholderText(_tr.m[3179, "Password"])
        self.passwordtext.setToolTip(_tr.m[3180, "Password"])
        ...
        self.connectbutton = gui.button(self.serverbox, self, _tr.m[3181, "Connect"],
    ...
    def on_connection_success(self):
        ...
        self.database_desc = OrderedDict((
            (_tr.m[3182, "Host"], self.host), (_tr.m[3183, "Port"], self.port),
            (_tr.m[3184, "Database"], self.database), (_tr.m[3185, "User name"], self.username)
        ))
    ...
    def send_report(self):
        if not self.database_desc:
            self.report_paragraph(_tr.m[3186, "No database connection."])
            return
        self.report_items(_tr.m[3187, "Database"], self.database_desc)
        if self.data_desc_table:
            self.report_items(_tr.m[3188, "Data"],
