...
INITIALIZATIONS = [("PCA", "pca"), (_tr.m[3052, "Spectral"], "spectral")]
DISTANCE_METRICS = [(_tr.m[3053, "Euclidean"], "l2"), (_tr.m[3054, "Manhattan"], "l1"), (_tr.m[3055, "Cosine"], "cosine")]
    ...
    def validate(self) -> "Task":
        ...
        if self.distance_matrix is not None:
            if self.distance_metric != "precomputed":
                error(
                    ("`distance_metric` must be set to `precomputed` when using " + "a distance matrix")
                )
            if self.initialization_method != "spectral":
                error(
                    ("`initialization_method` must be set to `spectral` when " + "using a distance matrix")
                )

        if self.distance_matrix is None:
            if self.distance_metric == "precomputed":
                error(
                    ("`distance_metric` cannot be set to `precomputed` when no " + "distance matrix is provided")
                )
...
class TSNERunner:
    ...
    def compute_tsne_preprocessing(task: Task, state: TaskState, **_) -> None:
        state.set_status(_tr.m[3056, "Preprocessing data..."])
    ...
    def compute_normalization(task: Task, state: TaskState, **_) -> None:
        state.set_status(_tr.m[3057, "Normalizing data..."])
    ...
    def compute_pca(task: Task, state: TaskState, **_) -> None:
        ...
        state.set_status(_tr.m[3058, "Computing PCA..."])
    ...
    def compute_initialization(task: Task, state: TaskState, **_) -> None:
        ...
        state.set_status(_tr.m[3059, "Preparing initialization..."])
    ...
    def compute_affinities(task: Task, state: TaskState, **_) -> None:
        state.set_status(_tr.m[3060, "Finding nearest neighbors..."])
    ...
    def compute_tsne(task: Task, state: TaskState, progress_callback=None) -> None:
        ...
        state.set_status(_tr.m[3061, "Running optimization..."])
...
class OWtSNE(OWDataProjectionWidget, ConcurrentWidgetMixin):
    name = "t-SNE"
    description = _tr.m[3062, "Two-dimensional data projection with t-SNE."]
    icon = "icons/TSNE.svg"
    priority = 920
    keywords = _tr.m[3063, "t-sne, tsne"]
    ...
    class Inputs(OWDataProjectionWidget.Inputs):
        distances = Input(_tr.m[3064, "Distances"], DistMatrix)

    class Information(OWDataProjectionWidget.Information):
        modified = Msg((_tr.m[3065, "The parameter settings have been changed. Press "] + _tr.m[3066, "\"Start\" to rerun with the new settings."]))

    class Warning(OWDataProjectionWidget.Warning):
        consider_using_pca_preprocessing = Msg(
            (_tr.m[3067, "The input data contains a large number of features, which may slow"] + _tr.m[3068, " down t-SNE computation. Consider enabling PCA preprocessing."])
        )

    class Error(OWDataProjectionWidget.Error):
        not_enough_rows = Msg(_tr.m[3069, "Input data needs at least 2 rows"])
        not_enough_cols = Msg(_tr.m[3070, "Input data needs at least 2 attributes"])
        constant_data = Msg(_tr.m[3071, "Input data is constant"])
        no_valid_data = Msg(_tr.m[3072, "No projection due to no valid data"])

        distance_matrix_not_symmetric = Msg(_tr.m[3073, "Distance matrix is not symmetric"])
        distance_matrix_too_small = Msg(_tr.m[3074, "Input matrix must be at least 2x2"])

        dimension_mismatch = Msg(_tr.m[3075, "Data and distance dimensions do not match"])
    ...
    def _add_controls_start_box(self):
        self.preprocessing_box = gui.vBox(self.controlArea, box=_tr.m[3076, "Preprocessing"])
        self.normalize_cbx = gui.checkBox(
            self.preprocessing_box, self, "normalize", _tr.m[3077, "Normalize data"],
            callback=self._normalize_data_changed, stateWhenDisabled=False,
        )
        self.pca_preprocessing_cbx = gui.checkBox(
            self.preprocessing_box, self, "use_pca_preprocessing", _tr.m[3078, "Apply PCA preprocessing"],
            callback=self._pca_preprocessing_changed, stateWhenDisabled=False,
        )
        self.pca_component_slider = gui.hSlider(
            self.preprocessing_box, self, "pca_components", label=_tr.m[3079, "PCA Components:"],
            minValue=2, maxValue=_MAX_PCA_COMPONENTS, step=1,
            callback=self._pca_slider_changed,
        )

        self.parameter_box = gui.vBox(self.controlArea, box=_tr.m[3080, "Parameters"])
        form = QFormLayout(
        ...
        form.addRow(_tr.m[3081, "Initialization:"], self.initialization_combo)
        ...
        form.addRow(_tr.m[3082, "Distance metric:"], self.distance_metric_combo)
        ...
        form.addRow(_tr.m[3083, "Perplexity:"], self.perplexity_spin)

        form.addRow(gui.checkBox(
            self.controlArea, self, "multiscale", label=_tr.m[3084, "Preserve global structure"],
        ...
        form.addRow(_tr.m[3085, "Exaggeration:"], sbe)

        self.parameter_box.layout().addLayout(form)

        self.run_button = gui.button(
            self.parameter_box, self, _tr.m[3086, "Start"], callback=self._toggle_run
        )
    ...
    def _stop_running_task(self):
        self.cancel()
        self.run_button.setText(_tr.m[3087, "Start"])
    ...
    def _toggle_run(self):
        ...
        if self.task is not None:
            self.cancel()
            self.run_button.setText(_tr.m[3088, "Resume"])
    ...
    def enable_controls(self):
        ...
        if has_distance_matrix:
            ...
            self.normalize_cbx.setToolTip(
                _tr.m[3089, "Precomputed distances provided. Preprocessing is unnecessary!"]
            )

            self.pca_preprocessing_cbx.setDisabled(True)
            self.pca_preprocessing_cbx.setToolTip(
                _tr.m[3090, "Precomputed distances provided. Preprocessing is unnecessary!"]
            )

            # Only spectral init is valid with a precomputed distance matrix
            spectral_init_idx = self.initialization_combo.findText(_tr.m[3091, "Spectral"])
            self.initialization_combo.setCurrentIndex(spectral_init_idx)
            self.initialization_combo.setDisabled(True)
            self.initialization_combo.setToolTip(
                (_tr.m[3092, "Only spectral intialization is supported with precomputed "] + _tr.m[3093, "distance matrices."])
            )
            ...
            self.distance_metric_combo.setToolTip(
                _tr.m[3094, "Precomputed distances provided."]
            )
        ...
        if not has_distance_matrix and has_data and self.data.is_sparse():
            self.normalize_cbx.setDisabled(True)
            self.normalize_cbx.setToolTip(
                _tr.m[3095, "Data normalization is not supported on sparse matrices."]
            )
    ...
    def run(self):
        ...
        if self.data is not None:
            self.run_button.setText(_tr.m[3096, "Stop"])
    ...
    def on_done(self, task):
        # type: (Task) -> None
        self.run_button.setText(_tr.m[3097, "Start"])
    ...
    def cancel(self):
        self.run_button.setText(_tr.m[3098, "Start"])
        super().cancel()
