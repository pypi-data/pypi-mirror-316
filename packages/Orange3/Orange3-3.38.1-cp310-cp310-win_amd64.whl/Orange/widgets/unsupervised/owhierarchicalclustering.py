...
LINKAGE = [_tr.m[2743, "Single"], _tr.m[2744, "Average"], _tr.m[2745, "Weighted"], _tr.m[2746, "Complete"], _tr.m[2747, "Ward"]]
LINKAGE_ARGS = ["single", "average", "weighted", "complete", "ward"]
DEFAULT_LINKAGE = _tr.m[2748, "Ward"]
...
class OWHierarchicalClustering(widget.OWWidget):
    name = _tr.m[2749, "Hierarchical Clustering"]
    description = (_tr.m[2750, "Display a dendrogram of a hierarchical clustering "] + _tr.m[2751, "constructed from the input distance matrix."])
    icon = "icons/HierarchicalClustering.svg"
    priority = 2100
    keywords = _tr.m[2752, "hierarchical clustering"]

    class Inputs:
        distances = Input(_tr.m[2753, "Distances"], Orange.misc.DistMatrix)
        subset = Input(_tr.m[2754, "Data Subset"], Orange.data.Table, explicit=True)

    class Outputs:
        selected_data = Output(_tr.m[2755, "Selected Data"], Orange.data.Table, default=True)
    ...
    annotation = settings.ContextSetting(_tr.m[2756, "Enumeration"])
    #: Out-of-context setting for the case when the "Name" option is available
    annotation_if_names = settings.Setting(_tr.m[2757, "Name"])
    #: Out-of-context setting for the case with just "Enumeration" and "None"
    annotation_if_enumerate = settings.Setting(_tr.m[2758, "Enumeration"])
    ...
    basic_annotations = [None, _tr.m[2759, "Enumeration"]]

    class Error(widget.OWWidget.Error):
        empty_matrix = Msg(_tr.m[2760, "Distance matrix is empty."])
        not_finite_distances = Msg(_tr.m[2761, "Some distances are infinite"])
        not_symmetric = widget.Msg(_tr.m[2762, "Distance matrix is not symmetric."])

    class Warning(widget.OWWidget.Warning):
        subset_on_no_table = \
            Msg(_tr.m[2763, "Unused data subset: distances do not refer to data instances"])
        subset_not_subset = \
            Msg(_tr.m[2764, "Some data from the subset does not appear in distance matrix"])
        subset_wrong = \
            Msg(_tr.m[2765, "Subset data refers to a different table"])
        pruning_disables_colors = \
            Msg(_tr.m[2766, "Pruned cluster doesn't show colors and indicate subset"])
        many_clusters = \
            Msg((_tr.m[2767, "Variables with too many values may "] + _tr.m[2768, "degrade the performance of downstream widgets."]))
    ...
    def __init__(self):
        ...
        gui.comboBox(
            self.controlArea, self, "linkage", items=LINKAGE, box=_tr.m[2769, "Linkage"],
            callback=self._invalidate_clustering)

        model = itemmodels.VariableListModel(placeholder=_tr.m[2770, "None"])
        model[:] = self.basic_annotations

        grid = QGridLayout()
        gui.widgetBox(self.controlArea, _tr.m[2771, "Annotations"], orientation=grid)
        ...
        cb = gui.checkBox(
            None, self, "label_only_subset", _tr.m[2772, "Show labels only for subset"],
            disabled=True,
        ...
        grid.addWidget(
            gui.appendRadioButton(
                self.selection_box, _tr.m[2779, "Manual"], addToLayout=False),
            0, 0
        )
        grid.addWidget(
            gui.appendRadioButton(
                self.selection_box, _tr.m[2780, "Height ratio:"], addToLayout=False),
            1, 0
        )
        ...
        grid.addWidget(
            gui.appendRadioButton(
                self.selection_box, _tr.m[2781, "Top N:"], addToLayout=False),
            2, 0
        )
        ...
        self.zoom_slider = gui.hSlider(
            self.controlArea, self, "zoom_factor", box=_tr.m[2782, "Zoom"],
            minValue=-6, maxValue=3, step=1, ticks=True, createLabel=False,
            callback=self.__update_font_scale)

        zoom_in = QAction(
            _tr.m[2783, "Zoom in"], self, shortcut=QKeySequence.ZoomIn,
            triggered=self.__zoom_in
        )
        zoom_out = QAction(
            _tr.m[2784, "Zoom out"], self, shortcut=QKeySequence.ZoomOut,
            triggered=self.__zoom_out
        )
        zoom_reset = QAction(
            _tr.m[2785, "Reset zoom"], self,
        ...
        if self.root and self._displayed_root:
            ...
            if self.annotation is None:
                if not self.pruning \
                    ...
                    labels = []
            elif self.annotation == _tr.m[2788, "Enumeration"]:
                labels = [str(i+1) for i in indices]
            elif self.annotation == _tr.m[2789, "Name"]:
    ...
    def commit(self):
        ...
        if isinstance(items, Orange.data.Table) and self.matrix.axis == 1:
            ...
            clust_name = get_unique_names(domain, _tr.m[2790, "Cluster"])
            ...
            ann_clust_var = Orange.data.DiscreteVariable(
                name=clust_name, values=values + [_tr.m[2791, "Other"]]
            )
            ...
            for clust, indices in chain(enumerate(maps, start=1),
                ...
                for i in indices:
                    ...
                    attr.attributes[_tr.m[2792, "cluster"]] = clust
                    attrs.append(attr)
    ...
    def send_report(self):
        ...
        if isinstance(self.annotation, str):
            annot = annot.lower()
        if self.selection_method == 0:
            sel = _tr.m[2793, "manual"]
        elif self.selection_method == 1:
            sel = _tr.m[2794, "at {:.1f} of height"].format(self.cut_ratio)
        else:
            sel = _tr.e(_tr.c(2795, f"top {self.top_n} {pl(self.top_n, 'cluster')}"))
        self.report_items((
            (_tr.m[2796, "Linkage"], LINKAGE[self.linkage]),
            (_tr.m[2797, "Annotation"], annot),
            (_tr.m[2798, "Pruning"],
             self.pruning != 0 and _tr.m[2799, "{} levels"].format(self.max_depth)),
            (_tr.m[2800, "Selection"], sel),
        ))
        self.report_plot()
