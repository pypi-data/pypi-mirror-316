...
class OWSOM(OWWidget):
    name = _tr.m[3018, "Self-Organizing Map"]
    description = _tr.m[3019, "Computation of self-organizing map."]
    icon = "icons/SOM.svg"
    keywords = _tr.m[3020, "self-organizing map, som"]

    class Inputs:
        data = Input(_tr.m[3021, "Data"], Table)

    class Outputs:
        selected_data = Output(_tr.m[3022, "Selected Data"], Table, default=True)
    ...
    class Information(OWWidget.Information):
        modified = Msg(
            (_tr.m[3023, 'The parameter settings have been changed. Press "Start" to '] + _tr.m[3024, "rerun with the new settings."])
        )

    class Warning(OWWidget.Warning):
        ignoring_disc_variables = Msg(_tr.m[3025, "SOM ignores categorical variables."])
        missing_colors = \
            Msg(_tr.m[3026, "Some data instances have undefined value of '{}'."])
        no_defined_colors = \
            Msg(_tr.m[3027, "'{}' has no defined values."])
        missing_values = Msg("{}")
        single_attribute = Msg(_tr.m[3028, "Data contains a single numeric column."])

    class Error(OWWidget.Error):
        no_numeric_variables = Msg(_tr.m[3029, "Data contains no numeric columns."])
        not_enough_data = Msg(_tr.m[3030, "SOM needs at least two data rows without missing values."])
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, box=_tr.m[3031, "SOM"])
        shape = gui.comboBox(
            box, self, "", items=(_tr.m[3032, "Hexagonal grid"], _tr.m[3033, "Square grid"]))
        ...
        auto_dim = gui.checkBox(
            box2, self, "auto_dimension", _tr.m[3034, "Set dimensions automatically"],
        ...
        initialization = gui.comboBox(
            ...
            self,
            "initialization",
            items=(_tr.m[3035, "Initialize with PCA"], _tr.m[3036, "Random initialization"], _tr.m[3037, "Replicable random"]),
            callback=self.on_parameter_change,
        )

        start = gui.button(
            box, self, _tr.m[3038, "Restart"], callback=self.restart_som_pressed,
        ...
        box = gui.vBox(self.controlArea, _tr.m[3039, "Color"])
        gui.comboBox(
            box, self, "attr_color", searchable=True,
            callback=self.on_attr_color_change,
            model=DomainModel(placeholder=_tr.m[3040, "(Same color)"],
                              valid_types=DomainModel.PRIMITIVE))
        gui.checkBox(
            box, self, "pie_charts", label=_tr.m[3041, "Show pie charts"],
            callback=self.on_pie_chart_change)
        gui.checkBox(
            box, self, "size_by_instances", label=_tr.m[3042, "Size by number of instances"],
        ...
        def set_warnings():
            ...
            if missing:
                self.Warning.missing_values(
                    _tr.e(_tr.c(3043, f'{missing} data {pl(missing, "instance")} with undefined value(s) {pl(missing, "is|are")} not shown.')))
    ...
    def enable_controls(self, enable):
        ...
        c.start.setText(_tr.m[3044, "Start"] if enable else _tr.m[3045, "Stop"])
    ...
    def _draw_same_color(self, sizes):
        ...
        for y in range(self.size_y):
            for x in range(self.size_x - self.hexagonal * (y % 2)):
                ...
                ellipse.setToolTip(_tr.e(_tr.c(3046, f"{n} instances")))
    ...
    def _tooltip(self, colors, distribution):
        ...
        values = list(values) + [_tr.m[3047, "(N/A)"]]
    ...
    def update_output(self):
        ...
        if ngroups > 1:
            sel_domain = make_domain(grp_values + [_tr.m[3048, "Unselected"], ], ngroups, 1)
        else:
            sel_domain = make_domain((_tr.m[3049, "No"], _tr.m[3050, "Yes"]), 0, 0)
    ...
    def send_report(self):
        ...
        if self.colors:
            self.report_caption(
                _tr.e(_tr.c(3051, f"Self-organizing map colored by '{self.attr_color.name}'")))
