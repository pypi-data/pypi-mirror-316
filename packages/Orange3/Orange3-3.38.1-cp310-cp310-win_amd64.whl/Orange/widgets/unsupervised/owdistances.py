...
MetricDefs: Dict[int, MetricDef] = {
    metric.id: metric for metric in (
        MetricDef(EuclideanNormalized, _tr.m[2674, "Euclidean (normalized)"],
                  _tr.m[2675, "Square root of summed difference between normalized values"],
                  distance.Euclidean, normalize=True),
        MetricDef(Euclidean, _tr.m[2676, "Euclidean"],
                  _tr.m[2677, "Square root of summed difference between values"],
                  distance.Euclidean),
        MetricDef(ManhattanNormalized, _tr.m[2678, "Manhattan (normalized)"],
                  _tr.m[2679, "Sum of absolute differences between normalized values"],
                  distance.Manhattan, normalize=True),
        MetricDef(Manhattan, _tr.m[2680, "Manhattan"],
                  _tr.m[2681, "Sum of absolute differences between values"],
                  distance.Manhattan),
        MetricDef(Mahalanobis, _tr.m[2682, "Mahalanobis"],
                  _tr.m[2683, "Mahalanobis distance"],
                  distance.Mahalanobis),
        MetricDef(Hamming, _tr.m[2684, "Hamming"], _tr.m[2685, "Hamming distance"],
                  distance.Hamming),
        MetricDef(Cosine, _tr.m[2686, "Cosine"], _tr.m[2687, "Cosine distance"],
                  distance.Cosine),
        MetricDef(Pearson, _tr.m[2688, "Pearson"],
                  _tr.m[2689, "Pearson correlation; distance = 1 - ρ/2"],
                  distance.PearsonR),
        MetricDef(PearsonAbsolute, _tr.m[2690, "Pearson (absolute)"],
                  _tr.m[2691, "Absolute value of Pearson correlation; distance = 1 - |ρ|"],
                  distance.PearsonRAbsolute),
        MetricDef(Spearman, _tr.m[2692, "Spearman"],
                  _tr.m[2693, "Spearman correlation; distance = 1 - ρ/2"],
                  distance.SpearmanR),
        MetricDef(SpearmanAbsolute, _tr.m[2694, "Spearman (absolute)"],
                  _tr.m[2695, "Absolute value of Pearson correlation; distance = 1 - |ρ|"],
                  distance.SpearmanRAbsolute),
        MetricDef(Jaccard, _tr.m[2696, "Jaccard"], _tr.m[2697, "Jaccard distance"],
                  distance.Jaccard)
    )
}
...
class DistanceRunner:
    ...
    def run(data: Orange.data.Table, metric: distance, normalized_dist: bool,
        ...
        state.set_status(_tr.m[2698, "Calculating..."])
...
class OWDistances(OWWidget, ConcurrentWidgetMixin):
    name = _tr.m[2699, "Distances"]
    description = _tr.m[2700, "Compute a matrix of pairwise distances."]
    icon = "icons/Distance.svg"
    keywords = _tr.m[2701, "distances"]

    class Inputs:
        data = Input(_tr.m[2702, "Data"], Orange.data.Table)

    class Outputs:
        distances = Output(_tr.m[2703, "Distances"], Orange.misc.DistMatrix, dynamic=False)
    ...
    class Error(OWWidget.Error):
        no_continuous_features = Msg(_tr.m[2704, "No numeric features"])
        no_binary_features = Msg(_tr.m[2705, "No binary features"])
        dense_metric_sparse_data = Msg(_tr.m[2706, "{} requires dense data."])
        distances_memory_error = Msg(_tr.m[2707, "Not enough memory"])
        distances_value_error = Msg(_tr.m[2708, "Problem in calculation:\n{}"])
        data_too_large_for_mahalanobis = Msg(
            _tr.m[2709, "Mahalanobis handles up to 1000 {}."])

    class Warning(OWWidget.Warning):
        ignoring_discrete = Msg(_tr.m[2710, "Ignoring categorical features"])
        ignoring_nonbinary = Msg(_tr.m[2711, "Ignoring non-binary features"])
        unsupported_sparse = Msg((_tr.m[2712, "Some metrics don't support sparse data\n"] + _tr.m[2713, "and were disabled: {}"]))
        imputing_data = Msg(_tr.m[2714, "Missing values were imputed"])
        no_features = Msg(_tr.m[2715, "Data has no features"])
    ...
    def __init__(self):
        ...
        gui.radioButtons(
            self.controlArea, self, "axis", [_tr.m[2716, "Rows"], _tr.m[2717, "Columns"]],
            box=_tr.m[2718, "Compare"], orientation=Qt.Horizontal, callback=self._invalidate
        )
        box = gui.hBox(self.controlArea, _tr.m[2719, "Distance Metric"])
        ...
        def _check_tractability():
            if metric is distance.Mahalanobis:
                if self.axis == 0:
                    ...
                    if len(data) > 1000:
                        self.Error.data_too_large_for_mahalanobis(_tr.m[2720, "rows"])
                        return False
                else:
                    if len(data.domain.attributes) > 1000:
                        self.Error.data_too_large_for_mahalanobis(_tr.m[2721, "columns"])
                        return False
            return True
    ...
    def send_report(self):
        # pylint: disable=invalid-sequence-index
        self.report_items((
            (_tr.m[2722, "Distances Between"], [_tr.m[2723, "Rows"], _tr.m[2724, "Columns"]][self.axis]),
            (_tr.m[2725, "Metric"], MetricDefs[self.metric_id].name)
        ))
