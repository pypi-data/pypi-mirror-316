...
def format_time_diff(start, end, round_up_after=2):
    ...
    if years >= round_up_after:
        return _tr.e(_tr.c(836, f'~{years} years'))
    elif months >= round_up_after:
        return _tr.e(_tr.c(837, f'~{months} months'))
    elif weeks >= round_up_after:
        return _tr.e(_tr.c(838, f'~{weeks} weeks'))
    elif days >= round_up_after:
        return _tr.e(_tr.c(839, f'~{days} days'))
    elif hours >= round_up_after:
        return _tr.e(_tr.c(840, f'~{hours} hours'))
    elif minutes >= round_up_after:
        return _tr.e(_tr.c(841, f'~{minutes} minutes'))
    else:
        return _tr.e(_tr.c(842, f'{seconds} seconds'))
...
class FeatureStatisticsTableModel(AbstractSortTableModel):
    ...
    class Columns(IntEnum):
        ...
        def name(self):
            return {self.ICON: '',
                    self.NAME: _tr.m[843, 'Name'],
                    self.DISTRIBUTION: _tr.m[844, 'Distribution'],
                    self.CENTER: _tr.m[845, 'Mean'],
                    self.MODE: _tr.m[846, 'Mode'],
                    self.MEDIAN: _tr.m[847, 'Median'],
                    self.DISPERSION: _tr.m[848, 'Dispersion'],
                    self.MIN: _tr.m[849, 'Min.'],
                    self.MAX: _tr.m[850, 'Max.'],
                    self.MISSING: _tr.m[851, 'Missing'],
                    }[self.value]
    ...
    def get_statistics_table(self):
        ...
        if any(contivars):
            ...
            attrs = [ContinuousVariable(name)
                     for name in (_tr.m[852, "Entropy"], self.Columns.MISSING.name)]
        ...
        meta_attrs = [StringVariable(_tr.m[853, 'Feature']), StringVariable(_tr.m[854, 'Mode'])]

        domain = Domain(attributes=attrs, metas=meta_attrs)
        statistics = Table.from_numpy(domain, x, metas=metas)
        statistics.name = _tr.e(_tr.c(855, f'{self.table.name} (Feature Statistics)'))
        return statistics
    ...
    def _argsortData(self, data, order):
        if data.ndim == 1:
            ...
            assert np.issubdtype(data.dtype, np.number), \
                ('We do not deal with non numeric values in sorting by ' + 'multiple values')
            if order == Qt.DescendingOrder:
                data[:, -1] = -data[:, -1]
...
class OWFeatureStatistics(widget.OWWidget):
    name = _tr.m[856, 'Feature Statistics']
    description = _tr.m[857, 'Show basic statistics for data features.']
    icon = 'icons/FeatureStatistics.svg'

    class Inputs:
        data = Input(_tr.m[858, 'Data'], Table, default=True)

    class Outputs:
        reduced_data = Output(_tr.m[859, 'Reduced Data'], Table, default=True)
        statistics = Output(_tr.m[860, 'Statistics'], Table)
