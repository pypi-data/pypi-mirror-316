...
def run_mds(matrix: DistMatrix, max_iter: int, step_size: int, init_type: int,
    ...
    iterations_done = 0
    init = embedding
    state.set_status(_tr.m[2932, "Running..."])
...
class OWMDS(OWDataProjectionWidget, ConcurrentWidgetMixin):
    name = _tr.m[2933, "MDS"]
    description = (_tr.m[2934, "Two-dimensional data projection by multidimensional "] + _tr.m[2935, "scaling constructed from a distance matrix."])
    icon = "icons/MDS.svg"
    keywords = _tr.m[2936, "mds, multidimensional scaling, multi dimensional scaling"]

    class Inputs(OWDataProjectionWidget.Inputs):
        distances = Input(_tr.m[2937, "Distances"], DistMatrix)
    ...
    #: Refresh rate
    RefreshRate = [
        (_tr.m[2938, "Every iteration"], 1),
        (_tr.m[2939, "Every 5 steps"], 5),
        (_tr.m[2940, "Every 10 steps"], 10),
        (_tr.m[2941, "Every 25 steps"], 25),
        (_tr.m[2942, "Every 50 steps"], 50),
        (_tr.m[2943, "None"], -1)
    ]
    ...
    class Error(OWDataProjectionWidget.Error):
        not_enough_rows = Msg(_tr.m[2944, "Input data needs at least 2 rows"])
        matrix_not_symmetric = Msg(_tr.m[2945, "Distance matrix is not symmetric"])
        matrix_too_small = Msg(_tr.m[2946, "Input matrix must be at least 2x2"])
        no_attributes = Msg(_tr.m[2947, "Data has no attributes"])
        mismatching_dimensions = \
            Msg(_tr.m[2948, "Data and distances dimensions do not match."])
        out_of_memory = Msg(_tr.m[2949, "Out of memory"])
        optimization_error = Msg(_tr.m[2950, "Error during optimization\n{}"])
    ...
    def _add_controls(self):
        ...
        self.gui.add_control(
            self._effects_box, gui.hSlider, _tr.m[2952, "Show similar pairs:"],
    ...
    def _add_controls_optimization(self):
        ...
        box = gui.vBox(self.controlArea, box=_tr.m[2953, "Optimize"], spacing=0)
        hbox = gui.hBox(box, margin=0)
        gui.button(hbox, self, _tr.m[2954, "PCA"], callback=self.do_PCA, autoDefault=False)
        gui.button(hbox, self, _tr.m[2955, "Randomize"], callback=self.do_random, autoDefault=False)
        gui.button(hbox, self, _tr.m[2956, "Jitter"], callback=self.do_jitter, autoDefault=False)
        gui.separator(box, height=18)
        grid = QGridLayout()
        gui.widgetBox(box, orientation=grid)
        self.run_button = gui.button(None, self, _tr.m[2957, "Start"], self._toggle_run)
        ...
        grid.addWidget(QLabel(_tr.m[2958, "Refresh:"]), 1, 0)
        grid.addWidget(
            gui.comboBox(
            ...
            1, 1)
        self.stress_label = QLabel(_tr.m[2959, "Kruskal Stress: -"])
    ...
    def _toggle_run(self):
        if self.task is not None:
            self.cancel()
            self.run_button.setText(_tr.m[2960, "Resume"])
    ...
    def _run(self, steps=None):
        if self.effective_matrix is None \
            ...
            return
        self.run_button.setText(_tr.m[2961, "Stop"])
        ...
        init_type = _tr.m[2962, "PCA"] if self.initialization == OWMDS.PCA else "random"
    ...
    def on_done(self, result: Result):
        ...
        self.run_button.setText(_tr.m[2963, "Start"])
        self.step_button.setEnabled(True)
        self.commit.deferred()
    ...
    def update_stress(self):
        ...
        self.stress_label.setText(_tr.e(_tr.c(2964, f"Kruskal Stress: {stress_val}")))
    ...
    def on_exception(self, ex: Exception):
        ...
        self.run_button.setText(_tr.m[2965, "Start"])
        self.step_button.setEnabled(True)
    ...
    def do_initialization(self, init_type: int):
        self.run_button.setText(_tr.m[2966, "Start"])
        self.step_button.setEnabled(True)
    ...
    def get_size_data(self):
        if self.attr_size == _tr.m[2967, "Stress"]:
