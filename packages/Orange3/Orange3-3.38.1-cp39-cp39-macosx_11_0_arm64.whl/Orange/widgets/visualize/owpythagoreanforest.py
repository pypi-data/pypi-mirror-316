...
class OWPythagoreanForest(OWWidget):
    name = _tr.m[3708, "Pythagorean Forest"]
    description = _tr.m[3709, 'Pythagorean forest for visualising random forests.']
    icon = 'icons/PythagoreanForest.svg'
    settings_version = 2
    keywords = _tr.m[3710, "pythagorean forest, fractal"]

    priority = 1001

    class Inputs:
        random_forest = Input(_tr.m[3711, "Random Forest"], RandomForestModel, replaces=[_tr.m[3712, "Random forest"]])

    class Outputs:
        tree = Output(_tr.m[3713, "Tree"], TreeModel)
    ...
    SIZE_CALCULATION = [
        (_tr.m[3714, 'Normal'], lambda x: x),
        (_tr.m[3715, 'Square root'], lambda x: sqrt(x)),
        (_tr.m[3716, 'Logarithmic'], lambda x: log(x + 1)),
    ]
    ...
    def __init__(self):
        ...
        # CONTROL AREA
        # Tree info area
        box_info = gui.widgetBox(self.controlArea, _tr.m[3717, 'Forest'])
        self.ui_info = gui.widgetLabel(box_info)

        # Display controls area
        box_display = gui.widgetBox(self.controlArea, _tr.m[3718, 'Display'])
        ...
        self.ui_depth_slider = gui.hSlider(
            box_display, self, 'depth_limit', label=_tr.m[3719, 'Depth'], ticks=False,
            maxValue=900
        )  # type: QSlider
        self.ui_target_class_combo = gui.comboBox(
            box_display, self, 'target_class_index', label=_tr.m[3720, 'Target class'],
            orientation=Qt.Horizontal, items=[], contentsLength=8,
            searchable=True
        )
        self.ui_size_calc_combo = gui.comboBox(
            box_display, self, 'size_calc_idx', label=_tr.m[3721, 'Size'],
            orientation=Qt.Horizontal,
            items=list(zip(*self.SIZE_CALCULATION))[0], contentsLength=8,
        )
        self.ui_zoom_slider = gui.hSlider(
            box_display, self, 'zoom', label=_tr.m[3722, 'Zoom'], ticks=False, minValue=100,
    ...
    def _update_info_box(self):
        self.ui_info.setText(_tr.m[3723, 'Trees: {}'].format(len(self.forest.trees)))
    ...
    def _update_target_class_combo(self):
        ...
        if self.instances.domain.has_discrete_class:
            label_text = _tr.m[3724, 'Target class']
            values = [c.title() for c in self.instances.domain.class_vars[0].values]
            values.insert(0, _tr.m[3725, 'None'])
        else:
            label_text = _tr.m[3726, 'Node color']
    ...
    def _clear_info_box(self):
        self.ui_info.setText(_tr.m[3727, 'No forest on input.'])
    ...
    def send_report(self):
        ...
        def item_html(row):
            ...
            return (f'<img style="width:100%" ' + f'src="data:image/png;base64,{img_encoded}"/>')
