...
class ScoringSheetTable(QTableWidget):
    def __init__(self, main_widget, parent=None):
        ...
        self.setHorizontalHeaderLabels([_tr.m[3800, "Attribute Name"], _tr.m[3801, "Points"], _tr.m[3802, "Selected"]])
...
class RiskSlider(QWidget):
    ...
    def setup_labels(self):
        ...
        self.points_label = QLabel(_tr.m[3803, "<b>Total:</b>"])
        ...
        self.probability_label = QLabel(_tr.m[3804, "<b>Probabilities (%):</b>"])
    ...
    def handle_hover_event(self, pos):
        ...
        if thumbRect.contains(pos) and self.points:
            ...
            tooltip = str(
                (f"<b>{self.target_class}</b>\n " + (f"<hr style='margin: 0px; padding: 0px; border: 0px; height: 1px; background-color: #000000'>" + (_tr.e(_tr.c(3805, f"<b>Points:</b> {int(points)}<br>")) + _tr.e(_tr.c(3806, f"<b>Probability:</b> {probability:.1f}%")))))
            )
...
class OWScoringSheetViewer(OWWidget):
    ...
    name = _tr.m[3807, "Scoring Sheet Viewer"]
    description = _tr.m[3808, "Visualize the scoring sheet model."]
    ...
    class Inputs:
        classifier = Input(_tr.m[3809, "Classifier"], Model)
        data = Input(_tr.m[3810, "Data"], Table)

    class Outputs:
        features = Output(_tr.m[3811, "Features"], AttributeList)

    target_class_index = ContextSetting(0)

    class Error(OWWidget.Error):
        invalid_classifier = Msg(
            _tr.m[3812, "Scoring Sheet Viewer only accepts a Scoring Sheet model."]
        )

    class Information(OWWidget.Information):
        multiple_instances = Msg(
            _tr.m[3813, "The input data contains multiple instances. Only the first instance will be used."]
        )
    ...
    def _setup_gui(self):
        ...
        combo_box_layout.layout().addWidget(QLabel(_tr.m[3814, "Target class:"]))
