...
class OWTreeGraph(OWTreeViewer2D):
    ...
    name = _tr.m[3866, "Tree Viewer"]
    icon = "icons/TreeViewer.svg"
    priority = 35
    keywords = _tr.m[3867, "tree viewer"]

    class Inputs:
        # Had different input names before merging from
        # Classification/Regression tree variants
        tree = Input(_tr.m[3868, "Tree"], TreeModel, replaces=["Classification Tree", "Regression Tree"])

    class Outputs:
        selected_data = Output(_tr.m[3869, "Selected Data"], Table, default=True, id="selected-data")
    ...
    COL_OPTIONS = [_tr.m[3870, "Default"], _tr.m[3871, "Number of instances"], _tr.m[3872, "Mean value"], _tr.m[3873, "Variance"]]
    ...
    def __init__(self):
        ...
        self.color_label = QLabel(_tr.m[3874, "Target class: "])
        ...
        gui.checkBox(box, self, "show_intermediate",
                     _tr.m[3875, "Show details in non-leaves"],
    ...
    def ctree(self, model=None):
        ...
        if model is None:
            self.infolabel.setText(_tr.m[3876, 'No tree.'])
            self.root_node = None
            self.dataset = None
            self.tree_adapter = None
        else:
            ...
            if class_var.is_discrete:
                self.color_label.setText(_tr.m[3877, "Target class: "])
                self.color_combo.addItem(_tr.m[3878, "None"])
                self.color_combo.addItems(self.domain.class_vars[0].values)
                self.color_combo.setCurrentIndex(self.target_class_index)
            else:
                self.color_label.setText(_tr.m[3879, "Color by: "])
            ...
            self.infolabel.setText(_tr.e(_tr.c(3880, f'{nodes} {pl(nodes, "node")}, {leaves} {pl(leaves, "leaf|leaves")}')))
        self.setup_scene()
    ...
    def node_tooltip(self, node):
        ...
        if rule:
            rule = _tr.e(_tr.c(3881, f"<p><b>Selection</b></p><p>{rule}</p>"))
        ...
        if self.domain.class_var.is_discrete:
            ...
            content = _tr.e(_tr.c(3882, f"{nbp}<b>Distribution of</b> '{name}'</p><p>")) \
                + "<table>" + "".join(
                    ("<tr>" + (f"<td><span style='color: {color_to_hex(color)}'>◼</span> " + (f"{escape(value)}</td>" + (f"<td>{indent}</td>" + (f"<td align='right'>{prop:g}</td>" + (f"<td>{indent}</td>" + (f"<td align='right'>{prop / total * 100:.1f} %</td>" + "</tr>")))))))
            ...
            content = f"{nbp}{class_var.name} = {mean:.3g} ± {var:.3g}<br/>" + \
                _tr.e(_tr.c(3883, f"({self.tree_adapter.num_samples(node.node_inst)} instances)</p>"))

        split = self._update_node_info_attr_name(node, "")
        if split:
            split = _tr.e(_tr.c(3884, f"<p style='white-space:pre'><b>Next split: </b>{split}</p>"))
    ...
    def send_report(self):
        if not self.model:
            return
        items = [(_tr.m[3885, "Tree size"], self.infolabel.text()),
                 (_tr.m[3886, "Edge widths"],
                  (_tr.m[3887, "Fixed"], _tr.m[3888, "Relative to root"], _tr.m[3889, "Relative to parent"])[
                      # pylint: disable=invalid-sequence-index
                      self.line_width_method])]
        if self.domain.class_var.is_discrete:
            items.append((_tr.m[3890, "Target class"], self.color_combo.currentText()))
        elif self.regression_colors != self.COL_DEFAULT:
            items.append((_tr.m[3891, "Color by"], self.COL_OPTIONS[self.regression_colors]))
    ...
    def node_content_reg(self, node):
        ...
        text += _tr.e(_tr.c(3892, f"{insts} instances"))
        return text
