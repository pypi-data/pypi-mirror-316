...
class OWSVM(OWBaseLearner):
    name = 'SVM'
    description = (_tr.m[2465, "Support Vector Machines map inputs to higher-dimensional "] + _tr.m[2466, "feature spaces."])
    icon = "icons/SVM.svg"
    replaces = [
    ...
    priority = 50
    keywords = _tr.m[2467, "svm, support vector machines"]

    LEARNER = SVMLearner

    class Outputs(OWBaseLearner.Outputs):
        support_vectors = Output(_tr.m[2468, "Support Vectors"], Table, explicit=True,
                                 replaces=[_tr.m[2469, "Support vectors"]])

    class Warning(OWBaseLearner.Warning):
        sparse_data = Msg(_tr.m[2470, 'Input data is sparse, default preprocessing is to scale it.'])
    ...
    _default_gamma = "auto"
    kernels = ((_tr.m[2471, "Linear"], "x⋅y"),
               (_tr.m[2472, "Polynomial"], "(g x⋅y + c)<sup>d</sup>"),
               ("RBF", "exp(-g|x-y|²)"),
               (_tr.m[2473, "Sigmoid"], "tanh(g x⋅y + c)"))
    ...
    def _add_type_box(self):
        ...
        self.type_box = box = gui.radioButtonsInBox(
            self.controlArea, self, "svm_type", [], box=_tr.m[2474, "SVM Type"],
        ...
        form.addWidget(QLabel(_tr.m[2475, "Cost (C):"]), 0, 1, Qt.AlignRight)
        form.addWidget(self.c_spin, 0, 2)
        form.addWidget(QLabel(
            _tr.m[2476, "Regression loss epsilon (ε):"]), 1, 1, Qt.AlignRight)
        ...
        form.addWidget(QLabel(_tr.m[2477, "Regression cost (C):"]), 2, 1, Qt.AlignRight)
        form.addWidget(self.nu_C_spin, 2, 2)
        form.addWidget(QLabel(_tr.m[2478, "Complexity bound (ν):"]), 3, 1, Qt.AlignRight)
    ...
    def _add_kernel_box(self):
        ...
        box = gui.hBox(self.controlArea, _tr.m[2479, "Kernel"])
        ...
        gui.label(parambox, self, _tr.m[2480, "Kernel: %(kernel_eq)s"])
    ...
    def get_learner_parameters(self):
        items = OrderedDict()
        if self.svm_type == self.SVM:
            items[_tr.m[2484, "SVM type"]] = "SVM, C={}, ε={}".format(self.C, self.epsilon)
        else:
            items[_tr.m[2485, "SVM type"]] = "ν-SVM, ν={}, C={}".format(self.nu, self.nu_C)
        self._report_kernel_parameters(items)
        items[_tr.m[2486, "Numerical tolerance"]] = "{:.6}".format(self.tol)
        items[_tr.m[2487, "Iteration limt"]] = self.max_iter if self.limit_iter else _tr.m[2488, "unlimited"]
        return items

    def _report_kernel_parameters(self, items):
        gamma = self.gamma or self._default_gamma
        if self.kernel_type == 0:
            items[_tr.m[2489, "Kernel"]] = _tr.m[2490, "Linear"]
        elif self.kernel_type == 1:
            items[_tr.m[2491, "Kernel"]] = \
                _tr.m[2492, "Polynomial, ({g:.4} x⋅y + {c:.4})<sup>{d}</sup>"].format(
                    g=gamma, c=self.coef0, d=self.degree)
        elif self.kernel_type == 2:
            items[_tr.m[2493, "Kernel"]] = "RBF, exp(-{:.4}|x-y|²)".format(gamma)
        else:
            items[_tr.m[2494, "Kernel"]] = _tr.m[2495, "Sigmoid, tanh({g:.4} x⋅y + {c:.4})"].format(
