...
class OWDistanceMatrix(widget.OWWidget):
    name = _tr.m[2654, "Distance Matrix"]
    description = _tr.m[2655, "View distance matrix."]
    icon = "icons/DistanceMatrix.svg"
    priority = 200
    keywords = _tr.m[2656, "distance matrix"]

    class Inputs:
        distances = Input(_tr.m[2657, "Distances"], DistMatrix)

    class Outputs:
        distances = Output(_tr.m[2658, "Distances"], DistMatrix, dynamic=False)
        table = Output(_tr.m[2659, "Selected Data"], Table, replaces=[_tr.m[2660, "Table"]])

    class Error(widget.OWWidget.Error):
        empty_matrix = widget.Msg(_tr.m[2661, "Distance matrix is empty."])
    ...
    def __init__(self):
        ...
        self.annot_combo = gui.comboBox(
            self.buttonsArea, self, "annotation_idx", label=_tr.m[2662, "Labels: "],
            orientation=Qt.Horizontal,
            callback=self._invalidate_annotations, contentsLength=12)
        self.annot_combo.setModel(VariableListModel())
        self.annot_combo.model()[:] = [_tr.m[2663, "None"], _tr.m[2664, "Enumeration"]]
    ...
    def set_distances(self, distances):
        ...
        annotations = [_tr.m[2665, "None"], _tr.m[2666, "Enumerate"]]
        view = self.tableview
        ...
        if distances is not None:
            ...
            if not distances.is_symmetric():
                ...
                if distances.row_items is not None \
                    ...
                    annotations.append(_tr.m[2667, "Labels"])
                    pending_idx = 2
            else:
                ...
                if items and not distances.axis:
                    annotations.append(_tr.m[2668, "Attribute names"])
                    pending_idx = 2
                elif isinstance(items, list) and \
                        all(isinstance(item, Variable) for item in items):
                    annotations.append(_tr.m[2669, "Name"])
                    pending_idx = 2
    ...
    def _choose_label(data: Table):
        attr = max((attr for attr in data.domain.metas
                   ...
                   default=None)
        return attr or data.domain.class_var or _tr.m[2670, "Enumerate"]
    ...
    def _update_labels(self):
        ...
        elif self.annot_combo.model()[self.annotation_idx] == _tr.m[2671, "Attribute names"]:
            attr = self.distances.row_items.domain.attributes
            ver_labels = hor_labels = [
                str(attr[i]) for i in range(self.distances.shape[0])]

        elif self.annot_combo.model()[self.annotation_idx] == _tr.m[2672, "Labels"]:
...
if __name__ == "__main__":  # pragma: no cover
    ...
    data = Orange.data.Table(_tr.m[2673, "zoo"])
