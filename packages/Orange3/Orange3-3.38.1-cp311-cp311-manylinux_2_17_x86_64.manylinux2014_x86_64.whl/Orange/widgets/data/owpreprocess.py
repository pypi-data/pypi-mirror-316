...
class DiscretizeEditor(BaseEditor):
    ...
    Names = {
        NoDisc: _tr.m[1214, "None"],
        EqualWidth: _tr.m[1215, "Equal width discretization"],
        EqualFreq: _tr.m[1216, "Equal frequency discretization"],
        Drop: _tr.m[1217, "Remove numeric features"],
        EntropyMDL: _tr.m[1218, "Entropy-MDL discretization"]
    }
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.__slbox = slbox = QGroupBox(
            title=_tr.m[1219, "Number of intervals (for equal width/frequency)"],
            flat=True
        )
    ...
    def __repr__(self):
        n_int = _tr.m[1220, ", Number of intervals: {}"].format(self.__nintervals) \
...
class ContinuizeEditor(BaseEditor):
    ...
    Continuizers = OrderedDict([
        (Continuize.FrequentAsBase, _tr.m[1221, "Most frequent is base"]),
        (Continuize.Indicators, _tr.m[1222, "One feature per value"]),
        (Continuize.RemoveMultinomial, _tr.m[1223, "Remove non-binary features"]),
        (Continuize.Remove, _tr.m[1224, "Remove categorical features"]),
        (Continuize.AsOrdinal, _tr.m[1225, "Treat as ordinal"]),
        (Continuize.AsNormalizedOrdinal, _tr.m[1226, "Divide by number of values"])
    ])
...
class RemoveSparseEditor(BaseEditor):
    options = [_tr.m[1227, "missing values"], _tr.m[1228, "zeros"]]
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.layout().addWidget(QLabel(_tr.m[1229, "Remove features with too many"]))
        ...
        filter_settings = QGroupBox(title=_tr.m[1230, 'Threshold:'], flat=True)
        ...
        btn_perc = QRadioButton(self, text=_tr.m[1231, 'Percentage'], checked=not self.useFixedThreshold)
        ...
        btn_fix = QRadioButton(self, text=_tr.m[1232, 'Fixed'], checked=self.useFixedThreshold)
    ...
    def __repr__(self):
        desc = _tr.e(_tr.c(1233, f"remove features with too many {self.options[self.filter0]}, threshold: "))
        if self.useFixedThreshold:
            desc += _tr.e(_tr.c(1234, f"{self.fixedThresh} {pl(self.fixedThresh, 'instance')}"))
        else:
...
class ImputeEditor(BaseEditor):
    ...
    Names = {
        NoImputation: _tr.m[1235, "Don't impute."],
        Constant: _tr.m[1236, "Replace with constant"],
        Average: _tr.m[1237, "Average/Most frequent"],
        Model: _tr.m[1238, "Model based imputer"],
        Random: _tr.m[1239, "Replace with random value"],
        DropRows: _tr.m[1240, "Remove rows with missing values."],
    }
...
class UnivariateFeatureSelect(QWidget):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        box = QGroupBox(title=_tr.m[1241, "Score"], flat=True)
        ...
        box = QGroupBox(title=_tr.m[1242, "Number of features"], flat=True)
        self.__group = group = QButtonGroup(self, exclusive=True)
        self.__spins = {}

        form = QFormLayout()
        fixedrb = QRadioButton(_tr.m[1243, "Fixed:"], checked=True)
        ...
        percrb = QRadioButton(_tr.m[1244, "Proportion:"])
...
class FeatureSelectEditor(BaseEditor):
    ...
    MEASURES = [
        (_tr.m[1245, "Information Gain"], preprocess.score.InfoGain),
        (_tr.m[1246, "Gain ratio"], preprocess.score.GainRatio),
        (_tr.m[1247, "Gini index"], preprocess.score.Gini),
        ("ReliefF", preprocess.score.ReliefF),
        (_tr.m[1248, "Fast Correlation Based Filter"], preprocess.score.FCBF),
        ("ANOVA", preprocess.score.ANOVA),
        ("Chi2", preprocess.score.Chi2),
        ("RReliefF", preprocess.score.RReliefF),
        (_tr.m[1249, "Univariate Linear Regression"], preprocess.score.UnivariateLinearRegression)
    ]
    ...
    def __init__(self, parent=None):
        ...
        self.__uni_fs.setItems(
            [{"text": _tr.m[1250, "Information Gain"], "tooltip": ""},
             {"text": _tr.m[1251, "Gain Ratio"]},
             {"text": _tr.m[1252, "Gini Index"]},
             {"text": "ReliefF"},
             {"text": _tr.m[1253, "Fast Correlation Based Filter"]},
             {"text": "ANOVA"},
             {"text": "Chi2"},
             {"text": "RReliefF"},
             {"text": _tr.m[1254, "Univariate Linear Regression"]}
            ]
        )
    ...
    def __repr__(self):
        ...
        return _tr.m[1255, "Score: {}, Strategy (Fixed): {}"].format(
...
class RandomFeatureSelectEditor(BaseEditor):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        box = QGroupBox(title=_tr.m[1256, "Number of features"], flat=True)
        self.__group = group = QButtonGroup(self, exclusive=True)
        self.__spins = {}

        form = QFormLayout()
        fixedrb = QRadioButton(_tr.m[1257, "Fixed"], checked=True)
        ...
        percrb = QRadioButton(_tr.m[1258, "Percentage"])
    ...
    def __repr__(self):
        if self.__strategy == self.Fixed:
            ...
            num = self.__k
            return _tr.e(_tr.c(1259, f"select {num} {pl(num,'feature')}"))
        else:
            perc = self.__p
            return _tr.e(_tr.c(1260, f"select {perc} % features"))
...
class Scale(BaseEditor):
    ...
    Names = {
        NormalizeBySD: _tr.m[1261, "Standardize to μ=0, σ²=1"],
        CenterByMean: _tr.m[1262, "Center to μ=0"],
        ScaleBySD: _tr.m[1263, "Scale to σ²=1"],
        NormalizeSpan_NonZeroBased: _tr.m[1264, "Normalize to interval [-1, 1]"],
        NormalizeBySpan_ZeroBased: _tr.m[1265, "Normalize to interval [0, 1]"],
    }
...
class Randomize(BaseEditor):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.__rand_type_cb.addItems([_tr.m[1266, "Classes"],
                                      _tr.m[1267, "Features"],
                                      _tr.m[1268, "Meta data"]])
        ...
        form.addRow(_tr.m[1269, "Randomize:"], self.__rand_type_cb)
        form.addRow(_tr.m[1270, "Replicable shuffling:"], self.__rand_seed_ch)
    ...
    def __repr__(self):
        return "{}, {}".format(self.__rand_type_cb.currentText(),
                               _tr.m[1271, "Replicable"] if self.__rand_seed_ch.isChecked()
                               else _tr.m[1272, "Not replicable"])
...
class PCA(BaseEditor):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        form.addRow(_tr.m[1273, "Components:"], self.cspin)
        self.layout().addLayout(form)
    ...
    def __repr__(self):
        return _tr.m[1274, "Components: {}"].format(self.cspin.value())
...
class CUR(BaseEditor):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        form.addRow(_tr.m[1275, "Rank:"], self.rspin)
        form.addRow(_tr.m[1276, "Relative error:"], self.espin)
        self.layout().addLayout(form)
    ...
    def __repr__(self):
        return _tr.m[1277, "Rank: {}, Relative error: {}"].format(self.rspin.value(),
                                                     self.espin.value())
...
PREPROCESS_ACTIONS = [
    PreprocessAction(
        _tr.m[1278, "Discretize"], "orange.preprocess.discretize", _tr.m[1279, "Discretization"],
        Description(_tr.m[1280, "Discretize Continuous Variables"],
                    icon_path("Discretize.svg")),
        DiscretizeEditor
    ),
    PreprocessAction(
        _tr.m[1281, "Continuize"], "orange.preprocess.continuize", _tr.m[1282, "Continuization"],
        Description(_tr.m[1283, "Continuize Discrete Variables"],
                    icon_path("Continuize.svg")),
        ContinuizeEditor
    ),
    PreprocessAction(
        _tr.m[1284, "Impute"], "orange.preprocess.impute", _tr.m[1285, "Impute"],
        Description(_tr.m[1286, "Impute Missing Values"],
                    icon_path("Impute.svg")),
        ImputeEditor
    ),
    PreprocessAction(
        _tr.m[1287, "Feature Selection"], "orange.preprocess.fss", _tr.m[1288, "Feature Selection"],
        Description(_tr.m[1289, "Select Relevant Features"],
                    icon_path("SelectColumns.svg")),
        FeatureSelectEditor
    ),
    PreprocessAction(
        _tr.m[1290, "Random Feature Selection"], "orange.preprocess.randomfss",
        _tr.m[1291, "Random Feature Selection"],
        Description(_tr.m[1292, "Select Random Features"],
                    icon_path("SelectColumnsRandom.svg")),
        RandomFeatureSelectEditor
    ),
    PreprocessAction(
        _tr.m[1293, "Normalize"], "orange.preprocess.scale", _tr.m[1294, "Scale"],
        Description(_tr.m[1295, "Normalize Features"],
                    icon_path("Normalize.svg")),
        Scale
    ),
    PreprocessAction(
        _tr.m[1296, "Randomize"], "orange.preprocess.randomize", _tr.m[1297, "Randomization"],
        Description(_tr.m[1298, "Randomize"],
                    icon_path("Random.svg")),
        Randomize
    ),
    PreprocessAction(
        _tr.m[1299, "Remove Sparse"], "orange.preprocess.remove_sparse", _tr.m[1300, "Feature Selection"],
        Description(_tr.m[1301, "Remove Sparse Features"],
                    icon_path("PurgeDomain.svg")),
        RemoveSparseEditor
    ),
    PreprocessAction(
        "PCA", "orange.preprocess.pca", "PCA",
        Description(_tr.m[1302, "Principal Component Analysis"],
                    icon_path("PCA.svg")),
        PCA
    ),
    PreprocessAction(
        _tr.m[1303, "CUR"], "orange.preprocess.cur", _tr.m[1304, "CUR"],
        Description(_tr.m[1305, "CUR Matrix Decomposition"],
                    icon_path("SelectColumns.svg")),
        CUR
    )
]
...
class OWPreprocess(widget.OWWidget, openclass=True):
    name = _tr.m[1306, "Preprocess"]
    description = _tr.m[1307, "Construct a data preprocessing pipeline."]
    category = _tr.m[1308, "Transform"]
    icon = "icons/Preprocess.svg"
    priority = 2100
    keywords = _tr.m[1309, "preprocess, process"]

    settings_version = 2

    class Inputs:
        data = Input(_tr.m[1310, "Data"], Orange.data.Table)

    class Outputs:
        preprocessor = Output(_tr.m[1311, "Preprocessor"], preprocess.preprocess.Preprocess, dynamic=False)
        preprocessed_data = Output(_tr.m[1312, "Preprocessed Data"], Orange.data.Table)
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[1313, "Preprocessors"])
        ...
        self.overlay.layout().addWidget(
            QLabel(_tr.m[1314, "Drag items from the list on the left"], wordWrap=True))
    ...
    def send_report(self):
        ...
        if pp:
            self.report_items(_tr.m[1315, "Settings"], pp)
