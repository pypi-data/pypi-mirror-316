...
class OWCreateInstance(OWWidget):
    name = _tr.m[449, "Create Instance"]
    description = _tr.m[450, "Interactively create a data instance from sample dataset."]
    icon = "icons/CreateInstance.svg"
    category = _tr.m[451, "Transform"]
    keywords = _tr.m[452, "create instance, simulator"]
    priority = 2310

    class Inputs:
        data = Input(_tr.m[453, "Data"], Table)
        reference = Input(_tr.m[454, "Reference"], Table)

    class Outputs:
        data = Output(_tr.m[455, "Data"], Table)

    class Information(OWWidget.Information):
        nans_removed = Msg((_tr.m[456, "Variables with only missing values were "] + _tr.m[457, "removed from the list."]))

    want_main_area = False
    BUTTONS = [_tr.m[458, "Median"], _tr.m[459, "Mean"], _tr.m[460, "Random"], _tr.m[461, "Input"]]
    ACTIONS = ["median", "mean", "random", "input"]
    HEADER = [["name", _tr.m[462, "Variable"]],
              ["variable", _tr.m[463, "Value"]]]
    Header = namedtuple(
    ...
    def __init__(self):
        ...
        gui.checkBox(self.buttonsArea, self, "append_to_data",
                     _tr.m[464, "Append this instance to input data"],
    ...
    def _append_to_data(self, instance: Table) -> Table:
        ...
        name = get_unique_names(self.data.domain, _tr.m[466, "Source ID"])
    ...
    def send_report(self):
        if not self.data:
            return
        self.report_domain(_tr.m[467, "Input"], self.data.domain)
        self.report_domain(_tr.m[468, "Output"], self.data.domain)
        items = []
        ...
        self.report_table(_tr.m[469, "Values"], items)
