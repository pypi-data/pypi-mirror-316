...
class OWRuleViewer(widget.OWWidget):
    name = _tr.m[3742, "CN2 Rule Viewer"]
    description = _tr.m[3743, "Review rules induced from data."]
    icon = "icons/CN2RuleViewer.svg"
    priority = 1140
    keywords = _tr.m[3744, "cn2 rule viewer"]

    class Inputs:
        data = Input(_tr.m[3745, "Data"], Table)
        classifier = Input(_tr.m[3746, "Classifier"], _RuleClassifier)

    class Outputs:
        selected_data = Output(_tr.m[3747, "Selected Data"], Table, default=True)
    ...
    def __init__(self):
        ...
        self.model.set_horizontal_header_labels(
            [_tr.m[3748, "IF conditions"], "", _tr.m[3749, "THEN class"], _tr.m[3750, "Distribution"],
             _tr.m[3751, "Probabilities [%]"], _tr.m[3752, "Quality"], _tr.m[3753, "Length"]])
        ...
        gui.checkBox(widget=self.buttonsArea, master=self, value="compact_view",
                     label=_tr.m[3754, "Compact view"], callback=self.on_update)
        gui.rubber(self.buttonsArea)

        original_order_button = gui.button(
            self.buttonsArea, self,
            _tr.m[3755, "Restore original order"],
            autoDefault=False,
    ...
    def send_report(self):
        if self.classifier is not None:
            self.report_table(_tr.m[3756, "Induced rules"], self.view)
...
class CustomRuleViewerTableModel(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        def _display_role():
            if column == 0:
                delim = _tr.m[3757, " AND "] if self._compact_view else _tr.m[3758, " AND\n"]
                return _tr.m[3759, "TRUE"] if not rule.selectors else delim.join(
        ...
        def _tooltip_role():
            if column == 0:
                return _display_role().replace(_tr.m[3760, " AND "], _tr.m[3761, " AND\n"])
            if column == 1:
                return None
            if column == 3:
                # list of int, float
