...
template <typename B, typename R, class API_polar>
Decoder_polar_SCL_MEM_fast_sys<B,R,API_polar>* Decoder_polar_SCL_MEM_fast_sys<B,R,API_polar>
::clone() const
{
	auto m = new Decoder_polar_SCL_MEM_fast_sys(*this);
	m->deep_copy(*this);
	return m;
}
...
template <typename B, typename R, class API_polar>
const std::vector<bool>& Decoder_polar_SCL_MEM_fast_sys<B,R,API_polar>
...
::update_paths_rep(const int r_d, const int off_l, const int off_s, const int n_elmts)
{
	...
	if (n_active_paths <= L / 2)
	{
		...
		{
			const auto path = paths[i];
			const auto new_path = duplicate_tree(path, off_l, off_s, r_d);
...
::update_paths_rep(const int off_l, const int off_s)
{
	...
	if (n_active_paths <= L / 2)
	{
		...
		{
			const auto path = paths[i];
			const auto new_path = duplicate_tree(path, off_l, off_s, REV_D);
