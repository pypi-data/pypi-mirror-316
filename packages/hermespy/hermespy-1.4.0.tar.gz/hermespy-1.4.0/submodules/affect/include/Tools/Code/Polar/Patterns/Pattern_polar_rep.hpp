/*!
 * \file
 * \brief Class tools::Pattern_polar_rep.
 */
...
{
protected:
	inline Pattern_polar_rep(const int &N, const Binary_node<Pattern_polar_i>* node,
	                         const int min_level = 1, const int max_level = -1);

public:
	inline Pattern_polar_rep(const int min_level = 1, const int max_level = -1);

	virtual inline Pattern_polar_rep* alloc(const int &N, const Binary_node<Pattern_polar_i>* node) const;
...
#include "Tools/Code/Polar/Patterns/Pattern_polar_rep.hxx"
#endif
