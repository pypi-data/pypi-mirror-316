...
class Encoder(nn.Module):
	def __init__(self, in_dim, layers, emb_dim, k): 
		...
		# 		self.hidden_layers.append(MolConv(in_dim=layers[i-1], out_dim=layers[i], k=k, remove_xyz=False))
		# v2: 
		self.hidden_layers = nn.ModuleList([MolConv2(in_dim=in_dim, out_dim=layers[0], k=k, remove_xyz=True)])
		for i in range(1, len(layers)): 
			if i == 1:
				self.hidden_layers.append(MolConv2(in_dim=layers[i-1], out_dim=layers[i], k=k, remove_xyz=False))
			else:
				self.hidden_layers.append(MolConv2(in_dim=layers[i-1], out_dim=layers[i], k=k, remove_xyz=False))
