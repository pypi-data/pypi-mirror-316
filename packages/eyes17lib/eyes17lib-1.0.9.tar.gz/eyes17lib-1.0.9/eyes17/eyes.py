...
class Interface():
    ...
    * loads calibration values.
    ...
    TMR3HLD = 0x0108
    T2CON = 0x0110
    def __prepareCounter__(self, chan):
        self.get_high_freq(chan)


    def startCounter(self,chan):
        ## REGISTERS
        self.__prepareCounter__(chan)
        self.__write_data_address__(self.TMR3HLD,0) #Hold.
        self.__write_data_address__(self.TMR2,0) #reset.

    def pauseCounter(self):
        t2con = self.read_data_address(self.T2CON)
        self.__write_data_address__(self.T2CON,t2con&0x7FFF) #clear bit 15.

    def resumeCounter(self):
        t2con = self.read_data_address(self.T2CON)
        self.__write_data_address__(self.T2CON,t2con|0x8000) #set bit 15.

    def getCounts(self):
        lsb = self.read_data_address(self.TMR2) #lsb. copies msb TMR3 to TMR3HLD
        msb = self.read_data_address(self.TMR3HLD)
        return (msb<<16) | lsb
    ...
    def set_state(self, **kwargs):
        ...
        **Arguments**
        ...
        data = 0
        if 'SQ1' in kwargs:
            kwargs['SQR1'] = kwargs.get('SQ1')
        if 'SQ2' in kwargs:
            kwargs['SQR2'] = kwargs.get('SQ2')
