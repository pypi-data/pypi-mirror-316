...
class RSGRUCell(layers.Layer):  
    def __init__(self, units, **kwargs):  
        super(RSGRUCell, self).__init__(**kwargs)  
        self.units = units  
        self.state_size = units  
        # 初始化权重  
        self.kernel = self.add_weight(shape=(units, units * 3), initializer='glorot_uniform', name='kernel')  
        self.recurrent_kernel = self.add_weight(shape=(units, units * 3), initializer='glorot_uniform', name='recurrent_kernel')  
        self.bias = self.add_weight(shape=(units * 3,), initializer='zeros', name='bias')  
  
    def call(self, inputs, states):  
        h_tm1 = states[0]  
        z = tf.sigmoid(tf.matmul(inputs, self.kernel) + tf.matmul(h_tm1, self.recurrent_kernel) + self.bias)  
        z_r, z_i, z_n = tf.split(z, num_or_size_splits=3, axis=1)  
        r = tf.sigmoid(z_r)  
        i = tf.sigmoid(z_i)  
        n = tf.tanh(z_n)  
        h = (1 - i) * h_tm1 + i * n  
...
class RSGRUCell(layers.Layer):
    def __init__(self, units, **kwargs):
        ...
        # Weights for standard GRU gates
        self.kernel = self.add_weight(shape=(units, units * 3), initializer='he_normal', name='kernel')
        self.recurrent_kernel = self.add_weight(shape=(units, units * 3), initializer='orthogonal', name='recurrent_kernel')
        self.bias = self.add_weight(shape=(units * 3,), initializer='zeros', name='bias')

        # Additional weights for retention and selection gates
        self.retention_kernel = self.add_weight(shape=(units, units), initializer='orthogonal', name='retention_kernel')
        self.retention_bias = self.add_weight(shape=(units,), initializer='zeros', name='retention_bias')
        self.selection_kernel = self.add_weight(shape=(units, units), initializer='orthogonal', name='selection_kernel')
        self.selection_bias = self.add_weight(shape=(units,), initializer='zeros', name='selection_bias')
    ...
    def call(self, inputs, states):
        ...
        candidate = tf.tanh(tf.matmul(inputs, self.kernel[:, self.units * 2:]) + 
                            tf.matmul(r * h_tm1, self.recurrent_kernel[:, self.units * 2:]) + 
                            self.bias[self.units * 2:])

        # Retention gate
        retention = tf.sigmoid(tf.matmul(h_tm1, self.retention_kernel) + self.retention_bias)
        retained_state = retention * h_tm1 + (1 - retention) * candidate

        # Selection gate
        selection_weights = tf.sigmoid(tf.matmul(retained_state, self.selection_kernel) + self.selection_bias)
        selected_state = tf.reduce_sum(selection_weights * retained_state, axis=1, keepdims=True)

        # Final update
        h = z * h_tm1 + (1 - z) * selected_state

        # Clipping to avoid NaN values
        h = tf.clip_by_value(h, clip_value_min=-10.0, clip_value_max=10.0)

        return h, [h]
class RSGRU(models.Model):  
    def __init__(self, units, input_shape):  
        super(RSGRU, self).__init__()  
        self.gru = layers.RNN(RSGRUCell(units), return_sequences=True, input_shape=input_shape)  
