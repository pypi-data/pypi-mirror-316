...
from decentnet.consensus.dev_constants import ENCRYPTION_DEBUG, RUN_IN_DEBUG
...
from decentnet.modules.convert.byte_to_base64_utils import *
...
class Beam:
    ...
    def init_comm_blockchain(self, insert: bool, do_metric: bool = None):
        """
    ...
    def set_encryptors(self, data_enc_pwd: bytes, encryption_algorithm: str, encryption_bits: int,
        ...
        if encryption_algorithm == "AES":
            ...
            if relay:
                self.encryptor_relay = ChaChaCipher(data_enc_pwd, encryption_bits)
            else:
                self.encryptor_beacon = ChaChaCipher(data_enc_pwd, encryption_bits)
        else:
    ...
    def __connect_incoming(self, genesis_block: Block, target_key: str):
        ...
        if USED_ALGORITHM == "AES":
            self.encryptor_relay = AESCipher(pwd, BEAM_AES_ENCRYPTION_KEY_SIZE)
        elif USED_ALGORITHM == "CC20":
            self.encryptor_relay = ChaChaCipher(pwd, BEAM_AES_ENCRYPTION_KEY_SIZE)
    ...
    def __send_data(self, data_raw: str | bytes, ack: bool, blockchain: Blockchain,
        ...
        response = self.client.send_message(packaged_data, ack=ack)
    ...
    def fetch_message(self, do_metrics=None) -> tuple[Block, dict, bytes | bytearray | str]:
        """
        ...
        if block.index == 0 and len(self.comm_bc) == 1:
            ...
            self.init_comm_blockchain(False, do_metric=do_metrics)
        ...
        if verified_csig is not None:  # Broadcast is faulty or handled incorrectly
            ...
            return self.fetch_message(do_metrics=do_metrics)
        elif block.index == 0:
    ...
    def handle_connection_data(self, incoming_data: dict, incoming_block: Block):
        ...
        if incoming_block.index == 0 and len(self.conn_bc) > 0:
            if (cmd := incoming_data.get("cmd", None)) and cmd == CMD.BROADCAST.value:
                asyncio.run(self.flow_net.add_edge(incoming_data["pub"], self.target_key,
                                                   DEFAULT_CAPACITY))
