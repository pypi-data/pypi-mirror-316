...
class BasicInterface:
    ...
    def _single_relay_connect_outgoing(self, owner_key_id: int, owner_enc_key_id: int) -> Tuple[
        ...
        if not beacon:
            ...
            exit(1)
        self._init_out_encryption(beam)
        return beacon, beam
    ...
    def _init_out_encryption(self, beam: Beam, difficulty: Difficulty = BlockchainParams.low_diff_argon):
        """
    ...
    def send(self, data: bytes, difficulty: Difficulty = BlockchainParams.low_diff_argon,
        ...
        self._beam_send.send_communication_data(e_data)

        if require_ack:
            dec_data = self._fetch_ack(self._beam_send)
            return dec_data["status"] == "OK"
        return None

    def fetch(self, return_latency_ms: bool = False) -> dict | None | Tuple[dict, float]:
        """
        ...
        block = self._beam_receive.fetch_message(do_metrics=False)[0]
        data = self._beam_receive.encryptor_beacon.decrypt(bytes(block.data))
        if data[-4:] == b'ACK!':
            self._beam_receive.send_communication_data(HANDSHAKE_MSG)
        try:
