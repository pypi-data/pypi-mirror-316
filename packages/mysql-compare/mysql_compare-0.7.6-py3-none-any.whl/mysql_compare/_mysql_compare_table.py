...
class MysqlCompareTable:
    ...
    def running_compare(self):
        with connect(**self.src_dsn) as source_con, connect(**self.dst_dsn) as target_con:
            ...
            while True:
                query_statement_src, query_params_src = get_query_full_table_statement_params(
                    self.src_db, self.src_tab, self.table_keys, ["*"], self.batch_size, self.proc_keyval
                )
                query_statement_dst, query_params_dst = get_query_full_table_statement_params(
                    self.dst_db, self.dst_tab, self.table_keys, ["*"], self.batch_size, self.proc_keyval
                )
                ...
                self.logger.debug(
                    f"batch_id[{_batch_id}] processed rows number: {_process_rows}/{self.table_rows_total}, progress: {processed_progress}%, elapsed time: {get_elapsed_time(_start1, 2)}s."
                )
                _batch_id += 1

                if self.is_repair >= 1 and len(_diff_all_rows) >= 10000:
                    ...
                    for diff_row in _diff_all_rows:
                        self.logger.debug(f"repair row: src[{diff_row}]")
                        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
    ...
    def start(self):
        ...
        self.logger.debug(f"table compare done, diff count: {self.diff_rows_cnt}")
