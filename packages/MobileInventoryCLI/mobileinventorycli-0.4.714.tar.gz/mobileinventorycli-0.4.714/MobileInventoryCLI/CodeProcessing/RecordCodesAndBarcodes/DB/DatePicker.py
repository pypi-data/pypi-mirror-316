...
from MobileInventoryCLI.CodeProcessing.RecordCodesAndBarcodes.DB.Prompt import Prompt
...
{Fore.light_salmon_1}{Style.underline}today {Style.reset}-{Fore.light_sea_green} t|today {Fore.orange_red_1}{Style.bold}({Fore.light_steel_blue}{datetime.today()}{Style.reset}{Style.bold}{Fore.orange_red_1}){Style.reset}
    ...
    while True:
        try:
            pass
            month=Prompt.__init2__(None,func=mkint,ptext=f"Month[0..12|Default:{date.today().month}]",helpText=f"month to be used in date returned{extra}",data='month')
            if month in [None,]:
                return None
            elif month in ['y',]:
               return mkYesterday()
