    ...
    def work_note(self):
        with Session(ENGINE) as session:
            while True:
                ...
                print(f"{Fore.light_yellow}#b{Fore.light_green} will cancel the current note and start a new note{Style.reset}")
                print(f"{Fore.light_yellow}f{Fore.light_green} will proceeds to review, where 'f'' finishes,'y/yes/1' will review,'<Enter>/<Return>/n/f/0' will act as finish{Style.reset}")
                print(f"{Fore.light_yellow}p{Fore.light_green} at field filling lines goes to previous field{Style.reset}")
                print(f"{Fore.light_yellow}d{Fore.light_green} use default{Style.reset}")
                print(f"{Fore.light_yellow}m{Fore.light_green} use manually entered data present under m key option{Style.reset}")
                ...
                print(f"{Fore.light_green}Processing Note for Barcodes...{Style.reset}")
                tmp=[]
                ...
                for xnum,i in enumerate(process):
                    results=session.query(Entry).filter(Entry.Barcode==i).all()
                    ct=len(results)
                    if ct > 0:
                        ...
                        for num,ii in enumerate(results):
                            ...
                            htext.append(msg)
                        htext='\n'.join(htext) 
                        print(f"There are {ct} Entry Results for - {Fore.light_magenta}{i}/{Fore.dark_goldenrod}{xnum}[Index]/{Fore.cyan}[Count]{xnum+1}{Fore.medium_violet_red} out of {len(process)}[Total] space-deliminated words searched in Entry.Barcode{Style.reset}")
                        selected=Prompt.__init2__(None,func=FormBuilderMkText,ptext="Select all that apply, separating with a comma:",helpText=htext,data="list")
                        if selected in [None,]:
                            if self.next_barcode():
                                continue
                            else:
                            ...
                            try:
                                for select in selected:
                                    try:
                                        ...
                                        related_text=''
                                        excludes=['EntryId',]
                                        fields=[i for i in Entry.__table__.columns if str(i.name) not in excludes]
                                        for field in fields:
                                            related_text+=f"\t-\t{field} = {getattr(results[select],str(field))}\n"
                                        header=f'{results[select].Barcode} - START\n'
                                        footer=f'{results[select].Barcode} - END\n'
                ...
                for i in process:
                    ...
                    if ct > 0:
                        ...
                        htext='\n'.join(htext)
                        print(htext)
                        print(f"There are {ct} Roster Results for - {Fore.light_magenta}{i}/{Fore.dark_goldenrod}{xnum}[Index]/{Fore.cyan}[Count]{xnum+1}{Fore.medium_violet_red} out of {len(process)}[Total] space-deliminated words searched in Roster{Style.reset}")
                        selected=Prompt.__init2__(None,func=FormBuilderMkText,ptext="Select all that apply, separating with a comma:",helpText=htext,data="list")
                        if selected in [None,]:
                            if self.next_barcode():
                                continue
                ...
                while True:
                    try:
                        when=Prompt.__init2__(None,func=FormBuilderMkText,ptext="When Did this happend?",helpText="when did this happen?",data="datetime")
                        if when in [None,]:
                            if self.next_barcode():
                                continue
                            else:
                ...
                while True:
                    try:
                        where=Prompt.__init2__(None,func=FormBuilderMkText,ptext="where Did this happend?",helpText="where did this happen?",data="string")
                        if where in [None,]:
                            if self.next_barcode():
                                continue
                            else:
...
{Fore.orange_red_1}'work note','wn','note','n','worknote','work_note','wn'{Fore.grey_70}- {Fore.light_green}Create a work note with fewer prompts{Style.reset}
{Fore.orange_red_1}'scn','searchnote','search note','sch nt','schnt'{Fore.grey_70}- {Fore.light_green}search note text{Style.reset}
            """
            while True:
                def mkT(text,self):
                    return text
