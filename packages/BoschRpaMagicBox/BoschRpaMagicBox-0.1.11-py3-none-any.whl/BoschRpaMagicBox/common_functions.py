...
class WebSapCommonFunctions:
    ...
    def fill_input_filed_with_single_value(self, single_value: str, css_selector: str, is_enter: bool = False, is_tab=False, click_tip: bool = False,
                                           tip_css_selector='', target_element=None):
    ...
    def input_filed_single_value(self, field_title, field_index, field_value, is_enter=False, is_tab=False, need_click_tip=False):
        ...
        if target_input_element is not None:
            ...
            sleep(1)
            self.fill_input_filed_with_single_value(field_value, '', is_enter, is_tab, need_click_tip, '', target_input_element)
            sleep(1)
    ...
    def check_button_popup_and_click(self, button_title, try_times=10):
        ...
        while try_time <= try_times:
            ...
            try:
                ...
                self.browser.find_element(By.CSS_SELECTOR, button_css_selector)
            except NoSuchElementException:
                sleep(1)
                try_time += 1
            else:
                self.click_or_input_by_css_selector(button_css_selector, 'click')
                sleep(1)
                self.wait_element_invisible_by_css_selector(button_css_selector)
                sleep(1)
                break
        ...
        for button_index, multiple_selection_button in enumerate(multiple_selection_buttons):
            if button_index + 1 == multiple_selection_index:
                ...
                if paste_method == 'clipboard':
                    ...
                    self.copy_paste_values(value_list, input_css_path, second_paste=second_paste)
                # elif paste_method == 'file':
                #     upload_file_path = self.copy_paste_values_by_file(value_list)
                #     os.remove(upload_file_path)
    ...
    def copy_paste_values(self, value_list: list, input_css_selector: str, wait_alert: bool = True, is_enter: bool = True, second_paste=False):
        ...
        self.press_keyboard_shortcut([Keys.CONTROL, 'v'])
        sleep(2)
        ...
        sleep(1)
        try:
            first_row_element = self.browser.find_element(by=By.CSS_SELECTOR, value=input_css_selector)
