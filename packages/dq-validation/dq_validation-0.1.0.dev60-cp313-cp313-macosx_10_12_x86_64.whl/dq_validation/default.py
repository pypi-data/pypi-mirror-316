...
def create_pandas_schema(config: ValidationConfig):
...
def read_csv(path: str, config: ValidationConfig) -> Optional[pd.DataFrame]:
    try:
        ...
        df = pd.read_csv(
            path,
            skip_blank_lines=True,  # NOTE: this behavior must match Rust and Spark
            header=None,
            names=columns,
            dtype=dtype,
            # If we have lines with more cols than specified in the validation
            # config, then the rust pipeline will write them out the same way,
            # but pandas won't be able to read them.
            # Since there is no way why these rows should go into the final output
            # (either validation failed, in which case we don't output a file,
            # or dropInvalidRows is turned on), it is safe to skip them here.
            on_bad_lines="skip",
        )
        df.columns = columns
        return df
