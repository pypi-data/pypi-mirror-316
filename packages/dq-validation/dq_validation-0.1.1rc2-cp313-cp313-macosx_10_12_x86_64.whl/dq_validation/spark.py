...
def write_df_as_single_file(df: DataFrame, path: str, temp_dir: str):
    ...
    with tempfile.TemporaryDirectory(dir=temp_dir) as d:
        ...
        df.write.csv(csv_parts_dir, header=None)
        csv_parts = [
...
def run(
    ...
    # Environment variables with which to override spark settings.
    if bool(os.environ.get("VALIDATION_SPARK_HEAP_SIZE_EXTRA_ROOM")):
        heap_size_extra_room = int(os.environ["VALIDATION_SPARK_HEAP_SIZE_EXTRA_ROOM"])
    else:
        heap_size_extra_room = None
    ...
    with spark_session(
        temp_dir,
        name="Validation",
        input_files=[annotated_dataset_path] if os.path.exists(annotated_dataset_path) else None,
        config=list(spark_settings.items()),
        heap_size_extra_room=heap_size_extra_room,
    ) as ss:
        dataset_schema = create_spark_schema_from_validation_config(
            config, string_only=True
        )
        column_names = [f.name for f in dataset_schema]
        string_column_names = [
            column_names[jx]
