...
class OpenWeatherMapThread(RCloudThread):
...
class OpenWeatherMap(RCloud):
    ...
    def run(self) -> None:
        ...
        self.worker = cast(
            OpenWeatherMapThread, Base.instantiate(OpenWeatherMap.workerThreadId)
        )
        self.worker.init(
            self.forecast_topic,
            self.location,
    ...
    def from_dict(self, data: Dict[str, Any]) -> None:
        ...
        if "_openweathermap" in data:
            for key, value in data["_openweathermap"].items():
