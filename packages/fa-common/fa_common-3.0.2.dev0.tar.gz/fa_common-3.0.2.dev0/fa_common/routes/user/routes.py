...
from fa_common.routes.user.models import AppDB, RoleDB, UserDB
from fa_common.routes.user.service import create_app, get_app, get_current_app_user, list_apps, update_app
from fa_common.routes.user.types import BaseRole, PermissionDef, UpdateUser, UpdateUserMe
...
async def find_users_route(
    name: str = "", limit: int = 5, current_user: UserDB = Security(get_current_app_user, scopes=[get_admin_scope()])
) -> List[UserDB]:
    return await service.find_users(name, limit)
...
async def patch_user(
    ...
    return await service.update_user(user, update_data=user_in.model_dump(exclude_unset=True))
