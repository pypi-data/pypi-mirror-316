...
from fa_common.routes.user.types import BaseRole, PermissionDef
...
async def get_current_app_user(
    ...
    if current_user is not None and current_user.sub is not None:
        ...
        if user is not None and isinstance(user, UserDB):
            ...
            await user.refresh_permissions()
...
async def create_user(new_user: AuthUser) -> UserDB:
    ...
    if get_settings().USE_APP_ROLES:
        ...
        await user.update_scopes(save=False)
...
async def delete_role(role_name: str):
    ...
    if role is not None:
        ...
        await UserDB.find(UserDB.app_roles.name == role.name).update({"$pull": {"appRoles": {"name": role.name}}})
