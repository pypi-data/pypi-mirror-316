...
class ArgoClient(WorkflowBaseClient):
    ...
    async def run_job(self, job_base: JobTemplate, verbose: bool = False) -> WorkflowRun:
        ...
        return await self.submit(template, config.base.namespace, verbose=verbose, verify_ssl=config.base.verify_ssl)
    ...
    async def _prepare_job(
        ...
        if params.get("upload-base-path", None) and params.get("upload-base-path") != "":
            default_path = f'{params["upload-base-path"]}/{workflow_id}/{node.pod_name}'
        if params.get("upload-custom-path", None) and params.get("upload-custom-path") != "":
