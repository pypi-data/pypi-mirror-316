...
class ScriptRunner(QtWidgets.QMainWindow):
    ...
    def takeScreenshot(self):
        ...
        options.setInitialDir(os.path.expanduser('~'))
        file_name, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save Screenshot", "", "Images (*.png);;All Files (*)", options=options)
        if file_name:
            self.central_widget.grab().save(file_name)
        ...
        if self.editMode:
            if self.selected_script:
                ...
                if arg_spec.varkw: #kwargs specified in the Expt class init
                    ...
                    kwargs = {'scope_thread':self.scope_thread}
    ...
    def load_script(self, opt, title=None, helpname=None,calc=False, **kwargs):
        print("load_script", opt, title, helpname, calc, kwargs)
