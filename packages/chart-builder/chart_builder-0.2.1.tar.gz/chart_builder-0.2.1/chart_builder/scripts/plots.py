...
from chart_builder.scripts.utils import dynamic_parameters, top_other_by_col_bubble, top_by_col_bubble, colors, clean_values, clean_values_dollars, ranked_cleaning, to_percentage, rank_by_col, rank_by_columns, normalize_to_percent 
...
def sorted_multi_line(df, title, save=False, colors=combined_colors, mode='lines', col=None, sort_col=None,
                      ...
                      annotations=False,max_annotation=False):
...
def bubble_chart(df, groupby, num_col, keep_topn=False, other=False, topn=10, title='l1_fdv_cluster',
                 scaling=dict(marker_scale=2e10), seed=42, use_physics=True,
                 show_legend=False, marker=dict(sizemin=10, mode="markers+text", opacity=1),
                 text=dict(general=18, annotation=18), annotation_dict=dict(yshift=-20), groupby_color=None, scale_y=False,x_num_col=None,
                    tickprefix=None,ticksuffix=None,exclude_largest=False):
    ...
    if groupby_color:
        unique_categories = pos_df[groupby_color].unique()
        category_color_map = {category: combined_colors[idx % len(combined_colors)] for idx, category in enumerate(unique_categories)}
    ...
    for idx, (_, row) in enumerate(pos_df.iterrows()):
        ...
        print(F'groupby_color, groupby: {row[groupby_color], row[groupby]}')
        ...
        legend_sum = legend_sums[legend_sums[groupby_color] == row[groupby_color]][num_col].values[0]
        print(f'color: {color}')
        ...
        size = row[num_col] / scaling['marker_scale']
        if size <= 0:
        ...
        # legend_marker_size = 20  # Fixed size for legend items

        # if row[groupby_color] not in categories_in_legend:
    ...
    margin_t = 100 if show_legend else 0
    ...
    fig.update_layout(
        ...
        width=730,
        height=351,  # Adjust as needed
    )

    # Save the figure as an SVG
    pio.write_image(fig, f'../img/{title}.svg')
