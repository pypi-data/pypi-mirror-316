...
@overload
def grid(
    data: Iterable[Tuple[float, float, float]],
    extent: Optional[Tuple[float, float, float, float]] = None,
    crs: Union[int, CRS, None] = None,
) -> Grid:
    """Creates a new grid from an iterable of tuples (x, y, value) using radial basis function (RBF) interpolation.
...
def grid(
    ...
    match data:
        ...
        case _:
            from glidergun._functions import interp_rbf

            return interp_rbf(data, extent, get_crs(crs or 4326))  # type: ignore
