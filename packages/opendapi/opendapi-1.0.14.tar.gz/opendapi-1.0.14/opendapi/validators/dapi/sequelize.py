...
class SequelizeDapiValidator(PackageScopedDapiValidatorBase):
    """Validator class for DAPI files created for Sequelize v6 models"""

    INTEGRATION_NAME = "sequelize"
    ...
    def get_sequelize_tables(self) -> List[Table]:
        ...
        def delegate(filename, node, _):
            ...
            if self.is_sequelize_define_call(node):
                table = SequelizeDefineTable(
                    ...
                    table_config.override.playbooks,
                    project,
                    node,
                )

                if is_model_in_allowlist(
                    table.name, table.filename, table_config.override.model_allowlist
                ):
                    tables.append(table)
        ...
        for class_name in model_classes:
            ...
            if is_model_in_allowlist(
                table.name,
                table.filename,
                table_config.override.model_allowlist,
            ):
                tables.append(table)
        return tables
