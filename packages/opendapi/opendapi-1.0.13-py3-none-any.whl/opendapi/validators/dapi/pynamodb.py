...
class PynamodbProjectInfo(ProjectInfo):
    ...
    def _get_tables(self):
        ...
        for import_string in self.override.artifact_path.split(","):
    ...
    def tables(self) -> List["Table"]:
        ...
        if process.exitcode:
            raise ImportError(
                f"Error generating tables: {self.override.artifact_path} in {self.full_path}"
            )
...
class PynamodbDapiValidator(DapiValidator):
    ...
    def get_project(self, override_config: OverrideConfig) -> PynamodbProjectInfo:
        ...
        override.artifact_path = override.artifact_path or self.APP_IDENTIFIERS
    ...
    def validate_projects(self, projects: List[PynamodbProjectInfo]):
    ...
    def _get_dapis_for_project(self, project: PynamodbProjectInfo) -> Dict[str, Dict]:
        ...
        for table in project.tables:
            ...
            if not is_model_in_allowlist(
                table_name, project.full_path, project.override.model_allowlist
            ):
                continue
            ...
            entry = {
                ...
                "dapi": {
                    ...
                    "owner_team_urn": construct_owner_team_urn_from_playbooks(
                        project.override.playbooks, table_name, project.full_path
                    ),
                    "description": None,
                    "type": "entity",
                    "datastores": self.build_datastores_for_table(project, table_name),
