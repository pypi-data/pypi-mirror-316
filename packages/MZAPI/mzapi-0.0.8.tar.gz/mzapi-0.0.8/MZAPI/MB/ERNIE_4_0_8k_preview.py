...
class ERNIE_4_0_8K_preview:
    """
    ERNIE_4_0_8K 类用于与百度ERNIE 4.0 8K模型进行交互。
    ...
    def __init__(self, client_name, ak, sk):
        ...
        self.apm_client = APMClient(
            ...
            host_name="ERNIE-4.0-8K-preview",
            token="kCrxvCIYEzhZfAHETXEB",
            peer_service="ERNIE-4.0-8K-preview",
            peer_instance=" 180.97.107.95:443",
            peer_address=" 180.97.107.95",
            peer_ipv6="240e:ff:e020:934:0:ff:b0dc:3636",
            http_host="https://aip.baidubce.com/rpc/2.0/ai_custom/v1/wenxinworkshop/chat/ernie-4.0-8k-preview",
            server_name="MZAPI",
        )
    ...
    def get_response(
        ...
        with self.tracer.start_as_current_span("ERNIE-4.0-8K") as span:
            ...
            self.log.start_process_log(response.json(), "ERNIE-4.0-8K-preview", W)
            M = response.json()
            uuid1 = Request_id
            question = data
            answer = M.get("result")
            ...
            self.sql.get_response(uuid1, question, answer, W, self.ip)
            return X
