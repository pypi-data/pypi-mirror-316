...
def _url(
    ...
    source: Optional[str] = "pyavrio-sqlalchemy",
    ...
    platform: Optional[str] = "data_products"
) -> str:
    """
    ...
    # Ensure source is added first if provided
    trino_url += f"?source={quote_plus(source)}"

    # Add platform as an additional query parameter
    trino_url += f"&platform={quote_plus(platform)}"
