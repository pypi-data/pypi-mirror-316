...
class DataforseoLabsAppleAppCompetitorsLiveItem(BaseModel):
    """
    ...
    avg_position: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="average position of the app in App Store SERP Note: average position is calculated for intersected keywords only; the value for a given application may differ when combined with different target applications")
    sum_position: Optional[StrictInt] = Field(default=None, description="sum of all app positions in App Store SERP Note: sum position is calculated for intersected keywords only; the value for a given application may differ when combined with different target applications")
    intersections: Optional[StrictInt] = Field(default=None, description="number of intersecting keywords")
    competitor_metrics: Optional[AppStoreMetricsBundleInfo] = None
    full_metrics: Optional[AppStoreMetricsBundleInfo] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "competitor_metrics": AppStoreMetricsBundleInfo.from_dict(obj["competitor_metrics"]) if obj.get("competitor_metrics") is not None else None,
            "full_metrics": AppStoreMetricsBundleInfo.from_dict(obj["full_metrics"]) if obj.get("full_metrics") is not None else None
        })
        return _obj
