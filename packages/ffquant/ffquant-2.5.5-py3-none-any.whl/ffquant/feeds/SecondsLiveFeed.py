...
class SecondsLiveFeed(bt.feeds.DataBase):
                ...
                if last_time_open is not None:
                    if time_open > last_time_open + 60 * 1000:
                        ...
                        while missing_ts < time_open:
                            if self.p.debug:
                                stdout_log(f"SecondsLiveFeed, missing kline: {missing_ts} {datetime.fromtimestamp(missing_ts / 1000, timezone.utc).astimezone().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def backpeek_for_result(self, current_kline_time):
        ...
        for i in range(1, self.p.backpeek_size):
            k = (current_kline_time - timedelta(seconds=i * 5)).strftime('%Y-%m-%d %H:%M:%S')
            backpeek_v = self.cache.get(k, None)
            if backpeek_v is not None:
                if self.p.debug:
                    stdout_log(f"{self.__class__.__name__}, backpeek index: {i}, datetime: {(current_kline_time - timedelta(seconds=i * 5)).astimezone().strftime('%Y-%m-%d %H:%M:%S')}, backpeek_v: {backpeek_v}")
