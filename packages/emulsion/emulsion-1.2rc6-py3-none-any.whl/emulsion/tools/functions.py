...
def balinski_young_algorithm(Nb, proportions):
    """Balinksi-Young algorithm is used in apportionment problems. It follows the quota rule (all values are either
    the floor or ceiling of the unrounded allocation) and is free of the Alabama paradox (ensures that if N increases,
    nobody loses a seat.

    Parameters
    ----------
    Nb: int
        the amount to divide according to the proportions
    proportions: list
        an int list or array of the proportions (sums to 1)

    Returns
    -------
    array:
        the list of all amounts (sums to Nb) fitting the proportions as fairly as possible

    """
    # if sum(proportions)
    # compute ideal values
    N = int(Nb)
    ideal_values = [N * p for p in proportions]
    rounded_values = np.round(ideal_values).astype(int)
    # compute difference to target
    current_sum = np.sum(rounded_values)
    difference = N - current_sum
    # adjust to reach target
    if difference != 0:
        rounding_errors = ideal_values - rounded_values
        # sort indices by decreasing rounding error
        # if equal errors, highest values are adjusted first
        adjustment_indices = np.argsort(rounding_errors) if difference > 0 else np.argsort(-rounding_errors)
        # adjust values
        if abs(difference) > len(adjustment_indices):
            raise(SemanticException(f'The proportions {proportions} cannot lead to a fair apportionment. Please ensure that proportions sum to 1'))
        for i in range(abs(difference)):
            rounded_values[adjustment_indices[i]] += np.sign(difference)
...
def AND(*conditions):
    """Return a logical AND (conjunction) between all the conditions.
...
def OR(*conditions):
    """Return a logical OR (disjunction) between all the conditions.
...
def PROPORTION(N, pos, *proportions):
    """Return the integer corresponding to the proportion of N of rank pos, based on the given proportions.
    The calculation is based on the Balinski-Young algorithm.
    ...
    pos: int
        the rank of the proportion to consider (between 1 and K)
    proportions: list
        list of K proportions (with a sum = 1)

    Returns
    -------
    int:
        the integer nearest to proportions[pos] * N according to a fair apportionment (considering other proportions)
    """
    if (pos < 1) or (pos > len(proportions)):
        raise(SemanticException(f'Incorrect index {pos} for PROPORTION. Value should be between 1 and {len(proportions)}'))
    return balinski_young_algorithm(N, proportions)[pos - 1]
...
PROPORTION.__USER_FUNCTION__ = ['Functions Available for Models']
