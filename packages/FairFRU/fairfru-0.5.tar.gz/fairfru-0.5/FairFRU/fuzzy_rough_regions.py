...
class FRU:
  ...
  def membership_values(self, 
    ...
    if not complete:
      ...
      Writearray(self.df[columns], alpha, 'full', method=similarity, target).sim_array(h5file = h5file, group = group, hide_progress = hide_progress)
    ...
    for s_attr in columns:
      ...
      Writearray(dataset, alpha, s_attr, method=similarity, target).sim_array(h5file = h5file, group = group, hide_progress = hide_progress)
