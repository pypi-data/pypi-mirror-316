...
from test_fixtures import PATH_TO_SPRINKLER_LEARNING_PAYLOAD
...
def test_send_valid_http_requests():
    ...
    try:
        for method in ["GET", "get", "PUT", "put", "POST", "post"]:
            response = send_http_request(default_agent_config.agent_url, method, "test")
            assert isinstance(
                response, Response
...
def test_send_invalid_http_requests():
    with pytest.raises(
        ValueError,
        match="Invalid HTTP request method. Must be 'GET', 'PUT', or 'POST'.",
    ):
        send_http_request(default_agent_config.agent_url, "unsupported_method", "test")


def test_inference_payload_valid():
    payload = inference_payload("var1", {"evidence1": 1})
    assert payload == {
        "variable_id": "var1",
...
def test_control_payload_with_array():
    payload = control_payload(np.array([1, 0, 0]), 3)
    assert payload == {"library": "pymdp", "observation": [1, 0, 0], "policy_len": 3}
...
def test_plot_categorical_simple():
    ...
    ) as mock_xticks, patch("matplotlib.pyplot.xlabel") as mock_xlabel, patch(
        "matplotlib.pyplot.ylabel"
    ) as mock_ylabel, patch("matplotlib.pyplot.title") as mock_title, patch(
