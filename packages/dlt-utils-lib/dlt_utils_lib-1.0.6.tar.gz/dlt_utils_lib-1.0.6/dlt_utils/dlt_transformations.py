...
def update_cdc_timestamp(df: DataFrame, time_diff_threshold: int) -> DataFrame:
    ...
    timestamp_cols = [col.name for col in df.schema.fields if isinstance(col.dataType, (TimestampType, DateType)) and col.name not in  ['cdc_timestamp', 'commit_timestamp']]
