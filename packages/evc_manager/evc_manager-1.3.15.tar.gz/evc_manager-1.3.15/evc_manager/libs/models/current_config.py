...
class CurrentConfig(object):
    ...
    def backend_evc_id(self, backend_evc_id):
        """Setter"""
        try:
            # OESS has evc_id in INT.
            self._backend_evc_id = evaluate_integer(backend_evc_id, "backend_evc_id")
        except ValueError:
            # Kytos has evc_id in HEX.
            self._backend_evc_id = evaluate_str(backend_evc_id, "backend_evc_id")
    ...
    def get_current_path(self):
        """Return current path"""
        return " ".join([nni.name for nni in self.current_path])
