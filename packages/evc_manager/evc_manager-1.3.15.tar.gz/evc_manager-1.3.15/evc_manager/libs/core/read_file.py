...
def read_file(source_file=None):
    ...
    warn(f"Opening file {source_file}")
    ...
    with open(source_file, "r") as stream:
        try:
            ...
            info(f"Error: on file {source_file}: {err}. Exiting now.")
    ...
    for path in paths:
        ...
        for subpath in path:
            if isinstance(subpath, dict):
                if "avoid" in subpath:
                    info(f'links to avoid: {subpath["avoid"]}')
                if "include" in subpath:
                    info(f'links to include: {subpath["include"]}')
...
def evaluate_input(yaml_input_content):
    ...
    if yaml_input_content["action"] not in ["add", "add_range", "delete", "change", "move"]:
        raise ValueError(
            ...
            "Valid options: add, add_range, delete, change, move"  # pylint: disable=W1404
        )
