...
def create_template(version, action):
    ...
    for name in ["a", "z"]:
        uni = dict()
        uni["device"] = f"device_name_{name}"
        uni["interface_name"] = f"interface_name_{name}"
        uni["tag"] = dict()
        uni["tag"]["type"] = "vlan"
        if action == "add_range":
...
def create_template_files(version="1.0", parent_dir="./", folder="evc_manager_templates"):
    ...
    for action in ["add", "add_range", "delete", "move", "change"]:
        ...
        filename = f"{folder}/evc_{action}"
        with open(filename, "w") as template_file:
            dump(
                create_template(version, action),
                template_file,
...
def fix_add_range_quotes(filename):
    ...
    if filename.find("add_range") > 0:
        sed_option = "s/\\'//g"
        bash_command = f'sed "{sed_option}" {filename}'
        os.system(f"{bash_command} > {filename}.tmp")
