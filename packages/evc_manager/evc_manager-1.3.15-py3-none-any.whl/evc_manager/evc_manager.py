...
class EvcManager(object):
    ...
    def add_evc(self, new_evc):
        ...
        if not found_evc:
            info(f"Creating EVC {new_evc.name}...")
            return self.backend.add_evc(new_evc)
        else:
            return {
                "result": "error",
                "msg": (
                    f"EVC {new_evc.name} has the same UNIs/VLANs as"
                    f" an existing EVC named {found_evc.name}"
                ),
            }
    ...
    def change_evc(self, evc_to_change):
        ...
        if found_evc:
            info(f"Changing EVC {evc_to_change.name}...")
            ...
            return {"result": "error", "msg": f"EVC {evc_to_change.name} does not exist."}
    ...
    def delete_evc(self, evc_to_delete):
        ...
        if found_evc:
            ...
            return {"result": "error", "msg": f"EVC {evc_to_delete.name} not found/deleted."}
    ...
    def move_evc(self, evc_to_move):
        ...
        if found_evc:
            ...
            return self.backend.move_evc(
                evc_to_move, found_evc.get_primary_path(), found_evc.get_backup_path(), nni
            )
        else:
            return {"result": "error", "msg": f"EVC {evc_to_move.name} does not exist."}
