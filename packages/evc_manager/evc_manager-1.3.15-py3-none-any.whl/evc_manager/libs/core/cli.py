...
def create_parser():
    ...
    actions.add_argument(
        "-L", "--list-evc", help="List all EVCs and output them using YAML", action="store_true"
    )
    actions.add_argument(
        "-A", "--add-evc", help="Add EVCs provided in the YAML file", action="store_true"
    )
    actions.add_argument(
        "-R",
        "--add-range-evcs",
    ...
    actions.add_argument(
        "-C", "--change-evc", help="Change EVCs provided in the YAML file", action="store_true"
    )
    actions.add_argument(
        "-D", "--delete-evc", help="Delete all EVCs provided in the YAML file", action="store_true"
    )
    actions.add_argument(
        "-M", "--move-evc", help="Move one or all EVCs out the NNI provided.", action="store_true"
    )
    actions.add_argument(
        "-X",
        "--template",
    ...
    parser.add_argument("--move-from-nni", help="Move out of the provided NNI")

    parser.add_argument(
        "-f", "--source-file", help="Source YAML file used by options -A, -R, -C, -D or -M"
    )
    parser.add_argument(
        "-F",
        "--destination-file",
    ...
    filters.add_argument("--has-uni-device", help="Filter output based on the UNI's device")
    filters.add_argument("--has-uni-interface", help="Filter output based on the UNI's interface")
    filters.add_argument(
        "--has-uni-tag-value",
        help=(
            "Filter output based on the UNI's tag value (VLAN ID). Possible filter values "
            "include integers, 'untagged', or a range of integers in increasing value, "
            "such as 10-12."
        ),
    )
    filters.add_argument(
        "--has-uni-tag-values",
        help=(
            "Filter output based on the UNI's tag value (VLAN ID) using comma separated values. "
            "Possible filter values include integers, 'untagged', or a range of integers in "
            "increasing value. Example: To search for EVCS with 'untagged', 10, or values "
            "between 20-30, use --has-uni-tag-values untagged,10,20-30"
        ),
    )

    filters.add_argument("--has-nni-name", help="Filter output based on the NNI's name")
    filters.add_argument(
        "--has-nni-name-primary", help="Filter output when NNI's name is in primary path"
    )
    filters.add_argument(
        "--has-nni-name-backup", help="Filter output when NNI's name is in backup path"
    )
    filters.add_argument("--has-evc-name", help="Filter output based on the EVC's name")
    filters.add_argument(
        "--has-partial-evc-name",
        help=(
            "Filter output based on the EVC's name or regex operation."
            "If you're receiving an error, "
            "try putting your value in quotes ('')"
        ),
    )
    filters.add_argument(
        "--has-primary-path",
        help="Filter output based on whether the EVC has a primary path or not.",
        action="store_true",
    )
    filters.add_argument(
        "--has-backup-path",
        help="Filter output based on whether the EVC has a backup path or not.",
        action="store_true",
    )
    filters.add_argument(
        "--has-dynamic-path",
        help="Filter output based on whether the EVC has a dynamic path or not.",
        action="store_true",
    )

    group = parser.add_argument_group("authentication")
    group.add_argument("-u", "--user", help="Backend user for authentication")
    group.add_argument("-t", "--tenant", help="Backend user group for authentication")

    passwords = parser.add_mutually_exclusive_group()
    passwords.add_argument("-p", "--password", help="Provide pass for authentication")
    passwords.add_argument(
        "-P", "--prompt_password", help="Prompt pass for authentication", action="store_true"
    )

    # backends = parser.add_mutually_exclusive_group(required=True)
    backends = parser.add_mutually_exclusive_group()
    backends.add_argument("-O", "--oess-url", help="Use OESS backend. Provide OESS's URL")
    backends.add_argument("-K", "--kytos-url", help="Use Kytos E-Line backend's URL.")
    ...
    parser.add_argument("-q", "--quiet", help="Set Quiet Mode", action="store_true")

    output = parser.add_mutually_exclusive_group()
    output.add_argument("-y", "--to-yaml", help="Print using YAML.", action="store_true")
    output.add_argument(
        "-Y",
        "--to-yaml-minimal",
    ...
    output.add_argument("-j", "--to-json", help="Print using JSON.", action="store_true")
    output.add_argument(
        "-z", "--to_zabbix", help="Converts output to Zabbix LLD format", action="store_true"
    )
    output.add_argument(
        "-n",
        "--to-screen",
    ...
    parser.add_argument(
        "-T",
        "--to_table",
        help="Converts output to a table format. " "Use Syntax: Primary|Backup|Any:Circuit_Name",
    )
    parser.add_argument(
        "-s",
        "--gen_stats_per_nni",
        help="List number of EVCs per NNI" "-s Any: list all NNIs" " To filter use -s NNI[:JSON]",
    )

    parser.add_argument(
        "-S", "--shortcut-file", help="Use a provided shortcut file. Default is .evc_manager"
    )
...
class CliOptions(metaclass=Singleton):
    ...
    def __init__(self, parser=None, args=None):
        ...
        self.has_partial_evc_filters = False
        self._has_evc_name = None
        self._has_partial_evc_name = None

        self.has_path_filters = False
        self._has_primary_path = False
        self._has_backup_path = False
        self._has_dynamic_path = False
    ...
    def evaluate_input(self, parser, args):
        ...
        if args.has_uni_tag_value and args.has_uni_tag_values:
            parser.error(
                (
                    "--has-uni-tag-value cannot be used at the same time"
                    "as --has-uni-tag-values"  # pylint: disable=W1404
                )
            )
        ...
        if args.has_partial_evc_name and not args.list_evc:
            parser.error("--has-partial-evc-name should be used with -L")

        if args.has_evc_name and args.has_partial_evc_name:
            parser.error("--has-evc-name cannot be used at the same time as --has-partial-evc-name")
    ...
    def has_uni_tag_value(self, tag_value):
        ...
        if isinstance(tag_value, str):
            self._has_uni_tag_value = self.convert_vlan_tags(tag_value)
            self.has_uni_filters = True

    @property
    def has_uni_tag_values(self):
        """Getter"""
        return self._has_uni_tag_values

    @has_uni_tag_values.setter
    def has_uni_tag_values(self, tag_values: str):
        """Setter"""
        if isinstance(tag_values, str):
            try:
                vlan_tags = [self.convert_vlan_tags(vlan) for vlan in str.split(tag_values, ",")]
            except Exception as exc:
                raise ValueError(f"Incorrect tag values: {tag_values}") from exc
            self._has_uni_tag_values = vlan_tags
            self.has_uni_filters = True
    ...
    @property
    def has_partial_evc_name(self):
        """Getter"""
        return self._has_partial_evc_name

    @has_partial_evc_name.setter
    def has_partial_evc_name(self, evc_name):
        """Setter"""
        if isinstance(evc_name, str) and len(evc_name) > 0:
            self._has_partial_evc_name = evc_name
            self.has_partial_evc_filters = True
        else:
            self.has_partial_evc_filters = False
    ...
    @property
    def has_primary_path(self):
        """Getter"""
        return self._has_primary_path

    @has_primary_path.setter
    def has_primary_path(self, has_primary_path):
        """Setter"""
        if has_primary_path:
            self._has_primary_path = has_primary_path
            self.has_path_filters = True

    @property
    def has_backup_path(self):
        """Getter"""
        return self._has_backup_path

    @has_backup_path.setter
    def has_backup_path(self, has_backup_path):
        if has_backup_path:
            self._has_backup_path = has_backup_path
            self.has_path_filters = True

    @property
    def has_dynamic_path(self):
        """Getter"""
        return self._has_dynamic_path

    @has_dynamic_path.setter
    def has_dynamic_path(self, has_dynamic_path):
        """Setter"""
        if has_dynamic_path:
            self._has_dynamic_path = has_dynamic_path
            self.has_path_filters = True
    ...
    def assign_values(self, args):
        ...
        self.has_uni_tag_values = args.has_uni_tag_values
        ...
        self.has_partial_evc_name = args.has_partial_evc_name

        self.has_primary_path = args.has_primary_path
        self.has_backup_path = args.has_backup_path
        self.has_dynamic_path = args.has_dynamic_path
    ...
    def convert_vlan_tags(self, tag_value):
        """Convert tag_value into an int, "untagged", or a List[int]"""
        if tag_value.isdigit():
            return int(tag_value)
        elif tag_value == "untagged":
            return tag_value
        try:
            result = []
            prev_value = -1
            for tag in str.split(tag_value, "-"):
                vlan_value = int(tag)
                result.append(vlan_value)
                if vlan_value < prev_value or len(result) > 2:
                    raise ValueError(
                        "Incorrect range format: Range should be two numbers in increasing order."
                    )
                prev_value = vlan_value
            return result
        except Exception as exc:
            raise exc
