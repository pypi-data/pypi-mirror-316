...
class Strategy:
    ...
    def replace_data_map(self, new_map):
        keys = new_map.keys()
        for key in keys:
            new_map[key] = collections.deque(new_map[key], maxlen=self.max_deque_length)
        self.data_map = new_map

    def replace_data_map_topic(self, new_map, topic):
        self.data_map[topic] = collections.deque(new_map[topic], maxlen=self.max_deque_length)
    ...
    def send_signal(self, id: str, side: OrderSide, signal_params: Any, retry_count=5):
        failed = []
        retry_ok = []
        count = 0
        for url in self.manager_url:
            try:
                req = requests.post(url=url, json={"id": id, "side": side.__str__().strip('"'), "signal_params": signal_params}, headers={"Content-Type": "application/json"})
                if not req.ok:
                    failed.append(url)
            except Exception as e:
                failed.append(url)
        ...
        while count < retry_count and len(failed) > 0:
            time.sleep(1.0)
            for url in failed:
                try:
                    req = requests.post(url=url, json={"id": id, "side": side.__str__().strip('"'), "signal_params": signal_params}, headers={"Content-Type": "application/json"})
