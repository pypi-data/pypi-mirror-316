...
class EditGui(object):
    def __init__(
        ...
        self._inputTempo = inputFrame.TextField(
            chordsSubPanel, width=3, text="Tempo: ", packing=tk.LEFT, state=tk.DISABLED
        )
    ...
    def _createNewSong(self, copiedSong=""):
        if self._dataBase:
            ...
            newSong.text = f"{settings.GENSETTINGS.get('Syntax', 'newslide')[0]}"

            if newSong not in self._dataBase or (
                newSong in self._dataBase
                and tkMessageBox.askyesno(
                    "Création",
                    f'Le chant "{newSong.nom}" existe déjà, voulez-vous l\'écraser ?',
                )
            ):
                self._newSongWindow.destroy()
                self._newSongWindow = None
                ...
                if not self._saveSong():
                    ...
                    tkMessageBox.showinfo(
                        "Confirmation", f'Le chant "{newSong.nom}" a été créé.'
                    )
