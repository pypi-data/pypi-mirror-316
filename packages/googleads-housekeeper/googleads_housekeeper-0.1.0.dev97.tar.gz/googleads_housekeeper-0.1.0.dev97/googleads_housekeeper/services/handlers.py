...
def run_manual_exclusion_task(
  ...
  with uow:
    ...
    placement_exclusion_lists = fetcher.get_placement_exclusion_lists(
      customer_ids=list(map(int, cmd.customer_ids.split(',')))
    )
...
def run_preview_placements(
  ...
  Args:
    cmd:
      The command containing parameters for previewing placements.

    uow:
      The unit of work managing database transactions.

    ads_api_client:
      The client used to interact with the Google Ads API.

    always_fetch_youtube_preview_mode:
      Whether to always fetch placements in YouTube preview mode.
      Defaults to True.

    save_to_db:
      Whether to save the fetched results to the database.
      Defaults to True.
  """
  report_fetcher = AdsReportFetcher(api_client=ads_api_client)
  task_obj = task.Task(
...
def save_task(
  ...
  with uow:
    if hasattr(cmd, 'task_id') and cmd.task_id:
      ...
      if cmd.schedule:
        uow.published_events.append(
          events.TaskWithScheduleCreated(task_id, cmd.name, cmd.schedule)
        )
    task_id = task_obj.id
    return str(task_id)
...
def save_channel_info(
  cmd: commands.SaveChannelInfo, uow: unit_of_work.AbstractUnitOfWork
) -> None:
  with uow:
