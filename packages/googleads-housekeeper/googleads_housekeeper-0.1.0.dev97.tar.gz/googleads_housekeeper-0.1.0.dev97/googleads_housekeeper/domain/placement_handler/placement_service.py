...
def find_placements_for_exclusion(
  task_obj: task.Task,
  uow: unit_of_work.AbstractUnitOfWork,
  report_fetcher: gaarf.query_executor.AdsReportFetcher | None = None,
  runtime_options: RuntimeOptions = RuntimeOptions(),
  mode: Literal['preview', 'action'] = 'action',
  ...
  """
  fetcher = placement_fetcher.PlacementFetcher(report_fetcher)
  extractor = placement_info_extractor.PlacementInfoExtractor(uow)
  ...
  for account in task_obj.accounts:
    ...
    if not (
      placements := fetcher.get_placements_for_account(account, task_obj, mode)
    ):
      continue
    ...
    if ads_specs := specification.ads_specs_entries:
      placements = ads_specs.apply_specifications(
        placements, include_reason=False, include_matching_placement=False
      )
      if not placements:
        continue
    if runtime_options.always_fetch_youtube_preview_mode:
      youtube_channel_specification = (
        exclusion_specification.ExclusionSpecification(
          specifications=[
            ...
            [
              exclusion_specification.YouTubeVideoExclusionSpecificationEntry(
                'likesCount > 0'
              )
            ],
          ]
    ...
    if placements := specification.apply_specifications(
      placements, include_matching_placement=False
    ):
...
def inject_extra_data(
  ...
  extractor: placement_info_extractor.PlacementInfoExtractor,
  ...
  Args:
    ...
    extractor:
      Responsible for getting parsed placement metadata data from DB.
  ...
  for placement in placements:
    ...
    placement['extra_info'] = extractor.extract_placement_info(placement)
  return placements
