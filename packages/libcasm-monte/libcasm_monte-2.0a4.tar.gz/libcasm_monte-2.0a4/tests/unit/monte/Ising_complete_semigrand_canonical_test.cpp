...
#include "casm/monte/ising_cpp/complete_semigrand_canonical.hh"
...
using namespace CASM::monte::ising_cpp::complete_semigrand_canonical;
...
state_type make_complete_Ising_default_state(Index rows = 25, Index cols = 25,
                                             int fill_value = 1,
...
TEST(IsingCompleteSemiGrandCanonicalTest, IsingConfiguration1) {
  Eigen::VectorXi shape(2);
  shape << 25, 25;
  int fill_value = 1;
  IsingConfiguration configuration(shape, fill_value);
...
TEST(IsingCompleteSemiGrandCanonicalTest, SemiGrandCanonicalConditions1) {
  typedef SemiGrandCanonicalConditions conditions_type;
...
TEST(IsingCompleteSemiGrandCanonicalTest, SemiGrandCanonicalIsingState1) {
  state_type state = testing::make_complete_Ising_default_state(25, 25);

  ASSERT_EQ(state.configuration.n_sites, 25 * 25);
  ASSERT_EQ(state.conditions.vector_values["exchange_potential"].size(), 1);
}

TEST(IsingCompleteSemiGrandCanonicalTest,
     IsingSemiGrandCanonicalEventGenerator1) {
...
TEST(IsingCompleteSemiGrandCanonicalTest, IsingFormationEnergy1) {
...
TEST(IsingCompleteSemiGrandCanonicalTest, IsingParamComposition1) {
  ...
  int fill_value = 1;
  state_type state =
      testing::make_complete_Ising_default_state(25, 25, fill_value);
...
TEST(IsingCompleteSemiGrandCanonicalTest, SemiGrandCanonicalPotential1) {
  /// Construct a state to calculate the semi-grand canonical energy of
  int fill_value = 1;
  double temperature = 2000.0;
  double mu = 2.0;
  state_type state = testing::make_complete_Ising_default_state(
      25, 25, fill_value, temperature, mu);
...
TEST(IsingCompleteSemiGrandCanonicalTest, SemiGrandCanonicalRun1) {
  /// Construct a state to calculate the semi-grand canonical energy of
  int fill_value = 1;
  double temperature = 2000.0;
  double mu = 0.0;
