...
class MtTaskMQ:
    ...
    async def pull_messages(self, qty: int = 5, max_poll_seconds: int = 5, poll_interval_ms: int = 100):
        logger.info(f"pull_messages {self.queue_name} {qty} {max_poll_seconds} {poll_interval_ms}")
        read_messages: list[Any] = await self.mq.read_with_poll(
        # read_messages: list[Message] = self.read_with_poll(
            self.queue_name , vt=30, qty=qty, max_poll_seconds=max_poll_seconds, poll_interval_ms=poll_interval_ms
        )
