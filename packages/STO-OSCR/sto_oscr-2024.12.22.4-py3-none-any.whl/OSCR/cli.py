...
import OSCR
from OSCR.datamodels import OverviewTableRow
...
def list_combats(parser: OSCR):
    ...
    for combat in combats:
        print(
            f"<{combat[1]} {combat[4] + ' ' if combat[4] else ''}at {combat[2]} {combat[3]}>"
        )
    print("++++++++++++++++++++++++++++++++")
...
def analyzation(args, parser: OSCR):
    ...
    for combat in parser.combats:
        ...
        if args.metadata:
            ...
            print(
                ", ".join(
                    f"[{c.name} count={c.count}]" for c in combat.critters.values()
                )
            )
        if args.analysis:
            ...
            for player in combat.players.values():
                ...
                paddings = [
                    max(e1, e2) for e1, e2 in zip(paddings, map(len, formatted_row))
                ]
            print("###         Analysis         ###")
