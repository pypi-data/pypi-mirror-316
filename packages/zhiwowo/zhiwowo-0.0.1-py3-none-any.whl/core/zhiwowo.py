...
class ZhiwoAssistant:
    ...
    def send_user_msg(self, text):
        user_msg = UserMsg(text, self.param)
        self.send_msg(user_msg=user_msg)

    def send_msg(self, user_msg=None, ding_talk_webhook=None):
        headers = {
...
if __name__ == '__main__':
    ...
    sql_data = assistant.get_sql_data_by_job_number('xxx', '5007 5008')
    print(json.dumps(sql_data))


    assistant.send_user_msg("测试websocket消息")
    assistant.send_ding_talk_webhook("webhook", ['phone'], webhook_id="id")
    assistant.send_msg(user_msg=UserMsg('websocket plus', param), ding_talk_webhook=DingTalkWebhook('webhook plus', ['phone'], webhook_url=''))
