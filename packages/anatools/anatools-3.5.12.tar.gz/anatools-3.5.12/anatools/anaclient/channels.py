...
def build_managed_channel(self, channelfile):
...
USER root"""
    ...
    image, logs = dockerclient.images.build(path=builddir, tag=channelfile.split('.')[0], rm=True)
    print('done.')

    __check_docker_image_size(image)
...
def __check_docker_image_size(image):
    ...
    if size is None and virtual_size is None:
        print('\033[93m' + 'WARNING: Docker Image size cannot be determined. Skipping size check.' + '\033[0m')
        return True
    ...
    print('\033[93m' + f'Docker image size: {size_in_gb:.2f}' + '\033[0m')
    return
...
def deploy_managed_channel(self, channelId=None, channelfile=None, image=None):
    ...
    try: 
        ...
        __check_docker_image_size(channelimage)
