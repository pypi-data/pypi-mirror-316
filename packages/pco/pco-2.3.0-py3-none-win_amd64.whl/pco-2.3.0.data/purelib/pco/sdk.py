...
class Sdk:
    ...
    # -------------------------------------------------------------------------
    def get_camera_setup(self, setup_type):
        """
        ...
        :rtype: dict
            * 'type' : str <setup_type>
            * 'setup' : str <setup_mode>
            * 'supported pixel formats' : list of str
        ...
        pixel_format = {
            ...
            "10" : 0x00000008,
            "8" : 0x00000010,
            "14 packed" : 0x00000020,
            "12 packed" : 0x00000040,
            "10 packed" : 0x00000080,
        }
        
        if setup_type == "shutter mode":
            mode_dict = shutter_mode
        elif setup_type == "pixel format":
            mode_dict = pixel_format
        else:
            raise KeyError("Unknown setup type!")
    
        wType = C.c_uint16(setup_types[setup_type])
        ...
        if error == 0:
            ret.update(
                {
                    "type": list(setup_types.keys())[list(setup_types.values()).index(wType.value)], # inverse dict
                    "setup": list(mode_dict.keys())[list(mode_dict.values()).index(dwSetup[0])],
                }
            )
            if setup_type == "pixel format":
                supported_pf = []
                if dwSetup[1] & 0x00000001: # PIXELFORMAT_SETUP_16)
                    supported_pf.append("16")
                
                if dwSetup[1] & 0x00000002: # PIXELFORMAT_SETUP_14)
                    supported_pf.append("14")
                
                if dwSetup[1] & 0x00000004: # PIXELFORMAT_SETUP_12)
                    supported_pf.append("12")
                
                if dwSetup[1] & 0x00000008: # PIXELFORMAT_SETUP_10)
                    supported_pf.append("10")
                
                if dwSetup[1] & 0x00000010: # PIXELFORMAT_SETUP_8)
                    supported_pf.append("8")
                
                if dwSetup[1] & 0x00000020: # PIXELFORMAT_SETUP_14P)
                    supported_pf.append("14 packed")
                
                if dwSetup[1] & 0x00000040: # PIXELFORMAT_SETUP_12P)
                    supported_pf.append("12 packed")
                
                if dwSetup[1] & 0x00000080: # PIXELFORMAT_SETUP_10P)
                    supported_pf.append("10 packed")

                ret.update({"supported pixel formats" : supported_pf})
    ...
    # -------------------------------------------------------------------------
    def set_camera_setup(self, setup_type, setup_mode):
        """
        ...
        dwSetup = (C.c_uint32 * 4)()
        if setup_type == "shutter mode":
            dwSetup[0] = shutter_mode[setup_mode]
        elif setup_type == "pixel format":
            dwSetup[0] = pixel_format[setup_mode]
        else:
            raise KeyError("Unknown setup type!")
