...
class PCO_SensorInfo(C.Structure):
    _pack_ = 8  # This packing is intentional for the int that is platform/C-Compiler dependant and there is an uneven amount of ints in the struct. 
    _fields_ = [
        ("wSize", C.c_uint16),
        ("wDummy", C.c_uint16),
...
class PCO_Convert(C.Structure):
    ...
    _fields_ = [
        ...
        ("szConvertInfoGPU", C.c_char * 60),
        ("dwzzDummy1", C.c_uint32 * 27),
    ]
...
class Convert:
    ...
    def get_version_info_pco_conv(self):
        ...
        error = self.PCO_Convert.PCO_GetVersionInfoPCO_CONV(
            pszName, iNameLength, pszPath, iPathLength, iMajor, iMinor, iPatch, iBuild)
        duration = time.perf_counter() - time_start
        error_msg = self.get_error_text(error)
