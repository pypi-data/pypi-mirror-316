...
class TestRecipe(unittest.TestCase):
    ...
    def test_has_properties(self):
        ...
        for prop in properties:
            with self.subTest(prop=prop):
                self.assertTrue(hasattr(self.recipe, prop))
    ...
    def test_settings_property_is_dict_with_fields(self):
        ...
        self.assertEqual(
            ...
            list(self.recipe.settings["colors"].keys()),
        )

    def test_directories_property_is_dict_with_fields(self):
        self.assertEqual(
            ["output", "datasets_source"],
    ...
    def test_import_from_yaml_importer_with_task_sets_task(self):
        ...
        for key in self.task:
            self.assertEqual(
                getattr(self.recipe.tasks[0], key), self.task[key]
            )
    ...
    def test_import_from_yaml_importer_with_datasets_sets_datasets(self):
        ...
        for dataset_ in self.recipe.datasets:
            self.assertTrue(
                isinstance(self.recipe.datasets[dataset_], dataset.Dataset)
            )
    ...
    def test_from_dict_with_dataset_without_dataset_factory_sets_factory(
        self,
    ):
    ...
    def test_from_dict_with_dataset_sets_dataset(self):
        ...
        self.recipe.from_dict(dict_)
        self.assertTrue(
            isinstance(self.recipe.datasets[self.dataset], dataset.Dataset)
        )
    ...
    def test_from_dict_with_dataset_issues_log_message(self):
        ...
        with self.assertLogs(__package__, level="INFO") as cm:
            self.recipe.from_dict(dict_)
        self.assertIn(
            'Import dataset "{}" as "{}"'.format(self.dataset, id_),
            cm.output[0],
        )
    ...
    def test_from_dict_with_dataset_and_source_dir_normalises_path_in_id(
        self,
    ):
        dict_ = {
            "datasets": ["foo"],
            "directories": {"datasets_source": "/bla/../blub"},
        }
        self.recipe.from_dict(dict_)
        self.assertEqual("/blub/foo", self.recipe.datasets["foo"].id)

    def test_from_dict_with_dataset_normalises_path_in_id(self):
        dict_ = {
            "datasets": ["/bla/../blub/foo"],
        }
        self.recipe.from_dict(dict_)
        self.assertEqual(
            "/blub/foo", self.recipe.datasets["/bla/../blub/foo"].id
        )
    ...
    def test_from_dict_with_dataset_as_dict_sets_dataset_label(self):
        ...
        self.recipe.from_dict(dict_)
        self.assertTrue(
            isinstance(self.recipe.datasets[id_], dataset.Dataset)
        )
    ...
    def test_from_dict_with_dataset_as_dict_and_importer(self):
        ...
        dict_ = {
            "datasets": [
                {"source": self.dataset_filename, "importer": "TxtImporter"}
            ]
        }
        self.recipe.from_dict(dict_)
        self.assertEqual(
            data, self.recipe.datasets[self.dataset_filename].data.data
        )
    ...
    def test_from_dict_with_dataset_as_dict_and_importer_parameters(self):
        ...
        self.recipe.from_dict(dict_)
        self.assertEqual(
            data[1], self.recipe.datasets[self.dataset_filename].data.data
        )
    ...
    def test_from_dict_with_dataset_dict_package_and_importer_parameters(
        self,
    ):
        data = np.random.random(2)
        parameters = {"skiprows": 1}
    ...
    def test_from_dict_with_task_adds_task_with_correct_type(self):
        ...
        self.recipe.from_dict(dict_)
        self.assertTrue(
            isinstance(self.recipe.tasks[0], tasks.ProcessingTask)
        )
    ...
    def test_from_dict_without_colors_settings_retains_colors_subdict(self):
        dict_ = {"settings": {"default_package": "foo"}}
        self.recipe.dataset_factory = self.dataset_factory
        self.recipe.from_dict(dict_)
        self.assertIsInstance(self.recipe.settings["colors"], dict)

    def test_from_dict_with_partial_colors_settings_retains_colors_keys(self):
        dict_ = {"settings": {"colors": {"default_colormap": "foo"}}}
        self.recipe.dataset_factory = self.dataset_factory
        self.recipe.from_dict(dict_)
        self.assertListEqual(
            ["default_colormap", "number_of_colors", "first_color"],
            list(self.recipe.settings["colors"].keys()),
        )
    ...
    def test_to_dict_contains_directories_dict(self):
        ...
        self.assertDictEqual(
            self.recipe.directories, recipe_dict["directories"]
        )
    ...
    def test_to_dict_with_dataset_with_id_returns_dataset_as_dict(self):
        ...
        dict_ = self.recipe.to_dict()
        self.assertDictEqual(
            {"source": self.dataset, "id": id_}, dict_["datasets"][0]
        )
    ...
    def test_to_dict_with_dataset_with_label_returns_dataset_as_dict(self):
        ...
        dict_ = self.recipe.to_dict()
        self.assertDictEqual(
            {"source": self.dataset, "label": label}, dict_["datasets"][0]
        )

    def test_to_dict_with_dataset_w_label_and_id_returns_dataset_as_dict(
        self,
    ):
        id_ = "foobar"
        label = "foobar"
        ...
        dict_ = self.recipe.to_dict()
        self.assertDictEqual(
            {"source": self.dataset, "label": label, "id": id_},
            dict_["datasets"][0],
        )
    ...
    def test_to_dict_with_dataset_with_importer_parameters(self):
        ...
        self.assertDictEqual(
            dict_["datasets"][0], self.recipe.to_dict()["datasets"][0]
        )
    ...
    def test_to_dict_with_datasets_source_directory(self):
        ...
        id_ = "foobar"
        tmp = dataset.Dataset()
        tmp.id = os.path.join(self.recipe.directories["datasets_source"], id_)
        tmp.label = id_
        self.recipe.datasets[id_] = tmp
        dict_ = self.recipe.to_dict()
    ...
    def test_get_dataset_without_identifier_raises(self):
        with self.assertRaises(
            aspecd.exceptions.MissingDatasetIdentifierError
        ):
...
class TestChef(unittest.TestCase):
    def setUp(self):
        ...
        self.annotation_task = {"kind": "annotation", "type": "Comment"}
        self.plotting_task = {"kind": "singleplot", "type": "SinglePlotter"}
    ...
    def test_cook_recipe_with_processing_task_with_apply_to_performs_task(
        self,
    ):
        recipe = self.recipe
    ...
    def test_cook_recipe_with_plotting_task_performs_task(self):
        ...
        self.chef.cook(recipe=recipe)
        self.assertTrue(
            self.chef.recipe.datasets[self.dataset].representations
        )
    ...
    def test_system_info_value_in_history_is_ordered_dict(self):
        ...
        self.chef.cook(recipe)
        self.assertIs(
            collections.OrderedDict, type(self.chef.history["system_info"])
        )
    ...
    def test_info_key_in_history_contains_correct_start_timestamp(self):
        ...
        self.assertTrue(
            self.chef.history["info"]["start"].startswith(timestamp)
        )
    ...
    def test_info_key_in_history_contains_correct_end_timestamp(self):
        ...
        try:
            ...
            timestamp = datetime.datetime.now().strftime("%Y-%m-%dT%H:%M")
        self.assertTrue(
            self.chef.history["info"]["end"].startswith(timestamp)
        )
...
class TestTask(unittest.TestCase):
    ...
    def test_get_object_with_not_existing_object_attributes_logs_message(
        self,
    ):
        kind = "processing"
        type_ = "SingleProcessingStep"
        ...
        self.assertIn(
            '"{}" has no property "{}".'.format(type_, "foo"), cm.output[0]
        )
    ...
    def test_set_object_attributes_sets_field_in_dict(self):
        ...
        self.assertEqual(
            metadata["parameters"]["foo"], processing_step.parameters["foo"]
        )
    ...
    def test_set_object_attributes_sets_fields_in_dict(self):
        ...
        self.assertEqual(
            metadata["parameters"]["bar"], processing_step.parameters["bar"]
        )
    ...
    def test_set_object_attributes_sets_fields_in_object_in_list(self):
        ...
        self.task._set_object_attributes(processing_step)
        self.assertEqual(
            metadata["parameters"]["foo"][0]["unit"],
    ...
    def test_set_object_attributes_sets_fields_in_objects_in_list(self):
        ...
        processing_step.parameters = {
            "foo": [aspecd.dataset.Axis(), aspecd.dataset.Axis()]
        }
        ...
        self.assertEqual(
            metadata["parameters"]["foo"][1]["unit"],
    ...
    def test_set_object_attributes_replaces_results_dataset_from_recipe(self):
        ...
        self.task._set_object_attributes(processing_step)
        self.assertEqual(
            self.task.recipe.results["bar"], processing_step.parameters["foo"]
        )
    ...
    def test_set_object_attributes_replaces_results_from_recipe(self):
        ...
        self.task.properties = metadata
        self.task.recipe = tasks.Recipe()
        self.task.recipe.results["bar"] = [0.0, 1.72, 3.14]
        self.task._set_object_attributes(processing_step)
        self.assertEqual(
            self.task.recipe.results["bar"], processing_step.parameters["foo"]
        )
    ...
    def test_set_object_attributes_replaces_figure_from_recipe(self):
        ...
        self.assertEqual(
            self.task.recipe.figures["bar"], report_step.context["foo"]
        )
    ...
    def test_set_object_attributes_replaces_path(self):
        ...
        self.task._set_object_attributes(processing_step)
        self.assertEqual(
            "/foo/bar/test.pdf", processing_step.parameters["foo"]
        )
    ...
    def test_to_dict_replaces_dataset_in_parameters_subdict_with_label(self):
        ...
        dict_ = self.task.to_dict()
        self.assertEqual(
            dict_["properties"]["parameters"]["foo"]["bar"], "foo"
        )

    def test_to_dict_replaces_dataset_in_parameters_subsubdict_with_label(
        self,
    ):
        dataset = aspecd.dataset.Dataset()
        recipe = tasks.Recipe()
...
class TestProcessingTask(unittest.TestCase):
    ...
    def test_perform_task_issues_log_message(self):
        ...
        with self.assertLogs(__package__, level="INFO") as cm:
            self.task.perform()
        self.assertIn(
            'Perform "{}" on dataset "{}"'.format(
                self.processing_task["type"], self.dataset[0]
            ),
            cm.output[0],
        )
    ...
    def test_perform_task_with_result_and_multiple_datasets_adds_results(
        ...
        self.prepare_recipe()
        result = ["foo", "bar"]
        recipe_dict = {"datasets": result, "tasks": [self.processing_task]}
        self.recipe.from_dict(recipe_dict)
        self.processing_task["result"] = ["fooz", "barz"]
    ...
    def test_to_dict_with_multiple_datasets_and_same_params_returns_dict(
        self,
    ):
        self.dataset = ["foo", "bar"]
    ...
    def test_processing_after_plot_task_with_multiple_datasets(self):
        ...
        plot_task.from_dict({"kind": "singleplot", "type": "SinglePlotter1D"})
    ...
    def test_to_dict_with_multiple_datasets_and_dataset_ids(self):
        ...
        self.processing_task["properties"] = {
            "parameters": {"kind": "minus", "dataset": "foo"}
        }
        self.prepare_recipe()
        self.recipe.datasets["foo"].data.data = np.random.random(10) + 5
        self.recipe.datasets["bar"].data.data = np.random.random(10) + 3
...
class TestMultiProcessingTask(unittest.TestCase):
    ...
    def test_perform_task_with_result_issues_log_message(self):
        ...
        with self.assertLogs(__package__, level="INFO") as cm:
            self.task.perform()
        self.assertIn(
            'Perform "{}" on datasets "{}" resulting in "{}"'.format(
...
class TestSingleAnalysisTask(unittest.TestCase):
    ...
    def test_perform_task_issues_log_message(self):
        ...
        self.assertIn(
            'Perform "{}" on dataset "{}"'.format(
                self.analysis_task["type"], self.dataset[0]
            ),
            cm.output[0],
        )
    ...
    def test_perform_task_with_result_sets_resulting_dataset_id(self):
        ...
        with patch(
            ...
            result=aspecd.dataset.Dataset(),
        ) as mock_obj:
            with patch(
                "aspecd.tasks.Task._create_object", return_value=mock_obj
            ):
                self.task.perform()
    ...
    def test_perform_task_with_result_and_multiple_datasets_adds_results(
        ...
        self.prepare_recipe()
        result = ["foo", "bar"]
        recipe_dict = {"datasets": result, "tasks": [self.analysis_task]}
        self.recipe.from_dict(recipe_dict)
        self.analysis_task["result"] = result
...
class TestMultiAnalysisTask(unittest.TestCase):
    ...
    def test_perform_task_issues_log_message(self):
        ...
        self.assertIn(
            'Perform "{}" on datasets "{}"'.format(
                self.analysis_task["type"], ", ".join(self.dataset)
            ),
            cm.output[0],
        )
...
class TestAggregatedAnalysisTask(unittest.TestCase):
    ...
    def test_perform_task_returns_calculated_dataset_in_result(self):
        ...
        self.assertIsInstance(
            self.recipe.results[self.result], aspecd.dataset.CalculatedDataset
        )
    ...
    def test_perform_task_sets_values_in_result(self):
        ...
        self.task.perform()
        self.assertEqual(
            len(self.dataset), len(self.recipe.results[self.result].data.data)
        )
    ...
    def test_perform_task_sets_correct_values_in_result(self):
        ...
        self.task.recipe = self.recipe
        self.task.perform()
        self.assertEqual(0.0, self.recipe.results[self.result].data.data[0])
        self.assertEqual(1.0, self.recipe.results[self.result].data.data[1])
    ...
    def test_perform_task_sets_index_in_result(self):
        ...
        self.task.perform()
        self.assertEqual(
            self.dataset, self.recipe.results[self.result].data.axes[0].index
        )
...
class TestAnnotationTask(unittest.TestCase):
    ...
    def test_perform_task_issues_log_message(self):
        ...
        self.assertIn(
            'Perform "{}" on dataset "{}"'.format(
                self.annotation_task["type"], self.dataset[0]
            ),
            cm.output[0],
        )
...
class TestSinglePlotTask(unittest.TestCase):
    ...
    def test_perform_task_issues_log_message(self):
        ...
        self.assertIn(
            'Perform "{}" on dataset "{}"'.format(
                self.plotting_task["type"], self.dataset[0]
            ),
            cm.output[0],
        )
    ...
    def test_figure_added_to_recipe_contains_filename_with_output_dir(self):
        ...
        self.task.perform()
        self.assertEqual(
            os.path.join("outputdir", "foo.pdf"),
    ...
    def test_perform_task_with_filename_issues_log_message(self):
        ...
        with self.assertLogs(__package__, level="INFO") as cm:
            self.task.perform()
        self.assertIn(
            'Save figure from "{}" to file "{}"'.format(
                self.plotting_task["type"], self.figure_filename
            ),
            cm.output[1],
        )
    ...
    def test_perform_task_with_filename_and_output_directory_saves_plot(self):
        ...
        self.assertTrue(
            os.path.exists(
                os.path.join(self.output_directory, self.figure_filename)
            )
        )
    ...
    def test_perform_task_with_target_sets_figure_and_axes(self):
        ...
        task2.perform()
        self.assertEqual(
            self.task._task.figure.number, task2._task.figure.number
        )

    def test_perform_task_with_label_adds_label_to_dataset_figure_record(
        self,
    ):
        self.prepare_recipe()
        label = "foo"
    ...
    def test_perform_task_wo_label_adds_default_label_to_dataset_fig_rec(
        self,
    ):
        self.prepare_recipe()
        self.task.from_dict(self.plotting_task)
    ...
    def test_default_colormap_gets_set_to_plotter(self):
        colormap = "viridis"
        self.prepare_recipe()
        self.recipe.datasets[self.dataset[0]].data.data = np.random.random(
            [5, 5]
        )
        self.recipe.settings["colors"]["default_colormap"] = colormap
        ...
        dict_ = self.task.to_dict()
        self.assertEqual(
            colormap, dict_["properties"]["properties"]["colormap"]
        )
    ...
    def test_default_colormap_does_not_override_task_colormap(self):
        ...
        self.recipe.settings["colors"]["default_colormap"] = default_colormap
        self.plotting_task["type"] = "SinglePlotter2D"
        self.plotting_task["properties"] = {
...
class TestMultiPlotTask(unittest.TestCase):
    ...
    def tearDown(self):
        if os.path.exists(self.figure_filename):
            os.remove(self.figure_filename)
        if "filename" in self.task.properties and os.path.exists(
    ...
    def test_perform_task_issues_log_message(self):
        ...
        self.assertIn(
            'Perform "{}" on datasets "{}"'.format(
                self.plotting_task["type"], ", ".join(self.dataset)
            ),
            cm.output[0],
        )
    ...
    def test_perform_task_without_filename_saves_plot_to_default_filename(
        self,
    ):
        self.dataset = ["foo", "bar"]
        ...
        self.figure_filename = "".join(
            [
                "_".join(self.plotting_task["apply_to"]),
                "_",
                self.plotting_task["type"],
                ".pdf",
            ]
        )
        self.prepare_recipe()
    ...
    def test_perform_task_wo_filename_adds_default_filename_to_properties(
        self,
    ):
        self.dataset = ["foo", "bar"]
        self.plotting_task["apply_to"] = self.dataset
        ...
        self.task.perform()
        self.assertEqual(
            self.figure_filename, self.task.properties["filename"]
        )
    ...
    def test_number_of_colors_gets_set_to_plotter(self):
        number_of_colors = 5
        self.prepare_recipe()
        self.recipe.settings["colors"]["number_of_colors"] = number_of_colors
        self.plotting_task["type"] = "MultiPlotter1D"
        self.task.from_dict(self.plotting_task)
        self.task.recipe = self.recipe
        self.task.perform()
    ...
    def test_first_color_gets_set_to_plotter(self):
        first_color = 5
        self.prepare_recipe()
        self.recipe.settings["colors"]["first_color"] = first_color
        self.plotting_task["type"] = "MultiPlotter1D"
        self.task.from_dict(self.plotting_task)
        self.task.recipe = self.recipe
        self.task.perform()
        ...
        self.assertEqual(
            first_color,
            dict_["properties"]["properties"]["first_color"],
        )

    def test_first_color_does_not_override_task_first_color(self):
        first_color = 5
        task_first_color = 10
        self.prepare_recipe()
        self.recipe.settings["colors"]["first_color"] = first_color
        self.plotting_task["type"] = "MultiPlotter1D"
        self.plotting_task["properties"] = {
            "properties": {"first_color": task_first_color}
        }
        self.task.from_dict(self.plotting_task)
        self.task.recipe = self.recipe
        self.task.perform()
...
class TestCompositePlotTask(unittest.TestCase):
    def setUp(self):
        ...
        self.dataset = ["foo"]
        self.figure_filename = "foo.pdf"
        self.singleplotting_task = {
            "kind": "singleplot",
            "type": "SinglePlotter1D",
            "apply_to": self.dataset,
            "result": "singleplotter",
        }
        self.multiplotting_task = {
            "kind": "multiplot",
            "type": "MultiPlotter1D",
            "apply_to": self.dataset,
            "result": "multiplotter",
    ...
    def test_perform_task_issues_log_message(self):
        ...
        with self.assertLogs(__package__, level="INFO") as cm:
            self.task.perform()
        self.assertIn(
            'Perform "{}"'.format(self.plotting_task["type"]), cm.output[0]
        )
    ...
    def test_task_to_dict_replaces_plotter_with_label(self):
        ...
        dict_ = self.task.to_dict()
        self.assertEqual(
            self.singleplotting_task["result"],
...
class TestPlotAnnotationTask(unittest.TestCase):
    def setUp(self):
        ...
        self.annotation_task = {
            "kind": "plotannotation",
            "type": "VerticalLine",
            "parameters": {"positions": [0.5]},
            "apply_to": self.dataset,
        }
        self.recipe_dict = {
            "settings": {"autosave_plots": False},
            "datasets": self.dataset,
            "tasks": [self.annotation_task],
        }
    ...
    def test_perform_task_with_result_adds_plotannotation_to_recipe(self):
        ...
        self.task.perform()
        self.assertIsInstance(
            self.recipe.plotannotations[label],
...
class TestReportTask(unittest.TestCase):
    ...
    def test_perform_task_issues_log_message(self):
        ...
        self.assertIn(
            'Perform "{}" on dataset "{}"'.format(
                self.report_task["type"], self.dataset
            ),
            cm.output[0],
        )
    ...
    def test_perform_task_adds_figure_filenames_to_includes(self):
        ...
        self.task.perform()
        self.assertIn(
            figure_record.filename, self.task.properties["includes"]
        )

    def test_perform_task_does_not_add_empty_figure_filename_to_includes(
        self,
    ):
        self.prepare_recipe()
        figure_record = tasks.FigureRecord()
    ...
    def test_report_gets_saved_to_output_directory_if_provided(self):
        ...
        self.task.perform()
        self.assertTrue(
            os.path.exists(os.path.join(self.output_directory, self.filename))
        )
...
class TestFigureReportTask(unittest.TestCase):
    ...
    def test_perform_task_issues_log_message(self):
        ...
        with self.assertLogs(__package__, level="INFO") as cm:
            self.task.perform()
        self.assertIn(
            'Perform "{}" on figure "{}"'.format(
                self.report_task["type"], self.figure
            ),
            cm.output[0],
        )
...
class TestModelTask(unittest.TestCase):
    ...
    def prepare_recipe(self):
        ...
        dataset_factory.importer_factory = aspecd.io.DatasetImporterFactory()
        self.recipe.dataset_factory = dataset_factory
        recipe_dict = {"datasets": [self.dataset], "tasks": [self.model_task]}
        self.recipe.from_dict(recipe_dict)
    ...
    def test_perform_task_issues_log_message(self):
        ...
        with self.assertLogs(__package__, level="INFO") as cm:
            self.task.perform()
        self.assertIn(
            'Create model "{}"'.format(self.model_task["type"]), cm.output[0]
        )
    ...
    def test_output_model_returns_model(self):
        ...
        self.task.perform()
        self.assertEqual(
            aspecd.model.Model, type(self.recipe.results[result])
        )
...
class TestExportTask(unittest.TestCase):
    ...
    def test_perform_task_issues_log_message(self):
        ...
        with self.assertLogs(__package__, level="INFO") as cm:
            self.task.perform()
        self.assertIn(
            'Export "{}" to file "{}"'.format(self.dataset, self.filename),
            cm.output[0],
        )
...
class TestTabulateTask(unittest.TestCase):
    ...
    def test_perform_task_issues_log_message(self):
        ...
        self.assertIn(
            'Perform "{}" on dataset "{}"'.format(
                self.tabulate_task["type"], self.dataset[0]
            ),
            cm.output[0],
        )
    ...
    def test_perform_task_with_filename_issues_log_message(self):
        ...
        with self.assertLogs(__package__, level="INFO") as cm:
            self.task.perform()
        self.assertIn(
            'Save table from "{}" to file "{}"'.format(
                self.tabulate_task["type"], self.table_filename
            ),
            cm.output[1],
        )
...
class TestChefDeService(unittest.TestCase):
    ...
    def test_serve_with_recipe_cooks_recipe(self):
        self.create_recipe()
        self.history_filename = self.chef_de_service.serve(
            recipe_filename=self.recipe_filename
        )
        self.assertTrue(os.path.exists(self.figure_filename))
    ...
    def test_written_history_can_be_used_as_recipe(self):
        ...
        history_filename = self.chef_de_service.serve(
            recipe_filename=self.history_filename
        )
...
class TestServe(unittest.TestCase):
    ...
    def test_call_without_recipe_prints_help(self):
        ...
        self.assertIn(
            "error: the following arguments are required", result.stderr
        )

    def test_help_contains_program_name(self):
        result = subprocess.run(
            ["serve", "-h"], capture_output=True, text=True
        )
    ...
    def test_serve_console_entry_point_prints_log_info(self):
        self.create_recipe()
        result = subprocess.run(
            ["serve", self.recipe_filename], capture_output=True, text=True
        )
