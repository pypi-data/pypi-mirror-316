...
from ..utils import __is_valid_scan__, __dicomdump__
...
class HasUncompressedPixelData(ScanTest):
    ...
    def run(self, experiment_id, scan_id):
        ...
        options = {'src': '/archive/projects/%s/experiments/%s/scans/%s' %
                   ...
                   'format': 'csv'}
        transfer_syntaxes = __dicomdump__(self.xnat_instance, options)
...
class IsClassicDICOM(ScanTest):
    ...
    def run(self, experiment_id, scan_id):
        ...
        options = {'src': '/archive/projects/%s/experiments/%s/scans/%s' %
                   ...
                   'format': 'csv'}
        sopclass_tags = __dicomdump__(self.xnat_instance, options)
...
class HasPhilipsPrivateTags(ScanTest):
    ...
    def run(self, experiment_id, scan_id):
        ...
        options = {'src': '/archive/projects/%s/experiments/%s/scans/%s' %
                          (project_id, experiment_id, scan_id),
                   'format': 'csv'}
        dicomdump_list = __dicomdump__(self.xnat_instance, options)
...
class HasCorrectSequences(ExperimentTest):
    ...
    def run(self, experiment_id):
        ...
        for k, v in pr['obligatory'].items():
            ...
            key = 'file_count'
            if k in trans_table and 'CHESS' not in trans_table[k]:
                key = 'frames'
...
class HasCorrectSequenceAttributes(ExperimentTest):
    ...
    def _compare_scan_attributes(self, scan_attributes, expected_attributes):
        ...
        for k, v in scan_attributes.items():
            if k == 'xnat:mrScanData/parameters/ti' and v == '':
...
class IsPhilipsVersion519(ExperimentTest):
    ...
    def run(self, experiment_id):
        ...
        tag = __dicomdump__(self.xnat_instance, options)
        if not tag:
        ...
        if tag_value[0] == self.mri_philips_version:
            result = True
            data = []
