...
def img(board: Board, p1: str, p2: str, move: Move = None):
    ...
    Roboto_fp = str(importlib.resources.files('chesssnake.chesslib').joinpath('data/Roboto-Black.ttf'))

    big_font = ImageFont.truetype(Roboto_fp, 60)
    small_font = ImageFont.truetype(Roboto_fp, 15)

    # gets the blank template
    img = Image.open(str(importlib.resources.files('chesssnake.chesslib').joinpath('data/img/template.png')))
    
    wboard = Image.open(str(importlib.resources.files('chesssnake.chesslib').joinpath('data/img/blankboard.png')))
    bboard = Image.open(str(importlib.resources.files('chesssnake.chesslib').joinpath('data/img/blankboard.png')))

    if move is not None:
        # adds orange squares
        orange_square = Image.open(str(importlib.resources.files('chesssnake.chesslib').joinpath('data/img/orange.png'))).convert('RGBA')
    ...
    for x in range(8):
        for y in range(8):
            ...
            if board[x, y].piece is not None:
                ...
                piece = Image.open(str(importlib.resources.files('chesssnake.chesslib').joinpath(f'data/img/{ptype}{color}.png'))).convert('RGBA')
