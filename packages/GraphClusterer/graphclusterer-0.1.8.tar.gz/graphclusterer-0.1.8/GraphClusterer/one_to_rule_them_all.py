import warnings
from . import functions, summarize, evaluate, plot
import os
import json
...
def run_summarization(_name: str, _vertices, key, key2) -> object:
    """
    Run the summarization for the given name and summarize_kwargs.
    :param _name: the name of the dataset.
    :param _vertices: the vertices of the graph.
    :param key: the API key for the cohere model.
    :param key2: the API key for the llama model.
    :return:
    """
    # load the graph.
    ...
    titles = summarize.summarize_per_color(_subgraphs, _name, _vertices, key, key2)
    return titles
...
def the_almighty_function(pipeline_kwargs: dict):
    ...
    key = pipeline_kwargs.get('key', None)  # cohere_key
    key2 = pipeline_kwargs.get('llama_key', None)  # llama_key
    ...
    titles = run_summarization(name, vertices, key, key2)
    ...
    for i in range(iteration_num):
        ...
        sr = sg.starr(name, vertices, G, key)
        if print_info:
    ...
    success_rate = sg.starr(name, vertices, G, key)  # Evaluate the success rate

    # Improve the summaries iteratively
    for iter_ in range(iteration_num):
        rel, coh, con, flu, scores = evaluate.metrics_evaluations(name, vertices, G, key)
        if print_info:
            print(f"Metrics for '{name}' in iteration {iter_ + 1}:")
            print(f"Relevancy: {rel:.2f}, Coherence: {coh:.2f}, Consistency: {con:.2f}, Fluency: {flu:.2f}")
            print(50 * "-")
        summarize.improve_summaries(name, vertices, scores, key, key2)

    # Update the metrics dictionary
    rel, coh, con, flu, _ = evaluate.metrics_evaluations(name, vertices, G, key)
