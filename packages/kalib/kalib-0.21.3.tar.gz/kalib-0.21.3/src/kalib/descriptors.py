...
# class level cached property, pass original class when property used
# as first positional argument, and replace original data-descriptor
...
class Property(BaseProperty):
    ...
    # replace data-descriptor with calculated result, async supported
    Cached = Cached

    # just replace getter with calculated result, for simple cases
    Bind = cached_property

    ## class properties

    # pass class as first positional argument
    Class = ClassProperty

    # pass ORIGINAL class where it's decorator used, then
    # replace own data-descriptor with calculated result IN ORIGINAL PARENT
    Class.Parent = ClassCachedProperty

    # pass INHERITED class and SET CHILD CLASS attribute with calculated result
    Class.Cached = InheritedClass.make_from(ClassCachedProperty)

    # pass class or (!) instance
    Mixed = MixedProperty

    # replace data-descriptor with calculated result for instances
    # but when used in class context, pass class and just return result
