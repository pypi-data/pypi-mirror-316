...
from kalib.descriptors import Class, Property, cache, parent_call
...
@lazy_proxy_to('state', 'description', 'phrase', 'value', Class.Property.Cached)
...
class HTTPException(Exception, Logging.Mixin):  # noqa: N818
    ...
    def catch(cls, response, include=None, exclude=None):
        ...
        def to_int(x):
            if issubstance(x, HTTPException):
                return int(x.value)
