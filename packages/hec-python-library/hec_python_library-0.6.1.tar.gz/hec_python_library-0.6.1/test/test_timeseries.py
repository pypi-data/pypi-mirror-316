...
def test_math_ops_scalar() -> None:
    ...
    times = pd.Index(  # type: ignore
        [(start_time + i * intvl).datetime() for i in range(value_count)], name="time"
    )
...
def test_aggregate_ts() -> None:
    ...
    ts = TimeSeries.aggregate_ts(
        lambda i: math.fmod(i[0], i[1]), [timeseries[ts_count - 1], timeseries[0]]
    )
    for i in range(value_count):
        expected = math.fmod(
...
def test_aggregate_values() -> None:
    ...
    times = pd.Index(  # type: ignore
        [(start_time + i * intvl).datetime() for i in range(len(values))], name="time"
    )
...
def test_min_max() -> None:
    ...
    assert ts.minValueTime() == start_time + 2 * intvl
    assert ts.maxValueTime() == start_time + 5 * intvl
    ts.selection_state = SelectionState.DURABLE
    ts.iselect(lambda tsv: 10 < tsv.value < 100)
    assert ts.minValue() == 11.0
    assert ts.maxValue() == 33.0
    assert ts.minValueTime() == start_time + intvl
    assert ts.maxValueTime() == start_time + 23 * intvl
...
def test_value_counts() -> None:
    ...
    # no selection #
    # --------------#
    assert ts.numberValues == len(values)
    assert ts.numberInvalidValues == 5
    assert ts.numberValidValues == len(values) - 5
    assert ts.numberMissingValues == 2
    assert ts.numberQuestionedValues == 1
    assert ts.numberRejectedValues == 1
    assert ts.firstValidValue == 1030
    assert HecTime("2024-10-10T03:00:00") == ts.firstValidTime
    assert ts.lastValidValue == 1345
    assert HecTime("2024-10-10T24:00:00") == ts.lastValidTime
    # ----------------#
    # with selection #
    # ----------------#
    ts2 = ts.select(
    ...
    assert ts2.numberValues == 10
    assert ts2.numberInvalidValues == 2
    assert ts2.numberValidValues == 8
    assert ts2.numberMissingValues == 1
    assert ts2.numberQuestionedValues == 0
    assert ts2.numberRejectedValues == 0
    assert ts2.firstValidValue == 1165
    assert HecTime("2024-10-10T12:00:00") == ts2.firstValidTime
    assert ts2.lastValidValue == 1270
    assert HecTime("2024-10-10T19:00:00") == ts2.lastValidTime
...
def test_unit() -> None:
    ...
    assert ts.isEnglish
    assert not ts.isMetric
    assert ts.canDetermineUnitSystem
