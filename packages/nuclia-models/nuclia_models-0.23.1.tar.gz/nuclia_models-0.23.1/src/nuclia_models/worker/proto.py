...
class DataAugmentation(BaseModel):
    ...
    filter: typing.Optional[Filter] = Field(
        default=None,
        description="Filter to apply the data augmentation, if no filter is provided or the supplied filter does not specify `field_types` or `not_field_types`, the data augmentation will ignore generic (`a`) fields such as titles or summaries",
    )
