...
from pydentity.interfaces import IUserClaimsPrincipalFactory, IUserConfirmation, ILogger
...
class SignInManager(Generic[TUser]):
    ...
    async def password_sign_in(
        ...
        if not user:
            ...
            self.user_manager.password_hasher.hash_password(user, password)  # type: ignore
