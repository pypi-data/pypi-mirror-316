...
class UserManager(Generic[TUser]):
    ...
    @overload
    async def get_user_id(self, user: TUser) -> str | None:
        """
    ...
    async def find_by_id(self, user_id: str) -> TUser | None:
        """
    ...
    async def find_by_name(self, username: str) -> TUser | None:
        ...
        assert normalized_username
        return await self.store.find_by_name(normalized_username)
    ...
    async def get_lockout_end_date(self, user: TUser) -> dt.datetime | None:
        """
    ...
    async def access_failed(self, user: TUser) -> IdentityResult:
        ...
        await store.set_lockout_end_date(user, datetime.now().add(self.options.lockout.default_lockout_timespan))
    ...
    def _get_random_recovery_code_char(self) -> str:
        return "".join(secrets.choice("23456789BCDFGHJKMNPQRTVWXY") for i in range(8))
