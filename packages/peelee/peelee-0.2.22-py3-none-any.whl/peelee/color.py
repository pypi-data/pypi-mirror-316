...
COLOR_8BIT_HEX_CODE_LIST = [
    ...
    "#FF0000", "#FF00FF", "#FFFF00", "#FFFFFF", "#000000", "#00005F",
...
def calculate_relative_luminance(hex_color):
    ...
    r, g, b = tuple(_srgb / 12.92 if _srgb <= 0.03928 else ((_srgb + 0.055) /
                                                            1.055)**2.4
...
def convert_to_best_light_color(
    ...
    contrast_ratio = calculate_contrast_ratio(base_light_color,
                                              target_dark_color)
    ...
    filter_better_colors = list(
        filter(
            ...
            min_contrast_ratio and calculate_contrast_ratio(
                x, target_dark_color) <= max_contrast_ratio,
            better_colors,
        ))
...
def convert_to_best_dark_color(
    ...
    contrast_ratio = calculate_contrast_ratio(target_light_color,
                                              base_dark_color)
    if contrast_ratio >= min_contrast_ratio and contrast_ratio <= max_contrast_ratio:
        return best_color
    ...
    filter_better_colors = list(
        filter(
            ...
            min_contrast_ratio and calculate_contrast_ratio(
                target_light_color, x) <= max_contrast_ratio,
            better_colors,
        ))
...
def find_the_cloest_color(hex_color, hex_color_list):
    ...
    The closest hex color in hex_color_list. E.g. #575757.
        """
    color_luminances = [
        calculate_relative_luminance(x) for x in hex_color_list
    ]
    hex_color_luminance = calculate_relative_luminance(hex_color)
    luminance_distances = [
        abs(hex_color_luminance - x) for x in color_luminances
    ]

    color_hues = [CU.hex2hls(x)[0] for x in hex_color_list]
    hex_color_hue = CU.hex2hls(hex_color)[0]
    hex_distances = [
        abs(hex_color_hue - color_hue) for color_hue in color_hues
    ]

    luminance_hue_multi_distances = [
        luminance_distances[i] * hex_distances[i]
        for i in range(0, len(hex_color_list))
    ]

    color_ratio_map = dict(zip(hex_color_list, luminance_hue_multi_distances))
    sorted_hex_color_list = sorted(hex_color_list,
                                   key=lambda x: color_ratio_map[x])
...
def convert_hex_to_8bit(hex_color: str) -> int:
    ...
    assert re.match("#[0-9a-zA-Z]{6,8}",
                    hex_color), "f{hex_color} is not a valid HEX color code."
