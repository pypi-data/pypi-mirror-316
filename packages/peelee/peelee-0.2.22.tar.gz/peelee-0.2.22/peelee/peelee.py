...
def get_scheme_colors(hex_color, n_colors=7):
    ...
    assert (n_colors is not None and isinstance(n_colors, int)
            and n_colors > 0), f"Invalid argument: n_colors = {n_colors}"
    ...
    while CU.is_black_or_gray(random_color):
        random_color = generate_random_hex_color_code(
            0, 255, color_name=ColorName.RANDOM)
    ...
    base_colors = [hls2hex(hls_color)
                   for hls_color in triadic_colors][0:n_colors]
...
def generate_random_colors(
    ...
    **kwargs,
    ...
    if base_color:
        logger.debug(
            f"set hsl({hue}, {saturation}, {lightness}) to {base_color} - {palette_mode}"
        )
        base_color = CU.set_hls_values(base_color, hue, saturation, lightness)
    is_black_or_gray_color = CU.is_black_or_gray(base_color)
    logger.debug(f"{base_color} is black or gray: {is_black_or_gray_color}")
    ...
    logger.debug(f"Base Color: {base_color}, Random Color: {random_color}")
    ...
    for _base_color in base_colors:
        slice_colors = get_slice_colors(_base_color, color_gradations,
                                        color_slice_type, **kwargs)
        random_colors_list.append(slice_colors)

    return random_colors_list


def generate_slice_colors(min_color=0,
                          max_color=231,
                          color_gradations=24,
                          hue=None,
                          saturation=None,
...
def generate_random_hex_color_code(
    ...
    if diff < COLOR_RANGE_DENOMINATOR:
        ...
        for index in range(1, COLOR_RANGE_DENOMINATOR,
                           int(COLOR_RANGE_DENOMINATOR / 3.0)):
            random_int = random.randint(min_color + (index * step),
                                        min_color + ((index + 2) * step))
            _random_color = padding(hex(random_int))
            random_hex_color_code.append(_random_color)
    ...
    if not isinstance(color_name, ColorName):
        random_hex_color = set_hls_values(random_hex_color, hue, saturation,
                                          lightness)
...
class Palette:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert (self.colors_min
                <= self.colors_max), "colors_min must be less than colors_max."
        self.colors_hue = kwargs.get("colors_hue",
        ...
        if self.colors_hue is not None:
            assert (self.colors_hue >= 0 and self.colors_hue
                    <= 1), "colors_hue must be greater than 0 and less than 1."
        ...
        assert (self.dark_colors_colors_gradations
                > 0), "dark_colors_colors_gradations must be greater than 0."
        self.dark_colors_min = dark_colors_min
        self.dark_colors_max = dark_colors_max
        assert (self.dark_colors_min <= self.dark_colors_max
        ...
        if self.dark_colors_saturation is not None:
            assert (
                self.dark_colors_saturation >= 0
                and self.dark_colors_saturation <= 1
            ), "dark_colors_saturation must be greater than 0 and less than 1."
        if self.dark_colors_lightness is not None:
            assert (
                self.dark_colors_lightness >= 0
                and self.dark_colors_lightness <= 1
            ), "dark_colors_lightness must be greater than 0 and less than 1."
        dark_base_color_name: str = kwargs.get("dark_base_color_name",
                                               ColorName.RANDOM)
        if not any(member.name == dark_base_color_name
                   for member in ColorName):
            self.dark_base_color_name = ColorName.RANDOM
        else:
            self.dark_base_color_name = ColorName[dark_base_color_name]
        palette_mode: str = kwargs.get("palette_mode", "DARK")
        ...
        logger.debug(self.dark_base_color)
    ...
    def generate_palette_colors(self):
        ...
        normal_colors = generate_random_colors(
            ...
            keep_saturation=self.keep_saturation)
        ...
        for palette_mode, slice_color_map in slice_colors.items():
            for color_name, color_codes in slice_color_map.items():
                ...
                for index, color in enumerate(color_codes):
                    color_index = padding(index,
                                          max(len(str(color_codes_total)), 2))
                    palette_colors[
                        f"{palette_mode.value}_{color_name.value}_{color_index}"] = color
...
def generate_palette_plain_text():
    ...
    return "\n".join(
        [f"{color_name}:{color}" for color_name, color in palette.items()])
...
def generate_theme_template(theme_filepath):
    ...
    with open(theme_filepath, 'r', encoding="utf-8") as f_theme:
        ...
        for hex_color in original_hex_color_list:
            try:
                logger.debug(CU.hex2hls(hex_color))
            except ValueError:
                invalid_hex_colors.append(hex_color)

        original_hex_colors = sorted(
            list(
                set(original_hex_color_list).difference(
                    set(invalid_hex_colors))))
        logger.debug(len(original_hex_colors))
...
def generate_new_theme(original_theme_file,
                       theme_root: str,
                       theme_name: str,
    ...
    if os.path.exists(theme_template_filepath):
        ...
        with open(theme_template_filepath, "w",
                  encoding="utf-8") as theme_template_file:
    ...
    # save palette
    palette_filepath = f"{theme_root}/palette.json"
    with open(palette_filepath, "w", encoding="utf-8") as palette_file:
        json.dump(palette_colors, palette_file, indent="  ")
    ...
    theme_filepath = f"{theme_root}/{theme_name}"
