...
def EvaluationWithBuildingFootprint(main_dir, method_name, country=None, building_footprint=None):
        ...
        for folder_name in os.listdir(method_path):
            if folder_name == "MaskedFIMwithBoundary":
                ...
                if building_footprint is None:
                    building_footprint = find_existing_footprint(out_dir)
                    if not building_footprint:
                        msf.getBuildingFootprint(country, boundary, out_dir)
                        building_footprint = os.path.join(out_dir, "building_footprint.gpkg")
        ...
        if os.path.isdir(folder_path):
            ...
            for folder_name in os.listdir(method_path):
                if folder_name == "MaskedFIMwithBoundary":
                    ...
                    # Reset footprint to ensure per-folder uniqueness
                    ...
                    if building_footprint is None:
                        ...
                        if not building_footprint:
                            msf.getBuildingFootprint(country, boundary, out_dir)
                            building_footprint = os.path.join(out_dir, f"building_footprint.gpkg")
