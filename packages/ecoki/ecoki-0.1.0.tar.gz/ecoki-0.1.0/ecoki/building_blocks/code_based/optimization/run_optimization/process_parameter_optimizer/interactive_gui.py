...
class InteractiveGUI(AbstractInteractiveGUI):
    ...
    def create_boundaries_layout(self, column = None, operator=None, value=None):
        ...
        text_database_name = pn.widgets.Select(name='Operator', options=operator_options, placeholder='', value = operator)
        ...
        boundaries_layout = pn.Row(select_database_url, text_database_name, value_database_name, remove_button_column,styles=dict(background='LightGray'))
    ...
    def create_optimization_target_layout(self, process_parameter = None, left_bound = None, right_bound=None, exploration_radius=None,resolution=None):
        ...
        if process_parameter is None:
            ...
            range_slider = pn.widgets.RangeSlider(name='Grenzen für die Optimierung', start=0, end=1, value=(0, 1), step=0.1, bar_color='#228b22')
            exploration_radius_input = pn.widgets.FloatInput(name='Explorationsradius', value=0, placeholder='', start = 0,end = 1, step = 0.1)
            resolution_input = pn.widgets.FloatInput(name='Auflösung/Schrittweite', value=0, placeholder='', start = 0,end = 1,step = 0.1)
        ...
        # for the case values are set
            ...
            process_parameter = pn.widgets.Select(name='Process_Parameter', options=column_options, value = process_parameter)
            range_slider = pn.widgets.RangeSlider(name='Choose left and right bound', start=min_value-exploration_radius, end=max_value+exploration_radius, value=(left_bound, right_bound), step=resolution, format = determine_format(resolution,min_value,max_value), bar_color='#228b22')
            exploration_radius_input = pn.widgets.FloatInput(name='Explorationsradius definieren', value=exploration_radius, placeholder='', start = exploration_radius_calc_rounded/10,end = exploration_radius_calc_rounded*10, step = resolution)
            resolution_input = pn.widgets.FloatInput(name='Auflösung/Schrittweite definieren', value=resolution, placeholder='', start = resolution_stepsize/100,end = resolution_stepsize*1000,step = resolution_stepsize)
        ...
        def process_param_callback(event,row,training_data):
            ...
            resolution = 10 ** ceil(log10((training_data[selected_parameter].max() - training_data[selected_parameter].min()) / 1000))
            exploration_radius_rounded = myround(exploration_radius, resolution)
            resolution_stepsize = 10 ** ceil(log10(resolution / 10))

            # set range slider values according
            row[1].format = determine_format(resolution, myround(min_value-exploration_radius,resolution), myround(max_value+exploration_radius,resolution))
        ...
        def resolution_input_callback(event,row,training_data):
            ...
            if row[3].value > row[3].start:
                if row[3].value < row[3].end:
                    ...
                    row[1].format = determine_format(resolution, myround(row[1].start,resolution), myround(row[1].end,resolution))
        ...
        process_parameter.param.watch(partial(process_param_callback, row=optimization_params_layout,training_data = training_data), 'value')

        # add the callback when the resolution is changed
        resolution_input.param.watch(partial(resolution_input_callback, row=optimization_params_layout,training_data = training_data), 'value')

        # add the callback when the exploration_radius is changed
        exploration_radius_input.param.watch(partial(exploration_radius_input_callback, row=optimization_params_layout,training_data = training_data), 'value')
    ...
    def run_interactive_gui(self, **kwargs):
        ...
        save_button = pn.widgets.Button(name='Speichere Konfiguration und fahre mit Pipeline-Konfiguration fort', button_type='primary')

        # create save button widget for GUI 2
        save_button_1 = pn.widgets.Button(name='Speichere Konfiguration und fahre mit Pipeline-Konfiguration fort', button_type='primary')
        ...
        def on_save_button_click(event):
            ...
            for idx in range(len(self.layout)-2):
                if self.layout[idx][2].value is not None:
                    boundary_conditions["condition_"+str(counter)] = {"label_name":self.layout[idx][0].value,"operator":self.layout[idx][1].value,"boundary":self.layout[idx][2].value}
                else:
            ...
            if valid_boundaries:
                ...
                self.layout[-1] = pn.Column(pn.pane.Alert("Die Einstellungen zur Optimierungszielfunktion wurden erfolgreich übernommen. Bitte fahren Sie mit der Konfiguration des nächsten Bausteins fort", alert_type="success"))
                ...
                self.layout[-1],= pn.Column(pn.pane.Alert("Die Einstellungen zur Optimierungszielfunktion sind nicht gültig oder unvollständig. Bitte überprüfen Sie die Angaben.", alert_type="danger"))
        ...
        def on_save_button_click_1(event):
            ...
            for idx in range(len(self.layout_2)-2):
                ...
                optimisation_parameters[self.layout_2[idx][0].value] = {"left_bound":self.layout_2[idx][1].value[0],"right_bound":self.layout_2[idx][1].value[1],"exploration_radius":self.layout_2[idx][2].value,"resolution":self.layout_2[idx][3].value}
            ...
            if valid_boundaries:
                ...
                # give message through GUI
                self.layout_2[-1]= pn.Column(pn.pane.Alert("Die Einstellungen zur Optimierungszielfunktion wurden erfolgreich übernommen. Bitte fahren Sie mit der Konfiguration des nächsten Bausteins fort", alert_type="success"))
        ...
        self.layout_optimization_target = pn.Column(
            pn.Row(pn.widgets.Select(name='Wählen Sie das zu minimierende Merkmal aus:', options=list(training_labels),value=""))
        )

        # init the GUI with the existing settings
        for condition,condition_value in self.building_block.settings['objective_function']['boundary_conditions'].items():
            self.layout.insert(0, self.create_boundaries_layout(column = condition_value["label_name"], operator=condition_value["operator"], value=condition_value["boundary"]))

        for process_param_name,process_param_values in self.building_block.settings['optimisation_parameters'].items():
            self.layout_2.insert(0, self.create_optimization_target_layout(process_parameter = process_param_name, left_bound = process_param_values["left_bound"], right_bound=process_param_values["right_bound"], exploration_radius=process_param_values["exploration_radius"],resolution=process_param_values["resolution"]))
