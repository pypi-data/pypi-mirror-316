...
def myround(x, base):
    ...
    Args:
        x (numpy.array): The values to be rounded.
        base (float): The interval of values to be rounded to.

    Returns:
        numpy.array: Rounded values.
    """
...
def optimisation_function(pp, *params):
    ...
    Returns:
        numpy.array: Overall rating for the optimization.
    """
    ...
    # penalize combinations far away from neighbors (factor 100)
    qr = np.asarray(
...
class ProcessParameterOptimizer(BuildingBlock):
    """
    A building block for optimizing process parameters based on a machine learning model.

    This class performs black-box optimization using a machine learning model. The optimization
    function is defined based on the output labels of the used model. All settings can be
    adjusted in the pipeline settings.
    ...
    Attributes:
        ...
        description (str): A description of the building block's functionality.
    """

    def __init__(self, **kwargs):
        """
        Initialize the ProcessParameterOptimizer.
    ...
    def predict(self, features):
        ...
        Args:
            features (pandas.DataFrame): The input features for prediction.
        ...
        return pd.DataFrame(columns=self.column_names, data=y_pred)

    def rate_quality_results(self, predictions):
        """
        Rate the quality of the predictions.

        Args:
            predictions (pandas.DataFrame): The predictions to be rated.

        Returns:
            numpy.array: The ratings for the predictions.
        """
    ...
    def objective_function(self, predictions, include_predictions=True, only_rating=False):
        ...
        Args:
            predictions (pandas.DataFrame): The predictions to be evaluated.
            include_predictions (bool, optional): Whether to include predictions in the output. Defaults to True.
            only_rating (bool, optional): Whether to return only the rating. Defaults to False.

        Returns:
            pandas.DataFrame: The predictions with added rating column.
        """
    ...
    def execute(self, input_data):
        """
        Execute the process parameter optimization.

        Args:
            input_data (list): A list containing training features, test features, training labels, and test labels.

        Returns:
            dict: A dictionary containing the output data, split train-test data, and settings.
        """
        ...
        return {"output_data": result_df, "output_data_split_train_test": input_data, "output_data_settings": self.settings}


    def optimize_batch(self, features):
        """
        Optimize a batch of samples according to a given cost function.

        Args:
            features (pandas.DataFrame): The features of the samples to be optimized.

        Returns:
            pandas.DataFrame: A dataframe containing the proposed parameter proposals.
        """
    ...
    def optimize_sample(self, feature):
        ...
        Args:
            feature (pandas.DataFrame): The features of the sample to be optimized.

        Returns:
            pandas.DataFrame: A dataframe containing the proposed parameter proposal.
        """
        ...
        # define combinations for discrete process parameters
        ...
        # for all discrete parameter combinations
