...
def _gather_dataset_info(glottolog_data, save=True):
    """
    ...
    data = []
    fields = ['id', 'name', 'lang', 'paralex_version', 'version', 'contributors', 'title', 'doi', 'comment']
    for record in records:
        rec_id = record['id']
        if (old is None) or not (rec_id in old.index):
...
def list_datasets(args):
    ...
    df = _gather_dataset_info(glottocode_data, save=False)
    if args.iso:
        log.info('Returning datasets for ISO code %s only', args.iso)
        df = df[df.lang.isin(args.iso)]
    df = df[['doi', 'lang', 'lang_name', 'family', 'version', 'paralex_version']]
    if args.output:
