...
@click.option(
    ...
    help=f"Submission date, default is today: {datetime.date.today()}.",
)
@click.option(
    "--expiration_date",
    type=click.DateTime(formats=["%Y-%m-%d"]),
    default=str((datetime.datetime.now() + relativedelta(years=3)).date()),
    required=True,
    prompt="Enter the expiration date of this FedRAMP document.",
    help=f"Expiration date, default is {str((datetime.datetime.now() + relativedelta(years=3)).date())}.",
)
def load_fedramp_oscal(file_path, submission_date, expiration_date):
    ...
    if not expiration_date:
        ...
        expiration_date = datetime.date(today_dt.year + 3, today_dt.month, today_dt.day)
...
def import_fedramp_inventory(
    ...
    if link_path.is_dir():
        ...
        for file in files:
            upload(
                inventory=file, sheet_name=sheet_name, record_id=regscale_id, module=regscale_module, version=version
            )
...
def import_fedramp_poam_template(
    ...
    import warnings
    from regscale.integrations.public.fedramp.models.poam_importer import POAM
...
def import_ciscrm(
    ...
    from regscale.integrations.public.fedramp.fedramp_common import parse_and_import_ciscrm
