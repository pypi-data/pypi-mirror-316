...
def import_command_with_timing(
    ...
    if logger.getEffectiveLevel() == logging.DEBUG:
        ...
        if elapsed_time > 0.01 and logger.level >= logging.DEBUG:
            logger.warning(
                f"Warning: Importing {command_name} from {module_name} took {elapsed_time} seconds to import!"
            )
...
fedramp = import_command_with_timing("regscale.core.app.public", "fedramp")
...
aws = import_command_with_timing(COMMERCIAL, "aws")
...
durosuite = import_command_with_timing(COMMERCIAL, "durosuite")
