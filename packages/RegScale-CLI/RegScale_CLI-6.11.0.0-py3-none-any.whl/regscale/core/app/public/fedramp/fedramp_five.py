...
from regscale.core.app.public.fedramp.markdown_parser import MDDocParser
...
from regscale.core.app.public.fedramp.rosetta import RosettaStone
...
def extract_parts(html_string):
    """
    Extract parts from an HTML string, where each part is labeled as 'Part a', 'Part b', etc.

    Args:
        html_string (str): The input string containing HTML content with labeled parts.

    Returns:
        dict: A dictionary where keys are 'Part a', 'Part b', etc., and values are the full HTML content of each part.
    """
    parts_dict = {}
    # Regex to capture "Part <letter>" and the following HTML content until the next "Part <letter>"

    for match in PART_PATTERN.finditer(html_string):
        part_content = match.group(1).strip()  # Full match including "Part <letter>" and HTML
        part_key_match = re.search(r"<p>(Part\s[a-zA-Z]):</p>", part_content)
        if part_key_match:
            part_key = part_key_match.group(1).strip()  # Extract the "Part a" as the key
            parts_dict[part_key] = part_content  # Save full content under the key
...
def handle_parts(
    ...
    for index, part in enumerate(parts):
        ...
        matching_objectives = (
            [o for o in control_objectives if o.name.startswith(part_letter)]
...
def map_responsibility(responsibility: str) -> str:
    ...
    if responsibility == "Service Provider System Specific":
        return ImplementationObjectiveResponsibility.PROVIDER_SYSTEM_SPECIFIC.value
    if responsibility == SERVICE_PROVIDER_CORPORATE:
        return ImplementationObjectiveResponsibility.PROVIDER.value
    if responsibility == "Provided by Customer (Customer System Specific)":
        return ImplementationObjectiveResponsibility.CUSTOMER.value
    if responsibility == "Configured by Customer (Customer System Specific)":
        return ImplementationObjectiveResponsibility.CUSTOMER_CONFIGURED.value
    if responsibility == "Service Provider Hybrid (Corporate and System Specific)":
        return ImplementationObjectiveResponsibility.HYBRID.value
    if responsibility == "Inherited from pre-existing FedRAMP Authorization":
        return ImplementationObjectiveResponsibility.INHERITED.value
    if responsibility == ImplementationObjectiveResponsibility.NOT_APPLICABLE.value:
        return ImplementationObjectiveResponsibility.NOT_APPLICABLE.value
    if responsibility == "Shared (Service Provider and Customer Responsibility)":
        return ImplementationObjectiveResponsibility.SHARED.value
...
def handle_implementation_objectives(
    ...
    imp_obj = ImplementationObjective(
        ...
        responsibility=map_responsibility(responsibility=origination) if origination else None,
    )
...
def handle_params(
    ...
    for param in parameters:
        gen_param_name = f"Parameter {param.get('name').strip()}"
...
def update_existing_control(
    ...
    try:
        ...
        justify = (
            state_text or "Unknown"
            if map_implementation_status(status) == ControlImplementationStatus.NA.value
            else None
        )
        control.parentId = parent_id
        ...
        logger.warning(f"Error updating control: {e}")
