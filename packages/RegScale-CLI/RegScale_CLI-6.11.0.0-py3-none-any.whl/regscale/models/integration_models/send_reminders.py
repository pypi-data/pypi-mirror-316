...
class SendReminders:
    ...
    def __init__(self, app: Application, days: int = 30):
        ...
        self.days = days
        self.base_url = self.config["domain"] + "/api/"
        self.users = []
    ...
    def get_and_send_reminders(self) -> None:
        ...
        url = f'{self.base_url}accounts/find/{self.config["userId"]}'
        ...
        response = self.api.get(url=f"{self.base_url}accounts/{ten_id}/True")
        activated_users_response = self.api.get(url=f"{self.base_url}accounts")
