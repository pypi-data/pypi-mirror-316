...
from regscale.core.app.utils.file_utils import read_file, find_files, move_file, iterate_files, get_processed_file_path
...
class NessusIntegration(ScannerIntegration):
    ...
    def _check_path(self, path: Optional[str] = None) -> None:
        """
    ...
    def parse_finding(self, vuln: Any, asset_id: str) -> Optional[IntegrationFinding]:
        ...
        try:
            ...
            identifier = vuln.get("cve") or vuln.get("pluginName", "")
            ...
            return IntegrationFinding(
                ...
                plugin_name=vuln.get("cve") or plugin_name,
                ip_address=asset_id,
                dns=None,
    ...
    def fetch_assets(self, *args: Any, path: str) -> Iterator[IntegrationAsset]:  # type: ignore
        """
    ...
    def get_vulnerability_data(cls, file_vuln: Any) -> dict:
        ...
        cvss_base_score = safe_float(get("cvss_base_score"))
        cve = get("cve")
    ...
    def move_files(file_collection: List[Union[Path, str]]) -> None:
        ...
        for file in file_collection:
            ...
            logger.info(f"Moved Nessus file {file} to {new_file}")
