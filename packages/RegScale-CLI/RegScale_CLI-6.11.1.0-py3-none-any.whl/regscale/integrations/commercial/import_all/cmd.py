...
from regscale.core.app.utils.file_utils import get_files_by_folder
...
@import_all.command(name="import_all")
...
def import_all_scans(
    ...
    from regscale.integrations.commercial.aws import import_scans as import_aws
    ...
    scans = set_scans(
        get_files_by_folder(folder_path, exclude_non_scan_files=True), load_fingerprints(FINGERPRINT_FILE_PATH)
    )
    move_files(scans, folder_path)
    ...
    if input(f"Do you want to proceed with processing {total_files} scan file(s)? (y/n): ").strip().lower() != "y":
        ...
        import_scans(imports_to_process, scan_folders, regscale_ssp_id, scan_date)


def import_scans(imports_to_process: dict, scan_folders: dict, regscale_ssp_id: int, scan_date: datetime) -> None:
    """
    ...
    for scan_type, import_detail in imports_to_process.items():
        ...
        custom_mapping = import_detail["custom_mapping"]
        if custom_mapping:
            ctx.invoke(
                click_function,
                folder_path=scan_folders[scan_type],
                regscale_ssp_id=regscale_ssp_id,
                scan_date=scan_date,
                mappings_path=custom_mapping,
            )
        else:
            ctx.invoke(
                click_function,
...
def load_fingerprints(fingerprint_file_path: str) -> dict:
    ...
    if os.path.exists(fingerprint_file_path):
        ...
        with pkg_resources.open_text("regscale.integrations.commercial.import_all", "capabilities.json") as file:
