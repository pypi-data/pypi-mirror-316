...
from regscale.models import OpenIssueDict, ScanHistory, regscale_models
...
class IntegrationFinding:
    ...
    :param Optional[str] cvs_sv3_base_score: The CVSS v3 base score of the finding, defaults to None.
    ...
    def get_issue_status(self) -> regscale_models.IssueStatus:
        """
        Gets the issue status based on the finding status
...
class ScannerIntegration(ABC):
    ...
    def __init__(
        ...
        self.regscale_version: str = regscale_models.Issue._api_handler.regscale_version  # noqa
    ...
    def create_or_update_issue_from_finding(
        ...
        cisa_kevs = pull_cisa_kev()

        with finding_id_lock:
            issue_title = self.get_issue_title(finding) or title
            description = finding.description
            remediation_description = finding.recommendation_for_mitigation or finding.remediation
            ...
            if ScannerVariables.issueCreation.lower() != "perasset":
                ...
                if issue_status == regscale_models.IssueStatus.Open:
                    ...
                    if issue:
                        ...
                        if finding.cve:
                            issue = self.lookup_kev_and_upate_issue(cve=finding.cve, issue=issue, cisa_kevs=cisa_kevs)

                        issue.status = issue_status
                        issue.dateLastUpdated = get_current_datetime()
                        if not issue.dateFirstDetected:
                            # This could be conceivably null if the issue was manually created, a bit of an edge case.
                            issue.dateFirstDetected = self.scan_date or finding.date_created
                        # Possible overridden fields, need to make sure they're always updated
                ...
                # Check if we should consolidate closed issues based on integrationFindingId and issueDueDates
                    ...
                    if matching_closed_issue:
                        # Update the existing closed issue
                        existing_asset_identifiers = set((matching_closed_issue.assetIdentifier or "").split(delimiter))
                        if finding.asset_identifier not in existing_asset_identifiers:
                            existing_asset_identifiers.add(finding.asset_identifier)
                            matching_closed_issue.assetIdentifier = delimiter.join(existing_asset_identifiers)

                        matching_closed_issue.dateLastUpdated = get_current_datetime()
                        matching_closed_issue = self.group_by_plugin(matching_closed_issue, finding)
                        return matching_closed_issue.save()

            def get_other_identifier():
            ...
            new_issue = regscale_models.Issue(
                parentId=parent_id,
            ...
            if finding.cve:
                new_issue = self.lookup_kev_and_upate_issue(cve=finding.cve, issue=new_issue, cisa_kevs=cisa_kevs)
            return new_issue.create_or_update(defaults={"otherIdentifier": get_other_identifier})

    @staticmethod
    def lookup_kev_and_upate_issue(
        cve: str, issue: regscale_models.Issue, cisa_kevs: Optional[dict] = None
    ) -> regscale_models.Issue:
        """
        ...
        :param Optional[dict] cisa_kevs: The CISA KEV data to search the findings
        ...
        if cisa_kevs:
            try:
                ...
                kev_data = next(
                    dat
                    for dat in cisa_kevs["vulnerabilities"]
                    if "vulnerabilities" in cisa_kevs and cve and dat["cveID"].lower() == cve.lower()
                )
        ...
        if kev_data:
            ...
            if kevlist := issue.kevList:
                issue.kevList = kevlist if cve in kevlist else f"{kevlist}, {cve}"
            else:
                issue.kevList = cve
        return issue
    ...
    def update_issue_for_passing_finding(
        ...
        :rtype: None
        """
        asset_identifiers = (issue.assetIdentifier or "").split(",")
    ...
    def close_issue(
        ...
        changes_text = f"{get_current_datetime('%b %d, %Y')} - Closed by {self.title} due to no associated assets."
        if issue.changes:
            issue.changes += f"\n{changes_text}"
        else:
    ...
    def update_regscale_checklists(self, findings: List[IntegrationFinding]) -> int:
        ...
        def process_finding(finding_to_process: IntegrationFinding) -> None:
            ...
            :param IntegrationFinding finding_to_process:
            """
    ...
    def update_regscale_findings_and_vulnerabilities(self, findings: Iterator[IntegrationFinding]) -> int:
        ...
        # We'll track current assets per finding_id as we go
        current_assets_for_finding: Dict[str, Set[str]] = defaultdict(set)

        def process_finding_threaded(finding_to_process: IntegrationFinding) -> None:
            """
            ...
            try:
                # Record the asset for this finding
                finding_id = self.get_finding_identifier(finding_to_process)
                current_assets_for_finding[finding_id].add(finding_to_process.asset_identifier)

                processed = self.process_finding(
                    finding_to_process, existing_issues_dict, scan_history, current_vulnerabilities
                )
                with count_lock:
        ...
        scan_history.save()
        logger.info("Closing outdated issues and vulnerabilities...")
        self.close_outdated_items(current_vulnerabilities)

        # Now remove stale asset identifiers from any remaining open issues
        self.remove_stale_asset_identifiers_from_issues(current_assets_for_finding)
    ...
    def get_existing_issues_dict(self) -> Dict[str, regscale_models.Issue]:
        ...
        return {f"{i.integrationFindingId}_{i.assetIdentifier}": i for i in issues}
        ...
        with self._get_lock("poam_id"):
            ...
            if not isinstance(self._max_poam_id, int):
                ...
                self._max_poam_id = max(
                    (
                        parsed_id
                        for poam in self.get_existing_issues_dict().values()
                        if poam.otherIdentifier and (parsed_id := self.parse_poam_id(poam.otherIdentifier)) is not None
                    ),
                    default=0,
                )
    ...
    def process_finding(
        ...
        if finding.cci_ref:  # Assume STIG if we have a CCI
            ...
            finding = self.update_integration_finding_dates(
                scan_history=scan_history, finding=finding, existing_issues_dict=existing_issues_dict
            )
            self.process_control_findings(finding=finding)
            self.process_vulnerability(finding, scan_history, current_vulnerabilities)
    ...
    def process_vulnerability(
        ...
        if asset and (vulnerability_id := self.handle_vulnerability(finding, asset, scan_history)):
            current_vulnerabilities[asset.id].add(vulnerability_id)

    def remove_stale_asset_identifiers_from_issues(self, current_assets_for_finding: Dict[str, Set[str]]) -> None:
        """
        Removes stale asset identifiers from issues that are no longer reported by the latest scan.
        Also closes associated vulnerability mappings when assets are removed.

        :param Dict[str, Set[str]] current_assets_for_finding: A dictionary mapping integrationFindingId to a set of asset identifiers
                                       currently reported by the scan.
        """
        logger.info("Removing stale asset identifiers from issues if they are no longer applicable.")

        # Fetch all open issues for this security plan
        ...
        for issue in open_issues:
            # If issue.integrationFindingId is present, then this issue is tied to a particular finding
            if issue.integrationFindingId:
                finding_id = issue.integrationFindingId
                # Assets currently reported for this finding_id by the latest scan
                current_assets = current_assets_for_finding.get(finding_id, set())
                # Assets recorded in the issue
                stored_assets = set((issue.assetIdentifier or "").split("\n"))

                # Determine which assets are stale (no longer appear in current scan)
                assets_to_remove = stored_assets - current_assets

                if assets_to_remove:
                    logger.info(
                        f"stored_assets: {stored_assets}, current_assets: {current_assets}, assets_to_remove: {assets_to_remove}"
                    )
                    updated_assets = stored_assets - assets_to_remove

                    # Close vulnerability mappings for removed assets
                    ...
                    for mapping in vuln_mappings:
                        # Get the asset for this mapping
                        if mapping.assetId and any(
                            asset.id == mapping.assetId for asset in self.get_assets_by_identifiers(assets_to_remove)
                        ):
                            mapping.status = regscale_models.IssueStatus.Closed
                            mapping.dateClosed = get_current_datetime()
                            mapping.save()
                            logger.info(
                                "Closed vulnerability mapping %d for issue %d and asset %d",
                                mapping.id,
                                issue.id,
                                mapping.assetId,
                            )
                    ...
                    if updated_assets:
                        # Update the issue with only the currently valid assets
                        issue.assetIdentifier = ",".join(updated_assets)
                        issue.dateLastUpdated = get_current_datetime()
                        issue.save()
                        logger.info("Removed stale asset identifiers %s from issue %d", assets_to_remove, issue.id)
                    else:
                        # If no assets remain, close the issue
                        issue.status = regscale_models.IssueStatus.Closed
                        issue.dateCompleted = get_current_datetime()
                        issue.save()
                        logger.info("Closed issue %d as no applicable assets remain.", issue.id)

    def get_assets_by_identifiers(self, asset_identifiers: Set[str]) -> List[regscale_models.Asset]:
        """
        Get assets by their identifiers.

        :param Set[str] asset_identifiers: Set of asset identifiers to look up
        :return: List of matching assets
        :rtype: List[regscale_models.Asset]
        """
        return [asset for identifier in asset_identifiers if (asset := self.get_asset_by_identifier(identifier))]
    ...
    def sync_findings(cls, plan_id: int, **kwargs) -> int:
        ...
        instance.scan_date = str(kwargs.get("scan_date") or "")
        ...
        with instance.finding_progress:
            ...
            if cls.type == ScannerIntegrationType.CHECKLIST:
                findings_processed = instance.update_regscale_checklists(findings=list(findings))
            else:
