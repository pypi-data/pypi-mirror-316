...
from regscale.core.app.utils.app_utils import reformat_str_date
...
class Assessment(RegScaleModel):
    ...
    def other_identifier_starts_with(cls, other_id: str) -> List["Assessment"]:
        ...
        response = cls._api_handler.get(
            endpoint=cls.get_endpoint("other_identifier_starts_with").format(
                module_slug=cls._module_slug, strID=other_id
            )
        )
    ...
    def get_enum_values(cls, field_name: str) -> list:
        ...
        if field_name == "status":
            return ["Scheduled", "In Progress", "Complete", "Cancelled"]
