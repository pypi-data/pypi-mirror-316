...
class AnthropicClient(Client):
    ...
    async def stream_chat_response(  # type: ignore
        ...
        async for chunk in response_stream:
            if chunk.type == "content_block_start":
                ...
                yield f"Tool: {tool_name}"  # yield tool name (first chunk)
            elif chunk.type == "content_block_delta":
                tool_input_buffer += chunk.delta.partial_json
                yield chunk.delta.partial_json  # yield chunk of the input
            elif chunk.type == "message_stop":
                tool_input_dict = json.loads(tool_input_buffer)
                tool_use_block_raw = {
                    "id": tool_id,
                    "name": tool_name,
                ...
                tool_use_block = ToolUseBlock(**tool_use_block_raw)
                tool_call = get_tool_call(tool_use_block)
                yield AIMessage(content=[tool_call])
                tool_id = tool_name = tool_input_buffer = ""
    ...
    def _get_params(
        ...
        kwargs = {
            ...
            "model": self.model,
            "system": self.format_content(system),
