...
def deferred(func) -> DeferredFunc:
    ...
    class DeferrerProperty:
            ...
            class Deferred:
                ...
                def __call__(cls):
                    ...
                    if data.commit_depth:
                        ...
                        raise RuntimeError(
                            ("This function is deferred; explicitly call " + f"{name}.deferred or {name}.now"))
        ...
        def __set__(self, instance, value):
            raise ValueError(
                (f"decorated {name} can't be mocked; " + f"mock '{name}.now' and/or '{name}.deferred'."))
...
def auto_commit(widget, master, value, label, auto_label=None, box=False,
    ...
    if not auto_label:
        if checkbox_label:
            ...
            auto_label = _tr.e(_tr.c(2, f"{label.title()} Automatically"))
...
def auto_apply(widget, master, value="auto_apply", **kwargs):
    ...
    :return: the box
    """
    return auto_commit(widget, master, value, _tr.m[5, "Apply"], _tr.m[6, "Apply Automatically"], **kwargs)
    ...
    def action(self, value):
        def action_str():
            ...
            try:
                ...
                if items:
                    msg = (f"Combo '{combo.objectName()}' has no item '{value}'; " + f"current items are {', '.join(map(repr, items))}.")
                else:
        ...
        def action_int():
            if value < combo.count():
                ...
                if combo.count():
                    msg = (f"index {value} is out of range " + (f"for combo box '{combo.objectName()}' " + f"with {combo.count()} item(s)."))
                else:
