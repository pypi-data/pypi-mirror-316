...
class R2DHandler(SpawnHandler):
    ...
    async def get(self, repotype_, repoowner_, repo_, ref_=''):
        ...
        if len(arguments_dict_bytes.get("name", [])) > 0:
            user_options["name"] = arguments_dict_bytes.get("name")[0].decode('utf-8')
        else:
        ...
        if len(arguments_dict_bytes.get("labpath", [])) > 0:
            user_options["notebook_type"] = "File"
            user_options["notebook"] = arguments_dict_bytes.get("labpath")[0].decode('utf-8')
        elif len(arguments_dict_bytes.get("urlpath", [])) > 0:
            user_options["notebook_type"] = "URL"
            user_options["notebook"] = arguments_dict_bytes.get("urlpath")[0].decode('utf-8')
                
        # Get System
        auth_state = await user.get_auth_state()
        if len(arguments_dict_bytes.get("system", [])) > 0:
            system = arguments_dict_bytes.get("system")[0].decode('utf-8')
        ...
        # Get Flavor
        if len(arguments_dict_bytes.get("flavor", [])) > 0:
            flavor = arguments_dict_bytes.get("flavor")[0].decode('utf-8')
        else:
