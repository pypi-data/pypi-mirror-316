...
from Buke_autoTrade_Pypi.indicator import sma, ibs, pivot_standard, 순거래대금, RSI, pivot_standard_for_sellprice
from Buke_autoTrade_Pypi.function import rank, ts_max, ts_min
from Buke_autoTrade_Pypi.parameter import Market, MovingAverage
from Buke_autoTrade_Pypi.tools import get_stock_exchange_tax, get_ask_price, get_bid_price
...
def add_indicator(day_price: DataFrame, index_day_price: DataFrame) -> DataFrame:
    ...
    for key, group in grouped:
        ...
        group['순거래대금_최댓값_5'] = ts_max(group['순거래대금'], 5)
        ...
        group['RSI_5'] = RSI(group['종가'], 5)
        group['RSI_5_이평선_5'] = sma(group['RSI_5'], 5)

        group['변동폭'] = group['종가'] / group['고가']
        group['변동폭_이평선_3'] = sma(group['변동폭'], 3)
        group['변동폭_이평선_10'] = sma(group['변동폭'], 10)

        group['변동폭_저가'] = group['종가'] / group['저가']
        group['변동폭_저가_이평선_5'] = sma(group['변동폭_저가'], 5)
        group['변동폭_저가_이평선_10'] = sma(group['변동폭_저가'], 10)
    ...
    for key, group in grouped:
        ...
        상승종목갯수 = (group['상승비율'] != 0).sum() + 1
        매도가격 = sum(group['상승비율']) / 상승종목갯수
        group['매도가격'] = 매도가격
        group['매도가격'] = group['매도가격'].fillna(0, inplace = True)
    ...
    for key, group in grouped:
        ...
        increase_condition2 = group['변동폭_저가'] < group['변동폭_저가_이평선_10']
        increase_condition3 = group['순거래대금'] < group['순거래대금_최댓값_5']
        increase_condition4 = group['RSI_5'] < group['RSI_5_이평선_5']
        increase_condition5 = group['변동폭_이평선_3'] < group['변동폭_이평선_10']

        increase_condition = increase_condition1 & increase_condition2 & increase_condition3 & increase_condition4 & \
                             increase_condition5

        liquidity_condition1 = group['거래대금_이평선_5_순위'] > 0.4
        liquidity_condition2 = group['시가총액_순위'] > 0.5
        ...
        group['#priority'] = -1 * group['변동폭_저가'] / group['변동폭_저가_이평선_10']
    ...
    index_day_price['#market_timing'] = (index_day_price['종가'] > index_day_price[['이평선_3', '이평선_5', '이평선_10']].min(
        axis=1)) & \
                ...
                for ticker in basket:
                    ...
                    if holding_condition1:
                        ...
                        try:
                            ...
                            if ticker in kospi_ticker_list:
                                tomorrow_target_sell_price = get_bid_price(tomorrow_pivot_standard_price * (1.005 + tomorrow_sell_price/2),
                                                                  Market.kospi, day)
                            else:
                                tomorrow_target_sell_price = get_bid_price(tomorrow_pivot_standard_price * (1.005 + tomorrow_sell_price/2),
                                                                  Market.kosdaq, day)
