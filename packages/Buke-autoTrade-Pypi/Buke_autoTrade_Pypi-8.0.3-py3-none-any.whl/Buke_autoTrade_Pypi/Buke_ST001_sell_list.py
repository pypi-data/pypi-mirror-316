...
from Buke_autoTrade_Pypi.indicator import sma, ibs, pivot_standard, stochastic_fast_k, pdi\
        , stochastic_fast_d, pivot_standard_for_sellprice, 순거래대금
from Buke_autoTrade_Pypi.function import rank
...
def add_indicator(day_price: DataFrame, index_day_price: DataFrame) -> DataFrame:
    ...
    for key, group in grouped:
        ...
        group['스토캐스틱_slow_20_5'] = stochastic_fast_d(group['고가'], group['저가'], group['종가'], 20, 5, MovingAverage.sma)
    ...
    for key, group in grouped:
        ...
        increase_condition3 = group['ibs'] < 0.9
        increase_condition4 = group['ibs_이평선_10'] > 0.45
        increase_condition5 = group['ibs_이평선_3'] < 0.6
        increase_condition6 = group['스토캐스틱_fast_20'] < group['스토캐스틱_slow_20_5']
