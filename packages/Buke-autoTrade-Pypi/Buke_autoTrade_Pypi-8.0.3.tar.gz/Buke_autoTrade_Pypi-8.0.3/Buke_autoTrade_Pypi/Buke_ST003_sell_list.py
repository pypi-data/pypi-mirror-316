...
from Buke_autoTrade_Pypi.indicator import sma, pivot_standard, stochastic_fast_k, stochastic_fast_d, pivot_standard_for_sellprice
from Buke_autoTrade_Pypi.function import rank, ts_min, ts_max
from Buke_autoTrade_Pypi.parameter import Market, MovingAverage
from Buke_autoTrade_Pypi.tools import get_ask_price, get_stock_exchange_tax, get_bid_price
...
def add_indicator(day_price: DataFrame, index_day_price: DataFrame) -> DataFrame:
    ...
    for key, group in grouped:
        ...
        group['거래대금_이평선_10'] = sma(group['거래대금'], 10)
        group['거래대금_이평선_20'] = sma(group['거래대금'], 20)
        ...
        group['상승비율'] = group['종가'] / group['종가'].shift(1) - 1
        group['상승비율_양수'] = group['상승비율'].apply(lambda x: max(0, x))

        group['종가_이평선_3'] = sma(group['종가'], 3)
        group['종가_이평선_20'] = sma(group['종가'], 20)

        group['종가_최댓값_5'] = ts_max(group['종가'], 5)

        group['변동폭'] = group['종가'] / group['고가']
        group['변동폭_이평선_3'] = sma(group['변동폭'], 3)
        group['변동폭_이평선_5'] = sma(group['변동폭'], 5)

        group['변동폭_저가'] = group['종가'] / group['저가']
        group['변동폭_저가_이평선_3'] = sma(group['변동폭_저가'], 3)

        group['스토캐스틱_fast_10'] = stochastic_fast_k(group['고가'], group['저가'], group['종가'], 10)
        group['스토캐스틱_slow_10_3'] = stochastic_fast_d(group['고가'], group['저가'], group['종가'], 10, 3, MovingAverage.sma)
    ...
    for key, group in grouped:
        ...
        상승종목갯수 = (group['상승비율_양수'] > 0).sum()
        상승비율합 = group['상승비율_양수'].dropna().sum()
        매도가격 = 상승비율합 / 상승종목갯수
    ...
    for key, group in grouped:
        ...
        increase_condition2 = group['종가_최댓값_5'] > group['종가_최댓값_5'].shift(5)
        increase_condition3 = group['종가_최댓값_5'].shift(5) > group['종가_최댓값_5'].shift(10)
        increase_condition4 = group['변동폭_이평선_3'] < group['변동폭_이평선_5'].shift(3)
        increase_condition5 = group['종가'] < group['종가_최댓값_5']
        increase_condition6 = group['변동폭_저가_이평선_3'] < group['변동폭_저가_이평선_3'].shift(3)
        increase_condition7 = group['종가'] / group['종가_이평선_3'] < 1
        ...
        group['#priority'] = group['스토캐스틱_slow_10_3'] - group['스토캐스틱_fast_10']
    ...
    index_day_price['종가_이평선_min'] = index_day_price[['이평선_3', '이평선_5', '이평선_10']].min(axis=1)

    #
    index_day_price['종가대비'] = index_day_price['종가'] - index_day_price['종가'].shift(1)
    index_day_price['종가대비_최솟값_10'] = ts_min(index_day_price['종가대비'], 10)

    index_day_price['#market_timing'] = (index_day_price['종가'] > 0.) & \
                                        (index_day_price['종가대비'] > index_day_price['종가대비_최솟값_10']) & \
                                        (index_day_price['종가'] > index_day_price['종가_이평선_min'])
            ...
            if len(basket) < max_basket_size and buy_check == 1:
                ...
                for ticker in kospi_ticker_list + kosdaq_ticker_list:
                    ...
                    try:
                        ...
                        tomorrow_vol = int(day_price[ticker]['거래량'].loc[day])
                        vol_condition = tomorrow_vol > 0
                ...
                if len(pre_buy_list) > 0:
                    ...
                    for priority_value, ticker in pre_buy_list[:max_basket_size - len(basket)]:
                        ...
                        stock_info = {'stock_num': stock_num, 'buy_price': buy_price, 'holding_days': 0,
                                      'name': today_name, 'buy_date': day, 'real_buy_price': real_buy_price, 'sell_price': sell_price}
            ...
            if len(basket) > 0:
                ...
                for ticker in basket:
                    ...
                    if holding_condition1:
                        ...
                        tomorrow_sell_price = float(day_price[ticker]['매도가격'].loc[day])


                        target_sell_price = float(basket[ticker]['sell_price'])

                        if ticker in kospi_ticker_list:
                            tomorrow_target_sell_price = get_bid_price(tomorrow_pivot_standard_price * (1 + tomorrow_sell_price/3),
                                                              Market.kospi, day)
                        else:
                            tomorrow_target_sell_price = get_bid_price(tomorrow_pivot_standard_price * (1 + tomorrow_sell_price/3),
                                                              Market.kosdaq, day)
                        ...
                        print(f"<Sell> date: {day}, ticker: {ticker}, sell_price: {sell_price}, profit: {avg}%")
...
def Buke_ST003_sell_list(dir, today):
