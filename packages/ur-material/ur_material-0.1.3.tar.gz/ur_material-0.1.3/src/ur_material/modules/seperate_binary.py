...
def process_image(image, num_shapes=100, reduce_percentage=0.4, method='white',file_name=None, save=False, random_seed=42):
    """
    ...
    for _ in range(num_shapes):
        ...
        if shape_type == 'circle':
            # 随机生成圆形
            radius = random.randint(5, 10)  # 半径范围，可根据需要调整
    ...
    if method == 'white':
        # 涂白处理
        if channels == 1:
            processed_image[final_mask == 255] = 0
        else:
            for c in range(channels):
                img_channel = processed_image[:, :, c]
                img_channel[final_mask == 255] = 0
