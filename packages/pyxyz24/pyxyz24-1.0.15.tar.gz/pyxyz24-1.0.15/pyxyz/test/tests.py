...
def iterators():
    p = _load_ensemble()
    indices = [m.idx for m in p]
    ref_indices = list(range(len(p)))
    assert indices == ref_indices

    indices = [m.idx for m in p[:3]]
    ref_indices = list(range(3))
    assert indices == ref_indices
...
def rmsd():
    ...
    manual_rmsd, rotation, j_center, reorder = p[i].rmsd(p[j])
    ...
    i_xyz: np.ndarray = p[i].xyz[reorder]
    j_xyz: np.ndarray = p[j].xyz[ordered_atoms]
    i_xyz -= i_xyz.mean(axis=0)
