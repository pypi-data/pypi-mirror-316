    ...
    def run(self, args) -> None:
        ...
        for observed_header, handler_function in self.OBSERVED_HEADERS_MODULES.items():
            ...
            if observed_header.lower() in (header.lower() for header in raw_headers.keys()):
                ...
                if observed_header == "Content-Security-Policy" and "Content-Security-Policy-Report-Only".lower() in (header.lower() for header in raw_headers.keys()):
                    # Add a warning instead of marking it as missing
