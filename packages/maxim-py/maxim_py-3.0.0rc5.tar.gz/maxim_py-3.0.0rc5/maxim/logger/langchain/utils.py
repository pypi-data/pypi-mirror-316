...
def parse_base_message_to_maxim_generation(message: BaseMessage):
    ...
    usage = message.__dict__["usage_metadata"] if message.__dict__[
        "usage_metadata"] else {}
    return {
        "id": str(uuid4()),
...
def parse_langchain_message(message: BaseMessage):
    ...
    if message_type == "ai":
        ...
        tool_calls = ai_message.tool_calls or message.lc_attributes.get(
            "tool_calls") or []
        return {
            "index": 0,
            "message": {
...
def parse_langchain_chat_generation_chunk(generation: ChatGeneration):
    ...
    finish_reason = generation.message.response_metadata.get(
        "stop_reason") if generation.message.response_metadata else "stop"
    if finish_reason is None:
        finish_reason = generation.generation_info.get(
            "finish_reason") if generation.generation_info else "stop"
    choices.append({
        'index': 0,
        'message':  {
            'role': "assistant",
...
def parse_langchain_generation(generation: Generation):
    ...
    elif generation.type == "ChatGenerationChunk":
    ...
    if generations is not None:
        for _, generation in enumerate(generations):
            for _, gen in enumerate(generation):
                usage_data = gen.__dict__.get(
                    "message").__dict__.get("usage_metadata")
                if usage_data is not None:
                    if usage_data.get("input_tokens") is not None:
                        prompt_tokens += usage_data.get("input_tokens", 0)
                        output_tokens += usage_data.get("output_tokens", 0)
                        total_tokens += usage_data.get(
                            "input_tokens", 0) + usage_data.get("output_tokens", 0)
                        continue
                    elif usage_data.get("prompt_tokens") is not None:
                        prompt_tokens += usage_data.get("prompt_tokens", 0)
                        output_tokens += usage_data.get("completion_tokens", 0)
                        total_tokens += usage_data.get(
                            "prompt_tokens", 0) + usage_data.get("completion_tokens", 0)
                        continue
                resp_metadata = gen.__dict__.get(
                    "message").__dict__.get("response_metadata")
                if resp_metadata is not None:
                    ...
                    usage_data = resp_metadata.get(
                        "amazon-bedrock-invocationMetrics") if resp_metadata else None
                    if usage_data is not None:
                        prompt_tokens += usage_data.get("inputTokenCount", 0)
                        output_tokens += usage_data.get("outputTokenCount", 0)
                        total_tokens += usage_data.get(
                            "inputTokenCount", 0) + usage_data.get("outputTokenCount", 0)
    return {
