...
def trace(
    logger: Logger,
    id: Optional[str] or Optional[Callable] = None,
    sessionId: Optional[str] or Optional[Callable] = None,
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            token_logger = _maxim_ctx_var_logger.set(logger)
            try:
                result = func(*args, **kwargs)
                print("tracing function end")
                return result
            finally:
                trace.end()
                _maxim_ctx_var_trace.reset(token)
                _maxim_ctx_var_logger.reset(token_logger)
