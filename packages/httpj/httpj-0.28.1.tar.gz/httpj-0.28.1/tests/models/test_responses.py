...
def test_response_content():
    response = httpj.Response(200, content="Hello, world!")
...
def test_response_text():
    response = httpj.Response(200, text="Hello, world!")
...
def test_response_html():
    response = httpj.Response(200, html="<html><body>Hello, world!</html></body>")
...
def test_response_json():
    response = httpj.Response(200, json={"hello": "world"})
...
def test_raise_for_status():
    ...
    response = httpj.Response(200, request=request)
    response.raise_for_status()

    # 1xx status codes are informational responses.
    response = httpj.Response(101, request=request)
    ...
    response = httpj.Response(303, headers=headers, request=request)
    ...
    response = httpj.Response(403, request=request)
    ...
    response = httpj.Response(500, request=request)
...
def test_iter_raw_with_chunksize():
    response = httpj.Response(200, content=streaming_body())
...
def test_iter_raw_doesnt_return_empty_chunks():
    ...
    response = httpj.Response(200, content=streaming_body_with_empty_chunks())
...
async def test_aiter_raw():
    response = httpj.Response(200, content=async_streaming_body())
...
def test_cannot_read_after_response_closed():
    ...
    response.close()
    with pytest.raises(httpj.StreamClosed):
        response.read()
...
def test_value_error_without_request(header_value):
    ...
    broken_compressed_body = b"xxxxxxxxxxxxxx"
    with pytest.raises(httpj.DecodingError):
        httpj.Response(200, headers=headers, content=broken_compressed_body)
...
def test_set_request_after_init():
    ...
    response.request = httpj.Request("GET", "https://www.example.org")
...
def test_generator_with_content_length_header():
    ...
    response = httpj.Response(200, content=content(), headers=headers)
...
async def test_response_async_streaming_picklable():
    response = httpj.Response(200, content=async_streaming_body())
    pickle_response = pickle.loads(pickle.dumps(response))
    with pytest.raises(httpj.ResponseNotRead):
...
def test_response_decode_text_using_autodetect():
    ...
    response = httpj.Response(200, content=content, default_encoding=autodetect)
...
def test_response_decode_text_using_explicit_encoding():
    ...
    response = httpj.Response(200, content=content, default_encoding="cp1252")
