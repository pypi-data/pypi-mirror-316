...
def test_cookies():
    cookies = httpj.Cookies({"name": "value"})
...
def test_cookies_with_domain():
    ...
    with pytest.raises(httpj.CookieConflict):
        cookies["name"]
...
def test_multiple_set_cookie():
    ...
    response = httpj.Response(200, request=request, headers=headers)

    cookies = httpj.Cookies(jar)
...
def test_cookies_can_be_a_list_of_tuples():
    ...
    cookies = httpj.Cookies(cookies_val)
