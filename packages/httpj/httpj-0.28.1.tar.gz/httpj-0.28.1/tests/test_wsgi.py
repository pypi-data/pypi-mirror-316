...
def test_wsgi():
    transport = httpj.WSGITransport(app=application_factory([b"Hello, World!"]))
...
def test_wsgi_upload():
    transport = httpj.WSGITransport(app=echo_body)
    client = httpj.Client(transport=transport)
...
def test_wsgi_upload_with_response_stream():
    transport = httpj.WSGITransport(app=echo_body_with_response_stream)
    client = httpj.Client(transport=transport)
    response = client.post("http://www.example.org/", content=b"example")
    assert response.status_code == 200
    assert response.text == "example"
...
def test_wsgi_exc():
    transport = httpj.WSGITransport(app=raise_exc)
    client = httpj.Client(transport=transport)
...
def test_wsgi_http_error():
    transport = httpj.WSGITransport(app=partial(raise_exc, exc=RuntimeError))
    client = httpj.Client(transport=transport)
...
def test_wsgi_generator():
    ...
    transport = httpj.WSGITransport(app=application_factory(output))
    client = httpj.Client(transport=transport)
    response = client.get("http://www.example.org/")
    assert response.status_code == 200
...
def test_logging():
    buffer = StringIO()
    transport = httpj.WSGITransport(app=log_to_wsgi_log_buffer, wsgi_errors=buffer)
    client = httpj.Client(transport=transport)
    response = client.post("http://www.example.org/", content=b"example")
