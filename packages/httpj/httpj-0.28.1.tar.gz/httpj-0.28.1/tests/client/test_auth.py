...
class App:
    ...
    def __call__(self, request: httpj.Request) -> httpj.Response:
        headers = {"www-authenticate": self.auth_header} if self.auth_header else {}
        data = {"auth": request.headers.get("Authorization")}
        return httpj.Response(self.status_code, headers=headers, json=data)
...
class DigestApp:
    ...
    def challenge_send(self, request: httpj.Request) -> httpj.Response:
        ...
        headers = {
            "www-authenticate": f'Digest realm="httpj@example.org", {challenge_str}',
        }
        return httpj.Response(401, headers=headers)


class RepeatAuth(httpj.Auth):
    """
    ...
    def auth_flow(
        self, request: httpj.Request
    ) -> typing.Generator[httpj.Request, httpj.Response, None]:
        nonces = []
...
class ResponseBodyAuth(httpj.Auth):
    """
...
class SyncOrAsyncAuth(httpj.Auth):
    """
    ...
    async def async_auth_flow(
        self, request: httpj.Request
    ) -> typing.AsyncGenerator[httpj.Request, httpj.Response]:
...
async def test_basic_auth() -> None:
    ...
    async with httpj.AsyncClient(transport=httpj.MockTransport(app)) as client:
...
async def test_basic_auth_with_stream() -> None:
    """
    See: https://github.com/encode/httpj/pull/1312
    """
    url = "https://example.org/"
    auth = ("user", "password123")
    app = App()

    async with httpj.AsyncClient(
        transport=httpj.MockTransport(app), auth=auth
    ) as client:
...
async def test_custom_auth() -> None:
    ...
    def auth(request: httpj.Request) -> httpj.Request:
...
def test_netrc_auth_credentials_exist() -> None:
    ...
    app = App()
    auth = httpj.NetRCAuth(netrc_file)

    with httpj.Client(transport=httpj.MockTransport(app), auth=auth) as client:
...
def test_auth_hidden_url() -> None:
    ...
    assert url == httpj.URL(url)
    assert expected == repr(httpj.URL(url))
...
async def test_auth_property() -> None:
    ...
    async with httpj.AsyncClient(transport=httpj.MockTransport(app)) as client:
        ...
        assert isinstance(client.auth, httpj.BasicAuth)
...
async def test_auth_invalid_type() -> None:
    ...
    with pytest.raises(TypeError):
        client = httpj.AsyncClient(
            transport=httpj.MockTransport(app),
...
async def test_digest_auth_returns_no_auth_if_no_digest_header_in_response() -> None:
    url = "https://example.org/"
    auth = httpj.DigestAuth(username="user", password="password123")
    app = App()
...
def test_digest_auth_returns_no_auth_if_alternate_auth_scheme() -> None:
    ...
    client = httpj.Client(transport=httpj.MockTransport(app))
...
async def test_digest_auth(
    ...
    assert digest_data["realm"] == '"httpj@example.org"'
...
async def test_digest_auth_qop_must_be_auth_or_auth_int() -> None:
    ...
    async with httpj.AsyncClient(transport=httpj.MockTransport(app)) as client:
        with pytest.raises(httpj.ProtocolError):
            await client.get(url, auth=auth)
...
@pytest.mark.parametrize(
    "auth_header",
    [
        'Digest realm="httpj@example.org", qop="auth"',  # missing fields
        'Digest realm="httpj@example.org", qop="auth,au',  # malformed fields list
    ],
)
@pytest.mark.anyio
...
def test_sync_digest_auth_raises_protocol_error_on_malformed_header(
    ...
    with httpj.Client(transport=httpj.MockTransport(app)) as client:
        with pytest.raises(httpj.ProtocolError):
...
class ConsumeBodyTransport(httpj.MockTransport):
    async def handle_async_request(self, request: httpj.Request) -> httpj.Response:
...
async def test_digest_auth_unavailable_streaming_body():
    ...
    async with httpj.AsyncClient(transport=ConsumeBodyTransport(app)) as client:
