...
def test_get(server):
    url = server.url
    with httpj.Client(http2=True) as http:
...
def test_get_invalid_url(server, url):
    with httpj.Client() as client:
        with pytest.raises((httpj.UnsupportedProtocol, httpj.LocalProtocolError)):
            client.get(url)
    ...
    with httpj.Client() as client:
        for status_code in (200, 400, 404, 500, 505):
            ...
            if 400 <= status_code < 600:
                with pytest.raises(httpj.HTTPStatusError) as exc_info:
...
def test_base_url(server):
    base_url = server.url
    with httpj.Client(base_url=base_url) as client:
        response = client.get("/")
    assert response.status_code == 200
...
def test_merge_absolute_url():
    client = httpj.Client(base_url="https://www.example.com/")
...
def test_merge_relative_url_with_path():
    client = httpj.Client(base_url="https://www.example.com/some/path")
    request = client.build_request("GET", "/testing/123")
...
def test_merge_relative_url_with_encoded_slashes():
    ...
    client = httpj.Client(base_url="https://www.example.com/base%2Fpath")
    request = client.build_request("GET", "/testing")
    assert request.url == "https://www.example.com/base%2Fpath/testing"


def test_context_managed_transport():
    class Transport(httpj.BaseTransport):
        def __init__(self) -> None:
            self.events: list[str] = []

        def close(self):
            # The base implementation of httpj.BaseTransport just
    ...
    transport = Transport()
    with httpj.Client(transport=transport):
        pass
...
def test_context_managed_transport_and_mount():
    ...
    with httpj.Client(transport=transport, mounts={"http://www.example.org": mounted}):
        pass
...
def test_client_closed_state_using_implicit_open():
    client = httpj.Client(transport=httpj.MockTransport(hello_world))
...
def test_client_closed_state_using_with_block():
    with httpj.Client(transport=httpj.MockTransport(hello_world)) as client:
        assert not client.is_closed
        client.get("http://example.com")
...
def echo_raw_headers(request: httpj.Request) -> httpj.Response:
    data = [
...
def test_raw_client_header():
    ...
    client = httpj.Client(
        transport=httpj.MockTransport(echo_raw_headers), headers=headers
    )
...
def unmounted(request: httpj.Request) -> httpj.Response:
    data = {"app": "unmounted"}
    return httpj.Response(200, json=data)


def mounted(request: httpj.Request) -> httpj.Response:
    data = {"app": "mounted"}
    return httpj.Response(200, json=data)


def test_mounted_transport():
    transport = httpj.MockTransport(unmounted)
    mounts = {"custom://": httpj.MockTransport(mounted)}

    client = httpj.Client(transport=transport, mounts=mounts)
...
def test_all_mounted_transport():
    mounts = {"all://": httpj.MockTransport(mounted)}
...
def test_client_decode_text_using_autodetect():
    ...
    transport = httpj.MockTransport(cp1252_but_no_content_type)
    with httpj.Client(transport=transport, default_encoding=autodetect) as client:
