...
def redirects(request: httpj.Request) -> httpj.Response:
    if request.url.scheme not in ("http", "https"):
        raise httpj.UnsupportedProtocol(f"Scheme {request.url.scheme!r} not supported.")

    if request.url.path == "/redirect_301":
        status_code = httpj.codes.MOVED_PERMANENTLY
        content = b"<a href='https://example.org/'>here</a>"
        headers = {"location": "https://example.org/"}
        return httpj.Response(status_code, headers=headers, content=content)

    elif request.url.path == "/redirect_302":
        status_code = httpj.codes.FOUND
        headers = {"location": "https://example.org/"}
        return httpj.Response(status_code, headers=headers)

    elif request.url.path == "/redirect_303":
        status_code = httpj.codes.SEE_OTHER
        headers = {"location": "https://example.org/"}
        return httpj.Response(status_code, headers=headers)
        ...
        return httpj.Response(status_code, headers=raw_headers)
        ...
        params = httpj.QueryParams(request.url.query)
        count = int(params.get("count", "0"))
        redirect_count = count - 1
        status_code = httpj.codes.SEE_OTHER if count else httpj.codes.OK
        if count:
    ...
    if request.url.path == "/redirect_loop":
        ...
        status_code = httpj.codes.OK
        data = {
            "body": request.content.decode("ascii"),
            "headers": dict(request.headers),
        }
        return httpj.Response(status_code, json=data)

    elif request.url.path == "/redirect_body":
        status_code = httpj.codes.PERMANENT_REDIRECT
    ...
    if request.method == "HEAD":
        return httpj.Response(200)

    return httpj.Response(200, html="<html><body>Hello, world!</body></html>")


def test_redirect_301():
    client = httpj.Client(transport=httpj.MockTransport(redirects))
    response = client.post("https://example.org/redirect_301", follow_redirects=True)
    assert response.status_code == httpj.codes.OK
...
def test_next_request():
    ...
    assert response.status_code == httpj.codes.SEE_OTHER
...
async def test_async_next_request():
    async with httpj.AsyncClient(transport=httpj.MockTransport(redirects)) as client:
        request = client.build_request("POST", "https://example.org/redirect_303")
...
def test_malformed_redirect():
    # https://github.com/encode/httpj/issues/771
    client = httpj.Client(transport=httpj.MockTransport(redirects))
    response = client.get(
...
def test_invalid_redirect():
    client = httpj.Client(transport=httpj.MockTransport(redirects))
    with pytest.raises(httpj.RemoteProtocolError):
...
async def test_async_too_many_redirects():
    async with httpj.AsyncClient(transport=httpj.MockTransport(redirects)) as client:
        with pytest.raises(httpj.TooManyRedirects):
            await client.get(
...
def test_can_stream_if_no_redirect():
    ...
    with client.stream("GET", url, follow_redirects=False) as response:
        pass
    assert response.status_code == httpj.codes.MOVED_PERMANENTLY
    assert response.headers["location"] == "https://example.org/"


class ConsumeBodyTransport(httpj.MockTransport):
    def handle_request(self, request: httpj.Request) -> httpj.Response:
...
def test_cannot_redirect_streaming_body():
    client = httpj.Client(transport=ConsumeBodyTransport(redirects))
    url = "https://example.org/redirect_body"
...
def cookie_sessions(request: httpj.Request) -> httpj.Response:
    if request.url.path == "/":
        ...
        return httpj.Response(200, content=content)
...
def test_redirect_cookie_behavior():
    client = httpj.Client(
        transport=httpj.MockTransport(cookie_sessions), follow_redirects=True
    )
...
def test_redirect_custom_scheme():
    client = httpj.Client(transport=httpj.MockTransport(redirects))
    with pytest.raises(httpj.UnsupportedProtocol) as e:
