...
async def test_context_managed_transport():
    class Transport(httpj.AsyncBaseTransport):
        def __init__(self) -> None:
            self.events: list[str] = []

        async def aclose(self):
            # The base implementation of httpj.AsyncBaseTransport just
    ...
    transport = Transport()
    async with httpj.AsyncClient(transport=transport):
        pass
...
async def test_client_closed_state_using_implicit_open():
    client = httpj.AsyncClient(transport=httpj.MockTransport(hello_world))
...
async def test_client_closed_state_using_with_block():
    async with httpj.AsyncClient(transport=httpj.MockTransport(hello_world)) as client:
        assert not client.is_closed
        await client.get("http://example.com")
...
async def test_mounted_transport():
    ...
    async with httpj.AsyncClient(transport=transport, mounts=mounts) as client:
...
async def test_async_mock_transport():
    async def hello_world(request: httpj.Request) -> httpj.Response:
        return httpj.Response(200, text="Hello, world!")

    transport = httpj.MockTransport(hello_world)
...
async def test_cancellation_during_stream():
    ...
    * `SystemExit`
    ...
    See https://github.com/encode/httpj/issues/2139
    """
    stream_was_closed = False

    def response_with_cancel_during_stream(request):
        class CancelledStream(httpj.AsyncByteStream):
        ...
        return httpj.Response(
            200, headers={"Content-Length": "12"}, stream=CancelledStream()
        )

    transport = httpj.MockTransport(response_with_cancel_during_stream)
