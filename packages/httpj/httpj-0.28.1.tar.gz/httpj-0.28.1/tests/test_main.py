...
def test_help():
    runner = CliRunner()
    result = runner.invoke(httpj.main, ["--help"])
...
def test_get(server):
    url = str(server.url)
    runner = CliRunner()
    result = runner.invoke(httpj.main, [url])
    assert result.exit_code == 0
...
def test_binary(server):
    ...
    runner = CliRunner()
    content = "Hello, world!"
    result = runner.invoke(httpj.main, [url, "-c", content])
    assert result.exit_code == 0
    assert remove_date_header(splitlines(result.output)) == [
        "HTTP/1.1 200 OK",
        "server: uvicorn",
...
def test_follow_redirects(server):
    url = str(server.url.copy_with(path="/redirect_301"))
    runner = CliRunner()
    result = runner.invoke(httpj.main, [url, "--follow-redirects"])
    assert result.exit_code == 0
    assert remove_date_header(splitlines(result.output)) == [
        "HTTP/1.1 301 Moved Permanently",
        "server: uvicorn",
...
def test_post(server):
    ...
    runner = CliRunner()
    result = runner.invoke(httpj.main, [url, "-m", "POST", "-j", '{"hello": "world"}'])
    assert result.exit_code == 0
    assert remove_date_header(splitlines(result.output)) == [
        "HTTP/1.1 200 OK",
        "server: uvicorn",
...
def test_verbose(server):
    url = str(server.url)
    runner = CliRunner()
    result = runner.invoke(httpj.main, [url, "-v"])
    assert result.exit_code == 0
    assert remove_date_header(splitlines(result.output)) == [
        ...
        f"User-Agent: python-httpj/{httpj.__version__}",
        "",
        "HTTP/1.1 200 OK",
        "server: uvicorn",
        "content-type: text/plain",
...
def test_auth(server):
    url = str(server.url)
    runner = CliRunner()
    result = runner.invoke(httpj.main, [url, "-v", "--auth", "username", "password"])
    print(result.output)
    assert result.exit_code == 0
    assert remove_date_header(splitlines(result.output)) == [
        "* Connecting to '127.0.0.1'",
...
def test_download(server):
    ...
    with runner.isolated_filesystem():
        runner.invoke(httpj.main, [url, "--download", "index.txt"])
...
def test_errors():
    runner = CliRunner()
    result = runner.invoke(httpj.main, ["invalid://example.org"])
    assert result.exit_code == 1
