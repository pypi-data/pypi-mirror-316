...
import { getRecipientOptions } from '@/helpers/companies';
...
const recipientOptions = computed(() =>
  getRecipientOptions(clientQuoteAddresses?.value?.recipients)
);

const fromOptions = computed(() => getRecipientOptions(clientQuoteAddresses?.value?.senders));
...
watch(clientQuoteAddresses, () => {
  if (clientQuoteAddresses.value?.recipients) {
    const filteredRecipients = clientQuoteAddresses.value.recipients.filter(
      (recipient) => recipient.is_pre_selected
    );
    recipients.value = getRecipientOptions(filteredRecipients) as IClientQuoteRecipient[];
  }
});
</script>
