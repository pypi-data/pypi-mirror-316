def cscan(requests, head_start, total_tracks):
    sequence = []
    total_movement = 0
    ...
    total_movement += (total_tracks - 1)
    current_head = 0
    print(0, end=" ")
    
    for request in [r for r in remaining if r < head_start]:
        movement = abs(request - current_head)
        total_movement += movement
        current_head = request
        sequence.append(request)
...
cscan(requests.copy(), head_start, total_tracks)