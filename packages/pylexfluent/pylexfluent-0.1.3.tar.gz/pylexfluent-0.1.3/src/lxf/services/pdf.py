...
@measure_time_async
async def get_text_and_tables_from_pdf(filename:str,extract_tables:bool=True,max_pages:int=-1)->tuple[str,List[lxfTable]|None] :
    """
    """
    queue:Queue=Queue()
    ...
    p.start()
    p.join()
    result:tuple[str,List[lxfTable]|None] = queue.get(True)
    if result !=None :
        return result
    else :
