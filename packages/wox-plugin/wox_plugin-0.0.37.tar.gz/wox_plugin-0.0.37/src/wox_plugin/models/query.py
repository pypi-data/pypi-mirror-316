...
class QueryEnv:
    ...
    def from_json(cls, json_str: str) -> "QueryEnv":
        ...
        data = orjson.loads(json_str)
        return cls(
            active_window_title=data["ActiveWindowTitle"],
            active_window_pid=data["ActiveWindowPid"],
            active_browser_url=data["ActiveBrowserUrl"],
        )
...
class Query:
        ...
        return orjson.dumps(
            {
                ...
                "Selection": orjson.loads(self.selection.to_json()),
                "Env": orjson.loads(self.env.to_json()),
    ...
    def from_json(cls, json_str: str) -> "Query":
        ...
        return cls(
            ...
            selection=Selection.from_json(orjson.dumps(data["Selection"]).decode("utf-8")),
            env=QueryEnv.from_json(orjson.dumps(data["Env"]).decode("utf-8")),
...
class ChangeQueryParam:
    ...
    query_selection: Selection = field(default=Selection())
    ...
    def to_json(self) -> str:
        ...
        if self.query_selection:
            data["QuerySelection"] = orjson.loads(self.query_selection.to_json())
    ...
    def from_json(cls, json_str: str) -> "ChangeQueryParam":
        ...
        if "QuerySelection" in data:
            query_selection = Selection.from_json(orjson.dumps(data["QuerySelection"]).decode("utf-8"))
        return cls(
            query_type=QueryType(data["QueryType"]),
