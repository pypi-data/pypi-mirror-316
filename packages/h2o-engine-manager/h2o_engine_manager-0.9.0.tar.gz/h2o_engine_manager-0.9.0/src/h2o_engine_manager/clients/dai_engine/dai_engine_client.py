...
class DAIEngineClient:
    ...
    def __init__(
        ...
        self.adjusted_profile_client = AdjustedDAIProfileClient(
            connection_config=connection_config,
            verify_ssl=verify_ssl,
            ssl_ca_cert=ssl_ca_cert,
        )
    ...
    def create_engine(
        ...
        Args:
            ...
            storage_bytes (str, optional): Quantity of bytes. Example `250G`, `2T`. Same syntax applies as `memory_bytes` parameter.
                If not specified, a default value will be set by server.
            max_idle_duration (str, optional): Maximum time an engine can be idle. When exceeded, the engine will pause. Must be specified as a number with `s` suffix. Example `3600s`.
                If not specified, a default value will be set by server.
            max_running_duration (str, optional):  Maximum time na engine can be running. When exceeded, the engine will pause. Must be specified as a number with `s` suffix. Example `36000s`.
                If not specified, a default value will be set by server.
            profile_id (str, optional): Deprecated. Use "profile" instead.
                The ID of the profile to use for the engine. Profile will be adjusted to fit the workspace contraints.
                If specified, any unspecified parameters (cpu, gpu, memory_bytes, storage_bytes) will be set to the profile's values.
            workspace_id (str, optional): The workspace ID where the engine is to be created. Defaults to DriverlessClients default value (`default` if not set).
            display_name (str): Human-readable name of the DAIEngine. Must contain at most 63 characters. Does not have to be unique.
            config (Dict[str, str], optional): Additional Driverless AI configuration. Defaults to {}.
            annotations (Dict[str, str], optional): Additional arbitrary metadata associated with the DAIEngine. Defaults to {}.
            ...
            profile(str, optional): resource name of the DAIEngineProfile that is used by this DAIEngine.
        ...
        if profile_id is not None and profile != "":
            raise Exception("cannot set both profile_id and profile")

        # Apply adjusted profile values if profile_id is specified.
        if profile_id is not None:
            adjusted_profile = self.adjusted_profile_client.get_adjusted_profile(
                workspace_id=workspace_id, profile_id=profile_id
            )
            if cpu is None:
                cpu = adjusted_profile.cpu
            if gpu is None:
                gpu = adjusted_profile.gpu
            if memory_bytes is None:
                memory_bytes = quantity_convertor.quantity_to_number_str(adjusted_profile.memory_bytes)
            if storage_bytes is None:
                storage_bytes = quantity_convertor.quantity_to_number_str(adjusted_profile.storage_bytes)
    ...
    def list_versions(
        ...
        Args:
            page_size: Maximum number of DAIVersions to return in a response.
                If unspecified (or set to 0), at most 50 DAIVersions will be returned.
                The maximum value is 1000; values above 1000 will be coerced to 1000.
            page_token: Leave unset to receive the initial page.
                To list any subsequent pages use the value of 'next_page_token' returned from the
                 DAIVersionsPage.
            order_by: Expression used to specify the sorting order. For example: "version asc".
                When unset, DAIVersions are ordered by their version name in descending order. This is equivalent to
                "version desc".
                The only supported field is 'version'.
            filter: Expression used to filter DAIEngines. For example: "version < latest AND version >= 1.10.1".
                The only supported field is 'version'.

        Returns:
            Page of DAIVersions.
        """
        return self.dai_version_client.list_versions(
    ...
    def list_all_versions(
        ...
        Args:
            order_by (str, optional): Identical to the list_versions function order_by parameter.
            filter (str, optional): Identical to the list_version function filter parameter.

        Returns:
            List[DAIVersion]: A list of Driverless AI versions.
        """

        return self.dai_version_client.list_all_versions(
    ...
    def list_adjusted_profiles(
        ...
        return self.adjusted_profile_client.list_adjusted_profiles(
            workspace_id=workspace_id, page_size=page_size, page_token=page_token
        )
    ...
    def list_all_adjusted_profiles(self, workspace_id: str, ) -> List[AdjustedDAIProfile]:
        ...
        return self.adjusted_profile_client.list_all_adjusted_profiles(
            workspace_id=workspace_id,
        )
    ...
    def generate_engine_id(
        ...
        for n in range(50):
            ...
            try:
                ...
                r = self.get_engine(engine_id=engine_id_candidate, workspace_id=workspace_id)
                print(r)
