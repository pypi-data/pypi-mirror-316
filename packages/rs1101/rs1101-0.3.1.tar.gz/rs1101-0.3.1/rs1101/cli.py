...
def cli(ctx, c, cs, l, ss):
    ...
    if ctx.invoked_subcommand is None:
        if l is None:
            l = default_length
        s = rs.random_string(l, candidate)
        print(s)
        show_strength(ctx.obj)
    else:
        if l is not None or ss:
            raise click.BadOptionUsage(
                message="You should not use the -l and/or -ss option here when using the subcommand."
            )
