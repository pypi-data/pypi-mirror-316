...
class ModelReview(object):
    ...
    def score_actuals(self, actuals_path = None, actual_records=None, actuals_ds=None,
        ...
        if 'actual' in ds_actuals.columns:
        ...
        elif self.target_feature in ds_actuals.columns:# and ds_actuals.df[self.target_feature].any():
        ...
        if primary_prediction_group_id:
            ...
            for (_, df) in DataSourceAPIPandas.load_from_files(files, features=['prediction_id']):
                primary_ds = df
        ...
        for (file, df_prediction_results) in DataSourceAPIPandas.load_from_files(prediction_files):
    ...
    def build_review_data(self, data_path=None):
        if not data_path:
            data_path = self.options['data_path']

        review_data_path = os.path.splitext(data_path)[0] + "_review_%s.feather.zstd"%(get_uid())

        ds_train = DataSourceAPIPandas.create_dataframe(data_path)

        all_files = FSClient().listFolder(os.path.join(self.model_path, "predictions/*_actuals.feather.zstd"),
            wild=True, removeFolderName=False, meta_info=True)
        all_files.sort(
            key=lambda f: f['last_modified'],
            reverse=True
        )

        for (file, ds_actuals) in DataSourceAPIPandas.load_from_files(all_files):
        ...
        ds_train.saveToFeatherFile(review_data_path)
    ...
    def count_actuals_by_prediction_id(self):
        ...
        for (file, df) in DataSourceAPIPandas.load_from_files(all_files, features):
    ...
    def score_model_performance_daily(self, date_from, date_to):
        ...
        for (curr_date, files) in ModelReview._prediction_files_by_day(
            ...
            df_actuals = DataSourceAPIPandas({})

            for (file, df) in DataSourceAPIPandas.load_from_files(files, features):
    ...
    def set_support_review_model_flag(self, flag_value):
        ...
        FSClient().writeJSONFile(path, self.options, atomic=True)

    def remove_model(self):
        FSClient().removeFolder(self.model_path)
        return True

    def clear_model_results_and_actuals(self):
        FSClient().removeFolder(os.path.join(self.model_path, "predictions"))
        return True
    ...
    def _prediction_files_by_day(model_path, date_from, date_to, path_suffix):
        ...
        while curr_date <= date_to:
            ...
            files = FSClient().listFolder(path, wild=True, removeFolderName=False, meta_info=False)
    ...
    def _get_prediction_files(model_path, prediction_group_id=None):
        ...
        files = FSClient().listFolder(predictions_path, wild=True, removeFolderName=False, meta_info=True)
