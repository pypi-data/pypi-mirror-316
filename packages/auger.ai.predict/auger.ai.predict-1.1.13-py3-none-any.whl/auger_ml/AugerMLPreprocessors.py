...
class AugerMLPreprocessors(object):
    ...
    def preprocess_target(self, ds):
        ...
        if self.options.get('multilabel', False):
            if isinstance( self.options.get('targetFeature'), list):
                ds.df['multilabel_target'] = list(ds.df[self.options.get('targetFeature')].apply(lambda x: x.astype(int)).values)
                target = ds.df['multilabel_target']
                self.options['originalTargetFeature'] = self.options['targetFeature']
                self.options['targetFeature'] = 'multilabel_target'
            else:    
                target = ds.df[self.options.get('targetFeature')].apply(lambda x: json.loads(x))
                mlb = MultiLabelBinarizer()
                res = mlb.fit_transform(target)

                self.options['multilabel_categories'] = mlb.classes_.tolist()                
                ds.df[self.options.get('targetFeature')] = list(res)

        elif self.options.get('targetFeature') in categoricals:
            ds.convertToCategorical(self.options.get('targetFeature'), is_target=True)
    ...
    def get_preprocessor_params(self, pp_name):
        ...
        if pp_name == 'text.TextPreprocessor':
            params['text_cols'] = self.options.get('textFeatures', [])
    ...
    def get_columns_to_load(self):
        ...
        if self.options.get('targetFeature') is not None:
            if isinstance(self.options.get('targetFeature'), list):
                selected_cols.extend(self.options.get('targetFeature'))
            else:    
