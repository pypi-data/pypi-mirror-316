...
def train(
    ...
    '''
    Train scEve model using parameters tuned by scparadise.sceve.tune function
    ...
    protein_modality_name : str, (default: 'adt')
        Name of protein (ADT) modality in MuData object.
    ...
    data_adt = pd.DataFrame(data=mdata[protein_modality_name].X.toarray(), columns=mdata[protein_modality_name].var_names)
...
def hyperparameter_tuning(
    ...
    """
    Hyperparameter tuning using automatic optimization framework optuna.
    ...
    def objective(
        ...
        if tune_params == 'auto':
            ...
            if max_epochs != None:
                ...
                params['max_epochs'] = trial.suggest_int("max_epochs", 5, 200, step = 5)
...
def train_tuned(
    ...
    path_tuned : str, path object
        Path to folder with tuned parameters by scparadise.scadam.tune function.
    detailed_annotation : str, (default: None)
        The most detailed level of cell annotation. Key in mdata.obs dataframe.
        If given may increase model evaluation score.
    ...
    cash.to_csv(os.path.join(path, model_name, 'history.csv').replace("\\","/"))
    print()
    print('Successfully saved training history')
...
# Function for prediction proteins using trained model
def predict(
    adata, 
    path_model = '',
    rna_modality_name = 'rna',
    ...
    '''
    Predict cell types and cell type probilities using pretrained scEve model.
    ...
    return_mdata : bool, (default: True)
        If set 'True' return MuData object. If set 'False' return AnnData object.
    ...
    print('Successfully loaded list of proteins used for training model')
    print()
    ...
    # Create DataFrame with predicted proteins
    predictions = pd.DataFrame(predictions, columns = proteins)
    
    # Create anndata object using predicted proteins
    adata_prot = anndata.AnnData(X = sparse.csc_matrix(predictions.values), 
                                 obs = adata.obs,
                                 var = pd.DataFrame(index = predictions.columns)
                                )
    
    # Create mdata object using predicted proteins and rna from adata
    mdata = mu.MuData({rna_modality_name: adata, protein_modality_name: adata_prot})
...
def available_models(
    ...
    '''
    Display available for download and usage models.
...
def download_model(
    ...
    '''
    Download pretrained tuned model for highly accurate cell surface protein abundance prediction.
    ...
    save_path : str, path object
        Path to save tuned pretrained scEve model.
