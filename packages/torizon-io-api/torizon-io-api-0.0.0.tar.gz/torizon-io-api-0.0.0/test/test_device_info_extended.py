# coding: utf-8

"""
    Torizon OTA

     This API is rate limited and will return the following headers for each API call.    - X-RateLimit-Limit - The total number of requests allowed within a time period   - X-RateLimit-Remaining - The total number of requests still allowed until the end of the rate limiting period   - X-RateLimit-Reset - The number of seconds until the limit is fully reset  In addition, if an API client is rate limited, it will receive a HTTP 420 response with the following header:     - Retry-After - The number of seconds to wait until this request is allowed  

    The version of the OpenAPI document: 2.0-Beta
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from torizon_io_api.models.device_info_extended import DeviceInfoExtended

class TestDeviceInfoExtended(unittest.TestCase):
    """DeviceInfoExtended unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> DeviceInfoExtended:
        """Test DeviceInfoExtended
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `DeviceInfoExtended`
        """
        model = DeviceInfoExtended()
        if include_optional:
            return DeviceInfoExtended(
                device_uuid = '',
                device_name = '',
                device_id = '',
                last_seen = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                activated_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                device_status = 'NotSeen',
                notes = '',
                hibernated = True,
                last_updated = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                device_fleets = [
                    torizon_io_api.models.fleet.Fleet(
                        id = '', 
                        name = '', 
                        created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        fleet_type = 'static', 
                        expression = '', )
                    ],
                device_packages = [
                    torizon_io_api.models.device_package.DevicePackage(
                        component = '', 
                        name = '', 
                        version = '', 
                        checksum = '', )
                    ],
                device_tags = [
                    torizon_io_api.models.tuple2_device_tag_id_device_tag_value.Tuple2_DeviceTagId_DeviceTagValue(
                        _1 = '', 
                        _2 = '', )
                    ],
                network_info = torizon_io_api.models.network_info.NetworkInfo(
                    device_uuid = '', 
                    local_ip_v4 = '', 
                    hostname = '', 
                    mac_address = '', )
            )
        else:
            return DeviceInfoExtended(
                device_uuid = '',
                device_name = '',
                device_id = '',
                created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                device_status = 'NotSeen',
                hibernated = True,
                network_info = torizon_io_api.models.network_info.NetworkInfo(
                    device_uuid = '', 
                    local_ip_v4 = '', 
                    hostname = '', 
                    mac_address = '', ),
        )
        """

    def testDeviceInfoExtended(self):
        """Test DeviceInfoExtended"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
