...
class CausalVisualizer:
    ...
    def NN_multifit(self, n, trials=50, epochs=5000, learning_rate=0.1, min_lr=0.01, switch_threshold=0.01, patience=1000, min_delta=1e-7, decay_rate=[0.8,50], penalty_strength=1, pop_ratio=0.75):
        """
        ...
        返回:
            mse_list: list, trials次训练的MSE值
            predictions_dict: dict, 键为变量名，值为trials次训练中每次对该变量100个点的预测值
            thresholds_dict: dict, 键为变量名，值为trials次训练中每次的折点位置列表
            residuals_list: list, 每个trial的残差值列表
        """
        import copy
    ...
    def plot_average_effects(self, results, x_name,k=5000,caculate_significance = False, train_control_model_setting=[5,1000,0.1,[0.9,50],0.5],bboxtoanchor2=(1.04,1),bboxtoanchor3=(1.04,1)):
        """
        ...
        if caculate_significance == True:
            ...
            for i in range(len(breakpoints)-1):
                ...
                best_controlmodels,mse_list_control = self.train_control_model(nn,
                ...
                if len(x_data_in_range) > 0:
                    ...
                    weights_main = [1/mse**2 for mse in mse_list]
                    weights_main = np.array(weights_main) / sum(weights_main)  # 归一化权重
                    experiment_sigma2 = np.sum([mse * weight for mse, weight in zip(mse_list, weights_main)])
                    ''' with torch.no_grad():
    ...
    def analyze_moderation_effect(self, results, x_name):
        ...
        for l in control_x_indices:
            ...
            for center in bin_centers:
                ...
                modified_x[:, x_index] = center
                
                # 计算该bin center下的条件均值
                conditional_means,interval_points_num = self.calculate_conditional_means(mse_list,model_list, modified_x, x_index, l)
                slope_list.append(conditional_means)
            ...
            for K in slope_list_T:
                # 使用三次样条进行平滑拟合
                spline = UnivariateSpline(list(range(len(K))), K, s=9)  # s参数控制平滑程度
                K_smooth = spline(list(range(len(K))))  # 计算平滑后的K值
    ...
    def calculate_conditional_means(self,mse_list, model_list, modified_x,main_x_idx,control_x_idx):
        ...
        # 计算区间边界
        bins = np.linspace(control_var.min(), control_var.max(), 11)  # 10个区间需要11个边界点
        ...
        for i in range(len(bins)-1):
            ...
            interval_points_num.append(points_in_interval)
        ...
        for model in model_list:
            ...
            for i in range(len(bins)-1):
                ...
                if len(interval_points) > 0:  # 如果区间内有点
                    ...
                    interval_points[:, control_x_idx] = bin_centers[i]
        ...
        conditional_means = np.average(MEAN_PREDICTIONS, axis=0, weights=weights)

        
        return np.array(conditional_means),interval_points_num
    ...
    def plot_moderation_effect(self, results, main_x_name, control_x_name,caculate_moderation_LRT=False,train_control_model_setting = [10,5000,0.1, 0.01, 0.01, 1000, 1e-7, [0.8,50],0.5]):
        ...
        conditional_mp,bin_centers = self.calculate_conditional_predictions(mse_list, model_list, x,main_x_index,control_x_index)
        ...
        alphas = [1.0, 0.8, 0.6, 0.8, 1]
        for i in range(5):
            plt.plot(main_x_points, result_dict[bin_centers[i]]['conditional_means'], 
                    label=f'{control_x_name}={bin_centers[i]:.2f}', 
                    color=colors[i],
                    alpha=alphas[i])
    ...
    def calculate_conditional_predictions(self,mse_list, model_list, x,main_x_idx,control_x_idx):
        ...
        # 计算区间边界
        bins = np.linspace(control_var.min(), control_var.max(), 6)  # 10个区间需要11个边界点
        bin_centers = [(bins[i] + bins[i+1]) / 2 for i in range(len(bins)-1)]
            ...
            for i in range(len(bins)-1):
                ...
                if torch.sum(mask) > 0:
                    ...
                    for i in range(len(main_var_intervals)):
                        # 计算当前区间的范围
                        interval_start = (i-3)*dx + main_var_min
                        interval_end = (i+3)*dx + main_var_min
        ...
        return conditional_mp,bin_centers
