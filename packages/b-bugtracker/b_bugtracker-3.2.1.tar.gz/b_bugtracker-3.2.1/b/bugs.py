...
# ----------------------------------------------------------------------------------------------------------------------
    def initialize(self, force: bool):
        ...
        # Generate a readme into the .bugs directory so that version control picks it up and so that the user has some
        # context for how to properly work with bug files.
        source = os.path.join(os.path.dirname(__file__), 'ReadmeTemplate.md')
        destination = os.path.join(self.bugsdir, 'README.md')
