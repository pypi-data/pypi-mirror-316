...
from shasta_python._utils import convert_function_to_tool
...
try:
  __version__ = metadata.version('shasta')
except metadata.PackageNotFoundError:
  __version__ = '0.0.0'

import httpx

from shasta_python._types import (
  ChatRequest,
  ChatResponse,
  CreateRequest,
  CopyRequest,
...
class BaseClient:
  def __init__(
    ...
    **kwargs,
    ...
    self._client = client(
      base_url=_parse_host(host or os.getenv('SHASTA_HOST')),
      ...
      headers={
        ...
        for k, v in {
          **(headers or {}),
          ...
          'User-Agent': f'shasta-python/{__version__} ({platform.machine()} {platform.system().lower()}) Python/{platform.python_version()}',
        }.items()
      },
      **kwargs,
    )
...
class Client(BaseClient):
  ...
  def chat(
    ...
    *,
    ...
    Args:
      tools:
        A JSON schema as a dict, an Shasta Tool or a Python Function.
        Python functions need to follow Google style docstrings to be converted to an Shasta Tool.
...
def _parse_host(host: Optional[str]) -> str:
  ...
  'http://[0001:002:003:0004::1]:5668/path'
