...
class SpecializedFactory(ABC, Generic[T]):
    ...
    def register_type(self, name: str, module_path: str) -> None:
        ...
        self.type_mapping[name.lower()] = module_path
    ...
    def unregister_type(self, name: str) -> None:
        ...
        """
        self.type_mapping.pop(name.lower(), None)
    ...
    def build(self, config: Dict[str, Any]) -> T:
        ...
        try:
            ...
            if normalized_config.class_name.lower() in self.special_handlers:
                return self._validate_instance(
                    self.special_handlers[normalized_config.class_name.lower()](
    ...
    def _create_builtin_implementation(self, config: FactoryConfig) -> T:
        ...
        class_name = config.class_name.lower()
        if class_name not in self.type_mapping:
        ...
        instance = self.class_instance_factory.create({
            "module": self.type_mapping[class_name],
            "class": config.class_name,
            "args": config.args,
            "kwargs": config.kwargs
        })
