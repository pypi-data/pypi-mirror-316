...
from IPython.display import display, clear_output, Markdown  # type: ignore
import types
import inspect
import requests
...
class Submission:
    def __init__(self, exam_id: str):
        self.exam = requests.get(
            f"https://cspyclient.up.railway.app/exams/{exam_id}").json()
        self.questions = self.exam["data"]
        self.answers = [
            {"question": q["question"], "answer": ""} for q in self.questions
        ]
    ...
    def register_student(self):
        ...
        email_field = widgets.Text(
            value="", placeholder="Email..", description="Your email", disabled=False
        )
        ...
        def submit_all(btn):
            with output:
                ...
                response = requests.post(
                    "https://cspyclient.up.railway.app/submissions", json=payload
                )
