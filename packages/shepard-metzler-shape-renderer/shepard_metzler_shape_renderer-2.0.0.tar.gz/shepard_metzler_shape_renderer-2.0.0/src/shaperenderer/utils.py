...
class InPlaneRandomRotation:
    """
    Rotates the perspective image of 3D shape in the picture plane
    by a random angle in a counterclockwise direction.
    """
    def __init__(
        self,
        degrees: tuple[float, float] = (0., 360.),
    ) -> None:
        """
        Parameters
        ----------
        degrees: tuple[float, float], default=(0., 360.)
            The range of degrees from which a rotation angle is randomly sampled
        """
        self.degrees = degrees


    def __call__(self, vertices: list[np.array]) -> list[np.array]:
        """
        Parameters
        ----------
        vertices: list[np.array]
            The list of 2D coordinates in the screen space grouped by 3D shape faces

        Returns
        -------
        A new set of 2D coordinates in the same format after in-plane rotation transformation
        """
        # randomly sample the rotation angle from the range defined by 'self.degress'
        angle_in_radians = math.radians(np.random.uniform(low=self.degrees[0], high=self.degrees[1], size=1))
        ...
        return [R @ face.T.reshape(4, 2, 1,).T.reshape(2, 4) for face in vertices]
