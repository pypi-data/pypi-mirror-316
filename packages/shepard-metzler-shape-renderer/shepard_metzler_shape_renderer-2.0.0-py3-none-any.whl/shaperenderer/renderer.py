...
class Renderer:
    ...
    def render(
        ...
        object: Object3D,
        camera: Camera,
        transforms: list[Callable[[list[np.array]], list[np.array]]] | None = None
    ) -> None:
        """
        """
        ...
        # apply optional transformations to the vertices from self.vbuffer
        if transforms:
            for transform in transforms:
                self.vbuffer = transform(self.vbuffer)
    ...
    def save_figure_to_numpy(
        ...
        figure_numpy = np.array(canvas.buffer_rgba())[:, :, :3]
