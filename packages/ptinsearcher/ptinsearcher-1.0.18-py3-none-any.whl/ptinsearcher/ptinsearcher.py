...
class PtInsearcher:
    def __init__(self, args):
        ...
        self.website_scraper        = _website_scraper.WebsiteScraper(args=args, things_to_extract=self.extract_types, url_list_len=len(self.url_list), ptjsonlib=self.ptjsonlib)
        self.file_scraper           = _files_scraper.FileScraper(args=args, things_to_extract=self.extract_types, ptjsonlib=self.ptjsonlib)

    def run(self, args: argparse.Namespace):
        for index, target in enumerate(self.url_list):
            scrape_result = self.process_target(target)
            self.result_list.append(scrape_result) if scrape_result else self.handle_whitespace(index)
            if scrape_result and not any([args.grouping, args.grouping_complete]):
                # Print current result
                results.print_result([self.result_list[-1]], args)
                self.handle_whitespace(index)

        # Print all results at once (when done scraping)
        if self.result_list and any([args.grouping_complete, args.grouping]):
    ...
    def process_target(self, target: str):
        ...
        """
        if self.target_is_url(target):
            scrape_result = self.website_scraper.scrape_url(url=target)
            return scrape_result
        elif self.target_is_file(target):
            scrape_result = self.file_scraper.process_file(path_to_local_file=os.path.abspath(target), args=self.args, things_to_extract=self.extract_types, ptjsonlib=self.ptjsonlib)
            return scrape_result
        else:
            error_message = f"The provided source ('{target}') contains neither an existing file nor a valid URL."
    ...
    def target_is_file(self, source: str) -> bool:
