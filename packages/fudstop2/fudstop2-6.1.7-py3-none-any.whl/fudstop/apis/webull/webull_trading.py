...
# Define the dictionary with all the rules
    ...
    async def multi_quote(self, tickers=['AAPL', 'SPY']):
        ...
        tasks = [self.ticker_to_id_map.get(str(ticker)) for ticker in tickers]
    ...
    async def deals(self, symbol: str, headers):
        try:
            ...
            async with self.client.get(endpoint, headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
                    deal_data = data.get('data', [])
                    return Deals(deal_data)
                else:
                    raise Exception(f"Failed to fetch deals data. HTTP Status: {response.status}")
        except Exception as e:
    ...
    async def get_short_interest(self, symbol: str):
        try:
            ...
            async with self.client.get(endpoint) as response:  # Use the client without closing it
                if response.status == 200:
        ...
        try:
            ...
            async with self.client.get(url) as response:
                if response.status == 200:
                    ...
                    raise Exception(f"Failed to fetch quote data: {response.status}")
        except Exception as e:
        ...
        try:
            ...
            async with self.client.get(endpoint, headers=headers) as response:
                if response.status == 200:
                    ...
                    raise Exception(f"Failed to fetch news data: {response.status}")
        except Exception as e:
        ...
        try:
            ...
            async with httpx.AsyncClient(headers=headers) as client:
                ...
                if response.status_code == 200:
                    ...
                    raise Exception(f"Failed to fetch order flow data. HTTP Status: {response.status}")
        except Exception as e:
    ...
    async def async_get_td9(self, ticker, interval, headers, count: str = '13'):
        try:
            ...
            async with self.client.get(base_fintech_gw_url, headers=headers) as response:
                if response.status == 200:
        ...
        try:
            ...
            async with self.client.get(url) as response:
                if response.status == 200:
                    ...
                    raise Exception(f"Failed to fetch price target data. HTTP Status: {response.status}")
        except Exception as e:
