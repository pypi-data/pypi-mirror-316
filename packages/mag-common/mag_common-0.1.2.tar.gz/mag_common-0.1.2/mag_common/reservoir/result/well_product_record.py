...
class WellProductRecord(BaseData):
    ...
    def __init__(self, report_time:Optional[float], work_time:Optional[float],
                 water_product_rate:Optional[float], gas_product_rate:Optional[float], oil_product_rate:Optional[float],
                 water_inject_rate:Optional[float], gas_inject_rate:Optional[float], water_product_total:Optional[float],
                 gas_product_total:Optional[float], oil_product_total:Optional[float], water_inject_total:Optional[float],
                 gas_inject_total:Optional[float], bottom_hole_pressure:Optional[float], tubing_head_pressure:Optional[float],
                 liquid_product_rate:Optional[float], liquid_product_total:Optional[float], water_cut:Optional[float],
                 water_gas_ratio:Optional[float], gas_oil_ratio:Optional[float], increase_time:Optional[float], molar_flow_rate:Optional[float]):
        """
        井生产数据类，包含了井的生产和注入数据。

        参数：
        :param report_time: 报告时间，单位为  DAYS
    ...
    def from_line(column_names, line_text):
        ...
        try:
            return WellProductRecord(report_time=float(data[ProductColumnName.REPORT_TIME]),
                                     work_time=float(data[ProductColumnName.WORK_TIME]),
                                     ...
                                     increase_time=float(data[ProductColumnName.INCREASE_TIME]),
                                     molar_flow_rate=None)
        except Exception:
            print(f"{data}")

    def to_line(self, column_names:List[ProductColumnName])->str:
        """
        ...
        format_values = StringFormat.pad_values(values, pad_length, self._text_format.justify_type, '\t')
...
if __name__ == '__main__':
    ...
    _lines = record.to_line(col_nums)
    for line in _lines:
        print(line)
