import WIPClient.WIPClient
import WIPClient.WIPClient_advance
...
class WIPDevice(WIPClientAbstractModel):
	...
	def __init__(self, ip="127.0.0.1", port=10008, use_advanced_protocol=True, device_id="device1", device_pw="00000000", isAutoConnect=True, live_receive=True, live_send=False, live_send_interval=1):
		...
		if use_advanced_protocol:
			self.mWIPClient = WIPClient.WIPClient_advance.WIPClient(ip, port, device_id, device_pw, isAutoConnect, live_receive, live_send, live_send_interval)
		else:
			self.mWIPClient = WIPClient.WIPClient.WIPClient(ip, port, device_id, device_pw, isAutoConnect, live_receive, live_send, live_send_interval)
