...
class WIPClient(WIPClientAbstractModel, SocketManagerV2.SocketClient):
        ...
        while(self.socketConnectThread_Enable):
            try:
                ...
                if(not self.ser_socket_connect_status):
                    # 재접속 전 이전 소켓 관련 쓰레드나 데이터 들은 미리 정리한다.
                    self.isCertified = False # 인증 변수 초기화

                    # 이미 생성된 쓰레드가 존재할 때
                    if(self.socketSendThread is not None):
                        if(self.socketSendThread.isAlive()):
                            print("[" + self.log_title + "] Waitting for socketSendThread to stop")
                            self.socketSendThreadLock.acquire()
                            self.live_send_run = False
                            self.socketSendThreadLock.release()
                            self.socketSendThread.join(1.5)

                        self.socketSendThread = None

                    if(self.socketReceiveThread is not None):
                        if(self.socketReceiveThread.isAlive()):
                            print("[" + self.log_title + "] Waitting for socketReceiveThread to stop")
                            self.socketReceiveThreadLock.acquire()
                            self.live_receive_run = False
                            self.socketReceiveThreadLock.release()
                            self.socketReceiveThread.join(1.5)
                    ...
                    time.sleep(0.5) # 인증시도 전 조금의 대기 진행
                ...
                # 접속 후 5초 넘게 인증이 완료되지 않았을 경우 재요청
                    ...
                    if self.isCertified and self.ser_socket_connect_status:
                        if time.time() - self.last_received_hb_time >= 120: # 2분동안 HB 데이터를 서버에서 못 받았을 때
                            print("[" + self.log_title + "] No heart beat data longer ")
                            self.close()
                ...
                print("[" + self.log_title + "] Connect to server process Exception " + str(e))
    ...
    def sendData(self, values):
        try:
            ...
            self.close()
            print("[" + self.log_title + "][In sendData] {0}:{1} The connection to the server has been lost.".format(self.ip, self.port))
            pass
        except socket.error as e:
            self.close()
            print("[" + self.log_title + "][In sendData] {0}:{1} The connection to the server has been lost.".format(self.ip, self.port))
    ...
    def sendDataDirect(self, values):
        try:
            ...
            self.close()
            print("[" + self.log_title + "][In sendDataDirect] {0}:{1} The connection to the server has been lost.".format(self.ip, self.port))
            pass
        except socket.error as e:
            self.close()
            print("[" + self.log_title + "][In sendDataDirect] {0}:{1} The connection to the server has been lost.".format(self.ip, self.port))
    ...
    def _sendThread(self):
        ...
        print("[" + self.log_title + "] Send thread is started")
        while(self.live_send_run):
            try:
                ...
                time.sleep(0.01)
        print("[" + self.log_title + "] Send thread is ended")

    # 데이터 자동 수신 쓰레드 호출 시 사용되는 함수 (사용자 정의)
    def _receiveThread(self):
        data = ""
        print("[" + self.log_title + "] Receive thread is started")
                ...
                if data is not None:
                    if not (len(data) == 0):
                                    ...
                                    if (
                                        ...
                                        if(cmd == api.CMD_LOGIN01):
                                            if(
                                                ...
                                                self.last_received_hb_time = time.time()
                        ...
                        if self.ser_socket_connect_status:
                            print("[" + self.log_title + "] Socket close")
                            self.close()
                        break

            except ConnectionResetError:
                print("[" + self.log_title + "] ConnectionResetError in  _receiveThread")
                self.close()
                break
            except socket.timeout as e:
                pass
            except socket.error as e:
                print("[" + self.log_title + "] socket.error in  _receiveThread" + str(e))
                self.close()
                break
            finally:
                time.sleep(0.001)
        
        print("[" + self.log_title + "] Receive thread is ended")
