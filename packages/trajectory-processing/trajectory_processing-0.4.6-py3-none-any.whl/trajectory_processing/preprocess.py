...
def to_all(input_filepath, output_filepath):
    gdf = import_and_preprocess_excel(input_filepath)
    csv_output_path = output_filepath
    ...
    if '速度' in list_df.columns:
        list_df.drop('速度', axis=1, inplace=True)
    list_df = spatial.create_distance_column(list_df)
    print('距离添加成功')
    ...
    list_df = list_df.sort_values(by="定位时间")
    ...
    for i in range(1, len(list_df)):
        lon_a, lat_a = list_df.iloc[i - 1, [list_df.columns.get_loc('经度'), list_df.columns.get_loc('纬度')]]
        lon_b, lat_b = list_df.iloc[i, [list_df.columns.get_loc('经度'), list_df.columns.get_loc('纬度')]]
        latA, lonA = math.radians(lat_a), math.radians(lon_a)
        latB, lonB = math.radians(lat_b), math.radians(lon_b)
        dlat = latB - latA
        dlon = lonB - lonA
    ...
    list_df.to_csv(csv_output_path, index=False)

    file_dir, file_name_with_ext = os.path.split(csv_output_path)
    file_name, _ = os.path.splitext(file_name_with_ext)
    excel_output_path = os.path.join(file_dir, f"{file_name}.xlsx")
    # 读取 CSV 文件到 DataFrame
    df = pd.read_csv(csv_output_path)
    df.to_excel(excel_output_path, index=False)

    # 将 DataFrame 写入到同名的 Excel 文件中
    df.to_excel(excel_output_path, index=False)
