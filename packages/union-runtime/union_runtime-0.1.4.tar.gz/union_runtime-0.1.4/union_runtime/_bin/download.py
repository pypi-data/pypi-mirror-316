...
# TODO: when obstore gets updated to 0.3.0 change offset, length to start, end
def _get_chunk(store, path_name: str, offset: int, length: int):
    failed_reads = 0
    while True:
        try:
...
def _download_file(store, path_name: str, dest: str, chunk_size: int = DEFAULT_CHUNK_SIZE):
    ...
    with open(dest, "wb") as f:
        for n_chunk in range(n_chunks):
            data = _get_chunk(store, path_name, n_chunk * chunk_size, chunk_size)
            f.write(data)

        # There is one more uneven chunk
        remainder = file_size % chunk_size
        if remainder > 0:
            data = _get_chunk(store, path_name, n_chunks * chunk_size, remainder)
            f.write(data)
