...
class IdentityVerification:
    ...
    def exract_passport_info(self, passport, country, nationality):
        ...
        if (nationality and nationality == 'JOR') or (not nationality and country == 'JOR'):
            ...
            from idvpackage.ocr_utils import  detect_photo_on_screen, detect_screenshot, document_on_printed_paper
            from idvpackage.jor_passport_extraction import jordan_passport_extraction, convert_date
            from idvpackage.common import load_and_process_image_deepface
            ...
            try:
                ...
                st = time.time()
                passport_details = jordan_passport_extraction(passport_text)
                logging.info(f'----------------Time taken for data formatting and final extraction passport: {time.time() - st} seconds\n')
                

                passport_details['issuing_date'] = convert_date(passport_details['issuing_date'])
                ...
                blurred, glare = self.get_blurred_and_glared_for_doc(image)
                valid_nationality_result = self.check_nationality_in_iso_list(passport_details.get('nationality'))
                print(valid_nationality_result)
                print(passport_details.get('nationality'))
                logging.info(f'----------------Time taken for fraud detection attributes passport: {time.time() - st} seconds\n')
