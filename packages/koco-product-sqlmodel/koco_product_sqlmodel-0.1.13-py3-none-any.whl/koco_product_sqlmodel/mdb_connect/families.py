...
from koco_product_sqlmodel.mdb_connect.mdb_connector import (
    translate_url_type,
)
from koco_product_sqlmodel.mdb_connect.spectables import (
    collect_overview_spectables,
    collect_family_spectables,
)
...
from koco_product_sqlmodel.mdb_connect.select import select_category_mapping_of_family


def create_family(family: CFamily) -> CFamily:
    if not family:
        return
...
def create_family_DB(family: CFamilyPost) -> CFamily:
    if not family:
        return
    with Session(mdb_engine) as session:
        session.add(family)
...
def update_family_DB(id: int | None, fam_post: CFamilyPost) -> CFamily | None:
    if id == None:
        return
    with Session(mdb_engine) as session:
        statement = select(CFamily).where(CFamily.id == id)
...
def get_families_db(product_group_id: int = None) -> list[CFamily]:
    if not product_group_id:
        statement = select(CFamily)
    else:
        statement = select(CFamily).where(CFamily.product_group_id == product_group_id)
    with Session(mdb_engine) as session:
...
def get_family_db_by_id(id: int) -> CFamily:
    statement = select(CFamily).where(CFamily.id == id)
    with Session(mdb_engine) as session:
        return session.exec(statement=statement).one_or_none()
