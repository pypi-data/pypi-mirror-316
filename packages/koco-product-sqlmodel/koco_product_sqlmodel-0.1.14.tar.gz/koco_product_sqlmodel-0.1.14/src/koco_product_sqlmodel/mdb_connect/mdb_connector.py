...
def delete_object_by_id(
    ...
    if object_type == sqm.CSpecTable:
        delete_spectable_by_id(
            log_func=log_func, spectable_id=obj_id, delete_connected_items=True, user_id=user_id
        )
        return
    if object_type == sqm.CArticle:
        delete_article_by_id(
            log_func=log_func, article_id=obj_id, delete_connected_items=True, user_id=user_id
        )
        return
    if object_type == sqm.CFamily:
        delete_family_by_id(
            log_func=log_func, family_id=obj_id, delete_connected_items=True, user_id=user_id
        )
        return
    if object_type == sqm.CProductGroup:
        delete_product_group_by_id(
            log_func=log_func, product_group_id=obj_id, delete_connected_items=True, user_id=user_id
        )
        return
    if object_type == sqm.CCatalog:
        delete_catalog_by_id(
            log_func=log_func, catalog_id=obj_id, delete_connected_items=True, user_id=user_id
        )
        return
...
def delete_catalog_by_id(
    log_func: Callable[[str, int, str, int, str],None],
    catalog_id: int, delete_connected_items: bool = False, user_id: int | None = None
):
    with Session(mdb_engine) as session:
        cat = session.exec(
    ...
    if cat:
        delete_catalog(
            log_func=log_func, catalog=cat, delete_connected_items=delete_connected_items, user_id=user_id
        )
...
def delete_catalog(
    ...
    if delete_connected_items:
        _delete_product_groups_from_catalog(
            log_func=log_func, catalog=catalog, delete_connected_items=True, user_id=user_id
        )
    with Session(mdb_engine) as session:
        log_func(
...
def delete_product_group(
    ...
    if delete_connected_items:
        _delete_families_from_product_group(
            log_func=log_func, product_group=product_group, delete_connected_items=True, user_id=user_id
        )
    with Session(mdb_engine) as session:
        log_func(
...
def _delete_families_from_product_group(
    ...
    for fam in families:
        delete_family(
            log_func=log_func, family=fam, delete_connected_items=delete_connected_items, user_id=user_id
        )


def delete_family_by_id(
    log_func: Callable[[str, int, str, int, str],None],
    family_id: int, delete_connected_items: bool = False, user_id: int | None = None
):
    with Session(mdb_engine) as session:
        fam = session.exec(
...
def delete_family(
    log_func: Callable[[str, int, str, int, str],None],
    family: sqm.CFamily, delete_connected_items: bool = True, user_id: int | None = None
):
    if delete_connected_items:
        ...
        _delete_applications_from_family(log_func=log_func, family=family, user_id=user_id)
        _delete_spectables_from_family(
            log_func=log_func, family=family, delete_connected_items=True, user_id=user_id
        )
...
def delete_category_mapping_by_id(
    mapping_id: int
):
    with Session(mdb_engine) as session:
...
def _delete_family_from_category_mapper(family: sqm.CFamily) -> None:
    with Session(mdb_engine) as session:
        statement = select(sqm.CCategoryMapper).where(sqm.CCategoryMapper.family_id == family.id)
        res = session.exec(statement)
        mappings = res.all()
    for mapping in mappings:
        delete_category_mapping_by_id(
...
def _delete_articles_from_family(
    ...
    for art in articles:
        delete_article(
            log_func=log_func, article=art, delete_connected_items=delete_connected_items, user_id=user_id
        )
...
def _delete_spectables_from_family(
    ...
    for st in res:
        delete_spectable(
            log_func=log_func, spectable=st, delete_connected_items=delete_connected_items, user_id=user_id
        )


def _delete_urls_from_family(
    log_func: Callable[[str, int, str, int, str],None],
    family: sqm.CFamily, user_id: int | None = None
    ):
    with Session(mdb_engine) as session:
        statement = (
            select(sqm.CUrl)
...
def delete_spectable_by_id(
    log_func: Callable[[str, int, str, int, str],None],
    spectable_id: int, delete_connected_items: bool = True, user_id: int | None = None
):
    with Session(mdb_engine) as session:
        st = session.exec(
...
def delete_spectable(
    ...
    if delete_connected_items:
        _delete_spectableitems_from_spectable(log_func=log_func, spectable=spectable, user_id=user_id)
    with Session(mdb_engine) as session:
        if user_id != None:
            log_func(
...
def _delete_spectableitems_from_spectable(
    log_func: Callable[[str, int, str, int, str],None],
    spectable: sqm.CSpecTable, user_id: int | None = None
):
    with Session(mdb_engine) as session:
        stis = session.exec(
...
def delete_article_by_id(
    log_func: Callable[[str, int, str, int, str],None],
    article_id: int, delete_connected_items: bool = True, user_id: int | None = None
):
    with Session(mdb_engine) as session:
        art = session.exec(
...
def delete_article(
    ...
    if delete_connected_items:
        ...
        _delete_spectables_from_article(
            log_func=log_func, article=article, delete_connected_items=True, user_id=user_id
        )
    with Session(mdb_engine) as session:
...
def _delete_urls_from_article(
        log_func: Callable[[str, int, str, int, str],None],
        article: sqm.CArticle, user_id: int | None = None
        ):
    with Session(mdb_engine) as session:
        statement = (
            select(sqm.CUrl)
...
def _delete_url(
        log_func: Callable[[str, int, str, int, str],None],        
        url: sqm.CUrl, user_id: int | None = None):
    with Session(mdb_engine) as session:
        if user_id != None:
            log_func(
                entity_type="curl",
...
def _delete_option(
        log_func: Callable[[str, int, str, int, str],None],
        option: sqm.COption, user_id: int | None = None):
    with Session(mdb_engine) as session:
        if user_id != None:
            log_func(
                entity_type="coption",
...
def _delete_application(
        log_func: Callable[[str, int, str, int, str],None],
        application: sqm.CApplication, user_id: int | None = None):
    with Session(mdb_engine) as session:
        if user_id != None:
            log_func(
...
def _get_ct_children(session, pid: int = None, level: int = None):
    ...
    for child in layer1:
        statement = (
            select(sqm.CUrl)
            .where(sqm.CUrl.parent == "categorytree")
            .where(sqm.CUrl.parent_id == child.id)
            .where(sqm.CUrl.type == "photo")
        )
