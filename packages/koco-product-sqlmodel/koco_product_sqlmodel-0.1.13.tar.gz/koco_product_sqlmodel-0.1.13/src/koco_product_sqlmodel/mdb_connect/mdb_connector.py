...
def delete_object_by_id(
    ...
    if object_type == sqm.CSpecTable:
        delete_spectable_by_id(
            spectable_id=obj_id, delete_connected_items=True, user_id=user_id
        )
        return
    if object_type == sqm.CArticle:
        delete_article_by_id(
            article_id=obj_id, delete_connected_items=True, user_id=user_id
        )
        return
    if object_type == sqm.CFamily:
        delete_family_by_id(
            family_id=obj_id, delete_connected_items=True, user_id=user_id
        )
        return
    if object_type == sqm.CProductGroup:
        delete_product_group_by_id(
            product_group_id=obj_id, delete_connected_items=True, user_id=user_id
        )
        return
    if object_type == sqm.CCatalog:
        delete_catalog_by_id(
            catalog_id=obj_id, delete_connected_items=True, user_id=user_id
        )
        return
