...
from inceptionlogger.settings_inc import get_settings
...
async def add_server_info_to_response(response: Response, correlation_id: str):
    ...
    response.headers["x-response-server-hostname"] = settings.hostname
    response.headers["x-response-server-ip-address"] = settings.ip_address
    response.headers["x-response-server-application-name"] = settings.application_name
    response.headers["x-response-server-application-version"] = settings.application_version
    response.headers["x-response-server-log-facility"] = settings.facility
    response.headers["x-response-server-log-host"] = settings.graylog_host
    return response
