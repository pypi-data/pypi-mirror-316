    ...
    def get(self):
        if self.obj is None:
            if self.h5group is None:
                raise ValueError(
                    "Trying to read object from H5PickleProxy but neither object nor underlying hdf5 storage is available."
                )
            elif not self.h5group:
                raise ValueError(
                    "Trying to read object from H5PickleProxy but the underlying file has been closed."
                )
...
class H5Buffer:
    ...
    def checkdset(self):
        if self.h5dset is None:
            raise ValueError(
                "Trying to read from H5Buffer but neither buffer nor dset are available."
            )
        elif not self.h5dset:
            raise ValueError(
                "Trying to read from H5Buffer but underlying file has been closed."
            )
...
def make_Hist(axes, storage, metadata, label, name, h5buf):
    ...
    hist_read = hist.Hist(
        *axes, storage=storage, metadata=metadata, label=label, name=name
    )
    hist_read_view = get_histogram_view(hist_read)
    h5buf.readinto(hist_read_view)
    return hist_read
...
class H5IO:
    ...
    def __init__(self, name, h5out, mode="r"):
        if mode == "r":
            ...
            self.dset = h5out.create_dataset(
                name,
                shape=(0,),
                dtype=np.ubyte,
                maxshape=(None,),
...
class H5Pickler(pickle.Pickler):
    ...
    def reduce_H5PickleProxy(self, proxy):
        ...
        if PROXIED_OBJECTS_NAME in current_location:
            ...
            proxied_objects = current_location.create_group(
                PROXIED_OBJECTS_NAME, track_order=True
            )

        iproxy = len(proxied_objects)
        proxied_group = pickle_dump_h5py(
            f"proxied_object_{iproxy}", proxy.obj, proxied_objects
        )
    ...
    def reduce_H5Buffer(self, h5buf):
        ...
        if H5BUFFER_GROUP_NAME in current_location:
            ...
            h5_buffers = current_location.create_group(
                H5BUFFER_GROUP_NAME, track_order=True
            )

        ibuf = len(h5_buffers)
        flat_view = pickle.PickleBuffer(h5buf.getbuffer()).raw()
        bufdset = h5_buffers.create_dataset(
            f"h5_buffer_{ibuf}",
            data=flat_view,
            chunks=True,
            **hdf5plugin.Blosc(cname="lz4"),
...
def pickle_load_h5py(h5group):
    ...
    try:
        ...
        raise ValueError(
            "h5py dataset does not contain a python object pickled by narf."
        )

    if narf_protocol_version < MIN_PROTOCOL_VERSION:
        raise ValueError(
            f"Unsuppported narf protocol version {narf_protocol_version}, minimum supported version is {MIN_PROTOCOL_VERSION}"
        )
    elif narf_protocol_version > CURRENT_PROTOCOL_VERSION:
        raise ValueError(
            f"Unsuppported narf protocol version {narf_protocol_version}, maximum supported version is {CURRENT_PROTOCOL_VERSION}"
        )
