...
pub fn exp_gate(matrix: Matrix, param: f64, inverse: bool) -> Matrix {
...
pub fn decompose_c1u2(
    ...
    let mut instructions = vec![
        Instruction::Gate {
            gate: QuantumGate::Phase(Angle::Scalar(alpha)),
            target: control,
            control: vec![],
        },
        Instruction::Gate {
            gate: QuantumGate::RotationZ(Angle::Scalar((delta - beta) / 2.0)),
            target,
            control: vec![],
        },
    ];

    instructions.extend(u4_gate_type.cnot(control, target));
    ...
    instructions.extend([
        ...
        Instruction::Gate {
            gate: QuantumGate::RotationY(Angle::Scalar(-gamma / 2.0)),
            target,
            control: vec![],
        },
    ]);
    ...
    instructions.extend([
        ...
        Instruction::Gate {
            gate: QuantumGate::RotationZ(Angle::Scalar(beta)),
            target,
            control: vec![],
        },
    ]);
...
pub fn c1gate(
    ...
    decompose_c1u2(gate.matrix(), control, target, u4_gate_type)
}
...
mod tests {
    ...
    #[test]
    fn print_phase() {
        let phase = extract_phase(QuantumGate::Hadamard.matrix());
        println!("{}", phase)
    }
}
