...
class QTL2631_pcie (PowerModule):
    ...
    class QTL2631_pcieCalibration (Calibration):
        ...
        def report(self, action, data):
            ...
            if action == "calibrate":
                report.append("\t" + '{0:>11}'.format('Set Value ') + self.units +
                              '   ' + '{0:>10}'.format('Raw Value ') + self.units +
                              '   ' + '{0:>12}'.format('Reference ') + self.units +
                              '   ' + '{0:>10}'.format('Result ') + self.units +
                              '   ' + '{0:>10}'.format('Error ') + self.units +
                              '   ' + '{0:>13}'.format('+/-(Abs Error,% Error)') +
                              '   ' + '{0:>10}'.format('Pass'))
            elif action == "verify":
                report.append("\t" + '{0:>11}'.format('Reference ') + self.units +
                              '   ' + '{0:>12}'.format('Set Value ') + self.units +
                              '   ' + '{0:>10}'.format('Result ') + self.units +
                              '   ' + '{0:>10}'.format('Error ') + self.units +
                              '   ' + '{0:>13}'.format('+/-(Abs Error,% Error)') +
                              '   ' + '{0:>10}'.format('Pass'))
            ...
            for thisLine in data:
                ...
                ppmValue = thisLine[0]
                setValue = thisLine[2]  # Assuming this is the "Set Value" passed in `data`
                ...
                if action == "calibrate":
                    report.append("\t" + '{:>11.3f}'.format(setValue) + '     ' +
                                  '{:>10.1f}'.format(ppmValue) + '     ' +
                                  '{:>12.3f}'.format(reference) + '     ' +
                                  '{:>10.1f}'.format(calibratedValue) + '     ' +
                                  "{:>10.3f}".format(actError) + '     ' +
                                  '{0:>16}'.format(errorSign + "(" + str(absError) + self.units + "," + "{:.3f}".format(
                                      relError) + "%)") + '     ' +
                                  '{0:>10}'.format(passfail(result)))
                elif action == "verify":
                    report.append("\t" + '{:>11.3f}'.format(setValue) + '     ' +
                                  '{:>12.3f}'.format(reference) + '     ' +
                                  "{:>10.3f}".format(actError) + '     ' +
            ...
            if action == "calibrate":
                report.append("Calculated Multiplier: " + str(self.multiplier.originalValue()) +
                              ", Calculated Offset: " + str(self.offset.originalValue()))
                report.append("Stored Multiplier: " + str(self.multiplier.storedValue()) +
                              ", Stored Offset: " + str(self.offset.storedValue()))
                report.append("Multiplier Register: " + self.multiplier.hexString(4) +
                              ", Offset Register: " + self.offset.hexString(4))
    ...
    class QTL2631_pcie_VoltageVerification (QTL2631_pcieCalibration):
        ...
        def init(self):
            ...
            # Check Host Power is present
            #while (super().checkLoadVoltage(500,500) != True):
                #self.powerModule.setConnections(self.thisChannel,None,reset=True)gma
