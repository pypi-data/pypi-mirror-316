    ...
    try:
        ...
        while True:  # While doing any calibration task
            ...
            if (calAction == 'quit'):
                ...
                addOverviewSectionToReportFile(reportFile, listOfTestResults, calAction,
                                               calculateTestPass(listOfFailures))
                reportFile.close()
        ...
        quarchpy.user_interface.logWarning("Unexpected Exception! Aborting tests and closing all device connections. " + str(thisException))
        logging.error("Unexpected Exception! Aborting tests and closing all device connections. " + str(thisException), exc_info=True)
        try:
            dut.close_all()
            myPpmDevice.closeConnection()
        # Handle case where exception may have been thrown before instrument was set up
            ...
            pass
    printText("reached the end of runCalibration ") #TODO sb db remove
...
def create_dut_object():
    ...
    while True:
        ...
        if (calibrationResources["moduleString"] == None):
            deviceString = userSelectDevice(scanFilterStr=scanFilterStr, nice=True,
...
def main(argstring):
    ...
    return runCalibration(loadAddress=args.instr, calPath=args.path, moduleAddress=args.module,
                                  logLevel=args.logging, calAction=args.action)
