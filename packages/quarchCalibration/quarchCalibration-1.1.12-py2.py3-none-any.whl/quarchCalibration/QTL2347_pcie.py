...
class QTL2347 (PowerModule):
    ...
    has_noise_test = True
    channel_dict_delta_limits = {"+3.3V mV": "400", "+12V mV": "400", "+3.3Vaux mV": "400"}
    channel_dict_min_limits = {"+3.3V mV": "2", "+12V mV": "2", "+3.3Vaux mV": "2"}
    channel_dict_average_limits = {"+3.3V mV": "121", "+12V mV": "121", "+3.3Vaux mV": "121"}
    ...
    class QTL2347Calibration (Calibration):
        ...
        def init_cal(self,voltage):
            ...
            try:
                ...
                logging.debug("Unable to set record averaging to 32k")
        ...
        def report(self, action, data):
            ...
            # Determine table headers based on the action type
            if action == "calibrate":
            ...
            # Convert report data to a table in string format
            ...
            # Add additional calibration data if action is "calibrate"
            if action == "calibrate":
                report.append(
            ...
            # Return final report dictionary
    ...
    class QTL2347_LeakageCalibration (QTL2347Calibration):
        ...
        def setup(self):
            self.powerModule.calInstrument.initialSetupForReferenceVoltage(currentLimit=1e-3,currentRange="1mA")
