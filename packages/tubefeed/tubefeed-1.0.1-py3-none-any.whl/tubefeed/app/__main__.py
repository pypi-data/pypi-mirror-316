...
async def get_channel(handle: str, include: str = None, limit: int | None = None):
    if not include:
    ...
    async with Database(DATABASE_PATH) as db:
        ...
        # build feed
        feed = Feed(BASE_URL, limit if limit is not None else FSL, UNSAFE_DOWNLOAD_METHOD)
...
@app.get('/playlist/{id}')
async def get_playlist(id: str, limit: int | None = None):
...
async def get_video_audio_m4a(id: str):
    """
    ...
    """
    try:
        audio_url = await get_audio_url(f'https://www.youtube.com/watch?v={id}', 'bestaudio[ext=m4a]')
...
async def get_video_audio_m4a_remuxed(id: str):
    """
    ...
    chapters_iter = re.finditer(r'^\s*((\d+):)?(\d+):(\d+)\s.*?(\w.*?)$', video.description, re.MULTILINE)
    chapters = []
    ...
    for chapter in chapters_iter:
        ...
        title = chapter.group(5).strip()

        chapters.append(((hours * 60 + minutes) * 60 + seconds, title))

    chapters.append((video.duration, None))
    ...
    if len(chapters) > 1:
        with NamedTemporaryFile(suffix='_chapters.metadata', mode='w', encoding='utf-8', delete=False) as metadata_file:
            ...
            for (start, title), (end, _) in zip(chapters, chapters[1:]):
                ...
                metadata_file.write(f'START={start}\n')
                metadata_file.write(f'END={end}\n')
    ...
    async def generate_stream():
        ...
        store_job = store_audio(video.url, 'bestaudio[ext=m4a]', metadata_path, m4a_path, r=MAX_DOWNLOAD_RATE)
