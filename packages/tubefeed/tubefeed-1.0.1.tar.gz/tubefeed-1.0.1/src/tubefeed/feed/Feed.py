...
class Feed(ET.Element):
    def __init__(self, base_url: str, item_limit: int | None, remuxed: bool):
    ...
    async def add_channel(self, channel: Channel, *playlists: Playlist):
        ...
        ET.SubElement(el, 'itunes:new-feed-url').text = f'{self.base_url}/channel/{channel.custom_url}'
        ET.SubElement(el, 'itunes:author').text = channel.title

        # image
        # thumbnail_url = f'{self.base_url}/channel/{channel.custom_url}/avatar.jpg'

        avatars = await channel.avatars()
        if len(avatars) > 0:
            thumbnail_url = max(avatars, key=lambda a: a.width).url
    ...
    async def add_video(self, channel_el, position: int, video: Video):
        ...
        if self.remuxed:
            ET.SubElement(el, 'enclosure', url=f'{self.base_url}/video/{video.id}/audio_remuxed.m4a')
        else:
            ET.SubElement(el, 'enclosure', url=f'{self.base_url}/video/{video.id}/audio.m4a')
