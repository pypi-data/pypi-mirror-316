    ...
    def _get_nvm_info(self):
        ...
        for nvm_name, nvm_data in nvm_dict.items():
            ...
            if nr_of_unused_signals > 0:
                default_datatype = self.zc_nvm.project_nvm_definitions[nvm_name]["default_datatype"]
                nvm_data["init"].extend([0] * nr_of_unused_signals)
                for idx in range(nr_of_unused_signals):
                    data_types[data_type_name]["elements"][f"eUnused_{idx}"] = default_datatype
