...
class BuildProjConfig:
    ...
    def include_all_enums(self):
        """Return True if all enums encountered in the enum definition files should be included in the generated code,
        even if they are not used in any models. Requires the ProjectInfo.enumDefDir key to be present in the project
        config.

        Returns:
            bool: True if all enumerations should be generated in source code, False otherwise.
        """
        if not self.get_enum_def_dir():
            return False
        return self._prj_cfg['ProjectInfo'].get('includeAllEnums', False)

    def map_to_rte_enums(self):
        """Return True if enums defined in external rte headers should be used when available. Default is False.

        Returns:
            (bool): True if enums defined in external RTE headers should be used if possible.
        """
        return self._prj_cfg['CodeGenerationConfig'].get('mapEnumsToRteCounterpart', False) and self.get_enum_def_dir()
