...
class NVMDef(ProblemLogger):
    ...
    def _generate_nvm_config_headers(self, individual_unused=False):
        """Generate nvm config h file.

        Optionally, also patch the defined variables with the "schedulerPrefix".

        Args:
            individual_unused (bool): If True, generate individual unused signals in the NVM struct.
        """
        ...
        def write_signals():
            for memory_area in self._nvm_memory_areas:
                ...
                if free > 0:
                    if individual_unused:
                        for idx in range(int(free / elem_size)):
                            hptr.write(
                                f'   {self.nvm_definitions[memory_area_index]["default_datatype"]} unused_{idx};\n'
                            )
                    else:
                        hptr.write(
...
class ZCNVMDef(NVMDef):
    ...
    def generate_nvm_rte_files(self):
        ...
        self._generate_nvm_structs_updated()
        self._generate_nvm_config_headers(individual_unused=True)
        self._generate_nvm_config_source()
