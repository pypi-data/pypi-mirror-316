        ...
        if response.status_code == 200:
            ...
            if data['model'] == 200:
                ...
                if hasattr(model, 'w_') and hasattr(global_model, 'w_'):
                    ...
                    # Update V_ (Latent Factor Matrix) without diluting averages for padded values
                    for row in range(max_rows):
                        for col in range(max_cols):
                            val_model = padded_model_V[row, col]
                            val_global = padded_global_V[row, col]

                            # Average only if both values are non-zero
                            if val_model != 0 and val_global != 0:
                                padded_model_V[row, col] = (val_model + val_global) / 2
                            elif val_model == 0:  # Use the non-zero value from global_model
                                padded_model_V[row, col] = val_global
                            elif val_global == 0:  # Use the non-zero value from model
                                padded_model_V[row, col] = val_model
