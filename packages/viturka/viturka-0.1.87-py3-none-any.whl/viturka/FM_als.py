...
def train_model(
    ...
    if vectorizer is None:
        ...
        categorical_features_train = vectorizer.fit_transform(X[categorical_columns].to_dict(orient="records"))
    else:
        # Combine the vocabulary of the provided vectorizer with the new training data
        combined_vocab = set(vectorizer.feature_names_)
        temp_vectorizer = DictVectorizer(sparse=True)
        temp_vectorizer.fit(X[categorical_columns].to_dict(orient="records"))

        combined_vocab = set(vectorizer.feature_names_).union(temp_vectorizer.feature_names_)

        # Fit on actual data + placeholders for combined_vocab
        vectorizer = DictVectorizer(sparse=True)
        vectorizer.fit(
            X[categorical_columns].to_dict(orient="records") +
            [{key: 0 for key in combined_vocab}]
        )
