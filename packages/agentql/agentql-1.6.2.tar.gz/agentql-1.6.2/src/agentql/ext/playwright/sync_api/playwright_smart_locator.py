...
from agentql.ext.playwright.sync_api.response_proxy import AQLResponseProxy, Locator
...
def _goto(
    ...
    *,
    ...
    result = self._original_goto(url=url, timeout=timeout, wait_until=wait_until, referer=referer)
...
@experimental_api
def _get_data_by_prompt_experimental(
    self,
    prompt: str,
    timeout: int = DEFAULT_QUERY_ELEMENTS_TIMEOUT_SECONDS,
    wait_for_network_idle: bool = DEFAULT_WAIT_FOR_NETWORK_IDLE,
    ...
    query, accessibility_tree = _generate_query(
        self,
        prompt=prompt,
        timeout=timeout,
        wait_for_network_idle=wait_for_network_idle,
...
def _query_elements(
    ...
    response, query_tree = _execute_query(
        self,
        query=query,
        timeout=timeout,
        include_hidden=include_hidden,
...
def _generate_query(
    ...
    log.debug(f"Generating query: {prompt} on {page}")

    page.wait_for_page_ready_state(wait_for_network_idle=wait_for_network_idle)

    try:
        accessibility_tree = get_accessibility_tree(page, include_hidden=include_hidden)

    except Exception as e:
        raise AccessibilityTreeError() from e

    response = generate_query_from_agentql_server(prompt, accessibility_tree, timeout, page.url)
...
def _execute_query(
    ...
    log.debug(
        f"Querying {'data' if is_data_query else 'elements'}: {minify_query(query)} on {page}"
    )
    ...
    if not accessibility_tree:
        try:
            accessibility_tree = get_accessibility_tree(page, include_hidden)
...
# pylint: disable=W0212
def _wait_for_page_ready_state(self, wait_for_network_idle: bool = DEFAULT_WAIT_FOR_NETWORK_IDLE):
...
setattr(_Page, "get_data_by_prompt_experimental", _get_data_by_prompt_experimental)
