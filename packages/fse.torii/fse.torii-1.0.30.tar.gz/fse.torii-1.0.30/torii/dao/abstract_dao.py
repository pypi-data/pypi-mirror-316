...
class AbstractDao:
    ...
    def get_collection(self, db_torii, db_bo, collection_name):
        """
        ...
        :param collection_name: BO Class name
        :return: Mongo collection from db_bo param
        """
        bo_class = next((item for item in self.CACHED_BO_CLASSES if item['name'] == collection_name), None)
        if bo_class: return db_bo.get_collection(bo_class['collection'])
        collection = db_torii.get_collection(self.COLLECTION_BO_CLASSES)
        bo_class = collection.find_one({'name': collection_name})
        self.CACHED_BO_CLASSES.append(bo_class)
        return db_bo.get_collection(bo_class['collection'])
    ...
    def prepare_query(self, query: dict) -> dict:
        ...
        return self.ensure_string_keys(query)
    ...
    def prepare_update(self, document: dict):
        ...
        return self.ensure_string_keys(document)
    ...
    @staticmethod
    def ensure_string_keys(document):
        """
        Helper method.
