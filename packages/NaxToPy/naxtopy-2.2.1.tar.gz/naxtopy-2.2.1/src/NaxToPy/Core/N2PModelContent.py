...
from NaxToPy.Core.Classes.N2PProperty import N2PProperty, N2PComp, N2PShell, N2PSolid, N2PBeam, N2PRod
...
class N2PModelContent:
    ...
    def __init__(self, path, parallelprocessing, solver=None, dict_gen: dict = None, filter: Literal['ELEMENTS', 'PROPERTIES', 'PARTS', 'NODES'] = None):
        ...
        error = self.__vzmodel.BuildMesh2(filter, dict_gen)
            ...
            class LazyDict(dict):
                ...
                def __getitem__(self, key):
                    ...
                    if key not in self or self.get(key) is None:
                        value = CONSTRUCTDICT[key.CardType.ToString()](key)
                        self[key] = value
                        return value
                    else:
                        return self.get(key)
    ...
    def MaterialDict(self) -> dict[int, N2PMaterial]:
        """
        Dictionary of materials. Key is the solver ID and value is a N2PMaterial
    ...
    def PropertyDict(self) -> dict[int, N2PProperty]:
        """
        Dictionary of properties. Key is the solver ID and value is a N2PProperty
    ...
    def get_elements_adjacent(self,
                              ...
                              domain: list[N2PElement, N2PConnector, ...] = None) -> list[
        N2PElement, N2PConnector, ...]:
    ...
    def get_result_by_LCs_Incr(self, list_lc_incr: Union[list[tuple["N2PLoadCase", "N2PIncrement"], ...],
                               list[tuple[int, int], ...]], result: str, component: str, sections=None, aveSections=-1,
                               cornerData=False, aveNodes=-1, variation=100, realPolar=0, coordsys: int = -1000,
                               v1: tuple = (1, 0, 0), v2: tuple = (0, 1, 0)) -> dict[tuple: "ndarray"]:
...
def load_model(path: str, parallelprocessing: bool = True,
    ...
    N2PModelContent Object. It also can read models from input files in Nastran format.
        ...
        Args:
            ...
            file_type: str -> Optional. It specifies if it is Nastran input file ("InputFileNastran"), Abaqus input file
            ...
            dict_gem: dict -> Optional. Dictionary that represents what elements/properties/parts/nodes to load. For 
