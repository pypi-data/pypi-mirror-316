...
class N2PGetLoadFasteners: 
    ...
    Attributes: 
        results_files: list[str] -> list of paths of OP2 results files. It is a compulsory input unless the model 
        loaded in model or in get_fasteners has results loaded in. 
        get_fasteners: N2PGetFasteners -> N2PGetFasteners model. Either this, or both _list_joints and _model, is a 
        compulsory input and an error will occur if this is not present. 2
        list_joints: list[N2PJoint] -> list of N2PJoints to be analyzed. Either both this and _model, or 
        get_fasteners, are compulsory inputs and an error will occur if they are not present. 
        model: N2PModelContent -> model to be analyzed. Either both this and _list_joints, or _get_fasteners, are 
        compulsory inputs and an error will occur if they are not present. 
        adjacency_level: int = 4 -> number of adjacent elements that will be loaded.
        load_cases: list[int] -> list of the IDs of all load cases to be analyzed. If no list is given, it is assumed 
        that all load cases should be analyzed. 
        corner_data: bool = False -> boolean which shows whether the corner data is to be used or not. 
        bypass_parameters: dict -> dictionary with the parameters used in the bypass loads calculation. Even though 
        the user may change any of these parameters, it is not recomended. 
        default_diameter: float -> default diameter for joints with no diameter. 
        analysis_name: str -> name of the CSV file where the results will be exported. By default, the name 
        'JointAnalysis' is used, but it could of course be changed by the user. This is done so that the user doesn't h
        ave to always set a name. The name should not include spaces. 
        export_location: str -> path where the aforementioned CSV file will be created.
        type_export: Literal["NAXTOPY", "ALTAIR"] = "NAXTOPY" -> style of the export results, either in the NaxToPy 
        style or in the Altair style.        
        results: dict -> results dictionary which should not be altered by the user. 
    """
    ...
    def __init__(self): 
        ...
        Calling example: 
            ...
            >>> model1 = NP.get_model(route.fem) # model loaded 
            ...
            >>> loads.BypassParameters = {"max iterations" = 50, "PROJECTION TOLERANCE" = 1e-6} # some bypass parameters 
            are changed (optional and not recommended)
    ...
    def ResultsFiles(self) -> list[str]: 
        """
        Property that returns the results_files attribute, that is, the list of paths of OP2 result files. 
        """
    ...
    def GetFasteners(self) -> N2PGetFasteners: 
        ...
        """
        Property that returns the get_fasteners attribute, that is, the N2PGetFasteners model. 
        """
    ...
    def Model(self) -> N2PModelContent: 
        ...
        """
        Property that returns the model attribute, that is, the N2PModelContent model to be analyzed. 
        """
    ...
    def AdjacencyLevel(self) -> int: 
        ...
        """
        Property that returns the adjacency_level attribute, that is, the number of adjacent elements that are loaded 
        into the model. 
        """
    ...
    def LoadCases(self) -> list[int]: 
        ...
        """
        Property that returns the load_cases attributes, that is, the list of the IDs of the load cases to be analyzed. 
        """
    ...
    def CornerData(self) -> bool: 
        ...
        """
        Property that returns the corner_data attribute, that is, whether there is data on the corners or not to 
        extract the results. 
        """
    ...
    def BypassParameters(self) -> dict: 
        ...
        """
        Property that returns the bypass_parameters attribute, that is, a set of parameters to be used in the bypass 
        loads calculations. 
        """
    ...
    def DefaultDiameter(self) -> float: 
        ...
        """
        Property that returns the default_diameter attribute, that is, the diameter to be applied to joints with no 
        previously assigned diameter. 
        """
    ...
    def AnalysisName(self) -> Literal["NAXTOPY", "ALTAIR"]: 
        ...
        """
        Property that returns the analysis_name attribute, that is, the name of the CSV file where the results are to 
        be exported. 
        """
    ...
    def TypeExport(self) -> str: 
        ...
        the NaxToPy style or in the Altair style.
        """
    ...
    def Results(self) -> dict: 
        ...
        """
        Property that returns the results attribute, that is, the results obtained in get_results_joints(). 
        """
