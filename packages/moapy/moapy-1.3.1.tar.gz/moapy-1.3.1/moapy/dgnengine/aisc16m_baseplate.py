...
@auto_schema(
    title="AISC-LRFD16M Base Plate Design",
    description=(
        "The AISC-LRFD16M standard outlines the requirements for designing base plates that connect steel columns "
...
def report_aisc16m_baseplate(baseplate: BasePlate = BasePlate.create_default(unit_system=enUnitSystem.SI),
                             sect: SteelSection = SteelSection.create_default(name="HP410X151", enum_list=enum_to_list(en_H_AISC10_SI)),
                             force: SectionForce = SectionForce.create_default(unit_system=enUnitSystem.SI),
                             anchor: AnchorBolt = AnchorBolt.create_default(unit_system=enUnitSystem.SI)) -> ResultBytes:
    dll = load_dll()
...
def calc_aisc16m_baseplate(baseplate: BasePlate = BasePlate.create_default(unit_system=enUnitSystem.SI),
                           sect: SteelSection = SteelSection.create_default(name="HP410X151", enum_list=enum_to_list(en_H_AISC10_SI)),
                           force: SectionForce = SectionForce.create_default(unit_system=enUnitSystem.SI),
                           anchor: AnchorBolt = AnchorBolt.create_default(unit_system=enUnitSystem.SI)) -> dict:
    dll = load_dll()
