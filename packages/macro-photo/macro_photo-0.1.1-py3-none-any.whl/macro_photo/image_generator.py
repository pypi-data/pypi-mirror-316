...
class MacroImageGenerator:
    """A class for generating images using advanced AI technology."""
    ...
    def _init_headers(self):
        ...
        self.headers = {
            ...
            'referer': 'https://chat.mistral.ai/chat/a10ee6a7-c04e-4649-98c6-20650cc3bb50',
    ...
    def generate_image(self, prompt: str, filename: Optional[str] = None, 
        ...
        json_data = {
            'chatId': 'a10ee6a7-c04e-4649-98c6-20650cc3bb50',
            'mode': 'append',
            'model': 'mistral-small',
            'messageInput': prompt,
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not url_match:
                    print(f"{Fore.RED}❌ No image URL found in response{Style.RESET_ALL}")
                    if attempt < max_retries - 1:
                        print(f"{Fore.YELLOW}⏳ Retrying in {retry_delay} seconds...{Style.RESET_ALL}")
                        time.sleep(retry_delay)
                        continue
                ...
                if image_response.status_code == 200:
                    ...
                    print(f"{Fore.GREEN}✨ Creation success! Image saved to: {Style.BRIGHT}{filepath}{Style.RESET_ALL}")
                    return filepath
                else:
                    print(f"{Fore.RED}❌ Failed to download image: Status code {image_response.status_code}{Style.RESET_ALL}")
    ...
    def batch_generate(self, prompts: List[str], prefix: str = "") -> List[str]:
        ...
        print(f"\n{Fore.CYAN}🎯 Starting batch generation of {total} images...{Style.RESET_ALL}\n")
        
        for i, prompt in enumerate(prompts, 1):
            print(f"{Fore.CYAN}📸 Processing image {i}/{total}{Style.RESET_ALL}")
        ...
        print(f"\n{Fore.GREEN}✅ Batch generation completed!")
        print(f"📊 Successfully generated: {success_count}/{total} images{Style.RESET_ALL}")
