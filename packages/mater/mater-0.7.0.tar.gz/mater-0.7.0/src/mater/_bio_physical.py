...
class _MaterProcessBioPhysical(_MaterPreProcess):
    ...
    def _log_normal_distribution_computation(self, k: int):
        r"""
        ...
        self.df_log_normal = _log_normal(
            ...
            self.lifetime_standard_deviation_log,
            k - 1,
    ...
    def _self_disposal_flow_computation(self, k: int):
        ...
        self_disposal_flow_distribution = pd.concat(
            ...
            keys=[k - 1],
            names=["age_cohort"],
        ).reorder_levels(["location", "object", "age_cohort"])
    ...
    def _traded_control_flow_computation(self, k: int):
        ...
        if self.trade:
            ...
            self.outputs["traded_control_flow"] = traded_control_flow.combine_first(self.outputs["traded_control_flow"])
        else:
    ...
    def _controlled_process_computation(self, k: int):
        ...
        if self.exogenously_controlled_process[k].empty:
            self.controlled_process = (
                self.endogenously_controlled_process.mul(self.inputs.process_shares[k], fill_value=1)
            ).reorder_levels(["location", "object", "process"])
        else:
            self.controlled_process = (
                (self.endogenously_controlled_process.mul(self.inputs.process_shares[k]))
                .reorder_levels(["location", "object", "process"])
    ...
    def _free_process_computation(self, k: int):
        ...
        self.free_process = _groupby_sum_empty(
            self.outputs["reference_intensity_of_use"][k - 1].mul(self.outputs1["in_use_stock"]),
    ...
    def _process_computation(self, k: int):
        ...
        if self.controlled_process.empty and self.free_process.empty:
            ...
            self.outputs["process"] = self.controlled_process.combine_first(self.free_process)
    ...
    def _control_flow_composition_computation(self, k: int):
        ...
        self.control_flow_composition = _groupby_sum_empty(
        ...
        if self.trade:
            traded_control_flow_composition = (
            ...
            self.control_flow_composition = self.control_flow_composition.mul(
                traded_control_flow_composition, fill_value=1
            ).div(
                traded_control_flow_composition, fill_value=1
            )  # only needed if the the control flow trade matrix is not complete.
            self.control_flow_composition = traded_control_flow_composition.combine_first(self.control_flow_composition)
            # only needed if the the control flow trade matrix is not complete.
    ...
    def _thermodynamic_process_flow_computation(self, k: int):
        ...
        process_flow = self.inputs.thermodynamic_process[k].mul(self.process).dropna(axis=0, how="all")
        if process_flow.empty:
            df = process_flow.copy()
        else:
            # sum of flows per process
            df = (
                process_flow.sub(self.control_flow_composition, fill_value=0).add(
                    self.outputs["secondary_production"], fill_value=0
                )
    ...
    def _in_use_stock_computation(self, k: int):
        ...
        extraneous_flow = pd.concat(
            {k: self.outputs["extraneous_flow"].groupby(level=["location", "object"]).sum()},
            names=["age_cohort"],
        ).reorder_levels(["location", "object", "age_cohort"])
        # control flow with production date
        control_flow = pd.concat({k: self.outputs["control_flow"]}, names=["age_cohort"]).reorder_levels(
            ["location", "object", "age_cohort"]
        )
        # in-use stock
