...
class _MaterProcessNonPhysical(_MaterProcessBioPhysical):
    ...
    def _control_flow_computation(self, k: int):
        ...
        if self.outputs1["reference_stock"].empty:
            ...
            reference_stock = self.inputs.exogenous_stock[k].combine_first(self.outputs1["reference_stock"])
        full_state_feedback = reference_stock.sub(
            self.outputs1["in_use_stock"].groupby(level=["location", "object"]).sum().loc[reference_stock.index],
            fill_value=0,
        )
        ...
        feedforward = full_state_feedback.add(aligned_self_disposal_flow, fill_value=0)
        # positivity of control_flow
        #### WARNING: first simulation step, if a "free" object has lifetime he will have a control flow.
