...
@click.option("-f", "--final-time", type=int, default=2100, help="The final time step.")
def run(input_file: str, output_folder: str, start_time: int, final_time: int):
    model = Mater()
    model.run_from_excel(output_folder, input_file, simulation_start_time=start_time, simulation_end_time=final_time)
...
def plot(output_folder: str):
    os.system(f"streamlit run .\\src\\mater\\plot.py -- --output-folder {output_folder}")
