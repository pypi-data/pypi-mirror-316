...
#==============================================================================
def fDf_removeDoublons(df_in, l_subset = None):
...
def fInt_FindIndex(df, str_valueToFind, bl_resetIndex = False):
    try:
        ...
        logger.error('  ERROR in dfram.fInt_FindIndex: |{}| - |{}| '.format(str_valueToFind, err))
        raise
...
def fInt_FindInde_like(df, str_valueToFind, bl_resetIndex = False):
    ...
    try:
        ...
        logger.error('  ERROR in dfram.fInt_FindInde_like: |{}| - |{}| - |{}|'.format(str_valueToFind, str_msgErr, err))
        raise        
...
def fInt_FindColumn(df, str_valueToFind):
    ...
    try:
        ...
        logger.error('  ERROR in dfram.fInt_FindColumn: |{}| - |{}|'.format(str_valueToFind, err))
        raise        
...
def fDf_FindSubDataframe(df, str_valueToFind, str_valueToEnd = '!@#$%', int_addRowsStart = 0, int_addRowsEnd = 0, int_occurStart = 0,  
    ...
    try:
        ...
        logger.error('  ERROR in fDf_FindSubDataframe: |{}| - |{}| - |{}|'.format(str_valueToFind, str_valueToEnd, err))
        return df
    return df_sub
...
def fStr_VlookUp(df, v_valueToFind, int_colNb = 1):
    if isinstance(v_valueToFind, str):
        try:
            ...
            int_col =       fInt_FindColumn(df, v_valueToFind)
            int_row =       int(int_row.values[0])
            int_col =       int(int_col.values[0])
            int_col2 =      int_col + int(int_colNb)
            str_return =    df.iloc[ int_row, int_col2 ]
        except Exception as err:
            logger.error('  ERROR in fStr_VlokUp: Could not find the value in the file: |{}| - |{}| - |{}|'.format(v_valueToFind, int_row, err))
            pd.set_option('display.max_rows', 1000)
            logger.error(df)
        ...
        for _valueToFind in v_valueToFind:
            try:
                ...
                int_row =       int(int_row.values[0])
                int_col =       int(int_col.values[0])
                int_col2 =      int_col.values[0] + int(int_colNb)
                str_return =    df.iloc[ int_row, int_col2 ]
                return str_return
...
def fStr_HlookUp(df, v_valueToFind, int_rowNb = 2):
    if isinstance(v_valueToFind, str):
        try:
            ...
            int_row =       int(int_row.values[0])
            int_col =       int(int_col.values[0])
            int_row2 =      int_row + int(int_rowNb)
            str_return =    df.iloc[ int_row2, int_col ]
            return str_return
        except Exception as err:
            logger.error('  ERROR in fStr_HlokUp: Could not find the value in the file: |{}| - |{}| - |{}|'.format(v_valueToFind, int_rowNb, err))
            pd.set_option('display.max_rows', 1000)
            logger.error(df)
            # Return None avoid error and is more logic with the LIST behavior
            return None
...
def fDf_GroupBy_sevColumns(df_in, l_colPivot, str_colMeasure, d_aggPerCol={}):
    if str_colMeasure == '':
        df = dframe.fDf_CleanPrepareDF(df_in, l_colToDropNA=l_colPivot, o_fillNA_by=0)
    else:
        df = dframe.fDf_CleanPrepareDF(df_in, l_colToBeFloat=[str_colMeasure], l_colToDropNA=l_colPivot, o_fillNA_by=0)
    # Sum is by default
    if d_aggPerCol == {}:
        d_aggPerCol = {str_colMeasure: 'sum'}
    # Group and have the colPivot as Index
