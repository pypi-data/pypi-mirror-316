try:
    from pynut_3ftp.pyNutFtp import _lib as lib
except:
    try:
        from pyNutFtp import _lib as lib
    except:
        try:
...
class c_FTP:
    ...
    def __str__(self):
        logger.warning('  ** FTP Credentials: {}|{}|{}|{}|'.format(self.__str_host, self.__str_uid, self.__str_pwd, self.__int_portnumber))
        
    def __ftpConnexionClose(self):
        logger.warning(' ** Closing FTP Connection: {}'.format(self.__str_host))
        self.l_Folder = []
        try:    
    ...
    def __ftpClassic_Connect(self):
        try:
            ...
            logger.error(' ERROR: f_ftpConect in c_FTP | #{}#'.format(err))
            logger.error('  - ** ARGS : |{}|-|{}|-|{}|-|{}|'.format( self.__str_host, self.__str_uid, self.__str_pwd, self.__int_timeOut))
            raise
    ...
    def __ftpSSL_Connect(self):
        try:
            ...
            logger.error(' ERROR: ftpSSL_Connect | #{}#'.format(err))
            logger.error('  - ** ARGS : |{}|-|{}|-|{}|-|{}|'.format(self.__str_host, self.__str_uid, self.__str_pwd, self.__int_portnumber))
            raise
        self.o_ftpConnexion = ftp_connect

    def __message_ConnexionLost(self):
        if self.__bl_msgLosingConnection:
            logger.warning(' ** FTP Connection LOST: {}'.format(self.__str_host))
    ...
    def ftp_Connect(self, str_host, str_uid, str_pwd, bl_ssl = False, int_timeOut = -1, int_portnumber = 21):
        if self.__str_host == str_host and self.__str_uid == str_uid and self.__str_pwd == str_pwd and self.__bl_ssl == bl_ssl:
            try:
                ...
                # logger.warning(' FTP: An existing connexion already exist and will be used again | Server: {}'.format(ftpConnexion.host))
                return ftpConnexion
        ...
        try:
            ...
            logger.warning(' WARNING: A parameter in the FTP must have a space in it, please remove it ! | #{}#'.format(err))
    ...
    def ftp_changeFolder(self, l_Folder):
        ...
        try:
            ...
            logger.error(' ERROR: ftp_changeFolder, FTP could not change backwards folder | #{}#'.format(err))
            logger.error('  - {}'.format('|'.join(l_Folder)))
        ...
        try:
            ...
            logger.error(' ERROR: ftp_changeFolder, FTP could not change folder | #{}#'.format(err))
            logger.error('  - {}'.format('|'.join(l_Folder)))
            try:    logger.error(' - folder : |{}|'.format(str_folder))
            except: pass
            self.__str__()        
    ...
    def fL_fileInFTP_beforeDownload(self):
        ...
        try:
            ...
            logger.error(' ERROR in fL_fileInFTP_beforeDownload | {}'.format(err))
            raise
    ...
    def fL_fileInFTP(self):
        try:    self.l_nameFiles = self.o_ftpConnexion.nlst()
            ...
            logger.error(' ERROR in fL_fileInFTP')
            raise
        return self.l_nameFiles
    ...
    def fL_fileInFTP_wCharac(self):
        try:    self.l_nameFiles_wCharac = self.o_ftpConnexion.retrlines('LIST')
            ...
            logger.error(' ERROR in fL_fileInFTP_wCharac')
            raise
    ...
    def ftp_DownloadFile(self, str_fileName, str_folder):
        ...
        try:
            ...
            logger.error(' ERROR in ftp_DownloadFile - Creation of the file')
            logger.error('  - ** ARGS : |{}|-|{}|'.format(str_folder, str_fileName))
            raise
        # Fill the file with whats in FTP
        try:
            self.o_ftpConnexion.retrbinary('RETR {}'.format(str_fileName), Open_file.write)
        except Exception as err:  
            logger.error(' ERROR in ftp_DownloadFile - retrbinary | {}'.format(str(err)))
            logger.error('  - ** ARGS : |{}|-|{}|'.format(str_folder, str_fileName))
            logger.error('  - ** ARGS : |{}|-|{}|-|{}|'.format(self.__str_host, self.__str_uid, self.__str_pwd))
            logger.error('  - ** ARGS : |{}|-|{}|-|{}|'.format(self.__bl_ssl, self.__int_timeOut, self.__int_portnumber))
            Open_file.close()
            # delete the file
    ...
    def ftp_UploadFile(self, str_fileName, str_folder):
        ...
        try:
            ...
            logger.error(' ERROR in ftp_UploadFile - Opening of the file | {}'.format(str(err)))
            logger.error('  - ** ARGS : |{}|-|{}|'.format(str_folder, str_fileName))
            raise
        # Fill the file with whats in FTP
        try:
            ...
            Open_file.close()
            logger.error(' ERROR in ftp_UploadFile - storbinary | {}'.format(str(err)))
            logger.error('  - ** ARGS : |{}|-|{}|'.format(str_folder, str_fileName))
            logger.error('  - ** ARGS : |{}|-|{}|-|{}|'.format(self.__str_host, self.__str_uid, self.__str_pwd))
            logger.error('  - ** ARGS : |{}|-|{}|-|{}|'.format(self.__bl_ssl, self.__int_timeOut, self.__int_portnumber))
            raise
    ...
    def ftp_renameAfterUpload(self, str_oldName, str_newName):
        try:
            ...
            logger.error(' ERROR in ftp_renameAfterUpload - rename | {}'.format(str(err)))
            logger.error('  - ** ARGS : |{}| => |{}|'.format(str_oldName, str_newName))
        return True
...
class c_SFTP():
    ...
    def __sftpConnexionClose(self, bl_closingMsg = True):
        # MESSAGE
        if bl_closingMsg:
            logger.warning(' ** Closing SFTP Connection: {}'.format(self.__str_host))
        # If we lose the connexion, we need to reset the folder to Null
        self.l_Folder = []
    ...
    def __message_ConnexionLost(self):
        if self.__bl_msgLosingConnection:      
            logger.warning(' ** SFTP Connection LOST: {}'.format(self.__str_host))
        # This way, if connexion is not found, we have message Connection is lost
        self.__bl_msgLosingConnection = True
        # If we lose the connexion, we need to reset the folder to Null
        self.l_Folder = []
    ...
    def sftp_Connect(self, str_host, str_uid, str_pwd, int_timeOut = -1, int_portnumber = 22):
        if self.__str_host == str_host and self.__str_uid == str_uid and self.__str_pwd == str_pwd:
            try:
                ...
                return sftp_oppen
                logger.warning(' SFTP: An existing connexion already exists and will be used again | Server: {}'.format(sftpConnexion))
            except:     self.__message_ConnexionLost()
        else:
        ...
        try:
            ...
            logger.error('  ERROR in sftp_Connect (PARAMIKO) : |{}|'.format(err))
            logger.error('  - ** ARGS : |{}|-|{}|-|{}|-|{}|'.format(self.__str_host, self.__str_uid, self.__str_pwd, self.__int_portnumber))
            raise
        # OPEN
        try:
            ...
            logger.error('  ERROR in sftp_Connect - Open SFTP : |{}|'.format(err))
            logger.error('  - ** ARGS : |{}|-|{}|-|{}|-|{}|'.format(self.__str_host, self.__str_uid, self.__str_pwd, self.__int_portnumber))
            raise
        # Return
    ...
    def __checkFolder(self):
        ...
        if str_currentFolder is None:
            logger.warning('   ** SFTP has lost its current folder (None), we need to chDir again on : {}'.format(l_pastSessionFolder))
            l_pastSessionFolder = []
        elif str_pastSessionFolder != str_currentFolder:
            logger.warning('   ** SFTP has lost its folder (is currently in: {}, past session on : {})'.format(str_currentFolder, l_pastSessionFolder))
            l_pastSessionFolder = []
    ...
    def sftp_changeFolder(self, l_Folder):
        ...
        try:
            for str_folder in l_pastSessionFolder:
                if not str_folder == None:
                    if not str_folder == '':
                        logger.warning('  -- Folder going backwards on folder: ', str_folder)
                        self.o_sftpOpen.chdir("../")
        except Exception as err:
            logger.error(' ERROR: sftp_changeFolder, SFTP could not change backwards folder : |{}|'.format(err))
            logger.error(' - {}'.format('|'.join(l_Folder)))
            logger.error(' - folder Past Session : |{}|'.format(l_pastSessionFolder))
        # Run the List of Folder
        try:
            for str_folder in l_Folder:
                if not str_folder == None:
            ...
            logger.error(' ERROR: sftp_changeFolder, SFTP could not change folder : |{}|'.format(err))
            logger.error(' - {}'.format('|'.join(l_Folder)))
            try:    logger.error(' - folder : |{}|'.format(str_folder))
            except: pass
            try:    
                logger.error('  - SFTP is in the folder:')
                logger.error('     |{}|'.format(self.o_sftpOpen.getcwd()))
            except: pass
        # Fin: Assign the folder to a variable to check up later
        self.l_Folder = l_Folder
    ...
    def sftp_DownloadFile(self, str_fileName, str_folder):
        try:
            ...
            logger.error(' ERROR in sftp_DownloadFile - get : |{}| '.format(err))
            logger.error('  - ** ARGS : |{}|-|{}|-|{}|'.format(self.__str_host, self.__str_uid, self.__str_pwd))
            logger.error('  - ** ARGS : |{}|-|{}|-|{}|'.format(str_fileName, self.__int_timeOut, self.__int_portnumber))
    ...
    def sftp_UploadFile(self, str_fileName, str_folder, bl_confirm = True):
        try:
            self.o_sftpOpen.put(os.path.join(str_folder, str_fileName), str_fileName, confirm = bl_confirm)
        except Exception as err:
            logger.error(' ERROR in sftp_UploadFile - put : |{}| '.format(err))
            logger.error('  - ** ARGS : |{}|-|{}|-|{}|'.format(self.__str_host, self.__str_uid, self.__str_pwd))
            logger.error('  - ** ARGS : |{}|-|{}|-|{}|'.format(str_fileName, self.__int_timeOut, self.__int_portnumber))
            #self.__sftpConnexionClose()
            raise
    ...
    def sftp_renameAfterUpload(self, str_oldName, str_newName):
        try:
            ...
            logger.error(' ERROR in sftp_renameAfterUpload - rename | {}'.format(str(err)))
            logger.error(' - ** ARGS : |{}|-|{}|'.format(str_oldName, str_newName))
        return True
    ...
    def fL_fileInSFTP_beforeDownload(self):
        ...
        try:
            ...
            logger.error(' ERROR in fL_fileInSFTP | {}'.format(err))
            raise
        return self.l_nameFiles
    ...
    def fL_fileInSFTP_wCharac(self):
        try:    
            ...
            logger.error(' ERROR in fL_fileInSFTP_wCharac | {}'.format(err))
            raise
        return self.l_nameFiles_wCharac
...
def ssh_upFile(str_host, str_uid, str_pwd, l_ftpFolder, str_fileName, str_folder, int_timeout=-1, int_port=None,
               bl_FTP_PostCheck=True):
    inst_sftp = sftp_prep(str_host, str_uid, str_pwd, l_ftpFolder, int_timeout, int_port)
    inst_sftp.sftp_UploadFile(str_fileName, str_folder, bl_confirm=bl_FTP_PostCheck)
    return True
...
def ssh_getLastFile_inFolder(str_host, str_uid, str_pwd, l_ftpFolder, int_timeout = -1, int_port = None):
    ...
    try:
        ...
        logger.error('  RROR in ssh_getLastFile_inFolder : |{}|'.format(err))
        logger.error('  - ** ARGS : |{}|-|{}|-|{}|-|{}|'.format(str_host, str_uid, str_pwd, int_timeout))
        logger.error('  - {}'.format('|'.join(l_ftpFolder)))
        raise
...
# *** no class for now ***
# ------------------------------------------------------------------------
def sftp_PPK_DownloadFile(str_FTP_server, str_FTP_uid, str_ftpDir, str_fileName, str_folder,
                          int_port=22, str_pathKey_ppk=''):
    try:
        warnings.filterwarnings('ignore')
        cnopts = pysftp.CnOpts()
        cnopts.hostkeys = None

        with pysftp.Connection(host = str_FTP_server,
                               username = str_FTP_uid,
                               port = int_port,
                               private_key = str_pathKey_ppk,
                               cnopts = cnopts) as sftp:
            # print('Connection successfully established to {0}'.format(str_FTP_server))

            # Change Folder
            sftp.cwd(str_ftpDir)
            # WITH SEARCH
            if '{*}' in str_fileName:
                _fileName = str_fileName.replace('{*}', '*')
                for str_temp in sftp.listdir(str_ftpDir):
                    if fnmatch.fnmatch(str_temp, _fileName):
                        str_fileName = str_temp
                        break
            # Download
            sftp.get(str_fileName, fl.fStr_BuildPath(str_folder, str_fileName))
            # print('File downloaded to: {0}'.format(str_fileName))
    except Exception as err:
        print(' ERROR: sftp_PPK_DownloadFile | |{}|'.format(err))
        print(' - ', str_FTP_server, str_FTP_uid, str_ftpDir, str_fileName)
        raise
