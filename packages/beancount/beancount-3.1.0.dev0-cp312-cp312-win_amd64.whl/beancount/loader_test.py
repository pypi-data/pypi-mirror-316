...
class TestLoader(unittest.TestCase):
    ...
    @mock.patch(
        "importlib.import_module", functools.partial(mock_import_module, ValueError)
    )
    ...
    @mock.patch(
        "importlib.import_module", functools.partial(mock_import_module, SystemExit)
    )
    ...
    def test_load_string(self):
        with test_utils.capture():
            ...
            entries, errors, options_map = loader.load_string(
                TEST_INPUT, log_timings=logging.info
            )
            self.assertTrue(isinstance(entries, list))
            self.assertTrue(isinstance(errors, list))
            self.assertTrue(isinstance(options_map, dict))
...
class TestLoadIncludes(unittest.TestCase):
    ...
    def test_load_file_with_nonexist_include(self):
        ...
        self.assertEqual(
            ["root.beancount"], list(map(path.basename, options_map["include"]))
        )
    ...
    def test_load_file_with_absolute_include(self):
        ...
        self.assertEqual(
            ["apples.beancount", "oranges.beancount"],
...
class TestLoadCache(unittest.TestCase):
    def setUp(self):
        self.num_calls = 0
        cache_getter = functools.partial(
            loader.get_cache_filename, loader.PICKLE_CACHE_FILENAME
        )
        mock.patch(
            "beancount.loader._load_file",
            loader.pickle_cache_function(
                cache_getter,
                0,  # No time threshold.
                self._load_file,
            ),
