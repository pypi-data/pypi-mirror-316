...
class TestRealGetters(unittest.TestCase):
    def test_get(self):
        ra0 = create_simple_account()
        self.assertEqual("Assets", realization.get(ra0, "Assets").account)
        self.assertEqual("Assets:US:Bank", realization.get(ra0, "Assets:US:Bank").account)
        self.assertEqual(
...
class TestRealization(unittest.TestCase):
    ...
    def test_postings_by_account(self, entries, errors, _):
        ...
        self.assertEqual(
            [data.Open, data.TxnPosting], list(map(type, txn_postings_map["Assets:Cash"]))
        )

        self.assertEqual(
            [data.Open, data.TxnPosting, data.TxnPosting],
    ...
    def test_realize_min_accoumts(self):
        real_account = realization.realize([], account_types.DEFAULT_ACCOUNT_TYPES)
        self.assertTrue(isinstance(real_account, realization.RealAccount))
        self.assertEqual(real_account.account, "")
        self.assertEqual(len(real_account), 5)
        self.assertEqual(set(account_types.DEFAULT_ACCOUNT_TYPES), real_account.keys())
...
class TestRealFilter(unittest.TestCase):
    ...
    def test_filter_almost_all(self):
        ...
        self.assertEqual(
            {"Assets", "Liabilities:US:Bank"},
            set(ra0.account for ra0 in realization.iter_children(real_copy, True)),
        )
    ...
    def test_filter_misc(self):
        ...
        def even(real_account):
            return (
                not real_account.balance.is_empty()
                and real_account.balance.get_currency_units("NOK").number % 2 == 0
            )
...
class TestRealOther(test_utils.TestCase):
    ...
    def test_iterate_with_balance(self, entries, _, __):
        ...
        def simplify_rtuple(rtuple):
            return [
                (type(entry), len(postings), str(change), str(balance))
...
class TestComputeBalance(unittest.TestCase):
    ...
    def test_compute_postings_balance(self, entries, _, __):
        ...
        expected_balance.add_amount(
            A("32 HOOL"),
            position.Cost(D("45.203"), "USD", datetime.date(2014, 5, 30), None),
        )
