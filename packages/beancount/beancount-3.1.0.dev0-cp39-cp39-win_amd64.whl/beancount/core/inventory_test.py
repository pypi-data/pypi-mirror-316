...
def setUp(module):
    invariants.instrument_invariants(
        Inventory, inventory.check_invariants, inventory.check_invariants
    )
...
class TestInventory(unittest.TestCase):
    def test_from_string(self):
        ...
        self.assertEqual(Inventory([Position(A("10 USD"))]), inv)

        inv = inventory.from_string(" 10.00  USD ")
        self.assertEqual(Inventory([Position(A("10 USD"))]), inv)

        inv = inventory.from_string("1 USD, 2 CAD")
        self.assertEqual(Inventory([Position(A("1 USD")), Position(A("2 CAD"))]), inv)

        inv = inventory.from_string("2.2 HOOL {532.43 USD}, 3.413 EUR")
        self.assertEqual(
            Inventory(
                [
                    Position(A("2.2 HOOL"), Cost(D("532.43"), "USD", None, None)),
    ...
    def test_is_small__dict(self):
        ...
        for inv in test_inv, -test_inv:
            ...
            self.assertFalse(
                inv.is_small({"JPY": D("0.02999999999999"), "USD": D("0.003")})
            )
            ...
            self.assertTrue(
                inv.is_small({"JPY": D("0.05"), "USD": D("0.005"), "CAD": D("0.0005")})
            )
    ...
    def test_units1(self):
        ...
        inv = I("40.50 JPY, 40.51 USD {1.01 CAD}, 40.52 CAD")
        self.assertEqual(
            inv.reduce(convert.get_units), I("40.50 JPY, 40.51 USD, 40.52 CAD")
        )
    ...
    def test_units(self):
        inv = Inventory(self.POSITIONS_ALL_KINDS + [P("50.00 CAD")])
    ...
    def test_average(self):
        ...
        inv = I("2 HOOL {500 USD}")
        inv.add_amount(A("-2 HOOL"), Cost(D("500"), "USD", datetime.date(2000, 1, 1), None))
        self.assertEqual(inv.average(), I(""))
    ...
    def test_add_amount__withlots(self):
        ...
        position_, _ = inv.add_amount(A("-12 HOOL"), Cost(D("700"), "USD", None, None))
    ...
    def test_add_amount__allow_negative(self):
        ...
        self.assertIsNone(position_)
        position_, _ = inv.add_amount(
            A("-11 USD"), Cost(D("1.10"), "CAD", date(2012, 1, 1), None)
        )
        self.assertIsNone(position_)
