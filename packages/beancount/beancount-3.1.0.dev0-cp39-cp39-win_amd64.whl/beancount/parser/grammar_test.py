...
class TestPushPopMeta(unittest.TestCase):
    ...
    def test_pushmeta_invalid_pop(self, entries, errors, _):
        ...
        """
        self.assertEqual(1, len(errors))
        self.assertRegex(errors[0].message, "Attempting to pop absent metadata key")
...
class TestParserOptions(unittest.TestCase):
    ...
    def test_option_list_value(self, entries, errors, options_map):
        ...
        self.assertEqual(["/path/docs/a", "/path/docs/b", "/path/docs/c"], documents)
...
class TestParserPlugin(unittest.TestCase):
    ...
    def test_plugin(self, entries, errors, options_map):
        ...
        """
        self.assertFalse(errors)
        self.assertEqual([("beancount.plugin.unrealized", None)], options_map["plugin"])
    ...
    def test_plugin_with_config(self, entries, errors, options_map):
        ...
        self.assertFalse(errors)
        self.assertEqual(
            [("beancount.plugin.unrealized", "Unrealized")], options_map["plugin"]
        )
...
class TestTransactions(unittest.TestCase):
    ...
    def test_tags_after_first_line(self, entries, errors, _):
        ...
        check_list(self, errors, [])
        self.assertEqual(
            {"basetag", "tag1", "tag2", "tag3", "tag4", "tag6"}, entries[0].tags
        )
        self.assertEqual(
            {"baselink", "link1", "link2", "link3", "link4", "link5"}, entries[0].links
        )
...
class TestParseLots(unittest.TestCase):
    ...
    def test_cost_label(self, entries, errors, _):
        ...
        self.assertEqual(A("20 AAPL"), posting.units)
        self.assertEqual(
            CostSpec(MISSING, None, MISSING, None, "d82d55a0dbe8", False), posting.cost
        )
    ...
    def test_cost_repeated_label(self, entries, errors, _):
        ...
        self.assertEqual(1, len(errors))
        self.assertTrue(
            any(re.search("Duplicate label", error.message) for error in errors)
        )
    ...
    def test_cost_repeated_merge(self, entries, errors, _):
        ...
        self.assertTrue(
            any(re.search("Duplicate merge", error.message) for error in errors)
        )
    ...
    def test_cost_both_costs(self, entries, errors, _):
        ...
        posting = entries[0].postings[0]
        self.assertEqual(
            CostSpec(D("45.23"), D("9.95"), "USD", None, None, False), posting.cost
        )
    ...
    def test_cost_total_cost_only(self, entries, errors, _):
        ...
        posting = entries[0].postings[0]
        self.assertEqual(
            CostSpec(MISSING, D("9.95"), "USD", None, None, False), posting.cost
        )
    ...
    def test_cost_total_empty_total(self, entries, errors, _):
        ...
        self.assertEqual(A("20 AAPL"), posting.units)
        self.assertEqual(
            CostSpec(D("45.23"), MISSING, "USD", None, None, False), posting.cost
        )
...
class TestTotalsAndSigns(unittest.TestCase):
    ...
    def test_total_cost__invalid(self, entries, errors, _):
        ...
        self.assertEqual(1, len(errors))
        self.assertRegex(
            errors[0].message, "Per-unit cost may not be specified using total cost syntax"
        )
...
class TestMetaData(unittest.TestCase):
    ...
    def test_metadata_transaction__middle(self, entries, errors, _):
        ...
        self.assertEqual(1, len(entries))
        self.assertEqual(
            {"test": "Something"}, self.strip_meta(entries[0].postings[0].meta)
        )
    ...
    def test_metadata_transaction__end(self, entries, errors, _):
        ...
        self.assertEqual(1, len(entries))
        self.assertEqual(
            {"test": "Something"}, self.strip_meta(entries[0].postings[1].meta)
        )
    ...
    def test_metadata_transaction__many(self, entries, errors, _):
        ...
        self.assertEqual(
            {"test2": "has", "test3": "to"}, self.strip_meta(entries[0].postings[0].meta)
        )
        self.assertEqual(
    ...
    def test_metadata_transaction__repeated(self, entries, errors, _):
        ...
        self.assertEqual({"test": "Bananas"}, self.strip_meta(entries[0].postings[0].meta))
        self.assertEqual(3, len(errors))
        self.assertTrue(
            all(re.search("Duplicate.*metadata field", error.message) for error in errors)
        )
    ...
    def test_metadata_empty(self, entries, errors, _):
        ...
        self.assertEqual(
            {"oranges", "bananas", "filename", "lineno"}, entries[0].meta.keys()
        )
...
class TestArithmetic(unittest.TestCase):
    ...
    def test_number_expr__groups(self, entries, errors, _):
        ...
        self.assertEqual(1, len(entries))
        self.assertListEqual(
            [D("-4"), D("-2")], [posting.units.number for posting in entries[0].postings]
        )
    ...
    def test_number_expr__different_places(self, entries, errors, _):
        ...
        self.assertEqual(D("252.021"), entries[0].postings[0].cost.number_per)
        self.assertEqual(None, entries[0].postings[0].cost.number_total)
    ...
    * Grammar Builder Exception: A grammar rule is reduced successfully, a
    ...
    def test_lexer_errors_in_postings(self):
        ...
        for txn_string in txn_strings:
            ...
            self.assertEqual(1, len(entries))
            self.assertEqual(1, len(errors))
            self.assertRegex(errors[0].message, "(Invalid token|unexpected RPAREN)")
...
class TestIncompleteInputs(cmptest.TestCase):
    ...
    def test_cost_missing_number_total(self, entries, _, options_map):
        ...
        posting = entries[-1].postings[0]
        self.assertEqual(
            CostSpec(D("150"), MISSING, "USD", None, None, False), posting.cost
        )
    ...
    def test_cost_empty_with_other(self, entries, _, options_map):
        ...
        self.assertEqual(
            CostSpec(
                MISSING, None, MISSING, datetime.date(2015, 9, 21), "blablabla", False
            ),
            posting.cost,
        )
    ...
    def test_cost_average_missing_basis(self, entries, _, options_map):
        ...
        posting = entries[-1].postings[0]
        self.assertEqual(
            CostSpec(MISSING, None, MISSING, datetime.date(2015, 9, 21), "blablabla", True),
            posting.cost,
        )
    ...
    def test_cost_average_with_other(self, entries, _, options_map):
        ...
        self.assertEqual(
            CostSpec(
                D("100.00"), None, "CAD", datetime.date(2015, 9, 21), "blablabla", True
            ),
            posting.cost,
        )
