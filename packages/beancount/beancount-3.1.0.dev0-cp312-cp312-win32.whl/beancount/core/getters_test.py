...
class TestGetters(unittest.TestCase):
    ...
    def test_get_accounts_use_map(self):
        ...
        self.assertEqual(
            {
                "Expenses:Coffee": datetime.date(2012, 2, 1),
        ...
        self.assertEqual(
            {
                "Expenses:Coffee": datetime.date(2013, 6, 22),
    ...
    def test_get_entry_accounts(self):
        ...
        accounts = getters.get_entry_accounts(
            next(entry for entry in entries if isinstance(entry, data.Transaction))
        )
        self.assertEqual(
            {"Assets:US:Cash", "Expenses:Grocery", "Expenses:Restaurant"}, accounts
        )
    ...
    def test_get_dict_accounts(self):
        ...
        self.assertNotEqual(getters.get_dict_accounts(account_names), account_dict)
    ...
    def test_get_commodity_directives(self):
        ...
        self.assertTrue(
            all(isinstance(value, data.Commodity) for value in commodities.values())
        )
    ...
    def test_get_values_meta__multi(self):
        ...
        self.assertEqual(
            {"HOOL": ("Hooli Corp.", "NYSE:HOOLI"), "PIPA": ("Pied Piper", None)}, values
        )
