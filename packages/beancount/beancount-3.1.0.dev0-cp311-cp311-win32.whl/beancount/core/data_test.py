...
class TestData(unittest.TestCase):
    ...
    def test_create_simple_posting(self):
        ...
        posting = data.create_simple_posting(entry, "Assets:Bank:Checking", "123.45", "USD")
    ...
    def create_sort_data(self):
        ...
        for entry in entries:
            if isinstance(entry, data.Transaction):
                data.create_simple_posting(entry, "Assets:Bank:Checking", "123.45", "USD")
    ...
    def check_sorted(self, entries):
        self.assertEqual(
            ...
            list(map(type, entries)),
        )

        self.assertEqual(
            [900, 1002, 1001, 1008, 1009, 1000, 1100],
            [entry.meta["lineno"] for entry in entries],
        )
    ...
    def test_posting_sortkey(self):
        ...
        txn_postings = [
            (
                data.TxnPosting(entry, entry.postings[0])
                if isinstance(entry, data.Transaction)
                else entry
            )
            for entry in entries
        ...
        self.assertEqual(
            [900, 1002, 1001, 1008, 1009, 1000, 1100],
            [entry.meta["lineno"] for entry in map(data.get_entry, sorted_txn_postings)],
        )
    ...
    def test_filter_txns(self):
        ...
        self.assertTrue(all(isinstance(txn, data.Transaction) for txn in txns))
    ...
    def test_find_closest(self):
        ...
        # Try an exact match.
        self.assertTrue(data.find_closest(entries, "/tmp/apples.beancount", 105) is entry3)

        # Try a bit after.
        self.assertTrue(data.find_closest(entries, "/tmp/apples.beancount", 107) is entry3)

        # Try way closer to the next one.
        self.assertTrue(data.find_closest(entries, "/tmp/apples.beancount", 199) is entry3)

        # Get the next one.
        self.assertTrue(data.find_closest(entries, "/tmp/apples.beancount", 201) is entry1)

        # Get from the other file.
        self.assertTrue(data.find_closest(entries, "/tmp/bananas.beancount", 300) is entry2)
    ...
    def test_remove_account_postings(self):
        ...
        entry3 = data.Transaction(meta, date, FLAG, None, "Something", None, None, [])
    ...
    def test_iter_entry_dates(self):
        prototype = data.Transaction(
            data.new_metadata("misc", 200), None, "*", None, "", None, None, []
        )
        dates = [
        ...
        self.assertEqual(
            ...
            [
                entry.date
                for entry in data.iter_entry_dates(
                    entries, datetime.date(2016, 1, 14), datetime.date(2016, 1, 14)
                )
            ],
        )
        # Same date, absent.
        self.assertEqual(
            ...
            [
                entry.date
                for entry in data.iter_entry_dates(
                    entries, datetime.date(2016, 1, 12), datetime.date(2016, 1, 12)
                )
            ],
        )
        # Both dates exist.
        self.assertEqual(
            [datetime.date(2016, 1, 11), datetime.date(2016, 1, 14)],
            [
                entry.date
                for entry in data.iter_entry_dates(
                    entries, datetime.date(2016, 1, 11), datetime.date(2016, 1, 15)
                )
            ],
        )
        # First doesn't exist.
        self.assertEqual(
            ...
            [
                entry.date
                for entry in data.iter_entry_dates(
                    entries, datetime.date(2016, 1, 12), datetime.date(2016, 1, 15)
                )
            ],
        )
        ...
        self.assertEqual(
            ...
            [
                entry.date
                for entry in data.iter_entry_dates(
                    entries, datetime.date(2016, 1, 11), datetime.date(2016, 1, 13)
                )
            ],
        )
        # Neither exist.
        self.assertEqual(
            ...
            [
                entry.date
                for entry in data.iter_entry_dates(
                    entries, datetime.date(2016, 1, 13), datetime.date(2016, 1, 17)
                )
            ],
        )
        # Before.
        ...
        self.assertEqual(
            ...
            [
                entry.date
                for entry in data.iter_entry_dates(
                    entries, datetime.date(2016, 1, 21), datetime.date(2016, 1, 30)
                )
            ],
        )
        # Around.
