...
class TestPositionConversions(unittest.TestCase):
    ...
    def test_units(self):
        units = A("100 HOOL")
        self.assertEqual(
            units, convert.get_units(self._pos(units, Cost(D("514.00"), "USD", None, None)))
        )
    ...
    def test_cost__empty(self):
        self.assertEqual(A("100 HOOL"), convert.get_cost(self._pos(A("100 HOOL"), None)))

    def test_cost__not_empty(self):
        self.assertEqual(
            A("51400.00 USD"),
            convert.get_cost(
                self._pos(A("100 HOOL"), Cost(D("514.00"), "USD", None, None))
            ),
        )
    ...
    def test_cost__missing(self):
        self.assertEqual(
            A("100 HOOL"),
            convert.get_cost(self._pos(A("100 HOOL"), Cost(MISSING, "USD", None, None))),
        )
    ...
    def test_weight__no_cost(self):
        self.assertEqual(A("100 HOOL"), convert.get_weight(self._pos(A("100 HOOL"), None)))
    ...
    def test_value__no_currency(self):
        ...
        self.assertEqual(A("100 HOOL"), convert.get_value(pos, self.PRICE_MAP_HIT))
    ...
    def test_convert_position__success(self):
        pos = self._pos(A("100 HOOL"), Cost(D("514.00"), "USD", None, None))
        self.assertEqual(
            A("53000.00 USD"), convert.convert_position(pos, "USD", self.PRICE_MAP_HIT)
        )

    def test_convert_position__miss_but_same_currency(self):
        pos = self._pos(A("100 HOOL"), Cost(D("514.00"), "USD", None, None))
        self.assertEqual(
            A("100 HOOL"), convert.convert_position(pos, "USD", self.PRICE_MAP_EMPTY)
        )

    def test_convert_position__miss_and_miss_rate_to_rate(self):
        pos = self._pos(A("100 HOOL"), Cost(D("514.00"), "USD", None, None))
        self.assertEqual(
            A("100 HOOL"), convert.convert_position(pos, "JPY", self.PRICE_MAP_HIT)
        )
    ...
    def test_convert_position__miss_and_miss_value_rate(self):
        pos = self._pos(A("100 HOOL"), Cost(D("514.00"), "USD", None, None))
        self.assertEqual(
            A("100 HOOL"), convert.convert_position(pos, "CAD", self.PRICE_MAP_RATEONLY)
        )

    def test_convert_position__miss_and_miss_both(self):
        pos = self._pos(A("100 HOOL"), Cost(D("514.00"), "USD", None, None))
        self.assertEqual(
            A("100 HOOL"), convert.convert_position(pos, "CAD", self.PRICE_MAP_EMPTY)
        )

    def test_convert_position__miss_and_success_on_implieds(self):
        pos = self._pos(A("100 HOOL"), Cost(D("514.00"), "USD", None, None))
        self.assertEqual(
            A("63600.00 CAD"), convert.convert_position(pos, "CAD", self.PRICE_MAP_HIT)
        )
    ...
    def test_convert_amount__fail(self):
        amt = A("127.00 USD")
        self.assertEqual(amt, convert.convert_amount(amt, "CAD", self.PRICE_MAP_EMPTY))

    def test_convert_amount__success(self):
        amt = A("127.00 USD")
        self.assertEqual(
            A("152.40 CAD"), convert.convert_amount(amt, "CAD", self.PRICE_MAP_HIT)
        )

    def test_convert_amount__noop(self):
        amt = A("127.00 USD")
        self.assertEqual(amt, convert.convert_amount(amt, "USD", self.PRICE_MAP_EMPTY))
        self.assertEqual(amt, convert.convert_amount(amt, "USD", self.PRICE_MAP_HIT))
    ...
    def test_convert_amount_with_date(self, entries, _, __):
        ...
        for date, exp_amount in [
            ...
            self.assertEqual(
                exp_amount, convert.convert_amount(A("100 USD"), "CAD", price_map, date)
            )
...
class TestPostingConversions(TestPositionConversions):
    ...
    def test_weight_with_cost_and_price(self):
        self.assertEqual(
            A("51400.00 USD"),
            convert.get_weight(
                self._pos(A("100 HOOL"), Cost(D("514.00"), "USD", A("530.00 USD"), None))
            ),
        )
    ...
    def test_weight_with_only_price(self):
        self.assertEqual(
            A("53000.00 USD"),
            convert.get_weight(self._pos(A("100 HOOL"), None, A("530.00 USD"))),
        )
