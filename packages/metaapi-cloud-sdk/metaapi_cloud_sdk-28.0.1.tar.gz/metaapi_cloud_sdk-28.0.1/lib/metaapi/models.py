...
from typing import List, Optional, Generator, Any, Union, Coroutine
...
async def async_race(*args: Union[Generator[Any, None, Any], Coroutine[Any, Any, Any], Future]):
    ...
    for arg in args:
        if isinstance(arg, Future):
            ...
            task_set.add(loop.create_task(arg))

    done_first, pending = await asyncio.wait(task_set, return_when=asyncio.FIRST_COMPLETED)
...
def date(date_time: Union[str, float, int]) -> datetime:
...
def convert_iso_time_to_date(packet):
    for field in packet:
        value = packet[field]
        if isinstance(value, str) and re.search('time|Time', field) and not re.search('brokerTime|BrokerTime', field):
...
def format_error(err: Union[Exception, Any]):
    ...
    error = {
        ...
        'message': err if isinstance(err, str) or err is None else (err.args[0] if len(err.args) else None),
    }
...
def string_format_error(err: Union[Exception, Any]):
...
class HashEncoder(json.JSONEncoder):
    def iterencode(self, object, _one_shot=False):
        # TODO: maybe rename encoders to something more suitable
        encoders = [
            {'class': bool, 'encoder': self._encode_bool},
            {'class': float, 'encoder': self._encode_float},
            {'class': int, 'encoder': self._encode_int},
            {'class': list, 'encoder': self._encode_list},
            {'class': datetime, 'encoder': self._encode_datetime},
            {'class': dict, 'encoder': self._encode_dict},
        ]

        for encoder in encoders:
            if isinstance(object, encoder['class']):
                result = encoder['encoder'](object)
        ...
        for chunk in json.JSONEncoder.iterencode(self, object):
            yield chunk

    def _encode_bool(self, value: bool) -> Union[str, Generator[str, Any, None]]:
        return json.dumps(value)

    def _encode_float(self, value: float) -> Union[str, Generator[str, Any, None]]:
        return json.dumps(value)

    def _encode_int(self, value: int) -> Union[str, Generator[str, Any, None]]:
        return json.dumps(value)

    def _encode_list(self, value: List) -> Union[str, Generator[str, Any, None]]:
        last_index = len(value) - 1
        yield "["
        for i, o in enumerate(value):
            for chunk in HashEncoder.iterencode(self, o):
                yield chunk
            if i != last_index:
                yield ","
        yield "]"

    def _encode_datetime(self, value: datetime) -> Union[str, Generator[str, Any, None]]:
        return '"' + format_date(value) + '"'

    def _encode_dict(self, value: dict) -> Union[str, Generator[str, Any, None]]:
        last_index = len(value) - 1
        yield '{'
        i = 0
        for key, value in value.items():
            yield '"' + key + '":'
            for chunk in HashEncoder.iterencode(self, value):
                yield chunk
            if i != last_index:
                yield ","
            i += 1
...
def get_hash_encoder(account_type: str, integer_keys: List[str]):
    """Returns hash encoder for provided account type."""

    def dict_iterator(self, value: dict):
        last_index = len(value) - 1
        yield '{'
        i = 0
        for key, value in value.items():
    ...
    class G1Encoder(HashEncoder):
        def _encode_float(self, value: float):
            parts = format(decimal.Context(prec=8).create_decimal(repr(value)), 'f').split('.')
            fraction = parts[1] if len(parts) == 2 else ''

            yield parts[0]
            yield '.'
            yield fraction.ljust(8, "0")

        def _encode_int(self, value: int):
            return self._encode_float(value)

        def _encode_dict(self, value: dict):
            return dict_iterator(self, value)

    class G2Encoder(HashEncoder):
        def _encode_float(self, value: float):
            if int(value) == value:
                yield json.dumps(int(value))
            else:
                yield format(decimal.Context(prec=8).create_decimal(repr(value)), 'f')

        def _encode_int(self, value: int):
            yield json.dumps(round(value, 8))

        def _encode_dict(self, value: dict):
            return dict_iterator(self, value)

    hash_encoder_by_account_type = {'cloud-g1': G1Encoder, 'cloud-g2': G2Encoder}

    encoder = hash_encoder_by_account_type.get(account_type)
...
class MetatraderAccountInformation(TypedDict, total=False):
    ...
    type: str
    """Account type, one of ACCOUNT_TRADE_MODE_DEMO, ACCOUNT_TRADE_MODE_CONTEST, ACCOUNT_TRADE_MODE_REAL."""
...
class MetatraderSymbolSpecification(TypedDict, total=False):
    ...
    stopsLevel: float
    """minimal distance from the current market price to stop price (SL, TP, open price) in points.
    The SL, TP, open price can not be placed closer to market price than this distance."""
    freezeLevel: float
    """distance in points from market price to pending order open price to freeze updates on the pending order.
    If the distance between order open price and current market price is less than this value,
...
class RefreshedQuotes(TypedDict, total=False):
    """Refreshed quotes and some of the account information."""

    quotes: List[MetatraderSymbolPrice]
    """Refreshed quotes."""
    balance: float
...
class RefreshTerminalStateFlag(TypedDict, total=False):
    """Refresh terminal state option flag for some requests."""

    refreshTerminalState: Optional[bool]
    """Whether to refresh terminal state before retrieving the data, slowing down the request."""


class GetAccountInformationOptions(RefreshTerminalStateFlag):
    """Options for `getAccountInformation` rpc request."""

    pass


class GetPositionsOptions(RefreshTerminalStateFlag):
    """Options for `getPositions` rpc request."""

    pass


class GetPositionOptions(RefreshTerminalStateFlag):
    """Options for `getPosition` rpc request."""

    pass


class GetOrdersOptions(RefreshTerminalStateFlag):
    """Options for `getOrders` rpc request."""

    pass


class GetOrderOptions(RefreshTerminalStateFlag):
