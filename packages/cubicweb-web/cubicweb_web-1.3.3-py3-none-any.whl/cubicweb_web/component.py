...
class NavigationComponent(Component):
    ...
    cw_property_defs = {
        _("visible"): dict(
            type="Boolean", default=True, help=_("display the component or not")
        ),
    }
    ...
    start_param = "__start"
    stop_param = "__stop"
    page_link_templ = '<span class="slice"><a href="%s" title="%s">%s</a></span>'
    selected_page_link_templ = (
        '<span class="selectedSlice"><a href="%s" title="%s">%s</a></span>'
    )
    ...
    def no_previous_page_link(self):
        return '<img src="%s" alt="%s" class="prevnext_nogo"/>' % (
            self.prev_icon_url,
            self._cw._("there is no previous page"),
        )
    ...
    def no_next_page_link(self):
        return '<img src="%s" alt="%s" class="prevnext_nogo"/>' % (
            self.next_icon_url,
            self._cw._("there is no next page"),
        )

    @property
    def no_content_prev_link(self):
        return '<img src="%s" alt="%s" class="prevnext"/>' % (
    ...
    def no_content_next_link(self):
        return '<img src="%s" alt="%s" class="prevnext"/>' % (
            self.next_icon_url,
            self._cw._("no content next link"),
        )
...
class LayoutableMixIn:
    ...
    def layout_select_args(self):
        args = dict(rset=self.cw_rset, row=self.cw_row, col=self.cw_col, view=self)
...
class CtxComponent(LayoutableMixIn, AppObject):
    ...
    * other: 'ctxtoolbar'
    ...
    cw_property_defs = {
        _("visible"): dict(
            type="Boolean", default=True, help=_("display the box or not")
        ),
        _("order"): dict(type="Int", default=99, help=_("display order of the box")),
        _("context"): dict(
            type="String",
            default="left",
            vocabulary=(
    ...
    def render_items(self, w, items=None, klass="boxListing"):
        if items is None:
            items = self.items
        assert items
        w('<ul class="%s">' % klass)
...
class EntityCtxComponent(CtxComponent):
    ...
    def lazy_view_holder(self, w, entity, oid, registry="views"):
        ...
        return self._cw.ajax_replace_url(holderid, fname="render", arg=args, **form)
...
class RQLCtxComponent(CtxComponent):
    ...
    def render_body(self, w):
        ...
        if len(rset[0]) == 2:
            ...
            items = [self.link(e.dc_title(), e.absolute_url()) for e in rset.entities()]
...
class EditRelationMixIn(ReloadableMixIn):
    def box_item(self, entity, etarget, fname, label):
        ...
        jscall = js.cw.utils.callAjaxFuncThenReload(
            fname, self.rtype, args["subject"], args["object"]
        )
        return '[<a href="javascript: %s" class="action">%s</a>] %s' % (
            xml_escape(str(jscall)),
            label,
    ...
    def related_boxitems(self, entity):
        return [
            self.box_item(entity, etarget, "delete_relation", "-")
    ...
    def unrelated_boxitems(self, entity):
        return [
            self.box_item(entity, etarget, "add_relation", "+")
    ...
    def unrelated_entities(self, entity):
        ...
        """
        skip = set(
            str(e.eid) for e in entity.related(self.rtype, role(self), entities=True)
        )
        skip.add(None)
        ...
        form = self._cw.vreg["forms"].select(
            "edition", self._cw, rset=self.cw_rset, row=self.cw_row or 0
        )
        field = form.field_by_name(self.rtype, role(self), entity.e_schema)
        for __, eid in field.vocabulary(form):
            if eid not in skip:
...
class EditRelationCtxComponent(EditRelationMixIn, EntityCtxComponent):
    ...
    def render_title(self, w):
        w(display_name(self._cw, self.rtype, role(self), context=self.entity.cw_etype))
    ...
    def render_body(self, w):
        ...
        if related and unrelated:
            self.items.append('<hr class="boxSeparator"/>')
...
class AjaxEditRelationCtxComponent(EntityCtxComponent):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.rdef = self.entity.e_schema.relation_definition(
            self.rtype, self.role, self.target_etype
        )

    def render_title(self, w):
        w(
            self.rdef.relation_type.display_name(
                self._cw, self.role, context=self.entity.cw_etype
            )
        )
    ...
    def render_body(self, w):
        ...
        if related:
            maydel = None
            w('<table class="ajaxEditRelationTable">')
            for rentity in related.entities():
                ...
                if maydel:
                    ...
                    w(
                        '<tr><td class="dellink">[<a href="javascript: %s">-</a>]</td>'
                        '<td class="entity"> %s</td></tr>'
                        % (xml_escape(jscall), subview)
                    )
                else:
                    w('<tr><td class="entity">%s</td></tr>' % (subview))
            w("</table>")
        else:
        ...
        if mayadd:
            ...
            w("<table><tr><td>")
            jscall = str(
                js.ajaxBoxShowSelector(
                    self.__regid__,
                    entity.eid,
                    self.fname_vocabulary,
                    self.fname_validate,
                    self.added_msg and _(self.added_msg),
                    _(stdmsgs.BUTTON_OK[0]),
                    _(stdmsgs.BUTTON_CANCEL[0]),
                    multiple and self.separator,
                )
            )
            w(
                '<a class="button sglink" href="javascript: %s">%s</a>'
                % (
                    xml_escape(jscall),
            ...
            w("</td><td>")
            w('<div id="%sHolder"></div>' % self.domid)
            w("</td></tr></table>")
...
class EntityVComponent(Component, metaclass=class_deprecated):
    ...
    * a context (currently 'header' or 'footer')
    ...
    __deprecation_warning__ = (
        "[3.10] *VComponent classes are deprecated, use *CtxComponent instead (%(cls)s)"
    )
    ...
    cw_property_defs = {
        ...
        _("order"): dict(
            type="Int", default=99, help=_("display order of the component")
        ),
        _("context"): dict(
            type="String",
            default="navtop",
...
class RelatedObjectsVComponent(EntityVComponent):
    ...
    def cell_call(self, row, col, view=None):
        ...
        if not rset.rowcount:
            return
        self.w('<div class="%s">', self.cssclass)
