...
from cubicweb_web import Redirect, formwidgets as wdgs, facet, action
from cubicweb_web.views import add_etype_button
...
class CWSourceMainTab(tabs.PrimaryTab):
    ...
    def render_entity_attributes(self, entity):
        ...
        try:
            ...
            if hostconfig:
                self.w("<h3>%s</h3>", self._cw._("CWSourceHostConfig_plural"))
                self._cw.view(
                    "table",
                    hostconfig,
                    w=self.w,
...
class CWSourceImportsTab(EntityView):
    ...
    __select__ = is_instance("CWSource") & has_related_entities(
        "cw_import_of", "object"
    )
...
class CWSourceSyncView(EntityView):
    ...
    def entity_call(self, entity):
        ...
        msg = self._cw._(
            "Synchronization has been requested, refresh this page in a few " "minutes."
        )
...
class CWSourcesManagementView(StartupView):
    ...
    rql = (
        "Any S,ST,SP,SD,SN ORDERBY SN WHERE S is CWSource, S name SN, S type ST, "
...
class CWSourcesTable(tableview.EntityTableView):
    ...
    class LatestImportColRenderer(tableview.EntityTableColRenderer):
        def render_cell(self, w, rownum):
            ...
            rset = self._cw.execute(
                "Any X,XS,XST ORDERBY XST DESC LIMIT 1 WHERE "
                "X cw_import_of S, S eid %(s)s, X status XS, "
                "X start_timestamp XST",
                {"s": entity.eid},
            )
            if rset:
    ...
    column_renderers = {
        ...
        "latest_import": LatestImportColRenderer(
            header=_("latest import"), sortable=False
        ),
    }
...
class LogTableLayout(tableview.TableLayout):
    ...
    def render_table(self, w, actions, paginate):
        ...
        if default_level != "Debug":
            self._cw.add_onload(
                '$("select.log_filter").val("%s").change();'
                % self._cw.form.get("logLevel", default_level)
            )
        w('\n<form action="#"><fieldset>')
        w("<label>%s</label>" % self._cw._("Message threshold"))
        w(
            '<select class="log_filter" onchange="filterLog(\'%s\', this.options[this.selectedIndex].value)">'
            % self.view.domid
        )
        for level in ("DEBUG", "INFO", "WARNING", "ERROR", "FATAL"):
            w(
                '<option value="%s">%s</option>'
                % (level.capitalize(), self._cw._(level))
            )
        w("</select>")
        w("</fieldset></form>")
        super().render_table(w, actions, paginate)

    def table_attributes(self):
        attrs = super().table_attributes()
        attrs["id"] = "table" + self.view.domid
        return attrs

    def row_attributes(self, rownum):
        attrs = super().row_attributes(rownum)
    ...
    def cell_attributes(self, rownum, colnum, colid):
        attrs = super().cell_attributes(rownum, colnum, colid)
...
class LogTable(pyviews.PyValTableView):
    ...
    class SeverityRenderer(pyviews.PyValTableColRenderer):
        def render_cell(self, w, rownum):
            ...
            w(
                '<a class="internallink" href="javascript:;" title="%(title)s" '
                """onclick="document.location.hash='%(msg_id)s';">&#182;</a>"""
                "&#160;%(severity)s"
                % {
    ...
    class MessageRenderer(pyviews.PyValTableColRenderer):
        ...
        def render_cell(self, w, rownum):
            ...
            if len(lines) <= self.snip_over:
                w('<pre class="rawtext">%s</pre>' % msg)
            else:
                # The make_uid argument has no specific meaning here.
                div_snip_id = make_uid("log_snip_")
                div_full_id = make_uid("log_full_")
                ...
                snip = "\n".join(
                    (
                        ...
                        lines[1],
                        "  ...",
                        "    %i more lines [double click to expand]" % (len(lines) - 4),
                        "  ...",
                        lines[-2],
                        lines[-1],
                    )
                ...
                divs = (
                    (div_snip_id, snip, "expand", "class='collapsed'"),
                    (div_full_id, msg, "collapse", "class='hidden'"),
                )
                for div_id, content, button, h_class in divs:
                    text = self._cw._(button)
                    js = "toggleVisibility('%s'); toggleVisibility('%s');" % divs_id
                    w(f'<div id="{div_id}" {h_class}>')
                    w(
                        '<pre class="raw_test" ondblclick="javascript: %s" '
