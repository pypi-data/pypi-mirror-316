...
from cubicweb_web import InvalidSession
from cubicweb_web.views import authentication
...
class AbstractSessionManager(RegistrableObject):
    ...
    def __init__(self, repo):
        ...
        interval = (self.session_time or 0) / 2.0
        if vreg.config.anonymous_user()[0] is not None:
            self.cleanup_anon_session_time = (
                vreg.config["cleanup-anonymous-session-time"] or 5 * 60
            )
            ...
            interval = self.cleanup_anon_session_time / 2.0
...
class InMemoryRepositorySessionManager(AbstractSessionManager):
    ...
    def get_session(self, req, sessionid):
        ...
        try:
            self.authmanager.validate_session(req, session)
    ...
    def _update_last_login_time(self, session):
        ...
        try:
            with session.new_cnx() as cnx:
                cnx.execute(
                    "SET X last_login_time NOW WHERE X eid %(x)s",
                    {"x": session.user.eid},
                )
                cnx.commit()
