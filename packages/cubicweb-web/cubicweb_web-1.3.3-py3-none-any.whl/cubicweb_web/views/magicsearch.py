...
from cubicweb import rqlsuggestions
...
def _get_approriate_translation(translations_found, eschema):
    ...
    existing_relations = set(list(eschema.subject_relations.values()))
...
def resolve_ambiguities(var_types, ambiguous_nodes, schema):
    ...
    for relation, (var_name, translations_found) in ambiguous_nodes.items():
        ...
        eschema = schema.entity_schema_for(vartype)
...
def trmap(config, schema, lang):
    try:
        ...
        assert lang in config.translations, f"{lang} {config.translations}"
...
class QueryTranslator(BaseQueryProcessor):
    ...
    def preprocess_query(self, uquery):
        ...
        translate_rql_tree(
            rqlst, trmap(self._cw.vreg.config, schema, self._cw.lang), schema
        )
...
class QSPreProcessor(BaseQueryProcessor):
    ...
    def _get_entity_type(self, word):
        ...
        try:
            return trmap(self._cw.vreg.config, self._cw.vreg.schema, self._cw.lang)[
                etype
            ]
        except KeyError:
    ...
    def _get_attribute_name(self, word, eschema):
        ...
        try:
            ...
            raise BadRQLQuery(
                f"{word} is not a valid attribute for {eschema} entity type"
            )
    ...
    def _one_word_query(self, word):
        ...
        try:
            ...
            return ("{} {}".format(etype, etype[0]),)

    def _complete_rql(self, searchstr, etype, rtype=None, var=None, searchattr=None):
        searchop = ""
        if "%" in searchstr:
            if rtype:
                possible_etypes = self._cw.vreg.schema.relation_schema_for(
                    rtype
                ).objects(etype)
            else:
                possible_etypes = [self._cw.vreg.schema.entity_schema_for(etype)]
        ...
        if var is None:
            var = etype[0]
        return f"{var} {searchattr} {searchop}%(text)s"
    ...
    def _two_words_query(self, word1, word2):
        ...
        if " has_text " in restriction:
            ...
            rql = "{} {} WHERE {}".format(etype, etype[0], restriction)
    ...
    def _three_words_query(self, word1, word2, word3):
        ...
        if not self._cw.vreg.schema.relation_schema_for(rtype).final:
    ...
    def _expand_shortcut(self, etype, rtype, searchstr):
        ...
        mainvar = etype[0]
        searchvar = mainvar + "1"
        restriction = self._complete_rql(searchstr, etype, rtype=rtype, var=searchvar)
        if " has_text " in restriction:
            rql = "%s %s ORDERBY FTIRANK(%s) DESC " "WHERE %s %s %s, %s" % (
                etype,
                mainvar,
                searchvar,
                mainvar,
                rtype,
                searchvar,  # P worksAt C
                restriction,
            )
        else:
            rql = "{} {} WHERE {} {} {}, {}".format(
...
class FullTextTranslator(BaseQueryProcessor):
    ...
    def preprocess_query(self, uquery):
        ...
        return "Any X ORDERBY FTIRANK(X) DESC WHERE X has_text %(text)s", {
            "text": uquery
        }
...
class MagicSearchComponent(Component):
    ...
    def __init__(self, req, rset=None):
        ...
        for processorcls in self._cw.vreg["components"]["magicsearch_processor"]:
            ...
            if processor.name is not None:
                assert processor.name not in self.by_name
    ...
    def process_query(self, uquery):
        ...
        try:
            ...
            return proc.process_query(uquery)
        raise BadRQLQuery(
            self._cw._("sorry, the server is unable to handle this query")
        )
...
class RQLSuggestionsBuilder(Component, metaclass=class_deprecated):
    """
    This component was previously used by the `rql_suggest` ajax function to
    provide completion to the search bar. It is now deprecated in favor of the
    cubicweb.rqlsuggestions.RQLSuggestionsBuilder class.

    .. automethod:: cubicweb_web.views.magicsearch.RQLSuggestionsBuilder.build_suggestions
    """

    __regid__ = "rql.suggestions"

    def build_suggestions(self, user_rql):
        builder = rqlsuggestions.RQLSuggestionsBuilder(self._cw)
        return builder.build_suggestions(user_rql)


@ajaxfunc(output_type="json")
def rql_suggest(self):
    """Provides completions for the RQL search bar. Unregister it to disable
    this feature.
    """
    rql_builder = rqlsuggestions.RQLSuggestionsBuilder(self._cw)
