...
from cubicweb_web.view import View, StartupView
...
class UndoHistoryView(StartupView):
    ...
    def call(self, **kwargs):
        ...
        if txs:
            self.w("<ul class='undo-transactions'>")
            for a_tx in txs:
                self.cell_call(a_tx)
            self.w("</ul>")
...
class UndoableTransactionView(View):
    ...
    def build_undo_link(self, txuuid, redirect_path=None, redirect_params=None):
        ...
        msg = "<span class='undo'>%s</span>" % tags.a(_("undo"), href=link_url)
        return msg

    def call(self, txuuid, public=True, redirect_path=None, redirect_params=None):
        _ = self._cw._
        ...
        try:
            ...
            user = None
        undo_url = self.build_undo_link(
            txuuid, redirect_path=redirect_path, redirect_params=redirect_params
        )
        txinfo_dict = dict(
            dt=self._cw.format_date(txinfo.datetime, time=True),
            user_eid=txinfo.user_eid,
            user=user and user.view("outofcontext") or _("undefined user"),
            txuuid=txuuid,
            undo_link=undo_url,
        )
        self.w(_("By %(user)s on %(dt)s [%(undo_link)s]"), txinfo_dict, escape=False)
...
class UndoableActionBaseView(View):
    ...
    def _build_relation_info(self, rtype, eid_from, eid_to):
        return dict(
            rtype=display_name(self._cw, rtype),
    ...
    def _build_entity_info(self, etype, eid, changes):
        return dict(
            etype=display_name(self._cw, etype),
...
class UndoableAddActionView(UndoableActionBaseView):
    ...
    def call(self, tx_action):
        _ = self._cw._
        self.w(
            _("Added relation : %(entity_from)s %(rtype)s %(entity_to)s"),
            self._build_relation_info(
                tx_action.rtype, tx_action.eid_from, tx_action.eid_to
            ),
            escape=False,
        )
...
class UndoableRemoveActionView(UndoableActionBaseView):
    ...
    def call(self, tx_action):
        _ = self._cw._
        self.w(
            _("Delete relation : %(entity_from)s %(rtype)s %(entity_to)s"),
            self._build_relation_info(
                tx_action.rtype, tx_action.eid_from, tx_action.eid_to
            ),
            escape=False,
