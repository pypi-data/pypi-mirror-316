...
from cubicweb_web import component, facet as facetbase
...
@objectify_predicate
def contextview_selector(cls, req, rset=None, row=None, col=None, view=None, **kwargs):
    if view:
        try:
...
def filter_hiddens(w, baserql, wdgs, **kwargs):
    ...
    for key, val in kwargs.items():
        w('<input type="hidden" name="%s" value="%s" />' % (key, xml_escape(val)))


class FacetFilterMixIn:
    ...
    def generate_form(
        ...
        **kwargs,
        ...
        baserql, wdgs = facets(
            self._cw, rset, context=self.__regid__, mainvar=mainvar, **kwargs
        )
        assert wdgs
        self._cw.add_js(self.needs_js)
        self._cw.add_css(self.needs_css)
        self._cw.html_headers.define_var(
            "facetLoadingMsg", self._cw._("facet-loading-msg")
        )
        vidargs = {}
        # We need to keep the mode while we are using facets
        # to keep the correct searchstate
        mode = self._cw.form.get("__mode")
        if mode:
            vidargs["__mode"] = mode
        facetargs = xml_escape(json_dumps([divid, vid, paginate, vidargs]))
        w(
            '<form id="%sForm" class="%s" method="post" action="" '
        ...
        w('<input type="submit" class="hidden" />')
        #
        w("</fieldset>\n")
        w("</form>\n")
...
class FilterBox(FacetFilterMixIn, component.CtxComponent):
    ...
    bk_linkbox_template = '<div class="facetTitle">%s</div>'
    ...
    def bookmark_link(self, rset):
        req = self._cw
        bk_path = "rql=%s" % req.url_quote(rset.printable_rql())
        if req.form.get("vid"):
            bk_path += "&vid=%s" % req.url_quote(req.form["vid"])
        bk_path = "view?" + bk_path
        bk_title = req._("my custom search")
        linkto = "bookmarked_by:%s:subject" % req.user.eid
        bkcls = req.vreg["etypes"].etype_class("Bookmark")
        bk_add_url = bkcls.cw_create_url(
            req, path=bk_path, title=bk_title, __linkto=linkto
        )
        bk_base_url = bkcls.cw_create_url(req, title=bk_title, __linkto=linkto)
        bk_link = '<a cubicweb:target="%s" id="facetBkLink" href="%s">%s</a>' % (
    ...
    def focus_link(self, rset):
        return self.bk_linkbox_template % tags.a(
            self._cw._("focus on this selection"), href=self._cw.url(), id="focusLink"
        )
...
class FilterTable(FacetFilterMixIn, AnyRsetView):
    ...
    average_perfacet_uncomputable_overhead = 0.3
    ...
    def per_facet_height_overhead(self):
        return (
            css_em_num_value(self._cw.vreg, "facet_MarginBottom", 0.2)
            + css_em_num_value(self._cw.vreg, "facet_Padding", 0.2)
