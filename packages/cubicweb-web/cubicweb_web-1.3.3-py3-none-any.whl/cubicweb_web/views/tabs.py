...
class LazyViewMixin:
  ...
  jQuery('#lazy-%(vid)s').bind('%(event)s', function(event) {
    ...
    def lazyview(
        ...
        self.w(
            '<div id="lazy-%s" cubicweb:loadurl="%s">',
            tabid,
            xml_escape(self._cw.build_url("ajax", **urlparams)),
        )
        if show_spinbox:
            # Don't use ``alt`` since image is a *visual* helper for ajax
            self.w(
                '<img style="display: none" src="%s" alt="" id="%s-hole"/>',
                xml_escape(self._cw.data_url("loading.gif")),
                tabid,
            )
        else:
            self.w('<div id="%s-hole"></div>', tabid)
        self.w(
            '<noscript><p>%s <a id="seo-%s" href="%s">%s</a></p></noscript>',
...
class TabsMixin(LazyViewMixin):
    ...
    def render_tabs(self, tabs, default, entity=None):
        ...
        self.w('<div id="entity-tabs-%s">', uid)
        self.w("<ul>")
        active_tab_idx = None
        for i, (tabid, domid, tabkwargs) in enumerate(tabs):
            self.w("<li>")
            self.w('<a href="#%s">', domid)
            self.w(tabkwargs.pop("label", self._cw._(tabid)))
            self.w("</a>")
            self.w("</li>")
            if domid == active_tab:
                active_tab_idx = i
...
"""
            ...
            self._cw.add_onload(
                "jQuery('#entity-tabs-%(uid)s').tabs({active: %(tabindex)s});"
...
class TabbedPrimaryView(TabsMixin, primary.PrimaryView):
    ...
    def render_entity(self, entity):
        ...
        self.w('<div class="tabbedprimary"></div>')
