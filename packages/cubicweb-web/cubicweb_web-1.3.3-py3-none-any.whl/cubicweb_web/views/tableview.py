...
.. autoclass:: cubicweb_web.views.tableview.RsetTableView
   :members:

.. autoclass:: cubicweb_web.views.tableview.EntityTableView
   :members:

.. autoclass:: cubicweb_web.views.pyviews.PyValTableView
   :members:

All those classes are rendered using a *layout*:

.. autoclass:: cubicweb_web.views.tableview.TableLayout
   :members:

There is by default only one table layout, using the 'table_layout' identifier,
that is referenced by table views
:attr:`cubicweb_web.views.tableview.TableMixIn.layout_id`.  If you want to
...
.. autoclass:: cubicweb_web.views.tableview.TableMixIn
   :members:
"""
...
from cubicweb_web import jsonize, component
from cubicweb_web.htmlwidgets import TableWidget, TableColumn, MenuWidget, PopupBoxMenu
    ...
    * `header_column_idx`, if not `None`, should be a colum index or a set of
    ...
    def render(self, w, **kwargs):
        ...
        if facetsform and self.display_filter == "top":
            cssclass = "hidden" if self.hide_filter else ""
            facetsform.render(
                w, vid=self.view.__regid__, cssclass=cssclass, divid=self.view.domid
            )
        actions = []
        if self.display_actions:
            ...
            if (
                self.display_filter
                and self.hide_filter
                and (facetsform or not generate_form)
            ):
    ...
    def render_table(self, w, actions, paginate):
        ...
        w("<table %s>" % sgml_attributes(attrs))
    ...
    def row_attributes(self, rownum):
        return {
            "class": "odd" if (rownum % 2 == 1) else "even",
    ...
    def render_cell(self, w, rownum, colnum, renderer):
        ...
        if colnum in self.header_column_idx:
            ...
            tag = "td"
        w("<%s %s>" % (tag, sgml_attributes(attrs)))
    ...
    def cell_attributes(self, rownum, _colnum, renderer):
        ...
        if renderer.sortable:
            ...
            if sortvalue is not None:
                attrs["cubicweb:sortvalue"] = js_dumps(sortvalue)
        return attrs
...
class AbstractColumnRenderer:
    ...
    .. automethod:: cubicweb_web.views.tableview.AbstractColumnRenderer.bind
    .. automethod:: cubicweb_web.views.tableview.AbstractColumnRenderer.render_header
    .. automethod:: cubicweb_web.views.tableview.AbstractColumnRenderer.render_cell
    .. automethod:: cubicweb_web.views.tableview.AbstractColumnRenderer.sortvalue
...
class TableMixIn(component.LayoutableMixIn):
    ...
    .. autoattribute:: cubicweb_web.views.tableview.TableMixIn.table_size
    ...
    def facets_form(self, **kwargs):  # XXX extracted from jqplot cube
        return self._cw.vreg["views"].select_or_none(
            "facet.filtertable", self._cw, rset=self.cw_rset, view=self, **kwargs
        )
    ...
    def build_column_renderers(self):
        ...
        .. autoclass:: cubicweb_web.views.tableview.AbstractColumnRenderer
        """
        raise NotImplementedError()

    def table_actions(self):
        """Return a list of actions (:class:`~cubicweb_web.component.Link`) that
        ...
        for action in actionsbycat.get("mainactions", ()):
            for action in action.actual_actions():
                actions.append(
                    component.Link(
                        action.url(), req._(action.title), klass=action.html_class()
                    )
                )
        return actions
...
class RsetTableColRenderer(AbstractColumnRenderer):
    ...
    def render_cell(self, w, rownum):
        self._cw.view(
            self.cellvid, self.cw_rset, "empty-cell", row=rownum, col=self.colid, w=w
        )
    ...
    def sortvalue(self, rownum):
        ...
        if etype is None:
            return ""
        if self._cw.vreg.schema.entity_schema_for(etype).final:
...
class RsetTableView(TableMixIn, AnyRsetView):
    ...
    * `cellvids`, if not `None`, should be a dictionary with table column index
    ...
    def call(self, headers=None, displaycols=None, cellvids=None, paginate=None):
        if self.headers:
    ...
    def main_var_index(self):
        ...
        for i, etype in enumerate(self.cw_rset.description[0]):
            if not entity_schema_for(etype).final:
                return i
        return None
    ...
    def build_column_renderers(self):
        ...
        for colnum, colid in enumerate(displaycols):
            ...
            if cellvids is not None and colnum in cellvids:
                ...
                if (
                    coltype is not None
                    and self._cw.vreg.schema.entity_schema_for(coltype).final
                ):
                    cellvid = self.finalvid
                else:
                    cellvid = self.nonfinalvid
            # get renderer
            renderer = self.column_renderer(
                colid, header=title, trheader=False, addcount=addcount, cellvid=cellvid
            )
...
class EntityTableColRenderer(AbstractColumnRenderer):
    ...
    .. automethod:: cubicweb_web.views.tableview.EntityTableColRenderer.entity
    .. automethod:: cubicweb_web.views.tableview.EntityTableColRenderer.render_entity
    .. automethod:: cubicweb_web.views.tableview.EntityTableColRenderer.entity_sortvalue
    """
    ...
    def copy(self):
        ...
        try:
            acopy = copy(self)
            for aname, member in [("renderfunc", renderfunc), ("sortfunc", sortfunc)]:
...
class EntityTableView(TableMixIn, EntityView):
    ...
    .. autoclass:: cubicweb_web.views.tableview.EntityTableColRenderer
    .. autoclass:: cubicweb_web.views.tableview.MainEntityColRenderer
    .. autoclass:: cubicweb_web.views.tableview.RelatedEntityColRenderer
    .. autoclass:: cubicweb_web.views.tableview.RelationColRenderer
    """
...
class TableView(AnyRsetView, metaclass=class_deprecated):
    ...
    def form_filter(
        self, divid, displaycols, displayactions, displayfilter, paginate, hidden=True
    ):
        try:
        ...
        filterform.render(
            self.w, vid=self.__regid__, divid=divid, vidargs=vidargs, cssclass=cssclass
        )
    ...
    def displaycols(self, displaycols, headers):
        if displaycols is None:
            if "displaycols" in self._cw.form:
                ...
                displaycols = list(
                    range(len(self.cw_rset.syntax_tree().children[0].selection))
                )
        return displaycols
    ...
    def call(
        ...
        if self.initial_load:
            ...
            if title:
                self.w('<h2 class="tableTitle">%s</h2>\n', title)
            if displayfilter:
                actions += self.form_filter(
                    divid, displaycols, displayfilter, displayactions, paginate
                )
        elif displayfilter:
            actions += self.show_hide_actions(divid, True)
        self.w('<div id="%s">', divid)
        if displayactions:
            actionsbycat = self._cw.vreg["actions"].possible_actions(req, self.cw_rset)
            for action in actionsbycat.get("mainactions", ()):
                for action in action.actual_actions():
                    actions.append(
                        (action.url(), req._(action.title), action.html_class(), None)
                    )
        # render actions menu
        if actions:
    ...
    def show_hide_actions(self, divid, currentlydisplayed=False):
        ...
        if currentlydisplayed:
            return [
                (showhide, showlabel, "hidden", "%sShow" % divid),
                (showhide, hidelabel, None, "%sHide" % divid),
            ]
        return [
            (showhide, showlabel, None, "%sShow" % divid),
    ...
    def get_columns(
        ...
        return a list of columns description to be used by
               :class:`~cubicweb_web.htmlwidgets.TableWidget`
        """
        columns = []
        eschema = self._cw.vreg.schema.eschema
