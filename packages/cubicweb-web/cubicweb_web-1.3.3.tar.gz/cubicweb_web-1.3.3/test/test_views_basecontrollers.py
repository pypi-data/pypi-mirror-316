...
class ViewControllerTC(PyramidWebCWTC):
    def test_view_ctrl_with_valid_cache_headers(self):
        now = time.time()
        resp = self.webapp.get(urljoin(BASE_URL, "manage"))
        ...
        resp = self.webapp.get(urljoin(BASE_URL, "manage"), headers=cache_headers)
...
class EditControllerTC(PyramidWebCWTC):
 ...
 window.parent.handleFormValidationResponse('entityForm', null, null, [true, "{BASE_URL}view", null], null);
</script>""",
                self.ctrl_publish(req, "validateform").decode("ascii"),
            )
            self.assertEqual(
...
class AjaxControllerTC(WebCWTC):
    ...
    def test_maydel_perms(self):
        ...
        with self.temporary_permissions(
            (
                self.schema["tags"].relation_definitions["Tag", "CWUser"],
    ...
    def test_add_inserts(self):
        with self.remote_calling(
            "add_pending_inserts", [("12", "tags", "13"), ("12", "tags", "14")]
        ) as (_, req):
            inserts = get_pending_inserts(req)
            self.assertCountEqual(inserts, ["12:tags:13", "12:tags:14"])
            req.remove_pending_operations()
    ...
    def test_external_resource(self):
        with self.remote_calling("external_resource", "RSS_LOGO") as (res, _):
            self.assertEqual(
                json_dumps(self.config.uiprops["RSS_LOGO"]).encode("ascii"), res
            )
...
class UndoControllerTC(WebCWTC):
    ...
    def setup_database(self):
        with self.admin_access.repo_cnx() as cnx:
            self.toto = self.create_user(
                cnx, "toto", password="toto", groups=("users",), commit=False
            )
            self.txuuid_toto = cnx.commit()
            self.toto_email = cnx.create_entity(
                "EmailAddress", address="toto@logilab.org", reverse_use_email=self.toto
            )
