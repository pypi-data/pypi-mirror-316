...
class VidFromRsetCWTC(WebCWTC):
    ...
    def test_one_entity(self):
        with self.admin_access.web_request() as req:
            ...
            rset = req.execute('Any X WHERE X login "admin"')
            self.assertEqual(
                vid_from_rset(req, rset, self.schema), "outofcontext-search"
            )
...
class BaseViewsCWTC(WebCWTC):
    ...
    def test_incontext(self):
        with self.admin_access.web_request() as req:
            entity = req.create_entity(
                "CWUser", login="<script></script>", upassword="toto"
            )
            result = entity.view("incontext")
            expected = (
                f'<a href="{BASE_URL}{entity.eid}" title="">'
                f"&lt;script&gt;&lt;/script&gt;</a>"
            )
    ...
    def test_list(self):
        with self.admin_access.web_request() as req:
            ...
            expected = f"""<ul class="section">
<li><a href="{BASE_URL}{entity.eid}" title="">&lt;script&gt;&lt;/script&gt;</a></li>
<li><a href="{BASE_URL}cwuser/admin" title="">admin</a></li>
<li><a href="{BASE_URL}cwuser/anon" title="">anon</a></li>
</ul>
"""
            self.assertEqual(result, expected)
    ...
    def test_simplelist(self):
        with self.admin_access.web_request() as req:
            ...
            expected = (
                f'<div class="section">'
                f'<a href="{BASE_URL}{entity.eid}" title="">'
                f"&lt;script&gt;&lt;/script&gt;</a></div>"
                f'<div class="section">'
                f'<a href="{BASE_URL}cwuser/admin" title="">admin</a></div>'
                f'<div class="section">'
                f'<a href="{BASE_URL}cwuser/anon" title="">anon</a></div>'
            )
            self.assertEqual(result, expected)
    ...
    def test_sameetypelist2(self):
        with self.admin_access.web_request() as req:
            ...
            expected = (
                f'<a href="{BASE_URL}{entity.eid}" title="">'
                f"&lt;script&gt;&lt;/script&gt;</a>, "
                f'<a href="{BASE_URL}cwuser/admin" title="">admin</a>, '
                f'<a href="{BASE_URL}cwuser/anon" title="">anon</a>'
            )
            self.assertEqual(result, expected)
    ...
    def test_metadata(self):
        with self.admin_access.web_request() as req:
            ...
            entity.cw_set(creation_date="2000-01-01 00:00:00")
            entity.cw_set(modification_date="2015-01-01 00:00:00")
            result = entity.view("metadata")
            expected = (
                "<div>CWUser #%d - <span>latest update on</span>"
                ' <span class="value">2015/01/01</span>,'
                " <span>created on</span>"
                ' <span class="value">2000/01/01</span></div>' % entity.eid
            )
            self.assertEqual(result, expected)
