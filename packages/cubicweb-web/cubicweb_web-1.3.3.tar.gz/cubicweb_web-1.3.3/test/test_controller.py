...
class BaseControllerCWTC(WebCWTC):
    ...
    def test_parse_datetime_ko(self):
        with self.admin_access.web_request() as req:
            ctrl = self.vreg["controllers"].select("view", req)
            pd = ctrl._cw.parse_datetime
            self.assertRaises(ValueError, pd, "2006/06/24 12:188", "Datetime")
            self.assertRaises(ValueError, pd, "2006/06/240", "Datetime")
            self.assertRaises(ValueError, pd, "2006/06/24 12:18", "Date")
            self.assertRaises(ValueError, pd, "2006/24/06", "Date")
            self.assertRaises(ValueError, pd, "2006/06/240", "Date")
            self.assertRaises(ValueError, pd, "12:188", "Time")
