...
from cubicweb_web.view import EntityView, AnyRsetView, View
...
class OneLineView(EntityView):
    ...
    def cell_call(self, row, col, **kwargs):
        ...
        desc = cut(entity.dc_description(), 50)
        title = cut(
            entity.dc_title(), self._cw.property_value("navigation.short-line-size")
        )
        self.w(
            '<a href="%s" title="%s">%s</a>',
            xml_escape(entity.absolute_url()),
...
class SimpleListView(ListItemView):
    ...
    Similar to :class:~cubicweb_web.views.baseviews.ListView but using '<div>'
...
class GroupByView(EntityView):
    ...
    def index_url(self, basepath, key, **kwargs):
        if isinstance(key, (list, tuple)):
            key = "/".join(key)
        return self._cw.build_url(f"{basepath}/{key}", **kwargs)
...
class ArchiveView(GroupByView):
    ...
    def group_key(self, entity, **kwargs):
        value = super().group_key(entity, **kwargs)
    ...
    def index_link(self, basepath, key, items):
        ...
        year, month = key
        label = "{} {} [{}]".format(
            self._cw._(calendar.MONTHNAMES[int(month) - 1]),
            year,
            len(items),
        )
        etypes = {entity.cw_etype for entity in items}
        vtitle = "{} {}".format(
            ", ".join(display_name(self._cw, etype, "plural") for etype in etypes),
            label,
        )
