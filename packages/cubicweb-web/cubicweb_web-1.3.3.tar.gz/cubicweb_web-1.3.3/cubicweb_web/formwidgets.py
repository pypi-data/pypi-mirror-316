...
.. autoclass:: cubicweb_web.formwidgets.FieldWidget


HTML <input> based widgets
''''''''''''''''''''''''''

.. autoclass:: cubicweb_web.formwidgets.HiddenInput
.. autoclass:: cubicweb_web.formwidgets.TextInput
.. autoclass:: cubicweb_web.formwidgets.EmailInput
.. autoclass:: cubicweb_web.formwidgets.PasswordSingleInput
.. autoclass:: cubicweb_web.formwidgets.FileInput
.. autoclass:: cubicweb_web.formwidgets.ButtonInput


Other standard HTML widgets
'''''''''''''''''''''''''''

.. autoclass:: cubicweb_web.formwidgets.TextArea
.. autoclass:: cubicweb_web.formwidgets.Select
.. autoclass:: cubicweb_web.formwidgets.CheckBox
.. autoclass:: cubicweb_web.formwidgets.Radio


Date and time widgets
'''''''''''''''''''''

.. autoclass:: cubicweb_web.formwidgets.DateTimePicker
.. autoclass:: cubicweb_web.formwidgets.JQueryDateTimePicker
.. autoclass:: cubicweb_web.formwidgets.JQueryDatePicker
.. autoclass:: cubicweb_web.formwidgets.JQueryTimePicker


Ajax / javascript widgets
'''''''''''''''''''''''''

.. autoclass:: cubicweb_web.formwidgets.FCKEditor
.. autoclass:: cubicweb_web.formwidgets.AjaxWidget
.. autoclass:: cubicweb_web.formwidgets.AutoCompletionWidget
.. autoclass:: cubicweb_web.formwidgets.InOutWidget
...
.. autoclass:: cubicweb_web.formwidgets.PasswordInput
.. autoclass:: cubicweb_web.formwidgets.IntervalWidget
.. autoclass:: cubicweb_web.formwidgets.BitSelect
.. autoclass:: cubicweb_web.formwidgets.HorizontalLayoutWidget
.. autoclass:: cubicweb_web.formwidgets.EditableURLWidget
...
.. autoclass:: cubicweb_web.formwidgets.Button
.. autoclass:: cubicweb_web.formwidgets.SubmitButton
.. autoclass:: cubicweb_web.formwidgets.ResetButton
.. autoclass:: cubicweb_web.formwidgets.ImgButton
"""
...
from cubicweb_web import stdmsgs, INTERNAL_FIELD_VALUE, ProcessFormError
...
class FieldWidget:
    ...
    **Attributes**
    ...
    .. currentmodule:: cubicweb_web.formwidgets
    ...
    **Post handling methods**
    ...
    def values(self, form, field):
        ...
        4. field's typed value (returned by its
           :meth:`~cubicweb_web.formfields.Field.typed_value` method)
...
class Input(FieldWidget):
    ...
    def _render(self, form, field, renderer):
        ...
        inputs = [
            tags.input(
                name=field.input_name(form, self.suffix),
                type=self.type,
                value=value,
                **attrs,
            )
...
class PasswordSingleInput(Input):
    ...
    You may prefer using the :class:`~cubicweb_web.formwidgets.PasswordInput`
...
class PasswordInput(Input):
    ...
    def _render(self, form, field, renderer):
        ...
        inputs = [
            ...
            "<br/>",
            tags.input(
                name=field.input_name(form, "-confirm"),
                value=values[0],
                type=self.type,
                **attrs,
            ),
...
class TextArea(FieldWidget):
    ...
    def _render(self, form, field, renderer):
        ...
        return tags.textarea(value, name=field.input_name(form, self.suffix), **attrs)
    ...
    def _render(self, form, field, renderer):
        ...
        for option in vocab:
            ...
            if value is None:
                ...
                options.append("<optgroup %s>" % uilib.sgml_attributes(oattrs))
                optgroup_opened = True
            elif self.value_selected(value, curvalues):
                options.append(
                    tags.option(label, value=value, selected="selected", **oattrs)
                )
            else:
...
"""
    add_button = (
        '<input type="button" class="wdgButton cwinoutadd" '
        'value="&gt;&gt;" size="10" />'
    )
    remove_button = (
        '<input type="button" class="wdgButton cwinoutremove" '
        'value="&lt;&lt;" size="10" />'
    )
    ...
    def render_select(self, form, field, name, selected=False):
        ...
        for option in field.vocabulary(form):
            ...
            if selected:
                ...
                if value in values:
                    ...
                    # add hidden inputs
                    inputs.append(
                        tags.input(value=value, name=field.dom_id(form), type="hidden")
                    )
            else:
        ...
        if "id" in attrs:
            attrs.pop("id")
        return tags.select(
            name=name, multiple=self._multiple, id=name, options=options, **attrs
        ) + "\n".join(inputs)
...
class BitSelect(Select):
    ...
    See also :class:`~cubicweb_web.facet.BitFieldFacet`.
    """
...
class CheckBox(Input):
    ...
    def _render(self, form, field, renderer):
        ...
        for i, option in enumerate(field.vocabulary(form)):
            ...
            options.append("<label>%s&#160;%s</label>" % (tag, xml_escape(label)))
...
class DateTimePicker(TextInput):
    ...
    def _render_calendar_popup(self, form, field):
        ...
        return """<a onclick="toggleCalendar('%s', '%s', %s, %s);" class="calhelper">
...
class JQueryDatePicker(FieldWidget):
    ...
    def attributes(self, form, field):
        ...
        if self.max_of:
            attrs["data-max-of"] = "%s-subject:%s" % (
                self.max_of,
                form.edited_entity.eid,
            )
        if self.min_of:
            attrs["data-min-of"] = "%s-subject:%s" % (
                self.min_of,
                form.edited_entity.eid,
            )
        return attrs
    ...
    def _render(self, form, field, renderer):
        ...
        req.add_onload(
            'renderJQueryDatePicker("%s", "%s", "%s", %s, %s);'
            % (
                domid,
                req.uiprops["CALENDAR_ICON"],
                picker_fmt,
                json_dumps(min_date),
                json_dumps(max_date),
            )
    ...
    def _render_input(self, form, field):
        ...
        return tags.input(
            name=field.input_name(form, self.suffix), value=value, type="text", **attrs
        )
...
class JQueryTimePicker(JQueryDatePicker):
    ...
    def __init__(self, timestr=None, timesteps=30, separator=":", **kwargs):
    ...
    def _render(self, form, field, renderer):
        domid = field.dom_id(form, self.suffix)
        form._cw.add_onload(
            'cw.jqNode("%s").timePicker({step: %s, separator: "%s"})'
...
class JQueryDateTimePicker(FieldWidget):
    ...
    def __init__(self, initialtime=None, timesteps=15, separator=":", **kwargs):
...
def init_ajax_attributes(attrs, wdgtype, loadtype="auto"):
    try:
        attrs["class"] += " widget"
    except KeyError:
        attrs["class"] = "widget"
...
class AutoCompletionWidget(TextInput):
    ...
    def __init__(self, *args, **kwargs):
        self.autocomplete_settings = kwargs.pop(
            "autocomplete_settings", self.default_settings
        )
    ...
    def _get_url(self, entity, field):
        ...
        return entity._cw.build_url(
            "ajax", fname=fname, mode="remote", pageid=entity._cw.pageid
        )
...
class LazyRestrictedAutoCompletionWidget(RestrictedAutoCompletionWidget):
    ...
    def values_and_attributes(self, form, field):
        ...
        values, attrs = super(
            LazyRestrictedAutoCompletionWidget, self
    ...
    def _render(self, form, field, renderer):
        if self.attrs.get("display_label", True):
            ...
            fields = [
                subst
                % {
                    "label": renderer.render_label(form, f),
...
class EditableURLWidget(FieldWidget):
    ...
    def _render(self, form, field, renderer):
        ...
        if pathqname in form.form_previous_values:
            ...
            if value:
                ...
                path = qs = ""
            fqs = "\n".join("%s=%s" % (k, v) for k, v in req.url_parse_qsl(qs))
        attrs = dict(self.attrs)
        if self.setdomid:
        ...
        attrs.setdefault("onkeyup", "autogrow(this)")
        inputs += [tags.textarea(fqs, name=fqsqname, **attrs), "</td></tr></table>"]
        # surrounding div necessary for proper error localization
        return '<div id="%s">%s</div>' % (field.dom_id(form), "\n".join(inputs))
    ...
    def process_field_data(self, form, field):
        ...
        if not values:
            return path
        return "%s?%s" % (path, req.build_url_params(**values))
...
class ImgButton:
    ...
    def render(self, form, field=None, renderer=None):
        ...
        return (
            '<a id="%(domid)s" href="%(href)s">'
            '<img src="%(imgsrc)s" alt="%(label)s"/>%(label)s</a>'
            % {"label": label, "imgsrc": imgsrc, "domid": self.domid, "href": self.href}
        )
