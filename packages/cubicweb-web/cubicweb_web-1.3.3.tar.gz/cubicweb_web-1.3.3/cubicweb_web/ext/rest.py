...
from cubicweb_web.views import (
    vid_from_rset,
)  # XXX better not to import c.w.views here...
...
def eid_reference_role(role, rawtext, text, lineno, inliner, options={}, content=[]):
    try:
        try:
            eid_num, rest = text.split(":", 1)
        except ValueError:
        ...
        msg = inliner.reporter.error(
            'EID number must be a positive number; "%s" is invalid.' % text, line=lineno
        )
    ...
    try:
        ...
        ref = "#"
        rest += " " + context._cw._("(UNEXISTANT EID)")
    else:
...
def bookmark_role(role, rawtext, text, lineno, inliner, options={}, content=[]):
    ...
    try:
        ...
        msg = inliner.reporter.error(
            f"Could not parse bookmark path {bookmark.path} [{exc}].", line=lineno
        )
        prb = inliner.problematic(rawtext, rawtext, msg)
...
def winclude_directive(
    ...
    cw = context._cw
    state_machine.input_lines.source(lineno - state_machine.input_offset - 1)

    # source_dir = os.path.dirname(os.path.abspath(source))
    fid = arguments[0]

    for lang in chain(
        (cw.lang, cw.vreg.property_value("ui.language")),
        cw.vreg.config.available_languages(),
    ):
        rid = f"{fid}_{lang}.rst"
...
class RQLTableDirective(Directive):
    ...
    def run(self):
        ...
        rql = " ".join([rql_line.strip() for rql_line in self.content])
        ...
        try:
            ...
            raise self.error(f"fail to execute RQL query in {errid}: {exc!r}")
        ...
        try:
            ...
            raise self.error(f"fail to select '{vid}' view in {errid}: {exc!r}")
        ...
        if "headers" in self.options:
            ...
            if len(headers) != len(rset[0]):
                raise self.error(
                    "the number of 'headers' does not match the "
        ...
        if "colvids" in self.options:
            ...
            for f in self.options["colvids"].split(","):
                try:
                    ...
                    raise self.error("malformatted 'colvids' option in %s" % errid)
                cellvids[int(idx.strip())] = vid.strip()

        try:
            content = view.render(headers=headers, cellvids=cellvids)
        except Exception as exc:
            raise self.error(f"Error rendering {errid} ({exc})")
...
class CubicWebReSTParser(Parser):
    ...
    def __init__(self):
        ...
        self.statemachine = states.RSTStateMachine(
            state_classes=self.state_classes, initial_state=self.initial_state, debug=0
        )

    def parse(self, inputstring, document):
        """Parse `inputstring` and populate `document`, a document tree."""
        self.setup_parse(inputstring, document)
        inputlines = statemachine.string2lines(inputstring, convert_whitespace=1)
