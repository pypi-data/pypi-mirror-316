...
class SchemaViewer:
    ...
    def visit_schema(self, schema, display_relations=0, skiptypes=()):
        ...
        title = Title(self._("Schema %s") % schema.name, klass="titleUnderline")
    ...
    def _entity_attributes_data(self, eschema):
        ...
        attributes = sorted(eschema.attribute_definitions(), key=lambda el: el[0].type)
        for rschema, aschema in attributes:
            ...
            if aname == "eid":
                continue
            data.append("{} ({})".format(aname, _(aname)))
    ...
    def visit_entityschema(self, eschema, skiptypes=()):
        ...
        rel_defs = sorted(eschema.relation_definitions(), key=lambda el: el[0].type)
        ...
        for rschema, targetschemas, role in rel_defs:
            ...
            for oeschema in sorted(targetschemas, key=TYPE_GETTER):
                ...
                if role == "subject":
                    ...
                    cards = cards[::-1]
                label = "{} {} {}".format(CARD_MAP[cards[1]], label, CARD_MAP[cards[0]])
    ...
    def visit_relationschema(self, rschema, title=True):
        ...
        if rschema_objects:
            ...
            properties = [
                p
                for p in RelationDefinitionSchema.rproperty_defs(rschema_objects[0])
