...
.. autoclass:: cubicweb_web.formfields.Field
...
.. autoclass:: cubicweb_web.formfields.StringField()
.. autoclass:: cubicweb_web.formfields.PasswordField()
.. autoclass:: cubicweb_web.formfields.IntField()
.. autoclass:: cubicweb_web.formfields.BigIntField()
.. autoclass:: cubicweb_web.formfields.FloatField()
.. autoclass:: cubicweb_web.formfields.BooleanField()
.. autoclass:: cubicweb_web.formfields.DateField()
.. autoclass:: cubicweb_web.formfields.DateTimeField()
.. autoclass:: cubicweb_web.formfields.TZDatetimeField()
.. autoclass:: cubicweb_web.formfields.TimeField()
.. autoclass:: cubicweb_web.formfields.TimeIntervalField()

Compound fields
''''''''''''''''

.. autoclass:: cubicweb_web.formfields.RichTextField()
.. autoclass:: cubicweb_web.formfields.FileField()
.. autoclass:: cubicweb_web.formfields.CompoundField()

.. autoclass cubicweb_web.formfields.EditableFileField() XXX should be a widget

Entity specific fields and function
'''''''''''''''''''''''''''''''''''

.. autoclass:: cubicweb_web.formfields.RelationField()
.. autofunction:: cubicweb_web.formfields.guess_field
...
class Field:
    ...
    **Attributes**
    ...
    .. currentmodule:: cubicweb_web.formfields
    ...
    **Post handling methods**
    ...
    def actual_fields(self, form):
        ...
        :class:`~cubicweb_web.formfields.RichTextField` is containing a format
    ...
    def role_name(self):
        ...
        assert (
            self.name
        ), "field without a name (give it to constructor for explicitly built fields)"
    ...
    def typed_value(self, form, load_bytes=False):
        ...
        if self.eidparam and self.role is not None:
            ...
            if form._cw.vreg.schema.relation_schema_for(self.name).final:
    ...
    def _has_been_modified(self, form):
        ...
        try:
            if self.role == "subject":
                ...
                previous_value = getattr(form.edited_entity, "reverse_%s" % self.name)
        ...
        if isinstance(previous_value, (list, tuple)):
            ...
            previous_value = {e.eid for e in previous_value}
        try:
    ...
    def process_posted(self, form):
        ...
        field returned by :meth:`~cubicweb_web.formfields.Field.actual_fields`.
        """
        for field in self.actual_fields(form):
            if field is self:
                try:
                ...
                yield from field.process_posted(form)
...
class StringField(Field):
    ...
    * :class:`~cubicweb_web.formwidgets.Select` if some vocabulary is specified
      using `choices` attribute

    * :class:`~cubicweb_web.formwidgets.TextInput` if maximum size is specified
      using `max_length` attribute and this length is inferior to 257.

    * :class:`~cubicweb_web.formwidgets.TextArea` in all other cases
    """
...
class PasswordField(StringField):
    ...
    Unless explicitly specified, the widget for this field will be
    a :class:`~cubicweb_web.formwidgets.PasswordInput`.
    """
    ...
    def typed_value(self, form, load_bytes=False):
        if self.eidparam:
            ...
            if form.edited_entity.has_eid():
                return ""
            return self.initial_typed_value(form, load_bytes)
        return super().typed_value(form, load_bytes)
...
class RichTextField(StringField):
    ...
    :class:`~cubicweb_web.formwidgets.FCKEditor` or a
    :class:`~cubicweb_web.formwidgets.TextArea`. according to the field's
...
class FileField(StringField):
    ...
    * `name_field`, holding the file's name.
    ...
    Unless explicitly specified, the widget for this field will be a
    :class:`~cubicweb_web.formwidgets.FileInput`. Inner fields, if any,
    ...
    def __init__(
        self, format_field=None, encoding_field=None, name_field=None, **kwargs
    ):
        super().__init__(**kwargs)
        self.format_field = format_field
    ...
    def render(self, form, renderer):
        ...
        if self.format_field or self.encoding_field:
            ...
            wdgs.append('<div id="%s" class="hidden">' % divid)
            if self.name_field:
        ...
        if not self.required and self.typed_value(form):
            ...
            wdgs.append("<br/>")
            wdgs.append(
                tags.input(name=self.input_name(form, "__detach"), type="checkbox")
            )
    ...
    def render_subfield(self, form, field, renderer):
        return (
            renderer.render_label(form, field)
    ...
    def _process_form_value(self, form):
        ...
        if self.input_name(form, "__detach") in posted:
        ...
        try:
            if isinstance(value, list):
                value = value[0]
                form.warning(
                    "mutiple files provided, however " "only the first will be picked"
                )
...
class EditableFileField(FileField):
    ...
    :class:`~cubicweb_web.formfields.FileField` but expect that stream to
    actually contains some text.

    If the stream format is one of text/plain, text/html, text/rest,
    text/markdown
    then a :class:`~cubicweb_web.formwidgets.TextArea` will be additionally
    displayed, allowing to directly the file's content when desired, instead
    of choosing a file from user's file system.
    """

    editable_formats = ("text/plain", "text/html", "text/rest", "text/markdown")

    def render(self, form, renderer):
        wdgs = [super().render(form, renderer)]
        if self.format(form) in self.editable_formats:
            ...
            if data:
                ...
                try:
                    ...
                    wdgs.append("<p><b>%s</b></p>" % msg)
                    wdgs.append(
                        fw.TextArea(setdomid=False).render(form, self, renderer)
                    )
    ...
    def _process_form_value(self, form):
        ...
        return super()._process_form_value(form)
...
class BigIntField(Field):
    ...
    Unless explicitly specified, the widget for this field will be a
    :class:`~cubicweb_web.formwidgets.TextInput`.
    """
...
class IntField(BigIntField):
    ...
    :class:`~cubicweb_web.formfields.BigIntField` but set max length when text
...
class BooleanField(Field):
    ...
    Unless explicitly specified, the widget for this field will be a
    :class:`~cubicweb_web.formwidgets.Radio` with yes/no values. You
    ...
    def vocabulary(self, form):
        if self.choices:
            return super().vocabulary(form)
        if self.allow_none:
            return [
                (form._cw._("indifferent"), ""),
                (form._cw._("yes"), "1"),
                (form._cw._("no"), "0"),
            ]
    ...
    def format_single_value(self, req, value):
        ...
        if self.allow_none:
            ...
            if value is False:
                return "0"
        return super().format_single_value(req, value)
...
class FloatField(IntField):
    ...
    :class:`~cubicweb_web.formfields.IntField`.
        ...
        if isinstance(value, str):
            ...
            try:
                ...
                raise ProcessFormError(
                    form._cw._(
                        "a number (in seconds) or 20s, 10min, 24h or 4d are expected"
                    )
                )
...
class DateField(StringField):
    ...
    Unless explicitly specified, the widget for this field will be a
    :class:`~cubicweb_web.formwidgets.JQueryDatePicker`.
    """
...
class DateTimeField(DateField):
    ...
    Unless explicitly specified, the widget for this field will be a
    :class:`~cubicweb_web.formwidgets.JQueryDateTimePicker`.
    """
...
class TZDatetimeField(DateTimeField):
    ...
    def _ensure_correctly_typed(self, form, value):
        tz_naive = super()._ensure_correctly_typed(form, value)
        if not tz_naive:
            return None
...
class TimeField(DateField):
    ...
    Unless explicitly specified, the widget for this field will be a
    :class:`~cubicweb_web.formwidgets.JQueryTimePicker`.
    """
...
class RelationField(Field):
    ...
    Unless explicitly specified, the widget for this field will be a
    :class:`~cubicweb_web.formwidgets.Select`.
    """
    ...
    def relvoc_linkedto(self, form):
        ...
        if linkedto:
            ...
            return [(buildent(eid).view("combobox"), str(eid)) for eid in linkedto]
        return []
    ...
    def relvoc_init(self, form):
        ...
        if form.edited_entity.has_eid():
            ...
            vocab += [(e.view("combobox"), str(e.eid)) for e in rset.entities()]
        return vocab

    def relvoc_unrelated(self, form, limit=None):
        entity = form.edited_entity
        rtype = entity._cw.vreg.schema.relation_schema_for(self.name)
        if entity.has_eid():
            done = {row[0] for row in entity.related(rtype, self.role)}
        else:
            done = None
        result = []
        rsetsize = None
    ...
    def _relvoc_unrelated(self, form, targettype, limit, done):
        ...
        res = []
        entity = form.edited_entity
        for entity in entity.unrelated(
            self.name, targettype, self.role, limit, lt_infos=form.linked_to
        ).entities():
    ...
    def _process_form_value(self, form):
        ...
        if values is None:
            ...
            values = (values,)
        eids = set()
        rschema = form._cw.vreg.schema.relation_schema_for(self.name)
        for eid in values:
        ...
        if issubclass(fieldclass, FileField):
            ...
            for metadata in KNOWN_METAATTRIBUTES:
                ...
                if metaschema is not None:
                    ...
                    kwargs["%s_field" % metadata] = guess_field(
                        eschema, metaschema, req=req, **metakwargs
                    )
