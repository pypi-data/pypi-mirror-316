...
from connector.generated.models.token_credential import TokenCredential
...
class AuthCredential(BaseModel):
    ...
    """ # noqa: E501
    oauth: Optional[OAuthCredential] = Field(default=None, description="OAuth credentials, if using OAuth.")
    oauth_client_credentials: Optional[OAuthClientCredential] = Field(default=None, description="OAuth client credentials, if using OAuth Client Credentials flow.")
    basic: Optional[BasicCredential] = Field(default=None, description="Basic auth credentials, if using basic auth.")
    token: Optional[TokenCredential] = Field(default=None, description="Token credentials, if using token-based auth.")
    __properties: ClassVar[List[str]] = ["oauth", "oauth_client_credentials", "basic", "token"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of AuthCredential from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of oauth_client_credentials
        if self.oauth_client_credentials:
            _dict['oauth_client_credentials'] = self.oauth_client_credentials.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of AuthCredential from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "oauth": OAuthCredential.from_dict(obj["oauth"]) if obj.get("oauth") is not None else None,
            "oauth_client_credentials": OAuthClientCredential.from_dict(obj["oauth_client_credentials"]) if obj.get("oauth_client_credentials") is not None else None,
            "basic": BasicCredential.from_dict(obj["basic"]) if obj.get("basic") is not None else None,
            "token": TokenCredential.from_dict(obj["token"]) if obj.get("token") is not None else None
        })
        return _obj
