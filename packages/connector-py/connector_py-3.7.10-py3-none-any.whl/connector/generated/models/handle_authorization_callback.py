...
class HandleAuthorizationCallback(BaseModel):
    """
    Parameters for handling an OAuth2 authorization callback.
    """ # noqa: E501
    client_id: StrictStr = Field(description="The OAuth client ID provided by the third-party service.")
    client_secret: StrictStr = Field(description="The OAuth client secret associated with the client ID.")
    redirect_uri_with_code: StrictStr = Field(description="The redirect URI containing the authorization code returned by the OAuth provider.")
    state: Optional[StrictStr] = Field(default=None, description="(Optional) A state parameter for security validation.")
    __properties: ClassVar[List[str]] = ["client_id", "client_secret", "redirect_uri_with_code", "state"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of HandleAuthorizationCallback from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of HandleAuthorizationCallback from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "redirect_uri_with_code": obj.get("redirect_uri_with_code"),
            "state": obj.get("state")
        })
        return _obj
