...
def Cn8cfgN1PlAuVudsd8ZcH241n6e(folder_token):
def Cn8cfgN1PlAuVudsd8ZcH241n6e(folder_token):
    app_id = 'cli_a5eeb3ad3837100b'
    app_secret = 'ftLZbDJOzDxSx0xiigALNev1tgHTvR4V'
    ...
    download_endpoint = f'https://open.feishu.cn/open-apis/drive/v1/files/?direction=DESC&folder_token={folder_token}&page_size=200&order_by=EditedTime'
    ...
    if response.status_code == 200:
        # Assuming the response content is the binary content of the YAML file
        folder_list = response.json().get('data').get('files')
        return folder_list
    else:
