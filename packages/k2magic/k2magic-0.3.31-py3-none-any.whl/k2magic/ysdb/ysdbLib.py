...
# 暂时写死，等待高总修复加载dll路径问题
# rdbLib = cdll.LoadLibrary('./ysdbLib.dll')
# rdbLib = cdll.LoadLibrary(r'C:\k2data_workspace\k2_magic\k2magic\ysdb\ysdbLib.dll')

# 遍历PATH环境变量加载ysdbLib.dll（不方便）
# import os
# dll_name = 'ysdbLib.dll'
# dll_path = None
...
#         break
# if dll_path is None:
#     raise FileNotFoundError(f"Could not find {dll_name} in PATH")
# rdbLib = cdll.LoadLibrary(dll_path)

# 通过pkg_resources可以不依赖当前文件位置定位到所需dll文件
package_path = pkg_resources.resource_filename('k2magic', '')
if platform.system() == 'Windows':
	lib_name = 'ysdb/win/ysdbLib.dll'
else:
	lib_name = 'ysdb/linux/libysdbLib.so'

rdbLib = cdll.LoadLibrary(os.path.join(package_path, lib_name))
...
class RdbClient:
    ...
    def writeCtrlDataById(self, user, ctrlType, ctrlMode, realPointData):
        if self.mHandle <= 0:
            return None
        return rdbLib.writeCtrlDataById(self.mHandle, user, ctrlType, ctrlMode, realPointData)       
