...
class Auth:
    ...
    async def get_users(self, request):
        ...
        data = await request.json()
        self.LOG(1, 0, label="AUTH", label2='get_users', msg=f"1")
        role,org = await self.enforce_admin_perm(request, label2=label2)
        self.LOG(1, 0, label="AUTH", label2='get_users', msg=f"2, role={role}, org={org}")
        data = await request.json()
        ...
        self.LOG(1, 0, label="AUTH", label2='get_users', msg=f"3")
        res_to_recs(res, ["org"])
