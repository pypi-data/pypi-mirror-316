...
class Auth:
    ...
    async def get_users(self, request):
        ...
        label2 = "get_users"
        self.LOG(2, 0, label="AUTH", label2=label2, msg=f"IN")
        data = await request.json()
        self.LOG(2, 0, label="AUTH", label2=label2, msg=f"1")
        role,org = await self.enforce_admin_perm(request, label2=label2)
        self.LOG(2, 0, label="AUTH", label2=label2, msg=f"2, role={role}, org={org}")
        data = await request.json()
        ...
        self.LOG(2, 0, label="AUTH", label2=label2, msg=f"3")
        res_to_recs(res, ["org"])
