...
import orjson
import numpy as np
from dl2050utils.core import listify, oget, check_str, is_numeric_str, check, now
...
def convert_filter_val(val, max_sz=1024):
    """
    """
...
def parse_filter(f, max_sz=1024):
    ...
    The filter is valid if it contains both col and val attribute.
    col attribute must be string.
    val attributes can be string, int, float or lists of string, int, float for IN and NOT IN operators.
    ...
    # List values are only allowed for IN and NOT IN ops
    if op not in ['IN', 'NOT IN']: return None
    # Parse all list value elements
    val2 = []
    for e in val:
        e = convert_filter_val(e)
        if e is None: return None
        val2.append(e)
    return {'col':col, 'val':val, 'op':op}

def parse_filters(fs, tbl, max_sz=1024):
    """
    ...
    # Dict filters
    if type(fs) == dict: return [{"col":check_str(e, n=1024), "val":check(fs[e]), "op": "="} for e in fs]
    # List filters
    fs2 = []
    for f in fs:
...
def get_select_query_and_params(tbl, cols=None, filters=None, sfilters=None, join=None, lookups=None,
    ...
    fs1,sfs1 = parse_filters(filters, tbl),parse_filters(sfilters, tbl)
    fs,sfs = [],[]
    params = []
    k = 1
    for f in fs1:
...
class DB:
    ...
    async def insert(self, tbl, d, return_key=None):
        ...
        Returns:
            bool or value: False if the insertion was successful, True otherwise; if `return_key` specified, returns the key.
        """
        cols = list(d.keys())
    ...
    async def update_or_insert(self, tbl, ks, d):
        ...
        if row is None: return await self.insert(tbl, d)
