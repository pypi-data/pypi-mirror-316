...
class Auth:
    ...
    async def check_user(self, request):
        ...
        if user is None:
            log_and_raise_service(self.LOG, label="AUTH", label2=label2, msg=f"USER_NOT_REGISTERED: {email}", status_code=401)
        if user["ustatus"] == ACTIVE:
    ...
    async def get_users(self, request):
        ...
        label2 = "get_users"
        self.LOG(1, 0, label="AUTH", label2=label2, msg=f"IN")
        return rest_ok({"data": [], "nrows": 999})
        data = await request.json()
        self.LOG(1, 0, label="AUTH", label2=label2, msg=f"1")
        role,org = await self.enforce_admin_perm(request, label2=label2)
        self.LOG(1, 0, label="AUTH", label2=label2, msg=f"2, role={role}, org={org}")
        data = await request.json()
        ...
        self.LOG(1, 0, label="AUTH", label2=label2, msg=f"3")
        res_to_recs(res, ["org"])
