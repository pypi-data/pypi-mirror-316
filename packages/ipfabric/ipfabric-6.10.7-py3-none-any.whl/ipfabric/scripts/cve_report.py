...
def load_cve(ipf: IPFClient, nvd_api_key: str, timeout=60):
    vuln = Vulnerabilities(ipf=ipf, nvd_api_key=nvd_api_key, timeout=timeout)
...
def parse_cve(devices: Devices, cves):
    ...
    return (
        ...
        pd.DataFrame(
            error, columns=["device", "site", "serial", "ip", "vendor", "family", "version", "cpe_name", "error"]
        ),
    )
...
def main():
    ...
    if args.timeout and isinstance(args.timeout, str) and args.timeout.lower() == "none":
        args.timeout = None
    if args.filename:
    ...
    df_cve, df_clean, df_error = parse_cve(ipf.devices, load_cve(ipf, nvd_api_key, args.timeout))
    df_eol = load_eol(ipf)
