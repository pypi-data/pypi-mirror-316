...
class IPFClient(IPFabricAPI, BaseModel):
    ...
    def get_count(
        ...
        url, payload = self._fetch_setup(
            url, "json", None, snapshot_id, filters, reports, None, attr_filters, None, None
        )
    ...
    def share_link(  # noqa: C901
        ...
        Args:
            ...
            snasphot: Default False do not add selector, True use IPFClient snapshot, string specific snapshot.
        Returns:
            URL string
        """
        url = self._check_url(url)
