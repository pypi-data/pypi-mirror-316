...
class Setup(BaseSettings):
    ...
    def check_version(self, custom_api_version) -> tuple:
        ...
        """
        cfg_version = custom_api_version.lstrip("v").split(".")
        int_cfg_version = [int(_) for _ in cfg_version]
        ...
        os_api_version = resp["apiVersion"]
        int_os_api_version = [int(_) for _ in os_api_version.lstrip("v").split(".")]

        if int_os_api_version[0] != int_cfg_version[0]:
            raise RuntimeError(f"OS Major Version `{os_api_version}` does not match SDK Major Version `{api_version}`.")

        if int_cfg_version[0:2] > int_os_api_version[0:2]:
            logger.warning(
