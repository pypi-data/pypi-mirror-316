...
class DbtStepDecorator(StepDecorator):
    ...
    def step_init(
        ...
        # Do we need persisted state due to the selectors or not? Actual check has been postponed to task_decorate
        # due to re-evaluated attributes from Parameters affecting this.
        self.use_state = False

        # Make sure that any attribute passed as a Parameter has a valid default
        for k, v in self.attributes.items():
            if isinstance(v, Parameter) and "default" not in v.kwargs:
                raise MetaflowException(
                    "When defining @dbt attributes as Parameters, you need to supply a valid *default=* value as a fallback.\n"
                    "Provide a default value for the Parameter *%s*" % k
                )
    ...
    def task_decorate(
        ...
        # re-evaluate any attribute that was passed in as a Parameter
        for k, v in self.attributes.items():
            if isinstance(v, Parameter):
                # replace the attribute value with the actual parameter value
                self.attributes[k] = getattr(flow, v.name)

        # see if we need to retain state now with the re-evaluated models attribute
        self.use_state = self.attributes["models"] is not None and any(
