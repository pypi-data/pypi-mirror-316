...
from reboot.aio.tasks import Loop, TaskEffect
from reboot.aio.tracing import asynccontextmanager_span, function_span
...
from reboot.consensus.service_descriptor_validator import (
...
class SidecarStateManager(
    ...
    async def _load(
        ...
        *,
        ...
        state: Optional[Message] = self._states[state_type_name].get(state_ref)
        ...
        *,
        ...
        async def iterator(
            ...
            while True:
                ...
                # overwritten.
                state_copy = state_type()
                state_copy.CopyFrom(next_state)
    ...
    async def reactively(
        ...
        *,
        ...
        async def iterator(
        ) -> AsyncGenerator[tuple[Message, list[uuid.UUID]], None]:
    ...
    async def recover(
        ...
        *,
        ...
        # If there is a recovered file descriptor set, check for backwards
        # compatibility.
        if recover_response.HasField('file_descriptor_set'):
            validate_descriptor_sets_are_backwards_compatible(
                recover_response.file_descriptor_set,
                file_descriptor_set,
            )
        # We've passed any backwards compatibility checks, so we're safe to
        # overwrite the old file descriptor set with the new one.
        await self._sidecar_client.store(
            actor_upserts=[],
            task_upserts=[],
            colocated_upserts=[],
...
class LocalSidecarStateManager(SidecarStateManager):
    ...
    def __init__(
        ...
        try:
            ...
            # Capture a `SidecarServerFailed` as a cause of an
            # `InputError` so that we can properly identify it as a
            # _possible_ user issue when we handle it in
            # `applications.py`.
            raise InputError(
                reason="Failed to start sidecar server",
                causing_exception=e,
            )
