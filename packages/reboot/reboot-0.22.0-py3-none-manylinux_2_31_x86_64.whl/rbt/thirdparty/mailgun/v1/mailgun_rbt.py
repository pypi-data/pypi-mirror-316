...
class MessageServicerMiddleware(reboot.aio.internals.middleware.Middleware):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            state_type_name = reboot.aio.types.StateTypeName('rbt.thirdparty.mailgun.v1.Message'),
            service_names = [
                reboot.aio.types.ServiceName("rbt.thirdparty.mailgun.v1.MessageMethods"),
            ],
        ...
        self.request_type_by_method_name: dict[str, type[google.protobuf.message.Message]] = {
            ...
            'SendTask': rbt.thirdparty.mailgun.v1.mailgun_pb2.SendTaskRequest,
        }
        ...
        if self._token_verifier is not None and self._authorizer is None:
            raise RuntimeError(
                ...
                f"'rbt.thirdparty.mailgun.v1.Message'"
            )
    ...
    async def react_mutate(
        ...
        if method == 'Send':
            ...
            request = rbt.thirdparty.mailgun.v1.mailgun_pb2.SendTaskRequest()
            request.ParseFromString(request_bytes)
            ...
            while True:
                ...
                try:
                    ...
                    status = await rpc_status_async.from_call(call)
                    if status is not None:
                        raise Message.SendTaskAborted.from_status(
                            status
                        ) from None
                    raise Message.SendTaskAborted.from_grpc_aio_rpc_error(
                        error
                     ) from None
    ...
    async def dispatch(
        ...
        *,
        ...
        if 'Send' == task.method_name:
                ...
                async with self._state_manager.task_workflow(
                    ...
                    try:
                        response = await (MessageWorkflowStub(
                            ...
                            typing.cast(rbt.thirdparty.mailgun.v1.mailgun_pb2.SendRequest, task.request),
            ...
            if only_validate:
                # TODO(benh): validate 'task.request' is correct type.
                return rbt.thirdparty.mailgun.v1.mailgun_pb2.SendTaskResponse()
                ...
                async with self._state_manager.task_workflow(
                    ...
                    try:
                        response = await (MessageWorkflowStub(
                            ...
                            typing.cast(rbt.thirdparty.mailgun.v1.mailgun_pb2.SendTaskRequest, task.request),
                            bearer_token=__internal_magic_token__,
                            idempotency=reboot.aio.idempotency.Idempotency(
                                alias=f'Task {task.task_id.task_uuid}',
                            ),
                        ...
                        # Do not retry a task if the error was specified in the
                        # proto file.
                        if error_type in self._specified_errors_by_service_method_name.get('rbt.thirdparty.mailgun.v1.MessageMethods.SendTask', []):
                            await complete(task, (None, aborted.error))
                            return (None, aborted.error)
                        raise
    ...
    ) -> tuple[reboot.aio.contexts.WriterContext, rbt.thirdparty.mailgun.v1.mailgun_pb2.SendResponse]:
        ...
        *,
        ...
        try:
            ...
            assert_type(
                response,
                [
                    rbt.thirdparty.mailgun.v1.mailgun_pb2.SendTaskResponse,
                ],
            )
            # TODO: retrying a control loop might cause an effect
            # validation retry to either "hang" or "do more
            ...
            if isinstance(aborted, reboot.aio.aborted.SystemAborted):
                ...
                if not NODEJS:
                    logger.warning(
                        f"Unhandled (in 'rbt.thirdparty.mailgun.v1.Message.SendTask') {aborted}; propagating as 'Unknown'\n" +
                        ''.join(traceback.format_exception(aborted))
                    )
                raise reboot.aio.aborted.SystemAborted(
                    rbt.v1alpha1.errors_pb2.Unknown(),
                    ...
                    # debugging, it does expose implementation
                    # information.
                    message=f"unhandled (in 'rbt.thirdparty.mailgun.v1.Message.SendTask') {aborted}"
                )
            else:
                if (
                    aborted_type is not None and
                    ...
                    if context.task is None:
                        logger.warning(
                            f"Propagating unhandled (in 'rbt.thirdparty.mailgun.v1.Message.SendTask') {aborted}"
                        )
                elif (
                    aborted_type is None or
                    not isinstance(aborted, aborted_type)
            ...
            if not NODEJS:
                logger.warning(
                    f"Unhandled (in 'rbt.thirdparty.mailgun.v1.Message.SendTask') {type(exception).__name__}: {exception}; propagating as 'Unknown'\n" +
                    ''.join(traceback.format_exception(exception))
                )
            raise reboot.aio.aborted.SystemAborted(
                rbt.v1alpha1.errors_pb2.Unknown(),
                ...
                # debugging, it does expose implementation
                # information.
                message=f"unhandled (in 'rbt.thirdparty.mailgun.v1.Message.SendTask') {type(exception).__name__}: {exception}"
            )
    ...
    async def _SendTask(
        ...
        *,
        ...
        if idempotent_mutation is not None:
            response = rbt.thirdparty.mailgun.v1.mailgun_pb2.SendTaskResponse()
            response.ParseFromString(idempotent_mutation.response)
            return response
        ...
        async with self._state_manager.transactionally(
            ...
            async with self._state_manager.writer(
                ...
                self.tasks_dispatcher,
                authorize=self._maybe_authorize(
                    method_name='rbt.thirdparty.mailgun.v1.MessageMethods.SendTask',
                    headers=context._headers,
                    auth=context.auth,
                    request=request,
                ),
    ...
    async def _schedule_SendTask(
        ...
        *,
        ...
        headers: reboot.aio.headers.Headers,
        grpc_context: grpc.aio.ServicerContext,
    ) -> tuple[reboot.aio.contexts.WriterContext, rbt.thirdparty.mailgun.v1.mailgun_pb2.SendTaskResponse]:
        context: reboot.aio.contexts.WriterContext = self.create_context(
            headers=headers,
            state_type_name = reboot.aio.types.StateTypeName('rbt.thirdparty.mailgun.v1.Message'),
            context_type=reboot.aio.contexts.WriterContext,
    ...
    def _maybe_authorize(
        ...
        *,
    ...
    ) -> typing.Optional[typing.Callable[[typing.Optional[MessageStateType]], typing.Awaitable[None]]]:
        ...
        async def authorize(state: typing.Optional[MessageStateType]) -> None:
...
class _MessageStub(reboot.aio.stubs.Stub):

    __state_type_name__ = reboot.aio.types.StateTypeName('rbt.thirdparty.mailgun.v1.Message')
    ...
    async def Send(
        ...
        *,
        ...
        with self._idempotency_manager.idempotently(
            ...
            service_name=reboot.aio.types.ServiceName('rbt.thirdparty.mailgun.v1.MessageMethods'),
            method='Send',
            request=request,
            metadata=metadata,
            ...
            async with self._call(
                reboot.aio.types.StateTypeName('rbt.thirdparty.mailgun.v1.Message'),
                reboot.aio.types.ServiceName('rbt.thirdparty.mailgun.v1.MessageMethods'),
                'Send',
        ...
        *,
        ...
        with self._idempotency_manager.idempotently(
            ...
            async with self._call(
                ...
                reboot.aio.types.ServiceName('rbt.thirdparty.mailgun.v1.MessageMethods'),
                'SendTask',
                self._rbt_thirdparty_mailgun_v1_messagemethods_stub.SendTask,
                request,
                unary=True,
                reader=False,
                response_type=rbt.thirdparty.mailgun.v1.mailgun_pb2.SendTaskResponse,
                aborted_type=Message.SendTaskAborted,
                metadata=metadata,
                idempotency_key=idempotency_key,
                bearer_token=bearer_token,
                ...
                for (key, value) in await call.trailing_metadata():  # type: ignore[attr-defined]
                    if key == reboot.aio.headers.TASK_ID_UUID:
                        return tasks_pb2.TaskId(
                            state_type=reboot.aio.types.StateTypeName('rbt.thirdparty.mailgun.v1.Message'),
...
MessageStateType: typing.TypeAlias = rbt.thirdparty.mailgun.v1.mailgun_pb2.Message
MessageRequestTypes: typing.TypeAlias = \
        rbt.thirdparty.mailgun.v1.mailgun_pb2.SendRequest \
        | rbt.thirdparty.mailgun.v1.mailgun_pb2.SendTaskRequest

class MessageAuthorizer(
    reboot.aio.auth.authorizers.Authorizer[MessageStateType, MessageRequestTypes],
):
        ...
        *,
        ...
        if method_name == 'rbt.thirdparty.mailgun.v1.MessageMethods.Send':
            return await self.Send(
                context,
                typing.cast(rbt.thirdparty.mailgun.v1.mailgun_pb2.SendRequest, request),
            )
        elif method_name == 'rbt.thirdparty.mailgun.v1.MessageMethods.SendTask':
            return await self.SendTask(
                context,
                typing.cast(rbt.thirdparty.mailgun.v1.mailgun_pb2.Message, state),
                typing.cast(rbt.thirdparty.mailgun.v1.mailgun_pb2.SendTaskRequest, request),
            )
        else:
    ...
    # For 'rbt.thirdparty.mailgun.v1.MessageMethods.SendTask'.
    async def SendTask(
        self,
        context: reboot.aio.contexts.ReaderContext,
        state: rbt.thirdparty.mailgun.v1.mailgun_pb2.Message,
...
class MessageServicer(reboot.aio.servicers.Servicer):
    ...
    class Effects(reboot.aio.state_managers.Effects):
        def __init__(
            ...
            *,
            ...
            assert_type(state, [rbt.thirdparty.mailgun.v1.mailgun_pb2.Message])
    ...
    class SendEffects(Effects):
        def __init__(
            ...
            *,
            ...
            assert_type(state, [rbt.thirdparty.mailgun.v1.mailgun_pb2.Message])
            assert_type(response, [rbt.thirdparty.mailgun.v1.mailgun_pb2.SendResponse])
    ...
    class SendTaskEffects(Effects):
        def __init__(
            ...
            *,
            ...
            assert_type(state, [rbt.thirdparty.mailgun.v1.mailgun_pb2.Message])
            assert_type(response, [rbt.thirdparty.mailgun.v1.mailgun_pb2.SendTaskResponse])
    ...
    class _State:
                ...
                with context.idempotently(
                    ...
                    with servicer._middleware.use_context(
                        ...
                        if idempotent_mutation is not None:
                            ...
                            response.ParseFromString(idempotent_mutation.response)
                            result: MessageServicer.InlineWriterCallableResult = pickle.loads(response.value)
        ...
        @typing.overload
        def idempotently(self, *, key: uuid.UUID) -> MessageServicer._State._Idempotently:
            ...
...
Message_ScheduleTypeVar = typing.TypeVar('Message_ScheduleTypeVar', 'Message.WeakReference._Schedule', 'Message.WeakReference._WriterSchedule')
Message_IdempotentlyScheduleTypeVar = typing.TypeVar('Message_IdempotentlyScheduleTypeVar', 'Message.WeakReference._Schedule', 'Message.WeakReference._WriterSchedule')

class Message:

    Servicer: typing.TypeAlias = MessageServicer

    Effects: typing.TypeAlias = MessageServicer.Effects

    Authorizer: typing.TypeAlias = MessageAuthorizer

    State: typing.TypeAlias = rbt.thirdparty.mailgun.v1.mailgun_pb2.Message
    ...
    class SendFuture:
        ...
        def __await__(self) -> typing.Generator[None, None, rbt.thirdparty.mailgun.v1.mailgun_pb2.SendResponse]:
    ...
    SendEffects: typing.TypeAlias = Servicer.SendEffects

    class SendAborted(reboot.aio.aborted.Aborted):
    ...
    class SendTaskFuture:
        ...
        def __await__(self) -> typing.Generator[None, None, rbt.thirdparty.mailgun.v1.mailgun_pb2.SendTaskResponse]:
            """Awaits for task to finish and returns it's response."""

            async def wait_for_task() -> rbt.thirdparty.mailgun.v1.mailgun_pb2.SendTaskResponse:
                channel = self._channel_manager.get_channel_to_state(
                    self._task_id.state_type,
                    reboot.aio.types.StateRef(self._task_id.state_ref),
                )
                ...
                try:
                    ...
                    if (
                        ...
                        raise reboot.aio.aborted.SystemAborted(
                            ...
                            f"task with UUID {str(uuid.UUID(bytes=self._task_id.task_uuid))} "
                            f"has a response of type '{response_or_error.TypeName()}' "
                            "but expecting type 'rbt.thirdparty.mailgun.v1.mailgun_pb2.SendTaskResponse'; "
                            "are you waiting on a task of the correct method?",
                        ) from None
    ...
    SendTaskEffects: typing.TypeAlias = Servicer.SendTaskEffects

    class SendTaskAborted(reboot.aio.aborted.Aborted):
    ...
    class WeakReference(typing.Generic[Message_ScheduleTypeVar]):
        ...
        def _reader(
            ...
            if self._idempotency_manager is None:
                ...
                raise reboot.aio.call.MixedContextsError(
                    ...
                    f"`Message.lookup('{self.state_id}')`."
                )
        ...
        class _Idempotently(typing.Generic[Message_IdempotentlyScheduleTypeVar]):
            ...
            async def SendTask(
                ...
                __options__: reboot.aio.call.Options = reboot.aio.call.Options(),
                *,
                send_request: typing.Optional[rbt.thirdparty.mailgun.v1.mailgun_pb2.SendRequest] = None,
            ) -> rbt.thirdparty.mailgun.v1.mailgun_pb2.SendTaskResponse:
                if __options__.idempotency is not None:
                    raise RuntimeError(
                        'Found redundant idempotency in `Options`'
        ...
        class _Schedule:
            ...
            def __init__(
                self,
                tasks: typing.Callable[[reboot.aio.contexts.TransactionContext | reboot.aio.contexts.WorkflowContext | reboot.aio.external.ExternalContext], MessageTasksStub],
                *,
                when: typing.Optional[datetime | timedelta] = None,
                idempotency: typing.Optional[reboot.aio.idempotency.Idempotency] = None,
            ) -> None:
            ...
            async def SendTask(
                ...
                *,
                send_request: typing.Optional[rbt.thirdparty.mailgun.v1.mailgun_pb2.SendRequest] = None,
            ) -> reboot.aio.tasks.TaskEffect | Message.SendTaskFuture:
                if send_request is not None and not isinstance(
                    ...
                    raise TypeError(
                        ...
                        f"'rbt.thirdparty.mailgun.v1.mailgun_pb2.SendTaskRequest': field 'send_request' is not "
        ...
        async def SendTask(
            ...
            *,
            ...
            return await self._writer(__context__).SendTask(
                request,
                idempotency=idempotency,
                metadata=metadata,
                bearer_token=bearer_token,
    ...
    class _Construct:
        ...
        @typing.overload
        def idempotently(self, alias: typing.Optional[str] = None, *, each_iteration: bool = False) -> Message._ConstructIdempotently:
            ...

        @typing.overload
        def idempotently(self, *, key: uuid.UUID) -> Message._ConstructIdempotently:
            ...
