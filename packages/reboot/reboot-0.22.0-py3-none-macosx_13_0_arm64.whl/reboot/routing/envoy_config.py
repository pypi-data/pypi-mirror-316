...
def _http_filter_grpc_json_transcoder(
    ...
    if cluster_kind == ClusterKind.WEBSOCKET:
        # For websockets we don't need the transcoding filter; in fact it can only
        # confuse the traffic. Give it an emtpy configuration so it never activates.
        transcoder_config = transcoder_pb2.GrpcJsonTranscoder(
        ...
        assert cluster_kind == ClusterKind.GRPC
    ...
    typed_config = any_pb2.Any()
    typed_config.Pack(transcoder_config)
    return http_connection_manager_pb2.HttpFilter(
