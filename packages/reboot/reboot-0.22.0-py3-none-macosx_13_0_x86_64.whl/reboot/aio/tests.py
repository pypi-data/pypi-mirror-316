...
from reboot.aio.types import ConsensusId, ServiceName, StateRef
...
class Reboot:
    ...
    def __init__(
        ...
        *,
        ...
        global _initialize_tracing_once

        _initialize_multiprocessing_start_method_once()

        # Multiprocessing must have started before tracing starts, because tracing
        # spawns threads.
        ...
        *,
        ...
        if app_internal:
            if bearer_token is not None:
                raise ValueError(
                    "At most one of `app_internal` and `bearer_token` may be set."
                )
    ...
    async def up(
        ...
        *,
        ...
        an `ApplicationConfig` produced by a previous call to `up()` to restore
        the application to a previous configuration.
        ...
        if config is not None:
            if config.application_id() != self._application_id:
                raise ValueError(
                    "Config is for a different application than the one "
