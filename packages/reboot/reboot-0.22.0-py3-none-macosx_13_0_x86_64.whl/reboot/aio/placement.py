...
from reboot.aio.aborted import is_grpc_retryable_exception
...
class PlacementClient(ABC):
    ...
    @abstractmethod
    def known_service_names(self, application_id: str) -> list[ServiceName]:
        """
    ...
    @abstractmethod
    def known_consensuses(self, application_id: str) -> list[ConsensusId]:
        """
...
class StaticPlacementClient(PlacementClient):
    ...
    def known_state_type_names(self,
                               application_id: str) -> list[StateTypeName]:
        return []
...
class PlanOnlyPlacementClient(PlacementClient):
    ...
    async def wait_for_change(self):
        ...
        while True:
            self._raise_if_done()
            try:
                await asyncio.wait_for(super().wait_for_change(), timeout=1.0)
                return
            except asyncio.TimeoutError:
                # Not a problem, it's just time for us to check in on the health
                # of the control loop again before we continue to wait.
                continue
