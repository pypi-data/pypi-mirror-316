...
class StructDynSystemParm:
    def __init__(self, mass_values, damping_matrix, stiffness_matrix):
        ...
        if isinstance(mass_values, torch.Tensor):
            self.M0_tensor = mass_values.to(DEVICE)
        else:
            self.M0_tensor = torch.from_numpy(mass_values).to(DEVICE)

        if isinstance(damping_matrix, torch.Tensor):
            self.C0_tensor = damping_matrix.to(DEVICE)
        else:
            self.C0_tensor = torch.from_numpy(damping_matrix).to(DEVICE)

        if isinstance(stiffness_matrix, torch.Tensor):
            self.K0_tensor = stiffness_matrix.to(DEVICE)
        else:
            self.K0_tensor = torch.from_numpy(stiffness_matrix).to(DEVICE)

        self.M, self.C, self.K, self.dimensionsless_unit = self.non_dimensionalize()

    def non_dimensionalize(self):
        dimensionsless_unit = torch.max(self.M0_tensor).to(DEVICE)
