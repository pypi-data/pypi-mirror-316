...
class SVM_PtFunc:
    def __new__(cls, M, C, K, t, Pt, gamma, sigma2, InitialU=None, InitialU_dot=None, PHI=None):
        ...
        instance = super(SVM_PtFunc, cls).__new__(cls)

        # 初始化
        if isinstance(M, np.ndarray):
            dimensionsless_unit = np.max(M)
            instance.M = M /dimensionsless_unit
        else:
            dimensionsless_unit = np.max(M.numpy())
        ...
        # 将 time 转换为 torch.Tensor
        timeTensor = torch.tensor(time.reshape(-1, 1))
        pt_list = Pt(timeTensor)
        # 将 pt_list 中的所有张量转换为 numpy 数组，并进行列拼接
        P = torch.cat(pt_list, dim=1).cpu().numpy() /dimensionsless_unit
        ...
        return instance.Implement_SVMPt(instance.M, instance.C, instance.K, time, P, instance.InitialU, instance.InitialU_dot, PHI)
