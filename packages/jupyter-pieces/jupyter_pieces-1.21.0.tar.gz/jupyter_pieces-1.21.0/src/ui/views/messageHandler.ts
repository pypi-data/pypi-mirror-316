...
import { CopilotAppletMessageHandler } from './copilot/messageHandler';
import { copilotApplet } from './copilot';
...
export async function handleOnMessage(event: MessageEvent) {
  ...
  if (message.type === 'openSettings') {
    defaultApp.commands.execute('settingeditor:open');
    return;
  }
...
//   }
  ...
  //@ts-ignore
  const handlerFunction = messageTypeToFunction[messageType] || messageTypeToFunction.default;
