...
import { AppletExtensionMessageEnum, AppletWebviewMessageEnum, WebviewMsgExcludeUniEnum } from "./AppletMessageType.enum";
import { NotificationActionTypeEnum, NotificationParameters } from "./NotificationParameters";
...
const getErrorMessage = <T extends AppletWebviewMessageEnum>(type: T) =>
  `Pieces for Developers: Error ${webviewMessageEnumToErrorString[type] ?? 'unknown'}, please make sure that PiecesOS is installed, up to date, and running. If issues persist please contact support`;
...
export abstract class AppletMessageHandler {
	...
	handleCopilotFocus(message:AppletMessageData<AppletWebviewMessageEnum.FocusCopilotConversation>){
		defaultView.switchTab('gpt');
