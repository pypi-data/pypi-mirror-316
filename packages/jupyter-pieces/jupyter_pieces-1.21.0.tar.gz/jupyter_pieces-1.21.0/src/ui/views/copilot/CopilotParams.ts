...
import { AppletParams } from '../types/AppletParams';
import { NotificationAction, NotificationActionTypeEnum } from '../types/NotificationParameters';
import { AppletAnalytics } from '../types/AppletAnalytics.enum';
import { CopilotState } from '../types/ConversationState';
import { SavedMaterialsState } from '../types/SavedMaterialsState';
import { copilotApplet } from '.';
import { AppletMessageData } from '../types/AppletMessageData';
import { AppletWebviewMessageEnum } from '../types/AppletMessageType.enum';
...
export const copilotParams: AppletParams = {
	async updateApplication(application) {
		...
		const settingValue = application.capabilities === CapabilitiesEnum.Blended
			? 'Blended'
			: 'Local';
	...
	requestContextPicker: async (
		...
		if (paths) {
			const anchors = await Promise.all(
				paths.map((path) => ConnectorSingleton.getInstance().anchorsApi.anchorsCreateNewAnchor({
					transferables: false,
					seededAnchor: {
						type: type === 'folders'
	...
	getSavedMaterialsState: function (): Promise<SavedMaterialsState | undefined> {
		throw new Error('Function not implemented.');
	},
	focusCopilotConversation: function (conversation: string): void | Promise<void> {
