...
export const loadPieces = async (): Promise<returnedSnippet[]> => {
  //
  const config: ConnectorSingleton = ConnectorSingleton.getInstance();
  ...
  // sort the snippets by date created (desc)
...
const fetchSnapshot = async ({
  ...
  try {
    ...
    if (!retry) {
      ...
      notifications.error({
        message:
          'Failed to load snippets. Please restart PiecesOS, ensure that it is up-to-date, and try again. If the problem persists please reach out to support.',
        actions: [
          {
...
export const processAssets = ({ assets }: { assets: Asset[] }) => {
  const snippets: returnedSnippet[] = [];
...
export const processAsset = ({ asset }: { asset: Asset }): returnedSnippet => {
