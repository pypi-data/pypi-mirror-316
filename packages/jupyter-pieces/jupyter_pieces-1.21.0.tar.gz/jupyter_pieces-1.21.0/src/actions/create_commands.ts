...
import PiecesOSUpdating from '../ui/modals/PiecesOSUpdating';
...
export const createCommands = ({ palette }: { palette: ICommandPalette }) => {
  ...
  commands.addCommand("jupyter_pieces:debug", {
    label: 'Debug',
    caption: 'debug',
    execute: () => new PiecesOSUpdating().open(true)
  })
  palette.addItem({
    command: 'jupyter_pieces:debug',
...
const snippetizeNotebook = async () => {
  ...
  try {
    ...
    notifications.error({
      message:
        'Failed to snippetize notebook, are you sure PiecesOS is installed, running, and up to date?',
    });
  }
...
export const saveActiveCellToPieces = async () => {
  ...
  try {
    ...
    notifications.error({
      message:
        'Failed to save snippet to pieces, are you sure that PiecesOS is running?',
    });
  }
