...
def create_model_yml(
    ...
    if os.path.exists(yml_path) and not force:
        typer.echo(f"⏭️  Properties YML file for model '{model_name}' already exists at '{yml_path}'. Skipping creation.")
        return False
    ...
    if force and os.path.exists(yml_path):
        typer.echo(f"♻️  Model YML file for model '{model_name}' at '{yml_path}' created / has been overwritten.")
    else:
