...
def create_model_yml(
    ...
    if os.path.exists(yml_path) and not force:
        typer.echo(f"⏭️  Properties YML file for model '{alias}' already exists at '{yml_path}'. Skipping creation.")
        return False
    ...
    if force and os.path.exists(yml_path):
        typer.echo(f"♻️  YML file for {resource_type} '{model_name}' at '{yml_path}' created / has been overwritten.")
    else:
        typer.echo(f"✅  YML file for {resource_type} '{model_name}' created at '{yml_path}'")
    return True
...
def create_semantic_model_yml(
    ...
    - template_path: Path to a custom template for the semantic model .yml file.
    - custom_output_path: Custom directory path to store the .yml file.

    Returns:
    - bool: True if the file was created or updated, False if it was skipped.
    """
