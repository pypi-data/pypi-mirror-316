...
def earcut(vertices, holeIndices=None, dim=None):
    ...
    data = np.array(vertices).reshape(-1,2)
...
def _filter_points(start, end=None):
    ...
    while again or p != end:
        ...
        if not p.steiner and (_points_equal(p, p.next) or area(p.prev, p, p.next) == 0):
            _remove_node(p)
            p = end = p.prev
            if (p == p.next):
                return None
...
def earcutLinked(ear: "Node", triangles, dim, minX, minY, size, _pass=None):
    ...
    while ear.prev != ear.next:
        ...
        if ear == stop:
            ...
            if not _pass:
                earcutLinked(_filter_points(ear), triangles, dim, minX, minY, size, 1)
...
def _is_ear(ear):
    ...
    while p != ear.prev:
        if _point_in_triangle(a.x, a.y, b.x, b.y, c.x, c.y, p.x, p.y) and area(p.prev, p, p.next) >= 0:
                return False
        p = p.next
...
def isEarHashed(ear, minX, minY, size):
    ...
    while p and p.z <= maxZ:
        if p != ear.prev and p != ear.next and _point_in_triangle(a.x, a.y, b.x, b.y, c.x, c.y, p.x, p.y) and area(p.prev, p, p.next) >= 0:
            return False
        p = p.nextZ
...
def findHoleBridge(hole, outerNode):
    ...
    while p != stop:
        ...
        if hx >= p.x and p.x >= mx and _point_in_triangle(hx_or_qx, hy, mx, my, qx_or_hx, hy, p.x, p.y):
