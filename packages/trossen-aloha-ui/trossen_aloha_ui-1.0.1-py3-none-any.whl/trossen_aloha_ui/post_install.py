...
def check_conda_env(env_name="aloha_env"):
    ...
    if active_env != env_name:
        print(f"{BLUE} Current conda environment is '{active_env}'. Switching to '{env_name}'... {RESET}")
        conda_executable = os.environ.get("CONDA_EXE")
        if not conda_executable:
            raise EnvironmentError("Conda is not installed or not properly configured.")

        activate_script = Path(conda_executable).parent.parent / "etc" / "profile.d" / "conda.sh"
        if not activate_script.exists():
            raise EnvironmentError(f"Conda activation script not found at {activate_script}.")
...
def install_additional_packages():
    ...
    if not clone_dir.exists():
        ...
        run_command(["git", "clone", "--no-checkout", repo_url, str(clone_dir)])
    else:
        print(f"{YELLOW} Repository already exists at {clone_dir}. Fetching the latest changes... {RESET}")
        run_command(["git", "fetch"], cwd=str(clone_dir))

    # Step 2: Checkout the specific commit
    print(f"Checking out commit {commit_hash}...")
    run_command(["git", "checkout", commit_hash], cwd=str(clone_dir))
    ...
    run_command([sys.executable, "-m", "pip", "install", ".[intelrealsense,dynamixel]"], cwd=str(clone_dir))
...
def create_desktop_icon():
    ...
    application_script_path = site_packages_dir / "trossen_aloha_ui/application.sh"
    ...
    desktop_file_path = Path.home() / ".local/share/applications/trossen_aloha_ui.desktop"
    ...
    desktop_copy_path = Path.home() / "Desktop/trossen_aloha_ui.desktop"
    try:
