...
class iBot(iAgent):
    ...
    async def _get_session(self, url, **context) -> iSession:
        ...
        if session is None:
            ...
            session = self._sessions[uri["xhost"]] = await iSession.new(**context)
        return session
...
class SortPlugin(iPlugin):
    ...
    async def handle(self, stream: List[Dict], context: Dict):
        ...
        if not sort_key:
            ...
            for key in sort_key:
                value = sample[key]
                func = TYPES_MAP[value.__class__]
                candidates = {
                    k: v for k, v in sample.items() if k != key and func(sample[k]) == value
                }
                if candidates:
...
class iAsyncCrawler(iCrawler):
    ...
    async def inject(self, data, context):
        ...
        if item:
            ...
            context.update(data)
            result = all([await sync.put(item, **context) for sync in self.syncmodel])
