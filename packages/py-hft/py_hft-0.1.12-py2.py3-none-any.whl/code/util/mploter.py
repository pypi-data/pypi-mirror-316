...
class Plotor:
  ...
  @staticmethod
  def MultiPlot3(m, fig_name='default', show=False, prefix='', save_dir=os.environ['HOME']+'/MultiPlot', png_width=3, zoom=1.0, with_number = False, sc=1, full=False):
    ...
    ksize = len(m.keys())
    fig_size = ((15*zoom, 6*zoom) if not full else (18*zoom, 9*zoom)) if png_width > 1 else (8*zoom, 15*zoom)
    ...
    for count, (subfig_title, subfig_data) in enumerate(mitem):
      ...
      else: Plotor.plot_one(ax=this_ax, data=subfig_data, label='empty', mode='plot')
      this_ax.set_title(prefix+'@'+subfig_title); this_ax.grid(); this_ax.legend()
