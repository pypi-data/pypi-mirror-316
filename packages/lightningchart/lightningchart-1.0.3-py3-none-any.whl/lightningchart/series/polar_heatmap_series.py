...
from lightningchart.series import Series2D, ComponentWithPaletteColoring, SeriesWithIntensityInterpolation, Series
...
class PolarHeatmapSeries(
    ...
    def __init__(
        ...
        self.instance.send(self.id, 'addHeatmapSeries', {
            'chart': self.chart.id,
            'sectors': sectors,
            'annuli': annuli,
    ...
    def invalidate_intensity_values(self, values: list[list[int | float]], i_annulus: int = None, i_sector: int = None):
        ...
        self.instance.send(self.id, 'invalidateIntensityValuesPolar', {
            'iAnnulus': i_annulus,
            'iSector': i_sector,
            'values': values
        })
