...
def plot_text_with_heatmap(leap_data: LeapData, title: str) -> None:
    ...
    if heatmap is not None:
        ...
        x_pos = 0.1
        y_pos = 0.5
        for token, value in zip(text_data, heatmap):
            if token:
                color = plt.cm.jet(value)
                ax.text(x_pos, y_pos, token, fontsize=font_size, color=color, ha='left', va='center')
                x_pos += len(token) * 0.03 + 0.03  # Adjust x_pos for spacing between tokens
    else:
