...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="svg",
        help="File format of the generated cfg: svg, png, pdf, etc.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--cleanup", action="store_true", help="Remove DOT file")
    parser.add_argument(
    ...
    parser.add_argument(
        "--outfile", type=str, help="Base name of image file to generate."
    )
    parser.add_argument(
        "--diffable", action="store_true", help="Name of diffable file to generate."
    )
    args = parser.parse_args()
    os.environ["PY2CFG_SERVER_PORT"] = str(8000 if args.port is None else args.port)
    ...
    cfg = CFGBuilder(args.short).build_from_file(args.outfile, args.input_file)

    # Some options for wrapping:
    # cfg.build_visual(args.outfile, format='pdf', calls=True)
    # cfg.build_visual(args.outfile, format="svg", calls=True, show=False)
    # cfg.build_visual(args.outfile, format="png", calls=True, show=False)
