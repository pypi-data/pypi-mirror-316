...
def new_client(client, server):
    ...
    server.send_message(
        ...
        json.dumps(
            {
                "type": "key",
                "filepath": os.path.join("cfg", "key.svg"),
            }
        ),
    )
        ...
        if _pid == os.getpid():
            ...
            if _entry_frame is not None:
                ...
                if subcfg.outline_block(frame.f_lineno, blocks) < 0:
                    logger.warn(f"Failed to highlight block at {frame.f_lineno}")
                threading.Thread(
                    target=dispatch_cfg,
...
def wrap_user_exception(func, entry):
    def user_exception(dbg, frame, exc_tuple):
        if _pid == os.getpid():
            logger.info(f"exception: {frame.f_code.co_filename}:{frame.f_lineno}")
...
def debug_init(debug_class, entryfile, port=None) -> None:
    ...
    debug_class.user_return = wrap_user_return(debug_class.user_return, entryfile)
