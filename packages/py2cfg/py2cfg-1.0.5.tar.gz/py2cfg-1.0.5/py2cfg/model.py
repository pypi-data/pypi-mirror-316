...
class CFG(object):
    ...
    def stylize_node(self, block: Block, default: None = None) -> tuple:
        ...
        style = getattr(self, f"node_{name}", self._style_handler)(block, typeobject)
    ...
    def _build_visual(
        ...
        if self.entryblock is None:
            raise TypeError("Expected self.entryblock to be not None but type is None")
        self._visit_blocks(
            graph,
            self.entryblock,
            visited=set(),
    ...
    @staticmethod
    def fillcolor(block: Block, interactive: bool, color: str, default: str = ""):
        if interactive:
            if block.highlight:
                return color
            else:
    ...
    def build_visual(
        ...
        """
        if diffable:
            with open(f"{filepath}.ast", "w") as fp:
                for block in self:
                    print(
        ...
        return graph.render(filepath, view=show, cleanup=cleanup, directory=directory)
    ...
    def bsearch(self, lineno: int, lst: Optional[list] = None) -> Optional[Block]:
