...
def batch_run_cli(ctx, **kwargs):
    ...
    cluster_id = _batch_run(default_kwargs, **kwargs)
...
def _batch_run(default_kwargs, **kwargs) -> int | None:
    ...
    if n_task_workers:
        print(f"Running job array of {len(job_array_ids)} tasks on {n_task_workers} worker VMs...")
    else:
        print(f"Running job array of {len(job_array_ids)} tasks on 1 VM...")
    print(f"Each task will run: [green]{shlex.join(command)}[/green]")
