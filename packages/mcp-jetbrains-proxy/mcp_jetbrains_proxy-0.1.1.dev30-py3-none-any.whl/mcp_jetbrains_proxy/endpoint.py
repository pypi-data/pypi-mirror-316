...
class EndpointManager:
    ...
    async def get_endpoint(self) -> str:
        ...
        async with self._lock:
            ...
            if 'MCP_IDE_PORT' in os.environ:
                port = int(os.environ['MCP_IDE_PORT'])
                ...
                try:
                    ...
                    raise RuntimeError(f'MCP_IDE_PORT={port} is not responding')
            ...
            async with aiohttp.ClientSession():
                for port in range(self._port_range.start, self._port_range.end + 1):
                    ...
                    try:
                        ...
                        logger.debug(f'Endpoint {endpoint} returned status 404')
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if (
                    ...
                    logger.debug(f'Current tools: {current_response}')
                    logger.debug(f'Previous tools: {self._previous_tools_response}')
