...
from mcp_jetbrains_proxy import __version__, server_name
...
async def test_list_tools(mock_ide_server, mock_mcp_client):
    ...
    # Get tools list
    tools = await server._list_tools(f'http://localhost:{mock_ide_server["port"]}/api')
...
async def test_call_tool(mock_ide_server, mock_mcp_client):
    ...
    # Call tool
    content = await server._call_tool(f'http://localhost:{mock_ide_server["port"]}/api', 'test-tool', {'arg1': 'test'})
...
async def test_tools_changed_notification(mock_mcp_client, mock_ide_server):
    ...
    app.router.add_post('/api/mcp/call_tool', handle_call_tool)
    ...
    new_app.router.add_get('/api/mcp/list_tools', updated_list_tools)
    new_app.router.add_post('/api/mcp/call_tool', handle_call_tool)
...
async def test_invalid_tool_call(mock_ide_server, mock_mcp_client):
    ...
    with pytest.raises(RuntimeError, match='Failed to call tool: Unknown tool'):
        await server._call_tool(f'http://localhost:{mock_ide_server["port"]}/api', 'invalid-tool', {})
...
async def test_tool_error_handling(mock_ide_server, mock_mcp_client):
    ...
    with pytest.raises(RuntimeError, match='Failed to call tool: Tool execution failed'):
        await server._call_tool(f'http://localhost:{mock_ide_server["port"]}/api', 'test-tool', {'arg1': 'fail'})
...
async def test_concurrent_tool_calls(mock_ide_server, mock_mcp_client):
    ...
    async def call_tool(i: int):
        content = await server._call_tool(
            f'http://localhost:{mock_ide_server["port"]}/api', 'test-tool', {'arg1': f'test{i}'}
        )
        return content
...
async def test_list_tools_error_handling(mock_ide_server):
    ...
    app.router.add_get('/api/mcp/list_tools', health_check)
    ...
    server.endpoint_manager._endpoint = f'http://localhost:{mock_ide_server["port"]}/api'
    ...
    app2.router.add_get('/api/mcp/list_tools', list_tools)
