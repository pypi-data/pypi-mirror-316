...
class FunctionsResource(SyncAPIResource):
    ...
    def imageto3d(
        ...
        *,
        ...
        foreground_ratio: float | NotGiven = NOT_GIVEN,
        remesh_option: str | NotGiven = NOT_GIVEN,
        target_vertex_count: float | NotGiven = NOT_GIVEN,
        texture_resolution: float | NotGiven = NOT_GIVEN,
        # Use the following arguments if you need to pass additional parameters to the API that aren't available via kwargs.
        # The extra values given here take precedence over values defined on the client or passed to this method.
        extra_headers: Headers | None = None,
        extra_query: Query | None = None,
        ...
        Currently, requests can take \\~~2-5mins to complete.

        Args:
          image_request_id: The requestId from the /assets/create endpoint that the image was uploaded to

          foreground_ratio: Ratio of the foreground size to the image size.

          remesh_option: The remesh option to use for the generation. Allowed values are (none, triangle,
              quads)

          target_vertex_count: The desired polycount of the mesh output. If -1, no polycount reduction is
              performed.

          texture_resolution: Side length of the texture map in pixels (max 4096)
