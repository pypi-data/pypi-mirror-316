...
def main(target: str, name: str, user: str, run_type: Literal['normal', 'python_only', 'dotnet_only'], port: int, domain: str, whitelist_me: bool, dev_dll_path: str, via_proxy: bool):
    ...
    lp.ProviderManager(*providers, user=user, port=port, domain=domain, whitelist_me=whitelist_me, via_proxy=via_proxy, run_type=run_type, _dev_dll_path=dev_dll_path).run()
...
def run(target: str, name: str, user: str, port: int, domain: str, whitelist_me: bool, run_type: Literal['normal', 'python_only', 'dotnet_only'], dev_dll_path, via_proxy: bool):
    ...
    """
    main(target, name, user, run_type, port, domain, whitelist_me, dev_dll_path, via_proxy)
