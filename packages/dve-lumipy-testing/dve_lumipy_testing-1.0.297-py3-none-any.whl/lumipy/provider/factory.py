...
from lumipy.provider.common import get_dll_path, get_latest_local_python_provider_semver, get_certs_path, \
    find_dotnet_sdks, semver_has_dll
...
class Factory:
    ...
    def check_for_expired_certs(self):
        if self.expired_certs and self.startup_attempts < 2:
    ...
    def __print_process_output(self):
        ...
        try:
            for line in iter(self.process.stdout.readline, b''):
                ...
                if 'Client side SSL Certificate is PAST its expiry' in output:
