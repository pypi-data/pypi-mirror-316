...
from perin_parser.data.parser.to_mrp.abstract_parser import AbstractParser
...
class AMRParser(AbstractParser):
    def parse(self, prediction, approximate_anchors=False):
        ...
        output["id"] = self.dataset.id_field.vocab.itos[
            max(0, min(prediction["id"].item(), len(self.dataset.id_field.vocab.itos)))]
