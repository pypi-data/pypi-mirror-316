...
def check_probe_binding_chunk(sequences, reference_seqs, max_distance):
    ...
    Args:
        ...
        max_distance: Maximum allowed mismatches/gaps
    """
    results = []
...
def read_fastq_files(file_paths, subsample=None, reads=None, min_length=None, max_length=None, sampling_method='reservoir'):
    ...
    for file_path in file_paths:
        ...
        # First pass to count sequences if needed
        ...
        if reads is not None:
            target_reads = min(reads, total_seqs)
        elif subsample is not None:
            target_reads = int((subsample / 100.0) * total_seqs)
        else:
            target_reads = total_seqs

        with click.progressbar(length=target_reads,
                             label=f'Reading {os.path.basename(file_path)}',
                             show_percent=True,
                             show_pos=True) as bar:
            with opener(file_path, 'rt') as f:
                while processed_seqs < target_reads:
                    # Read chunk of FASTQ entries
                    chunk = []
                    for _ in range(4 * min(CHUNK_SIZE, target_reads - processed_seqs)):
                    ...
                    for i in range(0, len(chunk), 4):
                        ...
                        if processed_seqs >= target_reads:
                            break
        ...
        seq_counts.update(file_counter)
        total_sequences += processed_seqs

    return seq_counts, total_sequences
...
def check_probe_binding(probe_seq, reference_seq, max_distance):
    """Check if a probe sequence binds to a reference sequence using Biopython alignment.
    
    Args:
        probe_seq: The probe sequence to check (25bp optimized)
        reference_seq: The reference sequence to search against
        max_distance: Maximum allowed mismatches
    
    Returns:
        bool: True if probe binds within parameters, False otherwise
    """
    aligner = Align.PairwiseAligner()
    # Configure for short-read alignment like Bowtie
    aligner.mode = 'local'
    aligner.match_score = 1.0
    aligner.mismatch_score = -1.0
    aligner.open_gap_score = -10.0  # Strongly penalize gaps for short reads
    aligner.extend_gap_score = -10.0
    aligner.target_end_gap_score = 0.0
    aligner.query_end_gap_score = 0.0
    
    # Check both strands
    rc_probe = str(Seq(probe_seq).reverse_complement())
    for curr_probe in [probe_seq, rc_probe]:
        alignments = aligner.align(curr_probe, reference_seq)
        if alignments:
            best_alignment = alignments[0]
            mismatches = sum(a != b for a, b in zip(best_alignment[0], best_alignment[1]))
            if mismatches <= max_distance:
                return True
                
    return False

def reanalyze_results(results_file, new_elbow_point=None, num_probes=None, output_prefix=None):
    """Reanalyze results with a new elbow point or number of probes.
    
    Args:
        results_file: Path to original results TSV file
        new_elbow_point: Optional new elbow point for sequence selection
        num_probes: Optional number of probes to generate
    ...
    # Validate inputs
    if new_elbow_point is None and num_probes is None:
        raise ValueError("Either new_elbow_point or num_probes must be provided")
    
    if new_elbow_point is not None:
        ...
        if num_probes <= 0:
            raise ValueError("Number of probes must be positive")
        # Use all sequences but limit probe generation
    ...
    with click.progressbar(length=1, label='Writing new results file') as bar:
        with open(new_results_file, 'w') as f:
            f.write("Sequence\tReads\tCount\tPercentage\tCumulative_Percentage\tAligned\n")
            cumulative = 0
            for seq, (count, percentage) in selected_sequences:
                cumulative += percentage
                # Check if sequence aligns to reference if reference_file was provided
                aligned = "No"
                if reference_file and reference_seqs:
                    for ref_id, ref_seq in reference_seqs.items():
                        if check_probe_binding(seq, ref_seq, max_binding_dist):
                            aligned = "Yes"
                            break
                f.write(f"{seq}\t{count}\t{count}\t{percentage:.4f}\t{cumulative:.4f}\t{aligned}\n")
        bar.update(1)
    ...
    return selected_sequences, num_probes if num_probes else new_elbow_point
...
def write_probe_sequences(sorted_results, output_prefix, num_sequences=20, probe_length=25, min_probe_length=20, original_total_reads=None):
    ...
    final_probes = []  # Changed from set to list to maintain order
    uncovered_seqs = set(sequence_counts.keys())
    covered_reads = 0
    covered_seqs = 0
    
    while uncovered_seqs and probe_coverage:
        # Find probe that covers most uncovered reads
        best_probe = max(
            probe_coverage.keys(),
        ...
        # Calculate coverage for this probe
        newly_covered = probe_coverage[best_probe] & uncovered_seqs
        probe_read_coverage = sum(sequence_counts[seq] for seq in newly_covered)
        
        # Add the probe with its coverage info
        final_probes.append((best_probe, probe_read_coverage))
        covered_reads += probe_read_coverage
    ...
    # Sort probes by their coverage (descending)
    final_probes.sort(key=lambda x: x[1], reverse=True)
    
    # Write probes to file
    with open(f"{output_prefix}_probes.fasta", 'w') as f:
        for i, (probe, coverage) in enumerate(final_probes, 1):
            f.write(f">probe_{i} coverage={coverage}\n{probe}\n")
    
    return [probe for probe, _ in final_probes], covered_reads/filtered_total_reads, covered_reads/(original_total_reads or filtered_total_reads)
...
def process_fastq(input_files, output_prefix, min_length=None, max_length=None, 
    ...
    # Read and process FASTQ files
    seq_counts, total_sequences = read_fastq_files(
        input_files, 
        subsample=subsample, 
    ...
    # Read reference sequences if provided
    reference_seqs = {}
    if reference_file:
    ...
    if edit_distance > 0:
        ...
        seq_count_tuples = list(seq_counts.items())
        filtered_counts = cluster_sequences(seq_count_tuples, edit_distance)
    ...
    # Use elbow point to determine number of sequences to process
    num_sequences = max(num_sequences, elbow_point)  # Use either specified number or elbow point, whichever is larger
    click.echo(f"\nUsing elbow point of {elbow_point} sequences for probe generation...")
    
    # Generate probes using sequences up to the elbow point
    click.echo("\nGenerating probes...")
    probes, filtered_coverage, total_coverage = write_probe_sequences(
        sorted_results[:num_sequences],  # Use sequences up to the elbow point
        output_prefix, 
    ...
    # Generate hybridization report
    click.echo("\nAnalyzing probe hybridization...")
    write_hybridization_report(probes, output_prefix)
...
def write_hybridization_report(probes, output_prefix):
    """Generate report on probe self-hybridization and cross-hybridization.
    
    Args:
        probes: List of probe sequences
        output_prefix: Prefix for output files
    """
    # Define score thresholds for recommendations
    SELF_HYBRIDIZATION_THRESHOLDS = {
        'low': 10,     # Scores below this are good
        'medium': 20,  # Scores below this are acceptable
        'high': 30     # Scores above this are concerning
    }
    
    CROSS_HYBRIDIZATION_THRESHOLDS = {
        'low': 8,      # Scores below this are good
        'medium': 15,  # Scores below this are acceptable
        'high': 25     # Scores above this are concerning
    }
    
    output_file = f"{output_prefix}_hybridization.tsv"
    problem_probes = []
    
    with open(output_file, 'w') as f:
        # Write header
        f.write("Probe\tLength\tSelf_Hybridization_Score\tWorst_Cross_Hybridization\tWorst_Cross_Partner\tRecommendation\n")
        
        for probe in probes:
            # Calculate self-hybridization
            self_score, _ = calculate_hybridization_score(probe)
            
            # Calculate cross-hybridization with all other probes
            worst_cross_score = 0
            worst_partner = None
            
            for other_probe in probes:
                if other_probe != probe:
                    cross_score, _ = calculate_hybridization_score(probe, other_probe)
                    if cross_score > worst_cross_score:
                        worst_cross_score = cross_score
                        worst_partner = other_probe
            
            # Generate recommendation
            recommendation = get_probe_recommendation(
                probe, self_score, worst_cross_score, 
                SELF_HYBRIDIZATION_THRESHOLDS, 
                CROSS_HYBRIDIZATION_THRESHOLDS
            )
            
            # Track problematic probes
            if "Consider redesigning" in recommendation or "Warning" in recommendation:
                problem_probes.append((probe, self_score, worst_cross_score, recommendation))
            
            f.write(f"{probe}\t{len(probe)}\t{self_score}\t{worst_cross_score}\t{worst_partner}\t{recommendation}\n")
    
    # Generate summary statistics
    with click.progressbar(probes, label='Analyzing probe interactions') as probe_bar:
        scores = [calculate_hybridization_score(probe)[0] for probe in probe_bar]
    
    avg_self_score = sum(scores) / len(scores)
    max_self_score = max(scores)
    
    # Print analysis summary
    click.echo(f"\nHybridization Analysis Summary:")
    click.echo(f"  Average self-hybridization score: {click.style(f'{avg_self_score:.2f}', fg='yellow')}")
    click.echo(f"  Maximum self-hybridization score: {click.style(f'{max_self_score:.2f}', fg='yellow')}")
    
    # Print overall assessment
    overall_assessment = get_overall_assessment(avg_self_score, max_self_score, len(problem_probes), len(probes))
    click.echo(f"\nOverall Assessment:")
    click.echo(overall_assessment)
    
    # Print specific problems if any
    if problem_probes:
        click.echo(f"\nProbes Requiring Attention:")
        for probe, self_score, cross_score, recommendation in problem_probes:
            click.echo(f"  Probe: {click.style(f'{probe[:10]}...{probe[-10:]}', fg='yellow')}")
            click.echo(f"    Self-hybridization: {self_score:.1f}")
            click.echo(f"    Cross-hybridization: {cross_score:.1f}")
            click.echo(f"    {recommendation}")
    
    click.echo(f"\nDetailed hybridization report written to: {click.style(output_file, fg='blue')}")

def get_probe_recommendation(probe, self_score, cross_score, self_thresholds, cross_thresholds):
    """Generate specific recommendations for a probe based on its scores."""
    recommendations = []
    
    # Check GC content
    gc_content = (probe.count('G') + probe.count('C')) / len(probe) * 100
    if gc_content < 40 or gc_content > 60:
        recommendations.append(f"GC content ({gc_content:.1f}%) outside optimal range (40-60%)")
    
    # Self-hybridization assessment
    if self_score > self_thresholds['high']:
        recommendations.append("High self-hybridization potential - Consider redesigning")
    elif self_score > self_thresholds['medium']:
        recommendations.append("Moderate self-hybridization - Monitor performance")
    
    # Cross-hybridization assessment
    if cross_score > cross_thresholds['high']:
        recommendations.append("High cross-hybridization risk - Consider redesigning")
    elif cross_score > cross_thresholds['medium']:
        recommendations.append("Moderate cross-hybridization - Monitor performance")
    
    # Check for repetitive sequences
    if any(base * 4 in probe for base in 'ATGC'):
        recommendations.append("Contains repetitive sequence - May cause non-specific binding")
    
    if not recommendations:
        return "Probe design looks good"
    return "; ".join(recommendations)

def get_overall_assessment(avg_self_score, max_self_score, problem_count, total_probes):
    """Generate overall assessment of the probe set."""
    assessment = []
    
    # Calculate problem percentage
    problem_percentage = (problem_count / total_probes) * 100
    
    if problem_percentage == 0:
        assessment.append(click.style("✓ All probes pass hybridization criteria", fg='green'))
    elif problem_percentage < 10:
        assessment.append(click.style(f"! Minor concerns: {problem_percentage:.1f}% of probes need attention", fg='yellow'))
    else:
        assessment.append(click.style(f"⚠ Significant concerns: {problem_percentage:.1f}% of probes need attention", fg='red'))
    
    # Add specific recommendations
    if problem_count > 0:
        assessment.append("\nRecommended actions:")
        if problem_percentage > 20:
            assessment.append("- Consider adjusting probe design parameters")
        if max_self_score > 30:
            assessment.append("- Review probes with high self-hybridization scores")
        assessment.append("- Test problematic probes experimentally before use")
        assessment.append("- Consider using alternative probes for affected targets")
    
    return "\n".join(assessment)

def calculate_hybridization_score(seq1, seq2=None):
    """Calculate hybridization score between two sequences or self-hybridization.
    
    Args:
        seq1: First sequence
        seq2: Optional second sequence (if None, calculates self-hybridization)
    
    Returns:
        score: Estimated binding strength (higher means stronger binding)
        structure: Type of binding ('self' or 'cross')
    """
    # Simple scoring based on complementary base pairs
    complement = {'A': 'T', 'T': 'A', 'G': 'C', 'C': 'G'}
    
    if seq2 is None:
        # Self-hybridization check
        seq2 = ''.join([complement.get(base, 'N') for base in seq1[::-1]])
        structure = 'self'
    else:
        seq2 = ''.join([complement.get(base, 'N') for base in seq2[::-1]])
        structure = 'cross'
    
    # Calculate score using sliding window
    max_score = 0
        ...
        for j in range(len(seq2) - window_size + 1):
            ...
            for k in range(window_size):
                if seq1[i+k] == seq2[j+k]:
                    consecutive += 1
                    score += consecutive  # Reward consecutive matches more
                else:
                    consecutive = 0
            
            max_score = max(max_score, score)
    
    return max_score, structure