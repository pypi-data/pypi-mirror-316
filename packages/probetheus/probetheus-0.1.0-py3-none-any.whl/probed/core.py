...
def generate_probes(sequence, probe_length=25, min_probe_length=20):
    ...
    # Handle sequences shorter than probe_length but longer than min_probe_length
    if len(sequence) < probe_length:
        if len(sequence) >= min_probe_length:
            return [sequence]
        return []
    
    # Generate probes by sliding window
...
def write_probe_sequences(sorted_results, output_prefix, num_sequences=20, probe_length=25, min_probe_length=20):
    ...
    # Process top N sequences
    for seq, (count, percentage) in sorted_results[:num_sequences]:
    ...
    if sequences_skipped > 0:
        click.echo(f"Skipped {click.style(str(sequences_skipped), fg='yellow')} sequences "
                  f"shorter than {min_probe_length}nt")
    click.echo(f"These sequences represent {click.style(f'{total_percentage:.2f}%', fg='green')} "
               f"of total reads")
    click.echo(f"Probes written to: {click.style(output_file, fg='blue')}")
    
    return probes, total_percentage