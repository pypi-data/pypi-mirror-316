"""Command-line interface for PROBED."""
...
@cli.command()
@click.option('--input', '-i', required=True, multiple=True, help='Input FASTQ files (can be multiple)')
...
def process(input, output, min_length, max_length, skip_length_filter, edit_distance, top_n,
           ...
           reference, max_binding_dist, subsample):
    ...
    click.echo(click.style("Starting sequence processing...", fg='blue', bold=True))
    
    # Read and process sequences with length filtering included
    sorted_seqs = core.read_fastq_files(
        input,
        subsample=subsample,
    ...
    probes, probe_percentage = core.write_probe_sequences(
        optimal_sequences, 
        output_prefix,
        num_sequences=top_n,
    ...
    if reference:
        click.echo(click.style("\nChecking probe binding...", fg='blue'))
        reference_seqs = core.read_reference_sequences(reference)
        core.write_binding_results([seq for seq, _ in optimal_sequences], 
                                 reference_seqs, max_binding_dist, output_prefix)
