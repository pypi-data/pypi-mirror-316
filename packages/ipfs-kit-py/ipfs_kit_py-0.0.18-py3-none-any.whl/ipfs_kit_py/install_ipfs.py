...
class install_ipfs:
    ...
    def config_ipfs(self, **kwargs):
        if platform.system() == "Linux":
            self.config_ipfs_linux(**kwargs)
        elif platform.system() == "Darwin":
            # self.config_ipfs_darwin(**kwargs)
            pass
        elif platform.system() == "Windows":
            # self.config_ipfs_windows(**kwargs)
            pass
        else:
            raise Exception("Unsupported platform")	
        return None
                    
    def config_ipfs_windows(self, **kwargs):
        results = {}
        cluster_name = None
        secret = None
        disk_stats = None
    ...
    def config_ipfs_linux(self, **kwargs):
        results = {}
        cluster_name = None
        secret = None
        disk_stats = None
    ...
    def kill_process_by_pattern(self, pattern):
        if platform.system() == "Linux":
            return self.kill_process_by_pattern_linux(pattern)
        elif platform.system() == "Darwin":
            return self.kill_process_by_pattern_darwin(pattern)
        elif platform.system() == "Windows":
            return self.kill_process_by_pattern_windows(pattern)
    
    def kill_process_by_pattern_darwin(self, pattern):
        try:
            # Get process IDs using pgrep (more reliable on macOS)
            pid_cmd = f"pgrep -f {pattern}"
            ...
            for pid in pids:
                ...
                # Get process owner
                owner_cmd = f"ps -p {pid} -o user="
                try:
                    owner = subprocess.check_output(owner_cmd, shell=True).decode().strip()
                    
                    # Kill process if we own it or we're root
                    if owner == current_username or current_uid == 0:
            ...
            print(f"Error killing process by pattern {pattern}")
            print(e)
            return False
    
    def kill_process_by_pattern_windows(self, pattern):
        try:
            # Find processes matching the pattern
            find_cmd = f'tasklist /FI "IMAGENAME eq {pattern}*" /NH /FO CSV'
            process_list = subprocess.check_output(find_cmd, shell=True).decode()
            
            if process_list:
                # Split the CSV output and get process names
                processes = [line.split(',')[0].strip('"') for line in process_list.split('\n') if line]
    ...
    def kill_process_by_pattern_linux(self, pattern):
        pids = None
        try:
    ...
    def uninstall_ipfs_kit(self):
        if platform.system() == "Linux":
            return self.uninstall_ipfs_kit_linux()
        elif platform.system() == "Darwin":
            return self.uninstall_ipfs_kit_darwin()
        elif platform.system() == "Windows":
            return self.uninstall_ipfs_kit_windows()

    def uninstall_ipfs_kit_darwin(self, **kwargs):
        home_dir = os.path.expanduser("~")
        self.kill_process_by_pattern('ipfs')
    ...
    def uninstall_ipfs_kit_windows(self, **kwargs):
        home_dir = os.path.expanduser("~")
        self.kill_process_by_pattern('ipfs')
        self.remove_directory(self.ipfs_path)
        self.remove_directory(os.path.join(home_dir, '.ipfs-cluster-follow' ))
    ...
    def uninstall_ipfs_kit_linux(self, **kwargs):
        home_dir = os.path.expanduser("~")
        self.kill_process_by_pattern('ipfs')
        self.remove_directory(self.ipfs_path)
        self.remove_directory(os.path.join(home_dir, '.ipfs-cluster-follow' ))
