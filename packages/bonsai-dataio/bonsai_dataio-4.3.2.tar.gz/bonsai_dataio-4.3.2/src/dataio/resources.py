...
class CSVResourceRepository(ResourceRepository):
    ...
    def convert_dataframe(
        ...
        for column in data.columns:
            if column not in concordances:
                logger.info(
                    f"No concordance found for column {column}. Skipping {column}."
                )
                continue
    ...
    def load_with_classification(
        self, classifications: dict, **kwargs
    ) -> Tuple[pd.DataFrame, dict[str, set[str]]]:
        """
        ...
        return self.convert_dataframe(
            data, original_schema=schema_class, classifications=classifications
        )
