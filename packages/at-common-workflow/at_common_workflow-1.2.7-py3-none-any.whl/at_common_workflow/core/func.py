...
class Func:
    def __init__(self, func: Callable[..., Any]) -> None:
        ...
        self.func = func
        self.requires_context = hasattr(func, '__requires_context__')
    ...
    async def __call__(self, *args: Any, context: Optional[Context] = None, **kwargs: Any) -> Any:
        if self.requires_context:
            if context is None:
                raise ValueError("Context is required for this function")
            if iscoroutinefunction(self.func):
                result = await self.func(context, *args, **kwargs)
            else:
                result = await asyncio.get_event_loop().run_in_executor(
                    None, 
                    functools.partial(self.func, context, *args, **kwargs)
                )
        else:
            if iscoroutinefunction(self.func):
    ...
    def to_meta(self) -> MetaFunc:
        ...
        return MetaFunc(
            ...
            args=Schema({k: v for k, v in annotations.items() if k != 'return'}),
            rets=Schema(rets)
        )
