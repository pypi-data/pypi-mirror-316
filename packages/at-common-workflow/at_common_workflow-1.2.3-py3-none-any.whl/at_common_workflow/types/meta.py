...
class AllowedTypes(Enum):
    ...
    def get_types(cls) -> tuple:
        """Get tuple of all allowed base types and their generic variants."""
        base_types = tuple(member.value for member in cls)
        scalar_types = (str, int, float, bool)
        generic_types = (
            *(list[t] for t in scalar_types),
            *(dict[str, t] for t in scalar_types)
        )
        return base_types + generic_types
    
    @classmethod
    def get_type_map(cls) -> Dict[str, Type]:
        """Get mapping of type names to type objects, including generic types."""
        return {
            **{member.value.__name__: member.value for member in cls},
            **{
                f"{get_origin(t).__name__}[{', '.join(arg.__name__ for arg in t.__args__)}]": t 
    ...
    def _validate(self, schema: Dict[str, type]) -> None:
        ...
        for key, value in schema.items():
            ...
            if not isinstance(value, type):
                raise TypeError(f"Schema value for '{key}' must be a type")
