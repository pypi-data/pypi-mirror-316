...
def generate_config_ui(default_config_file: str,
    ...
    :param data_dir: 用户的配置数据存放的目录路径，程序会将用户的配置数据存放在该目录下的 user_config.json 文件中，如果为 None，程序会在当前目录下自动创建一个 "data" 目录
    :param app: 已存在的 FastAPI 应用
    ...
    if not (default_config_file.endswith(".json") or default_config_file.endswith(".yaml")):
    ...
    if generate_readme:  # 复制默认的 config_ui_readme.md 文件到 data_dir 目录下
        readme_file = os.path.join(data_dir, "config_ui_readme.md")
        if not os.path.exists(readme_file):  # 防止重复复制
            log.info(f"复制 config_ui_readme.md 文件到 {data_dir}")
            shutil.copy(os.path.join(ROOT_PATH, 'templates/html/config_template/config_ui_readme.md'),
                        os.path.join(data_dir, "config_ui_readme.md"))
