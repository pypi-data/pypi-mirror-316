...
class XunleiCloudDisk(XunleiBase):
    ...
    async def yunpan_clear_ads(self, async_callback: Callable[[Dict[str, Any], ...], Awaitable[None]],
        ...
        :param async_callback: 异步回调函数的第一个参数用于接收文件/文件夹信息，如果有多个参数，其他的参数必须有默认值，否则会报错。处理广告的 异步 回调函数，回调函数会接收到1个包含文件信息的字典，包含：file_name, file_id, real_path, is_folder 等字段
    ...
    async def yunpan_create_download_task(self, download_url: str, target_folder: str,
                                          ...
                                          sub_file_index: Optional[List[Union[int, str]]] = None) -> Optional[bool]:
        """
