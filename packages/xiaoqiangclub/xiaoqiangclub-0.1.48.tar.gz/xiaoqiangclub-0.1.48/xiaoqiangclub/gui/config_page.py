...
def add_config_routes(app: FastAPI, config_file: str, data_dir: str):
    """
    向现有 FastAPI 应用中添加应用配置页面路由。
    说明文档：xiaoqiangclub/templates/html/config_template/config_readme.md

    :param app: 已存在的 FastAPI 应用
    :param config_file: 配置文件路径
    :param data_dir: 数据目录路径
    """
    if not os.path.exists(config_file) or not os.path.exists(data_dir):
        raise FileNotFoundError("配置文件或数据目录不存在。")
            ...
            if isinstance(value, dict):
                ...
                for inner_key, inner_value in value.items():
                    ...
                    if isinstance(inner_value, dict):
                        orders.append(inner_value.get('order', float('inf')))  # 如果没有 'order'，设为无限大
                    else:
    ...
    async def get_settings_page(request: Request):
        ...
        return config_page_templates.TemplateResponse("config_page.html",
    ...
    async def reset_settings():
        """重置配置"""
        initial_config = await read_file_async(config_file)
