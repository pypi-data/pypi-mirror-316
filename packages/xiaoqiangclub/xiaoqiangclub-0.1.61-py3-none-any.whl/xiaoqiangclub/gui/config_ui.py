...
def generate_config_ui(default_config_file: str,
    ...
    if user_config_dir_or_file.endswith((".json", ".yaml", ".yml")):
        # 获取文件所在目录
        config_dir = os.path.dirname(user_config_dir_or_file)
        user_config_file = user_config_dir_or_file

    else:
        config_dir = os.path.join(os.getcwd(), "data")
        os.makedirs(user_config_dir_or_file, exist_ok=True)

        # 生成和 default_config_file 相同后缀的 user_config_file
        user_config_file_suffix = os.path.splitext(default_config_file)[1]
        user_config_file = os.path.join(config_dir, f"default_config{user_config_file_suffix}")

        # 如果没有用户配置文件，复制默认配置
        if not os.path.exists(user_config_file):
            shutil.copy(default_config_file, user_config_file)

    if generate_readme:  # 复制默认的 config_ui_readme.md 文件到 user_config_dir_or_file 目录下
        readme_file = os.path.join(config_dir, "config_ui_readme.md")
        if not os.path.exists(readme_file):  # 防止重复复制
            log.info(f"复制 config_ui_readme.md 说明文档到 {user_config_dir_or_file}，第一次生成需退出项目后可见！")
