...
# 起始配置文件路径，修改为你的配置文件路径
config_file = "config_page.json"  # 修改为你的其实默认配置文件路径
...
if not os.path.exists(user_config_file):
    ...
    # 复制 config_file 到 data 目录为 user_config.json
...
def sort_dict_by_order(dict_data: Dict[str, Any]) -> Dict[str, Any]:
    """
...
async def get_settings_page(request: Request):
    ...
    return templates.TemplateResponse("config_page.html",
...
async def save_settings(user_settings: dict):
    ...
    settings = sort_dict_by_order(user_settings)
...
async def reset_settings():
    """重置配置"""
    # 读取初始配置
    initial_config = await read_file_async(config_file)  # 假设config_file是初始设置的文件
