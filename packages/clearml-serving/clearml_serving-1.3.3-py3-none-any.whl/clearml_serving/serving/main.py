...
try:
    # General errors verbosity: whitespace-separated list of verbose items
    # err/error - print error details
    # req/request - print request
    # stack/stacktrace - print stack trace
    verbose_general_errors = set(shlex.split(os.environ.get("CLEARML_SERVING_VERBOSE_GENERAL_ERRORS", default="")))
except (ValueError, TypeError):
    verbose_general_errors = set()
...
async def serve_model(model_id: str, version: Optional[str] = None, request: Union[bytes, Dict[Any, Any]] = None):
    try:
        ...
        if "CUDA out of memory. " in str(ex) or "NVML_SUCCESS == r INTERNAL ASSERT FAILED" in str(ex):
            raise CUDAException(exception=str(ex))
        else:
            raise HTTPException(status_code=422, detail="Error [{}] processing request: {}".format(type(ex), ex))
        ...
        msg = "[{}] Exception [{}]".format(instance_id, type(ex))
        if verbose_general_errors:
            if verbose_general_errors.intersection(("err", "error")):
                msg += " {}".format(ex)
            if verbose_general_errors.intersection(("req", "request")):
                msg += " while processing request: {}".format(request)
            if verbose_general_errors.intersection(("stack", "stacktrace")):
                msg += "\n{}".format("".join(traceback.format_exc()))
        session_logger.report_text(msg)
        raise HTTPException(status_code=500, detail="Error [{}] processing request: {}".format(type(ex), ex))
    return return_value
