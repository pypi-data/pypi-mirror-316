...
class Worksheet(Item):
    ...
    def push(self, session: Session, pushed_workbook_id, item_map, datasource_output, existing_worksheet_identifiers,
             include_inventory, include_annotations, access_control, label=None, status: Status = None):
    ...
    def push(self, session: Session, pushed_workbook_id, item_map, datasource_output, existing_worksheet_identifiers,
        ...
        if include_annotations:
            self.journal.push(session, pushed_workbook_id, worksheet_output.id, item_map, datasource_output,
                              access_control, push_images=True, label=label)

        return worksheet_output

    def refresh_from(self, new_item, item_map: ItemMap, status: Status, session: Session):
        super().refresh_from(new_item, item_map, status, session)

        self.journal.refresh_from(new_item.journal, item_map, status, session)
...
class TopicDocument(Worksheet):
    ...
    def push(self, session: Session, pushed_workbook_id, item_map, datasource_output, existing_worksheet_identifiers,
        ...
        worksheet_output = super().push(session, pushed_workbook_id, item_map, datasource_output,
                                        ...
                                        access_control, label=label, status=status)

        if include_annotations:
            self.report.push(session, pushed_workbook_id, worksheet_output.id, item_map, datasource_output,
                             access_control, push_images=True, label=label, status=status)
    ...
    def refresh_from(self, new_item, item_map: ItemMap, status: Status, session: Session):
        ...
        self.report.refresh_from(new_item.report, item_map, status, session)
