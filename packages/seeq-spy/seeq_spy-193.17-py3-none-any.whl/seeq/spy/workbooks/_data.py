...
class StoredOrCalculatedItem(Item):
    ...
    def get_metadata_to_push(self, session: Session, datasource_maps, datasource_output, *, pushed_workbook_id=None,
                             item_map: ItemMap = None, label=None, override_max_interp=False,
                             scope_globals_to_workbook=False, dummy_items_workbook_context=None,
...
class StoredItem(StoredOrCalculatedItem):
    ...
    def get_metadata_to_push(self, session: Session, datasource_maps, datasource_output, *, pushed_workbook_id=None,
                             ...
                             item_search_preview: ItemSearchPreviewV1 = None):
        try:
            self._lookup(session, datasource_maps, datasource_output, pushed_workbook_id=pushed_workbook_id,
    ...
    def _push_dummy_item(self, label: str, item_map: ItemMap, datasource_output: DatasourceOutputV1,
                         dummy_items_workbook_context):
        ...
        metadata_dict['Data ID'] = self._construct_data_id(label, self.definition['ID'])
    ...
    def _push_local_item(self, label: str, item_map: ItemMap, datasource_output: DatasourceOutputV1,
                         pushed_workbook_id: str, item_exists: bool = False):
        metadata_dict = self.definition_dict.copy()

        metadata_dict['Dummy Item'] = (metadata_dict.get('Type') != 'Asset')

        if self.definition.get('Parent ID') == Item.ROOT:
            del metadata_dict['Parent ID']
        ...
        if not item_exists:
            ...
            if 'Parent ID' in metadata_dict:
                metadata_dict['Parent Data ID'] = self._construct_data_id(label, self.definition['Parent ID'])
                del metadata_dict['Parent ID']
    ...
    def _lookup(self, session: Session, datasource_maps, datasource_output, *, pushed_workbook_id=None,
                item_map: ItemMap = None, label=None, override_max_interp=False, dummy_items_workbook_context=None,
                item_exists: ItemExists = ItemExists.MAYBE, item_search_preview: Optional[ItemSearchPreviewV1] = None):
        ...
        if item is None:
            if local:
                self._push_local_item(label, item_map, datasource_output, pushed_workbook_id)
            else:
                if dummy_items_workbook_context is None:
                    raise SPyDependencyNotFound(item_map.explain(self.id))

                self._push_dummy_item(label, item_map, datasource_output, dummy_items_workbook_context)
        else:
            if local:
                ...
                if item_is_scoped_to_other_workbook or self_already_exists_and_may_need_update:
                    self._push_local_item(
                        label, item_map, datasource_output, pushed_workbook_id,
                        self_already_exists_and_may_need_update)
...
class CalculatedItem(StoredOrCalculatedItem):
    ...
    def get_metadata_to_push(self, session: Session, datasource_maps, datasource_output, *, pushed_workbook_id=None,
        ...
        item = self._lookup(session, datasource_maps, pushed_workbook_id, item_map, label, item_exists,
                            item_search_preview)
        ...
        if (label is None or self.provenance == Item.CONSTRUCTOR) and item_output is not None:
            ...
            if (metadata_dict['Datasource Class'] == datasource_output.datasource_class and
                ...
                if 'Parent ID' in self.definition:
                    metadata_dict['Parent Data ID'] = self._construct_data_id(label, self.definition['Parent ID'])
        else:
            del metadata_dict['ID']
            metadata_dict['Datasource Class'] = datasource_output.datasource_class
            metadata_dict['Datasource ID'] = datasource_output.datasource_id
            metadata_dict['Data ID'] = self._construct_data_id(label)
