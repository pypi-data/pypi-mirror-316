...
class Report(Annotation):
    ...
    def pull(self, session: Session, include_images=True, status=None):
        ...
        self._definition.update(Item._dict_via_attribute_map(annotation_output, {
    ...
    def _refresh_child_dict(existing_item_dict: dict, new_item_dict: dict, item_map: ItemMap, status: Status,
        ...
        for _original_id, existing_item in items:
            ...
            # Refresh item itself
            existing_item.refresh_from(new_item, item_map, status, session)

    def refresh_from(self, new_item, item_map: ItemMap, status: Status, session: Session):
        super().refresh_from(new_item, item_map, status, session)

        Report._refresh_child_dict(self.date_ranges, new_item.date_ranges, item_map, status, session)
        Report._refresh_child_dict(self.asset_selections, new_item.asset_selections, item_map, status, session)
        Report._refresh_child_dict(self.content, new_item.content, item_map, status, session)
