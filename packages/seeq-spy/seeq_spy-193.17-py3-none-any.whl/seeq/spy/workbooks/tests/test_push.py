...
def test_push_with_inventory_modifications():
    ...
    spy.workbooks.push(workbook, path=label, label=label, datasource=label, refresh=True)
...
def test_globals():
    ...
    #
    # Make sure a global item is globally scoped if scope_globals_to_workbook=False
    #
...
def test_push_tree_with_mixed_scopes():
    test_name = 'test_push_tree_with_mixed_scopes'
    ...
    try:
        ...
        spy.workbooks.push(workbooks_2, datasource=test_name, refresh=True)
        ...
        trees_api.remove_node_from_tree(id=signal_2_id)
