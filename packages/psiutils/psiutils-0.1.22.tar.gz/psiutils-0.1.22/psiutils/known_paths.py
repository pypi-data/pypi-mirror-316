"""
Various path utilities including:
    return to path to a Windows known folder.

Obtained from : https://gist.github.com/mkropat/7550097
...
def get_downloads_dir() -> Path:
    ...
    if platform.system() == 'Windows':
        from known_paths import get_path
        return get_path('Downloads')
    return ''
"""
...
def get_path(folder: str, user_handle: Any = UserHandle.common) -> str | None:
    fid = GUID(getattr(folder_id, folder))
    pPath = ctypes.c_wchar_p()
    S_OK = 0
    if _SHGetKnownFolderPath(
            ctypes.byref(fid), 0, user_handle, ctypes.byref(pPath)) != S_OK:
...
def get_downloads_dir() -> Path | str:
    """Return the path to Downloads depending on os."""
    return _get_known_path('Downloads')


def get_documents_dir() -> Path | str:
    """Return the path to Downloads depending on os."""
    return _get_known_path('Documents')


def _get_known_path(directory: str) -> Path | str:
    """Return the path to  a given directory depending on os."""
    if platform.system() == 'Linux':
        return Path(Path.home(), directory)
    return get_path(directory) if platform.system() == 'Windows' else ''


def resolve_path(path: str, root: Path | str = None) -> str:
    """Return the path depending on whether the app is frozen."""
    if getattr(sys, 'frozen', False):
        return os.path.abspath(Path(sys._MEIPASS, path))
    if root:
        return os.path.abspath(Path(Path(root).parent.parent, path))
    return os.path.abspath(Path(Path(__file__).parent.parent.parent, path))
