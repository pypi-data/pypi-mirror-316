...
class MyWidget(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        with open(os.path.join(os.path.dirname(__file__), 'cash', 'key.txt'), 'r') as file:
            self.key = file.read()
        self.count = 0
        ...
        try:
            ...
            if h:
                ...
                for i in range(len(self.actions)):
                    self.delActions(i)
                self.actions.clear()
                shutil.copy(h, os.path.join(os.path.dirname(__file__), 'templates', 'base.html'))
                ...
                self.webEngineView.setUrl(QUrl.fromLocalFile(os.path.join(os.path.dirname(__file__), 'templates', 'base.html')))
                self.toMainWidget()
    ...
    def play(self):
        if self.key == '':
            ...
            benefer.site.save(os.path.join(os.path.dirname(__file__), 'templates', 'base.html'))
            os.system(f'start /min python server.py')
            subprocess.run([fr'{os.path.join(os.path.dirname(__file__))}\ngrok', 'http', '5000'], check=True)
    ...
    def textSettings(self):
        if self.sender() == self.soderzhanie_button:
            ...
            try:
                ...
                self.site.save(os.path.join(os.path.dirname(__file__), 'templates', 'base.html'))
                self.webEngineView.setUrl(QUrl.fromLocalFile(os.path.join(os.path.dirname(__file__), 'templates', 'base.html')))
            except Exception as e:
                logging.warning(e)
    ...
    def hideListOfAction(self):
        self.count = 0
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(os.path.join(os.path.dirname(__file__), 'Picture', 'Actions.png')), QtGui.QIcon.Normal, QtGui.QIcon.Off)
    ...
    def newAction(self, mode, settings):
        ...
        if self.listOfActions.isHidden():
            ...
            if self.count < 9:
                icon = QtGui.QIcon()
                icon.addPixmap(QtGui.QPixmap(os.path.join(os.path.dirname(__file__), 'Picture',
                                                          f'Actions{self.count}.png')),
                                                          QtGui.QIcon.Normal, QtGui.QIcon.Off)
                self.ActionsButton.setIcon(icon)
            else:
                icon = QtGui.QIcon()
                icon.addPixmap(QtGui.QPixmap(os.path.join(os.path.dirname(__file__), 'Picture',
                                                          'Actions9.png')), QtGui.QIcon.Normal, QtGui.QIcon.Off)
                self.ActionsButton.setIcon(icon)
    ...
    def delActions(self, index):
        ...
        if self.listOfActions.isHidden():
            ...
            if self.count == 0:
                ...
                icon = QtGui.QIcon()
                icon.addPixmap(QtGui.QPixmap(os.path.join(os.path.dirname(__file__), 'Picture',
                                                          f'Actions{self.count}.png')), QtGui.QIcon.Normal, QtGui.QIcon.Off)
                self.ActionsButton.setIcon(icon)
            else:
                icon = QtGui.QIcon()
                icon.addPixmap(QtGui.QPixmap(os.path.join(os.path.dirname(__file__), 'Picture',
    ...
    def closeEvent(self, event):
        ...
        for f in files:
            os.remove(f)
        with open(os.path.join(os.path.dirname(__file__), 'templates', 'base.html'), 'w'):
            pass
        sys.exit(0)
...
class KeyWidget(QWidget, Ui_Key):
    ...
    def get_key(self):
        if self.lineEdit.text():
            ...
            with open(os.path.join(os.path.dirname(__file__), 'cash', 'key.txt'), 'w') as file:
