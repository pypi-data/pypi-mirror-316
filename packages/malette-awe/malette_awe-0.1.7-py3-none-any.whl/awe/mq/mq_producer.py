...
class MQProducer:
  ...
  def send_complete_message(self, message: ResponseMessage, payload: Payload):
    message_body = {
      "code": message.code if message.code else "0",
      "status": "SUCCEED",
      "success": True,
      "taskId": payload.taskId,
      "userId": payload.userId,
    ...
    if 'data' in message.response and 'images' in message.response['data'] and 'output' in message.response['data']['images']:
      message_body['results'] = message.response['data']['images']['output']
    self.publish_message(json.dumps(message_body), tag=payload.messageTag)
