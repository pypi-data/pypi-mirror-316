...
async function pullModel(modelName, userURL) {
    //** 
    // * Function that pulls ollama (LLaVA for our case) model.
    // */ 
    try {
...
async function fetchImageAsBase64(imageUrl) {
    //** 
    // * Function that fetches image from url and return in base64 format.
    // */ 
...
async function sendImageToModel(imageData, userURL) {
    //** 
    // * Function that sends and retrieves a chat completion to ollama model (LLaVA).
    // */ 
    try {
...
class AltCellList extends _lumino_widgets__WEBPACK_IMPORTED_MODULE_3__.Widget {
    ...
    async handleAltTextToggle(altTextToggle, altTextToggleLabel) {
        ...
        if (!this._ollamaDownloaded) {
            altTextToggleLabel.textContent = "Please wait, pulling LLaVA model.";
    ...
    addCell(cellId, buttonContent, buttonId) {
        ...
        spinnerDiv.style.marginLeft = '10px';
        spinnerDiv.style.display = 'flex';
        spinnerDiv.style.flexDirection = 'row';
        spinnerDiv.style.alignItems = 'center';
        spinnerDiv.style.display = 'none';
        const spinnerLabel = document.createElement('label');
        spinnerLabel.textContent = "Please wait, receiving alt-text from LLaVA model.";
        ...
        if (add) {
            ...
            spinnerDiv.appendChild(spinnerLabel);
            spinnerDiv.appendChild(spinner);
            listItemWrapper.appendChild(spinnerDiv);
    ...
    applyAltText(cellId, altTextContent) {
        //** 
        // * Function that injects alt-text to Jupyter Notebook cell.
        // */ 
        var _a;
...
//# sourceMappingURL=lib_index_js.6d6eb9582f46d7a2158c.js.map