...
from icflow.utils import RuntimeContext, TimerProfiler, ProfilerCollection

import iclearn.session
from iclearn.utils.torch import TorchProfiler
...
def setup_session(dataset_func: Callable, model_func: Callable, config: Config):
    ...
    logger.info("Initial config is: %s", config)
    ...
    session = iclearn.session.Session(model, ctx, config.result_dir, dataset)
