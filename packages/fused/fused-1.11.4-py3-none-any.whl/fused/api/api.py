...
from fused.models.udf.udf import AnyBaseUdf, RootAnyBaseUdf
...
class FusedAPI:
    ...
    def _get_udf(
        ...
        if id is None:  # first arg is id
            id = email_or_handle_or_id
            email = self._whoami()["email"]
            url = f"{self.base_url}/udf/by-user-email/{email}/by-slug/{id}"
        else:
            if "@" in email_or_handle_or_id:  # all valid email addresses contain @
                url = f"{self.base_url}/udf/by-user-email/{email_or_handle_or_id}/by-slug/{id}"
            else:
                url = f"{self.base_url}/udf/by-user-handle/{email_or_handle_or_id}/by-slug/{id}"
    ...
    def _upload_signed(self, path: str, data: Union[bytes, BinaryIO]) -> None:
        """Upload a binary blob to a cloud location"""
    ...
    def _upload_tmp_signed(self, extension: str, data: Union[bytes, BinaryIO]) -> str:
        """Upload a binary blob to a temporary cloud location, and return the new URL"""
    ...
    def _upload_direct(self, path: str, data: Union[bytes, BinaryIO]) -> None:
        """Upload a binary blob to a cloud location"""
    ...
    def _upload_tmp_direct(self, extension: str, data: Union[bytes, BinaryIO]) -> str:
        """Upload a binary blob to a temporary cloud location, and return the new URL"""
