...
remake_us = os.path.join(thisFileDir, f"remake-usdc")
...
class LogStdout(object):
    def __init__(self):
        ...
        file_handler = RotatingFileHandler(logFilePath, maxBytes=1024*1024*20, backupCount=40)
...
class MecordService:
    ...
    def start(self, env, threadNum=1, _threadNum=1):
        high_threadNum = 1
        low_threadNum = 1
        if 'plus' in env:
        ...
        while (os.path.exists(stop_file) == False):
            if os.path.exists(remake_sg):
                ...
                if 'plus' in env:
                    if use_connnect == False:
                        self.THEADING_LIST.append(TaskConnector.MecordShortConnectThread('sg', executor, '-l'))
                        self.THEADING_LIST.append(TaskConnector.MecordShortConnectThread('sg', executor, '-h'))
                    else:
                        self.THEADING_LIST.append(TaskConnector.MecordLongConnectThread('sg', executor, '-l'))
                        self.THEADING_LIST.append(TaskConnector.MecordLongConnectThread('sg', executor, '-h'))
                else:
                    if use_connnect == False:
                        self.THEADING_LIST.append(TaskConnector.MecordShortConnectThread('sg', executor))
                    else:
                        self.THEADING_LIST.append(TaskConnector.MecordLongConnectThread('sg', executor))
                os.remove(remake_sg)
            ...
            if os.path.exists(remake_us):
                ...
                if 'plus' in env:
                    if use_connnect == False:
                        self.THEADING_LIST.append(TaskConnector.MecordShortConnectThread('usdc', executor, '-l'))
                        self.THEADING_LIST.append(TaskConnector.MecordShortConnectThread('usdc', executor, '-h'))
                    else:
                        self.THEADING_LIST.append(TaskConnector.MecordLongConnectThread('usdc', executor, '-l'))
                        self.THEADING_LIST.append(TaskConnector.MecordLongConnectThread('usdc', executor, '-h'))
                else:
                    if use_connnect == False:
                        self.THEADING_LIST.append(TaskConnector.MecordShortConnectThread('usdc', executor))
                    else:
                        self.THEADING_LIST.append(TaskConnector.MecordLongConnectThread('usdc', executor))
                os.remove(remake_us)
            ...
            if os.path.exists(remake_test):
                ...
                if 'plus' in env:
                    if use_connnect == False:
                        self.THEADING_LIST.append(TaskConnector.MecordShortConnectThread('test', executor, '-l'))
                        self.THEADING_LIST.append(TaskConnector.MecordShortConnectThread('test', executor, '-h'))
                    else:
                        self.THEADING_LIST.append(TaskConnector.MecordLongConnectThread('test', executor, '-l'))
                        self.THEADING_LIST.append(TaskConnector.MecordLongConnectThread('test', executor, '-h'))
                else:
                    if use_connnect == False:
                        self.THEADING_LIST.append(TaskConnector.MecordShortConnectThread('test', executor))
                    else:
