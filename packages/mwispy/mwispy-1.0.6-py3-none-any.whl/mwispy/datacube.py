' mwispy module: datacube '

__version__ = '1.1'
__author__ = 'Shaobo Zhang'
__all__ = ['DataCube', 'openMWISP']
...
class DataCube(SpectralCube):
	...
	***check more keywords in SpectralCube.read
	...
	def __init__(self, *args, rms = None, **kws):
		...
		self._rms = DataCube._load_img(rms, self.header)
	...
	def _load_img(obj, hdr, name='rms'):
		...
		elif isinstance(obj, Projection): arr = obj._data
		elif isinstance(obj, np.ndarray): arr = obj
		else: raise TypeError('%s must be a FITS filename (str), a HDU (fits.PrimaryHDU), or an array (numpy.ndarray).' % name)
		arr = np.squeeze(arr)
		...
		else: raise ValueError('%s shape is not broadcastable to data shape: %s vs %s.' % (name, str(arr.shape), str(wcs.array_shape)))
	...
	def with_rms(self, rms):
		...
		new._rms = DataCube._load_img(rms, self.header)
		return new
	...
	V_MASK
	'''
	def with_mask(self, *args, **kws):
		new = super().with_mask(*args, **kws)
		new = new.with_rms(self.rms)
		return new
		...
		for s in range(step):
			for iax in axis:
				if iax == 0:
					...
					newmask[:,:,1:] = newmask[:,:,1:] | newmask[:,:,:-1]
					newmask[:,:,:-1] = newmask[:,:,:-1] | newmask[:,:,1:]
				#newmask = newmask | np.roll(newmask, -1, axis=ax) | np.roll(newmask, +1, axis=ax)
		return newmask
	...
	def pvslice(self, path_coord, path_frame='galactic', keep_width=False,
		...
		>>> pvhdulist = cube.pvslice(path, step=1, width=5, spline=True)
	...
	def rebinvelocity(self, start, stop, num, vunit='km/s', largecube=True):
		...
		if not largecube:
			...
			def _IntegralArray(ndarray, idxrange, axis=0):
				...
				intrange = np.round(idxrange).astype(int)
			...
			newdata = np.ndarray(dim, dtype = float)#self._data.dtype)
			for c in range(num):
			...
			if self.mask is None:
				...
				return newdc.with_mask(newmask)
	...
	def resample(self, *Xpar, vunit='km/s', largecube=False):
		...
		num = Nx
		return self.rebinvelocity(start, stop, num, vunit=vunit, largecube=largecube)
	...
	def channelmap(self, nrows = None, ncols = None, figureheight=6, vunit = 'km/s', subplot_kws = None, \
		...
		subplot_kws: dict, bool, optional
			keywords passed to subplot_adjust to adjust the subplots.
			accept left, right, bottom, top, hspace, wspace, etc.
		imshow_kws: dict, bool, optional
			keywords passed to ``imshow`` to show the background image.
			accept vmin, vmax, cmap, norm, etc.
			set to False to remove them.
		contour_kws: dict, bool, optional
			keywords passed to ``contour`` to plot the contours.
			accept levels, colors, linewidths, etc.
			set to False to remove them.
		text_kws: dict, bool, optional
			keywords passed to ``text`` to label the velocity.
			accept fontsize, ha, va, etc
			set to False to remove them.
		colorbar_kws: dict, bool, optional
			keywords passed to ``colorbar`` to plot the colorbar.
		...
		>>> #plot channel map
		>>> fig, ax = cube.channelmap(nrows=6, ncols=4, vunit='km/s', figureheight=8, \
		...
		xspan = self.header['NAXIS1'] * abs(self.header['CDELT1'])
		yspan = self.header['NAXIS2'] * abs(self.header['CDELT2'])
		npanel = self.header['NAXIS3']
		if nrows is None and ncols is None:
			for ncols in range(1, npanel+1):
				nrows = np.ceil(npanel / ncols).astype(int)
				if ncols * xspan > nrows * yspan: break
			##stop when width > height
		elif nrows is None:
			nrows = np.ceil(npanel / ncols).astype(int)
		elif ncols is None:
			ncols = np.ceil(npanel / nrows).astype(int)
			###some nrows is not valid
			ntemp = np.ceil(npanel / ncols).astype(int)
			if ntemp != nrows:
				warnings.warn('%i channels can not be arrange in %i rows, use %i rows.' % (npanel, nrows, ntemp))
			nrows = ntemp
		elif nrows*ncols < npanel:
				warnings.warn('nrows x ncols is less than the number of channels, extra channels will be ignored')
				npanel = nrows * ncols
		if nrows * ncols > 100: warnings.warn('plotting channel map over 100 panels.')
		...
		panelheight = figureheight * (subplot_params['top'] - subplot_params['bottom'])
		panelwidth = panelheight \
			/ (nrows * yspan + (nrows-1) * yspan * subplot_params['hspace']) \
			* (ncols * xspan + (ncols-1) * xspan * subplot_params['wspace'])
		figurewidth = panelwidth / (subplot_params['right'] - subplot_params['left'])
		...
		imshow_params = dict(vmin=vmin, vmax=vmax, cmap='Greys')
		...
		for panel, ax in enumerate(axes):
			...
			#velocity legend
			if npanel > 1 and text_kws != False:
		...
		if (imshow_kws != False) & (colorbar_kws != False):
			colorbar_params = dict(width="5%",  # width = 10% of parent_bbox width
				height="90%" if npanel > 1 else '100%',  # height : 50%
				loc='lower left',
			...
			axins = inset_axes(axes[-1], **colorbar_params)
			colorbar_params = dict(cax=axins)
			#add colorbar
			cb = fig.colorbar(im, cax=axins)

		#return fig, axes array
		while len(axes) % ncols != 0: axes.append(None)
		axes = np.array(axes).reshape(len(axes)//ncols, ncols)
		return fig, axes


	def gridmap(self, bg=None, figsize=(10,8), scale=1.0, imshow_kws=None, step_kws=None, colorbar_kws=None):
		'''
		...
		bgAx = fig.add_subplot(projection = self.wcs.sub(2))

		bgProj = DataCube._load_img(bg, self.header, name='background')
		if (bgProj is not None) and (imshow_kws != False):
			imshow_params = dict(vmin=np.nanmin(bgProj._data), vmax=np.nanmax(bgProj._data), cmap='RdBu_r')
			if imshow_kws: imshow_params.update(imshow_kws)
			im = bgAx.imshow(bgProj._data, **imshow_params)

			colorbar_params = dict(location='right', orientation = 'vertical')
			if colorbar_kws: colorbar_params.update(colorbar_kws)
			cbar = fig.colorbar(im, ax=bgAx, **colorbar_params)
			cbar.minorticks_on()

		###step kws
		step_params = dict(color='k')
		if step_kws: step_params.update(step_kws)
		...
		bgBbox = bgAx.get_position()
		axWidth = bgBbox.width / ncols * scale
		axHeight = bgBbox.height / nrows * scale

		gdAx = []
		for ix in range(ncols):
			for iy in range(nrows):
				spectrum = self._data[:, iy, ix]
				if np.isnan(spectrum).all(): gdAx.append(None)
				else:
					axLeft = bgBbox.x0 + (ix+0.5) * bgBbox.width / ncols - axWidth*0.5
					axBottom = bgBbox.y0 + (iy+0.5) * bgBbox.height / ncols - axHeight*0.5
					axBbox = [axLeft, axBottom, axWidth, axHeight]
					ax = fig.add_axes(axBbox, facecolor='none')

					#use this
					#new_axes = inset_axes(axes, width='4%', height='100%', borderpad=0, loc="lower right",\
					#	bbox_transform=axes.transAxes, bbox_to_anchor=(0.06, 0, 1, 1), axes_kwargs={'facecolor':'none'})
					#default = width='4%', height='100%', borderpad=0, loc="lower right",\
					#		bbox_transform=self.axes.transAxes, bbox_to_anchor=(0.06, 0, 1, 1), axes_kwargs={'facecolor':'none'}



					ax.step(vaxis, self._data[..., iy, ix], **step_params)
					ax.set_ylim(np.nanmin(self._data), np.nanmax(self._data))
					ax.set_xticklabels([])
					ax.set_yticklabels([])
					ax.tick_params(which='both', axis='both', direction='in')
					gdAx.append(ax)
		
		gdAx = np.array(gdAx).reshape(nrows, ncols)
		return fig, bgAx, gdAx
...
if __name__ == '__main__':
	...
	if 1:
		...
		cube = cube[:, 30:-30, 30:-30]
		rms = fits.open(rms)[0].data[30:-30, 30:-30]
		cube = cube.with_rms(rms)
		mask = cube.v_mask(threshold=5, consecutive=5)
		mask = DataCube.grow_mask(mask, 1, axis=(1,2))
		cube = cube.with_mask(mask)
		cube.moment(order=0).quicklook()
		plt.show()
	...
	###test gridmap
	if 0:
		new = cube[50:150, 115:130, 125:140]
		fig, ax, ax1 = new.gridmap(bg=new.moment(order=0), imshow_kws=dict(alpha=0.75), step_kws=dict(color='k'))
		plt.show()

	###test channel map
	if 0:
		new = DataCube.openMWISP('../test_channelmap.fits')[:]
		#new = cube[:, ].rebinvelocity(-10,10,21, largecube=True)
		#new._data[:,50:] = np.nan  nrows=4, ncols=6, 
		fig, ax = new.channelmap(ncols=7, figureheight=8, subplot_kws=dict(left=0.15, right=0.9), \
			imshow_kws=dict(cmap='RdBu_r'), contour_kws=False)
		ax[-1,0].set_xlabel('glon')
		ax[-1,0].set_ylabel('glat')
		ax[-1,0].coords[0].set_major_formatter('d.d')
		ax[-1,0].coords[1].set_major_formatter('d.d')
