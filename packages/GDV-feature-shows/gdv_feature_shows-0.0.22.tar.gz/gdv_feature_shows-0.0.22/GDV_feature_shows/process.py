...
def update_feature_extractor_with_settings(settings: dict):
    ...
    for setting_i in settings:
        if setting_i == settings_order[0]:
            fe.threshold_inner_circle = max(min(settings[setting_i], 255), 0)
        elif setting_i == settings_order[1]:
            fe.threshold_outer_circle = max(min(settings[setting_i], 255), 0)
        elif setting_i == settings_order[2]:
            fe.threshold_findings_petal = max(min(settings[setting_i], 255), 0)
        elif setting_i == settings_order[3]:
            fe.threshold_noise = max(min(settings[setting_i], 255), 0)
        else:
            print(f"update_feature_extractor_with_settings: Failed successfully. ")
            exit(1)


def fill_visualizations(pic_path: str, visualizations: dict[str: "VisualizationFrame"]):
    visualizations_names = get_visualization()
    fe = FeatureExtractor()

    features_data = {
        "General": ("#FF9999", {"Feature A": 1, "Feature B": 2, pic_path: hash(pic_path)}),
        "Circles": ("#99FF99", {"Feature C": 3, pic_path: hash(pic_path), "Feature D": 4, "Feature E": 5}),
        "Visualization 3": ("#9999FF", {"Feature F": 6, "Feature G": 7})
    }
    kimg = KImage(pic_path).to_gray()
    kimg = kimg.crop(*fe.find_crops(kimg))
    features = fill_all_features(kimg)
    ...
    kimg_peaks = kimg.copy().to_3_channels()
    ...
    for visualization_i in visualizations:
        if visualization_i == visualizations_names[0]:  # global
            visualizations[visualization_i].set_pic(kimg.resize_wh(200))
            ...
            visualizations[visualization_i].set_pic(kimg_with_radius.resize_wh(200))
            visualizations[visualization_i].set_features(features_data[visualization_i])
        elif visualization_i == visualizations_names[2]:  # petals 1
            visualizations[visualization_i].set_pic(kimg_with_petals.resize_wh(200))
            visualizations[visualization_i].set_features(features_data[visualization_i])
        elif visualization_i == visualizations_names[3]:  # petals 2
            visualizations[visualization_i].set_pic(kimg_with_petals_only.resize_wh(200))
            visualizations[visualization_i].set_features(features_data[visualization_i])
        elif visualization_i == visualizations_names[4]:  # Noised petals
            visualizations[visualization_i].set_pic(kimg_with_noised_petals.resize_wh(200))
            visualizations[visualization_i].set_features(features_data[visualization_i])
        elif visualization_i == visualizations_names[5]:  # Noises
            visualizations[visualization_i].set_pic(kimg_dct.resize_wh(200))
            visualizations[visualization_i].set_features(features_data[visualization_i])
        elif visualization_i == visualizations_names[6]:  # Extra
            visualizations[visualization_i].set_pic(kimg_peaks.resize_wh(200))
            visualizations[visualization_i].set_features(features_data[visualization_i])
        elif visualization_i == visualizations_names[7]:  # FFT
            visualizations[visualization_i].set_pic(kimg_fft.resize_wh(200))
            visualizations[visualization_i].set_features(features_data[visualization_i])
        else:
...
def fill_all_features(kimg_src: KImage) -> dict[int: Any]:
    kimg = kimg_src.copy()
    fe = FeatureExtractor()
    res, feas = {}, get_feature_names()
    for fea_i in feas:
    ...
    res[3], res[4], res[5] = fe.brightness_mean_std_range(kimg)  # Средняя яркость изображения,
                                                        # Стандартное отклонение яркости, Разность между пик и минимум
    ...
    res[-4] = fe.find_petals(kimg)  # Pentals
    ...
    res[25] = max(petal_d_seq)  # Самое большое расстоянием между лепестками
