...
from collections import namedtuple as create_namedtuple
from typing import Any, Callable, Coroutine, Dict, List, Optional, Tuple, Union, NamedTuple
...
class Client:
    ...
    def __init__(
        ...
        self.base_url = "https://api.aifn.io"

        # Setup clients
        self.sync_client = httpx.Client(http2=http2, timeout=timeout)
    ...
    def _make_request(self, endpoint: str, data: Dict[str, Any], is_async: Optional[bool] = False) -> Callable:
        ...
        if is_async:
            ...
            def _request():
                try:
                    response = self.sync_client.post(url, json=data, headers=headers)
                    response.raise_for_status()
                    return response.json()
                except HTTPStatusError as e:
                    # Handle HTTP errors (4xx and 5xx status codes)
    ...
    def _process_query_response(self, response: Dict[str, Any]) -> NamedTuple:
        ...
        -------
        NamedTuple
            A NamedTuple containing the output and metadata of the response. The metadata includes the number of input tokens, the number of output tokens, the latency in milliseconds, and the reasoning steps (if any).
        ...
        # Define a NamedTuple for the response and create an instance
        QueryResponse = create_namedtuple("QueryResponse", ["output", "metadata"])
        returned_response = QueryResponse(output=output, metadata=metadata)
    ...
    def _query(
        ...
        strict: bool,
    ) -> Union[NamedTuple, Coroutine[Any, Any, NamedTuple]]:
        ...
        return_reasoning : bool
            Whether to return reasoning for the output. If False, reasoning_steps will be None.
        ...
        -------
        Union[NamedTuple, Coroutine[Any, Any, NamedTuple]]
            A NamedTuple containing the query results, or a coroutine that returns such a NamedTuple.
            The NamedTuple includes two fields: 'output' (dict) and 'metadata' (dict).
            'output' contains the result of the query, and 'metadata' includes:
            - input tokens (in_tokens: int)
            - output tokens (out_tokens: int)
            - latency (latency_ms: float)
            - reasoning steps (reasoning_steps: Optional[List[str]]) which will be None if return_reasoning is False.
        ...
        for i, info in enumerate(image_info):
            if info["source"] in {"url", "base64", "local"}:
    ...
    async def aquery(
        ...
        return_reasoning : bool, optional
            A flag to indicate if the reasoning should be returned. Default is False. If False, reasoning_steps will be None.
    ...
    def _validate_batch_query(self, batch_inputs: List[Dict[str, Any]]):
        """
        Validates the batch inputs for a batch query function.

        This method ensures that the provided batch inputs adhere to the expected format and types.
        It checks that the inputs are a list of dictionaries, where each dictionary can contain
        the keys 'text_input' and 'images_input'. The 'text_input' should be a string, and the
        'images_input' should be a list of strings.

        Parameters
        ----------
        batch_inputs : List[Dict[str, Any]]
            A list of input dictionaries for the batch query. Each dictionary may include:
            - "text_input": A string representing the text input.
            - "images_input": A list of strings, each representing an image URL or base64 encoded image.

        Raises
        ------
        ValueError
            If the batch inputs are not a list, or if any input dictionary does not conform to the
            expected structure and types.
        """
        if not isinstance(batch_inputs, list):
            raise ValueError(
                "This is a batch function and requires a list of inputs. Did you mean to use a single query function?"
            )
        if len(batch_inputs) > 0:
            if not all(isinstance(inp, dict) for inp in batch_inputs):
                raise ValueError("Batch inputs must be dictionaries.")
            if not all(set(inp.keys()).issubset({"text_input", "images_input"}) for inp in batch_inputs):
                raise ValueError("Batch inputs must be dictionaries with keys 'text_input' and 'images_input'.")
            if not all(isinstance(inp.get("text_input", ""), str) for inp in batch_inputs):
                raise ValueError("Text input must be a string.")
            if not all(isinstance(inp.get("images_input", []), list) for inp in batch_inputs):
    ...
    async def _batch_query(
        ...
        return_reasoning : bool
            If True, includes reasoning in the response.

        strict : bool
            A flag to indicate if the function should be queried in strict mode.
    ...
    async def abatch_query(
        ...
        """Performs batch queries on a specified function version asynchronously.
