...
def assert_single_icon(rest_client, icons, ext, root):
    identifier = "icon" + ext
    ...
    icon1a = icons[0]
    response = rest_client.post(f"{root}/icon/{identifier}", json=icon1a)
    data = response.json()
    ...
    icon1b = icons[1]
    response = rest_client.put(f"{root}/icon/{identifier}", json=icon1b)
    data = response.json()
    assert response.status_code == 200, data
    assert data == icon1b
    ...
    response = rest_client.delete(f"{root}/icon/{identifier}")
    data = response.json()
...
def assert_multiple_icons(rest_client, icons, ext, existing, root):
    expected = list(existing)
    for i, icon in enumerate(icons):
        identifier = f"icon{i}{ext}"
        expected.append(identifier)
        response = rest_client.post(f"{root}/icon/{identifier}", json=icon)
        data = response.json()
        assert response.status_code == 200, data
        assert data == icon

    response = rest_client.get(f"{root}/icons")
    data = response.json()
    assert response.status_code == 200, data
