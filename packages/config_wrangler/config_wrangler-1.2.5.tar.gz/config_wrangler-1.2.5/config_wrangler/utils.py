...
def interpolate_values(container: Union[MutableMapping,List,BaseModel], root_config_data: MutableMapping) -> List[Tuple[str, str]]:
    errors = []
        ...
        if isinstance(value, MutableMapping) or isinstance(value, BaseModel) or isinstance(value, list) or isinstance(value, tuple):
            ...
            if new_value != value:
                if mode == ContainerType.Mapping:
                    ...
                    raise ValueError(f"Can't interpolate {value} inside tuple to {new_value}. Make it a list!")
                else: # Model
