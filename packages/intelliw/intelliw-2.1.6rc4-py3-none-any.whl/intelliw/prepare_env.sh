...
function report_info() {
  ...
  M=$2
  D=$3
  curl -i -H "Content-Type:application/json" -H "YYCToken:0" -H "X-tenantId:$TENANT_ID" -H "api-token:$TOKEN" -X POST --data "{\"id\":\"$INSTANCE_ID\",\"code\":200,\"token\":\"$TOKEN\",\"type\":\"$T\",\"message\":\"$M\",\"data\":$D}" "$REPORT_ADDR"
}
...
import() {
  ...
  if [[ $? != 0 ]]; then
    caught_error "{\"id\":\"$INSTANCE_ID\",\"code\":500,\"token\":\"$TOKEN\",\"type\":\"$REPORT_TYPE\",\"message\":\"$ERR_MASSAGE\",\"data\":\"\"}"
  fi
}
...
distributedtrain() {
  ...
  if [ "$DISTRIBUTED_TYPE" == "pytorch" ]; then
    ...
    caught_error "{\"id\":\"$INSTANCE_ID\",\"code\":500,\"token\":\"$TOKEN\",\"type\":\"$REPORT_TYPE\",\"message\":\"$message\",\"data\":\"\"}"
  fi
