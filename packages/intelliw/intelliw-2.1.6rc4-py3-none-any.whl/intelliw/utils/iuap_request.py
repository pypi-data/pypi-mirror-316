...
def trace_log(server_code, start_time, request_body, response_body):
    ...
    """
    if request_body and sys.getsizeof(request_body) >= 1024 * 1024 * 1:
        request_body = "RequestBody too long, Not Logged!!!"

    if response_body and sys.getsizeof(response_body) >= 1024 * 1024 * 1:
        response_body = "ResponseBody too long, Not Logged!!!"

    # 加签
    headers = {"Content-Type": "application/json"}
    auth_type = AuthType.AuthSDK
    url = f"https://bip-test.yonyoucloud.com/iuap-aip-console/self/rest/api/traceLogs"
    ...
    data = {
        ...
        "responseBody": json.dumps(response_body, ensure_ascii=False),
        "domain": "algorithm",
...
def download(url, output_path=None, method="GET", params=None, data=None, json=None, headers=None,
    ...
    resp = __do_request(method=method, url=url, headers=headers,
                        data=data, params=params, json=json, auth_type=auth_type)
...
def sign_yht(refresh=False):
    ...
    if not refresh and (expire != 0 and time.time() < expire):
        logger.warning("Get Cache Token: %s", token)
        return token['token']
    else:
        ...
        try:
            ...
            logger.warning("Get New Token: %s", token)
