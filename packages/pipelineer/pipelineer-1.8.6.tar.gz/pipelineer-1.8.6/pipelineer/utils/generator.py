...
def parse_oracle_schema(schema_file):
    """
    Analiza el archivo de definición de una tabla Oracle y extrae:
    - Campos (nombre, tipo, nullabilidad)
    - Clave(s) primaria(s)
    - Aplica el check de NOT NULL desde constraints (CHECK ("COLUMNA" IS NOT NULL))
    """

    with open(schema_file, 'r') as file:
        content = file.read()

    # Extraer todo el contenido entre CREATE TABLE ... ( ... )
    ...
    if not create_table_match:
        raise ValueError("No se encontró la definición CREATE TABLE en el esquema proporcionado.")

    table_def_content = create_table_match.group(1)
    ...
    # Lista de constraints luego para procesar PK y CHECKs
    constraints = []

    # Patrón sencillo para nombre de columna (con o sin comillas)
    col_name_pattern = re.compile(r'^"(?P<nameq>[^"]+)"|(?P<name>[A-Za-z0-9_]+)$')

    # Variables auxiliares
    columns_parsed = []
    parsing_columns = True

    # Identificar si una línea parece columna
    # Una línea de columna suele tener el formato:
    # "NOMBRE" TIPO [NOT NULL [ENABLE]] [COMMA]
    # o NOMBRE TIPO [NOT NULL [ENABLE]] [COMMA]
    #
    # Ajuste: haremos parsing manual.
    def parse_column_line(line):
        # Quitar coma final si existe
        line = line.rstrip(',')
        # Dividir por espacios inicialmente
        parts = line.split()

        # La primera parte debe ser el nombre de la columna (con o sin comillas)
        # Buscamos el nombre en la primera parte, podría estar entre comillas
        first = parts[0]
        col_match = col_name_pattern.match(first)
        if not col_match:
            return None

        col_name = col_match.group('nameq') or col_match.group('name')

        # El resto de la línea después del nombre es el tipo y opcionalmente NOT NULL
        rest = line[len(first):].strip()

        # Intentar extraer NOT NULL
        # Buscar 'NOT NULL' o 'NOT NULL ENABLE'
        not_null = False
        not_null_match = re.search(r'NOT NULL(?: ENABLE)?', rest, re.IGNORECASE)
        if not_null_match:
            not_null = True
            # Remover NOT NULL de la cadena de tipo
            rest = rest[:not_null_match.start()].strip()

        # Ahora rest debe contener solo el tipo (ej: NUMBER(10,0))
        # Si el tipo está separado por espacios extras, normalizar
        column_type = rest.upper()
        ...
        return (col_name.lower(), column_type, "NOT NULL" if not_null else "NULL")

    # Patrón para primary key
    pk_pattern = re.compile(r'PRIMARY KEY\s*\(([^)]+)\)', re.IGNORECASE)

    # Patrón para check NOT NULL
    # Ejemplo: CHECK ("COLUMNA" IS NOT NULL)
    check_not_null_pattern = re.compile(r'CHECK\s*\(\s*"([^"]+)"\s+IS\s+NOT\s+NULL\s*\)', re.IGNORECASE)

    for line in lines:
        upper_line = line.upper()
        if upper_line.startswith("CONSTRAINT"):
            # Es una constraint (PK u otra)
            constraints.append(line)
            parsing_columns = False
        elif (upper_line.startswith("PCTFREE") or
              upper_line.startswith("PCTUSED") or
              upper_line.startswith("INITRANS") or
              upper_line.startswith("MAXTRANS") or
              upper_line.startswith("NOCOMPRESS") or
              upper_line.startswith("LOGGING") or
              upper_line.startswith("STORAGE") or
              upper_line.startswith("TABLESPACE") or
              "SEGMENT CREATION" in upper_line or
              "ENABLE ROW MOVEMENT" in upper_line):
            # Parámetros de almacenamiento, ya no columnas
            parsing_columns = False
        elif parsing_columns:
            # Intentar parsear como columna
            col_parsed = parse_column_line(line)
            if col_parsed:
                fields.append(col_parsed)
            else:
                # No parece columna, probablemente algo que no necesitamos
                # Dejamos de parsear columnas
                parsing_columns = False
        else:
            # Ya no estamos en sección de columnas
            # Podrían ser más constraints o líneas irrelevantes
            if line.upper().startswith("CONSTRAINT"):
                constraints.append(line)

    # Procesar constraints para PK y CHECK NOT NULL
    for c_line in constraints:
        pk_match = pk_pattern.search(c_line)
        if pk_match:
            pk_cols = pk_match.group(1)
            pk_cols = [x.strip().strip('"').lower() for x in pk_cols.split(',')]
            primary_keys = pk_cols
        else:
            # Buscar CHECK NOT NULL
            cn_match = check_not_null_pattern.search(c_line)
            if cn_match:
                not_null_col = cn_match.group(1).lower()
                # Marcar la columna como NOT NULL si existe
...
def extract_column_comments(schema_file):
    """
    Extrae comentarios de columnas.
    Patrón: COMMENT ON COLUMN "ESQUEMA"."TABLA"."COLUMNA" IS 'comentario';
    """
    comments = {}
    pattern = re.compile(
