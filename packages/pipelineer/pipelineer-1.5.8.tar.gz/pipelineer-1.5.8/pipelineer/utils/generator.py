...
def extract_oracle_schemas(output_folder):
    """
    Extrae el esquema de tablas Oracle y genera archivos SQL.
    Las conexiones y el listado de tablas se leen desde un archivo .env.
    """
    ...
    tables = os.getenv("ORACLE_TABLES")

    print(f"Procesando tablas {os.getenv("ORACLE_TABLES")}")

    if not tables:
        raise ValueError("No se encontró la lista de tablas en el archivo .env (ORACLE_TABLES).")

    # Convertir la lista de tablas en un arreglo
