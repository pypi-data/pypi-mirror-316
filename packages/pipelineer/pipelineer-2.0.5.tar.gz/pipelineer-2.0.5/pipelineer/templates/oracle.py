...
DAG_NAME = "dag_test_multiples_tables"  # Reemplazar con el nombre del archivo .py (sin la extensión)
...
GCS_BUCKET_NAME = f"config_tables_dag_template_{CONFIG_VARS['env']}"
...
with DAG(
    ...
    for table in TABLES_TO_PROCESS:
        with TaskGroup(group_id=table['oracle_table'].lower()) as group:
            # Nombre de la tabla
            table_name = table['oracle_table'].lower()
            origen = table['origen']
            dominio = table['dominio']
            subdominio = table['subdominio']
            ingest_path = f"{CONFIG_VARS['output_bucket_ingest']}/{table['gcs_ingest_path']}"
            raw_path = f"{table['gcs_raw_path']}/{PROCESS_DATE}/{table_name}.avro"

            # Ruta y nombre del archivo SQL dentro del bucket (reemplazar con tu ruta)
            object_query_name = f"core/menu_andes/OPSANDES/{table['bigquery_table']}.sql"
            query = load_sql_query_from_gcs(BUCKET_QUERY, object_query_name)
            
            # Como se escriben los datos en bigquery 
            if table['merge']:
                write_disposition = "WRITE_TRUNCATE"
            elif table['merge'] != True and table['load_mode'] == 'append':
                write_disposition = "WRITE_APPEND"
            elif table['merge'] != True and table['load_mode'] == 'truncate':
            ...
            ingest = DataflowStartFlexTemplateOperator(
                ...
                body=create_dataflow_body(
                        ...
                        parameters={
                            ...
                            'output_path': f"gs://{ingest_path}",        # Ruta de salida en GCS para el archivo Avro
            ...
            raw = GCSToGCSOperator(
                ...
                source_object=f"{ingest_path}/{table['oracle_table']}.avro",  # Objeto (archivo Avro) de origen
                destination_bucket=CONFIG_VARS['output_bucket_raw'],  # Bucket de destino (capa raw)
                destination_object=raw_path,  # Objeto (ruta) de destino
            ...
            insert_to_bq = BigQueryInsertJobOperator(
                task_id=f'insert_to_bq_{table_name}',
                configuration={
                    "load": {
                        "sourceUris": [f"gs://{CONFIG_VARS['output_bucket_raw']}/{raw_path}"],
                        "destinationTable": {
                            "projectId": CONFIG_VARS['lakehouse_andes_project_id'],
                            "datasetId": table['dataset_destino'],
                            "tableId": table_name,
                        },
                        "sourceFormat": "AVRO",
            ...
            if table['merge']:
                ...
                merge_table = BigQueryInsertJobOperator(
                    ...
                    configuration={
                        "query": {
                            "query": f"CALL `{CONFIG_VARS['lakehouse_andes_project_id']}.staging_dataset.{table['store_procedure']}`()",  # Llamada al procedimiento almacenado
            ...
            if table['merge']:
                ...
                start >> ingest >> raw  >> insert_to_bq >> end
