...
# Agregar la carpeta connections al PYTHONPATH
current_dir = os.path.dirname(os.path.abspath(__file__))
connections_path = os.path.join(current_dir, "../../connections")
sys.path.append(connections_path)
from connections.oracle_config import get_oracle_config
...
def generate_bigquery_table_scripts(config_folder, schema_folder, config_file=None, output_folder="sql/bigquery/scripts/"):
    ...
    incluyendo PKs y comentarios de columnas.
    """
    # Verificar carpetas
    ...
    for file in config_files:
        ...
        comments = extract_column_comments(schema_file)  # Obtener comentarios de columnas
        primary_keys = extract_primary_keys(schema_file)  # Obtener PKs
        ...
        for zone in zones:
            ...
            # Definir PKs en BigQuery como restricción
            if primary_keys:
                pk_config = f"PRIMARY KEY ({', '.join(primary_keys)})"
                script_lines.append(pk_config)
            ...
            for column, comment in comments.items():
                script_lines.append(f"COMMENT ON COLUMN {bq_table_name}.{column} IS '{comment}'")
...
def extract_oracle_schemas(output_folder, config_path="schemas/config/oracle_tables.py"):
    """
    Extrae el esquema de tablas Oracle y genera archivos SQL.
    """
    # Cargar listado de tablas desde el archivo de configuración
    table_list = load_table_list(config_path)
