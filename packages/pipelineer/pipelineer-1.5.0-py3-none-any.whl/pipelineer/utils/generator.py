...
def generate_bigquery_table_scripts(config_folder, schema_folder, config_file=None, output_folder="sql/bigquery/scripts/"):
    ...
    config_files = [os.path.join(config_folder, config_file)] if config_file else [
        os.path.join(config_folder, f) for f in os.listdir(config_folder) if f.endswith(".json")
    ]
    ...
    for file in config_files:
        ...
        zones = config.get("zone", ["stg"])  # Por defecto: ["stg"]
        ...
        if not table_name or not dataset or not zones:
            raise ValueError(f"Faltan campos obligatorios en el archivo de configuración {file}")

        # Procesar cada zona (stg, dep, etc.)
        for zone in zones:
            ...
            # Obtener esquema desde el archivo Oracle
            schema_path = os.path.join(schema_folder, f"{table_name.lower()}.sql")
            if not os.path.exists(schema_path):
                raise FileNotFoundError(f"No se encontró el esquema Oracle para la tabla: {table_name}")

            # Leer y mapear los campos desde el esquema Oracle
            schema_fields = parse_oracle_schema(schema_path)
            for column_name, column_type, not_null in schema_fields:
                script_lines.append(f"  {column_name} {column_type} {not_null},")
            ...
            # Eliminar la última coma después del último campo
            if script_lines[-1].endswith(","):
                script_lines[-1] = script_lines[-1][:-1]

            script_lines.append(")")

            # Añadir configuración de partición
            if partition_field:
                if partition_data_type == "TIMESTAMP":
                    trunc_expr = f"TIMESTAMP_TRUNC({partition_field.lower()}, {partition_type})"
                elif partition_data_type == "DATE":
                    trunc_expr = f"{partition_field.lower()}"
                elif partition_data_type == "DATETIME":
                    trunc_expr = f"DATETIME_TRUNC({partition_field.lower()}, {partition_type})"
                else:
                    raise ValueError(f"Tipo de partición inválido: {partition_data_type}")
                script_lines.append(f"PARTITION BY {trunc_expr}")
...
def parse_oracle_schema(schema_file):
    ...
    columns = re.findall(
        r'^\s*"([^"]+)"\s+([A-Z0-9\(\),\s]+)(\s+NOT NULL ENABLE)?(?:,|$)',
...
"""
        ...
        # Guardar el archivo
        output_file = os.path.join(output_folder, f"sp_merge_{table_name.lower()}.sql")
        with open(output_file, 'w') as output:
