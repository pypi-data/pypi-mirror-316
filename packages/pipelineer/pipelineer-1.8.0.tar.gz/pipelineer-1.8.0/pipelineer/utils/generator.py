...
def parse_oracle_schema(schema_file):
    """
    Analiza un archivo SQL con una definición de tabla Oracle y extrae los campos, tipos, si son NOT NULL, 
    claves primarias y comentarios.
    """
    fields = []
    primary_keys = []
    ...
    for column_name, column_type in columns:
        ...
        is_not_null = "NOT NULL" if "NOT NULL" in column_type else "NULL"
    ...
    if pk_match:
        primary_keys = [key.strip().lower() for key in pk_match.group(2).split(",")]
