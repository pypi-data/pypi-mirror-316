...
def generate_bigquery_store_procedures(config_folder, output_folder):
    """
    Genera procedimientos almacenados para MERGE en BigQuery a partir de configuraciones.
    """
    # Verificar existencia de la carpeta de configuración
    if not os.path.isdir(config_folder):
        raise ValueError(f"La carpeta de configuración no existe: {config_folder}")
    ...
    for config_file in config_files:
        ...
        # Preparar nombres de tabla
        target_table = f"{dataset}.{table_name.lower()}"
        source_table = f"staging_dataset.{table_name.lower()}_staging"

        # Crear las cláusulas ON, UPDATE y INSERT
        on_clause = " AND ".join([f"T.{field} = S.{field}" for field in merge_fields])
        update_clause = ",\n      ".join([f"T.{field} = S.{field}" for field in merge_fields])
        insert_fields = ",\n      ".join(merge_fields)
        insert_values = ",\n      ".join([f"S.{field}" for field in merge_fields])
