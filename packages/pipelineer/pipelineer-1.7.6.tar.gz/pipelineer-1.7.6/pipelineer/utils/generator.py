...
def generate_bigquery_table_scripts(config_folder, schema_folder, config_file=None, output_folder="sql/bigquery/scripts/"):
    ...
    for file in config_files:
        ...
        primary_keys = extract_primary_keys(schema_file)  # Obtener claves primarias
...
def parse_oracle_schema(schema_file):
    ...
    # Extraer columnas con tipos de datos y restricciones de NULL o NOT NULL
    columns = re.findall(
        r'^\s*(\w+)\s+([A-Z0-9\(\),\s]+)(\s+NOT NULL)?(?:,|$)',
    ...
    for column_name, column_type, not_null in columns:
        ...
        column_type = map_oracle_to_bigquery(column_type.strip().upper())  # Mapear tipos de Oracle a BigQuery
