...
def parse_oracle_schema(schema_file):
    """
    Analiza un archivo SQL con una definición de tabla Oracle y extrae los campos, tipos, 
    restricciones de NULL, claves primarias y comentarios.
    """
    fields = []
    primary_keys = []
    ...
    # Nueva expresión regular para capturar:
    # 1. Nombre de la columna entre comillas
    # 2. Tipo de dato (y posibles parámetros) antes de NOT NULL
    # 3. Parte opcional de NOT NULL / NOT NULL ENABLE
    # Considera también que puede haber espacios o comas al final.
    columns = re.findall(
        r'^\s*"([^"]+)"\s+([A-Za-z0-9\(\),_\s]+)(NOT NULL(?: ENABLE)?)?\s*(?:,|$)',
        content, re.MULTILINE
    )

    for column_name, column_type, not_null_part in columns:
        ...
        # Primero asegurarnos de limpiar el tipo, ya que puede traer espacios
        column_type = column_type.strip().upper()
        ...
        if column_type.startswith("NUMBER"):
            if "," in column_type:  # NUMBER(p, s)
                ...
                if match:
                    ...
                    # Si no se pudo extraer precisión y escala, asignar por defecto NUMERIC
                    column_type = "NUMERIC"
            else:
                # NUMBER sin precisión ni escala => INT64
                column_type = "INT64"
    ...
    # Extraer PRIMARY KEY
    pk_match = re.search(r'CONSTRAINT\s+\w+\s+PRIMARY KEY\s*\(([^)]+)\)', content, re.MULTILINE)
    if pk_match:
...
def generate_bigquery_table_scripts(config_folder, schema_folder, config_file=None, output_folder="sql/bigquery/scripts/"):
    ...
    for file in config_files:
        ...
        fields, primary_keys = parse_oracle_schema(schema_file)  # Obtener campos y claves primarias
        column_comments = extract_column_comments(schema_file)  # Obtener comentarios de las columnas
