...
def parse_oracle_schema(schema_file):
    """
    Analiza un archivo SQL con una definición de tabla Oracle y extrae:
    - Campos (nombre, tipo, nullabilidad)
    - Claves primarias
    Las columnas pueden estar con o sin comillas, tener NOT NULL, etc.
    """

    with open(schema_file, 'r') as file:
        content = file.read()

    # Extraer el contenido entre CREATE TABLE y el paréntesis de cierre.
    # Se asume que el CREATE TABLE termina con un ')' seguido de ';'.
    ...
    if not table_def_match:
        raise ValueError("No se encontró definición CREATE TABLE en el esquema proporcionado.")
    ...
    lines = table_def_content.strip().split('\n')

    # Patrón para capturar:
    # 1. Nombre de columna (entre comillas o sin ellas)
    # 2. Tipo (con paréntesis, dígitos, etc.)
    # 3. Opcionalmente NOT NULL (o NOT NULL ENABLE)
    col_pattern = re.compile(
        r'^("(?P<colq>[^"]+)"|(?P<col>[A-Za-z0-9_]+))\s+'
        r'(?P<type>[A-Za-z0-9\(\),_\s]+)'
        r'(?P<notnull>NOT NULL(?: ENABLE)?)?\s*$', 
        re.IGNORECASE
    )

    for line in lines:
        line = line.strip()
        # Omitir líneas vacías o que comiencen con CONSTRAINT (no son columnas)
        ...
        if col_match:
            ...
            # Mapeo de tipos Oracle a BigQuery
    ...
    # Extraer PRIMARY KEY
    pk_match = re.search(r'CONSTRAINT\s+\w+\s+PRIMARY KEY\s*\(([^)]+)\)', content, re.MULTILINE | re.IGNORECASE)
    if pk_match:
        primary_keys = [key.strip().lower() for key in pk_match.group(1).split(",")]
...
def extract_column_comments(schema_file):
    ...
    with open(schema_file, 'r') as file:
        for line in file:
            match = re.match(r'COMMENT ON COLUMN ".*"\.(\w+) IS \'(.+?)\'', line)
            if match:
                column, comment = match.groups()
                comments[column.lower()] = comment
    return comments


def generate_bigquery_table_scripts(config_folder, schema_folder, config_file=None, output_folder="sql/bigquery/scripts/"):
    """
    Genera scripts de creación de tablas en BigQuery basados en un archivo de configuración y esquemas Oracle.
    """
