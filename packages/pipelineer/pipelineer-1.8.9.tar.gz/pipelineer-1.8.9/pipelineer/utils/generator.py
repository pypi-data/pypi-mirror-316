...
def parse_oracle_schema(schema_file):
    ...
    # Extraer el contenido dentro de CREATE TABLE ... ( ... )
    create_table_match = re.search(
        r'CREATE TABLE\s+"[^"]+"\."[^"]+"\s*\((.*?)\)\s*', content, re.DOTALL | re.IGNORECASE
    )
    if not create_table_match:
        raise ValueError("No se encontró el bloque CREATE TABLE en el archivo.")

    table_block = create_table_match.group(1)
    ...
    # Regex para columnas:
    # Ej: "CPA_CODIGO" NUMBER(10,0) NOT NULL ENABLE,
    ...
    def map_type(oracle_type):
        ...
        if oracle_type.startswith("NUMBER"):
            if "," in oracle_type:
                m = re.match(r"NUMBER\((\d+),\s*(\d+)\)", oracle_type)
                if m:
                    precision, scale = map(int, m.groups())
                    return "NUMERIC" if precision <= 38 else "BIGNUMERIC"
                else:
                    return "NUMERIC"
            else:
                return "INT64"
        elif oracle_type.startswith("VARCHAR2") or oracle_type.startswith("CHAR"):
            return "STRING"
        elif oracle_type.startswith("DATE"):
            return "DATETIME"
        elif oracle_type.startswith("RAW"):
            return "BYTES"
        elif oracle_type in ["CLOB", "NCLOB"]:
            return "STRING"
    ...
    # Identificar líneas de configuración que no son columnas
    storage_keywords = ("PCTFREE", "PCTUSED", "INITRANS", "MAXTRANS", "NOCOMPRESS",
                        "LOGGING", "STORAGE", "TABLESPACE", "SEGMENT CREATION", "ENABLE ROW MOVEMENT")

    for line in lines:
        upper_line = line.upper()
        if upper_line.startswith("CONSTRAINT"):
            # Es una constraint, no es columna
            parsing_columns = False
            constraints.append(line)
            continue
        if any(kw in upper_line for kw in storage_keywords):
            # Parámetros de almacenamiento o similares, no son columnas
            parsing_columns = False
            continue

        if parsing_columns:
            # Intentar parsear columna
            m = col_regex.match(line)
            if m:
                col_name = m.group('col').lower()
                col_type_raw = m.group('type')
                notnull_part = m.group('notnull')
                is_not_null = "NOT NULL" if notnull_part else "NULL"

                # Mapear tipo
                bq_type = map_type(col_type_raw)
                fields.append((col_name, bq_type, is_not_null))
            else:
                # No matchea como columna, puede ser constraint
                parsing_columns = False
                if upper_line.startswith("CONSTRAINT"):
                    constraints.append(line)
        else:
            # Ya no estamos parseando columnas, buscar constraints
            if upper_line.startswith("CONSTRAINT"):
                constraints.append(line)

    # Procesar constraints para buscar PK
    primary_keys = []
    for c in constraints:
        pk_match = pk_regex.search(c)
        if pk_match:
    ...
    for conf in config_files:
        ...
        for zone in zones:
            ...
            for column_name, column_type, is_not_null in fields:
                script_lines.append(f"  {column_name} {column_type} {is_not_null},")
