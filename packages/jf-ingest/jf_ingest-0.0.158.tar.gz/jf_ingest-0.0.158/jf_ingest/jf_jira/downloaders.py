...
def download_board_sprints(
    jira_connection: JIRA, all_jira_boards: list[dict]
) -> tuple[list[dict], list[dict]]:
    """
    Downloads sprints via paginated board sprint list (/boards/{board_id}/sprints).
    Args:
...
def download_sprints_by_id(
    ...
    for i in tqdm_to_logger(
        range(1, max_id_to_fetch + 1), total=max_id_to_fetch, desc='Pulling sprints by id'
    ):
        try:
            sprint = retry_for_status(
            ...
            if e.status_code != 404:
                ...
                logger.warning(f"Couldn't get sprint id {i} (HTTP Error Code {e.status_code})")
        except RetryLimitExceeded as e:
            logger.warning(
                f'Retry limit exceeded when attempting to pull sprints by id. Error: {e}'
            )
...
def download_boards_and_sprints(
    jira_connection: JIRA, download_sprints: bool, fetch_sprints_by_id: bool = False
    ...
    with record_span('download_sprints'):
        if download_sprints:
            if fetch_sprints_by_id:
                all_sprints, links = download_sprints_by_id(jira_connection, all_jira_boards)
            else:
                all_sprints, links = download_board_sprints(jira_connection, all_jira_boards)
