zeroset_colors = [(126, 30, 156),  # purple
                  (21, 176, 26),  # green
                  (3, 67, 223),  # blue
                  (255, 129, 192),  # pink
                  (101, 55, 0),  # brown
...
def get_color_by_index(idx: int):
    texts = [f'R{idx}R', f'G{idx}G', f'B{idx}B']
    rgb = []
    for text in texts:
        hash = 0
        for ch in text:
...
def get_color_by_string(s: str):
    s = f'{s.lower()}{s[::-1].upper()}{s.upper()}{s[::-1].lower()}'
    pos = len(s) // 3
    texts = [s[:pos], s[pos:2 * pos], s[2 * pos:]]
    rgb = []
    for text in texts:
        hash = 0
        for ch in text:
            hash = (hash * 281 ^ ord(ch) * 997) & 0xFFFFFFFF
        rgb.append(hash % 192 + 64)
    return tuple(rgb)


# https://github.com/ultralytics/ultralytics/issues/561
def draw_segmentation(img: np.ndarray, mask: np.ndarray, color_bgr: Tuple, name: str = "", alpha: float = 0.5):
    size = max(img.shape[:2])
    # draw mask
    colored_mask = np.expand_dims(mask, 0).repeat(3, axis=0)
    colored_mask = np.moveaxis(colored_mask, 0, -1)
    masked = np.ma.MaskedArray(img, mask=colored_mask, fill_value=color_bgr)
    image_overlay = masked.filled()
    image_combined = cv2.addWeighted(img, 1 - alpha, image_overlay, alpha, 0)
    # draw contour
    thickness = max(mask.shape[:2]) // 1000 + 1
    contours, hierarchy = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    # draw text
    fontface = cv2.FONT_HERSHEY_PLAIN
    fontscale = size // 1000 + 1
    font_thickness = size // 1000 + 1
    padding = size // 1000 + 1

    for contour in contours:
        if cv2.contourArea(contour) < size // 100:
            continue
        ...
        if name != "":
            ...
            org = (0 + cX - fw // 2, fh + baseline // 2 + cY - fh // 2)
            cv2.rectangle(image_combined,
                          (cX - padding - fw // 2, cY - padding - fh // 2),
                          (fw + cX + padding - fw // 2, fh + baseline + cY + padding - fh // 2),
...
def draw_detection(img: np.ndarray, bbox: list, color_bgr=Tuple, name: str = "", adjust_thickness: float = 1.0):
    """
    :param img:
    ...
    # draw rectangle
    thickness = int((max(img.shape[:2]) // 1000) * adjust_thickness) + 1
    cv2.rectangle(img, (x1, y1), (x2, y2), color_bgr, thickness=thickness)

    # Draw rectangle corner
    length = min(img.shape[:2]) // 20
    corner_thickness = thickness * 4
    # Top-Left
    cv2.rectangle(img, (x1, y1), (x1 + length, y1 + corner_thickness), color_bgr, thickness=cv2.FILLED)
    cv2.rectangle(img, (x1, y1), (x1 + corner_thickness, y1 + length), color_bgr, thickness=cv2.FILLED)
    # Top-Right
    cv2.rectangle(img, (x2, y1), (x2 - length, y1 + corner_thickness), color_bgr, thickness=cv2.FILLED)
    cv2.rectangle(img, (x2, y1), (x2 - corner_thickness, y1 + length), color_bgr, thickness=cv2.FILLED)
    # Bottom-Left
    cv2.rectangle(img, (x1, y2), (x1 + corner_thickness, y2 - length), color_bgr, thickness=cv2.FILLED)
    cv2.rectangle(img, (x1, y2), (x1 + length, y2 - corner_thickness), color_bgr, thickness=cv2.FILLED)
    # Bottom-Right
    cv2.rectangle(img, (x2, y2), (x2 - corner_thickness, y2 - length), color_bgr, thickness=cv2.FILLED)
    cv2.rectangle(img, (x2, y2), (x2 - length, y2 - corner_thickness), color_bgr, thickness=cv2.FILLED)

    # draw text
    fontface = cv2.FONT_HERSHEY_PLAIN
    fontscale = size // 1000 + 1
    font_thickness = size // 1000 + 1
    padding = 0  # size // 1000 + 1
    if name != "":
        (fw, fh), baseline = cv2.getTextSize(name, fontface, fontscale, font_thickness)
        ...
        org = (0 + (x1 + x2) // 2 - (fw // 2),
               fh + baseline // 2 + y1)
        cv2.rectangle(img,
                      ((x1 + x2) // 2 - padding - (fw // 2),
                       y1 - padding),
                      (fw + (x1 + x2) // 2 + padding - (fw // 2),
                       fh + baseline + y1 + padding),
                      color_bgr, cv2.FILLED)
        cv2.putText(img, name, org, fontface, fontscale, (255, 255, 255), font_thickness)
    pass
...
if __name__ == '__main__':
    ...
    img = cv2.imread("../data/computer04.jpg")

    with open("../data/computer04.json", "rt", encoding="utf-8") as f:
        bboxes = json.load(f)
    for bbox in bboxes:
        label = coco_labels[bbox["class"]]
        color = get_color_by_string(label)
        box_pt = [bbox["x1"], bbox["y1"], bbox["x2"], bbox["y2"]]
        draw_detection(img, box_pt, color, label, adjust_thickness=0.5)
    cv0.imshow("detection", img, mode=cv0.IMSHOW.FULLSCREEN).waitKey()
    exit()

    mask = cv2.imread("../data/computer04_mask.png", cv2.IMREAD_GRAYSCALE)

    print(f'W x H: {img.shape[1]} x {img.shape[0]}')
    names = ["person", "computer", "keyboard", "book"]
    img_palette = img.copy()

    for i in range(4):
        value = i + 1
        mask_i = np.where(mask == value, 255, 0).astype(np.uint8)
        color = get_color_by_string(names[i])
        img_palette = draw_segmentation(img_palette, mask_i, color, names[i])

    cv0.imshow("mask", img_palette, mode=cv0.IMSHOW.FULLSCREEN).waitKey()
    pass
