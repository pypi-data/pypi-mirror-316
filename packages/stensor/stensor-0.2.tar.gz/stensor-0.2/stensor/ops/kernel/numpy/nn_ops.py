...
from ._impl import logsumexp, softmax, sum_to, pair, im2col_array, col2im_array
...
class Linear(NumpyKernel):
    ...
    def backward(self, gy):
        x, W, b = self.xs[0], self.xs[1].T, self.xs[2]
...
class RMSNorm(NumpyKernel):
    ...
    def forward(self, x, gamma):
        self.inv_std = 1 / (np.sqrt(np.power(x, 2).mean(-1, keepdims=True) + self.eps))
    ...
    def backward(self, gy):
        ...
        if Config.recomputer:
            self.inv_std = 1 / (np.sqrt(np.power(x, 2).mean(-1, keepdims=True) + self.eps))
        # 计算 grad_weight
        grad_weight = (gy*x*self.inv_std).sum(axis=0,keepdims=False).sum(axis=0,keepdims=False).astype(x.dtype)
        # 计算 grad_x_norm
        grad_x_norm = gy * gamma
        # 计算 grad_inv_std
        grad_inv_std = np.sum(grad_x_norm * x, axis=-1, keepdims=True)
        # 计算 grad_mean_sq
        grad_mean_sq = -0.5 * np.power(self.inv_std, 3) * grad_inv_std
        # 计算 grad_x
    ...
    #     self.xivar = np.sqrt(np.power(x, 2).mean(-1, keepdims=True) + self.eps)
    #     output = x / self.xivar
    #     #print(np.power(x, 2).max())
    #     #print(np.power(x, 2).mean(-1, keepdims=True).max())
    ...
    #     xmu = x
    #     if Config.recomputer:
    #         self.xivar = np.sqrt(np.var(x, axis=-1, keepdims=True) + self.eps)
    #     xivar = self.xivar
    #     dout = gy
...
class Dropout(NumpyKernel):
    ...
    def backward(self, gy):
        if Config.recomputer:
            self.mask = np.random.rand(*self.xs[0].shape) > self.dropout_ratio
        gx = np.where(self.mask, gy, 0) / (1 - self.dropout_ratio)
        return gx
...
class Pooling2DGrad(NumpyKernel):
    def __init__(self, mpool2d):
        ...
        self.input_shape = mpool2d.xs[0].shape
        self.dtype = mpool2d.xs[0].dtype
...
class Pooling2DWithIndexes(NumpyKernel):
    def __init__(self, mpool2d):
        ...
        self.stride = mpool2d.stride
        self.pad = mpool2d.pad
        self.input_shpae = mpool2d.xs[0].shape
        self.dtype = mpool2d.xs[0].dtype
        self.indexes = mpool2d.indexes
...
class SoftmaxCrossEntropy(NumpyKernel):
    ...
    def backward(self, gy):
        ...
        t_onehot = np.eye(CLS_NUM, dtype=x.dtype)[t]
        y = (y - t_onehot) * gy
        return y
