...
def get_data():
    ...
    x = np.random.choice(word1, size=n, replace=True)
    x = x.tolist()
    x = [int(i) for i in x]
    y = x + x
    x = x + [0] * 8
...
def test_transformer_train():
    ...
    #model.save_weights('./Transformer_msgd_lr1e4_step3000.npz')
    model.save_weights('./Transformer1.npz')
...
def test_transformer_infer():
    # 测试
    model.load_weights('./Transformer1.npz')
    #evalation
    with stensor.no_grad():
