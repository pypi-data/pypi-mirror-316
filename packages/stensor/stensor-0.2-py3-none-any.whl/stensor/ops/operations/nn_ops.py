...
from stensor.ops.operations._impl import sum_to, pair, logsumexp, softmax, get_deconv_outsize, im2col_array, col2im_array
...
class Pooling2DWithIndexes(Primitive):
    def __init__(self, mpool2d):
        self.kernel_size = mpool2d.kernel_size
        self.stride = mpool2d.stride
        self.pad = mpool2d.pad
...
class AveragePooling(Primitive):
    def __init__(self, kernel_size, stride=1, pad=0):
        super().__init__()
        self.kernel_size = kernel_size
        self.stride = stride
...
class MeanSquaredError(Primitive):
...
class SoftmaxCrossEntropy(Primitive):
