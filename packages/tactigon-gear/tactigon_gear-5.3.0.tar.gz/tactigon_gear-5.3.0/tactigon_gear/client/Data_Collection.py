...
def collect(user_data: UserData, hal: HAL, data_collection_config: DataCollectionConfig):
    ...
    #check licence
    # if not License_Manager().check_license() and False:
    #     print("Invalid license: Data Collection not available")
    #     return
    # else:
    #     print("License ok")

    # import json config
    # data_collection_config = Config_Manager.from_file("data_collection.json")
    ...
    hand = data_collection_config.HAND
    interface = hal.INTERFACE
    num_sample = hal.NUM_SAMPLE

    if hand == "RIGHT":
        b_name = hal.BLE_RIGHT_NAME
        b_add = hal.BLE_RIGHT_ADDRESS
        b_hand = Hand.RIGHT
    else:
        b_name = hal.BLE_LEFT_NAME
...
def print_info(data_collection_config: DataCollectionConfig, hal_config: HAL, user_config: UserData):
    ...
    print("**********************************************************************")
    print("             Hand: " + data_collection_config.HAND)
    print("             Method: " + hal_config.INTERFACE)
