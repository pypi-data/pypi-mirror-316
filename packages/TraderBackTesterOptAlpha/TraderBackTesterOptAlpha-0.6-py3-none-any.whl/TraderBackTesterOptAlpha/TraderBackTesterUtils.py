...
class TraderBackTesterUtils:
    def __init__(self, files_path:str) -> None:
        ...
        self.index_ = ['BANKNIFTY', 'NIFTY']
        warnings.filterwarnings('ignore')

    def get_delta_strike_def(self, name:str) -> pd.DataFrame:
        tokens=pd.read_csv(self.files_path + 'angel_tokens.csv', low_memory=False)
    ...
    def exp_cal(self, row:Any, x_org:pd.DataFrame, dat:str = 'date', nxt_exp:Any = 0, montly:Any = False) -> str:
        ...
        try:
            ...
            if (x_org.ticker.iloc[0] == 'BANKNIFTY') and (date >= '2024-03-01') and (date <= '2024-12-22'):
                exp_day = 2
            ...
            if not monthly:
                ...
                while str(x) not in x_org.date.to_list():
                    if not self.is_holiday(pd.to_datetime(x).date()): break
                    x = x - datetime.timedelta(days=1)
                return str(x)
            else:
                _date_ = date
                for i in range(next_exp+1):
