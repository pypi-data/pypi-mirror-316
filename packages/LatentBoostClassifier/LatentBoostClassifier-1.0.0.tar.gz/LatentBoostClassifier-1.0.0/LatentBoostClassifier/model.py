...
def build_cgan(hp):
    ...
    generator_x = layers.Dense(10, activation="sigmoid")(generator_x)
...
def tune_cgan(X_train, Y_train, X_test, Y_test, shared_dict):
    ...
    try:
        ...
        shared_dict["best_cgan_generator"] = build_cgan(kt.HyperParameters()).get_layer("generator")

# Define a function to tune the Random Forest model
def tune_random_forest(latent_features, synthetic_data, Y_train, shared_dict):
    """
    Tune the Random Forest model using GridSearchCV.

    Args:
    latent_features (np.ndarray): The latent features.
    synthetic_data (np.ndarray): The synthetic data.
    Y_train (np.ndarray): The training labels.
    shared_dict (dict): A shared dictionary to store the results.
    """
    try:
