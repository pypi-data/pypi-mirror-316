...
from . import NegInfinity, PosInfinity, segment, segmentlist, segmentlistdict, __version__
...
def fromlalcache(cachefile, coltype = int):
	...
	return segmentlist(CacheEntry(l, coltype = coltype).segment for l in cachefile)
...
def fromsegwizard(fileobj, coltype = int, strict = True):
	...
	for line in fileobj:
		...
		try:
			...
			num = int(tokens[0])
			seg = segment(list(map(coltype, tokens[1:3])))
			duration = coltype(tokens[3])
			this_line_format = 4
		except ValueError:
			try:
				[tokens] = threecolsegpat.findall(line)
				seg = segment(list(map(coltype, tokens[0:2])))
...
def fromtama(fileobj, coltype = None):
	...
	for line in fileobj:
		try:
			...
			l.append(segment(list(map(coltype, tokens[0:2]))))
		except ValueError:
			break
	return l
...
def from_range_strings(ranges, boundtype = int):
	...
	"first:last" into an equivalent igwn_segments.segmentlist.  In the
	...
	segs = segmentlist([None] * len(ranges))
	...
	for i, range in enumerate(ranges):
		...
		segs[i] = segment(parts[0], parts[1])
...
def to_range_strings(seglist):
	...
	for i, seg in enumerate(seglist):
		if not seg:
			ranges[i] = str(seg[0])
		elif (seg[0] is NegInfinity) and (seg[1] is PosInfinity):
			ranges[i] = ":"
		elif (seg[0] is NegInfinity) and (seg[1] is not PosInfinity):
			ranges[i] = ":%s" % str(seg[1])
		elif (seg[0] is not NegInfinity) and (seg[1] is PosInfinity):
			ranges[i] = "%s:" % str(seg[0])
		elif (seg[0] is not NegInfinity) and (seg[1] is not PosInfinity):
...
def segmentlistdict_to_short_string(seglists):
	...
	igwn_ligolw library should be used.
	"""
	...
	while 1:
		if next(bitstream):
			...
			try:
				...
				if j - i >= minlen:
					yield segment(start + i * dt, start + j * dt)
...
def S2playground(extent):
	...
	>>> S2playground(segment(874000000, 874010000))
	[segment(874000013, 874000613), segment(874006383, 874006983)]
	"""
	lo = int(extent[0])
	lo -= (lo - 729273613) % 6370
	hi = int(extent[1]) + 1
	return segmentlist(segment(t, t + 600) for t in range(lo, hi, 6370)) & segmentlist([extent])
...
def Fold(seglist1, seglist2):
	...
	"""
	for seg in seglist2:
		yield (seglist1 & segmentlist([seg])).shift(-seg[0])
...
def vote(seglists, n):
	...
	for bound, delta in vote_generator(seglists):
		if delta > 0 and n - delta <= votes < n:
			...
			result.append(segment(start, bound))
