...
/*
 * The underscore in this function's name avoids a collision with a Windows
 * API symbol (https://learn.microsoft.com/en-us/windows/win32/multimedia/min).
 */
static PyObject *min_(PyObject *a, PyObject *b)
{
	int result;
...
/*
 * The underscore in this function's name avoids a collision with a Windows
 * API symbol (https://learn.microsoft.com/en-us/windows/win32/multimedia/max).
 */
static PyObject *max_(PyObject *a, PyObject *b)
{
	int result;
...
static PyListObject *segments_SegmentList_New(PyTypeObject *type, PyObject *sequence)
{
	...
	if(new && sequence)
		if(PyList_Extend((PyObject *) new, sequence)) {
			Py_DECREF(new);
			new = NULL;
		}
	return new;
...
{
	Py_ssize_t i;
	PyObject *abs = PyLong_FromLong(0);
	if(!abs)
		return NULL;
