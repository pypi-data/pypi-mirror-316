...
def find_workspace_dir() -> str:
    ...
    if ok:
        return expected
    return join(dirname(__file__), "workspace")
...
class BootstrapConfig(BaseModel):
    ...
    def save(self, dir_path: str = None):
        ...
        if dir_path is None:
            filename = join(abspath(".ghostos.yaml"))
        else:
            filename = join(dir_path, ".ghostos.yaml")
