...
class RTMPoseInference(BaseImageInference):
    ...
    def top_down_affine(
        self, input_size: Tuple[int, int], scale: np.ndarray, center: np.ndarray, img: np.ndarray
    ...
    def get_warp_matrix(
        self, center: np.ndarray, scale: np.ndarray, rot: float, output_size: Tuple[int, int]
    ) -> np.ndarray:
    ...
    def process_image(self, img_path: str) -> dict:
        ...
        outputs = self.model.run(
            None, {self.model.get_inputs()[0].name: [resized_img.transpose(2, 0, 1).astype(np.float32)]}
        )
