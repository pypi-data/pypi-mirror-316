...
class ExcHandlerInterceptor(BaseInterceptor):
    ...
    def exception_handler(
        ...
        status_code: StatusCode = exc.status_code if isinstance(exc, BaseGrpcError) else StatusCode.INTERNAL
        method: str = handler_call_details.method
        error: str = exc.__class__.__name__
        self._error_counter.labels(path=method, status_code=status_code.name, error=error).inc()
