...
class sTester:
    def __init__(self,
        ...
        object
        path
        metric :Regression metrics : {"max_error": "ma", "mean_absolute_error": "mae", "mean_squared_error": "mse",
        ...
        self.object            = object
        self.metric            = metric
        self.object.aR_info.period = check_period(dataFrame=self.dataFrame)
    #def date_range(self):
    ...
    def forecast(self):
        ...
        if bool(self.object.aR_info.forecastingStartDate) is True:
            if pd.to_datetime(self.dataFrame.index[-1]) != pd.to_datetime(self.object.aR_info.forecastingStartDate):
                ...
                data = np.exp(dTester.a_data(info=self.object.aR_info, dataFrame=self.dataFrame))
                data["Predicts"] = np.exp(forecasts.values)
                self.actual,self.Yhat = data.loc[:,data.columns != "Predicts"].values,forecasts.values
                return data
            else:
                forecasts= self.model.forecast(self.object.aR_info.forecastNumber, alpha=0.05,dynamic=True)
        else:
...
class dTester:
    def __init__(self,
        ...
        self.object.req_info.period = check_period(dataFrame=self.dataFrame)
    ...
    def generate_timeLag(self, pr_data, lag_cols):
        max_lag = max([x.split('lag')[1] for x in lag_cols])
        pr_data = self.pr.generate_time_lags(dataFrame=pr_data, col=self.object.req_info.targetCol, n_lags=int(max_lag))[-self.object.req_info.lookback:]
        return pr_data
    @staticmethod
    ...
    def create_date_range(info,dataFrame):
        ...
        if period.lower() == "montly":
            return pd.date_range(start=str(dataFrame.index[-1] + timedelta(days=30)),
    ...
    def predict( self,pr_data):
        if self._get_shape_data()[1] == 1:
            ...
            val_rescaled = self.fit_transform(pr_data)
            val_rescaled = np.reshape(val_rescaled, (1, -1, self.dataFrame.shape[1]))
            Yhat = self.model.predict(val_rescaled)
            self.scalerY.fit_transform(self.dataFrame[self.object.req_info.targetCol].values.reshape(-1, 1))
            Yhat = self.scalerY.inverse_transform(Yhat)
            return Yhat
