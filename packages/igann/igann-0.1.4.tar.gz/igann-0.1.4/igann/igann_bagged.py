...
if __name__ == "__main__":
    ...
    for dataset in dataset_list:
        ...
        X_train, y_train, X_test, y_test = loader.load_dataset(dataset, test=True)
        y_train, y_test = np.array(y_train), np.array(y_test)

        if np.isnan(y_train).sum() > 0 or np.isnan(y_test).sum() > 0:
            continue

        if loader.task_type == "binary":
            m = IGANN(task="classification", n_estimators=10)
            m.fit(X_train, y_train)
            performances[dataset + '_binary'] = roc_auc_score(y_test, m.predict_proba(X_test)[:,1])
        elif loader.task_type == "regression":
            m = IGANN(task="regression", n_estimators=10)
            m.fit(X_train, y_train)
            performances[dataset + '_regression'] = mean_squared_error(y_test, m.predict(X_test))
