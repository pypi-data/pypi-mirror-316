...
def step1_data_validation(resource, db, plot_figures=True):
    ...
    # aq_unit_hyphens = sag_wl.unit_hyphen.unique()
    ...
    for unit_hyphen in aq_unit_hyphens:
        ...
        unit_dsets = dsets[(dsets.unit_hyphen == unit_hyphen) & (dsets.sagd_param == param) & (dsets.label.str.contains("Best Available"))]
        if len(unit_dsets) == 0:
            logger.warning(f"  workaround: Skipping logger data inclusion from {unit_hyphen}. No Best Available {param} dataset found in AQTS.??")
            continue
        if len(unit_dsets) > 1:
            unit_dsets["dset_length"] = unit_dsets.apply(
                lambda row: pd.Timestamp(row.last_data_timestamp) - pd.Timestamp(row.first_data_timestamp),
                axis=1
            )
            unit_dsets = unit_dsets.sort_values("dset_length", ascending=False)
            logger.warning(f"  {len(unit_dsets)} {param} BA datasets found?\n{unit_dsets[['label', 'first_data_timestamp', 'last_data_timestamp', 'dset_length']]}")
            logger.warning(f"  Selecting the first as it is the longest. Please fix in AQTS.")
        ...
        aq_wl = gd.join_logger_data_intervals(aq_wl_dfs)
        aq_wl = aq_wl.rename(
            columns={
                "timestamp": "obs_date",
            }
        ...
        keep_cols += ["database", "obs_date", "rswl", "anomalous_ind", "grade"]
        aq_wl = aq_wl[[c for c in keep_cols if c in aq_wl.columns]]
        all_wl_dfs.append(aq_wl)
...
def step3_waterlevel_trends(resource, db, plot_figures=True):
    ...
    wldata = wldata.reset_index()
    trenddf = trenddf.reset_index()
    final_trends = trend_results[trend_results.include == True].reset_index()
    excluded_trends = trend_results[trend_results.include == False].reset_index()
    trendqc = trendqc.reset_index()
