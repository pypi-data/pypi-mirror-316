...
import json
from src.borgstats.borgstats import main
...
def test_subprocess_runs_borgmatic_command(mock_run):
    ...
    mock_run.assert_called_once_with(['borgmatic', 'info', '--json'], capture_output=True, check=True)
...
def test_subprocess_general_error(mock_run):
    ...
    mock_run.side_effect = subprocess.CalledProcessError(
        returncode=1, cmd='borgmatic', output='Repository "does_not_exist" not found in configuration files'
    )
