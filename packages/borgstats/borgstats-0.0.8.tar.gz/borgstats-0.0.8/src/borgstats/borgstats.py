...
def main(archive, noop, last, repo):
    ...
    with console.status('Running borgmatic...', spinner='dots10'):
        try:
            cmd = subprocess.run(borgmatic_command.split(), capture_output=True, check=True)
        except subprocess.CalledProcessError as e:
            console.print(f'\n[bright_red]Error running borgmatic:')
            console.print(Panel(e.stdout.decode("utf-8"), title='stdout', expand=False))
            console.print(Panel(e.stderr.decode("utf-8"), title='stderr', expand=False))
            sys.exit(1)
    try:
