...
class ElasticSearchVector(BaseVector):
    ...
    def _split(self, text, mode="textrank"):
        return " ".join(self.rag_tokenizer.extract_keywords(text, mode=mode))

    def search_by_full_text(self, query: str, **kwargs: Any) -> list[Document]:
        if query:
            query_str = {"match": {Field.CONTENT_SPLIT.value: self._split(query)}}
    ...
    def add_vector_data(self, vector_data: dict) -> str:
        ...
        content_split = self._split(vector_data["content"])
    ...
    def batch_add_vector_data(self, vds: list[dict], batch_num=100):
        ...
        for index, vd in enumerate(vds):
            ...
            content_split = self._split(vd["content"])
...
class ESVector:
    ...
    def init_vector(self) -> ElasticSearchVector:
        ...
        return ElasticSearchVector(
            ...
            rag_tokenizer=self.rag_tokenizer,
        )
