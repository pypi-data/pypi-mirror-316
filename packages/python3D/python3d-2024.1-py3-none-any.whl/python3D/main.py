...
def run(setup=lambda: None, loop=lambda x: None):
    ...
    pygame.display.set_caption(p['title'])
    ...
    log('log', f'{p["title"]} simulation started.')
    start_time = time()
    while running:
    ...
    log('log', f'{p["title"]} simulation ended after {round(time() - start_time, 1)} seconds.')
    log('break', '')
...
def render_text(screen, location, text, size, color, bold=False):
    if p['font'] is None:
        return
    try:
        font = pygame.font.Font(p['font'], size)
    except FileNotFoundError:
        log('error', 'Font file not found. Disabling text display for this run.', [p['font']])
        p['font'] = None
        return
    font.set_bold(bold)
