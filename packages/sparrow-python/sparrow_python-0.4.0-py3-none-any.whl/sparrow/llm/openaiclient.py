...
class OpenAIClient:
    ...
    async def wrap_to_request_params(self, messages: list, model: str, max_tokens=None, meta=None, preprocess_msg=False,
                                     **kwargs):
        if preprocess_msg:
    ...
    async def chat_completions(self, messages: list, model: str, return_raw=False, preprocess_msg=False, url=None,
                               **kwargs) -> Union[str, "RequestResult"]:
        result, _ = await self._client.process_requests(
            request_params=[
                await self.wrap_to_request_params(messages, model, preprocess_msg=preprocess_msg, **kwargs)],
    ...
    def chat_completions_sync(self, messages: list, model: str, return_raw=False, url=None, **kwargs):
        return asyncio.run(
            self.chat_completions(messages=messages, model=model, return_raw=return_raw, url=url, **kwargs))
    ...
    async def chat_completions_batch(self,
        ...
        if return_raw:
            return results, progress.summary(print_to_console=False) if return_summary else results
        content_list = []
        ...
        return content_list, progress.summary(print_to_console=False) if return_summary else content_list


    def chat_completions_batch_sync(self,
                                    messages_list: list[list],
                                    model: str,
                                    url=None,
                                    return_raw=False,
                                    ...
                                    return_summary=False,
                                    **kwargs):
        return asyncio.run(self.chat_completions_batch(messages_list=messages_list,
                                                       model=model,
                                                       url=url,
    ...
    async def iter_chat_completions_batch(
        ...
        async for batch_result in self._client.aiter_stream_requests(
            ...
            for result in batch_result.completed_requests:
                if return_raw:
                    yield result.data, batch_result.progress.summary(print_to_console=False) if return_summary else result.data
                else:
                    try:
                        content = result.data["choices"][0]["message"]["content"]
                    except Exception as e:
                        logger.warning(f"Error in chat_completions_batch: {e}\n {result=}\n set content to None")
                        content = None
                    yield content, batch_result.progress.summary(print_to_console=False) if return_summary else content
