...
class QrTranslator:
    ...
    def translate_quantum_circuit(run_input : QuantumCircuit, 
        ...
        if (qubit_layout is not None ):
            if (len(qubit_layout) == run_input.num_qubits):
                 qubit_lookup_vector =  qubit_layout
            else:
                raise Exception("Qubit Layout is not matching number of qubits in the circuit")
        else:
