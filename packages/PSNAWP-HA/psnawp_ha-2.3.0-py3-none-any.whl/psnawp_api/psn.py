...
class PlaystationNetwork:
    ...
    async def retrieve_npsso_token(self) -> str | None:
        """Returns NPSSO token if user is already authenticated to the playstation network"""
        client = aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=2))
        try:
            async with (
                    client as session,
                    session.get("https://ca.account.sony.com/api/v1/ssocookie") as response,
                ):
                if response.ok:
