...
def collect_2q_runs(dag):
    ...
    def filter_fn(node):
        if isinstance(node, DAGOpNode):
            return (
                isinstance(node.op, Instruction)
...
class MakeBlocks(TransformationPass):
    def run(self, dag):
        blocks = [b for b in collect_2q_runs(dag) if len(b) > 1]
...
def qc_to_rust(qc_blocks):
    ops = []
    cargs_dict = dict()
    has_conditional_gates = any(gi.operation.condition is not None for gi in qc_blocks)
    ...
    for op_id, gi in enumerate(qc_blocks):
        ...
        if len(gi.qubits) == 2:
            ...
            if gi.operation.condition is None and gi.operation.name in OPS_DICT:
        ...
        add_mrw = has_conditional_gates and (
            (gi.operation.condition is not None) or (len(gi.clbits) > 0)
        )
