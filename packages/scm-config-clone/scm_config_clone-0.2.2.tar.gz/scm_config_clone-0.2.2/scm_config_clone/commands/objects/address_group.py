...
def address_groups(
    ...
    try:
        ...
        address_group_objects = source_address_groups.list(
            folder=folder,
            exact_match=True,
        ...
        logger.info(
            f"Retrieved {len(address_group_objects)} address group objects from source tenant folder '{folder}'."
        )
    except Exception as e:
    ...
    if address_group_objects and not quiet_mode:
        group_table = []
        for group in address_group_objects:
            if group.static:
                group_type = "Static"
                group_value = ", ".join(group.static)
            elif group.dynamic:
                group_type = "Dynamic"
                group_value = group.dynamic.filter
            else:
                group_type = "Unknown"
                group_value = "N/A"
        ...
        typer.echo(
            tabulate(
                group_table,
                headers=["Name", "Folder", "Type", "Value", "Description"],
                tablefmt="fancy_grid",
            )
        )
    elif not address_group_objects:
        typer.echo("No address group objects found in the source folder.")

    # Prompt if not auto-approved and objects exist
    if address_group_objects and not auto_approve:
    ...
    for src_obj in address_group_objects:
        try:
    ...
    if created_objs and not quiet_mode:
        ...
        for obj in created_objs:
            if obj.static:
                group_type = "Static"
                group_value = ", ".join(obj.static)
            elif obj.dynamic:
                group_type = "Dynamic"
                group_value = obj.dynamic.filter
            else:
                group_type = "Unknown"
                group_value = "N/A"
