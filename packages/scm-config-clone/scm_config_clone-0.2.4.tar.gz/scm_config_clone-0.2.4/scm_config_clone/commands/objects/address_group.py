# scm_config_clone/commands/objects/address_group.py
...
def build_create_params(
    src_obj: AddressGroupResponseModel, folder: str
) -> Dict[str, Any]:
    """
    Construct the dictionary of parameters required to create a new address group object.

    Given an existing AddressGroupResponseModel (source object) and a target folder,
    this function builds a dictionary with all necessary fields for creating
    a new address group in the destination tenant. It identifies the group type
    (static or dynamic) and uses `model_dump` on a Pydantic model to ensure
    only valid, explicitly set fields are included.

    Args:
        src_obj: The AddressGroupResponseModel representing the source address group object.
        folder: The folder in the destination tenant where the object should be created.

    Returns:
        A dictionary containing the fields required for `AddressGroup.create()`.
        This dictionary is validated and pruned by AddressGroupCreateModel.

    Raises:
        ValueError: If the source object does not contain a valid group type.
    """
    data = {
        "name": src_obj.name,
        "folder": folder,
    ...
    # Determine which group type is set
    if src_obj.static:
        data["static"] = src_obj.static
    elif src_obj.dynamic:
        data["dynamic"] = {"filter": src_obj.dynamic.filter}
    else:
        raise ValueError(f"No valid group type found for {src_obj.name}")

    create_model = AddressGroupCreateModel(**data)
...
def address_groups(
    folder: Optional[str] = typer.Option(
        ...
        help="The folder to focus on when retrieving and cloning address groups.",
    ),
    ...
    """
    Clone address group objects from a source SCM tenant to a destination SCM tenant.

    This Typer CLI command automates the process of retrieving address group objects
    ...
    3. Authenticate to the source tenant and retrieve address group objects from the given folder.
    ...
    Args:
        folder: The source folder from which to retrieve address group objects.
    ...
    """
    typer.echo("🚀 Starting address group objects cloning...")
    ...
    # Retrieve address group objects from source
    try:
        source_address_groups = AddressGroup(source_client, max_limit=5000)
        source_objects = source_address_groups.list(
            folder=folder,
            exact_match=True,
        ...
        logger.info(
            f"Retrieved {len(source_objects)} address group objects from source tenant folder '{folder}'."
        )
    except Exception as e:
        logger.error(f"Error retrieving address group objects from source: {e}")
        raise typer.Exit(code=1)

    # Retrieve address group objects from destination
    try:
        destination_address_groups = AddressGroup(destination_client, max_limit=5000)
        destination_objects = destination_address_groups.list(
            folder=folder,
            exact_match=True,
            exclude_folders=exclude_folders_list,
            exclude_snippets=exclude_snippets_list,
        ...
        logger.info(
            f"Retrieved {len(destination_objects)} address group objects from destination tenant folder '{folder}'."
        )
    except Exception as e:
        logger.error(f"Error retrieving address group objects from destination: {e}")
        raise typer.Exit(code=1)
    ...
    # Create address group objects in destination
    destination_address_groups = AddressGroup(destination_client, max_limit=5000)
    created_objs: List[AddressGroupResponseModel] = []
    error_objects: List[List[str]] = []

    for src_obj in objects_to_create:
        if dry_run:
            logger.info(
                f"Skipping creation of address group object in destination (dry run): {src_obj.name}"
            )
            continue

        if create_report:
            with open("result.csv", "a") as f:
                f.write(f"Address Group,{src_obj.name},{src_obj.folder}\n")
        ...
        try:
            new_obj = destination_address_groups.create(create_params)
            created_objs.append(new_obj)
            logger.info(f"Created address group object in destination: {new_obj.name}")
        except (
            InvalidObjectError,
            MissingQueryParameterError,
            NameNotUniqueError,
    ...
    if created_objs and not quiet_mode:
        typer.echo("\nSuccessfully created the following address group objects:")
        created_table = []
    ...
    if error_objects and not quiet_mode:
        typer.echo("\nSome address group objects failed to be created:")
        typer.echo(
            tabulate(
                error_objects,
    ...
    if commit_and_push and created_objs:
        try:
            commit_params = {
                "folders": [folder],
                "description": "Cloned address group objects",
                "sync": True,
            }
            result = destination_address_groups.commit(**commit_params)
            job_status = destination_address_groups.get_job_status(result.job_id)
            logger.info(
                f"Commit job ID {result.job_id} status: {job_status.data[0].status_str}"
            )
        except Exception as e:
            logger.error(f"Error committing address group objects in destination: {e}")
            raise typer.Exit(code=1)
    else:
        if created_objs and not commit_and_push:
            ...
            logger.info("No new address group objects were created, skipping commit.")

    typer.echo("🎉 Address group objects cloning completed successfully! 🎉")
