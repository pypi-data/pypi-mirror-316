...
from .errors import BotifyError, InvalidAPIKey, NoGuildAccess, NotFound, QuotaExceeded
from .models import GuildActivity, MemberActivity, MemberStats, UserStats

DEFAULT_DAYS = 14
BASE_URL = "https://api.botify-bot.de/v1/"
...
class AsyncBotifyAPI:
    ...
    def __init__(self, api_key: str | None = None, session: httpx.AsyncClient | None = None):
        ...
        if api_key is None:
            load_dotenv()
            api_key = os.getenv("BOTIFY_KEY")
            if api_key is None:
                raise InvalidAPIKey(
                    "Please provide an API key or set the BOTIFY_KEY environment variable."
                )
