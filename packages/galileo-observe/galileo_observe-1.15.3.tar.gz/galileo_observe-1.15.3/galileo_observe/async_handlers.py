...
class GalileoObserveAsyncCallback(AsyncCallbackHandler):
        ...
        if record.node_id == record.chain_root_id:
            for node_id, batch_record in self.records.copy().items():
                if batch_record.chain_root_id == record.chain_root_id:
                    ...
                    if (
                        batch_record.node_type != NodeType.chain
                        or node_id in self.parent_child_mapping.keys()
                    ):
    ...
    async def on_chat_model_start(
        ...
        **kwargs: Any,
        ...
        metadata = kwargs.get("metadata")
        invocation_params = kwargs.get("invocation_params", {})
        model = invocation_params.get(
            "model", invocation_params.get("_type", "undefined-type")
        )
        temperature = invocation_params.get("temperature")
    ...
    async def on_llm_end(self, response: LLMResult, run_id: UUID, **kwargs: Any) -> Any:
        ...
        if response.llm_output:
            ...
            num_output_tokens = usage.get(
                "completion_tokens", usage.get("output_tokens", None)
            )
    ...
    async def on_tool_end(
        ...
        **kwargs: Any,
        ...
        model_dict.update(
            output_text=dumps(
                output, default=GalileoObserveAsyncCallback.json_serializer
            ),
            latency_ms=latency_ms,
            status_code=HTTPStatus.OK,
        )
