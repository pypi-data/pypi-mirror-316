...
class VMs():
    ...
    def __get_public_ip_address(self, publicipid, exit=False):
        url = f"https://management.azure.com/{publicipid}?api-version=2023-09-01"
    ...
    def __get_instanceview(self, vmid, exit=False):
        url = f"https://management.azure.com/{vmid}/instanceView?api-version=2024-03-01"
        response = self.request.do_request(self.auth.arm_access_token, url, "GET", None)
        if response.status_code == 200:
    ...
    def __get_network_interface(self, networkinterfaceid, exit=False):
        url = f"https://management.azure.com/{networkinterfaceid}?api-version=2023-09-01"
        response = self.request.do_request(self.auth.arm_access_token, url, "GET", None)
        if response.status_code == 200:
    ...
    def get_vms(self, subscriptionId, resourcegroup, exit=False):
        if self.auth.arm_access_token is not None:
            if resourcegroup is None:
                ...
                url = f"https://management.azure.com/subscriptions/{subscriptionId}/providers/Microsoft.Compute/virtualMachines?api-version=2024-03-01"
            else:
                print(f"{Format.BOLD_START}{Format.BLUE}\n[*] Reading virtual machine info [{resourcegroup}]{Format.END}")
                url = f"https://management.azure.com/subscriptions/{subscriptionId}/resourceGroups/{resourcegroup}/providers/Microsoft.Compute/virtualMachines?api-version=2024-03-01"
            response = self.request.do_request(self.auth.arm_access_token, url, "GET", None)
            if response.status_code == 200:
    ...
    def get_vm_perms(self, subscriptionId, resourcegroup, vmname, exit=False):
        if self.auth.arm_access_token is not None:
            url = f"https://management.azure.com/subscriptions/{subscriptionId}/resourceGroups/{resourcegroup}/providers/Microsoft.Compute/virtualMachines/{vmname}/providers/Microsoft.Authorization/permissions?api-version=2015-07-01"
            response = self.request.do_request(self.auth.arm_access_token, url, "GET", None)
            if response.status_code == 200:
                vms_data = json.loads(response.content.decode('utf-8'))
    ...
    def add_vmcommand(self, subscriptionId, resourceGroup, vmname, system, payloadfile, exit=False):
        if self.auth.arm_access_token is not None:
            url = f"https://management.azure.com/subscriptions/{subscriptionId}/resourceGroups/{resourceGroup}/providers/Microsoft.Compute/virtualMachines/{vmname}/runCommand?api-version=2022-03-01"
