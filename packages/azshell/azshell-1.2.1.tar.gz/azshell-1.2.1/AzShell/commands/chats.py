...
class Chats:
    ...
    def get_chats(self, userid, chatid, top, nextlinkcount=0, nextlink=None, exit=False):
        if nextlink is not None:
            ...
            if top is None:
                top = '5'
            url = f"https://graph.microsoft.com/v1.0/users/{userid}/chats/{chatid}/messages?$nextLink"
        elif self.search is None:
            print(f"{Format.BOLD_START}{Format.BLUE}\n[*] Reading all chats{Format.END}")
            if top is None:
                top = '50'
            url = f"https://graph.microsoft.com/v1.0/users/{userid}/chats?$top={top}&$expand=members"
        else:
            print(f"{Format.BOLD_START}{Format.BLUE}\n[*] Reading messages [{self.search}]{Format.END}")
            if top is None:
                top = '10'
            url = f"https://graph.microsoft.com/v1.0/users/{userid}/chats/getAllMessages?$top={top}"
            ...
            if chatid is None and self.search is not None:
                ...
                for message in messages_data["value"]:
                    if self.search.lower() in message["body"]["content"].lower():
                        print(f"{str(message['body']['content'])} [{message['chatId']}]")
            elif chatid is None:
    ...
    def add_chatmessage(self, userid, chatid, content, exit=False):
        ...
        if response.status_code == 204:
            ...
            print(f"{Format.CYAN} ChatMessageId: {Format.END}{message_data['id']}")
            print(f"{Format.CYAN} Content: {Format.END}{message_data['body']['content']}")
        elif response.status_code == 403:
            print(f"{Format.BOLD_START}{Format.RED}\n[!] Insufficient privileges to complete the operation{Format.END}")
        elif response.status_code == 401:
            print(f"{Format.BOLD_START}{Format.GREEN}\n[*] Access token expired, requesting a new one...{Format.END}")
    ...
    def del_chatmessage(self, userid, chatid, messageid, exit=False):
        url = f"https://graph.microsoft.com/v1.0/users/{userid}/chats/{chatid}/messages/{messageid}/softDelete"
