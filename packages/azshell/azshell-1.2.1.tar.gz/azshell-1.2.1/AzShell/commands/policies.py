...
class Policies:
    ...
    def __get_transitiveMembersSearch(self, groupid, userid, nextlink=None, exit=False):
        if nextlink is None:
            url = f"https://graph.microsoft.com/v1.0/groups/{groupid}/transitiveMembers/microsoft.graph.user?$top=999&$nextLink"
        else:
            url = nextlink
    ...
    def __get_rolemembers(self, roleid, userid, exit=False):
        url = f"https://graph.microsoft.com/beta/directoryRoles/roleTemplateId={roleid}/members"
        response = self.request.do_request(self.auth.graph_access_token, url, "GET", None)
        if response.status_code == 200:
            ...
            if response.status_code == 200:
                ...
                for policy in self.data["value"]:
                    ...
                    if print_policy:
                        ...
                        print(f"{Format.CYAN} PolicyId: {Format.END}{policy['id']}")
                        print(f"{Format.CYAN} State: {Format.END}{str(policy['state'])}")
                        print(f"{Format.CYAN} CreatedDateTime: {Format.END}{str(policy['createdDateTime'])}")
                        print(f"{Format.CYAN} ModifiedDateTime: {Format.END}{str(policy['modifiedDateTime'])}")
                        print(f"{Format.CYAN} Conditions: {Format.END}")
                        policylist = []
                        policydic = {}
                        roles = False
                        ...
                        for policyid in policylist:
                            if re.match(r'^([0-9a-f]{8})-([0-9a-f]{4})-([0-9a-f]{4})-([0-9a-f]{4})-([0-9a-f]{12})$', policyid.lower()):
                                idlist.append(policyid)
                        namedic = self.__get_by_ids(idlist)
                        for policyvalue in policydic:
                            print(f"{Format.PURPLE}  {policyvalue}: {Format.END}")
                            for policyelement in policydic[policyvalue]:
                                if policyelement in namedic:
                                    print(f"    {namedic[policyelement]} [{policyelement}]")
                                elif policyelement in rolesdic:
                                    print(f"    {rolesdic[policyelement]} [{policyelement}]")
                                else:
                                    print(f"    {policyelement}")
                        if policy["grantControls"] is not None:
                            print(f"{Format.PURPLE}  GrantControls: {Format.END}")
                            if policy["grantControls"]["builtInControls"]:
                                print(f"    builtInControls: {','.join(policy['grantControls']['builtInControls'])}")
                            if policy["grantControls"]["customAuthenticationFactors"]:
                                print(f"    customAuthenticationFactors:{','.join(policy['grantControls']['customAuthenticationFactors'])}")
                            if policy["grantControls"]["authenticationStrength"]:
                                print(f"    authenticationStrength: ")
                                print(f"     {policy['grantControls']['authenticationStrength']['displayName']} [{policy['grantControls']['authenticationStrength']['id']}]")
                                if "requirementsSatisfied" in policy["grantControls"]["authenticationStrength"]:
                                    print(f"      requirementsSatisfied: {policy['grantControls']['authenticationStrength']['requirementsSatisfied']}")
                                if "allowedCombinations" in policy["grantControls"]["authenticationStrength"]:
                                    print(f"      allowedCombinations: {str(policy['grantControls']['authenticationStrength']['allowedCombinations'])}")
                        if policy["sessionControls"] is not None:
                            print(f"{Format.PURPLE}  SessionControls: {Format.END}")
                            print(f"    disableResilienceDefaults: {str(policy['sessionControls']['disableResilienceDefaults'])}")
                            print(f"    applicationEnforcedRestrictions: {str(policy['sessionControls']['applicationEnforcedRestrictions'])}")
                            print(f"    cloudAppSecurity: {str(policy['sessionControls']['cloudAppSecurity'])}")
                            print(f"    persistentBrowser: {str(policy['sessionControls']['persistentBrowser'])}")
                            print(f"    signInFrequency: {str(policy['sessionControls']['signInFrequency'])}")
                if self.allinfo:
