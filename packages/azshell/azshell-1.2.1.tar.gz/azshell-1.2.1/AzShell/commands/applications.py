...
class Applications:
    ...
    def __get_applicationowners(self, appid, exit=False):
        url = f"https://graph.microsoft.com/v1.0/applications/{appid}/owners"
        ...
        if response.status_code == 200:
            ...
            for owner in owners_data["value"]:
                ...
                print(f"  {owner['displayName']} [{owner['id']}]")
    ...
    def __get_serviceprincipalowners(self, serviceprincipalid, exit=False):
        url = f"https://graph.microsoft.com/v1.0/servicePrincipals/{serviceprincipalid}/owners"
        response = self.request.do_request(self.auth.graph_access_token, url, "GET", None)
        if response.status_code == 200:
            owners_data = json.loads(response.content.decode('utf-8'))
            for owner in owners_data["value"]:
    ...
    def __get_approleassignments(self, appid, exit=False):
        url = f"https://graph.microsoft.com/v1.0/servicePrincipals(appId='{appid}')/appRoleAssignments"
        response = self.request.do_request(self.auth.graph_access_token, url, "GET", None)
        if response.status_code == 200:
            approles_data = json.loads(response.content.decode('utf-8'))
            for approle in approles_data["value"]:
                self.data.append(approle)
                msgraph_roles = {msgraphrole["id"]: msgraphrole["value"] for msgraphrole in self.auth.msgraphdata["appRoles"]}
                if approle["appRoleId"] in msgraph_roles:
                    print(f"{Format.PURPLE}  [Application] {Format.END}{msgraph_roles[approle['appRoleId']]}")
        elif response.status_code == 403:
            print(f"{Format.BOLD_START}{Format.RED}\n  [!] Insufficient privileges to complete the operation{Format.END}")
        elif response.status_code == 401:
            print(f"{Format.BOLD_START}{Format.GREEN}\n[*] Access token expired, requesting a new one...{Format.END}")
    ...
    def __get_delegated_permissions(self, clientid, exit=False):
        url = f"https://graph.microsoft.com/v1.0/oauth2PermissionGrants/?$filter=clientId eq '{clientid}'"
        response = self.request.do_request(self.auth.graph_access_token, url, "GET", None)
        if response.status_code == 200:
            ...
            for dpermission in dpermissions_data["value"]:
                ...
                if dpermission["resourceId"] == self.auth.msgraphdata["id"]:
                    for parse_dpermission in dpermission["scope"].split(" "):
                        print(f"{Format.PURPLE}  [Delegated] {Format.END}{parse_dpermission}")
        elif response.status_code == 403:
            print(f"{Format.BOLD_START}{Format.RED}\n  [!] Insufficient privileges to complete the operation{Format.END}")
        elif response.status_code == 401:
            print(f"{Format.BOLD_START}{Format.GREEN}\n[*] Access token expired, requesting a new one...{Format.END}")
        ...
        if not self.allinfo and self.search is None:
            ...
            if self.allinfo:
                ...
                if re.match('^([0-9a-f]{8})-([0-9a-f]{4})-([0-9a-f]{4})-([0-9a-f]{4})-([0-9a-f]{12})$', self.search.lower()):
                    url = f'https://graph.microsoft.com/v1.0/applications/{self.search}?$orderby=displayName'
                    search_id = True
                else:
                    url = f'https://graph.microsoft.com/v1.0/applications?$search="displayName:{self.search}" OR "publisherDomain:{self.search}" OR "identifierUris:{self.search}" OR "appId:{self.search}"&$orderby=displayName'
            response = self.request.do_request(self.auth.graph_access_token, url, "GET", None)
            if response.status_code == 200:
                ...
                for application in applications_data["value"]:
                    ...
                    print(f"{Format.CYAN} ObjectId: {Format.END}{application['id']}")
                    print(f"{Format.CYAN} AppId: {Format.END}{application['appId']}")
                    print(f"{Format.CYAN} CreatedDateTime: {Format.END}{str(application['createdDateTime'])}")
                    print(f"{Format.CYAN} PublisherDomain: {Format.END}{str(application['publisherDomain'])}")
                    print(f"{Format.CYAN} ApplicationRoles: {Format.END}")
                    for approles in application["appRoles"]:
                        print(f"  {approles['displayName']}: {approles['description']} (Origin: {approles['origin']}, isEnabled: {str(approles['isEnabled'])}) [{approles['id']}]")
                    print(f"{Format.CYAN} KeyCredentials: {Format.END}{str(application['keyCredentials'])}")
                    print(f"{Format.CYAN} PasswordCredentials: {Format.END}")
                    for passwordcredentials in application["passwordCredentials"]:
                        print(f"  {str(passwordcredentials['displayName'])}: {str(passwordcredentials['hint'])}***** (startDateTime: {str(passwordcredentials['startDateTime'])}, endDateTime: {str(passwordcredentials['endDateTime'])}) [{str(passwordcredentials['keyId'])}]")
                    print(f"{Format.CYAN} Owners: {Format.END}")
    ...
    def get_serviceprincipals(self, exit=False):
        if not self.allinfo and self.search is None:
            ...
            if self.allinfo:
                ...
                if re.match('^([0-9a-f]{8})-([0-9a-f]{4})-([0-9a-f]{4})-([0-9a-f]{4})-([0-9a-f]{12})$', self.search.lower()):
                    url = f'https://graph.microsoft.com/v1.0/serviceprincipals/{self.search}?$orderby=displayName'
                    search_id = True
                else:
                    url = f'https://graph.microsoft.com/v1.0/serviceprincipals?$search="displayName:{self.search}" OR "appId:{self.search}"&$orderby=displayName'
            response = self.request.do_request(self.auth.graph_access_token, url, "GET", None)
            if response.status_code == 200:
                ...
                for serviceprincipal in serviceprincipals_data["value"]:
                    ...
                    print(f"{Format.CYAN} ObjectId: {Format.END}{serviceprincipal['id']}")
                    print(f"{Format.CYAN} AppId: {Format.END}{serviceprincipal['appId']}")
                    print(f"{Format.CYAN} Description: {Format.END}{str(serviceprincipal['description'])}")
                    print(f"{Format.CYAN} CreatedDateTime: {Format.END}{str(serviceprincipal['createdDateTime'])}")
                    print(f"{Format.CYAN} Alternative Names: {Format.END}{str(serviceprincipal['alternativeNames'])}")
                    print(f"{Format.CYAN} Application roles: {Format.END}")
                    for approles in serviceprincipal["appRoles"]:
                        print(f"  {approles['displayName']}: {approles['description']} (Origin: {approles['origin']}, isEnabled: {str(approles['isEnabled'])}) [{approles['id']}]")
                    print(f"{Format.CYAN} Key credentials: {Format.END}{str(serviceprincipal['keyCredentials'])}")
                    print(f"{Format.CYAN} Password credentials: {Format.END}")
                    for passwordcredentials in serviceprincipal["passwordCredentials"]:
                        print(f"  {str(passwordcredentials['displayName'])}: {str(passwordcredentials['hint'])}***** (startDateTime: {str(passwordcredentials['startDateTime'])}, endDateTime: {str(passwordcredentials['endDateTime'])}) [{str(passwordcredentials['keyId'])}]")
                    print(f"{Format.CYAN} Permissions: {Format.END}")
    ...
    def add_application(self, name, exit=False):
        ...
        if response.status_code == 201:
            ...
            print(f"{Format.CYAN} ObjectId: {Format.END}{response_parse['id']}")
            print(f"{Format.CYAN} AppId: {Format.END}{response_parse['appId']}")
            print(f"{Format.CYAN} Description: {Format.END}{str(response_parse['description'])}")
            print(f"{Format.CYAN} CreatedDateTime: {Format.END}{str(response_parse['createdDateTime'])}")
        elif response.status_code == 403:
            print(f"{Format.BOLD_START}{Format.RED}\n[!] Insufficient privileges to complete the operation{Format.END}")
        elif response.status_code == 401:
            print(f"{Format.BOLD_START}{Format.GREEN}\n[*] Access token expired, requesting a new one...{Format.END}")
    ...
    def add_serviceprincipalsecret(self, serviceprincipalid, secretname, exit=False):
        url = f"https://graph.microsoft.com/v1.0/serviceprincipals/{serviceprincipalid}/addPassword"
        body = {
        ...
        if response.status_code == 200:
            ...
            response_parse = json.loads(response.content.decode('utf-8'))
            print(f"\n{Format.BOLD_START}{Format.YELLOW}{response_parse['displayName']}{Format.END}")
            print(f"{Format.CYAN} Password: {Format.END}{response_parse['secretText']}")
            print(f"{Format.CYAN} KeyId: {Format.END}{response_parse['keyId']}")
            print(f"{Format.CYAN} StartDateTime: {Format.END}{response_parse['startDateTime']}")
            print(f"{Format.CYAN} EndDateTime: {Format.END}{response_parse['endDateTime']}")
        elif response.status_code == 403:
            print(f"{Format.BOLD_START}{Format.RED}\n[!] Insufficient privileges to complete the operation{Format.END}")
        elif response.status_code == 401:
            print(f"{Format.BOLD_START}{Format.GREEN}\n[*] Access token expired, requesting a new one...{Format.END}")
    ...
    def del_application(self, appid, exit=False):
        url = f"https://graph.microsoft.com/v1.0/applications/{appid}"
    ...
    def del_serviceprincipal(self, serviceprincipalid, exit=False):
        url = f"https://graph.microsoft.com/v1.0/serviceprincipals/{serviceprincipalid}"
        response = self.request.do_request(self.auth.graph_access_token, url, "DELETE", None)
        if response.status_code == 204:
    ...
    def del_serviceprincipalsecret(self, serviceprincipalid, keyid, exit=False):
        url = f"https://graph.microsoft.com/v1.0/serviceprincipals/{serviceprincipalid}/removePassword"
        body = {
            'keyId': keyid
        }
        response = self.request.do_request(self.auth.graph_access_token, url, "POST", body)
    ...
    def add_appsecret(self, appid, secretname, exit=False):
        url = f"https://graph.microsoft.com/v1.0/applications(appId='{{{appid}}}')/addPassword"
        body = {
            'passwordCredential': {'displayName': secretname}
        }
        response = self.request.do_request(self.auth.graph_access_token, url, "POST", body)
    ...
    def del_appsecret(self, appid, keyid, exit=False):
        url = f"https://graph.microsoft.com/v1.0/applications(appId='{{{appid}}}')/removePassword"
        body = {
            'keyId': keyid
        }
        response = self.request.do_request(self.auth.graph_access_token, url, "POST", body)
    ...
    def add_approleassignment(self, serviceprincipalid, resourceid, approleid, exit=False):
        url = f'https://graph.microsoft.com/v1.0/servicePrincipals/{serviceprincipalid}/appRoleAssignedTo'
        body = {
        ...
        if response.status_code == 201:
            ...
            print(f"{Format.CYAN} AppRoleAssignmentId: {Format.END}{response_parse['id']}")
            print(f"{Format.CYAN} ResourceDisplayName: {Format.END}{response_parse['resourceDisplayName']}")
            print(f"{Format.CYAN} PermissionId: {Format.END}{response_parse['appRoleId']}")
        elif response.status_code == 403:
            print(f"{Format.BOLD_START}{Format.RED}\n[!] Insufficient privileges to complete the operation{Format.END}")
        elif response.status_code == 401:
            print(f"{Format.BOLD_START}{Format.GREEN}\n[*] Access token expired, requesting a new one...{Format.END}")
    ...
    def del_approleassignment(self, serviceprincipalid, approleassignmentid, exit=False):
        url = f'https://graph.microsoft.com/v1.0/servicePrincipals/{serviceprincipalid}/appRoleAssignedTo/{approleassignmentid}'
        response = self.request.do_request(self.auth.graph_access_token, url, "DELETE", None)
        if response.status_code == 204:
