    ...
    for reference_transcript in tqdm(gene, desc=f'Processing {mut_id}...'):
        ...
        if splicing_engine is None:
            ...
            if splicing_db is not None:
                missplicing = splicing_db.get_mutation_data(engine=splicing_engine, mut_id=mut_id, gene=gene.gene_name, transcript_id=reference_transcript.transcript_id)
                no_splicing_record = missplicing is None

            if missplicing is None:
                missplicing = Missplicing(find_transcript_missplicing_seqs(
