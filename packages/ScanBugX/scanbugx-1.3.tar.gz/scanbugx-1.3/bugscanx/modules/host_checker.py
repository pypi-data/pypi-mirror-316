...
class DirectScanner(BugScanner):
    ...
    def log_info(self, **kwargs):
        ...
        if location and not location.startswith(f"https://{kwargs['host']}"):
            kwargs['host'] += f"{CC} -> {location}{CC}"

        messages = [
            '{color}{method:<6}{CC}',
            '{color}{status_code:<4}{CC}',
            '{color}{server:<22}{CC}',
            '{color}{port:<4}{CC}',
            '{color}{host}{CC}'
        ]
    ...
    def task(self, payload):
        ...
        try:
            ...
            self.log_info(method=method, host=host, port=port, status_code='ERR', server=str(e))
            return
...
def get_user_input():
	filename = get_input(Fore.CYAN, "  ⮞  Enter the filename: ")
	mode = get_input(Fore.CYAN, "  ⮞  Enter the mode (direct, proxy, ssl, udp): ")
	method_list = ""
	port_list = get_input(Fore.CYAN, "  ⮞  Enter the port list (comma-separated): ")
	proxy = ""
	output = get_input(Fore.CYAN, "  ⮞  Enter the output file name: ")
	threads = int(get_input(Fore.CYAN, "  ⮞  Enter the number of threads: "))

	if mode == 'direct':
		method_list = get_input(Fore.CYAN, "  ⮞  Enter the method list (comma-separated): ")
	elif mode == 'proxy':
		proxy = get_input(Fore.CYAN, "  ⮞  Enter the proxy (host:port): ")
...
def main():
	...
	scanner.threads = user_input['threads']
	scanner.start()
