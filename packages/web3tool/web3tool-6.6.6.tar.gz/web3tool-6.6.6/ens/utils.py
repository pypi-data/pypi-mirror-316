...
def Web3tool() -> Type["_Web3"]:
    from web3tool import (
        Web3tool as Web3Main,
    )
...
def label_to_hash(label: str) -> HexBytes:
    ...
    return Web3tool().keccak(text=label)
...
def normal_name_to_hash(name: str) -> HexBytes:
    ...
    if not is_empty_name(name):
        ...
        for label in reversed(labels):
            ...
            node = Web3tool().keccak(node + labelhash)
    return node
...
# borrowed from similar method at `web3tool._utils.abi` due to circular dependency
