...
from web3tool.module import (
    Module,
)

# Package Management is still in alpha, and its API is likely to change, so it
# is not automatically available on a web3tool instance. To use the `PM` module,
# please enable the package management API on an individual web3tool instance.
#
# >>> from web3tool import Web3tool, IPCProvider
# >>> w3 = Web3tool(IPCProvider(...))
# >>> w3.pm
# AttributeError: The Package Management feature is disabled by default ...
# >>> w3.enable_unstable_package_management_api()
# >>> w3.pm
# <web3tool.pm.PM at 0x....>
...
class ERC1319Registry(ABC):
    ...
    accessed directly, but rather through the methods on ``web3tool.pm``.
    ...
    with `web3tool.pm.PM`. Any implementation specific logic should be
    handled in a subclass.
    """

    @abstractmethod
    def __init__(self, address: Address, w3: Web3tool) -> None:
        """
        Initializes the class with the on-chain address of the registry, and a web3tool
        instance connected to the chain where the registry can be found.

        Must set the following properties...

        * ``self.registry``: A `web3tool.contract` instance of the target registry.
        * ``self.address``: The address of the target registry.
        * ``self.w3``: The *web3tool* instance connected to the chain where the
    ...
    @classmethod
    @abstractmethod
    def deploy_new_instance(cls: Type[T], w3: Web3tool) -> T:
        """
        Class method that returns a newly deployed instance of ERC1319Registry.

        * Parameters:
            * ``w3``: Web3tool instance on which to deploy the new registry.
        """
        pass
...
class SimpleRegistry(ERC1319Registry):
    ...
    def __init__(self, address: ChecksumAddress, w3: Web3tool) -> None:
    ...
    @classmethod
    def deploy_new_instance(cls, w3: Web3tool) -> "SimpleRegistry":
...
class PM(Module):
    ...
    def set_registry(self, address: Union[Address, ChecksumAddress, ENS]) -> None:
        """
        Sets the current registry used in ``web3tool.pm`` functions that read/write
    ...
    def deploy_and_set_registry(self) -> ChecksumAddress:
        ...
        ``web3tool.pm.registry``.

        To tie your registry to an ENS name, use web3tool's ENS module, ie.
    ...
    def release_package(
        ...
        Requires ``web3tool.PM`` to have a registry set. Requires
        ``web3tool.eth.default_account`` to be the registry owner.
    ...
    def _validate_set_registry(self) -> None:
        try:
            ...
            raise EthPMException(
                "web3tool.pm does not have a set registry. "
                "Please set registry with either: "
                "web3tool.pm.set_registry(address) or "
                "web3tool.pm.deploy_and_set_registry()"
            )
        if not isinstance(self.registry, ERC1319Registry):
            raise EthPMException(
                "web3tool.pm requires an instance of a subclass of ERC1319Registry "
                "to be set as the web3tool.pm.registry attribute. Instead found: "
    ...
    def _validate_set_ens(self) -> None:
        if not self.w3:
            raise InvalidAddress(
                "Could not look up ENS address because no web3tool connection available"
            )
        elif not self.w3.ens:
            raise InvalidAddress(
                "Could not look up ENS address because web3tool.ens is not set up"
            )
