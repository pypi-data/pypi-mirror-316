...
class RequestManager:
    logger = logging.getLogger("web3tool.RequestManager")
    ...
    def __init__(
        ...
        if middlewares is None:
            middlewares = (
                ...
                else self.default_middlewares(cast("Web3tool", w3))
            )

        self.middleware_onion = NamedElementOnion(middlewares)

    w3: Union["AsyncWeb3", "Web3tool"] = None
    _provider = None
    ...
    @staticmethod
    def default_middlewares(w3: "Web3tool") -> List[Tuple[Middleware, str]]:
        """
    ...
    def _make_request(
        ...
        request_func = provider.request_func(
            cast("Web3tool", self.w3), cast(MiddlewareOnion, self.middleware_onion)
        )
