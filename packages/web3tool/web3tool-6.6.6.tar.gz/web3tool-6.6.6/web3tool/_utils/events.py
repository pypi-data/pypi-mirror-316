...
if TYPE_CHECKING:
    ...
    from web3tool._utils.filters import (  # noqa: F401
        AsyncLogFilter,
        LogFilter,
    )
...
class EventFilterBuilder(BaseEventFilterBuilder):
    def deploy(self, w3: "Web3tool") -> "LogFilter":
        if not isinstance(w3, web3tool.Web3tool):
            raise ValueError(f"Invalid web3tool argument: got: {w3!r}")
...
class AsyncEventFilterBuilder(BaseEventFilterBuilder):
    async def deploy(self, async_w3: "AsyncWeb3") -> "AsyncLogFilter":
        if not isinstance(async_w3, web3tool.AsyncWeb3):
            raise ValueError(f"Invalid web3tool argument: got: {async_w3!r}")
