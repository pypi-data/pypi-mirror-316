...
from web3tool._utils.utility_methods import (
    all_in_dict,
    any_in_dict,
)
from web3tool.constants import (
...
@curry
def fill_nonce(w3: "Web3tool", transaction: TxParams) -> TxParams:
...
@curry
def fill_transaction_defaults(w3: "Web3tool", transaction: TxParams) -> TxParams:
    """
...
def get_block_gas_limit(
    w3: "Web3tool", block_identifier: Optional[BlockIdentifier] = None
) -> int:
...
def get_buffered_gas_estimate(
    w3: "Web3tool", transaction: TxParams, gas_buffer: int = 100000
) -> int:
...
def get_required_transaction(w3: "Web3tool", transaction_hash: _Hash32) -> TxData:
...
def replace_transaction(
    w3: "Web3tool", current_transaction: TxData, new_transaction: TxParams
) -> HexBytes:
