...
if TYPE_CHECKING:
    from web3tool.main import BaseWeb3  # noqa: F401
...
def _validate_init_params_and_return_if_found(module_class: Any) -> List[str]:
    ...
    if len(module_init_params) > 1:
        raise UnsupportedOperation(
            "A module class may accept a single `Web3tool` instance as the first "
    ...
    for module_name, module_info in module_definitions.items():
        ...
        if hasattr(parent_module, module_name):
            raise AttributeError(
                ...
                " The web3tool object already has an attribute with that name"
            )

        # The parent module is the ``Web3tool`` instance on first run of the loop and w3 is
        ...
        if w3 is None:
            ...
            if isinstance(parent_module, Web3tool) or isinstance(parent_module, AsyncWeb3):
                w3 = parent_module

        module_init_params = _validate_init_params_and_return_if_found(module_class)
        if len(module_init_params) == 1:
            # Modules that need access to the ``Web3tool`` instance may accept the
            # instance as the first arg in their ``__init__()`` method. This is the
            # case for any module that inherits from ``web3tool.module.Module``.
            # e.g. def __init__(self, w3):
            setattr(parent_module, module_name, module_class(w3))
        else:
            # Modules need not take in a ``Web3tool`` instance in
