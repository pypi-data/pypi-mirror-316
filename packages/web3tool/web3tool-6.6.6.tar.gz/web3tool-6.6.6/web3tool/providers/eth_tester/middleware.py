...
def guess_from(w3: "Web3tool", _: TxParams) -> ChecksumAddress:
    if w3.eth.coinbase:
...
@curry
def fill_default(
    field: str, guess_func: Callable[..., Any], w3: "Web3tool", transaction: TxParams
) -> TxParams:
...
def default_transaction_fields_middleware(
    make_request: Callable[[RPCEndpoint, Any], Any], w3: "Web3tool"
...
async def async_ethereum_tester_middleware(  # type: ignore
    make_request, web3tool: "AsyncWeb3"
) -> Middleware:
    middleware = await async_construct_formatting_middleware(
        request_formatters=request_formatters, result_formatters=result_formatters
    )
    return await middleware(make_request, web3tool)
