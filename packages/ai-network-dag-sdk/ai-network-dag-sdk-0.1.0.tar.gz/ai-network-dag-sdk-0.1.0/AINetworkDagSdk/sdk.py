...
class AINetworkDagSdk:
    ...
    def add(self, cid, message, data, children):
        request = Content(cid=cid, message=message, data=data, children=children)
    ...
    def subscribe(self, topic):
        request = Subscription(topic=topic)
        response = self.stub.subscribe(request)
        print(f"Subscribe response: {response.success}")

    def close(self):
        self.channel.close()

    def uploadFile(self, file_path, chunk_size):
    ...
    def downloadFile(self, root_cid, output_path):
...
if __name__ == "__main__":
    ...
    try:
        ...
        cid = ai_network_dag_sdk.add("", "Test Message", b"Test Data", [])
        ...
        root_cid = ai_network_dag_sdk.uploadFile(large_file_path, chunk_size)
