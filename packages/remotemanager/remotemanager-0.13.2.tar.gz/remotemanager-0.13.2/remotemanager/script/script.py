...
class Script(SendableMixin):
    ...
    def __getattr__(self, item):
        if (
            item != "_subs"
            and hasattr(self, "_subs")
            and item in self._subs
        ):
            val = self._subs[item]
    ...
    def _link_subs(self) -> None:
        for sub in self.sub_objects:
            ...
            logger.debug(f"linking sub {sub.name}, value={sub.value}")
            # find all instances of an {expandable} section
            to_parse = re.findall(EVALUATION_PATTERN, str(sub.value))
            ...
            for section in to_parse:
                ...
                logger.debug(f"evaluating exchanged source: {tokenized.source}")
                try:
                    evaluated = eval(tokenized.source)
                except Exception as ex:
                    raise ValueError(
                        f"Exception ({type(ex)}) when evaluating source in:\n{section}"
                    )
                try:
    ...
    def required(self) -> list[str]:
        ...
        replaced = []  # if a value is marked as replaced, remove it
        for sub in self.sub_objects:
            if not sub.optional:
                required.append(sub.name)
            required += sub.requires
            replaced += sub.replaces
        return list(set(required) - set(replaced))
    ...
    def empty_treatment(self, style: Union[str | None]):
        if (
                ...
                style not in EMPTY_TREATMENT_STYLES
        ):
    ...
    def script(self, empty_treatment: Union[str, None] = None, **run_args) -> str:
        ...
        if (
                empty_treatment is not None and
                empty_treatment not in EMPTY_TREATMENT_STYLES
        ):
        ...
        for sub in self.sub_objects:
            if sub.hidden:  # don't print any values that are hidden
                ...
                value = str(sub.value)  # get the value in string form
            ...
            search = re.compile(
                REPLACEMENT_PATTERN.format(name=sub.name),
                re.IGNORECASE
            )
            script = re.sub(search, value, script)
        ...
        if values:
            for sub in self.sub_objects:
                ...
                if sub.value is not None and sub.value != sub.default:
                    ...
                    output = re.sub(search, value_insert, output, count=1)
