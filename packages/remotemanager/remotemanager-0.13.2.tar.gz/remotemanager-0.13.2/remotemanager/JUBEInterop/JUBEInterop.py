...
import logging
from remotemanager.connection.computers import BaseComputer, Substitution

logger = logging.getLogger(__name__)


class JUBETemplate(BaseComputer):
    """Extends BaseComputer to provide compatibility with JUBE platforms"""

    def __init__(self, template: str, platform: str):
    ...
    def from_repo(
        ...
        **url_args,
        ...
        url = os.path.join("/", branch, "max-inputs/platforms/", name)
        url = f"{repo}/-/raw" + url

        if local_dir is None:
            local_dir = os.path.split(name.strip("/"))[-1]
        ...
        return cls(template=template_path, platform=platform_path)
    ...
    def _extract_subs(self, link: bool = True) -> None:
        ...
        """
        super()._extract_subs(link=False)
        ...
        for subsituteset in root.findall("substituteset"):
            for change in subsituteset.iter():
                if change.tag != "sub":
                    ...
                    continue
                source = change.attrib["source"].strip("#").lower()
                dest = change.attrib["dest"].strip("$")

                if dest is None or dest == "":
                    # store this attribute for reporting if the destination is empty
                    empty[source] = change.attrib
                    continue

                if source in empty:
                    # if the source was previously flagged as missing a destination,
                    # but has one now, clear that flag here
                    del empty[source]
                ...
                if source in self.substitution_dict:

                    logger.debug("updating target %s to %s", source, dest)
                    obj = self.substitution_dict[source]
        ...
        for attrib in empty.values():
            print(f"empty destination for {attrib}")
        ...
        for parameterset in root.findall("parameterset"):
            for parameter in parameterset.iter():
                ...
                cache[name] = {"mode": mode, "default": default}
        ...
        for name, args in cache.items():
            ...
            sub = Substitution(
                name=name, mode=mode, target=f"#{name}#", default=default
            )
            if hasattr(self, sub.name):
                delattr(self, sub.name)
            setattr(self, sub.name, sub)

    def reformat_template(self, platform: Union[None, str] = None) -> str:
        """
        converts the original JUBE template to one that Substitutions can interact with
        """
        ...
        tree = ET.parse(self.platform_path)
        root = tree.getroot()
        for substituteset in root.findall("substituteset"):
            for substitution in substituteset.iter():
                if "source" in substitution.attrib:
                    source = substitution.attrib["source"]
                    dest = substitution.attrib.get("dest", None)

                    if dest is not None:
                        dest = f"#{dest.strip('$')}#"

                        template = template.replace(source, dest)
    ...
    def link_sub(self, sub: Substitution) -> None:
        ...
        for name in links:
            ...
            obj = self.argument_dict[name]  # get the actual associated object
    ...
    def link_jube_subs(self) -> None:
        """Ensures proper linking of JUBE substitutions"""
        for sub in self.substitution_objects:
            self.link_sub(sub)

    def script(self, empty_treatment="local", **kwargs) -> str:
        """
        Performs a translation evaluation step
        before calling the BaseComputer script() method
        """
        for k, v in kwargs.items():
            if k in self.arguments:
                self.argument_dict[k].temporary_value = v

        # ensure substitutions are linked
        self.link_jube_subs()

        # update the template
        self._template = self.reformat_template()

        return super().script(empty_treatment=empty_treatment)
