...
class Base:
    redis_client = connect_to_redis(select_redis_addr())

    # 기본 Redis 캐시 만료 시간 (1시간)
    DEFAULT_CACHE_EXPIRATION_SEC = 3600
    ...
    def get_cached_data(cls, redis_name: str) -> str:
        ...
        if cached_data is None:
            ...
            redis_logger.debug(type(cached_data.decode('utf-8')))
...
class C108(Corps):
    ...
    def get_recent_date(self, refresh=False) -> Optional[datetime]:
