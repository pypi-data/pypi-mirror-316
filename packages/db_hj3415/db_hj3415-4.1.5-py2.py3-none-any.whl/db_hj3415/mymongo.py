...
from .mypeewee import LogManager
...
class Base:
    ...
    @db.setter
    def db(self, db: str):
        mongo_logger.info(f'Change db : {self.db} -> {db}')
        self._db = self.mongo_client[db]
        try:
...
class Corps(Base):
    ...
    def _save_df(cls, code: str, page: str, df: pd.DataFrame, clear_table: bool):
        ...
        if df.empty:
            mongo_logger.warning(f'입력된 {code}/{page} 데이터프레이 비어서 저장은 하지 않습니다.')
            return
    ...
    def save(cls, code: str, dart_data: dict) -> bool:
        ...
        if (set(dart_template) - set(dart_data.keys())) == set():
            ...
            try:
                ...
                LogManager('dart').save('INFO', f"{code} / {dart_data['corp_name']} / {dart_data['report_nm'].strip()} 을 저장함.")
                return result.acknowledged
            except errors.DuplicateKeyError:
                return True
        else:
