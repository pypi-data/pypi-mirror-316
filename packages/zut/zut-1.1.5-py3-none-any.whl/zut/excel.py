...
from zut import Header, ColumnsProvider, Row, TabularDumper, Literal, files, is_iterable_of, parse_tz
...
def split_excel_path(path: str|Path, *, default_table_name: str = None) -> tuple[str,str]:
    """
    ...
    return (m[1], m[2] if m[2] else default_table_name)
...
class ExcelWorkbook:
    ...
    def __init__(self, path: str|Path, *, formula = _UNSET, archivate: bool|str|Path = False, save_as: str|Path|bool|None = None):
        """
        ...
        self._pyxl_workbook = None
        self._tables: dict[str,ExcelTable] = {}

    @classmethod
    def get_instance(cls, path: str|Path, *, formula = _UNSET, archivate: bool|str|Path = False, save_as: str|Path|bool|None = None) -> ExcelWorkbook:
        path = str(path) if not isinstance(path, str) else path
        canonical_path = os.path.realpath(path)
        if not canonical_path in cls._instances:
            cls._instances[canonical_path] = ExcelWorkbook(path, formula=formula, archivate=archivate, save_as=save_as)
        ...
        if self._pyxl_workbook is None:
            ...
            if files.exists(self.path):
                ...
                with files.open(self.path, 'rb') as fp:
                    self._pyxl_workbook: Workbook = load_workbook(fp) #NOTE: using keep_vba=True causes issues, example on openpyxl 3.1.2: load and save samples/empty-table.xlsx without modification corrupts the file
    ...
    def save(self, *, save_as: str|Path|None = None):
        ...
        if self._archivate:
            if save_as is False:
                _logger.debug("Would archivate Excel workbook %s", self.path)
            else:
                files.archivate(self.path, archive_dir=self._archivate)
        ...
        if save_as is False:
            ...
            if save_as is None:
                save_as = self.path
            elif not isinstance(save_as, str):
                save_as = str(save_as)
            else:
                pass # keep as is
...
class ExcelTable(ColumnsProvider):
    min_row_index: int
    ...
    def __init__(self, pyxl_table: Table|str, pyxl_worksheet: Worksheet, workbook: ExcelWorkbook, no_headers: bool = None):
        ...
        if isinstance(pyxl_table, str):
            ...
            self.min_col_index, col_count, self.min_row_index, self.row_count = self._get_table_data_area()
            self.has_headers = pyxl_table.headerRowCount > 0
            
            # NOTE: if no headers, default names are returned, example in French: ['Colonne1', 'Colonne2']
            # ROADMAP: basic type detection
            self._headers = [Header(name) for name in self.pyxl_table.column_names] 
            if len(self._headers) != col_count:
                raise ValueError(f'Invalid headers length ({len(self._headers)}, expected {col_count}): {self._headers}')
    ...
    def add_column(self, name: str):
        ...
        name = str(name)
        if any(h.name == name for h in self._headers):
        ...
        self._headers.append(Header(name)) # implies self.col_count += 1
    ...
    def redefine(self):
        ...
        for i in range(0, self.col_count):
            if self.has_headers:
                ...
                name = self._headers[i].name if i < len(self._headers) else None
    ...
    def get_value(self, row_index: int, col_index: int) -> Any:
        ...
        cell = self.pyxl_worksheet.cell(self.min_row_index + 1 + row_index, self.min_col_index + 1 + col_index)
        value = cell.value
        if self.workbook._formula is not _UNSET and isinstance(value, str) and value.startswith('='):
    ...
    def _build_column_formats(self) -> list[dict[str,Any]]:
        ...
        for column in self.pyxl_table.tableColumns:
            ...
            if column.dataDxfId is not None:
                ...
                if dxf.fill and dxf.fill.fill_type is not None:
                    fmt['fill'] = PatternFill(fill_type=dxf.fill.fill_type, bgColor=dxf.fill.fgColor, fgColor=dxf.fill.bgColor) # NOTE: fgcolor and bgcolor are inversed in DifferentialStyle
                else:
...
class ExcelDumper(TabularDumper[Union[str,Path]]):
    atexit = True
    _atexit_instances: list[ExcelDumper] = []
    ...
    def __init__(self,
                 ...
                 *,
                 ...
                 # ExcelDumper options
                 table: str|None = None,
                 atexit: bool|None = None,
                 # Destination mask values
                 **kwargs):
        
        if not self.is_available():
            raise ValueError(f"Cannot use ExcelDumper: package `openpyxl` is required")

        if notz is None:
            notz = True
        super().__init__(dst, headers=headers, truncate=truncate, archivate=archivate, title=title, name=name, dir=dir, delay=delay, optional=optional, add_columns=add_columns, after1970=after1970, notz=notz, **kwargs)

        self.path, dst_table = split_excel_path(self.dst)
        if dst_table:
            if not table:
                table = dst_table
            elif table != dst_table:
                raise ValueError(f"Invalid table in {self.dst} (expected {table})")
        self.table_name = table if table else 'Data'

        self._workbook: ExcelWorkbook = None
        self._table: ExcelTable = None
        self._table_created: bool = None

        self.atexit = atexit if atexit is not None else self.__class__.atexit
    
        if self.atexit:
            if not self.__class__._atexit_instances:
                atexit_register(self.__class__._close_atexit)
            self.__class__._atexit_instances.append(self)

    @classmethod
    def _close_atexit(cls):
        # First, perform parent closing (from class TabularDumper: handle delayed rows and print title)
        for instance in cls._atexit_instances:
            super(cls, instance).close()

        # Then, close the workbooks
        # Cannot be done before, because several OutTable instances may export to the same file
        for instance in cls._atexit_instances:
            if instance._workbook:
                instance._workbook.close()
                instance._workbook = None
                instance._table = None

    def open(self):
        if self.archivate:
            files.archivate(self.path, archive_dir=self.archivate, missing_ok=True)

        if self._workbook is None:
            self._workbook = ExcelWorkbook.get_instance(self.path)

        if self._table is None:
            self._table = self._workbook.get_table(self.table_name, default=None)
            if not self._table:
                self._table = self._workbook.create_table(self.table_name, no_headers=True if not self.headers else False)
        ...
        # NOTE: a copy of self._table.headers is necessary in order to avoid manipulating it from ExcelDumper
        return list(self._table.headers) if not self._table_created and self._table.has_headers else None
        
    def export_headers(self, headers: list[Header]):
        for header in headers:
            self._table.add_column(header.name)
    ...
    def export(self, row: list):
        ...
        for i, value in enumerate(row):
            if value is None: # keep default formula if any (applied during table.erase_cell(), called from table.insert_row())
                continue
            table_row[i] = value
