...
def make_application(cookies: str, debug: bool = False) -> Application:
    ...
    async def index(
        ...
        if share_code:
            ...
            url = await get_share_downurl(share_code, receive_code, id, dont_receive=dont_receive)
        else:
            if pickcode:
