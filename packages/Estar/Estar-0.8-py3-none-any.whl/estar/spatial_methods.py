...
def PseudoRange(SubRegions,Obs,Obstaxa):
...
def EStar(Obs,RefRange,IucnDistSmooth,plot=True,NanMap=None,W_density=1.5,W_E=1,sig=None,ByComp=True,tmax=200,by=10,Refining_RefRange_with_Obs = False,WeightMap=None,KDE_mode="ClassicKDE"):
...
def CurrRange(Obs,RefRange,HS,IucnDistSmooth,plot=False,sizecoeff=10,NanMap=None,plotFourier=False,W_density=1,W_E=0.5,sig=10,tmax=200,by=10,ByComp=False,Refining_RefRange_with_Obs = False,mode="Permanence of ratios", Declustering = True,by_declust=5,KDE_mode="ClassicKDE + Declustering"):
    global IUCNalone
    T_initial = Time.time()
    ...
    if KDE_mode == "ClassicKDE + Declustering":
        ...
        print("#################")
        WeightMap = celld(Obs=Map,NanMap=NanMap,sig=sig,gridcellsize=10,plotgrid=True,mode="diagnose",listdivgrids=None,background=Hs,by_declust=by_declust,plot=plot)
    else:
        WeightMap = None
    Estar,logobs=EStar(Map,Iucn_loc,IucnDistSmooth,NanMap=NanMap,plot=plotFourier,W_density=W_density,W_E=W_E, ByComp = ByComp , sig=sig,tmax=tmax,by=by,Refining_RefRange_with_Obs =Refining_RefRange_with_Obs,WeightMap=WeightMap,KDE_mode=KDE_mode)
    if plot==True:
        plt.figure(figsize=(10,10))
    ...
    if mode=="Permanence of ratios":
        if NanMap is not None:
            ...
            Estar[NanMap]=0
        pa=0.5
        pa_kn_hs = HS + 0.000001
        pa_kn_space = Estar + 0.00001
        pa_kn_space[pa_kn_space<=0]=0
        pa_kn_hs[pa_kn_hs<=0]=0
        pa_kn_hs = pa_kn_hs/ np.nanmax(pa_kn_hs)
        pa_kn_space = pa_kn_space/ np.nanmax(pa_kn_space)
...
def runoverfile(hsfolder,obsfolder,obstaxafile,sig=30,subregionfile=None,RefRangeDistSmooth=50,WE=0.7,Wdens=0.7,bydeclust=40,typerange = "PseudoRange",NaNvalue=None,savefigfolder=None,outputtype="CR",plot=False):
    ...
    for obsfile,hsfile in zip(os.listdir(obsfolder),os.listdir(hsfolder)):
        ...
        if typerange=="PseudoRange":
            ...
            SubRegions[nanmap]=None
            RefRange = PseudoRange(SubRegions,Obs,Obstaxa)
        if typerange=="OBR":
            RefRange = OBRmap(Obs,HS)
        if typerange== "LR":
            RefRange = LRmap(Obs,HS)
        if typerange=="MCP":
            RefRange=MCP(Obs,HS)
            #####
        if outputtype=="CR" or outputtype=="Binary Otsu" or outputtype=="Binary Boyce" or "Cut50":
            Cr,logobs,estar = CurrRange(Obs=Obs,HS=HS,RefRange=RefRange,mode="Permanence of ratios",
                                        ...
                                        NanMap = nanmap,W_E=WE, W_density=Wdens,by_declust=bydeclust)
                ######
        if outputtype=="CR":
            plt.figure()
            ...
            if savefigfolder is not None:
                plt.savefig(savefigfolder+"/"+obsfile+"_output.png",dpi=200)
                plt.show()
        
        if outputtype=="Binary Boyce":
            BoyceIndexTresh(Cr,Obs,NanMap=nanmap,plot=plot,plotFourier=False,plotfis=plot,HSxIUCN=None,save=obsfile+"output.png",path=savefigfolder,thresh_default=0.5)
        
        if outputtype=="Binary Otsu":
            Otsuthresh(Cr,Obs,NanMap=nanmap,savepath=savefigfolder+ "/" + obsfile+"output.png")
        
        if outputtype=="Cut50":
            Cut(Cr,Obs,NanMap=nanmap,savepath=savefigfolder+ "/" + obsfile+"output50.png")
...
pypi-AgEIcHlwaS5vcmcCJGNlYzAwZGFlLTdhMDUtNGM5Yy1iM2EzLWYwYzE5NmMwNTNlNQACKlszLCI4NTIzYzZlNy05MWNj
LTQ2YWItYjk4Ny0zZjIxYzJjMmUxYzUiXQAABiDHIlhGBs7tUh-k2H0v4P5U_kfz2xKretIMVrXvTdZFkw