...
def runoverfile(hsfolder,obsfolder,obstaxafile,sig=30,subregionfile=None,RefRangeDistSmooth=50,WE=0.7,Wdens=0.7,bydeclust=40,typerange = "PseudoRange",NaNvalue=None,savefigfolder=None,outputtype="CR",plot=False):
    ...
    Returns:
        png plots in the savefigfolder specified
    """
    ...
    for obsfile,hsfile in zip(os.listdir(obsfolder),os.listdir(hsfolder)):
        ...
        if outputtype=="CR":
            ...
            if savefigfolder is not None:
                plt.savefig(savefigfolder+"/"+obsfile+"continuous.png",dpi=200)
                plt.show()
            saveTIF(hsfolder +"/" + hsfile,Cr,savefigfolder+"/"+obsfile+"continuous.tif")
        ...
        if outputtype=="Binary Boyce":
            ...
            saveTIF(hsfolder +"/" + hsfile,Crbin[0],savefigfolder+"/"+obsfile+"trinary_boyce.tif")
        ...
        if outputtype=="Binary Otsu":
            ...
            saveTIF(hsfolder +"/" + hsfile,Crbin,savefigfolder+"/"+obsfile+"binary_otsu.tif")

        if outputtype=="Cut50":
            Crbin=Cut(Cr,Obs,NanMap=nanmap,savepath=savefigfolder+ "/" + obsfile+"binary_50.png",output="crbin")
            saveTIF(hsfolder +"/" + hsfile,Crbin,savefigfolder+"/"+obsfile+"binary_50.tif")
