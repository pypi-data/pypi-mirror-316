...
def BoyceIndexTresh(CR,Obs,NanMap,plot=False,plotFourier=False,plotfis=False,HSxIUCN=None,save="",path=None,thresh_default=0.5):
    ...
    def boyce(model):
        ...
        while loop==True:
            ...
            if ndivopt==11:
                loop=False
                ndivopt=1
            Lw,Lextent,ndivi=Find_focal_area(Obs,NanMap,divgrid=int(ndivopt),plotgrid=True)
            lFi=[]
            lFw=[]
    ...
    lFi,lFw,ndivopt = boyce(CR)
    
    if HSxIUCN is not None: # if we compare it to HSxIUCN
        lFi_hsxiucn , lFw_hsxiucn, ndivopt_hsxiucn = boyce(HSxIUCN)
    ...
    X1,X2=plotlFilFw(lFi,lFw,ndivopt,"CR","blue")
    ...
    if save!="":
        plt.savefig(path+"/"+save+'_plot.png', dpi=300)
