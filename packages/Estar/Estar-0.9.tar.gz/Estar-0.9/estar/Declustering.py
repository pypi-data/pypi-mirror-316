...
def celld(Obs,NanMap,sig,gridcellsize=10,plotgrid=True,mode="diagnose",listdivgrids=None,background=None,by_declust=1,returncellsize=False,plot=False):
    
    RefZ = applykernel_pr(Obs,gkern(sig=sig,hole=True)) # raw density before declusterisation
...
#             if mode_declust == "FastSmall":
#                 print("cell size = "+ str(divgrid)+"// ",end=" ")
#                 nmax=5
#                 counts = np.array([])
#                 for n in range(nmax):
#                     print(n/nmax*100,"%",end=" ")
#                     # Générer le tableau avec le motif quadrillé
#                     xsize,ysize = Obs.shape
#                     grid = generate_grid(xsize, ysize, divgrid)
#                     # Multiplie M par la grille pour obtenir la carte des cellules
#                     gridded_map = Obs * grid
#                     # Compte le nombre de "1" dans chaque cellule avec scipy.ndimage
#                     counts_r = ndimage.labeled_comprehension(
#                         Obs, gridded_map, np.unique(gridded_map), np.sum, float, 0, pass_positions=False
#                     )
#                     # Exclure les cellules vides (iD == 0)
#                     counts_r = counts_r[counts_r != 0] 
#                     count_rep = counts_r.astype("int64")
#                     counts_r = np.repeat(counts_r,count_rep)
#                     counts_r = 1/counts_r
#                     counts = np.concatenate((counts,counts_r))
