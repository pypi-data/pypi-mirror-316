...
def create_and_populate_moyenne_table(family, 
    ...
    import sqlite3
    import re
    import pikobs  # Asegúrate de que esta biblioteca esté importada correctamente
    ...
    # Configuración según 'channel' y 'id_stn'
    ...
    # Crear la tabla 'moyenne' si no existe
    ...
    # Insertar datos en la tabla 'moyenne'
    qr = f"""
    INSERT INTO moyenne (
       ...
       while current_date <= dateend:
               ...
               #   for pathin in pathins:
                    ...
                    data_dict = {
                      ...
                      'db_new': f'{pathwork}/{family}/scatter_{name}_{datestart1}_{dateend1}_bx{boxsizex}_by{boxsizey}_{flag_criteria}_{family}.db',
                      'region': region,
...
def create_data_list_plot(datestart1, dateend1, families, namein, pathwork, boxsizex, boxsizey,
                          fonctions, flag_criteria, regions, id_stn, channel, projs):
    data_list_plot = []
    for family in families:
     for region in regions: 
      for fonction in fonctions:
       for proj in projs:
        fileset = [f'{pathwork}/{family}/scatter_{namein[0]}_{datestart1}_{dateend1}_bx{boxsizex}_by{boxsizey}_{flag_criteria}_{family}.db']
        nameset = [namein[0]]
    
        if len(namein) > 1:
            fileb = f'{pathwork}/{family}/scatter_{namein[1]}_{datestart1}_{dateend1}_bx{boxsizex}_by{boxsizey}_{flag_criteria}_{family}.db'
