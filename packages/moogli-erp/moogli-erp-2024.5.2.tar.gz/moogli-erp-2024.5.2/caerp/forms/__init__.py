"""
Main deferreds functions used in MoOGLi
...
MAIL_ERROR_MESSAGE = "Veuillez entrer une adresse e-mail valide"
...
def _get_model_default_value(request, model, default_key="default", id_key="id"):
    """
    Return the model's default value
    """
    default = (
        request.dbsession.execute(
            select(model).filter(getattr(model, default_key)).limit(1)
        )
        .scalars()
        .first()
    )
...
def get_deferred_default(model, default_key="default", id_key="id"):
    ...
    def deferred_default(node, kw):
        ...
        """
        request = kw["request"]
        return _get_model_default_value(request, model, default_key, id_key)
