...
class APIClient:
    ...
    def create_user(self, user_input: dict):
        """
        Create or update a user.

        Parameters:
            user_input (dict): User data. Must have telegram_id.

        Returns:
            User data after creation or update.
        """
        url = f"{self.base_url}/telegram/users"
        response = requests.post(url, json=user_input, headers={"Authorization": f"Bearer {self.token}"})
        return self._handle_response(response)

    def update_user(self, user_input: dict):
        """
        Update a user.
        ...
        """
        url = f"{self.base_url}/telegram/users"
        response = requests.put(url, json=user_input, headers={"Authorization": f"Bearer {self.token}"})
        return self._handle_response(response)
