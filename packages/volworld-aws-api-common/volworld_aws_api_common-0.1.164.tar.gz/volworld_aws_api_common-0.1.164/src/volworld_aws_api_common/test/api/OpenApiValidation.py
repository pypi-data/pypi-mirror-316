...
class OpenApiValidation:
    ...
    def __init__(self, api_yaml_path=None):
        ...
        self.api_component = RefResolver.from_schema({
            OAPI.Components: self.api_yaml[OAPI.Components]
        })
        self.api_path = self.api_yaml[OAPI.Paths]
    ...
    def validate_request(self, req, path: str, method: str, attList):
        ...
        if OAPI.RequestBody not in self.api_path[path][method]:
            return

        request_body = self.api_path[path][method][OAPI.RequestBody]
        ...
        validate(req_fix_json, req_schema, resolver=self.api_component)
    ...
    def validate_response(self, res, path: str, method: str, code, attList):
        ...
        response_body = self.api_path[path][method][OAPI.Responses][str(code)]
        if '$ref' in response_body.keys():
            ...
            if ref_val.startswith('#/paths/'):
                ...
                response_body = self.api_path[ref_path][ref_method][OAPI.Responses][str(ref_code)]
        ...
        validate(res_fix_json, res_schema, resolver=self.api_component)
