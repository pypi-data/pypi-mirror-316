...
def extract_structural_metadata(
    ...
    os.makedirs(metadata_savepath, exist_ok=True)
...
def fill_dataset_series_tab(
    ...
    dataset_series[MetaMapper.SERIES_supply_type.value] = "Full "
...
def _unpack_nested_types(
    ...
    def _unpack_struct(node: dict, field_type: dict, key: str) -> None:
        if (
            ...
            records.append(node)
            records.extend(_unpack_nested_types(field_type[key]["fields"], node))
        else:
            records.append(node)
