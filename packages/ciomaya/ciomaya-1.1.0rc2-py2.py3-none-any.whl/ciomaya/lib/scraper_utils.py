...
def process_paths(paths, func):
    ...
    if not len(paths) > 0:
        return result
    is_string_list = isinstance(paths[0], str)
    if is_string_list:
        for entry in paths:
            try:
...
def run_scrapers(node, scripts):
    ...
    for script in scripts:
        try:
            ...
            raise ScraperError("Unknown problem with the script '{}'. Turn on Tracebacks and use the scraper test button in the submitter UI for more details".format(script), exc) from exc
    return result
