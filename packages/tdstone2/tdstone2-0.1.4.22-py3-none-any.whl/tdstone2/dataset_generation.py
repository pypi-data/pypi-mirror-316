...
def GenerateDataSet(n_x=9, n_f=1, n_partitions=2, n_rows=5,
    ...
    * cos(2.0*3.14159265358979323846 * CAST(RANDOM(0,999999999) AS FLOAT)
    ...
    if train_test_split_ratio > 1:
        train_test_split_condition = f"""CASE WHEN B.RNK<{train_test_split_ratio} THEN 'train' ELSE 'test' END"""
    else:
        train_test_split_condition = f"""CASE WHEN B.RNK<{train_test_split_ratio*n_rows} THEN 'train' ELSE 'test' END"""
...
def GenerateDataSetNew(n_x=9, n_f=1, n_partitions=2, n_rows=5,
    ...
    query = f"""
        ...
        ,   CASE WHEN B.ID < {int(n_rows * train_test_split_ratio)} THEN 'train' ELSE 'test' END as FOLD        
