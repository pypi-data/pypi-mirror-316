...
class TestBasic(TestCase):
    ...
    def test_entry_logging(self):
        ...
        log("test-user-created", "User created", targets={
            "user":     user,
        })

        group = Group.objects.create(name="Test Group")
        log("test-group-created", "Group created", targets={
            "group":    group,
        })

        group.user_set.add(user)
        log("test-group-added-user", "User added to group", targets={
            "user":     user,
            "group":    group,
        })

        log("test-extra-data", "Testing extra data", data={
            "key":      "value",
        })
        ...
        qs = Entry.objects.filter(event_type__startswith="test")
        ...
        self.assertEqual(qs.filter(related_to_q(user) | related_to_q(group)).distinct().count(), 3)
    ...
    def test_objects_represented(self):
        ...
        log("test-entry", "Test 1", targets={
            "user":     user1,
            "group":    some_group,
        })
        log("test-entry", "Test 2", targets={
            "user":     user2,
            "group":    some_group,
        })
        log("test-entry", "Test 3", targets={
            "user":     user2,
            "group":    some_group2,
        })
        log("test-entry", "Test 4", targets={
            "blah":     user1,
            "group":    some_group3,
        })
    ...
    def test_json_field_filtering(self):
        ...
        self.assertTrue(Entry.objects.filter(data__field="value").exists())
