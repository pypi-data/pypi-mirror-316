...
class AdminEventLoggerMixin:
    ...
    def get_urls(self):
        ...
        urlpatterns = [
            re_path(r'^(.+)/papertrail/', self.admin_site.admin_view(self.view_papertrail_item), name='{0}_{1}_papertrail'.format(*info)),
        ] + super().get_urls()
    ...
    def view_papertrail(self, request, queryset, extra_context=None):
        """Action for displaying the `papertrail` for one or more selected Admin Items."""
...
class EntryRelatedObjectInline(admin.StackedInline):
    ...
    def related_model(self, obj):
        ...
        try:
            return format_html(
                f"<a href='{admin_reverse_for_model(related_obj)}'>{related_obj}</a>")
        except NoReverseMatch:
            return format_html("{}", related_obj)
