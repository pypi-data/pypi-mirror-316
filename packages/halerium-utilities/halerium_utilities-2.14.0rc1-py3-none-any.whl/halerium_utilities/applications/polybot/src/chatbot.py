            ...
            async for event in agents.call_agent_async(**payload, parse_data=True):
                ...
                if event.event == "attachment":
                    attachments[event.data["filename"]] = {
                        mimetypes.guess_type(event.data["filename"])[0]: event.data["attachment"]
                    }
                ...
                #if chunk:
                #    if not "image/png" in str(chunk) and not str(chunk).startswith("!["):
                #                    full_message += chunk
                #                    yield chunk
                #    else:
                #        Chatbot.logger.debug(
                #            "ignored image or markdown image link."
                #        )
                #elif attachment:
