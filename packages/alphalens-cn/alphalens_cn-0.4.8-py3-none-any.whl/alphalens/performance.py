...
def factor_alpha_beta(
    ...
    for period in returns.columns.values:
        ...
        try:
            ...
            alpha_beta.loc["Ann. alpha[年化alpha]", period] = np.nan
            alpha_beta.loc["beta[衡量波动性]", period] = np.nan
        else:
            freq_adjust = pd.Timedelta("252Days") / pd.Timedelta(period)

            alpha_beta.loc["Ann. alpha[年化alpha]", period] = (1 + alpha) ** freq_adjust - 1
            alpha_beta.loc["beta[衡量波动性]", period] = beta
