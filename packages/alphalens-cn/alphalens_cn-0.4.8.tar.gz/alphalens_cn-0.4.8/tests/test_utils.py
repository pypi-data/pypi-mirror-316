...
class UtilsTestCase(TestCase):
    ...
    biased_factor_data["group"] = Series(
        index=biased_factor.index,
    ...
    def test_compute_forward_returns(self):
        ...
        ix = MultiIndex.from_product([dr, ["A", "B"]], names=["date", "asset"])
        expected = DataFrame(index=ix, columns=["1D", "2D"])
        expected["1D"] = [0.0, 1.0, 1.0, -0.5, nan, nan]
        expected["2D"] = [1.0, 0.0, nan, nan, nan, nan]
    ...
    def test_compute_forward_returns_index_out_of_bound(self):
        ...
        prices = DataFrame(
            index=dr,
            columns=["A", "B"],
            data=[[nan, nan], [nan, nan], [nan, nan], [1, 1], [1, 2], [2, 1]],
        )
    ...
    def test_compute_forward_returns_non_cum(self):
        ...
        expected = DataFrame(index=ix, columns=["1D", "2D"])
        expected["1D"] = [0.0, 1.0, 1.0, -0.5, nan, nan]
        expected["2D"] = [1.0, -0.5, nan, nan, nan, nan]
