...
def get_clean_factor(
    ...
    将因子数据、远期收益数据和分组映射整理成一个包含时间戳和资产对齐的MultiIndex索引的DataFrame。返回的数据将被格式化为适合Alphalens函数的形式。

    可以安全地跳过对此函数的调用，只要因子数据符合从get_clean_factor_and_forward_returns返回的格式，并且在此处有文档记录，仍然可以利用Alphalens的功能。
    Parameters
    ----------
    factor : pd.Series - MultiIndex
        A MultiIndex Series indexed by timestamp (level 0) and asset
...
def get_clean_factor_and_forward_returns(
    ...
    functionalities as long as the factor data conforms to the format returned
    from get_clean_factor_and_forward_returns and documented here
    将因子数据、定价数据和分组映射整理成一个包含时间戳和资产对齐的MultiIndex索引的DataFrame。返回的数据将被格式化为适合Alphalens函数的形式。
