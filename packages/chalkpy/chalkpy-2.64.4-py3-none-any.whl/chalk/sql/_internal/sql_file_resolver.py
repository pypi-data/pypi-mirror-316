        ...
        if feature is None:
            ...
            if not has_import_errors:
                error_builder.add_diagnostic_with_spellcheck(
                    ...
                    spellcheck_candidates=[
                        feature.fqn
                        for feature in FeatureSetBase.registry[build_namespaced_name(name=namespace)].features
                    ],
                    message=message,
                    code="153a",
                    label="output feature not recognized",
...
class GeneratedSQLFileResolverRegistry:
    ...
    def get_generated_sql_file_resolvers(self) -> Iterable[SQLStringResult]:
        for filepath, (sql_string, comment_dict) in self.filepath_to_sql_string.items():
            yield SQLStringResult(
                path=filepath,
                sql_string=sql_string,
                error=None,
                override_comment_dict=comment_dict,
                autogenerated=True,
            )
...
def make_sql_file_resolver(
    ...
    filename = (
        os.path.join(os.path.dirname(module_file), _get_generated_sql_resolver_filepath(filename=name))
        if module_file is not None
        else _get_generated_sql_resolver_filepath(filename=name)
    )
