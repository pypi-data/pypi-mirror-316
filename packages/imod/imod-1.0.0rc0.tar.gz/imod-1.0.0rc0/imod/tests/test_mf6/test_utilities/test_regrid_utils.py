...
def is_equal_regridder(instance_1, instance_2) -> bool:
    ...
    for key in keys:
        if hash(pickle.dumps(instance_1.weights[key])) != hash(
            pickle.dumps(instance_2.weights[key])
        ):
            return False
...
def test_regridders_weight_cache_returns_different_instance_when_name_does_not_match(
    ...
    second_instance = collection.get_regridder(
        grid, new_grid, RegridderType.BARYCENTRIC
    )
...
def test_regridders_weight_cache_grows_up_to_size_limit(
    ...
    _ = collection.get_regridder(
        grid, new_grid, RegridderType.RELATIVEOVERLAP, method="conductance"
    )
    assert len(collection.weights_cache) == cache_size
