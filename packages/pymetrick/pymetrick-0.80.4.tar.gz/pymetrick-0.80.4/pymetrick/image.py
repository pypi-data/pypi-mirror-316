            ...
            if kwargs['file_path'] and (kwargs['file_path'][-4:].lower() in IMAGE_ALLOWED_EXTENSIONS):
                ...
                if kwargs.get('size',None) and isinstance(kwargs['size'],(list,tuple)):
                    if (kwargs['size'][0]/_img_relation) <= kwargs['size'][1]:
                        _img = _img.resize((int(kwargs['size'][0]), int(kwargs['size'][0]/_img_relation)),Image.Resampling.LANCZOS)
                    else:
                        _img_relation = _img_height/_img_width
                        _img = _img.resize((int(kwargs['size'][1]/_img_relation),int(kwargs['size'][1])),Image.Resampling.LANCZOS)
                        
                if kwargs.get('source_file',None) and kwargs['source_file'][-4:] in IMAGE_ALLOWED_EXTENSIONS:
                    print(kwargs.get('source_file',None))
                    print(kwargs['source_file'][-4:])
                    print(kwargs['source_file'][-4:].replace('.','').upper())
                    _format = kwargs['source_file'][-4:].replace('.','').upper().replace('JPG','JPEG')
                    if kwargs.get('quality',None) and kwargs['quality'] != 0:
                        _img.save(kwargs['source_file'], _format, dpi=_dpi, quality=int(kwargs['quality']), optimize=True, progressive=True)
                    else:
                        _img.save(kwargs['source_file'], _format, dpi=_dpi, quality=100, optimize=True, progressive=True)
