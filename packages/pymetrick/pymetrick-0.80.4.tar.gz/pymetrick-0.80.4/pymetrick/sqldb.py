    ...
    def __init__(self,*args, **kwargs):
        ...
        if self.driver in ('mysql','mariadb'):
            try:
                try:
                    ...
                    try:
                        ...
                        if ( 'pymysql' in self._sql_driver.__dict__['__name__']):
                            self._sql_driver_errorcode = import_module('.err', package='pymysql')
                            #from pymysql.err import IntegrityError
                    except Exception:
                        LOG.error(f"""Driver {self.driver} not installed !!!""")
                        return
                

                self.config['host'] = kwargs.get('host','127.0.0.1')
                self.config['port'] = int(kwargs.get('port',3306))
                ...
                if 'mysql.connector' in self._sql_driver.__dict__['__name__']:
                    self.config['charset'] = kwargs.get('charset',"utf8mb4")   # utf8 DEFAULT
                    self.config['autocommit'] = kwargs.get('autocommit',True)
                    self.config['use_unicode'] = kwargs.get('use_unicode',True)
                    self.config['time_zone'] = kwargs.get('time_zone',"+0:00")
                    self.config['sql_mode'] = kwargs.get('sql_mode',"TRADITIONAL")
                    self.config['collation'] = kwargs.get('collation',"utf8mb4_general_ci") # utf8mb4_general_ci in 2.x  ó utf8mb4_general_ai_ci DEFAULT
    ...
    def reconnect(self):
        ...
        try:
            ...
            if self.driver in ('mysql','mariadb'):
                ...
                while n_retry > 0:
                    if ('mysql.connector' in self._sql_driver.__dict__['__name__'] and hasattr(self._sql_driver, 'reconnect')):
                        self._sql_driver.reconnect(attempts=RETRY_NUMBER,delay=ATTEMPS_SLEEP)
                    elif ('pymysql' in self._sql_driver.__dict__['__name__']):
                        ...
                        self.connect = self._sql_driver.connect(**self.config)
                        n_retry -= 1
                    if ('mysql.connector' in self._sql_driver.__dict__['__name__'] and self.connect.is_connected()) or (('pymysql' in self._sql_driver.__dict__['__name__']) and self.connect.open):
...
if __name__ == "__main__":
    ...
    try:
        ...
        __db__ = SQLdb(host='10.0.0.1',database='dacartoys',username='root',password='headcracker_sql',driver='mariadb',port='4306')
        print(type(__db__))
