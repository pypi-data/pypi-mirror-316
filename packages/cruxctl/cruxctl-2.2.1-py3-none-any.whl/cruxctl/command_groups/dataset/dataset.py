...
b.yaml and c.yaml will be combined before they are applied.'
    ...
    for yaml_group in pathlib_list_of_list:
        ...
        if response.status_code not in [200, 201]:
            error_console.print(
                f'Failed to apply {yaml_group}. Error: {json.loads(response.text)["message"]}'
            )
            raise typer.Exit(1)
        if not quiet:
...
def get_logs(
    dataset_id: Annotated[
        str,
        typer.Argument(help="The dataset ID to get logs for."),
    ],
    service: Annotated[
        str,
        typer.Option("--service", "-s", help="Name of service whose logs to retrieve"),
    ],
    profile: Annotated[
        str,
        typer.Option(help="profile to use: local, dev, staging, prod. Default: prod"),
    ...
    export_id: Annotated[
        str, typer.Option("--export-id", "-e", help="Optional Export ID to filter logs")
    ] = None,
    delivery_id: Annotated[
        str,
        typer.Option("--delivery-id", "-d", help="Optional Delivery ID to filter logs"),
    ] = None,
    cdu_id: Annotated[
        str, typer.Option("--cdu-id", "-c", help="Optional CDU ID to filter logs")
    ] = None,
    page_size: Annotated[
        int,
        typer.Option("--page-size", help="Optional Page size to limit entries"),
    ] = 100,
):
    """
    Retrieves logs for the provided dataset based on filter criteria.
    """
    if not profile:
        profile = get_current_profile()
    token: str = set_api_token(console, profile)
        ...
        try:
            ...
            if "entries" in response and response["entries"]:
                for entry in response["entries"]:
                    ...
                    text_payload = entry.get("textPayload", "No log message available")

                    console.print(f"[{entry_time}] {text_payload}")
            else:
                console.print("No log entries available.")
