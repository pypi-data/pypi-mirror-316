...
def test_case_68():
    ...
    material = Material(material_name, [MaterialComponent("N-BUTANE", 1.0)], component_count=1)
    ...
    flammable_output_configs = [FlammableOutputConfig(position = LocalPosition(5.0, 0.0, 0.0), transect = Transect(transect_start_point = LocalPosition(), transect_end_point = LocalPosition()), radiation_resolution= radiation_resolution)]
    ...
    if result_code == ResultCode.SUCCESS:
        if (len(distances_and_ellipses_to_radiation_levels_calculation.contour_points)!= 50):
        ...
        if (abs(distances_and_ellipses_to_radiation_levels_calculation.contour_points[len(distances_and_ellipses_to_radiation_levels_calculation.contour_points)-1].x -296.1082631873717)/296.1082631873717 > 1e-3):
        ...
        if (abs(distances_and_ellipses_to_radiation_levels_calculation.areas[0]-114055.56705017174)/114055.56705017174 > 1e-3):
