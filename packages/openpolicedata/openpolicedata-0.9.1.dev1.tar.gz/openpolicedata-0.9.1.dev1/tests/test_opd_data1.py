...
if __name__ == "__main__":
	...
	datasets = datasets[datasets['DataType']=='Excel']
	datasets = datasets[~datasets['source_url'].str.contains('openjustice.doj')]
	for k in range(271, len(datasets)):
		d = datasets.iloc[k]
		print(f"{k} of {len(datasets)}: Testing {d['SourceName']} {d['TableType']} table for year {d['Year']}")
		src = opd.Source(d['SourceName'], state=d['State'], agency=d["Agency"])
		src.load(d['TableType'], d['Year'], url_contains=d['URL'], id_contains=d['dataset_id'])
